...
reads `clients.yaml`. You can regenerate everything at once or a single client by name.

## Layout

```
.
├── clients.yaml              # registry: which clients to generate, from where, with what flags
├── clients/                  # generated nu modules (one file per entry in clients.yaml)
├── scripts/
│   └── generate.nu           # the generator driver — reads clients.yaml, invokes http-gen
└── .github/workflows/
    └── generate.yml          # workflow_dispatch + push trigger; commits regenerated clients
```
