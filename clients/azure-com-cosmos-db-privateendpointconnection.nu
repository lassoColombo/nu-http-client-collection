...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "subscriptions-resource-groups-providers-microsoft-document-db-database-accounts-private-endpoint-connections ListByDatabaseAccount" } } | get name | first)
...
export def "subscriptions-resource-groups-providers-microsoft-document-db-database-accounts-private-endpoint-connections ListByDatabaseAccount" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.DocumentDB/databaseAccounts/($accountName)/privateEndpointConnections" $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-document-db-database-accounts-private-endpoint-connections Delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.DocumentDB/databaseAccounts/($accountName)/privateEndpointConnections/($privateEndpointConnectionName)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "subscriptions-resource-groups-providers-microsoft-document-db-database-accounts-private-endpoint-connections CreateOrUpdate" [
  subscriptionId: string
  resourceGroupName: string
  accountName: string
  privateEndpointConnectionName: string
