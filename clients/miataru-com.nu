# Auto-generated client for Miataru v1.0.0
# Source: https://api.apis.guru/v2/specs/miataru.com/1.0.0/swagger.json
# Auth: --token flag or $env.MIATARU_TOKEN

const BASE_URL = "http://service.miataru.com/v1"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o MIATARU_TOKEN | default "" }
...
def base-url-completer [] { ["http://service.miataru.com/v1" "https://service.miataru.com/v1"] }
...
export def "get-location get" [
  ...
  let req_body = {"MiataruConfig": $miataru_config, "MiataruGetLocation": $miataru_get_location} | compact
...
export def "get-location-geo-json get" [
  ...
  let full_url = (build-url $base ({device_id: (encode-path-segment $device_id)} | format pattern "/GetLocationGeoJSON/{device_id}"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "get-location-history get" [
  ...
  let req_body = {"MiataruConfig": $miataru_config, "MiataruGetLocationHistory": $miataru_get_location_history} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: {}, body: $req_body}
...
export def "get-visitor-history get" [
  ...
  let req_body = {"MiataruGetVisitorHistory": $miataru_get_visitor_history} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: {}, body: $req_body}
...
export def "update-location update" [
  ...
  let req_body = {"MiataruConfig": $miataru_config, "MiataruLocation": $miataru_location} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: {}, body: $req_body}
