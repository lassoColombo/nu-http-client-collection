...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o MIATARU_TOKEN | default "" }
...
export def "get-location get" [
  ...
  let full_url = (build-url $base "/GetLocation" $auth.query)
...
export def "get-location-geo-json get" [
  ...
  let full_url = (build-url $base ({device_id: (encode-path-segment $device_id)} | format pattern "/GetLocationGeoJSON/{device_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "get-location-history get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/GetLocationHistory" $auth.query)
...
export def "get-visitor-history get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/GetVisitorHistory" $auth.query)
...
export def "update-location update" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/UpdateLocation" $auth.query)
