...
# Auth: --token flag or $env.TRANSPORT_DEPARTMENT_DADRA___NAGAR_HAVELI_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o TRANSPORT_DEPARTMENT_DADRA___NAGAR_HAVELI_TOKEN | default "" }
