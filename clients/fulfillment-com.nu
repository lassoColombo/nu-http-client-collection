...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o FULFILLMENT_COM_APIV2_TOKEN | default "" }
...
export def "accounting get" [
  ...
  let full_url = (build-url $base "/accounting" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"fromDate": $from_date, "toDate": $to_date, "page": $page, "limit": $limit, "warehouseIds": $warehouse_ids, "orderIds": $order_ids, "hydrate": $hydrate} | compact)
...
export def "inventory get" [
  ...
  let full_url = (build-url $base "/inventory" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"page": $page, "limit": $limit, "merchantIds": $merchant_ids, "warehouseIds": $warehouse_ids, "externalSkuNames": $external_sku_names} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "orders list" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"fromDate": $from_date, "toDate": $to_date, "merchantIds": $merchant_ids, "warehouseIds": $warehouse_ids, "page": $page, "limit": $limit, "hydrate": $hydrate} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "orders get" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"merchantId": $merchant_id, "hydrate": $hydrate} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "orders-ship update" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/orders/{id}/ship") $auth.query)
...
export def "orders-status update" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/orders/{id}/status") $auth.query)
...
export def "returns get" [
  ...
  let full_url = (build-url $base "/returns" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"fromDate": $from_date, "toDate": $to_date, "page": $page, "limit": $limit} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "returns update" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/returns" $auth.query)
...
export def "track get" [
  ...
  let full_url = (build-url $base "/track" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"trackingNumber": $tracking_number} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
