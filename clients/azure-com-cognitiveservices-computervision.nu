...
const BASE_URL = "https://{AzureRegion}.api.cognitive.microsoft.com/vision/v1.0"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o COMPUTER_VISION_TOKEN | default "" }
...
def base-url-completer [] { ["https://{AzureRegion}.api.cognitive.microsoft.com/vision/v1.0"] }
...
export def "analyze create-image" [
  ...
  let full_url = (build-url $base "/analyze" $qp $auth.query)
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"visualFeatures": $visual_features, "details": $details, "language": $language} | compact)
...
export def "describe get-image" [
  ...
  let full_url = (build-url $base "/describe" $qp $auth.query)
  let req_body = {"url": $url} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
    url: $full_url
    query: ({"maxCandidates": $max_candidates, "language": $language} | compact)
    headers: $auth.headers
    body: $req_body
    content_type: "application/json"
    timeout: ($max_time | default 30min)
...
export def "generate-thumbnail generate" [
  ...
  let full_url = (build-url $base "/generateThumbnail" $qp $auth.query)
  let req_body = {"url": $url} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"width": $width, "height": $height, "smartCropping": $smart_cropping} | compact)
    headers: $auth.headers
    body: $req_body
    content_type: "application/json"
    timeout: ($max_time | default 30min)
...
export def "models-analyze create-image-by-domain" [
  ...
  let full_url = (build-url $base ({model: (encode-path-segment $model)} | format pattern "/models/{model}/analyze") $qp $auth.query)
  let req_body = {"url": $url} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "ocr create-recognize-printed-text" [
  ...
  let full_url = (build-url $base "/ocr" $qp $auth.query)
  let req_body = {"url": $url} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
    url: $full_url
    query: ({"detectOrientation": $detect_orientation, "language": $language} | compact)
    headers: $auth.headers
    body: $req_body
    content_type: "application/json"
    timeout: ($max_time | default 30min)
...
export def "recognize-text create" [
  ...
  let full_url = (build-url $base "/recognizeText" $qp $auth.query)
  let req_body = {"url": $url} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
    url: $full_url
    query: ({"detectHandwriting": $detect_handwriting} | compact)
    headers: $auth.headers
    body: $req_body
    content_type: "application/json"
    timeout: ($max_time | default 30min)
...
export def "tag tag-image" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "language" $language "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/tag" $qp $auth.query)
  let req_body = {"url": $url} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "text-operations get-result" [
  ...
  let full_url = (build-url $base ({operation_id: (encode-path-segment $operation_id)} | format pattern "/textOperations/{operation_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
