...
export def "analyze create-image" [
  ...
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"visualFeatures": $visual_features, "details": $details, "language": $language} | compact), body: $req_body}
}
...
export def "describe get-image" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"maxCandidates": $max_candidates, "language": $language} | compact), body: $req_body}
}
...
export def "generate-thumbnail generate" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"width": $width, "height": $height, "smartCropping": $smart_cropping} | compact), body: $req_body}
}
...
export def "ocr create-recognize-printed-text" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"detectOrientation": $detect_orientation, "language": $language} | compact), body: $req_body}
}
...
export def "recognize-text create" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"detectHandwriting": $detect_handwriting} | compact), body: $req_body}
}
