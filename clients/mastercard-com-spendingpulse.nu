...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o SPENDING_PULSE_TOKEN | default "" }
...
export def "gasweekly get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"CurrentRow": $current_row, "Offset": $offset} | compact), body: null}
}
...
export def "spendingpulse get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"CurrentRow": $current_row, "Offset": $offset, "ProductLine": $product_line, "PublicationCoveragePeriod": $publication_coverage_period, "Country": $country, "ReportType": $report_type, "Period": $period, "Sector": $sector, "Ecomm": $ecomm} | compact), body: null}
}
