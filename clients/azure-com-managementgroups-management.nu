...
export def "providers-microsoft-management-check-name-availability check" [
  ...
  let full_url = (build-url $base "/providers/Microsoft.Management/checkNameAvailability" $qp $auth.query)
...
export def "providers-microsoft-management-get-entities list" [
  ...
  let full_url = (build-url $base "/providers/Microsoft.Management/getEntities" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"api-version": $api_version, "$skiptoken": $skiptoken, "$skip": $skip, "$top": $top, "$select": $select, "$search": $search, "$filter": $filter, "$view": $view, "groupName": $group_name} | compact)
    headers: $auth.headers
    body: null
    content_type: "application/json"
    timeout: ($max_time | default 30min)
...
export def "providers-microsoft-management-management-groups get" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"api-version": $api_version, "$expand": $expand, "$recurse": $recurse, "$filter": $filter} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "providers-microsoft-management-management-groups-descendants get" [
  ...
  let full_url = (build-url $base ({group_id: (encode-path-segment $group_id)} | format pattern "/providers/Microsoft.Management/managementGroups/{group_id}/descendants") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"api-version": $api_version, "$skiptoken": $skiptoken, "$top": $top} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "providers-microsoft-management-management-groups-subscriptions delete" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({group_id: (encode-path-segment $group_id), subscription_id: (encode-path-segment $subscription_id)} | format pattern "/providers/Microsoft.Management/managementGroups/{group_id}/subscriptions/{subscription_id}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Cache-Control": $cache_control} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "providers-microsoft-management-start-tenant-backfill start" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/providers/Microsoft.Management/startTenantBackfill" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "providers-microsoft-management-tenant-backfill-status create" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/providers/Microsoft.Management/tenantBackfillStatus" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
