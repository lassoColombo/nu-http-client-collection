# Auto-generated client for Security Center v2018-06-01
# Source: https://api.apis.guru/v2/specs/azure.com/security-pricings/2018-06-01/swagger.json
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "subscriptions-providers-microsoft-security-pricings list" } } | get name | first)
...
export def "subscriptions-providers-microsoft-security-pricings get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # API version for the operation
]: nothing -> record<properties: record<freeTrialRemainingTime: string, pricingTier: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
...
export def "subscriptions-providers-microsoft-security-pricings update" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # API version for the operation
  --properties: record # Pricing properties for the relevant scope — shape: {pricingTier: "Free"|"Standard"}
]: any -> record<properties: record<freeTrialRemainingTime: string, pricingTier: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
