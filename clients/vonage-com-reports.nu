...
export def "accounts-call-logs get" [
  ...
  let full_url = (build-url $base ({account_id: (encode-path-segment $account_id)} | format pattern "/accounts/{account_id}/call-logs") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"start:gte": $start_gte, "start:lte": $start_lte, "end:gte": $end_gte, "end:lte": $end_lte, "page_size": $page_size, "page": $page, "to": $qp_to, "from": $qp_from, "source_user": $source_user, "destination_user": $destination_user, "direction": $direction} | compact), body: null}
}
