...
export def "concepts list" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"ids": $ids, "types": $types} | compact), body: null}
}
...
export def "conditions get-list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"age.value": $age_value, "age.unit": $age_unit, "enable_triage_5": $enable_triage_5} | compact), body: null}
}
...
export def "info get-database" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"age.value": $age_value, "age.unit": $age_unit} | compact), body: null}
}
...
export def "lookup get-matching-observation" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"phrase": $phrase, "sex": $sex, "age.value": $age_value, "age.unit": $age_unit} | compact), body: null}
}
...
export def "red-flags create-compute" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"max_results": $max_results} | compact), body: $req_body}
}
...
export def "search get-matching-observations" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"phrase": $phrase, "sex": $sex, "age.value": $age_value, "age.unit": $age_unit, "max_results": $max_results, "type": $type} | compact), body: null}
}
