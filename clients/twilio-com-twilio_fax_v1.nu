...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o TWILIO_FAX_TOKEN | default "" }
...
export def "faxes list-fax" [
  ...
  let full_url = (build-url $base "/v1/Faxes" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"From": $qp_from, "To": $qp_to, "DateCreatedOnOrBefore": $date_created_on_or_before, "DateCreatedAfter": $date_created_after, "PageSize": $page_size} | compact)
...
export def "faxes-media list-fax" [
  ...
  let full_url = (build-url $base ({fax_sid: (encode-path-segment $fax_sid)} | format pattern "/v1/Faxes/{fax_sid}/Media") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "faxes-media delete-fax" [
  ...
  let full_url = (build-url $base ({fax_sid: (encode-path-segment $fax_sid), sid: (encode-path-segment $sid)} | format pattern "/v1/Faxes/{fax_sid}/Media/{sid}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "faxes delete-fax" [
  ...
  let base = ($base_url | default "https://fax.twilio.com")
  if ($sid | is-empty) { error make --unspanned { msg: "path parameter 'Sid' must be non-empty" } }
  let full_url = (build-url $base ({sid: (encode-path-segment $sid)} | format pattern "/v1/Faxes/{sid}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
