# Auto-generated client for Twilio - Fax v1.29.1
# Source: https://api.apis.guru/v2/specs/twilio.com/twilio_fax_v1/1.29.1/openapi.json
# Auth: --token flag or $env.TWILIO_FAX_TOKEN

const BASE_URL = "https://fax.twilio.com"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o TWILIO_FAX_TOKEN | default "" }
...
export def "faxes list-fax" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"From": $qp_from, "To": $qp_to, "DateCreatedOnOrBefore": $date_created_on_or_before, "DateCreatedAfter": $date_created_after, "PageSize": $page_size} | compact), body: null}
}
...
export def "faxes-media list-fax" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://fax.twilio.com")
  if ($fax_sid | is-empty) { error make --unspanned { msg: "path parameter 'FaxSid' must be non-empty" } }
