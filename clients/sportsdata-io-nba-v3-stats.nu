...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "all-stars AllStars" } } | get name | first)
...
export def "all-stars AllStars" [
  ...
  let full_url = (build-url $base $"/($format)/AllStars/($season)")
