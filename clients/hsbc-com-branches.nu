# Auto-generated client for Branch Locator API v2.2.1
# Source: https://api.apis.guru/v2/specs/hsbc.com/branches/2.2.1/swagger.json
# Auth: --token flag or $env.BRANCH_LOCATOR_API_TOKEN

const BASE_URL = "https://dikpeqbnwi3kx.cloudfront.net"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o BRANCH_LOCATOR_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://dikpeqbnwi3kx.cloudfront.net"] }
...
export def "open-banking-v2-2-branches get" [
  ...
  let full_url = (build-url $base "/open-banking/v2.2/branches" $auth.query)
...
export def "x-open-banking-v2-2-branches-country get" [
  ...
  let full_url = (build-url $base ({country: (encode-path-segment $country)} | format pattern "/x-open-banking/v2.2/branches/country/{country}") $auth.query)
  let accept_val = "application/prs.openbanking.opendata.v2.2+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "x-open-banking-v2-2-branches-country-town get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($country | is-empty) { error make --unspanned { msg: "path parameter 'country' must be non-empty" } }
  if ($town | is-empty) { error make --unspanned { msg: "path parameter 'town' must be non-empty" } }
  let full_url = (build-url $base ({country: (encode-path-segment $country), town: (encode-path-segment $town)} | format pattern "/x-open-banking/v2.2/branches/country/{country}/town/{town}") $auth.query)
  let accept_val = "application/prs.openbanking.opendata.v2.2+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "x-open-banking-v2-2-branches-geo-location-lat-long get" [
  ...
  let full_url = (build-url $base ({latitude: (encode-path-segment $latitude), longitude: (encode-path-segment $longitude)} | format pattern "/x-open-banking/v2.2/branches/geo-location/lat/{latitude}/long/{longitude}") $qp $auth.query)
  let accept_val = "application/prs.openbanking.opendata.v2.2+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"radius": $radius} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "x-open-banking-v2-2-branches-postcode get" [
  ...
  let full_url = (build-url $base ({postcode: (encode-path-segment $postcode)} | format pattern "/x-open-banking/v2.2/branches/postcode/{postcode}") $auth.query)
  let accept_val = "application/prs.openbanking.opendata.v2.2+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "x-open-banking-v2-2-branches-sortcode get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($sortcode | is-empty) { error make --unspanned { msg: "path parameter 'sortcode' must be non-empty" } }
  let full_url = (build-url $base ({sortcode: (encode-path-segment $sortcode)} | format pattern "/x-open-banking/v2.2/branches/sortcode/{sortcode}") $auth.query)
  let accept_val = "application/prs.openbanking.opendata.v2.2+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
