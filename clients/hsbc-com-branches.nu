...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o BRANCH_LOCATOR_API_TOKEN | default "" }
...
export def "open-banking-v22-branches get" [
  ...
  let full_url = (build-url $base "/open-banking/v2.2/branches")
...
export def "x-open-banking-v22-branches-country get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/x-open-banking/v2.2/branches/country/($country)")
  let accept_val = "application/prs.openbanking.opendata.v2.2+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "x-open-banking-v22-branches-country-town get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/x-open-banking/v2.2/branches/country/($country)/town/($town)")
  let accept_val = "application/prs.openbanking.opendata.v2.2+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "x-open-banking-v22-branches-geo-location-lat-long get" [
  ...
  let full_url = (build-url $base $"/x-open-banking/v2.2/branches/geo-location/lat/($latitude)/long/($longitude)" $qp)
  let accept_val = "application/prs.openbanking.opendata.v2.2+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "x-open-banking-v22-branches-postcode get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/x-open-banking/v2.2/branches/postcode/($postcode)")
  let accept_val = "application/prs.openbanking.opendata.v2.2+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "x-open-banking-v22-branches-sortcode get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/x-open-banking/v2.2/branches/sortcode/($sortcode)")
  let accept_val = "application/prs.openbanking.opendata.v2.2+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
