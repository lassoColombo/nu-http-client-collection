...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o POLICYSTATESCLIENT_TOKEN | default "" }
...
export def "providers-microsoft-policy-insights-operations list" [
  ...
  let full_url = (build-url $base "/providers/Microsoft.PolicyInsights/operations" $qp $auth.query)
...
export def "providers-management-groups-providers-microsoft-policy-insights-policy-states-query-results list" [
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"api-version": $api_version, "$top": $top, "$orderby": $orderby, "$select": $select, "$from": $qp_from, "$to": $qp_to, "$filter": $filter, "$apply": $apply} | compact)
    headers: $auth.headers
    body: null
...
export def "subscriptions-providers-microsoft-policy-insights-policy-states-query-results list" [
  ...
  if ($policy_states_resource | is-empty) { error make --unspanned { msg: "path parameter 'policyStatesResource' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar") (serialize-qp "$top" $top "scalar") (serialize-qp "$orderby" $orderby "scalar") (serialize-qp "$select" $select "scalar") (serialize-qp "$from" $qp_from "scalar") (serialize-qp "$to" $qp_to "scalar") (serialize-qp "$filter" $filter "scalar") (serialize-qp "$apply" $apply "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), policy_states_resource: (encode-path-segment $policy_states_resource)} | format pattern "/subscriptions/{subscription_id}/providers/Microsoft.PolicyInsights/policyStates/{policy_states_resource}/queryResults") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "subscriptions-providers-microsoft-policy-insights-policy-states-summarize create" [
  ...
  if ($policy_states_summary_resource | is-empty) { error make --unspanned { msg: "path parameter 'policyStatesSummaryResource' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar") (serialize-qp "$top" $top "scalar") (serialize-qp "$from" $qp_from "scalar") (serialize-qp "$to" $qp_to "scalar") (serialize-qp "$filter" $filter "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), policy_states_summary_resource: (encode-path-segment $policy_states_summary_resource)} | format pattern "/subscriptions/{subscription_id}/providers/Microsoft.PolicyInsights/policyStates/{policy_states_summary_resource}/summarize") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "providers-microsoft-policy-insights-policy-states-query-results list-for-resource" [
  ...
  let full_url = (build-url $base ({resource_id: (encode-path-segment $resource_id), policy_states_resource: (encode-path-segment $policy_states_resource)} | format pattern "/{resource_id}/providers/Microsoft.PolicyInsights/policyStates/{policy_states_resource}/queryResults") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
    url: $full_url
    query: ({"api-version": $api_version, "$top": $top, "$orderby": $orderby, "$select": $select, "$from": $qp_from, "$to": $qp_to, "$filter": $filter, "$apply": $apply, "$expand": $expand} | compact)
    headers: $auth.headers
    body: null
    content_type: "application/json"
    timeout: ($max_time | default 30min)
...
export def "providers-microsoft-policy-insights-policy-states-summarize create-for-resource" [
  ...
  if ($policy_states_summary_resource | is-empty) { error make --unspanned { msg: "path parameter 'policyStatesSummaryResource' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar") (serialize-qp "$top" $top "scalar") (serialize-qp "$from" $qp_from "scalar") (serialize-qp "$to" $qp_to "scalar") (serialize-qp "$filter" $filter "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({resource_id: (encode-path-segment $resource_id), policy_states_summary_resource: (encode-path-segment $policy_states_summary_resource)} | format pattern "/{resource_id}/providers/Microsoft.PolicyInsights/policyStates/{policy_states_summary_resource}/summarize") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "providers-microsoft-policy-insights-policy-states-metadata get-metadata" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({scope: (encode-path-segment $scope)} | format pattern "/{scope}/providers/Microsoft.PolicyInsights/policyStates/$metadata") $qp $auth.query)
