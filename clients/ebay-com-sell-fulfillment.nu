...
export def "order list" [
  ...
  --field-groups: string # The response type associated with the order. The only presently supported value is <code>TAX_BREAKDOWN</code>. This type returns a breakdown of tax and fee values associated with the order.
  ...
  --order-ids: string # A comma-separated list of the unique identifiers of the orders to retrieve (maximum 50). If one or more order ID values are specified through the <b>orderIds</b> query parameter, all other query parameters will be ignored.<br><br><span class="tablenote"><strong>Note:</strong> <b>getOrders</b> can return orders up to two years old. Do not provide the orderId for an order created more than two years in the past.</span>
...
export def "order get" [
  ...
  let full_url = (build-url $base ({order_id: $order_id} | format pattern "/order/{order_id}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "order-shipping-fulfillment list" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base ({order_id: $order_id} | format pattern "/order/{order_id}/shipping_fulfillment"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "order-shipping-fulfillment create" [
  ...
  --shipping-carrier-code: string # The unique identifier of the shipping carrier being used to ship the line item(s). Technically, the <strong>shippingCarrierCode</strong> and <strong>trackingNumber</strong> fields are optional, but generally these fields will be provided if the shipping carrier and tracking number are known. <br><br><span class="tablenote"><strong>Note:</strong> Use the Trading API's <a href="https://developer.ebay.com/devzone/XML/docs/Reference/eBay/GeteBayDetails.html " target="_blank">GeteBayDetails</a> call to retrieve the latest shipping carrier enumeration values. When making the <a href="https://developer.ebay.com/devzone/XML/docs/Reference/eBay/GeteBayDetails.html " target="_blank">GeteBayDetails</a> call, include the <strong>DetailName</strong> field in the request payload and set its value to <code>ShippingCarrierDetails</code>. Each valid shipping carrier enumeration value is returned in a <strong>ShippingCarrierDetails.ShippingCarrier</strong> field in the response payload.</span>
  ...
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base ({order_id: $order_id} | format pattern "/order/{order_id}/shipping_fulfillment"))
  let body = {"lineItems": $line_items, "shippedDate": $shipped_date, "shippingCarrierCode": $shipping_carrier_code, "trackingNumber": $tracking_number} | compact
...
export def "order-shipping-fulfillment get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base ({order_id: $order_id, fulfillment_id: $fulfillment_id} | format pattern "/order/{order_id}/shipping_fulfillment/{fulfillment_id}"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "order-issue-refund issueRefund" [
  ...
  --refund-items: list # The <b>refundItems</b> array is only required if the seller is issuing a refund for one or more individual order line items in a multiple line item order. Otherwise, the seller just uses the <b>orderLevelRefundAmount</b> container to specify the amount of the refund for the entire order. — item shape: {legacyReference?: record, lineItemId?: string, refundAmount?: record}
]: any -> record<refundId: string, refundStatus: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base ({order_id: $order_id} | format pattern "/order/{order_id}/issue_refund"))
  let body = {"comment": $comment, "orderLevelRefundAmount": $order_level_refund_amount, "reasonForRefund": $reason_for_refund, "refundItems": $refund_items} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json" $body
...
export def "payment-dispute get" [
  ...
  let full_url = (build-url $base ({payment_dispute_id: $payment_dispute_id} | format pattern "/payment_dispute/{payment_dispute_id}"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "payment-dispute-accept acceptPaymentDispute" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default "https://apiz.ebay.com{basePath}")
  let full_url = (build-url $base ({payment_dispute_id: $payment_dispute_id} | format pattern "/payment_dispute/{payment_dispute_id}/accept"))
  let body = {"returnAddress": $return_address, "revision": $revision} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json" $body
...
export def "payment-dispute-activity get-activities" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default "https://apiz.ebay.com{basePath}")
  let full_url = (build-url $base ({payment_dispute_id: $payment_dispute_id} | format pattern "/payment_dispute/{payment_dispute_id}/activity"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "payment-dispute-add-evidence create" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --dry-run(-n) # Return the request that would be sent without executing it
  --evidence-type: string # This field is used to indicate the type of evidence being provided through one or more evidence files. All evidence files (if more than one) should be associated with the evidence type passed in this field. See the <strong>EvidenceTypeEnum</strong> type for the supported evidence types. For implementation help, refer to <a href='https://developer.ebay.com/api-docs/sell/fulfillment/types/api:EvidenceTypeEnum'>eBay API documentation</a>
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default "https://apiz.ebay.com{basePath}")
  let full_url = (build-url $base ({payment_dispute_id: $payment_dispute_id} | format pattern "/payment_dispute/{payment_dispute_id}/add_evidence"))
  let body = {"evidenceType": $evidence_type, "files": $files, "lineItems": $line_items} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json" $body
...
export def "payment-dispute-contest contestPaymentDispute" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default "https://apiz.ebay.com{basePath}")
  let full_url = (build-url $base ({payment_dispute_id: $payment_dispute_id} | format pattern "/payment_dispute/{payment_dispute_id}/contest"))
  let body = {"note": $note, "returnAddress": $return_address, "revision": $revision} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json" $body
...
export def "payment-dispute-fetch-evidence-content get" [
  ...
  let full_url = (build-url $base ({payment_dispute_id: $payment_dispute_id} | format pattern "/payment_dispute/{payment_dispute_id}/fetch_evidence_content") $qp)
...
export def "payment-dispute-update-evidence update" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default "https://apiz.ebay.com{basePath}")
  let full_url = (build-url $base ({payment_dispute_id: $payment_dispute_id} | format pattern "/payment_dispute/{payment_dispute_id}/update_evidence"))
  let body = {"evidenceId": $evidence_id, "evidenceType": $evidence_type, "files": $files, "lineItems": $line_items} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json" $body
...
export def "payment-dispute-upload-evidence-file upload" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default "https://apiz.ebay.com{basePath}")
  let full_url = (build-url $base ({payment_dispute_id: $payment_dispute_id} | format pattern "/payment_dispute/{payment_dispute_id}/upload_evidence_file"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "payment-dispute-summary get-payment-dispute-summaries" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
