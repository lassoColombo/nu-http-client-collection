...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o KOTAK_MAHINDRA_GENERAL_INSURANCE_COMPANY_LTD_TOKEN | default "" }
...
# --certificateParameters shape: {PolicyNo: string, Val: string, Verification: string}
# --consentArtifact shape: {consent: record, signature: record}
export def "gicer-certificate gicer" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
