...
export def "subscriptions-resource-groups-providers-microsoft-azure-stack-registrations-products-upload-product-log upload" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client API Version. (default: 2017-06-01)
]: nothing -> record<details: string, endDate: string, error: string, id: string, operation: string, productId: string, registrationName: string, resourceGroupName: string, startDate: string, status: string, subscriptionId: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
