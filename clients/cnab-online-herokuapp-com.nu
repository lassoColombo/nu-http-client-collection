# Auto-generated client for Cnab Online v1.0.0
# Source: https://api.apis.guru/v2/specs/cnab-online.herokuapp.com/1.0.0/swagger.json
# Auth: --token flag or $env.CNAB_ONLINE_TOKEN

const BASE_URL = "https://cnab-online.herokuapp.com/v1"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o CNAB_ONLINE_TOKEN | default "" }
...
def base-url-completer [] { ["https://cnab-online.herokuapp.com/v1"] }
...
export def "file create" [
  ...
  file: path # Arquivo CNAB
]: any -> record<data: record<attributes: record<account_number: string, bank_code: int, bank_name: string, company_name: string, generation_date: string, name: string>, id: string, type: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/file" $auth.query)
...
export def "file get" [
  ...
  let full_url = (build-url $base ({file_id: (encode-path-segment $file_id)} | format pattern "/file/{file_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "file-lines get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($file_id | is-empty) { error make --unspanned { msg: "path parameter 'fileId' must be non-empty" } }
  let full_url = (build-url $base ({file_id: (encode-path-segment $file_id)} | format pattern "/file/{file_id}/lines") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "file-occurrences get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($file_id | is-empty) { error make --unspanned { msg: "path parameter 'fileId' must be non-empty" } }
  let full_url = (build-url $base ({file_id: (encode-path-segment $file_id)} | format pattern "/file/{file_id}/occurrences") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
