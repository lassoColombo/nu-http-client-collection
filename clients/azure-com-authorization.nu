...
export def "providers-microsoft-authorization-elevate-access create" [
  ...
  let full_url = (build-url $base "/providers/Microsoft.Authorization/elevateAccess" $qp $auth.query)
...
export def "role-assignments delete" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({role_assignment_id: (encode-path-segment $role_assignment_id)} | format pattern "/{role_assignment_id}") $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
