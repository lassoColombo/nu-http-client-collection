# Auto-generated client for Personnel Data v1.0
# Source: https://api.apis.guru/v2/specs/personio.de/personnel/1.0/openapi.json
# Auth: --token flag or $env.PERSONNEL_DATA_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o PERSONNEL_DATA_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.personio.de/v1"] }
...
export def "company-attendances get" [
  ...
  --start-date: string # First day of the period to be queried. It is inclusive, so the day specified as start_date will also be considered on the results (format: date)
  ...
  let qp = [(serialize-qp "start_date" $start_date "scalar") (serialize-qp "end_date" $end_date "scalar") (serialize-qp "updated_from" $updated_from "scalar") (serialize-qp "updated_to" $updated_to "scalar") (serialize-qp "employees" $employees "multi") (serialize-qp "limit" $limit "scalar") (serialize-qp "offset" $offset "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/company/attendances" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"start_date": $start_date, "end_date": $end_date, "updated_from": $updated_from, "updated_to": $updated_to, "employees": $employees, "limit": $limit, "offset": $offset} | compact), body: null}
}
...
export def "company-attendances create" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --attendances: list # item shape: {break: int, comment: string, date: string, employee: int, end_time: string, start_time: string}
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
...
export def "company-attendances update" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --body-break: int # Break in minutes (format: int32)
  --comment: string # Optional comment
  --date: string # Attendance date as YYYY-MM-DD (format: date)
  --end-time: string # End time as HH:MM
  --start-time: string # Start time as HH:MM
...
export def "company-time-off-types get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --limit: int # Pagination attribute to limit how many records will be returned per page (default: 200)
  --offset: int # Pagination attribute to identify which page you are requesting, by the form of telling an offset from the first record that would be returned. (default: 0)
]: nothing -> record<data: table<attributes: record, id: int, type: string>, success: bool> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "limit" $limit "scalar") (serialize-qp "offset" $offset "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/company/time-off-types" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "company-time-offs list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "start_date" $start_date "scalar") (serialize-qp "end_date" $end_date "scalar") (serialize-qp "updated_from" $updated_from "scalar") (serialize-qp "updated_to" $updated_to "scalar") (serialize-qp "employees" $employees "multi") (serialize-qp "limit" $limit "scalar") (serialize-qp "offset" $offset "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/company/time-offs" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"start_date": $start_date, "end_date": $end_date, "updated_from": $updated_from, "updated_to": $updated_to, "employees": $employees, "limit": $limit, "offset": $offset} | compact), body: null}
}
...
export def "company-time-offs create" [
  ...
  end_date: string # Absence end date as YYYY-MM-DD (format: date)
  --half-day-end: oneof<nothing, bool> # Weather the end date is a half-day off
  --half-day-start: oneof<nothing, bool> # Weather the start date is a half-day off
  start_date: string # Absence start date as YYYY-MM-DD (format: date)
  time_off_type_id: int # Time-off type identifier
]: any -> record<data: record<data: record<attributes: record, type: string>, success: bool>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
