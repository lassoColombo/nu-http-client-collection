...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o PERSONNEL_DATA_TOKEN | default "" }
...
export def "company-attendances get" [
  ...
  let full_url = (build-url $base "/company/attendances" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"start_date": $start_date, "end_date": $end_date, "updated_from": $updated_from, "updated_to": $updated_to, "employees": $employees, "limit": $limit, "offset": $offset} | compact)
...
export def "company-attendances create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/company/attendances" $auth.query)
...
export def "company-attendances delete" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/company/attendances/{id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "company-employees list" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/company/employees" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "company-employees get" [
  ...
  let full_url = (build-url $base ({employee_id: (encode-path-segment $employee_id)} | format pattern "/company/employees/{employee_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "company-employees-profile-picture get" [
  ...
  let full_url = (build-url $base ({employee_id: (encode-path-segment $employee_id), width: (encode-path-segment $width)} | format pattern "/company/employees/{employee_id}/profile-picture/{width}") $auth.query)
...
export def "company-time-off-types get" [
  ...
  let full_url = (build-url $base "/company/time-off-types" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "company-time-offs list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "start_date" $start_date "scalar") (serialize-qp "end_date" $end_date "scalar") (serialize-qp "updated_from" $updated_from "scalar") (serialize-qp "updated_to" $updated_to "scalar") (serialize-qp "employees" $employees "multi") (serialize-qp "limit" $limit "scalar") (serialize-qp "offset" $offset "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/company/time-offs" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "company-time-offs create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/company/time-offs" $auth.query)
...
export def "company-time-offs delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/company/time-offs/{id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
