...
# Source: https://api.apis.guru/v2/specs/ote-godaddy.com/countries/1.0.0/openapi.json
...
export def "countries get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"marketId": $market_id, "regionTypeId": $region_type_id, "regionName": $region_name, "sort": $qp_sort, "order": $order} | compact), body: null}
}
...
export def "countries get-country" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($country_key | is-empty) { error make --unspanned { msg: "path parameter 'countryKey' must be non-empty" } }
  ...
  let full_url = (build-url $base ({country_key: (encode-path-segment $country_key)} | format pattern "/v1/countries/{country_key}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"marketId": $market_id, "sort": $qp_sort, "order": $order} | compact), body: null}
}
