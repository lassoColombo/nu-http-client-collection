...
export def "blocked-numbers get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"min-id": $min_id, "limit": $limit} | compact), body: null}
}
...
export def "messages list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"limit": $limit, "filter": $filter, "sortOrder": $sort_order} | compact), body: null}
}
...
export def "messages create" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"deduplication-id": $deduplication_id, "auto-unicode": $auto_unicode, "schedule-date": $schedule_date, "schedule-description": $schedule_description} | compact), body: $req_body}
}
...
export def "messages-send get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"to": $qp_to, "body": $body, "deduplication-id": $deduplication_id} | compact), body: null}
}
