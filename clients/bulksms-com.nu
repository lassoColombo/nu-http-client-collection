...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o BULKSMS_JSON_REST_API_TOKEN | default "" }
...
export def "blocked-numbers get" [
  ...
  let full_url = (build-url $base "/blocked-numbers" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"min-id": $min_id, "limit": $limit} | compact)
...
export def "blocked-numbers create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/blocked-numbers" $auth.query)
  let req_body = $body
...
export def "credit-transfer create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/credit/transfer" $auth.query)
...
export def "messages list" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"limit": $limit, "filter": $filter, "sortOrder": $sort_order} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "messages create" [
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"deduplication-id": $deduplication_id, "auto-unicode": $auto_unicode, "schedule-date": $schedule_date, "schedule-description": $schedule_description} | compact)
    headers: $auth.headers
    body: $req_body
    content_type: "application/json"
    timeout: ($max_time | default 30min)
...
export def "messages-send get" [
  ...
  let full_url = (build-url $base "/messages/send" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"to": $qp_to, "body": $body, "deduplication-id": $deduplication_id} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "messages-related-received-messages get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/messages/{id}/relatedReceivedMessages") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "rmm-pre-sign-attachment create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/rmm/pre-sign-attachment" $auth.query)
