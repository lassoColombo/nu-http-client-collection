# Auto-generated client for Bulk WHOIS API v1.0
# Source: https://api.apis.guru/v2/specs/apispot.io/whois/1.0/openapi.json
# Auth: --token flag or $env.BULK_WHOIS_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o BULK_WHOIS_API_TOKEN | default "" }
...
def base-url-completer [] { ["http://localhost:5000" "https://apispot.io/api"] }
def auth-scheme-completer [] { ["x-api-key"] }

# Completers for enum parameters
def operation-completer [] { ["check" "whois"] }
def format-completer [] { ["formatted" "json" "raw"] }
...
]: nothing -> record<batches: table<completed: bool, count: int, created_at: string, id: string, operation: string, results: list, status: string>> {
...
export def "db queryDb" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --dry-run(-n) # Return the request that would be sent without executing it
  --qp-query: string # Query (contact name, dns, domain etc)
]: nothing -> record {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "query" $qp_query "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/db" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
