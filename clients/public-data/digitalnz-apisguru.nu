...
def andcategory-completer [] { ["Archives" "Articles" "Audio" "Books" "Data" "Groups" "Images" "Journals" "Manuscripts" "Music Score" "Newspapers" "Other" "Reference sources" "Research papers" "Sets" "Videos" "Websites"] }
def andusage-completer [] { ["All rights reserved" "Modify" "Share" "Unknown" "Use commercially"] }
def andhas-large-thumbnail-url-completer [] { ["Y"] }
def andhas-lat-lng-completer [] { ["false" "true"] }
...
export def "records-format get" [
  ...
  --andcategory: string@andcategory-completer # These are the same categories that are used across the tabs in [digitalnz.org](https://digitalnz.org/records?text=&tab=Videos)
  --andcontent-partner: string # Allows filtering for records from a particular Content Partner.   Examples: `"Ministry for Culture and Heritage"` `"Trove"` `"V.C. Browne & Son"`    *Tip* - To see a list of Content Partners available for filtering use the *facets* parameter, eg. *"&facets=content_partner"*.  
  --andprimary-collection: string # Allows filtering for records from a particular *primary_collection*.   Examples: `"Puke Ariki"` `"NZHistory"` `"TAPUHI"`      *Tip* - To see a list of Primary_Collections available for filtering use the *facets* parameter, eg. *"&facets=primary_collection"*.   
  ...
  --andusage: string@andusage-completer
  --andsubject: string # Examples: `"Cats"` `"Weddings"` `"climb*"`
  --anddc-type: string # Examples: `"Conference item"` `"Magazines"`
  --andformat: string # Examples: `"Photolithographs"` `"Glass*"`
  --andplacename: string # This field can be used for text-based location search. For a more advanced coordinate-based search, see the "geo_bbox" field below.   Examples: `"Scott Base"` `"Wainuiomata"` `"castle*"`
  --andcreator: string # Examples: `"Revelle Jackson"` `"Nicholas Chevalier"` `"Rita Angus"`
  --andtitle: string # Examples: `"Pukeko"` `"Club"` `"Break*"`"
  ...
  --andyear: string # This field allows searching specifically by year. The metadata is derived from the same date information that is searchable and returned in the date field. It is possible to search across a range using syntax the following syntax `[{start year} TO {end year}]`.   Example: `"1893"` `"[1982 TO 1987]"`
  --anddecade: string # This field allows searching specifically by decade. The metadata is derived from the same date information that is searchable and returned in the date field.   Example: `"1850"` `"1990"`
  --andcentury: string # This field allows searching specifically by century. The metadata is derived from the same date information that is searchable and returned in the date field.   Example: `"1900"` `"2000"`
  ...
  --qp-fields: string # Comma-separated whitelist of fields to be returned. The syntax *"&fields=verbose"* can be used to return the bulk of the fields, or you can customise which fields you are interested in, eg. *"&fields=id,title,subject,collection,landing_url,locations"*.
  ...
  let qp = [(serialize-qp "text" $text "scalar") (serialize-qp "and[category][]" $andcategory "scalar") (serialize-qp "and[content_partner][]" $andcontent_partner "scalar") (serialize-qp "and[primary_collection][]" $andprimary_collection "scalar") (serialize-qp "and[collection][]" $andcollection "scalar") (serialize-qp "and[usage][]" $andusage "scalar") (serialize-qp "and[subject][]" $andsubject "scalar") (serialize-qp "and[dc_type][]" $anddc_type "scalar") (serialize-qp "and[format][]" $andformat "scalar") (serialize-qp "and[placename][]" $andplacename "scalar") (serialize-qp "and[creator][]" $andcreator "scalar") (serialize-qp "and[title][]" $andtitle "scalar") (serialize-qp "and[date]" $anddate "scalar") (serialize-qp "and[year]" $andyear "scalar") (serialize-qp "and[decade]" $anddecade "scalar") (serialize-qp "and[century]" $andcentury "scalar") (serialize-qp "without[{filter_field}]" $without{filter_field} "scalar") (serialize-qp "and[or][{filter_field}][]" $andor{filter_field} "scalar") (serialize-qp "and[is_commercial_use]" $andis_commercial_use "scalar") (serialize-qp "and[has_large_thumbnail_url]" $andhas_large_thumbnail_url "scalar") (serialize-qp "and[has_lat_lng]" $andhas_lat_lng "scalar") (serialize-qp "geo_bbox" $geo_bbox "scalar") (serialize-qp "fields" $qp_fields "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "direction" $direction "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "per_page" $per_page "scalar") (serialize-qp "facets" $facets "csv") (serialize-qp "facets_page" $facets_page "scalar") (serialize-qp "facets_per_page" $facets_per_page "scalar") (serialize-qp "exclude_filters_from_facets" $exclude_filters_from_facets "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/records.($format)" $qp)
...
export def "records get" [
  ...
  let full_url = (build-url $base $"/records/($record_id).($format)" $qp)
  let extra_headers = {"Authentication-Token": $Authentication_Token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "records-more-like-this-format get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "query-api_key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "fields" $qp_fields "scalar") (serialize-qp "mlt_fields" $mlt_fields "scalar") (serialize-qp "filtering" $filtering "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/records/($record_id)/more_like_this.($format)" $qp)
  let extra_headers = {"Authentication-Token": $Authentication_Token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
