...
export def "zones-pools replicapoolpoolslist" [
  ...
  let full_url = (build-url $base ({project_name: $project_name, zone: $zone} | format pattern "/{project_name}/zones/{zone}/pools") $qp)
  let accept_val = "*/*"
...
export def "zones-pools replicapoolpoolsinsert" [
  ...
  --resource-views: list # The list of resource views that should be updated with all the replicas that are managed by this pool.
...
export def "zones-pools replicapoolpoolsget" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "alt" $alt "scalar") (serialize-qp "fields" $fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $pretty_print "scalar") (serialize-qp "quotaUser" $quota_user "scalar") (serialize-qp "userIp" $user_ip "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({project_name: $project_name, zone: $zone, pool_name: $pool_name} | format pattern "/{project_name}/zones/{zone}/pools/{pool_name}") $qp)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "zones-pools replicapoolpoolsdelete" [
  ...
  --quota-user: string # An opaque string that represents a user for quota purposes. Must not exceed 40 characters.
  --user-ip: string # Deprecated. Please use quotaUser instead.
  --abandon-instances: list # If there are instances you would like to keep, you can specify them here. These instances won't be deleted, but the associated replica objects will be removed.
]: any -> any {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "alt" $alt "scalar") (serialize-qp "fields" $fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $pretty_print "scalar") (serialize-qp "quotaUser" $quota_user "scalar") (serialize-qp "userIp" $user_ip "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({project_name: $project_name, zone: $zone, pool_name: $pool_name} | format pattern "/{project_name}/zones/{zone}/pools/{pool_name}") $qp)
  let body = {"abandonInstances": $abandon_instances} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
...
export def "zones-pools-replicas replicapoolreplicaslist" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "alt" $alt "scalar") (serialize-qp "fields" $fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $pretty_print "scalar") (serialize-qp "quotaUser" $quota_user "scalar") (serialize-qp "userIp" $user_ip "scalar") (serialize-qp "maxResults" $max_results "scalar") (serialize-qp "pageToken" $page_token "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({project_name: $project_name, zone: $zone, pool_name: $pool_name} | format pattern "/{project_name}/zones/{zone}/pools/{pool_name}/replicas") $qp)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "zones-pools-replicas replicapoolreplicasget" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "alt" $alt "scalar") (serialize-qp "fields" $fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $pretty_print "scalar") (serialize-qp "quotaUser" $quota_user "scalar") (serialize-qp "userIp" $user_ip "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({project_name: $project_name, zone: $zone, pool_name: $pool_name, replica_name: $replica_name} | format pattern "/{project_name}/zones/{zone}/pools/{pool_name}/replicas/{replica_name}") $qp)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "zones-pools-replicas replicapoolreplicasdelete" [
  ...
  let qp = [(serialize-qp "alt" $alt "scalar") (serialize-qp "fields" $fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $pretty_print "scalar") (serialize-qp "quotaUser" $quota_user "scalar") (serialize-qp "userIp" $user_ip "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({project_name: $project_name, zone: $zone, pool_name: $pool_name, replica_name: $replica_name} | format pattern "/{project_name}/zones/{zone}/pools/{pool_name}/replicas/{replica_name}") $qp)
  let body = {"abandonInstance": $abandon_instance} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json" $body
...
export def "zones-pools-replicas-restart replicapoolreplicasrestart" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "alt" $alt "scalar") (serialize-qp "fields" $fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $pretty_print "scalar") (serialize-qp "quotaUser" $quota_user "scalar") (serialize-qp "userIp" $user_ip "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({project_name: $project_name, zone: $zone, pool_name: $pool_name, replica_name: $replica_name} | format pattern "/{project_name}/zones/{zone}/pools/{pool_name}/replicas/{replica_name}/restart") $qp)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "zones-pools-resize replicapoolpoolsresize" [
  ...
  let full_url = (build-url $base ({project_name: $project_name, zone: $zone, pool_name: $pool_name} | format pattern "/{project_name}/zones/{zone}/pools/{pool_name}/resize") $qp)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "zones-pools-update-template replicapoolpoolsupdatetemplate" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "alt" $alt "scalar") (serialize-qp "fields" $fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $pretty_print "scalar") (serialize-qp "quotaUser" $quota_user "scalar") (serialize-qp "userIp" $user_ip "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({project_name: $project_name, zone: $zone, pool_name: $pool_name} | format pattern "/{project_name}/zones/{zone}/pools/{pool_name}/updateTemplate") $qp)
  let body = {"action": $action, "healthChecks": $health_checks, "version": $version, "vmParams": $vm_params} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json" $body
