# Auto-generated client for Twilio - Monitor v1.42.0
# Source: https://api.apis.guru/v2/specs/twilio.com/twilio_monitor_v1/1.42.0/openapi.json
# Auth: --token flag or $env.TWILIO_MONITOR_TOKEN

const BASE_URL = "https://monitor.twilio.com"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o TWILIO_MONITOR_TOKEN | default "" }
...
def base-url-completer [] { ["https://monitor.twilio.com"] }
...
export def "alerts list" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"LogLevel": $log_level, "StartDate": $start_date, "EndDate": $end_date, "PageSize": $page_size, "Page": $page, "PageToken": $page_token} | compact), body: null}
}
...
export def "alerts get" [
  ...
  let full_url = (build-url $base ({sid: (encode-path-segment $sid)} | format pattern "/v1/Alerts/{sid}"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "events list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"ActorSid": $actor_sid, "EventType": $event_type, "ResourceSid": $resource_sid, "SourceIpAddress": $source_ip_address, "StartDate": $start_date, "EndDate": $end_date, "PageSize": $page_size, "Page": $page, "PageToken": $page_token} | compact), body: null}
}
...
export def "events get" [
  ...
  let base = ($base_url | default "https://monitor.twilio.com")
  if ($sid | is-empty) { error make --unspanned { msg: "path parameter 'Sid' must be non-empty" } }
  let full_url = (build-url $base ({sid: (encode-path-segment $sid)} | format pattern "/v1/Events/{sid}"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
