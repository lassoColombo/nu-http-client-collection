# Auto-generated client for azureactivedirectory v2017-04-01
# Source: https://api.apis.guru/v2/specs/azure.com/azureactivedirectory/2017-04-01/swagger.json
# Auth: --token flag or $env.AZUREACTIVEDIRECTORY_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AZUREACTIVEDIRECTORY_TOKEN | default "" }
...
export def "providers-microsoft-aadiam-diagnostic-settings list" [
  ...
  let full_url = (build-url $base "/providers/microsoft.aadiam/diagnosticSettings" $qp $auth.query)
...
export def "providers-microsoft-aadiam-diagnostic-settings delete" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({name: (encode-path-segment $name)} | format pattern "/providers/microsoft.aadiam/diagnosticSettings/{name}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "providers-microsoft-aadiam-diagnostic-settings create-or-update" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client Api Version.
  --properties: any # The diagnostic settings. — shape: {eventHubAuthorizationRuleId?: string, eventHubName?: string, logs?: list, serviceBusRuleId?: string, storageAccountId?: string, workspaceId?: string}
...
export def "providers-microsoft-aadiam-diagnostic-settings-categories list-category" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/providers/microsoft.aadiam/diagnosticSettingsCategories" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "providers-microsoft-aadiam-operations list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/providers/microsoft.aadiam/operations" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
