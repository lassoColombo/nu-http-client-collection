# Auto-generated client for Himachal Pradesh Department of Revenue, Himachal Pradesh v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/epramanhp/3.0.0/openapi.json
# Auth: --token flag or $env.HIMACHAL_PRADESH_DEPARTMENT_OF_REVENUE_HIMACHAL_PRADESH_TOKEN

const BASE_URL = "https://apisetu.gov.in/epramanhp/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o HIMACHAL_PRADESH_DEPARTMENT_OF_REVENUE_HIMACHAL_PRADESH_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/epramanhp/v3"] }
...
export def "agcer-certificate create" [
  ...
  --certificate-parameters: record # shape: {Name: string, RefNo: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o HIMACHAL_PRADESH_DEPARTMENT_OF_REVENUE_HIMACHAL_PRADESH_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o HIMACHAL_PRADESH_DEPARTMENT_OF_REVENUE_HIMACHAL_PRADESH_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
...
export def "psprt-certificate create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o HIMACHAL_PRADESH_DEPARTMENT_OF_REVENUE_HIMACHAL_PRADESH_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o HIMACHAL_PRADESH_DEPARTMENT_OF_REVENUE_HIMACHAL_PRADESH_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/psprt/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
