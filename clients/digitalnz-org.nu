...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o DIGITALNZ_API_TOKEN | default "" }
...
export def "records-format get" [
  ...
  let full_url = (build-url $base ({format: (encode-path-segment $format)} | format pattern "/records.{format}") $qp $auth.query)
...
export def "records get" [
  ...
  let full_url = (build-url $base ({record_id: (encode-path-segment $record_id), format: (encode-path-segment $format)} | format pattern "/records/{record_id}.{format}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Authentication-Token": $authentication_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "records-more-like-this-format get" [
  ...
  let full_url = (build-url $base ({record_id: (encode-path-segment $record_id), format: (encode-path-segment $format)} | format pattern "/records/{record_id}/more_like_this.{format}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Authentication-Token": $authentication_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "get"
    url: $full_url
    query: ({"fields": $fields, "mlt_fields": $mlt_fields, "filtering": $filtering} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
