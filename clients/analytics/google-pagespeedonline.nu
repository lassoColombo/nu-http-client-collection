# Auto-generated client for PageSpeed Insights API vv5
# Source: https://api.apis.guru/v2/specs/googleapis.com/pagespeedonline/v5/openapi.json
# Auth: --token flag or $env.PAGESPEED_INSIGHTS_API_TOKEN

const BASE_URL = "https://pagespeedonline.googleapis.com"
const DEFAULT_AUTH = "bearer"

# Build auth: returns {headers: record, query: string}
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o PAGESPEED_INSIGHTS_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://pagespeedonline.googleapis.com"] }
def auth-scheme-completer [] { ["bearer"] }

# Completers for enum parameters
def xgafv-completer [] { ["1" "2"] }
def alt-completer [] { ["json" "media" "proto"] }
def strategy-completer [] { ["DESKTOP" "MOBILE" "STRATEGY_UNSPECIFIED"] }

# List all available API commands with their parameters
export def commands []: nothing -> table {
  let builtin_flags = ["base-url" "token" "auth-scheme" "insecure" "max-time" "raw" "allow-errors" "dry-run" "accept" "help"]
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "pagespeedonline-run-pagespeed pagespeedonlinepagespeedapirunpagespeed" } } | get name | first)
...
export def "pagespeedonline-run-pagespeed pagespeedonlinepagespeedapirunpagespeed" [
  ...
  let qp = [(serialize-qp "$.xgafv" $xgafv "scalar") (serialize-qp "access_token" $access_token "scalar") (serialize-qp "alt" $alt "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "fields" $qp_fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $prettyPrint "scalar") (serialize-qp "quotaUser" $quotaUser "scalar") (serialize-qp "upload_protocol" $upload_protocol "scalar") (serialize-qp "uploadType" $uploadType "scalar") (serialize-qp "url" $qp_url "scalar") (serialize-qp "captchaToken" $captchaToken "scalar") (serialize-qp "category" $category "multi") (serialize-qp "locale" $locale "scalar") (serialize-qp "strategy" $strategy "scalar") (serialize-qp "utm_campaign" $utm_campaign "scalar") (serialize-qp "utm_source" $utm_source "scalar")] | flatten | str join "&"
