...
export def "subscriptions-providers-microsoft-subscriptions-admin-subscriptions-acquired-plans List" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/providers/Microsoft.Subscriptions.Admin/subscriptions/($targetSubscriptionId)/acquiredPlans" $qp)
...
export def "subscriptions-providers-microsoft-subscriptions-admin-subscriptions-acquired-plans Delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/providers/Microsoft.Subscriptions.Admin/subscriptions/($targetSubscriptionId)/acquiredPlans/($planAcquisitionId)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
