# Auto-generated client for Locations v2.0
# Source: https://api.apis.guru/v2/specs/whapi.com/locations/2.0/swagger.json
# Auth: --token flag or $env.LOCATIONS_TOKEN

const BASE_URL = "https://sandbox.whapi.com/v2/locations"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o LOCATIONS_TOKEN | default "" }
...
def base-url-completer [] { ["https://sandbox.whapi.com/v2/locations"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "address-lookup get" } } | get name | first)
...
export def "address-lookup get" [
  ...
  let full_url = (build-url $base "/address/lookup/" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"houseNum": $house_num, "postCode": $post_code} | compact)
...
export def "countries get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> record<countries: table<code: string, currencyCode: string, dialCode: string, name: string>> {
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
...
export def "countries get-country" [
  ...
  let full_url = (build-url $base ({country_code: (encode-path-segment $country_code)} | format pattern "/countries/{country_code}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "currencies get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> record<currencies: table<code: string, exchangeRate: float, maxDeposit: float, maxWithdrawal: float, minDeposit: float, minWithdrawal: float, name: string>> {
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
...
export def "currencies get-currency" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  if ($currency_code | is-empty) { error make --unspanned { msg: "path parameter 'currencyCode' must be non-empty" } }
  let full_url = (build-url $base ({currency_code: (encode-path-segment $currency_code)} | format pattern "/currencies/{currency_code}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
