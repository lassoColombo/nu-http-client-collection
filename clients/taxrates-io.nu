# Auto-generated client for Taxrates.io API v1.0.0
# Source: https://api.apis.guru/v2/specs/taxrates.io/1.0.0/openapi.json
# Auth: --token flag or $env.TAXRATES_IO_API_TOKEN

const BASE_URL = "https://api.taxrates.io/api"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o TAXRATES_IO_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.taxrates.io/api"] }
...
export def "tax-countrycode get-rates-by-country-code" [
  ...
  --domain: string # Domain name: api.taxrates.io (e.g. api.taxrates.io)
  --country-code: string # Country code alpha 2 (e.g. US)
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"domain": $domain, "country_code": $country_code, "filter": $filter, "zip": $zip, "product_codes[]": $product_codes, "province ": $province, "date": $date} | compact), body: null}
}
...
export def "tax-ip get-rates-by-address" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --domain: string # Domain name: api.taxrates.io (e.g. api.taxrates.io)
  --ip: string # Customer's IP address (e.g. 86.139.70.49)
  --filter: string # For US sales tax you can filter the tax type (e.g. )
  --zip: string # For US sales tax a Zipcode must be proivded (e.g. )
  --product-code: string # Your can filter your taxes by product code (e.g. C010)
]: nothing -> table<country_name: string, taxes: list<record>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "domain" $domain "scalar") (serialize-qp "ip" $ip "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "zip" $zip "scalar") (serialize-qp "product_code" $product_code "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/v1/tax/ip" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"domain": $domain, "ip": $ip, "filter": $filter, "zip": $zip, "product_code": $product_code} | compact), body: null}
}
...
export def "tax-rates list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"domain": $domain, "filter": $filter, "cursor": $cursor, "Product_code": $product_code} | compact), body: null}
}
