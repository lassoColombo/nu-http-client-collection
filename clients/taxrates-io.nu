...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o TAXRATES_IO_API_TOKEN | default "" }
...
export def "tax-countrycode get-rates-by-country-code" [
  ...
  let full_url = (build-url $base "/v1/tax/countrycode" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"domain": $domain, "country_code": $country_code, "filter": $filter, "zip": $zip, "product_codes[]": $product_codes, "province ": $province, "date": $date} | compact)
...
export def "tax-ip get-rates-by-address" [
  ...
  let full_url = (build-url $base "/v1/tax/ip" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"domain": $domain, "ip": $ip, "filter": $filter, "zip": $zip, "product_code": $product_code} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "tax-rates list" [
  ...
  let full_url = (build-url $base "/v3/tax/rates" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"domain": $domain, "filter": $filter, "cursor": $cursor, "Product_code": $product_code} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
