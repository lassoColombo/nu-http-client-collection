...
export def "account-jwt create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-appwrite-project"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/account/jwt" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "account-sessions-anonymous create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-appwrite-project"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/account/sessions/anonymous" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "account-sessions-oauth2 create-o-auth2" [
  ...
  let full_url = (build-url $base ({provider: (encode-path-segment $provider)} | format pattern "/account/sessions/oauth2/{provider}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"success": $success, "failure": $failure, "scopes": $scopes} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
