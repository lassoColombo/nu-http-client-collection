# Auto-generated client for Azure Metrics v2018-09-01-preview
# Source: https://api.apis.guru/v2/specs/azure.com/monitor-metricsCreate_API/2018-09-01-preview/swagger.json
# Auth: --token flag or $env.AZURE_METRICS_TOKEN

const BASE_URL = "https://monitoring.azure.com"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o AZURE_METRICS_TOKEN | default "" }
...
def base-url-completer [] { ["https://monitoring.azure.com"] }
...
export def "subscriptions-resourcegroups-providers-metrics create" [
  ...
  if ($resource_provider | is-empty) { error make --unspanned { msg: "path parameter 'resourceProvider' must be non-empty" } }
  ...
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), resource_provider: (encode-path-segment $resource_provider), resource_type_name: (encode-path-segment $resource_type_name), resource_name: (encode-path-segment $resource_name)} | format pattern "/subscriptions/{subscription_id}/resourcegroups/{resource_group_name}/providers/{resource_provider}/{resource_type_name}/{resource_name}/metrics"))
  let req_body = {"data": $data, "time": $time} | compact
