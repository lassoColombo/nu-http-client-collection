# Auto-generated client for Importer-Exporter Details API v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/dgft/3.0.0/openapi.json
# Auth: --token flag or $env.IMPORTER_EXPORTER_DETAILS_API_TOKEN

const BASE_URL = "https://apisetu.gov.in/dgft"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o IMPORTER_EXPORTER_DETAILS_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/dgft"] }
...
export def "iec get-importer-exporter-code-verification" [
  ...
  if ($iec | is-empty) { error make --unspanned { msg: "path parameter 'iec' must be non-empty" } }
  let full_url = (build-url $base ({iec: (encode-path-segment $iec)} | format pattern "/v1/iec/{iec}") $auth.query)
