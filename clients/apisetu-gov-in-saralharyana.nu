...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o ANTYODAYA_SARAL_HARYANA_HARYANA_TOKEN | default "" }
...
export def "nbcer-certificate create" [
  ...
  let full_url = (build-url $base "/nbcer/certificate" $auth.query)
