# Auto-generated client for Antyodaya Saral Haryana, Haryana v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/saralharyana/3.0.0/openapi.json
# Auth: --token flag or $env.ANTYODAYA_SARAL_HARYANA_HARYANA_TOKEN

const BASE_URL = "https://apisetu.gov.in/saralharyana/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o ANTYODAYA_SARAL_HARYANA_HARYANA_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/saralharyana/v3"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "nbcer-certificate create" } } | get name | first)
...
export def "nbcer-certificate create" [
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o ANTYODAYA_SARAL_HARYANA_HARYANA_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o ANTYODAYA_SARAL_HARYANA_HARYANA_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/nbcer/certificate")
