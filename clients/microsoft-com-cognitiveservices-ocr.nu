...
# Source: https://api.apis.guru/v2/specs/microsoft.com/cognitiveservices-Ocr/2.1/openapi.json
...
def mode-completer [] { ["Handwritten" "Printed"] }
...
]: any -> record<code: any, message: string, requestId: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "ocp-apim-subscription-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/read/core/asyncBatchAnalyze" $auth.query)
...
export def "read-operations get-result" [
  ...
  let full_url = (build-url $base ({operation_id: (encode-path-segment $operation_id)} | format pattern "/read/operations/{operation_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "recognize-text create" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --mode: string@mode-completer # Type of text to recognize.
  url: string # Publicly reachable URL of an image.
]: any -> record<code: any, message: string, requestId: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "ocp-apim-subscription-key"))
