...
def callsState-completer [] { ["CALLS_STATE_UNSPECIFIED" "DISABLED" "ENABLED"] }
...
export def "locations mybusinessbusinesscallslocationsupdateBusinesscallssettings" [
  ...
  --callsState: string@callsState-completer # Required. The state of this location's enrollment in Business calls.
  --consentTime: string # Input only. Time when the end user provided consent to the API user to enable business calls. (format: google-datetime)
  ...
  let full_url = (build-url $base $"/v1/($name)" $qp)
  let body = {callsState: $callsState, consentTime: $consentTime, name: $body_name} | compact
...
export def "businesscallsinsights mybusinessbusinesscallslocationsbusinesscallsinsightslist" [
  ...
  --pageSize: int # Optional. The maximum number of BusinessCallsInsights to return. If unspecified, at most 20 will be returned. Some of the metric_types(e.g, AGGREGATE_COUNT) returns a single page. For these metrics, the page_size is ignored.
  ...
  let full_url = (build-url $base $"/v1/($parent)/businesscallsinsights" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
