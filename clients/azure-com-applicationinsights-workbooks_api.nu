# Auto-generated client for ApplicationInsightsManagementClient v2018-06-17-preview
# Source: https://api.apis.guru/v2/specs/azure.com/applicationinsights-workbooks_API/2018-06-17-preview/swagger.json
...
def category-completer [] { ["TSG" "performance" "retention" "workbook"] }
def kind-completer [] { ["shared" "user"] }
...
export def "subscriptions-resource-group-providers-microsoft-insights-workbooks list" [
  ...
  --tags: list<string> # Tags presents on each workbook returned.
  ...
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name)} | format pattern "/subscriptions/{subscription_id}/resourceGroup/{resource_group_name}/providers/microsoft.insights/workbooks") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"category": $category, "tags": $tags, "sourceId": $source_id, "canFetchContent": $can_fetch_content, "api-version": $api_version} | compact), body: null}
}
...
export def "subscriptions-resource-group-providers-microsoft-insights-workbooks delete" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # The API version to use for this operation.
]: nothing -> record<code: string, details: table<code: string, message: string, target: string>, message: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
  if ($resource_group_name | is-empty) { error make --unspanned { msg: "path parameter 'resourceGroupName' must be non-empty" } }
...
export def "subscriptions-resource-group-providers-microsoft-insights-workbooks get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # The API version to use for this operation.
]: nothing -> record<properties: record<category: string, displayName: string, serializedData: string, tags: list<string>, timeModified: string, userId: string, version: string>, id: string, kind: string, location: string, name: string, tags: record, type: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
  if ($resource_group_name | is-empty) { error make --unspanned { msg: "path parameter 'resourceGroupName' must be non-empty" } }
...
export def "subscriptions-resource-group-providers-microsoft-insights-workbooks update" [
  ...
  --properties: any # Properties that contain a workbook for PATCH operation. — shape: {category?: string, displayName?: string, serializedData?: string, tags?: list<string>}
  --tags: record # Resource tags
]: any -> record<properties: record<category: string, displayName: string, serializedData: string, tags: list<string>, timeModified: string, userId: string, version: string>, id: string, kind: string, location: string, name: string, tags: record, type: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
  ...
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), resource_name: (encode-path-segment $resource_name)} | format pattern "/subscriptions/{subscription_id}/resourceGroup/{resource_group_name}/providers/microsoft.insights/workbooks/{resource_name}") $qp)
  let req_body = {"kind": $kind, "properties": $properties, "tags": $tags} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "patch" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"sourceId": $source_id, "api-version": $api_version} | compact), body: $req_body}
}
...
export def "subscriptions-resource-group-providers-microsoft-insights-workbooks create-or-update" [
  ...
  --source-id: string # Azure Resource Id that will fetch all related workbooks.
  --api-version: string # The API version to use for this operation.
  --properties: any # Properties that contain a workbook. — shape: {category: string, displayName: string, serializedData: string, tags?: list<string>, userId: string, version?: string}
  --kind: string@kind-completer # The kind of workbook. Choices are user and shared.
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"sourceId": $source_id, "api-version": $api_version} | compact), body: $req_body}
}
