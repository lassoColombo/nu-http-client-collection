...
export def "subscriptions-resource-group-providers-microsoft-insights-workbooks list" [
  ...
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name)} | format pattern "/subscriptions/{subscription_id}/resourceGroup/{resource_group_name}/providers/microsoft.insights/workbooks") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"category": $category, "tags": $tags, "sourceId": $source_id, "canFetchContent": $can_fetch_content, "api-version": $api_version} | compact)
...
export def "subscriptions-resource-group-providers-microsoft-insights-workbooks update" [
  ...
  let req = {
    method: "patch"
    url: $full_url
    query: ({"sourceId": $source_id, "api-version": $api_version} | compact)
    headers: $auth.headers
    body: $req_body
    content_type: "application/json"
    timeout: ($max_time | default 30min)
