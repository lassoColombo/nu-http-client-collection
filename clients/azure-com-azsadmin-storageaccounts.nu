# Auto-generated client for StorageManagementClient v2019-08-08-preview
# Source: https://api.apis.guru/v2/specs/azure.com/azsadmin-storageaccounts/2019-08-08-preview/swagger.json
...
export def "subscriptions-providers-microsoft-storage-admin-locations-reclaim-storage-capacity create-accounts" [
...
]: nothing -> record<code: string, details: list<string>, message: string, target: string> {
  ...
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), location: (encode-path-segment $location)} | format pattern "/subscriptions/{subscription_id}/providers/Microsoft.Storage.Admin/locations/{location}/reclaimStorageCapacity") $qp)
...
export def "subscriptions-providers-microsoft-storage-admin-locations-storageaccounts list-accounts" [
  ...
  --summary: oneof<nothing, bool> # Switch for whether summary or detailed information is returned.
]: nothing -> record<_odata_nextLink: string, value: table<kind: string, properties: record, tags: record>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
  if ($location | is-empty) { error make --unspanned { msg: "path parameter 'location' must be non-empty" } }
  ...
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), location: (encode-path-segment $location)} | format pattern "/subscriptions/{subscription_id}/providers/Microsoft.Storage.Admin/locations/{location}/storageaccounts") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version, "$filter": $filter, "summary": $summary} | compact), body: null}
}
...
export def "subscriptions-providers-microsoft-storage-admin-locations-storageaccounts get-accounts" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # REST Api Version.
]: nothing -> record<kind: string, properties: record<accessTier: string, accountId: string, accountStatus: string, accountType: string, creationTime: string, deletedTime: string, encryption: record<keySource: string, services: record>, faultDomain: string, healthState: string, primaryEndpoints: any, primaryLocation: string, provisioningState: string, statusOfPrimary: string, supportsHttpsTrafficOnly: bool, tenantResourceGroupName: string, tenantStorageAccountName: string, tenantSubscriptionId: string, tenantViewId: string>, tags: record> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
  if ($location | is-empty) { error make --unspanned { msg: "path parameter 'location' must be non-empty" } }
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), location: (encode-path-segment $location), account_id: (encode-path-segment $account_id)} | format pattern "/subscriptions/{subscription_id}/providers/Microsoft.Storage.Admin/locations/{location}/storageaccounts/{account_id}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "subscriptions-providers-microsoft-storage-admin-locations-storageaccounts-undelete create-accounts" [
  ...
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), location: (encode-path-segment $location), account_id: (encode-path-segment $account_id)} | format pattern "/subscriptions/{subscription_id}/providers/Microsoft.Storage.Admin/locations/{location}/storageaccounts/{account_id}/undelete") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version, "newAccountName": $new_account_name} | compact), body: null}
}
