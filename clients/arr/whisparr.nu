# Auto-generated client for Whisparr v3.0.0
# Source: https://raw.githubusercontent.com/Whisparr/Whisparr/develop/src/Whisparr.Api.V3/openapi.json
# Auth: --token flag or $env.WHISPARR_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o WHISPARR_TOKEN | default "" }
...
def base-url-completer [] { ["http://localhost:6969" "https://localhost:6969"] }
...
def shouldMonitor-completer [] { ["entireSite" "none" "specificEpisode"] }
def siteMonitorType-completer [] { ["all" "existing" "firstSeason" "future" "latestSeason" "missing" "none" "unknown"] }
...
export def "calendar list" [
  ...
  --tags: string # default: 
]: nothing -> table<id: int, seriesId: int, tvdbId: int, episodeFileId: int, seasonNumber: int, title: string, releaseDate: record<year: int, month: int, day: int, dayOfWeek: string, dayOfYear: int, dayNumber: int>, lastSearchTime: string, runtime: int, overview: string, episodeFile: record<id: int, seriesId: int, seasonNumber: int, relativePath: string, path: string, size: int, dateAdded: string, sceneName: string, releaseGroup: string, languages: list, quality: record, customFormats: list, customFormatScore: int, mediaInfo: record, qualityCutoffNotMet: bool>, hasFile: bool, monitored: bool, absoluteEpisodeNumber: int, endTime: string, grabDate: string, seriesTitle: string, series: record<id: int, title: string, sortTitle: string, status: string, ended: bool, profileName: string, overview: string, nextAiring: string, previousAiring: string, network: string, images: list, originalLanguage: record, remotePoster: string, seasons: list, year: int, path: string, qualityProfileId: int, monitored: bool, useSceneNumbering: bool, runtime: int, tvdbId: int, firstAired: string, cleanTitle: string, titleSlug: string, rootFolderPath: string, folder: string, certification: string, genres: list, tags: list, added: string, addOptions: record, ratings: record, statistics: record, episodesChanged: bool>, actors: list<record>, images: list<record>, grabbed: bool> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
export def "calendar get" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
]: nothing -> record<id: int, seriesId: int, tvdbId: int, episodeFileId: int, seasonNumber: int, title: string, releaseDate: record<year: int, month: int, day: int, dayOfWeek: string, dayOfYear: int, dayNumber: int>, lastSearchTime: string, runtime: int, overview: string, episodeFile: record<id: int, seriesId: int, seasonNumber: int, relativePath: string, path: string, size: int, dateAdded: string, sceneName: string, releaseGroup: string, languages: list<record>, quality: record<quality: record, revision: record>, customFormats: list<record>, customFormatScore: int, mediaInfo: record<id: int, audioBitrate: int, audioChannels: float, audioCodec: string, audioLanguages: string, audioStreamCount: int, videoBitDepth: int, videoBitrate: int, videoCodec: string, videoFps: float, videoDynamicRange: string, videoDynamicRangeType: string, resolution: string, runTime: string, scanType: string, subtitles: string>, qualityCutoffNotMet: bool>, hasFile: bool, monitored: bool, absoluteEpisodeNumber: int, endTime: string, grabDate: string, seriesTitle: string, series: record<id: int, title: string, sortTitle: string, status: string, ended: bool, profileName: string, overview: string, nextAiring: string, previousAiring: string, network: string, images: list<record>, originalLanguage: record<id: int, name: string>, remotePoster: string, seasons: list<record>, year: int, path: string, qualityProfileId: int, monitored: bool, useSceneNumbering: bool, runtime: int, tvdbId: int, firstAired: string, cleanTitle: string, titleSlug: string, rootFolderPath: string, folder: string, certification: string, genres: list<string>, tags: list<int>, added: string, addOptions: record<ignoreEpisodesWithFiles: bool, ignoreEpisodesWithoutFiles: bool, episodesToMonitor: list, monitor: string, searchForMissingEpisodes: bool, searchForCutoffUnmetEpisodes: bool>, ratings: record<votes: int, value: float>, statistics: record<seasonCount: int, episodeFileCount: int, episodeCount: int, totalEpisodeCount: int, sizeOnDisk: int, releaseGroups: list, percentOfEpisodes: float>, episodesChanged: bool>, actors: table<tpdbId: int, name: string, character: string, gender: string, images: list>, images: table<coverType: string, url: string, remoteUrl: string>, grabbed: bool> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
# GET /feed/v3/calendar/whisparr.ics
export def "feed-calendar-whisparrics get" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let full_url = (build-url $base "/feed/v3/calendar/whisparr.ics" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "command post" [
  ...
  --duration: record # shape: {ticks?: int}
...
]: any -> record<id: int, name: string, commandName: string, message: string, body: record<sendUpdatesToClient: bool, updateScheduledTask: bool, completionMessage: string, requiresDiskAccess: bool, isExclusive: bool, isLongRunning: bool, name: string, lastExecutionTime: string, lastStartTime: string, trigger: string, suppressMessages: bool, clientUserAgent: string>, priority: string, status: string, result: string, queued: string, started: string, ended: string, duration: record<ticks: int, days: int, hours: int, milliseconds: int, minutes: int, seconds: int, totalDays: float, totalHours: float, totalMilliseconds: float, totalMinutes: float, totalSeconds: float>, exception: string, trigger: string, clientUserAgent: string, stateChangeTime: string, sendUpdatesToClient: bool, updateScheduledTask: bool, lastExecutionTime: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
]: nothing -> record<page: int, pageSize: int, sortKey: string, sortDirection: string, totalRecords: int, records: table<id: int, seriesId: int, tvdbId: int, episodeFileId: int, seasonNumber: int, title: string, releaseDate: record, lastSearchTime: string, runtime: int, overview: string, episodeFile: record, hasFile: bool, monitored: bool, absoluteEpisodeNumber: int, endTime: string, grabDate: string, seriesTitle: string, series: record, actors: list, images: list, grabbed: bool>> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
export def "downloadclient-test post" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/downloadclient/test")
  let body = {id: $id, name: $name, fields: $body_fields, implementationName: $implementationName, implementation: $implementation, configContract: $configContract, infoLink: $infoLink, message: $message, tags: $tags, presets: $presets, enable: $enable, protocol: $protocol, priority: $priority, removeCompletedDownloads: $removeCompletedDownloads, removeFailedDownloads: $removeFailedDownloads} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "config-downloadclient list" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
]: nothing -> record<id: int, downloadClientWorkingFolders: string, enableCompletedDownloadHandling: bool, autoRedownloadFailed: bool> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
]: any -> record<id: int, downloadClientWorkingFolders: string, enableCompletedDownloadHandling: bool, autoRedownloadFailed: bool> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/config/downloadclient/($id)")
  let body = {id: $body_id, downloadClientWorkingFolders: $downloadClientWorkingFolders, enableCompletedDownloadHandling: $enableCompletedDownloadHandling, autoRedownloadFailed: $autoRedownloadFailed} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
#
# --releaseDate shape: {year?: int, month?: int, day?: int, dayOfWeek?: "sunday"|"monday"|"tuesday"|"wednesday"|"thursday"|"friday"|"saturday"}
# --episodeFile shape: {id?: int, seriesId?: int, seasonNumber?: int, relativePath?: string, path?: string, size?: int, dateAdded?: string, sceneName?: string, releaseGroup?: string, languages?: list, quality?: record, customFormats?: list, customFormatScore?: int, mediaInfo?: record, qualityCutoffNotMet?: bool}
# --series shape: {id?: int, title?: string, sortTitle?: string, status?: "continuing"|"ended"|"upcoming"|"deleted", profileName?: string, overview?: string, nextAiring?: string, previousAiring?: string, network?: string, images?: list, originalLanguage?: record, remotePoster?: string, seasons?: list, year?: int, path?: string, qualityProfileId?: int, monitored?: bool, useSceneNumbering?: bool, runtime?: int, tvdbId?: int, firstAired?: string, cleanTitle?: string, titleSlug?: string, rootFolderPath?: string, folder?: string, certification?: string, genres?: list, tags?: list, added?: string, addOptions?: record, ratings?: record, statistics?: record, episodesChanged?: bool}
# --actors item shape: {tpdbId?: int, name?: string, character?: string, gender?: "female"|"male"|"other", images?: list}
# --images item shape: {coverType?: "unknown"|"logo"|"poster"|"banner"|"fanart"|"screenshot"|"headshot"|"clearlogo", url?: string, remoteUrl?: string}
export def "episode put" [
  ...
  --releaseDate: record # shape: {year?: int, month?: int, day?: int, dayOfWeek?: "sunday"|"monday"|"tuesday"|"wednesday"|"thursday"|"friday"|"saturday"}
  --lastSearchTime: string # nullable, format: date-time
  --runtime: int # format: int32
  --overview: string # nullable
  --episodeFile: record # shape: {id?: int, seriesId?: int, seasonNumber?: int, relativePath?: string, path?: string, size?: int, dateAdded?: string, sceneName?: string, releaseGroup?: string, languages?: list, quality?: record, customFormats?: list, customFormatScore?: int, mediaInfo?: record, qualityCutoffNotMet?: bool}
  ...
  --series: record # shape: {id?: int, title?: string, sortTitle?: string, status?: "continuing"|"ended"|"upcoming"|"deleted", profileName?: string, overview?: string, nextAiring?: string, previousAiring?: string, network?: string, images?: list, originalLanguage?: record, remotePoster?: string, seasons?: list, year?: int, path?: string, qualityProfileId?: int, monitored?: bool, useSceneNumbering?: bool, runtime?: int, tvdbId?: int, firstAired?: string, cleanTitle?: string, titleSlug?: string, rootFolderPath?: string, folder?: string, certification?: string, genres?: list, tags?: list, added?: string, addOptions?: record, ratings?: record, statistics?: record, episodesChanged?: bool}
  --actors: list # nullable — item shape: {tpdbId?: int, name?: string, character?: string, gender?: "female"|"male"|"other", images?: list}
  --images: list # nullable — item shape: {coverType?: "unknown"|"logo"|"poster"|"banner"|"fanart"|"screenshot"|"headshot"|"clearlogo", url?: string, remoteUrl?: string}
  --grabbed: string@bool-completer
]: any -> record<id: int, seriesId: int, tvdbId: int, episodeFileId: int, seasonNumber: int, title: string, releaseDate: record<year: int, month: int, day: int, dayOfWeek: string, dayOfYear: int, dayNumber: int>, lastSearchTime: string, runtime: int, overview: string, episodeFile: record<id: int, seriesId: int, seasonNumber: int, relativePath: string, path: string, size: int, dateAdded: string, sceneName: string, releaseGroup: string, languages: list<record>, quality: record<quality: record, revision: record>, customFormats: list<record>, customFormatScore: int, mediaInfo: record<id: int, audioBitrate: int, audioChannels: float, audioCodec: string, audioLanguages: string, audioStreamCount: int, videoBitDepth: int, videoBitrate: int, videoCodec: string, videoFps: float, videoDynamicRange: string, videoDynamicRangeType: string, resolution: string, runTime: string, scanType: string, subtitles: string>, qualityCutoffNotMet: bool>, hasFile: bool, monitored: bool, absoluteEpisodeNumber: int, endTime: string, grabDate: string, seriesTitle: string, series: record<id: int, title: string, sortTitle: string, status: string, ended: bool, profileName: string, overview: string, nextAiring: string, previousAiring: string, network: string, images: list<record>, originalLanguage: record<id: int, name: string>, remotePoster: string, seasons: list<record>, year: int, path: string, qualityProfileId: int, monitored: bool, useSceneNumbering: bool, runtime: int, tvdbId: int, firstAired: string, cleanTitle: string, titleSlug: string, rootFolderPath: string, folder: string, certification: string, genres: list<string>, tags: list<int>, added: string, addOptions: record<ignoreEpisodesWithFiles: bool, ignoreEpisodesWithoutFiles: bool, episodesToMonitor: list, monitor: string, searchForMissingEpisodes: bool, searchForCutoffUnmetEpisodes: bool>, ratings: record<votes: int, value: float>, statistics: record<seasonCount: int, episodeFileCount: int, episodeCount: int, totalEpisodeCount: int, sizeOnDisk: int, releaseGroups: list, percentOfEpisodes: float>, episodesChanged: bool>, actors: table<tpdbId: int, name: string, character: string, gender: string, images: list>, images: table<coverType: string, url: string, remoteUrl: string>, grabbed: bool> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/episode/($id)")
  let body = {id: $body_id, seriesId: $seriesId, tvdbId: $tvdbId, episodeFileId: $episodeFileId, seasonNumber: $seasonNumber, title: $title, releaseDate: $releaseDate, lastSearchTime: $lastSearchTime, runtime: $runtime, overview: $overview, episodeFile: $episodeFile, hasFile: $hasFile, monitored: $monitored, absoluteEpisodeNumber: $absoluteEpisodeNumber, endTime: $endTime, grabDate: $grabDate, seriesTitle: $seriesTitle, series: $series, actors: $actors, images: $images, grabbed: $grabbed} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "episodefile list" [
  ...
  --seriesId: int # format: int32
  --episodeFileIds: list
]: nothing -> table<id: int, seriesId: int, seasonNumber: int, relativePath: string, path: string, size: int, dateAdded: string, sceneName: string, releaseGroup: string, languages: list<record>, quality: record<quality: record, revision: record>, customFormats: list<record>, customFormatScore: int, mediaInfo: record<id: int, audioBitrate: int, audioChannels: float, audioCodec: string, audioLanguages: string, audioStreamCount: int, videoBitDepth: int, videoBitrate: int, videoCodec: string, videoFps: float, videoDynamicRange: string, videoDynamicRangeType: string, resolution: string, runTime: string, scanType: string, subtitles: string>, qualityCutoffNotMet: bool> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
export def "history get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "page" $page "scalar") (serialize-qp "pageSize" $pageSize "scalar") (serialize-qp "sortKey" $sortKey "scalar") (serialize-qp "sortDirection" $sortDirection "scalar") (serialize-qp "includeSeries" $includeSeries "scalar") (serialize-qp "includeEpisode" $includeEpisode "scalar") (serialize-qp "eventType" $eventType "scalar") (serialize-qp "episodeId" $episodeId "scalar") (serialize-qp "downloadId" $downloadId "scalar")] | flatten | str join "&"
...
export def "config-host put" [
  ...
  let body = {id: $body_id, bindAddress: $bindAddress, port: $port, sslPort: $sslPort, enableSsl: $enableSsl, launchBrowser: $launchBrowser, authenticationMethod: $authenticationMethod, authenticationRequired: $authenticationRequired, analyticsEnabled: $analyticsEnabled, username: $username, password: $password, logLevel: $logLevel, logSizeLimit: $logSizeLimit, consoleLogLevel: $consoleLogLevel, branch: $branch, apiKey: $apiKey, sslCertPath: $sslCertPath, sslCertPassword: $sslCertPassword, urlBase: $urlBase, instanceName: $instanceName, applicationUrl: $applicationUrl, updateAutomatically: $updateAutomatically, updateMechanism: $updateMechanism, updateScriptPath: $updateScriptPath, proxyEnabled: $proxyEnabled, proxyType: $proxyType, proxyHostname: $proxyHostname, proxyPort: $proxyPort, proxyUsername: $proxyUsername, proxyPassword: $proxyPassword, proxyBypassFilter: $proxyBypassFilter, proxyBypassLocalAddresses: $proxyBypassLocalAddresses, certificateValidation: $certificateValidation, backupFolder: $backupFolder, backupInterval: $backupInterval, backupRetention: $backupRetention} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "importlist list" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
]: nothing -> table<id: int, name: string, fields: list<record>, implementationName: string, implementation: string, configContract: string, infoLink: string, message: record<message: string, type: string>, tags: list<int>, presets: list<any>, enableAutomaticAdd: bool, shouldMonitor: string, siteMonitorType: string, rootFolderPath: string, qualityProfileId: int, listType: string, listOrder: int, minRefreshInterval: record<ticks: int, days: int, hours: int, milliseconds: int, minutes: int, seconds: int, totalDays: float, totalHours: float, totalMilliseconds: float, totalMinutes: float, totalSeconds: float>> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
# --fields item shape: {order?: int, name?: string, label?: string, unit?: string, helpText?: string, helpTextWarning?: string, helpLink?: string, value?: any, type?: string, advanced?: bool, selectOptions?: list, selectOptionsProviderAction?: string, section?: string, hidden?: string, privacy?: "normal"|"password"|"apiKey"|"userName", placeholder?: string, isFloat?: bool}
# --message shape: {message?: string, type?: "info"|"warning"|"error"}
# --presets item shape: {id?: int, name?: string, fields?: list, implementationName?: string, implementation?: string, configContract?: string, infoLink?: string, message?: record, tags?: list, presets?: list, enableAutomaticAdd?: bool, shouldMonitor?: "none"|"specificEpisode"|"entireSite", siteMonitorType?: "unknown"|"all"|"future"|"missing"|"existing"|"firstSeason"|"latestSeason"|"none", rootFolderPath?: string, qualityProfileId?: int, listType?: "program"|"plex"|"trakt"|"simkl"|"other"|"advanced", listOrder?: int, minRefreshInterval?: record}
# --minRefreshInterval shape: {ticks?: int}
export def "importlist post" [
  ...
  --siteMonitorType: string@siteMonitorType-completer
  ...
  --minRefreshInterval: record # shape: {ticks?: int}
]: any -> record<id: int, name: string, fields: table<order: int, name: string, label: string, unit: string, helpText: string, helpTextWarning: string, helpLink: string, value: any, type: string, advanced: bool, selectOptions: list, selectOptionsProviderAction: string, section: string, hidden: string, privacy: string, placeholder: string, isFloat: bool>, implementationName: string, implementation: string, configContract: string, infoLink: string, message: record<message: string, type: string>, tags: list<int>, presets: list<any>, enableAutomaticAdd: bool, shouldMonitor: string, siteMonitorType: string, rootFolderPath: string, qualityProfileId: int, listType: string, listOrder: int, minRefreshInterval: record<ticks: int, days: int, hours: int, milliseconds: int, minutes: int, seconds: int, totalDays: float, totalHours: float, totalMilliseconds: float, totalMinutes: float, totalSeconds: float>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  ...
  let body = {id: $id, name: $name, fields: $body_fields, implementationName: $implementationName, implementation: $implementation, configContract: $configContract, infoLink: $infoLink, message: $message, tags: $tags, presets: $presets, enableAutomaticAdd: $enableAutomaticAdd, shouldMonitor: $shouldMonitor, siteMonitorType: $siteMonitorType, rootFolderPath: $rootFolderPath, qualityProfileId: $qualityProfileId, listType: $listType, listOrder: $listOrder, minRefreshInterval: $minRefreshInterval} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "importlist-test post" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/importlist/test")
  let body = {id: $id, name: $name, fields: $body_fields, implementationName: $implementationName, implementation: $implementation, configContract: $configContract, infoLink: $infoLink, message: $message, tags: $tags, presets: $presets, enableAutomaticAdd: $enableAutomaticAdd, shouldMonitor: $shouldMonitor, siteMonitorType: $siteMonitorType, rootFolderPath: $rootFolderPath, qualityProfileId: $qualityProfileId, listType: $listType, listOrder: $listOrder, minRefreshInterval: $minRefreshInterval} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "indexer-test post" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/indexer/test")
  let body = {id: $id, name: $name, fields: $body_fields, implementationName: $implementationName, implementation: $implementation, configContract: $configContract, infoLink: $infoLink, message: $message, tags: $tags, presets: $presets, enableRss: $enableRss, enableAutomaticSearch: $enableAutomaticSearch, enableInteractiveSearch: $enableInteractiveSearch, supportsRss: $supportsRss, supportsSearch: $supportsSearch, protocol: $protocol, priority: $priority, seasonSearchMaximumSingleEpisodeAge: $seasonSearchMaximumSingleEpisodeAge, downloadClientId: $downloadClientId} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
]: nothing -> table<id: int, path: string, relativePath: string, folderName: string, name: string, size: int, series: record<id: int, title: string, sortTitle: string, status: string, ended: bool, profileName: string, overview: string, nextAiring: string, previousAiring: string, network: string, images: list, originalLanguage: record, remotePoster: string, seasons: list, year: int, path: string, qualityProfileId: int, monitored: bool, useSceneNumbering: bool, runtime: int, tvdbId: int, firstAired: string, cleanTitle: string, titleSlug: string, rootFolderPath: string, folder: string, certification: string, genres: list, tags: list, added: string, addOptions: record, ratings: record, statistics: record, episodesChanged: bool>, seasonNumber: int, episodes: list<record>, episodeFileId: int, releaseGroup: string, quality: record<quality: record, revision: record>, languages: list<record>, qualityWeight: int, downloadId: string, customFormats: list<record>, customFormatScore: int, rejections: list<record>> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
export def "metadata-test post" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/metadata/test")
  let body = {id: $id, name: $name, fields: $body_fields, implementationName: $implementationName, implementation: $implementation, configContract: $configContract, infoLink: $infoLink, message: $message, tags: $tags, presets: $presets, enable: $enable} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "config-naming list" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --accept: string@accept-completer # Response content type
]: nothing -> record<id: int, renameEpisodes: bool, replaceIllegalCharacters: bool, colonReplacementFormat: int, multiEpisodeStyle: int, standardEpisodeFormat: string, seriesFolderFormat: string, includeSeriesTitle: bool, includeEpisodeTitle: bool, includeQuality: bool, replaceSpaces: bool, separator: string, numberStyle: string> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
export def "config-naming put" [
  ...
  --includeSeriesTitle: string@bool-completer
  --includeEpisodeTitle: string@bool-completer
...
]: any -> record<id: int, renameEpisodes: bool, replaceIllegalCharacters: bool, colonReplacementFormat: int, multiEpisodeStyle: int, standardEpisodeFormat: string, seriesFolderFormat: string, includeSeriesTitle: bool, includeEpisodeTitle: bool, includeQuality: bool, replaceSpaces: bool, separator: string, numberStyle: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/config/naming/($id)")
  let body = {id: $body_id, renameEpisodes: $renameEpisodes, replaceIllegalCharacters: $replaceIllegalCharacters, colonReplacementFormat: $colonReplacementFormat, multiEpisodeStyle: $multiEpisodeStyle, standardEpisodeFormat: $standardEpisodeFormat, seriesFolderFormat: $seriesFolderFormat, includeSeriesTitle: $includeSeriesTitle, includeEpisodeTitle: $includeEpisodeTitle, includeQuality: $includeQuality, replaceSpaces: $replaceSpaces, separator: $separator, numberStyle: $numberStyle} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "config-naming-examples get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "renameEpisodes" $renameEpisodes "scalar") (serialize-qp "replaceIllegalCharacters" $replaceIllegalCharacters "scalar") (serialize-qp "colonReplacementFormat" $colonReplacementFormat "scalar") (serialize-qp "multiEpisodeStyle" $multiEpisodeStyle "scalar") (serialize-qp "standardEpisodeFormat" $standardEpisodeFormat "scalar") (serialize-qp "seriesFolderFormat" $seriesFolderFormat "scalar") (serialize-qp "includeSeriesTitle" $includeSeriesTitle "scalar") (serialize-qp "includeEpisodeTitle" $includeEpisodeTitle "scalar") (serialize-qp "includeQuality" $includeQuality "scalar") (serialize-qp "replaceSpaces" $replaceSpaces "scalar") (serialize-qp "separator" $separator "scalar") (serialize-qp "numberStyle" $numberStyle "scalar") (serialize-qp "id" $id "scalar") (serialize-qp "resourceName" $resourceName "scalar")] | flatten | str join "&"
...
export def "notification list" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
]: nothing -> table<id: int, name: string, fields: list<record>, implementationName: string, implementation: string, configContract: string, infoLink: string, message: record<message: string, type: string>, tags: list<int>, presets: list<any>, link: string, onGrab: bool, onDownload: bool, onUpgrade: bool, onRename: bool, onSeriesAdd: bool, onSeriesDelete: bool, onEpisodeFileDelete: bool, onEpisodeFileDeleteForUpgrade: bool, onHealthIssue: bool, onHealthRestored: bool, onApplicationUpdate: bool, onManualInteractionRequired: bool, supportsOnGrab: bool, supportsOnDownload: bool, supportsOnUpgrade: bool, supportsOnRename: bool, supportsOnSeriesAdd: bool, supportsOnSeriesDelete: bool, supportsOnEpisodeFileDelete: bool, supportsOnEpisodeFileDeleteForUpgrade: bool, supportsOnHealthIssue: bool, supportsOnHealthRestored: bool, supportsOnApplicationUpdate: bool, supportsOnManualInteractionRequired: bool, includeHealthWarnings: bool, testCommand: string> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
# --fields item shape: {order?: int, name?: string, label?: string, unit?: string, helpText?: string, helpTextWarning?: string, helpLink?: string, value?: any, type?: string, advanced?: bool, selectOptions?: list, selectOptionsProviderAction?: string, section?: string, hidden?: string, privacy?: "normal"|"password"|"apiKey"|"userName", placeholder?: string, isFloat?: bool}
# --message shape: {message?: string, type?: "info"|"warning"|"error"}
# --presets item shape: {id?: int, name?: string, fields?: list, implementationName?: string, implementation?: string, configContract?: string, infoLink?: string, message?: record, tags?: list, presets?: list, link?: string, onGrab?: bool, onDownload?: bool, onUpgrade?: bool, onRename?: bool, onSeriesAdd?: bool, onSeriesDelete?: bool, onEpisodeFileDelete?: bool, onEpisodeFileDeleteForUpgrade?: bool, onHealthIssue?: bool, onHealthRestored?: bool, onApplicationUpdate?: bool, onManualInteractionRequired?: bool, supportsOnGrab?: bool, supportsOnDownload?: bool, supportsOnUpgrade?: bool, supportsOnRename?: bool, supportsOnSeriesAdd?: bool, supportsOnSeriesDelete?: bool, supportsOnEpisodeFileDelete?: bool, supportsOnEpisodeFileDeleteForUpgrade?: bool, supportsOnHealthIssue?: bool, supportsOnHealthRestored?: bool, supportsOnApplicationUpdate?: bool, supportsOnManualInteractionRequired?: bool, includeHealthWarnings?: bool, testCommand?: string}
export def "notification post" [
  ...
  let body = {id: $id, name: $name, fields: $body_fields, implementationName: $implementationName, implementation: $implementation, configContract: $configContract, infoLink: $infoLink, message: $message, tags: $tags, presets: $presets, link: $link, onGrab: $onGrab, onDownload: $onDownload, onUpgrade: $onUpgrade, onRename: $onRename, onSeriesAdd: $onSeriesAdd, onSeriesDelete: $onSeriesDelete, onEpisodeFileDelete: $onEpisodeFileDelete, onEpisodeFileDeleteForUpgrade: $onEpisodeFileDeleteForUpgrade, onHealthIssue: $onHealthIssue, onHealthRestored: $onHealthRestored, onApplicationUpdate: $onApplicationUpdate, onManualInteractionRequired: $onManualInteractionRequired, supportsOnGrab: $supportsOnGrab, supportsOnDownload: $supportsOnDownload, supportsOnUpgrade: $supportsOnUpgrade, supportsOnRename: $supportsOnRename, supportsOnSeriesAdd: $supportsOnSeriesAdd, supportsOnSeriesDelete: $supportsOnSeriesDelete, supportsOnEpisodeFileDelete: $supportsOnEpisodeFileDelete, supportsOnEpisodeFileDeleteForUpgrade: $supportsOnEpisodeFileDeleteForUpgrade, supportsOnHealthIssue: $supportsOnHealthIssue, supportsOnHealthRestored: $supportsOnHealthRestored, supportsOnApplicationUpdate: $supportsOnApplicationUpdate, supportsOnManualInteractionRequired: $supportsOnManualInteractionRequired, includeHealthWarnings: $includeHealthWarnings, testCommand: $testCommand} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "notification-test post" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/notification/test")
  let body = {id: $id, name: $name, fields: $body_fields, implementationName: $implementationName, implementation: $implementation, configContract: $configContract, infoLink: $infoLink, message: $message, tags: $tags, presets: $presets, link: $link, onGrab: $onGrab, onDownload: $onDownload, onUpgrade: $onUpgrade, onRename: $onRename, onSeriesAdd: $onSeriesAdd, onSeriesDelete: $onSeriesDelete, onEpisodeFileDelete: $onEpisodeFileDelete, onEpisodeFileDeleteForUpgrade: $onEpisodeFileDeleteForUpgrade, onHealthIssue: $onHealthIssue, onHealthRestored: $onHealthRestored, onApplicationUpdate: $onApplicationUpdate, onManualInteractionRequired: $onManualInteractionRequired, supportsOnGrab: $supportsOnGrab, supportsOnDownload: $supportsOnDownload, supportsOnUpgrade: $supportsOnUpgrade, supportsOnRename: $supportsOnRename, supportsOnSeriesAdd: $supportsOnSeriesAdd, supportsOnSeriesDelete: $supportsOnSeriesDelete, supportsOnEpisodeFileDelete: $supportsOnEpisodeFileDelete, supportsOnEpisodeFileDeleteForUpgrade: $supportsOnEpisodeFileDeleteForUpgrade, supportsOnHealthIssue: $supportsOnHealthIssue, supportsOnHealthRestored: $supportsOnHealthRestored, supportsOnApplicationUpdate: $supportsOnApplicationUpdate, supportsOnManualInteractionRequired: $supportsOnManualInteractionRequired, includeHealthWarnings: $includeHealthWarnings, testCommand: $testCommand} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "parse get" [
  ...
  --title: string
  --path: string
]: nothing -> record<id: int, title: string, parsedEpisodeInfo: record<releaseTitle: string, seriesTitle: string, seriesTitleInfo: record<title: string, titleWithoutYear: string, year: int, allTitles: list>, quality: record<quality: record, revision: record>, airDate: string, languages: list<record>, releaseGroup: string, releaseHash: string, seasonPart: int, releaseTokens: string, isDaily: bool>, series: record<id: int, title: string, sortTitle: string, status: string, ended: bool, profileName: string, overview: string, nextAiring: string, previousAiring: string, network: string, images: list<record>, originalLanguage: record<id: int, name: string>, remotePoster: string, seasons: list<record>, year: int, path: string, qualityProfileId: int, monitored: bool, useSceneNumbering: bool, runtime: int, tvdbId: int, firstAired: string, cleanTitle: string, titleSlug: string, rootFolderPath: string, folder: string, certification: string, genres: list<string>, tags: list<int>, added: string, addOptions: record<ignoreEpisodesWithFiles: bool, ignoreEpisodesWithoutFiles: bool, episodesToMonitor: list, monitor: string, searchForMissingEpisodes: bool, searchForCutoffUnmetEpisodes: bool>, ratings: record<votes: int, value: float>, statistics: record<seasonCount: int, episodeFileCount: int, episodeCount: int, totalEpisodeCount: int, sizeOnDisk: int, releaseGroups: list, percentOfEpisodes: float>, episodesChanged: bool>, episodes: table<id: int, seriesId: int, tvdbId: int, episodeFileId: int, seasonNumber: int, title: string, releaseDate: record, lastSearchTime: string, runtime: int, overview: string, episodeFile: record, hasFile: bool, monitored: bool, absoluteEpisodeNumber: int, endTime: string, grabDate: string, seriesTitle: string, series: record, actors: list, images: list, grabbed: bool>, languages: table<id: int, name: string>, customFormats: table<id: int, name: string, includeCustomFormatWhenRenaming: bool, specifications: list>, customFormatScore: int> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
#
# --quality shape: {id?: int, name?: string, source?: "unknown"|"television"|"televisionRaw"|"web"|"webRip"|"dvd"|"bluray"|"vr"|"blurayRaw", resolution?: int}
export def "qualitydefinition put" [
  ...
  --accept: string@accept-completer # Response content type
  --body-id: int # format: int32
  --quality: record # shape: {id?: int, name?: string, source?: "unknown"|"television"|"televisionRaw"|"web"|"webRip"|"dvd"|"bluray"|"vr"|"blurayRaw", resolution?: int}
  --title: string # nullable
...
export def "queue-details get" [
  ...
  --includeSeries: string@bool-completer # default: false
  --includeEpisode: string@bool-completer # default: false
]: nothing -> table<id: int, seriesId: int, episodeId: int, seasonNumber: int, series: record<id: int, title: string, sortTitle: string, status: string, ended: bool, profileName: string, overview: string, nextAiring: string, previousAiring: string, network: string, images: list, originalLanguage: record, remotePoster: string, seasons: list, year: int, path: string, qualityProfileId: int, monitored: bool, useSceneNumbering: bool, runtime: int, tvdbId: int, firstAired: string, cleanTitle: string, titleSlug: string, rootFolderPath: string, folder: string, certification: string, genres: list, tags: list, added: string, addOptions: record, ratings: record, statistics: record, episodesChanged: bool>, episode: record<id: int, seriesId: int, tvdbId: int, episodeFileId: int, seasonNumber: int, title: string, releaseDate: record, lastSearchTime: string, runtime: int, overview: string, episodeFile: record, hasFile: bool, monitored: bool, absoluteEpisodeNumber: int, endTime: string, grabDate: string, seriesTitle: string, series: record, actors: list, images: list, grabbed: bool>, languages: list<record>, quality: record<quality: record, revision: record>, customFormats: list<record>, customFormatScore: int, size: float, title: string, sizeleft: float, timeleft: record<ticks: int, days: int, hours: int, milliseconds: int, minutes: int, seconds: int, totalDays: float, totalHours: float, totalMilliseconds: float, totalMinutes: float, totalSeconds: float>, estimatedCompletionTime: string, status: string, trackedDownloadStatus: string, trackedDownloadState: string, statusMessages: list<record>, errorMessage: string, downloadId: string, protocol: string, downloadClient: string, downloadClientHasPostImportCategory: bool, indexer: string, outputPath: string, episodeHasFile: bool> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
export def "release get" [
  ...
  --episodeId: int # format: int32
  --seasonNumber: int # format: int32
]: nothing -> table<id: int, guid: string, quality: record<quality: record, revision: record>, qualityWeight: int, age: int, ageHours: float, ageMinutes: float, size: int, indexerId: int, indexer: string, releaseGroup: string, subGroup: string, releaseHash: string, title: string, sceneSource: bool, languages: list<record>, languageWeight: int, airDate: string, seriesTitle: string, mappedSeriesId: int, mappedEpisodeInfo: list<record>, approved: bool, temporarilyRejected: bool, rejected: bool, tvdbId: int, rejections: list<string>, publishDate: string, commentUrl: string, downloadUrl: string, infoUrl: string, episodeRequested: bool, downloadAllowed: bool, releaseWeight: int, customFormats: list<record>, customFormatScore: int, magnetUrl: string, infoHash: string, seeders: int, leechers: int, protocol: string, isDaily: bool, seriesId: int, episodeId: int, episodeIds: list<int>, downloadClientId: int, shouldOverride: bool> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
export def "rename get" [
  ...
  --seriesId: int # format: int32
  --seasonNumber: int # format: int32
]: nothing -> table<id: int, seriesId: int, seasonNumber: int, releaseDates: list<string>, episodeFileId: int, existingPath: string, newPath: string> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
# --monitoringOptions shape: {ignoreEpisodesWithFiles?: bool, ignoreEpisodesWithoutFiles?: bool, episodesToMonitor?: list, monitor?: "unknown"|"all"|"future"|"missing"|"existing"|"firstSeason"|"latestSeason"|"none"}
export def "seasonpass post" [
  ...
  --monitoringOptions: record # shape: {ignoreEpisodesWithFiles?: bool, ignoreEpisodesWithoutFiles?: bool, episodesToMonitor?: list, monitor?: "unknown"|"all"|"future"|"missing"|"existing"|"firstSeason"|"latestSeason"|"none"}
]: any -> any {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
]: nothing -> table<id: int, title: string, sortTitle: string, status: string, ended: bool, profileName: string, overview: string, nextAiring: string, previousAiring: string, network: string, images: list<record>, originalLanguage: record<id: int, name: string>, remotePoster: string, seasons: list<record>, year: int, path: string, qualityProfileId: int, monitored: bool, useSceneNumbering: bool, runtime: int, tvdbId: int, firstAired: string, cleanTitle: string, titleSlug: string, rootFolderPath: string, folder: string, certification: string, genres: list<string>, tags: list<int>, added: string, addOptions: record<ignoreEpisodesWithFiles: bool, ignoreEpisodesWithoutFiles: bool, episodesToMonitor: list, monitor: string, searchForMissingEpisodes: bool, searchForCutoffUnmetEpisodes: bool>, ratings: record<votes: int, value: float>, statistics: record<seasonCount: int, episodeFileCount: int, episodeCount: int, totalEpisodeCount: int, sizeOnDisk: int, releaseGroups: list, percentOfEpisodes: float>, episodesChanged: bool> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
# --addOptions shape: {ignoreEpisodesWithFiles?: bool, ignoreEpisodesWithoutFiles?: bool, episodesToMonitor?: list, monitor?: "unknown"|"all"|"future"|"missing"|"existing"|"firstSeason"|"latestSeason"|"none", searchForMissingEpisodes?: bool, searchForCutoffUnmetEpisodes?: bool}
...
export def "series post" [
  ...
  --addOptions: record # shape: {ignoreEpisodesWithFiles?: bool, ignoreEpisodesWithoutFiles?: bool, episodesToMonitor?: list, monitor?: "unknown"|"all"|"future"|"missing"|"existing"|"firstSeason"|"latestSeason"|"none", searchForMissingEpisodes?: bool, searchForCutoffUnmetEpisodes?: bool}
...
]: any -> record<id: int, title: string, sortTitle: string, status: string, ended: bool, profileName: string, overview: string, nextAiring: string, previousAiring: string, network: string, images: table<coverType: string, url: string, remoteUrl: string>, originalLanguage: record<id: int, name: string>, remotePoster: string, seasons: table<seasonNumber: int, monitored: bool, statistics: record, images: list>, year: int, path: string, qualityProfileId: int, monitored: bool, useSceneNumbering: bool, runtime: int, tvdbId: int, firstAired: string, cleanTitle: string, titleSlug: string, rootFolderPath: string, folder: string, certification: string, genres: list<string>, tags: list<int>, added: string, addOptions: record<ignoreEpisodesWithFiles: bool, ignoreEpisodesWithoutFiles: bool, episodesToMonitor: list<int>, monitor: string, searchForMissingEpisodes: bool, searchForCutoffUnmetEpisodes: bool>, ratings: record<votes: int, value: float>, statistics: record<seasonCount: int, episodeFileCount: int, episodeCount: int, totalEpisodeCount: int, sizeOnDisk: int, releaseGroups: list<string>, percentOfEpisodes: float>, episodesChanged: bool> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/series")
  let body = {id: $id, title: $title, sortTitle: $sortTitle, status: $status, profileName: $profileName, overview: $overview, nextAiring: $nextAiring, previousAiring: $previousAiring, network: $network, images: $images, originalLanguage: $originalLanguage, remotePoster: $remotePoster, seasons: $seasons, year: $year, path: $path, qualityProfileId: $qualityProfileId, monitored: $monitored, useSceneNumbering: $useSceneNumbering, runtime: $runtime, tvdbId: $tvdbId, firstAired: $firstAired, cleanTitle: $cleanTitle, titleSlug: $titleSlug, rootFolderPath: $rootFolderPath, folder: $folder, certification: $certification, genres: $genres, tags: $tags, added: $added, addOptions: $addOptions, ratings: $ratings, statistics: $statistics, episodesChanged: $episodesChanged} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "series get" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --includeSeasonImages: string@bool-completer # default: false
]: nothing -> record<id: int, title: string, sortTitle: string, status: string, ended: bool, profileName: string, overview: string, nextAiring: string, previousAiring: string, network: string, images: table<coverType: string, url: string, remoteUrl: string>, originalLanguage: record<id: int, name: string>, remotePoster: string, seasons: table<seasonNumber: int, monitored: bool, statistics: record, images: list>, year: int, path: string, qualityProfileId: int, monitored: bool, useSceneNumbering: bool, runtime: int, tvdbId: int, firstAired: string, cleanTitle: string, titleSlug: string, rootFolderPath: string, folder: string, certification: string, genres: list<string>, tags: list<int>, added: string, addOptions: record<ignoreEpisodesWithFiles: bool, ignoreEpisodesWithoutFiles: bool, episodesToMonitor: list<int>, monitor: string, searchForMissingEpisodes: bool, searchForCutoffUnmetEpisodes: bool>, ratings: record<votes: int, value: float>, statistics: record<seasonCount: int, episodeFileCount: int, episodeCount: int, totalEpisodeCount: int, sizeOnDisk: int, releaseGroups: list<string>, percentOfEpisodes: float>, episodesChanged: bool> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
export def "series put" [
  ...
  let full_url = (build-url $base $"/api/v3/series/($id)" $qp)
  let body = {id: $body_id, title: $title, sortTitle: $sortTitle, status: $status, profileName: $profileName, overview: $overview, nextAiring: $nextAiring, previousAiring: $previousAiring, network: $network, images: $images, originalLanguage: $originalLanguage, remotePoster: $remotePoster, seasons: $seasons, year: $year, path: $path, qualityProfileId: $qualityProfileId, monitored: $monitored, useSceneNumbering: $useSceneNumbering, runtime: $runtime, tvdbId: $tvdbId, firstAired: $firstAired, cleanTitle: $cleanTitle, titleSlug: $titleSlug, rootFolderPath: $rootFolderPath, folder: $folder, certification: $certification, genres: $genres, tags: $tags, added: $added, addOptions: $addOptions, ratings: $ratings, statistics: $statistics, episodesChanged: $episodesChanged} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "series-editor put" [
  ...
  let body = {seriesIds: $seriesIds, monitored: $monitored, qualityProfileId: $qualityProfileId, rootFolderPath: $rootFolderPath, tags: $tags, applyTags: $applyTags, moveFiles: $moveFiles, deleteFiles: $deleteFiles, addImportListExclusion: $addImportListExclusion} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "system-status get" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
]: nothing -> record<appName: string, instanceName: string, version: string, buildTime: string, isDebug: bool, isProduction: bool, isAdmin: bool, isUserInteractive: bool, startupPath: string, appData: string, osName: string, osVersion: string, isNetCore: bool, isLinux: bool, isOsx: bool, isWindows: bool, isDocker: bool, mode: string, branch: string, authentication: string, sqliteVersion: record<major: int, minor: int, build: int, revision: int, majorRevision: int, minorRevision: int>, migrationVersion: int, urlBase: string, runtimeVersion: record<major: int, minor: int, build: int, revision: int, majorRevision: int, minorRevision: int>, runtimeName: string, startTime: string, packageVersion: string, packageAuthor: string, packageUpdateMechanism: string, packageUpdateMechanismMessage: string, databaseVersion: record<major: int, minor: int, build: int, revision: int, majorRevision: int, minorRevision: int>, databaseType: string> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
export def "system-task list" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --accept: string@accept-completer # Response content type
]: nothing -> table<id: int, name: string, taskName: string, interval: int, lastExecution: string, lastStartTime: string, nextExecution: string, lastDuration: record<ticks: int, days: int, hours: int, milliseconds: int, minutes: int, seconds: int, totalDays: float, totalHours: float, totalMilliseconds: float, totalMinutes: float, totalSeconds: float>> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
export def "system-task get" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
]: nothing -> record<id: int, name: string, taskName: string, interval: int, lastExecution: string, lastStartTime: string, nextExecution: string, lastDuration: record<ticks: int, days: int, hours: int, milliseconds: int, minutes: int, seconds: int, totalDays: float, totalHours: float, totalMilliseconds: float, totalMinutes: float, totalSeconds: float>> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
export def "update get" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
]: nothing -> table<id: int, version: record<major: int, minor: int, build: int, revision: int, majorRevision: int, minorRevision: int>, branch: string, releaseDate: string, fileName: string, url: string, installed: bool, installedOn: string, installable: bool, latest: bool, changes: record<new: list, fixed: list>, hash: string> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
