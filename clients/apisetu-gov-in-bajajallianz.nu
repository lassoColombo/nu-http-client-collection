# Auto-generated client for Bajaj Allianz General Insurance Company Ltd. (BAGIC) v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/bajajallianz/3.0.0/openapi.json
# Auth: --token flag or $env.BAJAJ_ALLIANZ_GENERAL_INSURANCE_COMPANY_LTD_BAGIC_TOKEN

const BASE_URL = "https://apisetu.gov.in/bajajallianz/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o BAJAJ_ALLIANZ_GENERAL_INSURANCE_COMPANY_LTD_BAGIC_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/bajajallianz/v3"] }
...
# operationId: cripc
# --certificateParameters shape: {FullName: string, policy_number: string}
...
export def "cripc-certificate create" [
  ...
  --certificate-parameters: record # shape: {FullName: string, policy_number: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o BAJAJ_ALLIANZ_GENERAL_INSURANCE_COMPANY_LTD_BAGIC_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o BAJAJ_ALLIANZ_GENERAL_INSURANCE_COMPANY_LTD_BAGIC_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
...
# --certificateParameters shape: {FullName: string, policy_number: string}
# --consentArtifact shape: {consent: record, signature: record}
export def "cyipc-certificate create" [
  --base-url(-b): string@base-url-completer # API base URL
  --token-apikey: string # Auth token for apiKey (X-APISETU-APIKEY)
  --token-clientid: string # Auth token for clientId (X-APISETU-CLIENTID)
  --insecure(-k) # Skip TLS verification
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o BAJAJ_ALLIANZ_GENERAL_INSURANCE_COMPANY_LTD_BAGIC_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o BAJAJ_ALLIANZ_GENERAL_INSURANCE_COMPANY_LTD_BAGIC_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/cyipc/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
