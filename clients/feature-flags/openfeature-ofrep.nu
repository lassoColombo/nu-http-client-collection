# Auto-generated client for OpenFeature Remote Evaluation Protocol (OFREP) v0.3.0
# Source: https://raw.githubusercontent.com/open-feature/protocol/main/service/openapi.yaml
# Auth: --token flag or $env.OPENFEATURE_REMOTE_EVALUATION_PROTOCOL_OFREP_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o OPENFEATURE_REMOTE_EVALUATION_PROTOCOL_OFREP_TOKEN | default "" }
...
# --context shape: {targetingKey: string}
export def "ofrep-evaluate-flags evaluateFlag" [
  key: string
...
# --context shape: {targetingKey: string}
export def "ofrep-evaluate-flags evaluateFlagsBulk" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  context: record # Evaluation context containing information used to evaluate feature flags. The context  includes a `targetingKey` (required) along with additional properties such as  user attributes, session data, or request metadata that can be used for targeting rules.  (e.g. {targetingKey: user-123, email: user@example.com, plan: premium, country: CA}) — shape: {targetingKey: string}
]: any -> record<flags: list<any>, metadata: record, eventStreams: table<type: string, url: string, endpoint: record, inactivityDelaySec: int>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "flagConfigEtag" $flagConfigEtag "scalar") (serialize-qp "flagConfigLastModified" $flagConfigLastModified "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/ofrep/v1/evaluate/flags" $qp)
  let body = {context: $context} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
