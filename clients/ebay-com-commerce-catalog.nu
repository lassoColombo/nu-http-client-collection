...
export def "product get" [
  ...
  if ($epid | is-empty) { error make --unspanned { msg: "path parameter 'epid' must be non-empty" } }
  let full_url = (build-url $base ({epid: (encode-path-segment $epid)} | format pattern "/product/{epid}") $auth.query)
...
export def "product-summary-search list" [
  ...
  let full_url = (build-url $base "/product_summary/search" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"X-EBAY-C-MARKETPLACE-ID": $x_ebay_c_marketplace_id} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "get"
    url: $full_url
    query: ({"aspect_filter": $aspect_filter, "category_ids": $category_ids, "fieldgroups": $fieldgroups, "gtin": $gtin, "limit": $limit, "mpn": $mpn, "offset": $offset, "q": $q} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
