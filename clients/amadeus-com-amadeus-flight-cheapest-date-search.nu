# Auto-generated client for Flight Cheapest Date Search v1.0.6
# Source: https://api.apis.guru/v2/specs/amadeus.com/amadeus-flight-cheapest-date-search/1.0.6/swagger.json
# Auth: --token flag or $env.FLIGHT_CHEAPEST_DATE_SEARCH_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o FLIGHT_CHEAPEST_DATE_SEARCH_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "shopping-flight-dates get" } } | get name | first)
...
export def "shopping-flight-dates get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"origin": $origin, "destination": $destination, "departureDate": $departure_date, "oneWay": $one_way, "duration": $duration, "nonStop": $non_stop, "maxPrice": $max_price, "viewBy": $view_by} | compact), body: null}
}
