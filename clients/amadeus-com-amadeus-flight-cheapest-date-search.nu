...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o FLIGHT_CHEAPEST_DATE_SEARCH_TOKEN | default "" }
...
export def "shopping-flight-dates get" [
  ...
  let full_url = (build-url $base "/shopping/flight-dates" $qp $auth.query)
  let accept_val = ($accept | default "application/vnd.amadeus+json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"origin": $origin, "destination": $destination, "departureDate": $departure_date, "oneWay": $one_way, "duration": $duration, "nonStop": $non_stop, "maxPrice": $max_price, "viewBy": $view_by} | compact)
