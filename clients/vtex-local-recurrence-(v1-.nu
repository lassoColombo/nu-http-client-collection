...
export def "subscriptions get-recurrencebyemail" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"email": $email, "cycleStatus": $cycle_status} | compact), body: null}
}
...
export def "subscriptions update-updatepartialrecurrence" [
  ...
  if ($recurrence_id | is-empty) { error make --unspanned { msg: "path parameter 'recurrenceId' must be non-empty" } }
  let full_url = (build-url $base ({recurrence_id: (encode-path-segment $recurrence_id)} | format pattern "/subscriptions/{recurrence_id}"))
  let req_body = {"deliveryDay": $delivery_day, "deliveryWeekday": $delivery_weekday, "status": $status} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Content-Type": $content_type, "Accept": $hdr_accept} | compact
