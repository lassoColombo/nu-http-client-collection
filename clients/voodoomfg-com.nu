...
export def "model-quote get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"model_id": $model_id, "material_id": $material_id, "quantity": $quantity, "units": $units, "options[orientation]": $options_orientation} | compact), body: null}
}
...
export def "model-quote-attrs get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"x": $x, "y": $y, "z": $z, "volume": $volume, "surface_area": $surface_area, "material_id": $material_id, "quantity": $quantity, "units": $units, "options[orientation]": $options_orientation} | compact), body: null}
}
