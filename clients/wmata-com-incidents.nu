# Auto-generated client for Incidents v1.0
# Source: https://api.apis.guru/v2/specs/wmata.com/incidents/1.0/swagger.json
# Auth: --token flag or $env.INCIDENTS_TOKEN

const BASE_URL = "http://api.wmata.com/Incidents.svc"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o INCIDENTS_TOKEN | default "" }
...
def base-url-completer [] { ["http://api.wmata.com/Incidents.svc" "https://api.wmata.com/Incidents.svc"] }
...
export def "bus-incidents get-54763641281d830c946a3d78" [
  ...
  let full_url = (build-url $base "/BusIncidents" $qp $auth.query)
  let accept_val = "application/xml"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"Route": $route} | compact)
...
export def "elevator-incidents get-54763641281d830c946a3d79" [
  ...
  let full_url = (build-url $base "/ElevatorIncidents" $qp $auth.query)
  let accept_val = "application/xml"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "incidents get-54763641281d830c946a3d7a" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "api_key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/Incidents" $auth.query)
  let accept_val = "application/xml"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "json-bus-incidents get-54763641281d830c946a3d75" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "Route" $route "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/json/BusIncidents" $qp $auth.query)
...
export def "json-elevator-incidents get-54763641281d830c946a3d76" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "StationCode" $station_code "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/json/ElevatorIncidents" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "json-incidents get-54763641281d830c946a3d77" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "api_key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/json/Incidents" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
