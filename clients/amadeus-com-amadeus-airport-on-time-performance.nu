# Auto-generated client for Airport On-Time Performance v1.0.4
# Source: https://api.apis.guru/v2/specs/amadeus.com/amadeus-airport-on-time-performance/1.0.4/swagger.json
# Auth: --token flag or $env.AIRPORT_ON_TIME_PERFORMANCE_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o AIRPORT_ON_TIME_PERFORMANCE_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "airport-predictions-on-time get" } } | get name | first)
...
export def "airport-predictions-on-time get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"airportCode": $airport_code, "date": $date} | compact), body: null}
}
