...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AIRPORT_ON_TIME_PERFORMANCE_TOKEN | default "" }
...
export def "airport-predictions-on-time get" [
  ...
  let full_url = (build-url $base "/airport/predictions/on-time" $qp $auth.query)
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"airportCode": $airport_code, "date": $date} | compact)
