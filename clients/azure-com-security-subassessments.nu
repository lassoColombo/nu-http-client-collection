...
export def "providers-microsoft-security-assessments-sub-assessments list" [
  ...
  let full_url = (build-url $base ({scope: (encode-path-segment $scope), assessment_name: (encode-path-segment $assessment_name)} | format pattern "/{scope}/providers/Microsoft.Security/assessments/{assessment_name}/subAssessments") $qp)
...
export def "providers-microsoft-security-assessments-sub-assessments get" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({scope: (encode-path-segment $scope), assessment_name: (encode-path-segment $assessment_name), sub_assessment_name: (encode-path-segment $sub_assessment_name)} | format pattern "/{scope}/providers/Microsoft.Security/assessments/{assessment_name}/subAssessments/{sub_assessment_name}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version} | compact), body: null}
}
...
export def "providers-microsoft-security-sub-assessments list" [
  ...
  if ($scope | is-empty) { error make --unspanned { msg: "path parameter 'scope' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({scope: (encode-path-segment $scope)} | format pattern "/{scope}/providers/Microsoft.Security/subAssessments") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version} | compact), body: null}
}
