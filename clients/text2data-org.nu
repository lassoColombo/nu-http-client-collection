...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o TEXT_ANALYTICS_SENTIMENT_ANALYSIS_API_API_TEXT2DATA_COM_TOKEN | default "" }
  let scheme = ($auth_scheme | default "bearer")
  if ($scheme == "none") or ($token_val | is-empty) { return {scheme: $scheme, headers: {}, query: "", location: "none"} }
  match $scheme {
    "query-PrivateKey" => { {scheme: $scheme, headers: {}, query: $"(encode-path-segment "PrivateKey")=(encode-path-segment $token_val)", location: "query"} }
...
export def "analyze get" [
  ...
  let full_url = (build-url $base "/v3/Analyze" $auth.query)
...
export def "categorize get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "query-PrivateKey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/v3/Categorize" $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "extract get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "query-PrivateKey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/v3/Extract" $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
