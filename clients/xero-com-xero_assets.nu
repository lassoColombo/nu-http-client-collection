...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o XERO_ASSETS_API_TOKEN | default "" }
...
export def "asset-types get" [
  ...
  let full_url = (build-url $base "/AssetTypes" $auth.query)
...
export def "assets list" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"status": $status, "page": $page, "pageSize": $page_size, "orderBy": $order_by, "sortDirection": $sort_direction, "filterBy": $filter_by} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
