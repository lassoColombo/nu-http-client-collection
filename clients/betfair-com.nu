# Auto-generated client for Betfair: Exchange Streaming API v1.0.1423
# Source: https://api.apis.guru/v2/specs/betfair.com/1.0.1423/openapi.json
# Auth: --token flag or $env.BETFAIR_EXCHANGE_STREAMING_API_TOKEN

const BASE_URL = "http://stream-api.betfair.com:443/api"
const DEFAULT_AUTH = "bearer"

# Build auth: returns {headers: record, query: string}
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o BETFAIR_EXCHANGE_STREAMING_API_TOKEN | default "" }
...
def base-url-completer [] { ["http://stream-api.betfair.com:443/api"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "request create" } } | get name | first)
...
export def "request create" [
  ...
  let req_body = {"authentication": $authentication, "heartbeat": $heartbeat, "marketSubscription": $market_subscription, "opTypes": $op_types, "orderSubscriptionMessage": $order_subscription_message} | compact
