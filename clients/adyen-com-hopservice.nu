...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o HOSTED_ONBOARDING_API_TOKEN | default "" }
...
export def "get-onboarding-url create" [
  ...
  let full_url = (build-url $base "/getOnboardingUrl" $auth.query)
...
export def "get-pci-questionnaire-url create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/getPciQuestionnaireUrl" $auth.query)
