...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AMAZON_SAGEMAKER_EDGE_MANAGER_TOKEN | default "" }
...
export def "get-deployments get" [
  ...
  let full_url = (build-url $base "/GetDeployments" $auth.query)
...
export def "get-device-registration get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/GetDeviceRegistration" $auth.query)
  let req_body = {"DeviceName": $device_name, "DeviceFleetName": $device_fleet_name} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "send-heartbeat send" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/SendHeartbeat" $auth.query)
