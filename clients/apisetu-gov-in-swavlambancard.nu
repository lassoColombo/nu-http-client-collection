# Auto-generated client for Department of Empowerment of Persons with Disabilities v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/swavlambancard/3.0.0/openapi.json
# Auth: --token flag or $env.DEPARTMENT_OF_EMPOWERMENT_OF_PERSONS_WITH_DISABILITIES_TOKEN

const BASE_URL = "https://apisetu.gov.in/swavlambancard/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o DEPARTMENT_OF_EMPOWERMENT_OF_PERSONS_WITH_DISABILITIES_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/swavlambancard/v3"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "dpicr-certificate create" } } | get name | first)
...
# operationId: dpicr
# --certificateParameters shape: {CERT_NO: string, FullName: string}
...
export def "dpicr-certificate create" [
  ...
  --certificate-parameters: record # shape: {CERT_NO: string, FullName: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o DEPARTMENT_OF_EMPOWERMENT_OF_PERSONS_WITH_DISABILITIES_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o DEPARTMENT_OF_EMPOWERMENT_OF_PERSONS_WITH_DISABILITIES_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
