...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o MERCHANT_IDENTIFIER_API_V2_TOKEN | default "" }
...
export def "merchant-ids get-identifiers" [
  ...
  let full_url = (build-url $base "/merchant-ids" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"merchant_id": $merchant_id, "type": $type} | compact)
