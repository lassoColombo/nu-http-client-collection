# Auto-generated client for Merchant Identifier API V2 v2.0.0
# Source: https://api.apis.guru/v2/specs/mastercard.com/MerchantIdentifier/2.0.0/swagger.json
# Auth: --token flag or $env.MERCHANT_IDENTIFIER_API_V2_TOKEN

const BASE_URL = "https://api.mastercard.com/merchant-id/v2"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o MERCHANT_IDENTIFIER_API_V2_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.mastercard.com/merchant-id/v2"] }
...
export def "merchant-ids get-identifiers" [
  ...
  let qp = [(serialize-qp "merchant_id" $merchant_id "scalar") (serialize-qp "type" $type "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/merchant-ids" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"merchant_id": $merchant_id, "type": $type} | compact), body: null}
}
