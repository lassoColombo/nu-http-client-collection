...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o FOOD_CIVIL_SUPPLIES_AND_CONSUMER_AFFAIRS_DEPARTMENT_KARNATAKA_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/aharakar/v3"] }
...
# operationId: ratcr
# --certificateParameters shape: {FullName: string, UID: string, rationCardNo: string}
...
export def "ratcr-certificate create" [
  ...
  --certificate-parameters: record # shape: {FullName: string, UID: string, rationCardNo: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o FOOD_CIVIL_SUPPLIES_AND_CONSUMER_AFFAIRS_DEPARTMENT_KARNATAKA_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o FOOD_CIVIL_SUPPLIES_AND_CONSUMER_AFFAIRS_DEPARTMENT_KARNATAKA_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
