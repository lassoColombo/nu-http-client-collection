# Auto-generated client for VTEX Do API v1.0
# Source: https://api.apis.guru/v2/specs/vtex.local/VTEX-Do-API/1.0/openapi.json
# Auth: --token flag or $env.VTEX_DO_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o VTEX_DO_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://vtex.local" "https://{accountName}.{environment}.com.br/api/do"] }
...
export def "notes get-notesbyorder" [
  ...
  let full_url = (build-url $base "/notes" $qp $auth.query)
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"target.id": $target_id, "perPage": $per_page, "page": $page, "reason": $reason} | compact)
...
export def "notes create-new" [
  ...
  let auth = (merge-auth [(build-auth ($token_appkey | default ($env | get -o VTEX_DO_API_APPKEY_TOKEN | default "")) "x-vtex-api-appkey") (build-auth ($token_apptoken | default ($env | get -o VTEX_DO_API_APPTOKEN_TOKEN | default "")) "x-vtex-api-apptoken")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/notes" $auth.query)
...
export def "notes get" [
  ...
  let full_url = (build-url $base ({note_id: (encode-path-segment $note_id)} | format pattern "/notes/{note_id}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Accept": $hdr_accept, "Content-Type": $content_type} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "tasks list-tasksbyassignee" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"assignee.email": $assignee_email, "target.id": $target_id, "context": $context, "page": $page, "perPage": $per_page, "status": $status} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "tasks-comments create" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($task_id | is-empty) { error make --unspanned { msg: "path parameter 'taskId' must be non-empty" } }
  let full_url = (build-url $base ({task_id: (encode-path-segment $task_id)} | format pattern "/tasks/{task_id}/comments") $auth.query)
