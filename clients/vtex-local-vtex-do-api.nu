...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o VTEX_DO_API_TOKEN | default "" }
...
export def "notes get-notesbyorder" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"target.id": $target_id, "perPage": $per_page, "page": $page, "reason": $reason} | compact), body: null}
}
...
export def "notes get" [
  ...
  let full_url = (build-url $base ({note_id: (encode-path-segment $note_id)} | format pattern "/notes/{note_id}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Accept": $hdr_accept, "Content-Type": $content_type} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "tasks get-listtasksbyassignee" [
  ...
  let extra_headers = {"Accept": $hdr_accept, "Content-Type": $content_type} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"assignee.email": $assignee_email, "target.id": $target_id, "context": $context, "page": $page, "perPage": $per_page, "status": $status} | compact), body: null}
}
