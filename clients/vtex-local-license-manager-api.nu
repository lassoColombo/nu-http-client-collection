...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o LICENSE_MANAGER_API_TOKEN | default "" }
...
export def "license-manager-site-pvt-logins-list-paged get-users" [
  ...
  let full_url = (build-url $base "/api/license-manager/site/pvt/logins/list/paged" $qp $auth.query)
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"numItems": $num_items, "pageNumber": $page_number, "sort": $qp_sort, "sortType": $sort_type} | compact)
...
export def "license-manager-site-pvt-roles-list-paged get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "numItems" $num_items "scalar") (serialize-qp "pageNumber" $page_number "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "sortType" $sort_type "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/api/license-manager/site/pvt/roles/list/paged" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Content-Type": $content_type} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "license-manager-users create" [
  ...
  let auth = (merge-auth [(build-auth ($token_appkey | default ($env | get -o LICENSE_MANAGER_API_APPKEY_TOKEN | default "")) "x-vtex-api-appkey") (build-auth ($token_apptoken | default ($env | get -o LICENSE_MANAGER_API_APPTOKEN_TOKEN | default "")) "x-vtex-api-apptoken")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/license-manager/users" $auth.query)
...
export def "license-manager-users get" [
  ...
  let full_url = (build-url $base ({user_id: (encode-path-segment $user_id)} | format pattern "/api/license-manager/users/{user_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Content-Type": $content_type} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "license-manager-users-roles get-rolesby" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($user_id | is-empty) { error make --unspanned { msg: "path parameter 'userId' must be non-empty" } }
  let full_url = (build-url $base ({user_id: (encode-path-segment $user_id)} | format pattern "/api/license-manager/users/{user_id}/roles") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Content-Type": $content_type} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "license-manager-users-roles delete-rolefrom" [
  ...
  let full_url = (build-url $base ({user_id: (encode-path-segment $user_id), role_id: (encode-path-segment $role_id)} | format pattern "/api/license-manager/users/{user_id}/roles/{role_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Content-Type": $content_type} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "vlm-account get" [
  ...
  let auth = (merge-auth [(build-auth ($token_appkey | default ($env | get -o LICENSE_MANAGER_API_APPKEY_TOKEN | default "")) "x-vtex-api-appkey") (build-auth ($token_apptoken | default ($env | get -o LICENSE_MANAGER_API_APPTOKEN_TOKEN | default "")) "x-vtex-api-apptoken")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/vlm/account" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "vlm-account-stores get" [
  ...
  let auth = (merge-auth [(build-auth ($token_appkey | default ($env | get -o LICENSE_MANAGER_API_APPKEY_TOKEN | default "")) "x-vtex-api-appkey") (build-auth ($token_apptoken | default ($env | get -o LICENSE_MANAGER_API_APPTOKEN_TOKEN | default "")) "x-vtex-api-apptoken")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/vlm/account/stores" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "vlm-appkeys get-appkeysfromaccount" [
  ...
  let auth = (merge-auth [(build-auth ($token_appkey | default ($env | get -o LICENSE_MANAGER_API_APPKEY_TOKEN | default "")) "x-vtex-api-appkey") (build-auth ($token_apptoken | default ($env | get -o LICENSE_MANAGER_API_APPTOKEN_TOKEN | default "")) "x-vtex-api-apptoken")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/vlm/appkeys" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Content-Type": $content_type} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "vlm-appkeys update" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/api/vlm/appkeys/{id}") $auth.query)
