# Auto-generated client for ApplicationInsightsManagementClient v2019-10-17-preview
# Source: https://api.apis.guru/v2/specs/azure.com/applicationinsights-workbookTemplates_API/2019-10-17-preview/swagger.json
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "subscriptions-resource-group-providers-microsoftinsights-workbooktemplates ListByResourceGroup" } } | get name | first)
...
export def "subscriptions-resource-group-providers-microsoftinsights-workbooktemplates ListByResourceGroup" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroup/($resourceGroupName)/providers/microsoft.insights/workbooktemplates" $qp)
...
export def "subscriptions-resource-group-providers-microsoftinsights-workbooktemplates Delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroup/($resourceGroupName)/providers/microsoft.insights/workbooktemplates/($resourceName)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "subscriptions-resource-group-providers-microsoftinsights-workbooktemplates Get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # The API version to use for this operation.
]: nothing -> record<properties: record<author: string, galleries: list<record>, localized: record, priority: int, templateData: record>, id: string, location: string, name: string, tags: record, type: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroup/($resourceGroupName)/providers/microsoft.insights/workbooktemplates/($resourceName)" $qp)
...
export def "subscriptions-resource-group-providers-microsoftinsights-workbooktemplates Update" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # The API version to use for this operation.
  --properties: any # Properties that contain a workbook template. — shape: {author?: string, galleries: list, localized?: record, priority?: int, templateData: record}
  --tags: record # Resource tags
]: any -> record<properties: record<author: string, galleries: list<record>, localized: record, priority: int, templateData: record>, id: string, location: string, name: string, tags: record, type: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
...
# --properties shape: {author?: string, galleries: list, localized?: record, priority?: int, templateData: record}
export def "subscriptions-resource-group-providers-microsoftinsights-workbooktemplates CreateOrUpdate" [
  subscriptionId: string
  resourceGroupName: string
  resourceName: string
  --base-url(-b): string@base-url-completer # API base URL
