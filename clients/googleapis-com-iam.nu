...
def base-url-completer [] { ["https://iam.googleapis.com"] }
...
export def "policies delete" [
  ...
  let auth = (merge-auth [(build-auth ($token_oauth2 | default ($env | get -o IDENTITY_AND_ACCESS_MANAGEMENT_IAM_API_OAUTH2_TOKEN | default "")) "bearer") (build-auth ($token_oauth2c | default ($env | get -o IDENTITY_AND_ACCESS_MANAGEMENT_IAM_API_OAUTH2C_TOKEN | default "")) "bearer")])
...
export def "policies update" [
  ...
  --display-name: string # A user-specified description of the `Policy`. This value can be up to 63 characters.
  --etag: string # An opaque tag that identifies the current version of the `Policy`. IAM uses this value to help manage concurrent updates, so they do not cause one update to be overwritten by another. If this field is present in a CreatePolicy request, the value is ignored.
  --body-name: string # Immutable. The resource name of the `Policy`, which must be unique. Format: `policies/{attachment_point}/denypolicies/{policy_id}` The attachment point is identified by its URL-encoded full resource name, which means that the forward-slash character, `/`, must be written as `%2F`. For example, `policies/cloudresourcemanager.googleapis.com%2Fprojects%2Fmy-project/denypolicies/my-deny-policy`. For organizations and folders, use the numeric ID in the full resource name. For projects, requests can use the alphanumeric or the numeric ID. Responses always contain the numeric ID.
  --rules: list # A list of rules that specify the behavior of the `Policy`. All of the rules should be of the `kind` specified in the `Policy`. — item shape: {denyRule?: record, description?: string}
  ...
  let qp = [(serialize-qp "$.xgafv" $xgafv "scalar") (serialize-qp "access_token" $access_token "scalar") (serialize-qp "alt" $alt "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "fields" $fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $pretty_print "scalar") (serialize-qp "quotaUser" $quota_user "scalar") (serialize-qp "upload_protocol" $upload_protocol "scalar") (serialize-qp "uploadType" $upload_type "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({name: (encode-path-segment $name)} | format pattern "/v2/{name}") $qp)
  let req_body = {"annotations": $annotations, "displayName": $display_name, "etag": $etag, "name": $body_name, "rules": $rules, "uid": $uid} | compact
...
export def "policies list" [
  ...
  --upload-protocol: string # Upload protocol for media (e.g. "raw", "multipart").
  --upload-type: string # Legacy upload protocol for media (e.g. "media", "multipart").
  --page-size: int # The maximum number of policies to return. IAM ignores this value and uses the value 1000.
  --page-token: string # A page token received in a ListPoliciesResponse. Provide this token to retrieve the next page.
]: nothing -> record<nextPageToken: string, policies: table<annotations: record, createTime: string, deleteTime: string, displayName: string, etag: string, kind: string, name: string, rules: list, uid: string, updateTime: string>> {
  let auth = (merge-auth [(build-auth ($token_oauth2 | default ($env | get -o IDENTITY_AND_ACCESS_MANAGEMENT_IAM_API_OAUTH2_TOKEN | default "")) "bearer") (build-auth ($token_oauth2c | default ($env | get -o IDENTITY_AND_ACCESS_MANAGEMENT_IAM_API_OAUTH2C_TOKEN | default "")) "bearer")])
  let base = ($base_url | default $BASE_URL)
  ...
  let full_url = (build-url $base ({parent: (encode-path-segment $parent)} | format pattern "/v2/{parent}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "policies create-policy" [
  ...
  --upload-protocol: string # Upload protocol for media (e.g. "raw", "multipart").
  --upload-type: string # Legacy upload protocol for media (e.g. "media", "multipart").
  --policy-id: string # The ID to use for this policy, which will become the final component of the policy's resource name. The ID must contain 3 to 63 characters. It can contain lowercase letters and numbers, as well as dashes (`-`) and periods (`.`). The first character must be a lowercase letter.
  --annotations: record # A key-value map to store arbitrary metadata for the `Policy`. Keys can be up to 63 characters. Values can be up to 255 characters.
  --display-name: string # A user-specified description of the `Policy`. This value can be up to 63 characters.
  --etag: string # An opaque tag that identifies the current version of the `Policy`. IAM uses this value to help manage concurrent updates, so they do not cause one update to be overwritten by another. If this field is present in a CreatePolicy request, the value is ignored.
  ...
  let base = ($base_url | default $BASE_URL)
  if ($parent | is-empty) { error make --unspanned { msg: "path parameter 'parent' must be non-empty" } }
  let qp = [(serialize-qp "$.xgafv" $xgafv "scalar") (serialize-qp "access_token" $access_token "scalar") (serialize-qp "alt" $alt "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "fields" $fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $pretty_print "scalar") (serialize-qp "quotaUser" $quota_user "scalar") (serialize-qp "upload_protocol" $upload_protocol "scalar") (serialize-qp "uploadType" $upload_type "scalar") (serialize-qp "policyId" $policy_id "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({parent: (encode-path-segment $parent)} | format pattern "/v2/{parent}") $qp)
  let req_body = {"annotations": $annotations, "displayName": $display_name, "etag": $etag, "name": $name, "rules": $rules, "uid": $uid} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"$.xgafv": $xgafv, "access_token": $access_token, "alt": $alt, "callback": $callback, "fields": $fields, "key": $key, "oauth_token": $oauth_token, "prettyPrint": $pretty_print, "quotaUser": $quota_user, "upload_protocol": $upload_protocol, "uploadType": $upload_type, "policyId": $policy_id} | compact), body: $req_body}
}
