...
export def "v1alpha1-claims-search list" [
  ...
  --language-code: string # The BCP-47 language code, such as "en-US" or "sr-Latn". Can be used to restrict results by language, though we do not currently consider the region.
  --max-age-days: int # The maximum age of the returned search results, in days. Age is determined by either claim date or review date, whichever is newer.
  ...
  --page-size: int # The pagination size. We will return up to that many results. Defaults to 10 if not set.
  ...
  --review-publisher-site-filter: string # The review publisher site to filter results by, e.g. nytimes.com.
...
export def "v1alpha1-pages list" [
  ...
  let auth = (merge-auth [(build-auth ($token_oauth2 | default ($env | get -o FACT_CHECK_TOOLS_API_OAUTH2_TOKEN | default "")) "bearer") (build-auth ($token_oauth2c | default ($env | get -o FACT_CHECK_TOOLS_API_OAUTH2C_TOKEN | default "")) "bearer")])
  let base = ($base_url | default $BASE_URL)
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"$.xgafv": $xgafv, "access_token": $access_token, "alt": $alt, "callback": $callback, "fields": $fields, "key": $key, "oauth_token": $oauth_token, "prettyPrint": $pretty_print, "quotaUser": $quota_user, "upload_protocol": $upload_protocol, "uploadType": $upload_type, "offset": $offset, "organization": $organization, "pageSize": $page_size, "pageToken": $page_token, "url": $url} | compact), body: null}
}
...
export def "v1alpha1-pages create" [
  ...
  --version-id: string # The version ID for this markup. Except for update requests, this field is output-only and should not be set by the user.
