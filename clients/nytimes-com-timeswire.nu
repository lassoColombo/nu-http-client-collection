# Auto-generated client for Times Newswire API v3.0.0
# Source: https://api.apis.guru/v2/specs/nytimes.com/timeswire/3.0.0/openapi.json
# Auth: --token flag or $env.TIMES_NEWSWIRE_API_TOKEN

const BASE_URL = "http://api.nytimes.com/svc/news/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o TIMES_NEWSWIRE_API_TOKEN | default "" }
...
def base-url-completer [] { ["http://api.nytimes.com/svc/news/v3" "https://api.nytimes.com/svc/news/v3"] }
...
export def "content-json get" [
  ...
  let full_url = (build-url $base "/content.json" $qp $auth.query)
...
export def "content list" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --limit: int # Limits the number of results, between 1 and 20 (default: 20)
  ...
  let full_url = (build-url $base ({source: (encode-path-segment $source), section: (encode-path-segment $section)} | format pattern "/content/{source}/{section}.json") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "content get" [
  ...
  let qp = [(serialize-qp "limit" $limit "scalar") (serialize-qp "offset" $offset "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({source: (encode-path-segment $source), section: (encode-path-segment $section), time_period: (encode-path-segment $time_period)} | format pattern "/content/{source}/{section}/{time_period}.json") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
