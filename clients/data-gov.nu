...
export def "docket-response-format get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"docketId": $docket_id} | compact), body: null}
}
...
export def "document-response-format get" [
  ...
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"documentId": $document_id, "federalRegisterNumber": $federal_register_number} | compact), body: null}
}
...
export def "documents-response-format get" [
  ...
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"countsOnly": $counts_only, "encoded": $encoded, "s": $s, "dct": $dct, "dktid": $dktid, "dkt": $dkt, "cp": $cp, "a": $a, "rpp": $rpp, "po": $po, "cs": $cs, "np": $np, "cmsd": $cmsd, "cmd": $cmd, "crd": $crd, "rd": $rd, "pd": $pd, "cat": $cat, "sb": $sb, "so": $so, "dktst": $dktst, "dktst2": $dktst2, "docst": $docst} | compact), body: null}
}
