...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o POLICY_SIMULATOR_API_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "v1beta policysimulatorprojectslocationsreplaysoperationslist" } } | get name | first)
...
export def "v1beta policysimulatorprojectslocationsreplaysoperationslist" [
  name: string
