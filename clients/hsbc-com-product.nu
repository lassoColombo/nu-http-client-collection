# Auto-generated client for Product Finder API v2.2.1
# Source: https://api.apis.guru/v2/specs/hsbc.com/product/2.2.1/swagger.json
# Auth: --token flag or $env.PRODUCT_FINDER_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o PRODUCT_FINDER_API_TOKEN | default "" }
...
export def "open-banking-v2-2-business-current-accounts get" [
  ...
  let full_url = (build-url $base "/open-banking/v2.2/business-current-accounts")
...
export def "open-banking-v2-2-commercial-credit-cards get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/open-banking/v2.2/commercial-credit-cards")
  let accept_val = "application/prs.openbanking.opendata.v2.2+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
...
export def "open-banking-v2-2-personal-current-accounts get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/open-banking/v2.2/personal-current-accounts")
  let accept_val = "application/prs.openbanking.opendata.v2.2+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
...
export def "open-banking-v2-2-unsecured-sme-loans get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/open-banking/v2.2/unsecured-sme-loans")
  let accept_val = "application/prs.openbanking.opendata.v2.2+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
...
export def "x-open-banking-v2-2-business-current-accounts-segment get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($segment | is-empty) { error make --unspanned { msg: "path parameter 'segment' must be non-empty" } }
