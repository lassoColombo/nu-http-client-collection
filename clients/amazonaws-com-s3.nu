...
export def "api abort-multipart-upload" [
  ...
  let full_url = (build-url $base ({bucket: (encode-path-segment $bucket), key: (encode-path-segment $key)} | format pattern "/{bucket}/{key}") $qp)
  ...
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"uploadId": $upload_id} | compact), body: null}
}
...
export def "api complete-multipart-upload" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"uploadId": $upload_id} | compact), body: $req_body}
}
...
export def "api list-parts" [
  ...
  --max-parts-2: string # Pagination limit (disambiguated-2)
  --part-number-marker-2: string # Pagination token (disambiguated-2)
  --x-amz-security-token: string
  --x-amz-request-payer: string@x-amz-request-payer-completer
  --x-amz-expected-bucket-owner: string # The account ID of the expected bucket owner. If the bucket is owned by a different account, the request fails with the HTTP status code 403 Forbidden (access denied).
  --x-amz-server-side-encryption-customer-algorithm: string # The server-side encryption (SSE) algorithm used to encrypt the object. This parameter is needed only when the object was created using a checksum algorithm. For more information, see Protecting data using SSE-C keys (https://docs.aws.amazon.com/AmazonS3/latest/dev/ServerSideEncryptionCustomerKeys.html) in the Amazon S3 User Guide.
  ...
  if ($bucket | is-empty) { error make --unspanned { msg: "path parameter 'Bucket' must be non-empty" } }
  if ($key | is-empty) { error make --unspanned { msg: "path parameter 'Key' must be non-empty" } }
  let qp = [(serialize-qp "max-parts" $max_parts "scalar") (serialize-qp "part-number-marker" $part_number_marker "scalar") (serialize-qp "uploadId" $upload_id "scalar") (serialize-qp "MaxParts" $max_parts_2 "scalar") (serialize-qp "PartNumberMarker" $part_number_marker_2 "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({bucket: (encode-path-segment $bucket), key: (encode-path-segment $key)} | format pattern "/{bucket}/{key}") $qp)
  let accept_val = "text/xml"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-request-payer": $x_amz_request_payer, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner, "x-amz-server-side-encryption-customer-algorithm": $x_amz_server_side_encryption_customer_algorithm, "x-amz-server-side-encryption-customer-key": $x_amz_server_side_encryption_customer_key, "x-amz-server-side-encryption-customer-key-MD5": $x_amz_server_side_encryption_customer_key_md5} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"max-parts": $max_parts, "part-number-marker": $part_number_marker, "uploadId": $upload_id, "MaxParts": $max_parts_2, "PartNumberMarker": $part_number_marker_2} | compact), body: null}
}
...
export def "api copy-object" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --x-amz-security-token: string
  --x-amz-acl: string@x-amz-acl-completer # The canned ACL to apply to the object. This action is not supported by Amazon S3 on Outposts.
  ...
  --x-amz-grant-full-control: string # Gives the grantee READ, READ_ACP, and WRITE_ACP permissions on the object. This action is not supported by Amazon S3 on Outposts.
  --x-amz-grant-read: string # Allows grantee to read the object data and its metadata. This action is not supported by Amazon S3 on Outposts.
  --x-amz-grant-read-acp: string # Allows grantee to read the object ACL. This action is not supported by Amazon S3 on Outposts.
  --x-amz-grant-write-acp: string # Allows grantee to write the ACL for the applicable object. This action is not supported by Amazon S3 on Outposts.
  ...
  --x-amz-server-side-encryption-bucket-key-enabled: oneof<nothing, bool> # Specifies whether Amazon S3 should use an S3 Bucket Key for object encryption with server-side encryption using AWS KMS (SSE-KMS). Setting this header to true causes Amazon S3 to use an S3 Bucket Key for object encryption with SSE-KMS. Specifying this header with a COPY action doesn’t affect bucket-level settings for S3 Bucket Key.
  ...
  --x-amz-request-payer: string@x-amz-request-payer-completer
  --x-amz-tagging: string # The tag-set for the object destination object this value must be used in conjunction with the TaggingDirective. The tag-set must be encoded as URL Query parameters.
  ...
  if ($bucket | is-empty) { error make --unspanned { msg: "path parameter 'Bucket' must be non-empty" } }
  if ($key | is-empty) { error make --unspanned { msg: "path parameter 'Key' must be non-empty" } }
  let full_url = (build-url $base ({bucket: (encode-path-segment $bucket), key: (encode-path-segment $key)} | format pattern "/{bucket}/{key}"))
  let req_body = $body
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else if (($input | is-not-empty) and ($req_body | is-empty)) { $input } else { $req_body }
  let accept_val = "text/xml"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let effective_ct = ($content_type | default "text/xml")
...
export def "api create" [
  ...
  --x-amz-grant-write: string # Allows grantee to create new objects in the bucket. For the bucket and object owners of existing objects, also allows deletions and overwrites of those objects.
  ...
  let base = ($base_url | default $BASE_URL)
  if ($bucket | is-empty) { error make --unspanned { msg: "path parameter 'Bucket' must be non-empty" } }
  let full_url = (build-url $base ({bucket: (encode-path-segment $bucket)} | format pattern "/{bucket}"))
  let req_body = $body
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else if (($input | is-not-empty) and ($req_body | is-empty)) { $input } else { $req_body }
  let accept_val = "text/xml"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
# Deletes the S3 bucket. All objects (including all object versions and delete markers) in the bucket must be deleted before the bucket itself can be deleted. Related Resources CreateBucket (https://docs.aws.amazon.com/AmazonS3/latest/API/API_CreateBucket.html) DeleteObject (https://docs.aws.amazon.com/AmazonS3/latest/API/API_DeleteObject.html)
#
# DELETE /{Bucket}
...
export def "api list-objects-by-bucket" [
  ...
  --max-keys-2: string # Pagination limit (disambiguated-2)
  ...
  let base = ($base_url | default $BASE_URL)
  if ($bucket | is-empty) { error make --unspanned { msg: "path parameter 'Bucket' must be non-empty" } }
  let qp = [(serialize-qp "delimiter" $delimiter "scalar") (serialize-qp "encoding-type" $encoding_type "scalar") (serialize-qp "marker" $marker "scalar") (serialize-qp "max-keys" $max_keys "scalar") (serialize-qp "prefix" $prefix "scalar") (serialize-qp "MaxKeys" $max_keys_2 "scalar") (serialize-qp "Marker" $marker_2 "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({bucket: (encode-path-segment $bucket)} | format pattern "/{bucket}") $qp)
  let accept_val = "text/xml"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-request-payer": $x_amz_request_payer, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"delimiter": $delimiter, "encoding-type": $encoding_type, "marker": $marker, "max-keys": $max_keys, "prefix": $prefix, "MaxKeys": $max_keys_2, "Marker": $marker_2} | compact), body: null}
}
...
export def "api create-multipart-upload" [
  ...
  let effective_ct = ($content_type | default "text/xml")
  let req_body_wire = if $effective_ct == "application/x-www-form-urlencoded" { $req_body | transpose k v | where v != null | reduce -f {} {|p, acc| $acc | upsert $p.k $p.v } | url build-query } else { $req_body }
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full $effective_ct $req_body_wire {query: ({"uploads": $uploads} | compact), body: $req_body}
}
...
export def "api delete-analytics-configuration" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"id": $id, "analytics": $analytics} | compact), body: null}
}
...
export def "api get-analytics-configuration" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"id": $id, "analytics": $analytics} | compact), body: null}
}
...
export def "api update-analytics-configuration" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"id": $id, "analytics": $analytics} | compact), body: $req_body}
}
...
export def "api delete-cors" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"cors": $cors} | compact), body: null}
}
...
export def "api get-cors" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"cors": $cors} | compact), body: null}
}
...
export def "api update-cors" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"cors": $cors} | compact), body: $req_body}
}
...
export def "api delete-encryption" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"encryption": $encryption} | compact), body: null}
}
...
export def "api get-encryption" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"encryption": $encryption} | compact), body: null}
}
...
export def "api update-encryption" [
  ...
  --encryption: oneof<nothing, bool> # allows empty value
  --x-amz-security-token: string
  --content-md5: string # The base64-encoded 128-bit MD5 digest of the server-side encryption configuration. For requests made using the Amazon Web Services Command Line Interface (CLI) or Amazon Web Services SDKs, this field is calculated automatically.
  --x-amz-sdk-checksum-algorithm: string@x-amz-sdk-checksum-algorithm-completer # Indicates the algorithm used to create the checksum for the object when using the SDK. This header will not provide any additional functionality if not using the SDK. When sending this header, there must be a corresponding x-amz-checksum or x-amz-trailer header sent. Otherwise, Amazon S3 fails the request with the HTTP status code 400 Bad Request. For more information, see Checking object integrity (https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the Amazon S3 User Guide. If you provide an individual checksum, Amazon S3 ignores any provided ChecksumAlgorithm parameter.
  --x-amz-expected-bucket-owner: string # The account ID of the expected bucket owner. If the bucket is owned by a different account, the request fails with the HTTP status code 403 Forbidden (access denied).
  --body: any
]: any -> any {
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "Content-MD5": $content_md5, "x-amz-sdk-checksum-algorithm": $x_amz_sdk_checksum_algorithm, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"encryption": $encryption} | compact), body: $req_body}
}
...
export def "api delete-intelligent-tiering-configuration" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"id": $id, "intelligent-tiering": $intelligent_tiering} | compact), body: null}
}
...
export def "api get-intelligent-tiering-configuration" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"id": $id, "intelligent-tiering": $intelligent_tiering} | compact), body: null}
}
...
export def "api update-intelligent-tiering-configuration" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"id": $id, "intelligent-tiering": $intelligent_tiering} | compact), body: $req_body}
}
...
export def "api delete-inventory-configuration" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"id": $id, "inventory": $inventory} | compact), body: null}
}
...
export def "api get-inventory-configuration" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"id": $id, "inventory": $inventory} | compact), body: null}
}
...
export def "api update-inventory-configuration" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"id": $id, "inventory": $inventory} | compact), body: $req_body}
}
...
export def "api delete-lifecycle" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"lifecycle": $lifecycle} | compact), body: null}
}
...
export def "api get-lifecycle-configuration" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"lifecycle": $lifecycle} | compact), body: null}
}
...
export def "api update-lifecycle-configuration" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"lifecycle": $lifecycle} | compact), body: $req_body}
}
...
export def "api delete-metrics-configuration" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"id": $id, "metrics": $metrics} | compact), body: null}
}
...
export def "api get-metrics-configuration" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"id": $id, "metrics": $metrics} | compact), body: null}
}
...
export def "api update-metrics-configuration" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"id": $id, "metrics": $metrics} | compact), body: $req_body}
}
...
export def "api delete-ownership-controls" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"ownershipControls": $ownership_controls} | compact), body: null}
}
...
export def "api get-ownership-controls" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"ownershipControls": $ownership_controls} | compact), body: null}
}
...
export def "api update-ownership-controls" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"ownershipControls": $ownership_controls} | compact), body: $req_body}
}
...
export def "api delete-policy" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"policy": $policy} | compact), body: null}
}
...
export def "api get-policy" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"policy": $policy} | compact), body: null}
}
...
export def "api update-policy" [
  ...
  --policy: oneof<nothing, bool> # allows empty value
  --x-amz-security-token: string
  --content-md5: string # The MD5 hash of the request body. For requests made using the Amazon Web Services Command Line Interface (CLI) or Amazon Web Services SDKs, this field is calculated automatically.
  --x-amz-sdk-checksum-algorithm: string@x-amz-sdk-checksum-algorithm-completer # Indicates the algorithm used to create the checksum for the object when using the SDK. This header will not provide any additional functionality if not using the SDK. When sending this header, there must be a corresponding x-amz-checksum or x-amz-trailer header sent. Otherwise, Amazon S3 fails the request with the HTTP status code 400 Bad Request. For more information, see Checking object integrity (https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the Amazon S3 User Guide. If you provide an individual checksum, Amazon S3 ignores any provided ChecksumAlgorithm parameter.
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"policy": $policy} | compact), body: $req_body}
}
...
export def "api delete-replication" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"replication": $replication} | compact), body: null}
}
...
export def "api get-replication" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"replication": $replication} | compact), body: null}
}
...
export def "api update-replication" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"replication": $replication} | compact), body: $req_body}
}
...
export def "api delete-tagging" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"tagging": $tagging} | compact), body: null}
}
...
export def "api get-tagging" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"tagging": $tagging} | compact), body: null}
}
...
export def "api update-tagging" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "Content-MD5": $content_md5, "x-amz-sdk-checksum-algorithm": $x_amz_sdk_checksum_algorithm, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"tagging": $tagging} | compact), body: $req_body}
}
...
export def "api delete-website" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"website": $website} | compact), body: null}
}
...
export def "api get-website" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"website": $website} | compact), body: null}
}
...
export def "api update-website" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "Content-MD5": $content_md5, "x-amz-sdk-checksum-algorithm": $x_amz_sdk_checksum_algorithm, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"website": $website} | compact), body: $req_body}
}
...
export def "api delete-object" [
  ...
  --x-amz-request-payer: string@x-amz-request-payer-completer
  --x-amz-bypass-governance-retention: oneof<nothing, bool> # Indicates whether S3 Object Lock should bypass Governance-mode restrictions to process this operation. To use this header, you must have the s3:BypassGovernanceRetention permission.
  --x-amz-expected-bucket-owner: string # The account ID of the expected bucket owner. If the bucket is owned by a different account, the request fails with the HTTP status code 403 Forbidden (access denied).
]: nothing -> any {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
...
export def "api get-object" [
  ...
  --response-expires: string # Sets the Expires header of the response. (format: date-time)
  --version-id: string # VersionId used to reference a specific version of the object.
  ...
  --range: string # Downloads the specified range bytes of an object. For more information about the HTTP Range header, see https://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.35 (https://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.35). Amazon S3 doesn't support retrieving multiple ranges of data per GET request.
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"response-cache-control": $response_cache_control, "response-content-disposition": $response_content_disposition, "response-content-encoding": $response_content_encoding, "response-content-language": $response_content_language, "response-content-type": $response_content_type, "response-expires": $response_expires, "versionId": $version_id, "partNumber": $part_number} | compact), body: null}
}
...
export def "api head-object" [
  ...
  --if-none-match: string # Return the object only if its entity tag (ETag) is different from the one specified; otherwise, return a 304 (not modified) error.
  --if-unmodified-since: string # Return the object only if it has not been modified since the specified time; otherwise, return a 412 (precondition failed) error.
  --range: string # Because HeadObject returns only the metadata for an object, this parameter has no effect.
  --x-amz-server-side-encryption-customer-algorithm: string # Specifies the algorithm to use to when encrypting the object (for example, AES256).
  --x-amz-server-side-encryption-customer-key: string # Specifies the customer-provided encryption key for Amazon S3 to use in encrypting data. This value is used to store the object and then it is discarded; Amazon S3 does not store the encryption key. The key must be appropriate for use with the algorithm specified in the x-amz-server-side-encryption-customer-algorithm header.
  --x-amz-server-side-encryption-customer-key-md5: string # Specifies the 128-bit MD5 digest of the encryption key according to RFC 1321. Amazon S3 uses this header for a message integrity check to ensure that the encryption key was transmitted without error.
  --x-amz-request-payer: string@x-amz-request-payer-completer
  --x-amz-expected-bucket-owner: string # The account ID of the expected bucket owner. If the bucket is owned by a different account, the request fails with the HTTP status code 403 Forbidden (access denied).
  --x-amz-checksum-mode: string@x-amz-checksum-mode-completer # To retrieve the checksum, this parameter must be enabled. In addition, if you enable ChecksumMode and the object is encrypted with Amazon Web Services Key Management Service (Amazon Web Services KMS), you must have permission to use the kms:Decrypt action for the request to succeed.
]: nothing -> record {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($bucket | is-empty) { error make --unspanned { msg: "path parameter 'Bucket' must be non-empty" } }
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "If-Match": $if_match, "If-Modified-Since": $if_modified_since, "If-None-Match": $if_none_match, "If-Unmodified-Since": $if_unmodified_since, "Range": $range, "x-amz-server-side-encryption-customer-algorithm": $x_amz_server_side_encryption_customer_algorithm, "x-amz-server-side-encryption-customer-key": $x_amz_server_side_encryption_customer_key, "x-amz-server-side-encryption-customer-key-MD5": $x_amz_server_side_encryption_customer_key_md5, "x-amz-request-payer": $x_amz_request_payer, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner, "x-amz-checksum-mode": $x_amz_checksum_mode} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "head" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"versionId": $version_id, "partNumber": $part_number} | compact), body: null}
}
...
export def "api update-object" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --x-amz-security-token: string
  --x-amz-acl: string@x-amz-acl-completer # The canned ACL to apply to the object. For more information, see Canned ACL (https://docs.aws.amazon.com/AmazonS3/latest/dev/acl-overview.html#CannedACL). This action is not supported by Amazon S3 on Outposts.
  --cache-control: string # Can be used to specify caching behavior along the request/reply chain. For more information, see http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.9 (http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.9).
  --content-disposition: string # Specifies presentational information for the object. For more information, see http://www.w3.org/Protocols/rfc2616/rfc2616-sec19.html#sec19.5.1 (http://www.w3.org/Protocols/rfc2616/rfc2616-sec19.html#sec19.5.1).
  ...
  --content-language: string # The language the content is in.
  --content-length: int # Size of the body in bytes. This parameter is useful when the size of the body cannot be determined automatically. For more information, see http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.13 (http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.13).
  ...
  --content-type: string # A standard MIME type describing the format of the contents. For more information, see http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.17 (http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.17).
  --x-amz-sdk-checksum-algorithm: string@x-amz-sdk-checksum-algorithm-completer # Indicates the algorithm used to create the checksum for the object when using the SDK. This header will not provide any additional functionality if not using the SDK. When sending this header, there must be a corresponding x-amz-checksum or x-amz-trailer header sent. Otherwise, Amazon S3 fails the request with the HTTP status code 400 Bad Request. For more information, see Checking object integrity (https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the Amazon S3 User Guide. If you provide an individual checksum, Amazon S3 ignores any provided ChecksumAlgorithm parameter.
  --x-amz-checksum-crc32: string # This header can be used as a data integrity check to verify that the data received is the same data that was originally sent. This header specifies the base64-encoded, 32-bit CRC32 checksum of the object. For more information, see Checking object integrity (https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the Amazon S3 User Guide.
  --x-amz-checksum-crc32c: string # This header can be used as a data integrity check to verify that the data received is the same data that was originally sent. This header specifies the base64-encoded, 32-bit CRC32C checksum of the object. For more information, see Checking object integrity (https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the Amazon S3 User Guide.
  --x-amz-checksum-sha1: string # This header can be used as a data integrity check to verify that the data received is the same data that was originally sent. This header specifies the base64-encoded, 160-bit SHA-1 digest of the object. For more information, see Checking object integrity (https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the Amazon S3 User Guide.
  --x-amz-checksum-sha256: string # This header can be used as a data integrity check to verify that the data received is the same data that was originally sent. This header specifies the base64-encoded, 256-bit SHA-256 digest of the object. For more information, see Checking object integrity (https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the Amazon S3 User Guide.
  --expires: string # The date and time at which the object is no longer cacheable. For more information, see http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.21 (http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.21).
  --x-amz-grant-full-control: string # Gives the grantee READ, READ_ACP, and WRITE_ACP permissions on the object. This action is not supported by Amazon S3 on Outposts.
  --x-amz-grant-read: string # Allows grantee to read the object data and its metadata. This action is not supported by Amazon S3 on Outposts.
  --x-amz-grant-read-acp: string # Allows grantee to read the object ACL. This action is not supported by Amazon S3 on Outposts.
  --x-amz-grant-write-acp: string # Allows grantee to write the ACL for the applicable object. This action is not supported by Amazon S3 on Outposts.
  --x-amz-server-side-encryption: string@x-amz-server-side-encryption-completer # The server-side encryption algorithm used when storing this object in Amazon S3 (for example, AES256, aws:kms).
  --x-amz-storage-class: string@x-amz-storage-class-completer # By default, Amazon S3 uses the STANDARD Storage Class to store newly created objects. The STANDARD storage class provides high durability and high availability. Depending on performance needs, you can specify a different Storage Class. Amazon S3 on Outposts only uses the OUTPOSTS Storage Class. For more information, see Storage Classes (https://docs.aws.amazon.com/AmazonS3/latest/dev/storage-class-intro.html) in the Amazon S3 User Guide.
  --x-amz-website-redirect-location: string # If the bucket is configured as a website, redirects requests for this object to another object in the same bucket or to an external URL. Amazon S3 stores the value of this header in the object metadata. For information about object metadata, see Object Key and Metadata (https://docs.aws.amazon.com/AmazonS3/latest/dev/UsingMetadata.html). In the following example, the request header sets the redirect to an object (anotherPage.html) in the same bucket: x-amz-website-redirect-location: /anotherPage.html In the following example, the request header sets the object redirect to another website: x-amz-website-redirect-location: http://www.example.com/ For more information about website hosting in Amazon S3, see Hosting Websites on Amazon S3 (https://docs.aws.amazon.com/AmazonS3/latest/dev/WebsiteHosting.html) and How to Configure Website Page Redirects (https://docs.aws.amazon.com/AmazonS3/latest/dev/how-to-page-redirect.html).
  --x-amz-server-side-encryption-customer-algorithm: string # Specifies the algorithm to use to when encrypting the object (for example, AES256).
  --x-amz-server-side-encryption-customer-key: string # Specifies the customer-provided encryption key for Amazon S3 to use in encrypting data. This value is used to store the object and then it is discarded; Amazon S3 does not store the encryption key. The key must be appropriate for use with the algorithm specified in the x-amz-server-side-encryption-customer-algorithm header.
  --x-amz-server-side-encryption-customer-key-md5: string # Specifies the 128-bit MD5 digest of the encryption key according to RFC 1321. Amazon S3 uses this header for a message integrity check to ensure that the encryption key was transmitted without error.
  --x-amz-server-side-encryption-aws-kms-key-id: string # If x-amz-server-side-encryption is present and has the value of aws:kms, this header specifies the ID of the Amazon Web Services Key Management Service (Amazon Web Services KMS) symmetrical customer managed key that was used for the object. If you specify x-amz-server-side-encryption:aws:kms, but do not provide x-amz-server-side-encryption-aws-kms-key-id, Amazon S3 uses the Amazon Web Services managed key to protect the data. If the KMS key does not exist in the same account issuing the command, you must use the full ARN and not just the ID.
  --x-amz-server-side-encryption-context: string # Specifies the Amazon Web Services KMS Encryption Context to use for object encryption. The value of this header is a base64-encoded UTF-8 string holding JSON with the encryption context key-value pairs.
...
export def "api delete-object-tagging" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"versionId": $version_id, "tagging": $tagging} | compact), body: null}
}
...
export def "api get-object-tagging" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"versionId": $version_id, "tagging": $tagging} | compact), body: null}
}
...
export def "api update-object-tagging" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"versionId": $version_id, "tagging": $tagging} | compact), body: $req_body}
}
...
export def "api delete-objects" [
  ...
  --x-amz-mfa: string # The concatenation of the authentication device's serial number, a space, and the value that is displayed on your authentication device. Required to permanently delete a versioned object if versioning is configured with MFA delete enabled.
  --x-amz-request-payer: string@x-amz-request-payer-completer
  --x-amz-bypass-governance-retention: oneof<nothing, bool> # Specifies whether you want to delete this object even if it has a Governance-type Object Lock in place. To use this header, you must have the s3:BypassGovernanceRetention permission.
  --x-amz-expected-bucket-owner: string # The account ID of the expected bucket owner. If the bucket is owned by a different account, the request fails with the HTTP status code 403 Forbidden (access denied).
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"delete": $delete} | compact), body: $req_body}
}
...
export def "api delete-public-access-block" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"publicAccessBlock": $public_access_block} | compact), body: null}
}
...
export def "api get-public-access-block" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"publicAccessBlock": $public_access_block} | compact), body: null}
}
...
export def "api update-public-access-block" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "Content-MD5": $content_md5, "x-amz-sdk-checksum-algorithm": $x_amz_sdk_checksum_algorithm, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"publicAccessBlock": $public_access_block} | compact), body: $req_body}
}
...
export def "api get-accelerate-configuration" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"accelerate": $accelerate} | compact), body: null}
}
...
export def "api update-accelerate-configuration" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"accelerate": $accelerate} | compact), body: $req_body}
}
...
export def "api get-acl" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"acl": $acl} | compact), body: null}
}
...
export def "api update-acl" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"acl": $acl} | compact), body: $req_body}
}
...
export def "api update-lifecycle" [
  ...
  --lifecycle: oneof<nothing, bool> # allows empty value
  --x-amz-security-token: string
  --content-md5: string # For requests made using the Amazon Web Services Command Line Interface (CLI) or Amazon Web Services SDKs, this field is calculated automatically.
  --x-amz-sdk-checksum-algorithm: string@x-amz-sdk-checksum-algorithm-completer # Indicates the algorithm used to create the checksum for the object when using the SDK. This header will not provide any additional functionality if not using the SDK. When sending this header, there must be a corresponding x-amz-checksum or x-amz-trailer header sent. Otherwise, Amazon S3 fails the request with the HTTP status code 400 Bad Request. For more information, see Checking object integrity (https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the Amazon S3 User Guide. If you provide an individual checksum, Amazon S3 ignores any provided ChecksumAlgorithm parameter.
  --x-amz-expected-bucket-owner: string # The account ID of the expected bucket owner. If the bucket is owned by a different account, the request fails with the HTTP status code 403 Forbidden (access denied).
  --body: any
]: any -> any {
...
export def "api update-logging" [
  ...
  --logging: oneof<nothing, bool> # allows empty value
  --x-amz-security-token: string
  --content-md5: string # The MD5 hash of the PutBucketLogging request body. For requests made using the Amazon Web Services Command Line Interface (CLI) or Amazon Web Services SDKs, this field is calculated automatically.
  --x-amz-sdk-checksum-algorithm: string@x-amz-sdk-checksum-algorithm-completer # Indicates the algorithm used to create the checksum for the object when using the SDK. This header will not provide any additional functionality if not using the SDK. When sending this header, there must be a corresponding x-amz-checksum or x-amz-trailer header sent. Otherwise, Amazon S3 fails the request with the HTTP status code 400 Bad Request. For more information, see Checking object integrity (https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the Amazon S3 User Guide. If you provide an individual checksum, Amazon S3 ignores any provided ChecksumAlgorithm parameter.
  --x-amz-expected-bucket-owner: string # The account ID of the expected bucket owner. If the bucket is owned by a different account, the request fails with the HTTP status code 403 Forbidden (access denied).
  --body: any
]: any -> any {
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "Content-MD5": $content_md5, "x-amz-sdk-checksum-algorithm": $x_amz_sdk_checksum_algorithm, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"logging": $logging} | compact), body: $req_body}
}
...
export def "api get-notification-configuration" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"notification": $notification} | compact), body: null}
}
...
export def "api update-notification-configuration" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"notification": $notification} | compact), body: $req_body}
}
...
export def "api get-policy-status" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"policyStatus": $policy_status} | compact), body: null}
}
...
export def "api get-request-payment" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"requestPayment": $request_payment} | compact), body: null}
}
...
export def "api update-request-payment" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "Content-MD5": $content_md5, "x-amz-sdk-checksum-algorithm": $x_amz_sdk_checksum_algorithm, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"requestPayment": $request_payment} | compact), body: $req_body}
}
...
export def "api get-versioning" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"versioning": $versioning} | compact), body: null}
}
...
export def "api update-versioning" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"versioning": $versioning} | compact), body: $req_body}
}
...
export def "api get-object-acl" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-request-payer": $x_amz_request_payer, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"versionId": $version_id, "acl": $acl} | compact), body: null}
}
...
export def "api update-object-acl" [
  ...
  --acl: oneof<nothing, bool> # allows empty value
  --x-amz-security-token: string
  --x-amz-acl: string@x-amz-acl-completer # The canned ACL to apply to the object. For more information, see Canned ACL (https://docs.aws.amazon.com/AmazonS3/latest/dev/acl-overview.html#CannedACL).
  --content-md5: string # The base64-encoded 128-bit MD5 digest of the data. This header must be used as a message integrity check to verify that the request body was not corrupted in transit. For more information, go to RFC 1864.> (http://www.ietf.org/rfc/rfc1864.txt) For requests made using the Amazon Web Services Command Line Interface (CLI) or Amazon Web Services SDKs, this field is calculated automatically.
  --x-amz-sdk-checksum-algorithm: string@x-amz-sdk-checksum-algorithm-completer # Indicates the algorithm used to create the checksum for the object when using the SDK. This header will not provide any additional functionality if not using the SDK. When sending this header, there must be a corresponding x-amz-checksum or x-amz-trailer header sent. Otherwise, Amazon S3 fails the request with the HTTP status code 400 Bad Request. For more information, see Checking object integrity (https://docs.aws.amazon.com/AmazonS3/latest/userguide/checking-object-integrity.html) in the Amazon S3 User Guide. If you provide an individual checksum, Amazon S3 ignores any provided ChecksumAlgorithm parameter.
  --x-amz-grant-full-control: string # Allows grantee the read, write, read ACP, and write ACP permissions on the bucket. This action is not supported by Amazon S3 on Outposts.
  --x-amz-grant-read: string # Allows grantee to list the objects in the bucket. This action is not supported by Amazon S3 on Outposts.
  --x-amz-grant-read-acp: string # Allows grantee to read the bucket ACL. This action is not supported by Amazon S3 on Outposts.
  --x-amz-grant-write: string # Allows grantee to create new objects in the bucket. For the bucket and object owners of existing objects, also allows deletions and overwrites of those objects.
  --x-amz-grant-write-acp: string # Allows grantee to write the ACL for the applicable bucket. This action is not supported by Amazon S3 on Outposts.
  --x-amz-request-payer: string@x-amz-request-payer-completer
  --x-amz-expected-bucket-owner: string # The account ID of the expected bucket owner. If the bucket is owned by a different account, the request fails with the HTTP status code 403 Forbidden (access denied).
  --body: any
]: any -> any {
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"versionId": $version_id, "acl": $acl} | compact), body: $req_body}
}
...
export def "api get-object-attributes" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"versionId": $version_id, "attributes": $attributes} | compact), body: null}
}
...
export def "api get-object-legal-hold" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-request-payer": $x_amz_request_payer, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"versionId": $version_id, "legal-hold": $legal_hold} | compact), body: null}
}
...
export def "api update-object-legal-hold" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"versionId": $version_id, "legal-hold": $legal_hold} | compact), body: $req_body}
}
...
export def "api get-object-lock-configuration" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"object-lock": $object_lock} | compact), body: null}
}
...
export def "api update-object-lock-configuration" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"object-lock": $object_lock} | compact), body: $req_body}
}
...
export def "api get-object-retention" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-request-payer": $x_amz_request_payer, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"versionId": $version_id, "retention": $retention} | compact), body: null}
}
...
export def "api update-object-retention" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"versionId": $version_id, "retention": $retention} | compact), body: $req_body}
}
...
export def "api get-object-torrent" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-request-payer": $x_amz_request_payer, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"torrent": $torrent} | compact), body: null}
}
...
export def "api list-analytics-configurations" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"continuation-token": $continuation_token, "analytics": $analytics} | compact), body: null}
}
...
export def "api list-intelligent-tiering-configurations" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"continuation-token": $continuation_token, "intelligent-tiering": $intelligent_tiering} | compact), body: null}
}
...
export def "api list-inventory-configurations" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"continuation-token": $continuation_token, "inventory": $inventory} | compact), body: null}
}
...
export def "api list-metrics-configurations" [
  ...
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"continuation-token": $continuation_token, "metrics": $metrics} | compact), body: null}
}
...
export def "api list-multipart-uploads" [
  ...
  --max-uploads-2: string # Pagination limit (disambiguated-2)
  --key-marker-2: string # Pagination token (disambiguated-2)
  --upload-id-marker-2: string # Pagination token (disambiguated-2)
  --uploads: oneof<nothing, bool> # allows empty value
  --x-amz-security-token: string
  --x-amz-expected-bucket-owner: string # The account ID of the expected bucket owner. If the bucket is owned by a different account, the request fails with the HTTP status code 403 Forbidden (access denied).
]: nothing -> any {
  ...
  let base = ($base_url | default $BASE_URL)
  if ($bucket | is-empty) { error make --unspanned { msg: "path parameter 'Bucket' must be non-empty" } }
  let qp = [(serialize-qp "delimiter" $delimiter "scalar") (serialize-qp "encoding-type" $encoding_type "scalar") (serialize-qp "key-marker" $key_marker "scalar") (serialize-qp "max-uploads" $max_uploads "scalar") (serialize-qp "prefix" $prefix "scalar") (serialize-qp "upload-id-marker" $upload_id_marker "scalar") (serialize-qp "MaxUploads" $max_uploads_2 "scalar") (serialize-qp "KeyMarker" $key_marker_2 "scalar") (serialize-qp "UploadIdMarker" $upload_id_marker_2 "scalar") (serialize-qp "uploads" $uploads "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({bucket: (encode-path-segment $bucket)} | format pattern "/{bucket}") $qp)
  let accept_val = "text/xml"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"delimiter": $delimiter, "encoding-type": $encoding_type, "key-marker": $key_marker, "max-uploads": $max_uploads, "prefix": $prefix, "upload-id-marker": $upload_id_marker, "MaxUploads": $max_uploads_2, "KeyMarker": $key_marker_2, "UploadIdMarker": $upload_id_marker_2, "uploads": $uploads} | compact), body: null}
}
...
export def "api list-object-versions" [
  ...
  --max-keys-2: string # Pagination limit (disambiguated-2)
  --key-marker-2: string # Pagination token (disambiguated-2)
  --version-id-marker-2: string # Pagination token (disambiguated-2)
  ...
  let base = ($base_url | default $BASE_URL)
  if ($bucket | is-empty) { error make --unspanned { msg: "path parameter 'Bucket' must be non-empty" } }
  let qp = [(serialize-qp "delimiter" $delimiter "scalar") (serialize-qp "encoding-type" $encoding_type "scalar") (serialize-qp "key-marker" $key_marker "scalar") (serialize-qp "max-keys" $max_keys "scalar") (serialize-qp "prefix" $prefix "scalar") (serialize-qp "version-id-marker" $version_id_marker "scalar") (serialize-qp "MaxKeys" $max_keys_2 "scalar") (serialize-qp "KeyMarker" $key_marker_2 "scalar") (serialize-qp "VersionIdMarker" $version_id_marker_2 "scalar") (serialize-qp "versions" $versions "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({bucket: (encode-path-segment $bucket)} | format pattern "/{bucket}") $qp)
  let accept_val = "text/xml"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-amz-security-token": $x_amz_security_token, "x-amz-expected-bucket-owner": $x_amz_expected_bucket_owner} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"delimiter": $delimiter, "encoding-type": $encoding_type, "key-marker": $key_marker, "max-keys": $max_keys, "prefix": $prefix, "version-id-marker": $version_id_marker, "MaxKeys": $max_keys_2, "KeyMarker": $key_marker_2, "VersionIdMarker": $version_id_marker_2, "versions": $versions} | compact), body: null}
}
...
export def "api list-objects-by-bucket-1" [
  ...
  --max-keys-2: string # Pagination limit (disambiguated-2)
  --continuation-token-2: string # Pagination token (disambiguated-2)
...
export def "api create-restore-object" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"versionId": $version_id, "restore": $restore} | compact), body: $req_body}
}
...
export def "api create-select-object-content" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"select": $select, "select-type": $select_type} | compact), body: $req_body}
}
...
export def "api upload-part" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/xml" $req_body {query: ({"partNumber": $part_number, "uploadId": $upload_id} | compact), body: $req_body}
}
...
export def "api upload-part-copy" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"partNumber": $part_number, "uploadId": $upload_id} | compact), body: null}
}
...
export def "write-get-object-response get" [
  ...
  --x-amz-request-token: string # A single use encrypted token that maps WriteGetObjectResponse to the end user GetObject request.
  --x-amz-fwd-status: int # The integer status code for an HTTP response of a corresponding GetObject request. Status Codes 200 - OK 206 - Partial Content 304 - Not Modified 400 - Bad Request 401 - Unauthorized 403 - Forbidden 404 - Not Found 405 - Method Not Allowed 409 - Conflict 411 - Length Required 412 - Precondition Failed 416 - Range Not Satisfiable 500 - Internal Server Error 503 - Service Unavailable
  --x-amz-fwd-error-code: string # A string that uniquely identifies an error condition. Returned in the <Code> tag of the error XML response for a corresponding GetObject call. Cannot be used with a successful StatusCode header or when the transformed object is provided in the body. All error codes from S3 are sentence-cased. The regular expression (regex) value is "^[A-Z][a-zA-Z]+$".
  --x-amz-fwd-error-message: string # Contains a generic description of the error condition. Returned in the <Message> tag of the error XML response for a corresponding GetObject call. Cannot be used with a successful StatusCode header or when the transformed object is provided in body.
  ...
  --x-amz-fwd-header-x-amz-expiration: string # If the object expiration is configured (see PUT Bucket lifecycle), the response includes this header. It includes the expiry-date and rule-id key-value pairs that provide the object expiration information. The value of the rule-id is URL-encoded.
  ...
  --x-amz-fwd-header-x-amz-object-lock-mode: string@x-amz-fwd-header-x-amz-object-lock-mode-completer # Indicates whether an object stored in Amazon S3 has Object Lock enabled. For more information about S3 Object Lock, see Object Lock (https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-lock.html).
  ...
  --x-amz-fwd-header-x-amz-replication-status: string@x-amz-fwd-header-x-amz-replication-status-completer # Indicates if request involves bucket that is either a source or destination in a Replication rule. For more information about S3 Replication, see Replication (https://docs.aws.amazon.com/AmazonS3/latest/userguide/replication.html).
  ...
  --x-amz-fwd-header-x-amz-server-side-encryption-customer-key-md5: string # 128-bit MD5 digest of customer-provided encryption key used in Amazon S3 to encrypt data stored in S3. For more information, see Protecting data using server-side encryption with customer-provided encryption keys (SSE-C) (https://docs.aws.amazon.com/AmazonS3/latest/userguide/ServerSideEncryptionCustomerKeys.html).
  --x-amz-fwd-header-x-amz-storage-class: string@x-amz-fwd-header-x-amz-storage-class-completer # Provides storage class information of the object. Amazon S3 returns this header for all objects except for S3 Standard storage class objects. For more information, see Storage Classes (https://docs.aws.amazon.com/AmazonS3/latest/dev/storage-class-intro.html).
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/WriteGetObjectResponse")
  let req_body = $body
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else if (($input | is-not-empty) and ($req_body | is-empty)) { $input } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
