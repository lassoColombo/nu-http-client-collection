# Auto-generated client for Sailu Municipal Council, Maharashtra v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/npsailu/3.0.0/openapi.json
# Auth: --token flag or $env.SAILU_MUNICIPAL_COUNCIL_MAHARASHTRA_TOKEN

const BASE_URL = "https://apisetu.gov.in/npsailu/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o SAILU_MUNICIPAL_COUNCIL_MAHARASHTRA_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/npsailu/v3"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "ndcer-certificate create" } } | get name | first)
...
# operationId: ndcer
# --certificateParameters shape: {appid: string}
...
export def "ndcer-certificate create" [
  ...
  --certificate-parameters: record # shape: {appid: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o SAILU_MUNICIPAL_COUNCIL_MAHARASHTRA_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o SAILU_MUNICIPAL_COUNCIL_MAHARASHTRA_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
