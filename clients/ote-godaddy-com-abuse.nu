...
# Source: https://api.apis.guru/v2/specs/ote-godaddy.com/abuse/1.0.0/openapi.json
...
def type-completer [] { ["A_RECORD" "CHILD_ABUSE" "CONTENT" "FRAUD_WIRE" "IP_BLOCK" "MALWARE" "NETWORK_ABUSE" "PHISHING" "SPAM"] }
...
export def "abuse-tickets list" [
  ...
  --source-domain-or-ip: string # The domain name or ip address the abuse originated from (format: host-name-or-ip-address)
  --target: string # The brand/company the abuse is targeting. ie: brand name/bank name
  --created-start: string # The earliest abuse ticket creation date to pull abuse tickets for (format: iso-datetime)
  ...
  let full_url = (build-url $base "/v1/abuse/tickets" $qp)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"type": $type, "closed": $closed, "sourceDomainOrIp": $source_domain_or_ip, "target": $target, "createdStart": $created_start, "createdEnd": $created_end, "limit": $limit, "offset": $offset} | compact), body: null}
}
...
export def "abuse-tickets create" [
  ...
  --info-url: string # Reporter URL if housing additional information that may assist the abuse investigator (format: url)
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/v1/abuse/tickets")
  let req_body = {"info": $info, "infoUrl": $info_url, "intentional": $intentional, "proxy": $proxy, "source": $body_source, "target": $target, "type": $type} | compact
...
export def "abuse-tickets get" [
  ...
  let full_url = (build-url $base ({ticket_id: (encode-path-segment $ticket_id)} | format pattern "/v1/abuse/tickets/{ticket_id}"))
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
