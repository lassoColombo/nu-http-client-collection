...
export def "abuse-tickets list" [
  ...
  let full_url = (build-url $base "/v1/abuse/tickets" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"type": $type, "closed": $closed, "sourceDomainOrIp": $source_domain_or_ip, "target": $target, "createdStart": $created_start, "createdEnd": $created_end, "limit": $limit, "offset": $offset} | compact)
...
export def "abuse-tickets create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/v1/abuse/tickets" $auth.query)
...
export def "abuse-tickets get" [
  ...
  let full_url = (build-url $base ({ticket_id: (encode-path-segment $ticket_id)} | format pattern "/v1/abuse/tickets/{ticket_id}") $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
