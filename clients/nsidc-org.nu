...
export def "facets get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"searchTerms": $search_terms, "count": $count, "startIndex": $start_index, "spatial": $spatial, "sortKeys": $sort_keys, "startDate": $start_date, "endDate": $end_date, "facetFilters": $facet_filters, "source": $qp_source} | compact), body: null}
}
...
export def "suggest get" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"q": $q, "source": $qp_source} | compact), body: null}
}
