...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "v1beta-token ststoken" } } | get name | first)
...
export def "v1beta-token ststoken" [
  ...
  --subjectTokenType: string # Required. An identifier that indicates the type of the security token in the `subject_token` parameter. Supported values are `urn:ietf:params:oauth:token-type:jwt`, `urn:ietf:params:oauth:token-type:id_token`, `urn:ietf:params:aws:token-type:aws4_request`, and `urn:ietf:params:oauth:token-type:access_token`.
  ...
  let body = {audience: $audience, grantType: $grantType, options: $options, requestedTokenType: $requestedTokenType, scope: $scope, subjectToken: $subjectToken, subjectTokenType: $subjectTokenType} | compact
