...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o SECURITY_TOKEN_SERVICE_API_TOKEN | default "" }
...
export def "v1beta-token create" [
  ...
  let full_url = (build-url $base "/v1beta/token" $qp $auth.query)
