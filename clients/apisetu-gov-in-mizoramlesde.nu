...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o LABOUR_EMPLOYMENT_SKILL_DEVELOPMENT_AND_ENTREPRENEURSHIP_MIZORAM_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/mizoramlesde/v3"] }
...
# operationId: govid
# --certificateParameters shape: {DOB: string, FullName: string, REGNO: string}
...
export def "govid-certificate create" [
  ...
  --certificate-parameters: record # shape: {DOB: string, FullName: string, REGNO: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o LABOUR_EMPLOYMENT_SKILL_DEVELOPMENT_AND_ENTREPRENEURSHIP_MIZORAM_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o LABOUR_EMPLOYMENT_SKILL_DEVELOPMENT_AND_ENTREPRENEURSHIP_MIZORAM_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
