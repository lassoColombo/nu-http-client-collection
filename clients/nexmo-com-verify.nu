...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o VERIFY_API_TOKEN | default "" }
...
export def "check verify" [
  ...
  let full_url = (build-url $base ({format: (encode-path-segment $format)} | format pattern "/check/{format}") $auth.query)
...
export def "control verify" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($format | is-empty) { error make --unspanned { msg: "path parameter 'format' must be non-empty" } }
  let full_url = (build-url $base ({format: (encode-path-segment $format)} | format pattern "/control/{format}") $auth.query)
...
export def "network-unblock create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/network-unblock" $auth.query)
...
export def "psd2 verify-request" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($format | is-empty) { error make --unspanned { msg: "path parameter 'format' must be non-empty" } }
  let full_url = (build-url $base ({format: (encode-path-segment $format)} | format pattern "/psd2/{format}") $auth.query)
...
export def "search verify" [
  ...
  let full_url = (build-url $base ({format: (encode-path-segment $format)} | format pattern "/search/{format}") $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"api_key": $api_key, "api_secret": $api_secret, "request_id": $request_id, "request_ids": $request_ids} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
