...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o SUBSCRIPTION_V1_DEPRECATED_TOKEN | default "" }
...
export def "subscriptions get-recurrencebyemail" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"email": $email, "cycleStatus": $cycle_status} | compact)
...
export def "subscriptions-me get-selfrecurrence" [
  ...
  let auth = (merge-auth [(build-auth ($token_appkey | default ($env | get -o SUBSCRIPTION_V1_DEPRECATED_APPKEY_TOKEN | default "")) "x-vtex-api-appkey") (build-auth ($token_apptoken | default ($env | get -o SUBSCRIPTION_V1_DEPRECATED_APPTOKEN_TOKEN | default "")) "x-vtex-api-apptoken")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/subscriptions/me" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Content-Type": $content_type, "Accept": $hdr_accept} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "subscriptions-settings get-recurrencesettings" [
  ...
  let auth = (merge-auth [(build-auth ($token_appkey | default ($env | get -o SUBSCRIPTION_V1_DEPRECATED_APPKEY_TOKEN | default "")) "x-vtex-api-appkey") (build-auth ($token_apptoken | default ($env | get -o SUBSCRIPTION_V1_DEPRECATED_APPTOKEN_TOKEN | default "")) "x-vtex-api-apptoken")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/subscriptions/settings" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Content-Type": $content_type, "Accept": $hdr_accept} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "subscriptions get-recurrencebyrecurrence" [
  ...
  let full_url = (build-url $base ({recurrence_id: (encode-path-segment $recurrence_id)} | format pattern "/subscriptions/{recurrence_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Content-Type": $content_type, "Accept": $hdr_accept} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "subscriptions-addresses get-recurrenceaddresses" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($recurrence_id | is-empty) { error make --unspanned { msg: "path parameter 'recurrenceId' must be non-empty" } }
  let full_url = (build-url $base ({recurrence_id: (encode-path-segment $recurrence_id)} | format pattern "/subscriptions/{recurrence_id}/addresses") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Content-Type": $content_type, "Accept": $hdr_accept} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "subscriptions-items create-addrecurrenceitem" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($recurrence_id | is-empty) { error make --unspanned { msg: "path parameter 'recurrenceId' must be non-empty" } }
  let full_url = (build-url $base ({recurrence_id: (encode-path-segment $recurrence_id)} | format pattern "/subscriptions/{recurrence_id}/items") $auth.query)
  let req_body = $body
...
export def "subscriptions-reindex update-reindexrecurrence" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($recurrence_id | is-empty) { error make --unspanned { msg: "path parameter 'recurrenceId' must be non-empty" } }
  let full_url = (build-url $base ({recurrence_id: (encode-path-segment $recurrence_id)} | format pattern "/subscriptions/{recurrence_id}/reindex") $auth.query)
  let req_body = $body
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else if (($input | is-not-empty) and ($req_body | is-empty)) { $input } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "subscriptions-accounts get-paymentaccounts" [
  ...
  let full_url = (build-url $base ({recurrenceid: (encode-path-segment $recurrenceid)} | format pattern "/subscriptions/{recurrenceid}/accounts") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Content-Type": $content_type, "Accept": $hdr_accept} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
