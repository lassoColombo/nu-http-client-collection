...
# Source: https://api.apis.guru/v2/specs/ote-godaddy.com/shoppers/1.0.0/openapi.json
...
export def "shoppers-subaccount create" [
  ...
  let full_url = (build-url $base "/v1/shoppers/subaccount" $auth.query)
...
export def "shoppers delete" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($shopper_id | is-empty) { error make --unspanned { msg: "path parameter 'shopperId' must be non-empty" } }
  let qp = [(serialize-qp "auditClientIp" $audit_client_ip "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({shopper_id: (encode-path-segment $shopper_id)} | format pattern "/v1/shoppers/{shopper_id}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
    url: $full_url
    query: ({"auditClientIp": $audit_client_ip} | compact)
    headers: $auth.headers
    body: null
    content_type: "application/json"
    timeout: ($max_time | default 30min)
...
export def "shoppers update" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($shopper_id | is-empty) { error make --unspanned { msg: "path parameter 'shopperId' must be non-empty" } }
  let full_url = (build-url $base ({shopper_id: (encode-path-segment $shopper_id)} | format pattern "/v1/shoppers/{shopper_id}") $auth.query)
...
export def "shoppers-factors-password update-change" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --secret: string # The secret value used to set a subaccount's password (e.g. P@55w0rd+)
]: any -> record<customerId: string, shopperId: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($shopper_id | is-empty) { error make --unspanned { msg: "path parameter 'shopperId' must be non-empty" } }
  let full_url = (build-url $base ({shopper_id: (encode-path-segment $shopper_id)} | format pattern "/v1/shoppers/{shopper_id}/factors/password") $auth.query)
...
export def "shoppers-status get" [
  ...
  if ($shopper_id | is-empty) { error make --unspanned { msg: "path parameter 'shopperId' must be non-empty" } }
  let qp = [(serialize-qp "auditClientIp" $audit_client_ip "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({shopper_id: (encode-path-segment $shopper_id)} | format pattern "/v1/shoppers/{shopper_id}/status") $qp $auth.query)
  let accept_val = ($accept | default "application/javascript")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
