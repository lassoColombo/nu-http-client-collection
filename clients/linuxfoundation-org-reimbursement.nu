# Auto-generated client for Reimbursements API v1.0
# Source: https://api.apis.guru/v2/specs/linuxfoundation.org/reimbursement/1.0/swagger.json
# Auth: --token flag or $env.REIMBURSEMENTS_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o REIMBURSEMENTS_API_TOKEN | default "" }
...
export def "api-docs get" [
  ...
  let full_url = (build-url $base "/api-docs" $auth.query)
...
export def "reimbursement update" [
  ...
  let full_url = (build-url $base ({project_id: (encode-path-segment $project_id)} | format pattern "/reimbursement/{project_id}") $auth.query)
...
export def "reset reset-policy" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/reset" $auth.query)
