# Auto-generated client for Negotiation API vv1.1.0
# Source: https://api.apis.guru/v2/specs/ebay.com/sell-negotiation/v1.1.0/openapi.json
# Auth: --token flag or $env.NEGOTIATION_API_TOKEN

const BASE_URL = "https://api.ebay.com/sell/negotiation/v1"
const DEFAULT_AUTH = "bearer"

# Build auth: returns {headers: record, query: string}
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o NEGOTIATION_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.ebay.com/sell/negotiation/v1"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "find-eligible-items find" } } | get name | first)
...
export def "find-eligible-items find" [
...
export def "send-offer-to-interested-buyers send" [
  ...
  let req_body = {"allowCounterOffer": $allow_counter_offer, "message": $message, "offerDuration": $offer_duration, "offeredItems": $offered_items} | compact
