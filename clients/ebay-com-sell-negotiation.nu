...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o NEGOTIATION_API_TOKEN | default "" }
...
export def "find-eligible-items find" [
  ...
  let full_url = (build-url $base "/find_eligible_items" $qp $auth.query)
...
export def "send-offer-to-interested-buyers send" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/send_offer_to_interested_buyers" $auth.query)
