# Auto-generated client for groov View Public API vR4.2a
# Source: https://api.apis.guru/v2/specs/opto22.com/groov/R4.2a/swagger.json
# Auth: --token flag or $env.GROOV_VIEW_PUBLIC_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o GROOV_VIEW_PUBLIC_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://localhost/api"] }
def auth-scheme-completer [] { ["query-api_key"] }

# Completers for enum parameters
def minimum-log-level-completer [] { ["DEBUG" "ERROR" "FATAL" "INFO" "TRACE" "WARN"] }
...
]: nothing -> record<currentApiVersion: float, groovAdminVersion: string, groovBoxDescription: string, groovNodeRedVersion: string, isGroovBox: bool, isGroovBoxSetup: bool, isLicenseValid: bool, licenseType: string, minimumApiVersion: float, shouldShowTrialDialog: bool, timezoneName: string, timezoneOffset: float, trialPeriodExpiration: float, warVersion: string> {
...
export def "data-store-devices list" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> table<deviceType: string, id: float, name: string> {
  let auth = (build-auth $token ($auth_scheme | default "query-api_key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/v1/data-store/devices")
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
...
export def "data-store-devices-tags list" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/v1/data-store/devices/{id}/tags"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
...
export def "data-store-read get-batch-tags" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> table<valueType: string> {
  let auth = (build-auth $token ($auth_scheme | default "query-api_key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/v1/data-store/read")
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "data-store-read tag" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --index: float # Table index to start reading at. (format: int32, default: 0)
  --count: float # Number of elements to read from a table. (format: int32, default: 1)
]: nothing -> record<valueType: string> {
  let auth = (build-auth $token ($auth_scheme | default "query-api_key"))
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let qp = [(serialize-qp "index" $index "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/v1/data-store/read/{id}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"index": $index, "count": $count} | compact), body: null}
}
...
export def "data-store-tags list" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "query-api_key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/v1/data-store/tags")
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
...
export def "data-store-write tag" [
  ...
  --index: float # For array tags, the index to write the value to. (format: int32, default: 0)
]: nothing -> record<valueType: string> {
  let auth = (build-auth $token ($auth_scheme | default "query-api_key"))
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let qp = [(serialize-qp "value" $value "scalar") (serialize-qp "index" $index "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/v1/data-store/write/{id}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"value": $value, "index": $index} | compact), body: null}
}
...
export def "logging-groov-logs-json download" [
  ...
  let full_url = (build-url $base "/v1/logging/groovLogs.json" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"minimum-log-level": $minimum_log_level, "last-timestamp": $last_timestamp, "filter": $filter} | compact), body: null}
}
...
export def "logging-groov-logs-txt download-text" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "minimum-log-level" $minimum_log_level "scalar") (serialize-qp "last-timestamp" $last_timestamp "scalar") (serialize-qp "filter" $filter "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/v1/logging/groovLogs.txt" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"minimum-log-level": $minimum_log_level, "last-timestamp": $last_timestamp, "filter": $filter} | compact), body: null}
}
...
export def "whoami get-who-am-i" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> record<apiKey: string, email: string, groups: list<float>, id: float, needsPasswordChange: bool, role: string, username: string> {
  let auth = (build-auth $token ($auth_scheme | default "query-api_key"))
  let base = ($base_url | default $BASE_URL)
