# Auto-generated client for Football Prediction API v2
# Source: https://api.apis.guru/v2/specs/rapidapi.com/football-prediction/2/openapi.json
# Auth: --token flag or $env.FOOTBALL_PREDICTION_API_TOKEN

const BASE_URL = "https://football-prediction-api.p.rapidapi.com"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o FOOTBALL_PREDICTION_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://football-prediction-api.p.rapidapi.com"] }
...
export def "list-federations get" [
  ...
  let full_url = (build-url $base "/api/v2/list-federations" $auth.query)
...
export def "list-markets get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v2/list-markets" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"X-RapidApi-Key": $x_rapid_api_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "performance-stats get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v2/performance-stats" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"X-RapidApi-Key": $x_rapid_api_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "predictions list" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v2/predictions" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"X-RapidApi-Key": $x_rapid_api_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "predictions get" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/api/v2/predictions/{id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
