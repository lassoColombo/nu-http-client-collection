...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o TRIP_PURPOSE_PREDICTION_TOKEN | default "" }
...
export def "travel-predictions-trip-purpose get" [
  ...
  let full_url = (build-url $base "/travel/predictions/trip-purpose" $qp $auth.query)
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"originLocationCode": $origin_location_code, "destinationLocationCode": $destination_location_code, "departureDate": $departure_date, "returnDate": $return_date, "searchDate": $search_date} | compact)
