# Auto-generated client for Trip Purpose Prediction v1.1.4
# Source: https://api.apis.guru/v2/specs/amadeus.com/amadeus-trip-purpose-prediction/1.1.4/swagger.json
# Auth: --token flag or $env.TRIP_PURPOSE_PREDICTION_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o TRIP_PURPOSE_PREDICTION_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "travel-predictions-trip-purpose get" } } | get name | first)
...
export def "travel-predictions-trip-purpose get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"originLocationCode": $origin_location_code, "destinationLocationCode": $destination_location_code, "departureDate": $departure_date, "returnDate": $return_date, "searchDate": $search_date} | compact), body: null}
}
