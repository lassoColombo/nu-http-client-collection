...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o FEATURECLIENT_TOKEN | default "" }
...
export def "providers-microsoft-features-operations ListOperations" [
  ...
  let full_url = (build-url $base "/providers/Microsoft.Features/operations" $qp)
