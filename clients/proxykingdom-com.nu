# Auto-generated client for ProxyKingdom-Api vv1
# Source: https://api.apis.guru/v2/specs/proxykingdom.com/v1/openapi.json
# Auth: --token flag or $env.PROXYKINGDOM_API_TOKEN

const BASE_URL = "https://api.proxykingdom.com"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o PROXYKINGDOM_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.proxykingdom.com"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "proxy get" } } | get name | first)
...
export def "proxy get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"Token": $qp_token, "Address": $address, "Port": $port, "Protocol": $protocol, "AccessType": $access_type, "ResponseTime": $response_time, "IsSsl": $is_ssl, "Uptime": $uptime, "Country": $country, "Continent": $continent, "Timezone": $timezone, "LastTested": $last_tested} | compact), body: null}
}
