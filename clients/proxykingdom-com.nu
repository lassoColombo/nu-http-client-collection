...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o PROXYKINGDOM_API_TOKEN | default "" }
...
export def "proxy get" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"Token": $qp_token, "Address": $address, "Port": $port, "Protocol": $protocol, "AccessType": $access_type, "ResponseTime": $response_time, "IsSsl": $is_ssl, "Uptime": $uptime, "Country": $country, "Continent": $continent, "Timezone": $timezone, "LastTested": $last_tested} | compact)
