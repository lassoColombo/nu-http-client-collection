...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o NPR_IDENTITY_SERVICE_TOKEN | default "" }
...
export def "following create" [
  ...
  let full_url = (build-url $base "/v2/following" $auth.query)
...
export def "stations update" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/v2/stations" $auth.query)
  let req_body = $body
...
export def "user-inherit create-from-temp" [
  ...
  let full_url = (build-url $base "/v2/user/inherit" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Authorization": $authorization} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "post"
    url: $full_url
    query: ({"temp_user": $temp_user} | compact)
    headers: $auth.headers
    body: null
    content_type: "application/json"
    timeout: ($max_time | default 30min)
