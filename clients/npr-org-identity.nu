# Auto-generated client for NPR Identity Service v2
# Source: https://api.apis.guru/v2/specs/npr.org/identity/2/swagger.json
# Auth: --token flag or $env.NPR_IDENTITY_SERVICE_TOKEN

const BASE_URL = "https://identity.api.npr.org"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o NPR_IDENTITY_SERVICE_TOKEN | default "" }
...
def base-url-completer [] { ["https://identity.api.npr.org"] }
...
export def "following create" [
  ...
  --notif-following: list<string> # The topic in Firebase Cloud Messaging to which the device should subscribe if it supports notifications and the user wants notifications about the podcasts they follow.
  --notif-rated: list<string> # The topic in Firebase Cloud Messaging to which the device should subscribe if it supports notifications and the user wants notifications about the podcasts they have highly rated.
  --rating: float # The user's average rating for this affiliation on a scale of 0-1. Absent if user never listened to the aggregation. (format: float)
  ...
  let req_body = {"daysSinceLastListen": $days_since_last_listen, "following": $following, "href": $href, "id": $id, "notif_following": $notif_following, "notif_rated": $notif_rated, "rating": $rating, "title": $title} | compact
...
export def "user-inherit create-from-temp" [
  ...
  let extra_headers = {"Authorization": $authorization} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"temp_user": $temp_user} | compact), body: null}
}
