...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o WEBHOOK_API_TOKEN | default "" }
...
export def "webhook-logs list-event" [
  ...
  let full_url = (build-url $base "/webhook/logs" $qp $auth.query)
...
export def "webhook-w create-resolve" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id), service_id: (encode-path-segment $service_id)} | format pattern "/webhook/w/{id}/{service_id}") $qp $auth.query)
  let req_body = $body
...
export def "webhook-webhooks list" [
  ...
  let full_url = (build-url $base "/webhook/webhooks" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-apideck-app-id": $x_apideck_app_id} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "webhook-webhooks create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/webhook/webhooks" $auth.query)
...
export def "webhook-webhooks delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/webhook/webhooks/{id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-apideck-app-id": $x_apideck_app_id} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "webhook-webhooks-execute create" [
  ...
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  if ($service_id | is-empty) { error make --unspanned { msg: "path parameter 'serviceId' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id), service_id: (encode-path-segment $service_id)} | format pattern "/webhook/webhooks/{id}/execute/{service_id}") $auth.query)
  let req_body = $body
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else if ($input | describe | str starts-with "list") { $input } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "webhook-webhooks-x create-short-execute" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id), service_id: (encode-path-segment $service_id)} | format pattern "/webhook/webhooks/{id}/x/{service_id}") $qp $auth.query)
  let req_body = $body
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else if ($input | describe | str starts-with "list") { $input } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
