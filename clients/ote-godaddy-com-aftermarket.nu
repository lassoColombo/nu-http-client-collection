...
# Source: https://api.apis.guru/v2/specs/ote-godaddy.com/aftermarket/1.0.0/openapi.json
...
export def "aftermarket-listings delete" [
  ...
  --domains: list<string> # A comma separated list of domain names
]: nothing -> record<listingActionId: int> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "domains" $domains "csv")] | flatten | str join "&"
  let full_url = (build-url $base "/v1/aftermarket/listings" $qp)
  let accept_val = ($accept | default "application/javascript")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"domains": $domains} | compact), body: null}
}
...
export def "aftermarket-listings-expiry create" [
  ...
  --accept: string@accept-completer # Response content type
  --body: list
]: any -> record<listingActionId: int> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/v1/aftermarket/listings/expiry")
  let req_body = $body
