...
export def "aftermarket-listings delete" [
  ...
  let full_url = (build-url $base "/v1/aftermarket/listings" $qp $auth.query)
  let accept_val = ($accept | default "application/javascript")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
    url: $full_url
    query: ({"domains": $domains} | compact)
...
export def "aftermarket-listings-expiry create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/v1/aftermarket/listings/expiry" $auth.query)
  let req_body = $body
