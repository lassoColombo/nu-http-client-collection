...
export def "account-numbers get-owned" [
  ...
  let full_url = (build-url $base "/account/numbers" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"application_id": $application_id, "has_application": $has_application, "country": $country, "pattern": $pattern, "search_pattern": $search_pattern, "size": $size, "index": $index} | compact)
...
export def "number-buy create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o NUMBERS_API_APIKEY_TOKEN | default "")) "query-api_key") (build-auth ($token_apisecret | default ($env | get -o NUMBERS_API_APISECRET_TOKEN | default "")) "query-api_secret")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/number/buy" $auth.query)
...
export def "number-cancel cancel" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o NUMBERS_API_APIKEY_TOKEN | default "")) "query-api_key") (build-auth ($token_apisecret | default ($env | get -o NUMBERS_API_APISECRET_TOKEN | default "")) "query-api_secret")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/number/cancel" $auth.query)
  let req_body = {"country": $country, "msisdn": $msisdn, "target_api_key": $target_api_key} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "number-search get-available" [
  ...
  let full_url = (build-url $base "/number/search" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"country": $country, "type": $type, "pattern": $pattern, "search_pattern": $search_pattern, "features": $features, "size": $size, "index": $index} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "number-update update" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o NUMBERS_API_APIKEY_TOKEN | default "")) "query-api_key") (build-auth ($token_apisecret | default ($env | get -o NUMBERS_API_APISECRET_TOKEN | default "")) "query-api_secret")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/number/update" $auth.query)
