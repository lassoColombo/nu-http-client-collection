...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o SHOP_TOKEN | default "" }
...
export def "openai-details get" [
  ...
  let full_url = (build-url $base "/openai/details" $qp $auth.query)
...
export def "openai-search list" [
  ...
  let full_url = (build-url $base "/openai/search" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"query": $query, "price_min": $price_min, "price_max": $price_max, "similar_to_id": $similar_to_id, "num_results": $num_results} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
