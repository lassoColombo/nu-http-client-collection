# Auto-generated client for Shop vv1
# Source: https://api.apis.guru/v2/specs/shop.app/v1/openapi.json
# Auth: --token flag or $env.SHOP_TOKEN

const BASE_URL = "https://server.shop.app"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o SHOP_TOKEN | default "" }
...
def base-url-completer [] { ["https://server.shop.app"] }
...
export def "openai-details get" [
  ...
  let full_url = (build-url $base "/openai/details" $qp)
...
export def "openai-search list" [
  ...
  --num-results: string # How many results to return. Defaults to 5. It can be a number between 1 and 10.
]: nothing -> record<results: table<currency_code: string, description: string, price: float, title: string, url: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"query": $query, "price_min": $price_min, "price_max": $price_max, "similar_to_id": $similar_to_id, "num_results": $num_results} | compact), body: null}
}
