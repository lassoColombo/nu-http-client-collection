...
export def "nh-art get" [
  ...
  let full_url = (build-url $base $"/nh/art/($artwork)" $qp)
  let extra_headers = {"X-API-KEY": $X_API_KEY, "Accept-Version": $Accept_Version} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "nh-bugs get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "thumbsize" $thumbsize "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/nh/bugs/($bug)" $qp)
  let extra_headers = {"X-API-KEY": $X_API_KEY, "Accept-Version": $Accept_Version} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "nh-clothing get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "thumbsize" $thumbsize "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/nh/clothing/($clothing)" $qp)
  let extra_headers = {"X-API-KEY": $X_API_KEY, "Accept-Version": $Accept_Version} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "nh-fish get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "thumbsize" $thumbsize "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/nh/fish/($fish)" $qp)
  let extra_headers = {"X-API-KEY": $X_API_KEY, "Accept-Version": $Accept_Version} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "nh-fossils-all get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "thumbsize" $thumbsize "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/nh/fossils/all/($fossil)" $qp)
  let extra_headers = {"X-API-KEY": $X_API_KEY, "Accept-Version": $Accept_Version} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "nh-fossils-groups get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "thumbsize" $thumbsize "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/nh/fossils/groups/($fossil_group)" $qp)
  let extra_headers = {"X-API-KEY": $X_API_KEY, "Accept-Version": $Accept_Version} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "nh-fossils-individuals get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "thumbsize" $thumbsize "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/nh/fossils/individuals/($fossil)" $qp)
  let extra_headers = {"X-API-KEY": $X_API_KEY, "Accept-Version": $Accept_Version} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "nh-furniture get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "thumbsize" $thumbsize "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/nh/furniture/($furniture)" $qp)
  let extra_headers = {"X-API-KEY": $X_API_KEY, "Accept-Version": $Accept_Version} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "nh-interior get" [
  ...
  let full_url = (build-url $base $"/nh/interior/($item)" $qp)
  let extra_headers = {"X-API-KEY": $X_API_KEY, "Accept-Version": $Accept_Version} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "nh-items get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "thumbsize" $thumbsize "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/nh/items/($item)" $qp)
  let extra_headers = {"X-API-KEY": $X_API_KEY, "Accept-Version": $Accept_Version} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "nh-photos get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "thumbsize" $thumbsize "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/nh/photos/($item)" $qp)
  let extra_headers = {"X-API-KEY": $X_API_KEY, "Accept-Version": $Accept_Version} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "nh-recipes get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "thumbsize" $thumbsize "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/nh/recipes/($item)" $qp)
  let extra_headers = {"X-API-KEY": $X_API_KEY, "Accept-Version": $Accept_Version} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "nh-sea get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "thumbsize" $thumbsize "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/nh/sea/($sea_creature)" $qp)
  let extra_headers = {"X-API-KEY": $X_API_KEY, "Accept-Version": $Accept_Version} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "nh-tools get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "thumbsize" $thumbsize "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/nh/tools/($tool)" $qp)
  let extra_headers = {"X-API-KEY": $X_API_KEY, "Accept-Version": $Accept_Version} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
