...
export def "subscriptions list" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"productGroupKeys": $product_group_keys, "includes": $includes, "offset": $offset, "limit": $limit, "sort": $qp_sort} | compact)
...
export def "subscriptions-product-groups get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/v1/subscriptions/productGroups" $auth.query)
  let accept_val = ($accept | default "application/javascript")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"X-Shopper-Id": $x_shopper_id, "X-Market-Id": $x_market_id} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "subscriptions cancel" [
  ...
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id)} | format pattern "/v1/subscriptions/{subscription_id}") $auth.query)
