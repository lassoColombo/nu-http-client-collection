...
export def "subscriptions list" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"productGroupKeys": $product_group_keys, "includes": $includes, "offset": $offset, "limit": $limit, "sort": $qp_sort} | compact), body: null}
}
...
export def "subscriptions update" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --payment-profile-id: int # Unique identifier of the payment profile you want to be used for automatic renewal
  --renew-auto: oneof<nothing, bool> # Whether or not you want the Subscription renew automatically
]: any -> any {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
