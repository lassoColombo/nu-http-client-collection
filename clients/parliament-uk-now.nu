# Auto-generated client for Annunciator content API vv1
# Source: https://api.apis.guru/v2/specs/parliament.uk/now/v1/openapi.json
# Auth: --token flag or $env.ANNUNCIATOR_CONTENT_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o ANNUNCIATOR_CONTENT_API_TOKEN | default "" }
...
export def "message-message-current get" [
  ...
  if ($annunciator | is-empty) { error make --unspanned { msg: "path parameter 'annunciator' must be non-empty" } }
  let full_url = (build-url $base ({annunciator: (encode-path-segment $annunciator)} | format pattern "/api/Message/message/{annunciator}/current") $auth.query)
...
export def "message-message get" [
  ...
  let full_url = (build-url $base ({annunciator: (encode-path-segment $annunciator), date: (encode-path-segment $date)} | format pattern "/api/Message/message/{annunciator}/{date}") $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
