...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o JHARKHAND_STATE_BOARD_JHARKHAND_ACADEMIC_COUNCIL_JHARKHAND_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/jac/v3"] }
...
# operationId: hpcer
# --certificateParameters shape: {FullName: string, rollcode: string, rollno: string, year: string}
...
export def "hpcer-certificate create" [
  ...
  --certificate-parameters: record # shape: {FullName: string, rollcode: string, rollno: string, year: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o JHARKHAND_STATE_BOARD_JHARKHAND_ACADEMIC_COUNCIL_JHARKHAND_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o JHARKHAND_STATE_BOARD_JHARKHAND_ACADEMIC_COUNCIL_JHARKHAND_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
