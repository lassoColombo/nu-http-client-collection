# Auto-generated client for Benefits Intake v1.0.0
# Source: https://api.apis.guru/v2/specs/va.gov/benefits/1.0.0/openapi.json
# Auth: --token flag or $env.BENEFITS_INTAKE_TOKEN

const BASE_URL = "https://sandbox-api.va.gov/services/vba_documents/v1"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o BENEFITS_INTAKE_TOKEN | default "" }
...
def base-url-completer [] { ["https://sandbox-api.va.gov/services/vba_documents/v1" "https://api.va.gov/services/vba_documents/v1"] }
...
export def "uploads create-benefits-document" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> record<data: record<attributes: record<code: string, detail: string, guid: string, location: string, status: string, updated_at: string, uploaded_pdf: any>, id: string, type: string>> {
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/uploads" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "uploads-report get-benefits-document-status" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  ids: list<string> # List of IDs for previous document upload submissions
]: any -> record<data: table<attributes: record, id: string, type: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/uploads/report" $auth.query)
...
export def "uploads-validate-document create-benefits" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> record<data: record<attributes: record<status: string>, type: string>> {
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/uploads/validate_document" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "uploads get-benefits-document-status" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/uploads/{id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "uploads-download get-benefits-document" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/uploads/{id}/download") $auth.query)
