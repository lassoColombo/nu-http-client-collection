...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o LOCATIONS_API_TOKEN | default "" }
...
export def "atms-atm get" [
  ...
  let full_url = (build-url $base "/atms/v1/atm" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"PageOffset": $page_offset, "PageLength": $page_length, "AddressLine1": $address_line1, "AddressLine2": $address_line2, "City": $city, "CountrySubdivision": $country_subdivision, "PostalCode": $postal_code, "Country": $country, "Latitude": $latitude, "Longitude": $longitude, "DistanceUnit": $distance_unit, "Radius": $radius, "SupportEMV": $support_emv, "InternationalMaestroAccepted": $international_maestro_accepted} | compact)
...
export def "atms-country get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/atms/v1/country" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "atms-countrysubdivision get" [
  ...
  let full_url = (build-url $base "/atms/v1/countrysubdivision" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "merchants-category get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/merchants/v1/category" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "merchants-country get" [
  ...
  let full_url = (build-url $base "/merchants/v1/country" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "merchants-countrysubdivision get" [
  ...
  let full_url = (build-url $base "/merchants/v1/countrysubdivision" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"details": $details, "country": $country} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "merchants-merchant get" [
  ...
  let full_url = (build-url $base "/merchants/v1/merchant" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"Details": $details, "PageOffset": $page_offset, "PageLength": $page_length, "Category": $category, "AddressLine1": $address_line1, "AddressLine2": $address_line2, "City": $city, "CountrySubdivision": $country_subdivision, "PostalCode": $postal_code, "Country": $country, "Latitude": $latitude, "Longitude": $longitude, "DistanceUnit": $distance_unit, "Radius": $radius, "OfferMerchantID": $offer_merchant_id} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
