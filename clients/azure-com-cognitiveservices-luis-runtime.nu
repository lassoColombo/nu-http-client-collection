...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o LANGUAGE_UNDERSTANDING_INTELLIGENT_SERVICE_LUIS_ENDPOINT_API_FOR_RUNNING_PREDICTIONS_AND_EXTRACTING_USER_INTENTIONS_AND_ENTITIES_FROM_UTTERANCES_TOKEN | default "" }
...
export def "api get-prediction-resolve2" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"q": $q, "timezoneOffset": $timezone_offset, "verbose": $verbose, "staging": $staging, "spellCheck": $spell_check, "bing-spell-check-subscription-key": $bing_spell_check_subscription_key, "log": $log} | compact)
...
export def "api create-prediction-resolve" [
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"timezoneOffset": $timezone_offset, "verbose": $verbose, "staging": $staging, "spellCheck": $spell_check, "bing-spell-check-subscription-key": $bing_spell_check_subscription_key, "log": $log} | compact)
    headers: $auth.headers
    body: $req_body
