...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "api get-prediction-resolve2" } } | get name | first)
...
export def "api get-prediction-resolve2" [
  app_id: string
  ...
  --timezone-offset: float # The timezone offset for the location of the request.
  ...
  --spell-check: oneof<nothing, bool> # Enable spell checking.
  ...
  let qp = [(serialize-qp "q" $q "scalar") (serialize-qp "timezoneOffset" $timezone_offset "scalar") (serialize-qp "verbose" $verbose "scalar") (serialize-qp "staging" $staging "scalar") (serialize-qp "spellCheck" $spell_check "scalar") (serialize-qp "bing-spell-check-subscription-key" $bing_spell_check_subscription_key "scalar") (serialize-qp "log" $log "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({app_id: (encode-path-segment $app_id)} | format pattern "/{app_id}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"q": $q, "timezoneOffset": $timezone_offset, "verbose": $verbose, "staging": $staging, "spellCheck": $spell_check, "bing-spell-check-subscription-key": $bing_spell_check_subscription_key, "log": $log} | compact), body: null}
}
...
export def "api create-prediction-resolve" [
  app_id: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let base = ($base_url | default $BASE_URL)
  if ($app_id | is-empty) { error make --unspanned { msg: "path parameter 'appId' must be non-empty" } }
  let qp = [(serialize-qp "timezoneOffset" $timezone_offset "scalar") (serialize-qp "verbose" $verbose "scalar") (serialize-qp "staging" $staging "scalar") (serialize-qp "spellCheck" $spell_check "scalar") (serialize-qp "bing-spell-check-subscription-key" $bing_spell_check_subscription_key "scalar") (serialize-qp "log" $log "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({app_id: (encode-path-segment $app_id)} | format pattern "/{app_id}") $qp)
  let req_body = $body
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else if (($input | is-not-empty) and ($req_body | is-empty)) { $input } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"timezoneOffset": $timezone_offset, "verbose": $verbose, "staging": $staging, "spellCheck": $spell_check, "bing-spell-check-subscription-key": $bing_spell_check_subscription_key, "log": $log} | compact), body: $req_body}
}
