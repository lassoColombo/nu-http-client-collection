...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o GOV_UK_PAY_API_TOKEN | default "" }
...
export def "payments list" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"reference": $reference, "email": $email, "state": $state, "card_brand": $card_brand, "from_date": $from_date, "to_date": $to_date, "page": $page, "display_size": $display_size, "cardholder_name": $cardholder_name, "first_digits_card_number": $first_digits_card_number, "last_digits_card_number": $last_digits_card_number, "from_settled_date": $from_settled_date, "to_settled_date": $to_settled_date} | compact)
...
export def "payments-capture create" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($payment_id | is-empty) { error make --unspanned { msg: "path parameter 'paymentId' must be non-empty" } }
  let full_url = (build-url $base ({payment_id: (encode-path-segment $payment_id)} | format pattern "/v1/payments/{payment_id}/capture") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "payments-events get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($payment_id | is-empty) { error make --unspanned { msg: "path parameter 'paymentId' must be non-empty" } }
  let full_url = (build-url $base ({payment_id: (encode-path-segment $payment_id)} | format pattern "/v1/payments/{payment_id}/events") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "payments-refunds get" [
  ...
  let full_url = (build-url $base ({payment_id: (encode-path-segment $payment_id), refund_id: (encode-path-segment $refund_id)} | format pattern "/v1/payments/{payment_id}/refunds/{refund_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "refunds list" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"from_date": $from_date, "to_date": $to_date, "from_settled_date": $from_settled_date, "to_settled_date": $to_settled_date, "page": $page, "display_size": $display_size} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
