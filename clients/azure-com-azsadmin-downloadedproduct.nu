...
# Source: https://api.apis.guru/v2/specs/azure.com/azsadmin-DownloadedProduct/2016-01-01/swagger.json
...
export def "subscriptions-resourcegroups-providers-microsoft-azure-bridge-admin-activations-downloaded-products create" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client Api Version. (default: 2016-01-01)
  --properties: record # Properties for aggregate usage. — shape: {legalTerms?: string, links?: list, privacyPolicy?: string, productDetailsProperties?: record, provisioningState?: "Stopped"|"Starting"|"Running"|"Stopping"|"Succeeded"|"Downloading", vmExtensionType?: string, billingPartNumber?: string, compatibility?: record, description?: string, displayName?: string, galleryItemIdentity?: string, iconUris?: record, offer?: string, offerVersion?: string, payloadLength?: int, productKind?: string, productProperties?: record, publisherDisplayName?: string, ... (2 more fields)}
]: any -> record<properties: record<galleryPackageBlobSasUri: string, legalTerms: string, links: list<record>, privacyPolicy: string, productDetailsProperties: record, provisioningState: string, vmExtensionType: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
