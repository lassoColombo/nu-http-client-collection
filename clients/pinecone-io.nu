...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o PINECONE_API_TOKEN | default "" }
...
export def "collections delete" [
  ...
  let full_url = (build-url $base ({collection_name: (encode-path-segment $collection_name)} | format pattern "/collections/{collection_name}") $auth.query)
  let accept_val = "text/plain"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "describe-index-stats get" [
  ...
  let full_url = (build-url $base "/describe_index_stats" $auth.query)
...
export def "indexes delete-index" [
  ...
  let full_url = (build-url $base ({index_name: (encode-path-segment $index_name)} | format pattern "/indexes/{index_name}") $auth.query)
  let accept_val = "text/plain"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "vectors-delete delete" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "api-key"))
  let base = ($base_url | default "https://example-abcd1234.svc.us-east1-gcp.pincone.io")
  let full_url = (build-url $base "/vectors/delete" $auth.query)
...
export def "vectors-fetch get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "api-key"))
  let base = ($base_url | default "https://example-abcd1234.svc.us-east1-gcp.pincone.io")
  let full_url = (build-url $base "/vectors/fetch" $auth.query)
...
export def "vectors-update update" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "api-key"))
  let base = ($base_url | default "https://example-abcd1234.svc.us-east1-gcp.pincone.io")
  let full_url = (build-url $base "/vectors/update" $auth.query)
...
export def "vectors-upsert update" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "api-key"))
  let base = ($base_url | default "https://example-abcd1234.svc.us-east1-gcp.pincone.io")
  let full_url = (build-url $base "/vectors/upsert" $auth.query)
