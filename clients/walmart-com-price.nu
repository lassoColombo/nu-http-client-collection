...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o PRICE_MANAGEMENT_TOKEN | default "" }
...
export def "cppreference create-opt-cap-program-in-price" [
  ...
  let full_url = (build-url $base "/v3/cppreference" $auth.query)
...
export def "price update" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/v3/price" $auth.query)
