...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o TWILIO_MEDIA_TOKEN | default "" }
...
export def "media-processors list" [
  ...
  let full_url = (build-url $base "/v1/MediaProcessors" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"Order": $order, "Status": $status, "PageSize": $page_size, "Page": $page, "PageToken": $page_token} | compact)
...
export def "media-processors create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://media.twilio.com")
  let full_url = (build-url $base "/v1/MediaProcessors" $auth.query)
...
export def "media-processors get" [
  ...
  let full_url = (build-url $base ({sid: (encode-path-segment $sid)} | format pattern "/v1/MediaProcessors/{sid}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "media-recordings list" [
  ...
  let full_url = (build-url $base "/v1/MediaRecordings" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"Order": $order, "Status": $status, "ProcessorSid": $processor_sid, "SourceSid": $source_sid, "PageSize": $page_size, "Page": $page, "PageToken": $page_token} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "media-recordings delete" [
  ...
  let base = ($base_url | default "https://media.twilio.com")
  if ($sid | is-empty) { error make --unspanned { msg: "path parameter 'Sid' must be non-empty" } }
  let full_url = (build-url $base ({sid: (encode-path-segment $sid)} | format pattern "/v1/MediaRecordings/{sid}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "player-streamers list" [
  ...
  let base = ($base_url | default "https://media.twilio.com")
  let qp = [(serialize-qp "Order" $order "scalar") (serialize-qp "Status" $status "scalar") (serialize-qp "PageSize" $page_size "scalar") (serialize-qp "Page" $page "scalar") (serialize-qp "PageToken" $page_token "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/v1/PlayerStreamers" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "player-streamers create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://media.twilio.com")
  let full_url = (build-url $base "/v1/PlayerStreamers" $auth.query)
...
export def "player-streamers get" [
  ...
  let base = ($base_url | default "https://media.twilio.com")
  if ($sid | is-empty) { error make --unspanned { msg: "path parameter 'Sid' must be non-empty" } }
  let full_url = (build-url $base ({sid: (encode-path-segment $sid)} | format pattern "/v1/PlayerStreamers/{sid}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "player-streamers-playback-grant get" [
  ...
  let base = ($base_url | default "https://media.twilio.com")
  if ($sid | is-empty) { error make --unspanned { msg: "path parameter 'Sid' must be non-empty" } }
  let full_url = (build-url $base ({sid: (encode-path-segment $sid)} | format pattern "/v1/PlayerStreamers/{sid}/PlaybackGrant") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
