# Auto-generated client for Institute of Company Secretaries of India v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/icsi/3.0.0/openapi.json
# Auth: --token flag or $env.INSTITUTE_OF_COMPANY_SECRETARIES_OF_INDIA_TOKEN

const BASE_URL = "https://apisetu.gov.in/icsi/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o INSTITUTE_OF_COMPANY_SECRETARIES_OF_INDIA_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/icsi/v3"] }
...
export def "govid-certificate create" [
  ...
  --certificate-parameters: record # shape: {FullName: string, Membership_No: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o INSTITUTE_OF_COMPANY_SECRETARIES_OF_INDIA_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o INSTITUTE_OF_COMPANY_SECRETARIES_OF_INDIA_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
...
export def "mbcer-certificate create" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --certificate-parameters: record # shape: {CertType: string, FullName: string, Membership_No: string}
  --consent-artifact: any # shape: {consent: record, signature: record}
  format: string@format-completer # The format of the certificate in response.
  txn_id: string # A unique transaction id for this request in UUID format. It is used for tracking the request. (format: uuid, e.g. f7f1469c-29b0-4325-9dfc-c567200a70f7)
]: any -> any {
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o INSTITUTE_OF_COMPANY_SECRETARIES_OF_INDIA_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o INSTITUTE_OF_COMPANY_SECRETARIES_OF_INDIA_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/mbcer/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
