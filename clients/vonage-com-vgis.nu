...
export def "self-calls list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"fromDate": $from_date, "toDate": $to_date, "direction": $direction, "states": $states, "offset": $offset, "size": $size, "order": $order, "sort": $qp_sort} | compact), body: null}
}
...
export def "self-calls-count get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"fromDate": $from_date, "toDate": $to_date, "direction": $direction, "states": $states} | compact), body: null}
}
...
export def "self-events list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"types": $types, "fromDate": $from_date, "toDate": $to_date, "direction": $direction, "states": $states, "offset": $offset, "size": $size, "order": $order, "sort": $qp_sort} | compact), body: null}
}
