...
# Source: https://api.apis.guru/v2/specs/uscann.net/1.0/swagger.json
# Auth: --token flag or $env.API_DOCUMENTATION_TOKEN

const BASE_URL = "https://apibeta.uscann.net/apiv1"
...
def base-url-completer [] { ["https://apibeta.uscann.net/apiv1"] }
...
export def "authentication-forgot-password create" [
  ...
  let full_url = (build-url $base "/authentication/forgotPassword" $auth.query)
...
export def "authentication-set-forgot-password update" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/authentication/setForgotPassword" $auth.query)
  let req_body = {"activity": $activity, "password": $password, "token": $body_token} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Content-Type": $content_type} | compact
...
export def "authentication-token create-authenticate-user" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/authentication/token" $auth.query)
...
export def "authentication-validate-mail-token validate" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/authentication/validateMailToken" $auth.query)
  let req_body = {"activity": $activity, "token": $body_token} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Content-Type": $content_type} | compact
