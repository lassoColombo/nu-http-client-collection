...
export def "activatestatementcreditoffer create" [
  ...
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"FId": $f_id, "UserToken": $user_token, "OfferId": $offer_id} | compact), body: null}
}
...
export def "matchedoffers get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"FId": $f_id, "UserToken": $user_token, "Lang": $lang, "MerchantName": $merchant_name, "Category": $category, "OfferType": $offer_type, "PageNumber": $page_number, "ItemsPerPage": $items_per_page} | compact), body: null}
}
...
export def "offerdetails get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"FId": $f_id, "UserToken": $user_token, "OfferId": $offer_id} | compact), body: null}
}
...
export def "redeemedoffers get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"FId": $f_id, "UserToken": $user_token, "Lang": $lang, "PageNumber": $page_number, "ItemsPerPage": $items_per_page} | compact), body: null}
}
...
export def "statementcreditactivationdetail get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"FId": $f_id, "UserToken": $user_token, "ActivationId": $activation_id} | compact), body: null}
}
...
export def "userfeedback create" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"FId": $f_id, "UserToken": $user_token, "OfferId": $offer_id, "Feedback": $feedback} | compact), body: null}
}
...
export def "usersavings get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"FId": $f_id, "UserToken": $user_token} | compact), body: null}
}
...
export def "usertoken get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"FId": $f_id, "AuthInfo": $auth_info} | compact), body: null}
}
