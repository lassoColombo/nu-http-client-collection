...
export def "deal-item get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"category_ids": $category_ids, "commissionable": $commissionable, "delivery_country": $delivery_country, "limit": $limit, "offset": $offset} | compact), body: null}
}
...
export def "event get" [
  ...
  let full_url = (build-url $base ({event_id: (encode-path-segment $event_id)} | format pattern "/event/{event_id}"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"X-EBAY-C-MARKETPLACE-ID": $x_ebay_c_marketplace_id} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "event-item get" [
  ...
  let extra_headers = {"X-EBAY-C-MARKETPLACE-ID": $x_ebay_c_marketplace_id} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"category_ids": $category_ids, "delivery_country": $delivery_country, "event_ids": $event_ids, "limit": $limit, "offset": $offset} | compact), body: null}
}
