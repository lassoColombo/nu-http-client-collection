# Auto-generated client for Azure Resource Graph v2019-04-01
# Source: https://api.apis.guru/v2/specs/azure.com/resourcegraph/2019-04-01/swagger.json
# Auth: --token flag or $env.AZURE_RESOURCE_GRAPH_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AZURE_RESOURCE_GRAPH_TOKEN | default "" }
...
export def "providers-microsoft-resource-graph-operations list" [
  ...
  let full_url = (build-url $base "/providers/Microsoft.ResourceGraph/operations" $qp $auth.query)
...
# Docs: https://aka.ms/resource-graph/learntoquery — Learn more about the query syntax here
...
export def "providers-microsoft-resource-graph-resources create" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  subscriptions: list<string> # Azure subscriptions against which to execute the query.
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/providers/Microsoft.ResourceGraph/resources" $qp $auth.query)
  let req_body = {"facets": $facets, "options": $options, "query": $query, "subscriptions": $subscriptions} | compact
