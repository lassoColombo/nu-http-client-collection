...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o CUSTOM_IMAGE_SEARCH_CLIENT_TOKEN | default "" }
...
export def "images-search list-custom-instance" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"customConfig": $custom_config, "aspect": $aspect, "color": $color, "cc": $cc, "count": $count, "freshness": $freshness, "height": $height, "id": $id, "imageContent": $image_content, "imageType": $image_type, "license": $license, "mkt": $mkt, "maxFileSize": $max_file_size, "maxHeight": $max_height, "maxWidth": $max_width, "minFileSize": $min_file_size, "minHeight": $min_height, "minWidth": $min_width, "offset": $offset, "q": $q, "safeSearch": $safe_search, "size": $size, "setLang": $set_lang, "width": $width} | compact)
