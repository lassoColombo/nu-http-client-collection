# Auto-generated client for Crediwatch's Covid APIs v1.3.0
# Source: https://api.apis.guru/v2/specs/crediwatch.com/covid19/1.3.0/openapi.json
# Auth: --token flag or $env.CREDIWATCH_S_COVID_APIS_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o CREDIWATCH_S_COVID_APIS_TOKEN | default "" }
...
export def "status-call get" [
  ...
  let full_url = (build-url $base "/api/v1/status/call" $qp $auth.query)
...
export def "status-sku get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "query" $query "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/api/v1/status/sku" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "verify-name get" [
  ...
  let full_url = (build-url $base "/api/v1/verify/name" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"query": $query, "state": $state, "city": $city} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "verify-phone get-business" [
  ...
  let full_url = (build-url $base "/api/v1/verify/phone" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"query": $query, "locking_period": $locking_period} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "verify-sku get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "query" $query "scalar") (serialize-qp "locking_period" $locking_period "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/api/v1/verify/sku" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
