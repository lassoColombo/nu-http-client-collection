...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o HOTEL_SEARCH_API_TOKEN | default "" }
...
export def "shopping-hotel-offers get-multi" [
  ...
  let full_url = (build-url $base "/shopping/hotel-offers" $qp $auth.query)
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"hotelIds": $hotel_ids, "adults": $adults, "checkInDate": $check_in_date, "checkOutDate": $check_out_date, "countryOfResidence": $country_of_residence, "roomQuantity": $room_quantity, "priceRange": $price_range, "currency": $currency, "paymentPolicy": $payment_policy, "boardType": $board_type, "includeClosed": $include_closed, "bestRateOnly": $best_rate_only, "lang": $lang} | compact)
...
export def "shopping-hotel-offers get-pricing" [
  ...
  let full_url = (build-url $base ({offer_id: (encode-path-segment $offer_id)} | format pattern "/shopping/hotel-offers/{offer_id}") $qp $auth.query)
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
