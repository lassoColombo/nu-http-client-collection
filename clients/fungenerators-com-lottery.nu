# Auto-generated client for Random Lottery Number generator API v1.5
# Source: https://api.apis.guru/v2/specs/fungenerators.com/lottery/1.5/swagger.json
# Auth: --token flag or $env.RANDOM_LOTTERY_NUMBER_GENERATOR_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o RANDOM_LOTTERY_NUMBER_GENERATOR_API_TOKEN | default "" }
...
export def "lottery-countries get" [
  ...
  let full_url = (build-url $base "/lottery/countries")
...
export def "lottery-draw get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --accept: string@accept-completer # Response content type
  --game: string # Lottery Game Name (format: string)
  --count: int # Number of draws (max 5 per request) (format: integer)
]: nothing -> any {
  let auth = (build-auth $token ($auth_scheme | default "x-fungenerators-api-secret"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "game" $game "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/lottery/draw" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"game": $game, "count": $count} | compact), body: null}
}
...
export def "lottery-supported get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --accept: string@accept-completer # Response content type
  --country: string # Country Name (format: string)
]: nothing -> any {
  let auth = (build-auth $token ($auth_scheme | default "x-fungenerators-api-secret"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "country" $country "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/lottery/supported" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
