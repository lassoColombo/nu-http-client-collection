...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o RANDOM_LOTTERY_NUMBER_GENERATOR_API_TOKEN | default "" }
...
export def "lottery-countries get" [
  ...
  let full_url = (build-url $base "/lottery/countries" $auth.query)
...
export def "lottery-draw get" [
  ...
  let full_url = (build-url $base "/lottery/draw" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"game": $game, "count": $count} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "lottery-supported get" [
  ...
  let full_url = (build-url $base "/lottery/supported" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
