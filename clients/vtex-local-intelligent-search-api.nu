# Auto-generated client for Intelligent Search API v0.1.12
# Source: https://api.apis.guru/v2/specs/vtex.local/Intelligent-Search-API/0.1.12/openapi.json
# Auth: --token flag or $env.INTELLIGENT_SEARCH_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o INTELLIGENT_SEARCH_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://vtex.local" "https://{accountName}.vtexcommercestable.com.br/api/io/_v/api/intelligent-search"] }
...
export def "autocomplete-suggestions get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"query": $query, "locale": $locale} | compact), body: null}
}
...
export def "banners get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($facets | is-empty) { error make --unspanned { msg: "path parameter 'facets' must be non-empty" } }
  let qp = [(serialize-qp "query" $query "scalar") (serialize-qp "locale" $locale "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({facets: (encode-path-segment $facets)} | format pattern "/banners/{facets}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"query": $query, "locale": $locale} | compact), body: null}
}
...
export def "facets get" [
  ...
  let full_url = (build-url $base ({facets: (encode-path-segment $facets)} | format pattern "/facets/{facets}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"query": $query, "locale": $locale, "hideUnavailableItems": $hide_unavailable_items} | compact), body: null}
}
...
export def "product-search get" [
  ...
  let full_url = (build-url $base ({facets: (encode-path-segment $facets)} | format pattern "/product_search/{facets}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"query": $query, "simulationBehavior": $simulation_behavior, "count": $count, "page": $page, "sort": $qp_sort, "locale": $locale, "hideUnavailableItems": $hide_unavailable_items} | compact), body: null}
}
...
export def "top-searches get" [
  ...
  let full_url = (build-url $base "/top_searches" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"locale": $locale} | compact), body: null}
}
