...
# Source: https://api.apis.guru/v2/specs/azure.com/monitor-metrics_API/2018-01-01/swagger.json
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "providers-microsoft-insights-metrics list" } } | get name | first)
...
export def "providers-microsoft-insights-metrics list" [
  resource_uri: string
  ...
  let full_url = (build-url $base ({resource_uri: (encode-path-segment $resource_uri)} | format pattern "/{resource_uri}/providers/microsoft.insights/metrics") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"timespan": $timespan, "interval": $interval, "metricnames": $metricnames, "aggregation": $aggregation, "top": $top, "orderby": $orderby, "$filter": $filter, "resultType": $result_type, "api-version": $api_version, "metricnamespace": $metricnamespace} | compact), body: null}
}
