...
export def "providers-microsoft-insights-metrics list" [
  ...
  let full_url = (build-url $base ({resource_uri: (encode-path-segment $resource_uri)} | format pattern "/{resource_uri}/providers/microsoft.insights/metrics") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"timespan": $timespan, "interval": $interval, "metricnames": $metricnames, "aggregation": $aggregation, "top": $top, "orderby": $orderby, "$filter": $filter, "resultType": $result_type, "api-version": $api_version, "metricnamespace": $metricnamespace} | compact)
