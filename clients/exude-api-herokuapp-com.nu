...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o EXUDE_API_SERVICE_TOKEN | default "" }
...
export def "exude-data create-filter-stoppings" [
  ...
  let full_url = (build-url $base ({type: (encode-path-segment $type)} | format pattern "/exude/{type}/data"))
...
export def "exude-file create-filter-data-stoppings" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base ({type: (encode-path-segment $type)} | format pattern "/exude/{type}/file"))
