# Auto-generated client for Exude API Service v1.0.0
# Source: https://api.apis.guru/v2/specs/exude-api.herokuapp.com/1.0.0/openapi.json
# Auth: --token flag or $env.EXUDE_API_SERVICE_TOKEN

const BASE_URL = "https://exude-api.herokuapp.com"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o EXUDE_API_SERVICE_TOKEN | default "" }
...
def base-url-completer [] { ["https://exude-api.herokuapp.com" "http://localhost:8080"] }
...
export def "exude-data create-filter-stoppings" [
  ...
  let full_url = (build-url $base ({type: (encode-path-segment $type)} | format pattern "/exude/{type}/data") $auth.query)
  let req_body = {"data": $data, "links": $links} | compact
...
export def "exude-file create-filter-data-stoppings" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($type | is-empty) { error make --unspanned { msg: "path parameter 'type' must be non-empty" } }
  let full_url = (build-url $base ({type: (encode-path-segment $type)} | format pattern "/exude/{type}/file") $auth.query)
