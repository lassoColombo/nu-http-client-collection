# Auto-generated client for VA Forms v0.0.0
# Source: https://api.apis.guru/v2/specs/va.gov/forms/0.0.0/openapi.json
# Auth: --token flag or $env.VA_FORMS_TOKEN

const BASE_URL = "https://sandbox-api.va.gov/services/va_forms/v0"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o VA_FORMS_TOKEN | default "" }
...
def base-url-completer [] { ["https://sandbox-api.va.gov/services/va_forms/v0" "https://api.va.gov/services/va_forms/v0"] }
...
export def "forms list" [
  ...
  --query: string # Returns form data based on entered form name.
...
export def "forms find" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  if ($form_name | is-empty) { error make --unspanned { msg: "path parameter 'form_name' must be non-empty" } }
  let full_url = (build-url $base ({form_name: (encode-path-segment $form_name)} | format pattern "/forms/{form_name}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
