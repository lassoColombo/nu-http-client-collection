# Auto-generated client for United India Insurance Company Limited v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/uiic/3.0.0/openapi.json
# Auth: --token flag or $env.UNITED_INDIA_INSURANCE_COMPANY_LIMITED_TOKEN

const BASE_URL = "https://apisetu.gov.in/uiic/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o UNITED_INDIA_INSURANCE_COMPANY_LIMITED_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/uiic/v3"] }
...
# operationId: cripc
# --certificateParameters shape: {DOB: string, FullName: string, UDF1: string, UDF2: string, UDF3: string, UID: string}
...
export def "cripc-certificate create" [
  ...
  --certificate-parameters: record # shape: {DOB: string, FullName: string, UDF1: string, UDF2: string, UDF3: string, UID: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o UNITED_INDIA_INSURANCE_COMPANY_LIMITED_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o UNITED_INDIA_INSURANCE_COMPANY_LIMITED_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
