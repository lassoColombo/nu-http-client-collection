...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AWS_COST_AND_USAGE_REPORT_SERVICE_TOKEN | default "" }
...
export def "api get-report-definitions" [
  ...
  --max-results-body: int # The maximum number of results that AWS returns for the operation. (body field)
