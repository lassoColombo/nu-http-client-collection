# Auto-generated client for Qakka vv1
# Source: https://api.apis.guru/v2/specs/apache.org/qakka/v1/openapi.json
# Auth: --token flag or $env.QAKKA_TOKEN

const BASE_URL = "https://apache.local"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o QAKKA_TOKEN | default "" }
...
def base-url-completer [] { ["https://apache.local"] }
...
export def "queues get-list" [
  ...
  let full_url = (build-url $base "/queues")
...
export def "queues delete" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --confirm: oneof<nothing, bool> # default: false
]: nothing -> record<count: int, message: string, queueMessages: table<contentType: string, createDate: int, data: string, href: string, messageId: string, queueMessageId: string, queueName: string, receivingRegion: string, sendingRegion: string>, queues: table<name: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"confirm": $confirm} | compact), body: null}
}
...
export def "queues-data get-message" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($queue_name | is-empty) { error make --unspanned { msg: "path parameter 'queueName' must be non-empty" } }
  if ($queue_message_id | is-empty) { error make --unspanned { msg: "path parameter 'queueMessageId' must be non-empty" } }
...
export def "queues-messages get-next" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --count: string # Number of messages to get (default: 1)
]: nothing -> record<count: int, message: string, queueMessages: table<contentType: string, createDate: int, data: string, href: string, messageId: string, queueMessageId: string, queueName: string, receivingRegion: string, sendingRegion: string>, queues: table<name: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($queue_name | is-empty) { error make --unspanned { msg: "path parameter 'queueName' must be non-empty" } }
...
export def "queues-messages send-binary" [
  ...
  --delay: string
  --expiration: string
  --content-type: string # Content type of the data to be sent with Queue Message
  --body: list
]: any -> record<count: int, message: string, queueMessages: table<contentType: string, createDate: int, data: string, href: string, messageId: string, queueMessageId: string, queueName: string, receivingRegion: string, sendingRegion: string>, queues: table<name: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($queue_name | is-empty) { error make --unspanned { msg: "path parameter 'queueName' must be non-empty" } }
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/octet-stream" $req_body {query: ({"regions": $regions, "delay": $delay, "expiration": $expiration, "contentType": $content_type} | compact), body: $req_body}
}
