...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o QAKKA_TOKEN | default "" }
...
export def "queues get-list" [
  ...
  let full_url = (build-url $base "/queues" $auth.query)
...
export def "queues delete" [
  ...
  let full_url = (build-url $base ({queue_name: (encode-path-segment $queue_name)} | format pattern "/queues/{queue_name}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
    url: $full_url
    query: ({"confirm": $confirm} | compact)
    headers: $auth.headers
    body: null
    content_type: "application/json"
    timeout: ($max_time | default 30min)
...
export def "queues-config get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($queue_name | is-empty) { error make --unspanned { msg: "path parameter 'queueName' must be non-empty" } }
  let full_url = (build-url $base ({queue_name: (encode-path-segment $queue_name)} | format pattern "/queues/{queue_name}/config") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "queues-data get-message" [
  ...
  let full_url = (build-url $base ({queue_name: (encode-path-segment $queue_name), queue_message_id: (encode-path-segment $queue_message_id)} | format pattern "/queues/{queue_name}/data/{queue_message_id}") $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "queues-messages get-next" [
  ...
  let full_url = (build-url $base ({queue_name: (encode-path-segment $queue_name)} | format pattern "/queues/{queue_name}/messages") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "queues-messages send-binary" [
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"regions": $regions, "delay": $delay, "expiration": $expiration, "contentType": $content_type} | compact)
    headers: $auth.headers
    body: $req_body
...
export def "queues-messages delete-ack" [
  ...
  if ($queue_name | is-empty) { error make --unspanned { msg: "path parameter 'queueName' must be non-empty" } }
  if ($queue_message_id | is-empty) { error make --unspanned { msg: "path parameter 'queueMessageId' must be non-empty" } }
  let full_url = (build-url $base ({queue_name: (encode-path-segment $queue_name), queue_message_id: (encode-path-segment $queue_message_id)} | format pattern "/queues/{queue_name}/messages/{queue_message_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
