...
export def "subscriptions-providers-microsoft-compute-admin-locations-diskmigrationjobs list-disk-migration-jobs" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"status": $status, "api-version": $api_version} | compact), body: null}
}
...
export def "subscriptions-providers-microsoft-compute-admin-locations-diskmigrationjobs create-disk-migration-jobs" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"targetShare": $target_share, "api-version": $api_version} | compact), body: $req_body}
}
