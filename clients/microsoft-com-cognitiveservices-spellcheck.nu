...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o SPELL_CHECK_CLIENT_TOKEN | default "" }
...
def action-type-completer [] { ["Edit" "Load"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "spellcheck create-spell-checker" } } | get name | first)
...
export def "spellcheck create-spell-checker" [
  ...
  --action-type: string@action-type-completer # A string that's used by logging to determine whether the request is coming from an interactive session or a page load. The following are the possible values. 1) Edit—The request is from an interactive session 2) Load—The request is from a page load
  --app-name: string # The unique name of your app. The name must be known by Bing. Do not include this parameter unless you have previously contacted Bing to get a unique app name. To get a unique name, contact your Bing Business Development manager.
  ...
  --client-machine-name: string # A unique name of the device that the request is being made from. Generate a unique value for each device (the value is unimportant). The service uses the ID to help debug issues and improve the quality of corrections.
  --doc-id: string # A unique ID that identifies the document that the text belongs to. Generate a unique value for each document (the value is unimportant). The service uses the ID to help debug issues and improve the quality of corrections.
  ...
  --session-id: string # A unique ID that identifies this user session. Generate a unique value for each user session (the value is unimportant). The service uses the ID to help debug issues and improve the quality of corrections
  ...
  --user-id: string # A unique ID that identifies the user. Generate a unique value for each user (the value is unimportant). The service uses the ID to help debug issues and improve the quality of corrections.
  ...
  let qp = [(serialize-qp "ActionType" $action_type "scalar") (serialize-qp "AppName" $app_name "scalar") (serialize-qp "cc" $cc "scalar") (serialize-qp "ClientMachineName" $client_machine_name "scalar") (serialize-qp "DocId" $doc_id "scalar") (serialize-qp "mkt" $mkt "scalar") (serialize-qp "SessionId" $session_id "scalar") (serialize-qp "SetLang" $set_lang "scalar") (serialize-qp "UserId" $user_id "scalar") (serialize-qp "Mode" $mode "scalar") (serialize-qp "PreContextText" $pre_context_text "scalar") (serialize-qp "PostContextText" $post_context_text "scalar") (serialize-qp "Text" $text "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/spellcheck" $qp $auth.query)
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"ActionType": $action_type, "AppName": $app_name, "cc": $cc, "ClientMachineName": $client_machine_name, "DocId": $doc_id, "mkt": $mkt, "SessionId": $session_id, "SetLang": $set_lang, "UserId": $user_id, "Mode": $mode, "PreContextText": $pre_context_text, "PostContextText": $post_context_text, "Text": $text} | compact)
