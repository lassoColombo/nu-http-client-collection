...
export def "providers-microsoft-security-assessment-metadata list" [
  ...
  let full_url = (build-url $base "/providers/Microsoft.Security/assessmentMetadata" $qp)
...
export def "providers-microsoft-security-assessment-metadata get" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({assessment_metadata_name: (encode-path-segment $assessment_metadata_name)} | format pattern "/providers/Microsoft.Security/assessmentMetadata/{assessment_metadata_name}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version} | compact), body: null}
}
...
export def "subscriptions-providers-microsoft-security-assessment-metadata list" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id)} | format pattern "/subscriptions/{subscription_id}/providers/Microsoft.Security/assessmentMetadata") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version} | compact), body: null}
}
...
export def "subscriptions-providers-microsoft-security-assessment-metadata delete" [
  ...
  if ($assessment_metadata_name | is-empty) { error make --unspanned { msg: "path parameter 'assessmentMetadataName' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), assessment_metadata_name: (encode-path-segment $assessment_metadata_name)} | format pattern "/subscriptions/{subscription_id}/providers/Microsoft.Security/assessmentMetadata/{assessment_metadata_name}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
