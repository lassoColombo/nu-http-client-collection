...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o ENTITY_SEARCH_CLIENT_TOKEN | default "" }
...
export def "entities list" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"cc": $cc, "mkt": $mkt, "q": $q, "ResponseFilter": $response_filter, "ResponseFormat": $response_format, "SafeSearch": $safe_search, "SetLang": $set_lang} | compact)
