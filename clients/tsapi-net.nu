...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o TSAPI_TOKEN | default "" }
...
export def "surveys-interviews get" [
  ...
  let full_url = (build-url $base ({survey_id: (encode-path-segment $survey_id)} | format pattern "/Surveys/{survey_id}/Interviews") $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"start": $start, "maxLength": $max_length} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "surveys-metadata get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($survey_id | is-empty) { error make --unspanned { msg: "path parameter 'surveyId' must be non-empty" } }
  let full_url = (build-url $base ({survey_id: (encode-path-segment $survey_id)} | format pattern "/Surveys/{survey_id}/Metadata") $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
