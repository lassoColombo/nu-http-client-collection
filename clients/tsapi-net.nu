# Auto-generated client for TSAPI vv1
# Source: https://api.apis.guru/v2/specs/tsapi.net/v1/openapi.json
# Auth: --token flag or $env.TSAPI_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o TSAPI_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "surveys get" } } | get name | first)
...
]: nothing -> table<id: string, name: string, title: string> {
...
export def "surveys-interviews get" [
  ...
  let full_url = (build-url $base ({survey_id: (encode-path-segment $survey_id)} | format pattern "/Surveys/{survey_id}/Interviews") $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"start": $start, "maxLength": $max_length} | compact), body: null}
}
...
export def "surveys-metadata get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($survey_id | is-empty) { error make --unspanned { msg: "path parameter 'surveyId' must be non-empty" } }
  let full_url = (build-url $base ({survey_id: (encode-path-segment $survey_id)} | format pattern "/Surveys/{survey_id}/Metadata"))
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
