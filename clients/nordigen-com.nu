...
export def "accounts-premium-transactions get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"country": $country, "date_from": $date_from, "date_to": $date_to} | compact), body: null}
}
...
export def "institutions get-list-supported-in-given-country" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"country": $country, "payments_enabled": $payments_enabled} | compact), body: null}
}
...
export def "payments-creditors list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"account": $account, "address_country": $address_country, "agent": $agent, "currency": $currency, "limit": $limit, "name": $name, "offset": $offset, "type": $type} | compact), body: null}
}
