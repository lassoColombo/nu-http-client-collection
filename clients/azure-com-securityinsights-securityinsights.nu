# Auto-generated client for Security Insights v2020-01-01
# Source: https://api.apis.guru/v2/specs/azure.com/securityinsights-SecurityInsights/2020-01-01/swagger.json
# Auth: --token flag or $env.SECURITY_INSIGHTS_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o SECURITY_INSIGHTS_TOKEN | default "" }
...
def api-version-completer [] { ["2020-01-01"] }
def kind-completer [] { ["Fusion" "MicrosoftSecurityIncidentCreation" "Scheduled"] }
def kind-completer-1 [] { ["AmazonWebServicesCloudTrail" "AzureActiveDirectory" "AzureAdvancedThreatProtection" "AzureSecurityCenter" "MicrosoftCloudAppSecurity" "MicrosoftDefenderAdvancedThreatProtection" "Office365" "ThreatIntelligence"] }
...
export def "providers-microsoft-security-insights-operations list" [
  ...
  let full_url = (build-url $base "/providers/Microsoft.SecurityInsights/operations" $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-operational-insights-workspaces-providers-microsoft-security-insights-alert-rules list" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), workspace_name: (encode-path-segment $workspace_name)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.OperationalInsights/workspaces/{workspace_name}/providers/Microsoft.SecurityInsights/alertRules") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version} | compact), body: null}
}
...
export def "subscriptions-resource-groups-providers-microsoft-operational-insights-workspaces-providers-microsoft-security-insights-alert-rules delete" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), workspace_name: (encode-path-segment $workspace_name), rule_id: (encode-path-segment $rule_id)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.OperationalInsights/workspaces/{workspace_name}/providers/Microsoft.SecurityInsights/alertRules/{rule_id}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "subscriptions-resource-groups-providers-microsoft-operational-insights-workspaces-providers-microsoft-security-insights-alert-rules create-or-update" [
  ...
  kind: string@kind-completer # The kind of the alert rule
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), workspace_name: (encode-path-segment $workspace_name), rule_id: (encode-path-segment $rule_id)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.OperationalInsights/workspaces/{workspace_name}/providers/Microsoft.SecurityInsights/alertRules/{rule_id}") $qp)
  let req_body = {"etag": $etag, "kind": $kind} | compact
...
export def "subscriptions-resource-groups-providers-microsoft-operational-insights-workspaces-providers-microsoft-security-insights-alert-rules-actions delete" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), workspace_name: (encode-path-segment $workspace_name), rule_id: (encode-path-segment $rule_id), action_id: (encode-path-segment $action_id)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.OperationalInsights/workspaces/{workspace_name}/providers/Microsoft.SecurityInsights/alertRules/{rule_id}/actions/{action_id}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version} | compact), body: null}
}
...
export def "subscriptions-resource-groups-providers-microsoft-operational-insights-workspaces-providers-microsoft-security-insights-alert-rules-actions get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string@api-version-completer # API version for the operation
]: nothing -> record<etag: string, properties: record<workflowId: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
  if ($resource_group_name | is-empty) { error make --unspanned { msg: "path parameter 'resourceGroupName' must be non-empty" } }
...
export def "subscriptions-resource-groups-providers-microsoft-operational-insights-workspaces-providers-microsoft-security-insights-alert-rules-actions create-or-update" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string@api-version-completer # API version for the operation
  --properties: record # Action property bag. — shape: {triggerUri?: string, logicAppResourceId: string}
  --etag: string # Etag of the azure resource
]: any -> record<etag: string, properties: record<workflowId: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), workspace_name: (encode-path-segment $workspace_name), rule_id: (encode-path-segment $rule_id), action_id: (encode-path-segment $action_id)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.OperationalInsights/workspaces/{workspace_name}/providers/Microsoft.SecurityInsights/alertRules/{rule_id}/actions/{action_id}") $qp)
  let req_body = {"properties": $properties, "etag": $etag} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"api-version": $api_version} | compact), body: $req_body}
...
export def "subscriptions-resource-groups-providers-microsoft-operational-insights-workspaces-providers-microsoft-security-insights-data-connectors list" [
  ...
  if ($workspace_name | is-empty) { error make --unspanned { msg: "path parameter 'workspaceName' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), workspace_name: (encode-path-segment $workspace_name)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.OperationalInsights/workspaces/{workspace_name}/providers/Microsoft.SecurityInsights/dataConnectors") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version} | compact), body: null}
}
...
export def "subscriptions-resource-groups-providers-microsoft-operational-insights-workspaces-providers-microsoft-security-insights-data-connectors create-or-update" [
  ...
  --api-version: string@api-version-completer # API version for the operation
  --etag: string # Etag of the azure resource
  kind: string@kind-completer-1 # The kind of the data connector
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
