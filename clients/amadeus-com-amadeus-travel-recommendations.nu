...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o TRAVEL_RECOMMENDATIONS_API_TOKEN | default "" }
...
export def "reference-data-recommended-locations get" [
  ...
  let full_url = (build-url $base "/reference-data/recommended-locations" $qp $auth.query)
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"cityCodes": $city_codes, "travelerCountryCode": $traveler_country_code, "destinationCountryCodes": $destination_country_codes} | compact)
