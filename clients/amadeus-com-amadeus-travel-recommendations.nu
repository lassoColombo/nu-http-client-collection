# Auto-generated client for Travel Recommendations API v1.0.3
# Source: https://api.apis.guru/v2/specs/amadeus.com/amadeus-travel-recommendations/1.0.3/openapi.json
# Auth: --token flag or $env.TRAVEL_RECOMMENDATIONS_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o TRAVEL_RECOMMENDATIONS_API_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "reference-data-recommended-locations get" } } | get name | first)
...
export def "reference-data-recommended-locations get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"cityCodes": $city_codes, "travelerCountryCode": $traveler_country_code, "destinationCountryCodes": $destination_country_codes} | compact), body: null}
}
