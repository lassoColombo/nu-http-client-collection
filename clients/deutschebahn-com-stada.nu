...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o STATIONSDATENBEREITSTELLUNG_TOKEN | default "" }
...
export def "stations list" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"offset": $offset, "limit": $limit, "searchstring": $searchstring, "category": $category, "federalstate": $federalstate, "eva": $eva, "ril": $ril, "logicaloperator": $logicaloperator} | compact)
...
export def "stations get" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/stations/{id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "szentralen list" [
  ...
  let full_url = (build-url $base "/szentralen" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "szentralen get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/szentralen/{id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
