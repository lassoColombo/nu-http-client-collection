...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AMAZON_CLOUDSEARCH_DOMAIN_TOKEN | default "" }
...
export def "2013-01-01-search list" [
  ...
  let full_url = (build-url $base "/2013-01-01/search" $qp $auth.query)
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"cursor": $cursor, "expr": $expr, "facet": $facet, "fq": $fq, "highlight": $highlight, "partial": $partial, "q": $q, "q.options": $q_options, "q.parser": $q_parser, "return": $qp_return, "size": $size, "sort": $qp_sort, "start": $start, "stats": $stats, "format": $format, "pretty": $pretty} | compact)
...
export def "2013-01-01-suggest get" [
  ...
  let full_url = (build-url $base "/2013-01-01/suggest" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"X-Amz-Content-Sha256": $x_amz_content_sha256, "X-Amz-Date": $x_amz_date, "X-Amz-Algorithm": $x_amz_algorithm, "X-Amz-Credential": $x_amz_credential, "X-Amz-Security-Token": $x_amz_security_token, "X-Amz-Signature": $x_amz_signature, "X-Amz-SignedHeaders": $x_amz_signed_headers} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "get"
    url: $full_url
    query: ({"q": $q, "suggester": $suggester, "size": $size, "format": $format, "pretty": $pretty} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "2013-01-01-documents-batch upload" [
  ...
  let full_url = (build-url $base "/2013-01-01/documents/batch" $qp $auth.query)
