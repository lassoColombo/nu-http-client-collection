...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o DEPARTMENT_OF_LABOUR_GOVT_OF_PUNJAB_PUNJAB_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/pblabour/v3"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "alsfc-certificate create" } } | get name | first)
...
# operationId: alsfc
# --certificateParameters shape: {UID: string, lino: string}
...
export def "alsfc-certificate create" [
  ...
  --certificate-parameters: record # shape: {UID: string, lino: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o DEPARTMENT_OF_LABOUR_GOVT_OF_PUNJAB_PUNJAB_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o DEPARTMENT_OF_LABOUR_GOVT_OF_PUNJAB_PUNJAB_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
