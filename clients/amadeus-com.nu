...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o FLIGHT_OFFERS_SEARCH_TOKEN | default "" }
...
export def "shopping-flight-offers get" [
  ...
  let full_url = (build-url $base "/shopping/flight-offers" $qp $auth.query)
...
export def "shopping-flight-offers list" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/shopping/flight-offers" $auth.query)
  let req_body = $body
