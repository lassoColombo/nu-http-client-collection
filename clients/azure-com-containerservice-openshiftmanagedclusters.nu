# Auto-generated client for ContainerServiceClient v2019-04-30
# Source: https://api.apis.guru/v2/specs/azure.com/containerservice-openShiftManagedClusters/2019-04-30/swagger.json
...
]: nothing -> record<nextLink: string, value: table<id: string, location: string, name: string, tags: record, type: string, plan: record, properties: record>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: $subscription_id} | format pattern "/subscriptions/{subscription_id}/providers/Microsoft.ContainerService/openShiftManagedClusters") $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-container-service-open-shift-managed-clusters list-by" [
  subscription_id: string
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: $subscription_id, resource_group_name: $resource_group_name} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.ContainerService/openShiftManagedClusters") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "subscriptions-resource-groups-providers-microsoft-container-service-open-shift-managed-clusters delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: $subscription_id, resource_group_name: $resource_group_name, resource_name: $resource_name} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.ContainerService/openShiftManagedClusters/{resource_name}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "subscriptions-resource-groups-providers-microsoft-container-service-open-shift-managed-clusters get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client Api Version.
]: nothing -> record<id: string, location: string, name: string, tags: record, type: string, plan: record<name: string, product: string, promotionCode: string, publisher: string>, properties: record<agentPoolProfiles: list<record>, authProfile: record<identityProviders: list>, clusterVersion: string, fqdn: string, masterPoolProfile: record<count: int, name: string, osType: string, subnetCidr: string, vmSize: string>, networkProfile: record<peerVnetId: string, vnetCidr: string, vnetId: string>, openShiftVersion: string, provisioningState: string, publicHostname: string, routerProfiles: list<record>>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: $subscription_id, resource_group_name: $resource_group_name, resource_name: $resource_name} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.ContainerService/openShiftManagedClusters/{resource_name}") $qp)
...
]: any -> record<id: string, location: string, name: string, tags: record, type: string, plan: record<name: string, product: string, promotionCode: string, publisher: string>, properties: record<agentPoolProfiles: list<record>, authProfile: record<identityProviders: list>, clusterVersion: string, fqdn: string, masterPoolProfile: record<count: int, name: string, osType: string, subnetCidr: string, vmSize: string>, networkProfile: record<peerVnetId: string, vnetCidr: string, vnetId: string>, openShiftVersion: string, provisioningState: string, publicHostname: string, routerProfiles: list<record>>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
...
# --properties shape: {agentPoolProfiles?: list, authProfile?: any, masterPoolProfile?: any, networkProfile?: any, openShiftVersion: string, routerProfiles?: list}
export def "subscriptions-resource-groups-providers-microsoft-container-service-open-shift-managed-clusters create-or-update" [
  ...
  --tags: record # Resource tags
  --plan: any # Used for establishing the purchase context of any 3rd Party artifact through MarketPlace. — shape: {name?: string, product?: string, promotionCode?: string, publisher?: string}
  --properties: any # Properties of the OpenShift managed cluster. — shape: {agentPoolProfiles?: list, authProfile?: any, masterPoolProfile?: any, networkProfile?: any, openShiftVersion: string, routerProfiles?: list}
]: any -> record<id: string, location: string, name: string, tags: record, type: string, plan: record<name: string, product: string, promotionCode: string, publisher: string>, properties: record<agentPoolProfiles: list<record>, authProfile: record<identityProviders: list>, clusterVersion: string, fqdn: string, masterPoolProfile: record<count: int, name: string, osType: string, subnetCidr: string, vmSize: string>, networkProfile: record<peerVnetId: string, vnetCidr: string, vnetId: string>, openShiftVersion: string, provisioningState: string, publicHostname: string, routerProfiles: list<record>>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: $subscription_id, resource_group_name: $resource_group_name, resource_name: $resource_name} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.ContainerService/openShiftManagedClusters/{resource_name}") $qp)
  let body = {"location": $location, "tags": $tags, "plan": $plan, "properties": $properties} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
