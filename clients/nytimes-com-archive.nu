...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o ARCHIVE_API_TOKEN | default "" }
...
export def "archive get" [
  ...
  let full_url = (build-url $base ({year: (encode-path-segment $year), month: (encode-path-segment $month)} | format pattern "/{year}/{month}.json") $auth.query)
