...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AXESSO_API_TOKEN | default "" }
...
export def "amz-amazon-lookup-buy-recommendations request" [
  ...
  let full_url = (build-url $base "/amz/amazon-lookup-buy-recommendations" $qp $auth.query)
...
export def "amz-amazon-lookup-product request" [
  ...
  let full_url = (build-url $base "/amz/amazon-lookup-product" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"url": $url, "size": $size} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "amz-amazon-search-by-keyword list" [
  ...
  let full_url = (build-url $base "/amz/amazon-search-by-keyword" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"keyword": $keyword, "domainCode": $domain_code, "sortBy": $sort_by, "numberOfProducts": $number_of_products} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "amz-sort-options get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/amz/sort-options" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
