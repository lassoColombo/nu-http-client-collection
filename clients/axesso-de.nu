# Auto-generated client for Axesso Api v1.0.0
# Source: https://api.apis.guru/v2/specs/axesso.de/1.0.0/openapi.json
# Auth: --token flag or $env.AXESSO_API_TOKEN

const BASE_URL = "http://api.axesso.de"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o AXESSO_API_TOKEN | default "" }
...
export def "amz-amazon-lookup-product request" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"url": $url, "size": $size} | compact), body: null}
}
...
export def "amz-amazon-search-by-keyword list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"keyword": $keyword, "domainCode": $domain_code, "sortBy": $sort_by, "numberOfProducts": $number_of_products} | compact), body: null}
}
...
export def "amz-sort-options get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> record<responseMessage: string, responseStatus: string, resultSize: int, sortOptions: table<order: int, sortName: string, strategy: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/amz/sort-options")
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
