...
# Source: https://api.apis.guru/v2/specs/azure.com/automation-certificate/2015-10-31/swagger.json
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-certificates ListByAutomationAccount" } } | get name | first)
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-certificates ListByAutomationAccount" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Automation/automationAccounts/($automationAccountName)/certificates" $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-certificates Delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Automation/automationAccounts/($automationAccountName)/certificates/($certificateName)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-certificates Get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client Api Version.
]: nothing -> record<properties: record<creationTime: string, description: string, expiryTime: string, isExportable: bool, lastModifiedTime: string, thumbprint: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Automation/automationAccounts/($automationAccountName)/certificates/($certificateName)" $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-certificates Update" [
  ...
  --properties: any # The properties of the update certificate operation — shape: {description?: string}
]: any -> record<properties: record<creationTime: string, description: string, expiryTime: string, isExportable: bool, lastModifiedTime: string, thumbprint: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-certificates CreateOrUpdate" [
  resourceGroupName: string
  automationAccountName: string
  certificateName: string
  subscriptionId: string
  ...
  --api-version: string # Client Api Version.
  name: string # Gets or sets the name of the certificate.
  properties: any # The properties of the create certificate operation. — shape: {base64Value: string, description?: string, isExportable?: bool, thumbprint?: string}
]: any -> record<properties: record<creationTime: string, description: string, expiryTime: string, isExportable: bool, lastModifiedTime: string, thumbprint: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
