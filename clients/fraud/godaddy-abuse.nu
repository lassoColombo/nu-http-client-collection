...
export def "abuse-tickets list" [
  ...
  --createdEnd: string # The latest abuse ticket creation date to pull abuse tickets for (format: iso-datetime)
  ...
  let qp = [(serialize-qp "type" $type "scalar") (serialize-qp "closed" $closed "scalar") (serialize-qp "sourceDomainOrIp" $sourceDomainOrIp "scalar") (serialize-qp "target" $target "scalar") (serialize-qp "createdStart" $createdStart "scalar") (serialize-qp "createdEnd" $createdEnd "scalar") (serialize-qp "limit" $limit "scalar") (serialize-qp "offset" $offset "scalar")] | flatten | str join "&"
...
export def "abuse-tickets createTicket" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let body = {info: $info, infoUrl: $infoUrl, intentional: $intentional, proxy: $proxy, source: $body_source, target: $target, type: $type} | compact
...
export def "abuse-tickets get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v1/abuse/tickets/($ticketId)")
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
