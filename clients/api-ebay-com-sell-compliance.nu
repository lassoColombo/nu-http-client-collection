...
# Source: https://api.apis.guru/v2/specs/api.ebay.com/sell-compliance/1.4.1/openapi.json
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o COMPLIANCE_API_TOKEN | default "" }
...
export def "listing-violation get" [
  ...
  let full_url = (build-url $base "/listing_violation" $qp $auth.query)
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"compliance_type": $compliance_type, "offset": $offset, "listing_id": $listing_id, "limit": $limit, "filter": $filter} | compact)
...
export def "listing-violation-summary get" [
  ...
  let full_url = (build-url $base "/listing_violation_summary" $qp $auth.query)
  let accept_val = "application/json;charset=UTF-8"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"X-EBAY-C-MARKETPLACE-ID": $x_ebay_c_marketplace_id} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "get"
    url: $full_url
    query: ({"compliance_type": $compliance_type} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "suppress-listing-violation create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/suppress_listing_violation" $auth.query)
