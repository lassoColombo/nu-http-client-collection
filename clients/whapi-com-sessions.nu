...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o SESSIONS_API_TOKEN | default "" }
...
export def "tickets delete-log-out" [
  ...
  let full_url = (build-url $base ({tgt: (encode-path-segment $tgt)} | format pattern "/tickets/{tgt}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"apiKey": $api_key, "apiSecret": $api_secret, "territory": $territory} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "tickets-service-ticket get" [
  ...
  let full_url = (build-url $base ({tgt: (encode-path-segment $tgt)} | format pattern "/tickets/{tgt}/serviceTicket") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"apiKey": $api_key, "apiSecret": $api_secret, "territory": $territory} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "get"
    url: $full_url
    query: ({"languageAsPerTerritory": $language_as_per_territory, "target": $target, "fields": $fields, "include": $include, "exclude": $exclude} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
