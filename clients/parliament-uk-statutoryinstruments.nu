...
export def "business-item get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"LaidPaper": $laid_paper} | compact), body: null}
}
...
export def "proposed-negative-statutory-instrument list" [
  ...
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"Name": $name, "RecommendedForProcedureChange": $recommended_for_procedure_change, "DepartmentId": $department_id, "LayingBodyId": $laying_body_id, "Skip": $skip, "Take": $take} | compact), body: null}
}
