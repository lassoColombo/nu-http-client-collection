...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "subscriptions-resource-groups-providers-microsoft-sql-managed-instances-encryption-protector list" } } | get name | first)
...
export def "subscriptions-resource-groups-providers-microsoft-sql-managed-instances-encryption-protector list" [
...
export def "subscriptions-resource-groups-providers-microsoft-sql-managed-instances-encryption-protector get" [
  ...
  if ($resource_group_name | is-empty) { error make --unspanned { msg: "path parameter 'resourceGroupName' must be non-empty" } }
  if ($managed_instance_name | is-empty) { error make --unspanned { msg: "path parameter 'managedInstanceName' must be non-empty" } }
  if ($encryption_protector_name | is-empty) { error make --unspanned { msg: "path parameter 'encryptionProtectorName' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), managed_instance_name: (encode-path-segment $managed_instance_name), encryption_protector_name: (encode-path-segment $encryption_protector_name)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Sql/managedInstances/{managed_instance_name}/encryptionProtector/{encryption_protector_name}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version} | compact), body: null}
}
...
export def "subscriptions-resource-groups-providers-microsoft-sql-managed-instances-encryption-protector-revalidate create" [
  subscription_id: string
  resource_group_name: string
  managed_instance_name: string
  encryption_protector_name: string
