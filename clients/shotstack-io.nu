# Auto-generated client for Shotstack vv1
# Source: https://api.apis.guru/v2/specs/shotstack.io/v1/openapi.json
# Auth: --token flag or $env.SHOTSTACK_TOKEN

const BASE_URL = "https://api.shotstack.io/v1"
const DEFAULT_AUTH = "x-api-key"

# Build auth: returns {headers: record, query: string}
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o SHOTSTACK_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.shotstack.io/v1" "https://api.shotstack.io/stage" "https://api.shotstack.io/serve/v1" "https://api.shotstack.io/serve/stage"] }
def auth-scheme-completer [] { ["x-api-key"] }

# Completers for enum parameters
def disk-completer [] { ["local" "mount"] }
