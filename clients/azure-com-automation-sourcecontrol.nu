...
# Source: https://api.apis.guru/v2/specs/azure.com/automation-sourceControl/2017-05-15-preview/swagger.json
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-source-controls ListByAutomationAccount" } } | get name | first)
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-source-controls ListByAutomationAccount" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Automation/automationAccounts/($automationAccountName)/sourceControls" $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-source-controls Delete" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Automation/automationAccounts/($automationAccountName)/sourceControls/($sourceControlName)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-source-controls Get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client Api Version.
]: nothing -> record<properties: record<autoSync: bool, branch: string, creationTime: string, description: string, folderPath: string, lastModifiedTime: string, publishRunbook: bool, repoUrl: string, sourceType: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Automation/automationAccounts/($automationAccountName)/sourceControls/($sourceControlName)" $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-source-controls Update" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client Api Version.
  --properties: any # The properties of the update source control — shape: {autoSync?: bool, branch?: string, description?: string, folderPath?: string, publishRunbook?: bool, securityToken?: any}
]: any -> record<properties: record<autoSync: bool, branch: string, creationTime: string, description: string, folderPath: string, lastModifiedTime: string, publishRunbook: bool, repoUrl: string, sourceType: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-source-controls CreateOrUpdate" [
  resourceGroupName: string
  automationAccountName: string
  sourceControlName: string
  subscriptionId: string
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client Api Version.
  properties: any # The properties of the create source control operation. — shape: {autoSync?: bool, branch?: string, description?: string, folderPath?: string, publishRunbook?: bool, repoUrl?: string, securityToken?: any, sourceType?: "VsoGit"|"VsoTfvc"|"GitHub"}
]: any -> record<properties: record<autoSync: bool, branch: string, creationTime: string, description: string, folderPath: string, lastModifiedTime: string, publishRunbook: bool, repoUrl: string, sourceType: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
