...
# Source: https://api.apis.guru/v2/specs/seldon.local/wrapper/0.1/openapi.json
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o SELDON_EXTERNAL_API_TOKEN | default "" }
...
export def "aggregate get" [
  ...
  let full_url = (build-url $base "/aggregate" $qp $auth.query)
...
export def "aggregate create-aggregate2" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/aggregate" $auth.query)
...
export def "predict get-transform-input4" [
  ...
  let full_url = (build-url $base "/predict" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "predict create-transform-input3" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/predict" $auth.query)
  let req_body = {"json": $json} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "send-feedback send-feedback2" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "json" $json "multi")] | flatten | str join "&"
  let full_url = (build-url $base "/send-feedback" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "send-feedback send" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/send-feedback" $auth.query)
  let req_body = {"json": $json} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "transform-input get-input2" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "json" $json "multi")] | flatten | str join "&"
  let full_url = (build-url $base "/transform-input" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "transform-input create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/transform-input" $auth.query)
  let req_body = {"json": $json} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "transform-output get-output2" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "json" $json "multi")] | flatten | str join "&"
  let full_url = (build-url $base "/transform-output" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "transform-output create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/transform-output" $auth.query)
  let req_body = {"json": $json} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
