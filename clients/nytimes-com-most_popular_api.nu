# Auto-generated client for Most Popular API v2.0.0
# Source: https://api.apis.guru/v2/specs/nytimes.com/most_popular_api/2.0.0/openapi.json
# Auth: --token flag or $env.MOST_POPULAR_API_TOKEN

const BASE_URL = "http://api.nytimes.com/svc/mostpopular/v2"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o MOST_POPULAR_API_TOKEN | default "" }
...
def base-url-completer [] { ["http://api.nytimes.com/svc/mostpopular/v2" "https://api.nytimes.com/svc/mostpopular/v2"] }
...
export def "mostemailed get-json" [
  ...
  if ($time_period | is-empty) { error make --unspanned { msg: "path parameter 'time-period' must be non-empty" } }
  let full_url = (build-url $base ({section: (encode-path-segment $section), time_period: (encode-path-segment $time_period)} | format pattern "/mostemailed/{section}/{time_period}.json") $auth.query)
...
export def "mostshared get-json" [
  ...
  if ($section | is-empty) { error make --unspanned { msg: "path parameter 'section' must be non-empty" } }
  if ($time_period | is-empty) { error make --unspanned { msg: "path parameter 'time-period' must be non-empty" } }
  let full_url = (build-url $base ({section: (encode-path-segment $section), time_period: (encode-path-segment $time_period)} | format pattern "/mostshared/{section}/{time_period}.json") $auth.query)
...
export def "mostviewed get-json" [
  ...
  if ($section | is-empty) { error make --unspanned { msg: "path parameter 'section' must be non-empty" } }
  if ($time_period | is-empty) { error make --unspanned { msg: "path parameter 'time-period' must be non-empty" } }
  let full_url = (build-url $base ({section: (encode-path-segment $section), time_period: (encode-path-segment $time_period)} | format pattern "/mostviewed/{section}/{time_period}.json") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
