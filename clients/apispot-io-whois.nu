...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o BULK_WHOIS_API_TOKEN | default "" }
...
export def "batch delete" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/batch/{id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "db list" [
  ...
  let full_url = (build-url $base "/db" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "domains-check check" [
  ...
  let full_url = (build-url $base ({domain: (encode-path-segment $domain)} | format pattern "/domains/{domain}/check") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "domains-rank get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($domain | is-empty) { error make --unspanned { msg: "path parameter 'domain' must be non-empty" } }
  let full_url = (build-url $base ({domain: (encode-path-segment $domain)} | format pattern "/domains/{domain}/rank") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "domains-whois get" [
  ...
  let full_url = (build-url $base ({domain: (encode-path-segment $domain)} | format pattern "/domains/{domain}/whois") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
