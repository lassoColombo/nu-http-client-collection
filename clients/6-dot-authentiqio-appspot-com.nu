# Auto-generated client for Authentiq API v6
# Source: https://api.apis.guru/v2/specs/6-dot-authentiqio.appspot.com/6/openapi.json
# Auth: --token flag or $env.AUTHENTIQ_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o AUTHENTIQ_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://6-dot-authentiqio.appspot.com"] }
def auth-scheme-completer [] { ["bearer"] }

# Completers for enum parameters
def accept-completer [] { ["application/json" "application/jwt"] }
...
export def "key delete-nosecret" [
  ...
  let qp = [(serialize-qp "email" $email "scalar") (serialize-qp "phone" $phone "scalar") (serialize-qp "code" $code "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/key" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"email": $email, "phone": $phone, "code": $code} | compact), body: null}
}
...
export def "key create" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --body: any
]: any -> record<secret: string, status: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/jwt" $req_body {query: {}, body: $req_body}
}
...
export def "key delete" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"secret": $secret} | compact), body: null}
}
...
export def "key update-bind" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/jwt" $req_body {query: {}, body: $req_body}
}
...
export def "login push-request" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/jwt" $req_body {query: ({"callback": $callback} | compact), body: $req_body}
}
...
export def "scope request-sign" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --test: int # test only mode, using test issuer
  --body: any
]: any -> record<job: string, status: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/jwt" $req_body {query: ({"test": $test} | compact), body: $req_body}
}
...
export def "scope delete-sign" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($job | is-empty) { error make --unspanned { msg: "path parameter 'job' must be non-empty" } }
...
export def "scope update-sign" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> record<jwt: string, status: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($job | is-empty) { error make --unspanned { msg: "path parameter 'job' must be non-empty" } }
  let full_url = (build-url $base ({job: (encode-path-segment $job)} | format pattern "/scope/{job}"))
