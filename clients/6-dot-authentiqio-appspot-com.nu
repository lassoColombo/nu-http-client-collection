...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AUTHENTIQ_API_TOKEN | default "" }
...
export def "key delete-nosecret" [
  ...
  let full_url = (build-url $base "/key" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
    url: $full_url
    query: ({"email": $email, "phone": $phone, "code": $code} | compact)
...
export def "key create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/key" $auth.query)
  let req_body = $body
...
export def "key delete" [
  ...
  let full_url = (build-url $base ({pk: (encode-path-segment $pk)} | format pattern "/key/{pk}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "key get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($pk | is-empty) { error make --unspanned { msg: "path parameter 'PK' must be non-empty" } }
  let full_url = (build-url $base ({pk: (encode-path-segment $pk)} | format pattern "/key/{pk}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "scope request-sign" [
  ...
  let full_url = (build-url $base "/scope" $qp $auth.query)
  let req_body = $body
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else if (($input | is-not-empty) and ($req_body | is-empty)) { $input } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "scope delete-sign" [
  ...
  let full_url = (build-url $base ({job: (encode-path-segment $job)} | format pattern "/scope/{job}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "scope get-sign-head" [
  ...
  if $dry_run { return ({dry_run: true} | merge $req) }
  send-head $req $insecure $allow_errors $full [200 204]
}
