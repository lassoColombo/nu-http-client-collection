...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o NETWORKEXPERIMENTS_TOKEN | default "" }
...
export def "subscriptions-resource-groups-providers-microsoft-network-network-experiment-profiles-experiments-latency-scorecard get-reports" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version, "endDateTimeUTC": $end_date_time_utc, "country": $country, "aggregationInterval": $aggregation_interval} | compact), body: null}
}
...
export def "subscriptions-resource-groups-providers-microsoft-network-network-experiment-profiles-experiments-timeseries get-reports" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version, "startDateTimeUTC": $start_date_time_utc, "endDateTimeUTC": $end_date_time_utc, "aggregationInterval": $aggregation_interval, "timeseriesType": $timeseries_type, "endpoint": $endpoint, "country": $country} | compact), body: null}
}
