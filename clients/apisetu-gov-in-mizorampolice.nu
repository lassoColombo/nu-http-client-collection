# Auto-generated client for Mizoram Police, Mizoram v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/mizorampolice/3.0.0/openapi.json
# Auth: --token flag or $env.MIZORAM_POLICE_MIZORAM_TOKEN

const BASE_URL = "https://apisetu.gov.in/mizorampolice/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o MIZORAM_POLICE_MIZORAM_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/mizorampolice/v3"] }
...
# operationId: govid
# --certificateParameters shape: {CARD_NO: string, FullName: string}
...
export def "govid-certificate create" [
  ...
  --certificate-parameters: record # shape: {CARD_NO: string, FullName: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o MIZORAM_POLICE_MIZORAM_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o MIZORAM_POLICE_MIZORAM_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
