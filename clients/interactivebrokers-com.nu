...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o IBKR_3RD_PARTY_WEB_API_TOKEN | default "" }
...
export def "accounts-order-impact create" [
  ...
  let full_url = (build-url $base ({account: (encode-path-segment $account)} | format pattern "/accounts/{account}/order_impact") $auth.query)
  ...
  if $dry_run { return ({dry_run: true} | merge $req) }
  send-post $req $req_body $insecure $raw $allow_errors $full [200 202 204]
}
...
export def "accounts-orders list" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($account | is-empty) { error make --unspanned { msg: "path parameter 'account' must be non-empty" } }
  let full_url = (build-url $base ({account: (encode-path-segment $account)} | format pattern "/accounts/{account}/orders") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "accounts-orders delete" [
  ...
  let full_url = (build-url $base ({account: (encode-path-segment $account), customer_order_id: (encode-path-segment $customer_order_id)} | format pattern "/accounts/{account}/orders/{customer_order_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "accounts-orders update" [
  ...
  if $dry_run { return ({dry_run: true} | merge $req) }
  send-put $req $req_body $insecure $raw $allow_errors $full [200 202 204]
}
...
export def "accounts-positions get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($account | is-empty) { error make --unspanned { msg: "path parameter 'account' must be non-empty" } }
  let full_url = (build-url $base ({account: (encode-path-segment $account)} | format pattern "/accounts/{account}/positions") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "accounts-summary get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($account | is-empty) { error make --unspanned { msg: "path parameter 'account' must be non-empty" } }
  let full_url = (build-url $base ({account: (encode-path-segment $account)} | format pattern "/accounts/{account}/summary") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "marketdata-exchange-components get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "portal"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/marketdata/exchange_components" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "marketdata-snapshot get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> table<Bid: record<market: float, price: float, size: float>, Closing: record<price: float>, Complete: bool, Conid: string, Offer: record<market: float, price: float, size: float>, Temporality: float, Trade: record<market: float, price: float, size: float, time: float>> {
  let auth = (build-auth $token ($auth_scheme | default "portal"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/marketdata/snapshot" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "oauth-live-session-token create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "portal"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/oauth/live_session_token" $auth.query)
...
export def "oauth-request-token create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "portal"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/oauth/request_token" $auth.query)
...
export def "secdef get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> table<CompanyName: string, ContractId: float, Currency: string, Exchange: string, SecurityType: string, Ticker: string> {
  let auth = (build-auth $token ($auth_scheme | default "portal"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/secdef" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
