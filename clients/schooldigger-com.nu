...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o SCHOOLDIGGER_API_V1_TOKEN | default "" }
...
export def "districts get" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/v1/districts/{id}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"appID": $app_id, "appKey": $app_key} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "rankings-districts get-rank" [
  ...
  let full_url = (build-url $base ({st: (encode-path-segment $st)} | format pattern "/v1/rankings/districts/{st}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"year": $year, "page": $page, "perPage": $per_page, "appID": $app_id, "appKey": $app_key} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "rankings-schools get-rank" [
  ...
  let full_url = (build-url $base ({st: (encode-path-segment $st)} | format pattern "/v1/rankings/schools/{st}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"year": $year, "level": $level, "page": $page, "perPage": $per_page, "appID": $app_id, "appKey": $app_key} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "schools get-school10" [
  ...
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let qp = [(serialize-qp "appID" $app_id "scalar") (serialize-qp "appKey" $app_key "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/v1/schools/{id}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
