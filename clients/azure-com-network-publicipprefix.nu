...
# Source: https://api.apis.guru/v2/specs/azure.com/network-publicIpPrefix/2019-07-01/swagger.json
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "subscriptions-providers-microsoft-network-public-ip-prefixes list-list" } } | get name | first)
...
export def "subscriptions-providers-microsoft-network-public-ip-prefixes list-list" [
  ...
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id)} | format pattern "/subscriptions/{subscription_id}/providers/Microsoft.Network/publicIPPrefixes") $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-network-public-ip-prefixes list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Network/publicIPPrefixes") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json"
}
...
export def "subscriptions-resource-groups-providers-microsoft-network-public-ip-prefixes delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), public_ip_prefix_name: (encode-path-segment $public_ip_prefix_name)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Network/publicIPPrefixes/{public_ip_prefix_name}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
]: nothing -> record<etag: string, properties: record<ipPrefix: string, ipTags: list<record>, loadBalancerFrontendIpConfiguration: record<id: string>, prefixLength: int, provisioningState: string, publicIPAddressVersion: string, publicIPAddresses: list<record>, resourceGuid: string>, sku: record<name: string>, zones: list<string>, id: string, location: string, name: string, tags: record, type: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
...
]: any -> record<etag: string, properties: record<ipPrefix: string, ipTags: list<record>, loadBalancerFrontendIpConfiguration: record<id: string>, prefixLength: int, provisioningState: string, publicIPAddressVersion: string, publicIPAddresses: list<record>, resourceGuid: string>, sku: record<name: string>, zones: list<string>, id: string, location: string, name: string, tags: record, type: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
...
export def "subscriptions-resource-groups-providers-microsoft-network-public-ip-prefixes create-or-update" [
  ...
  --properties: any # Public IP prefix properties. — shape: {ipPrefix?: string, ipTags?: list, loadBalancerFrontendIpConfiguration?: any, prefixLength?: int, publicIPAddressVersion?: "IPv4"|"IPv6", publicIPAddresses?: list, resourceGuid?: string}
  --sku: any # SKU of a public IP prefix. — shape: {name?: "Standard"}
  --zones: list<string> # A list of availability zones denoting the IP allocated for the resource needs to come from.
