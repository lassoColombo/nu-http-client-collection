...
const BASE_URL = "{Endpoint}/anomalydetector/v1.0"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o ANOMALY_DETECTOR_CLIENT_TOKEN | default "" }
...
def base-url-completer [] { ["{Endpoint}/anomalydetector/v1.0"] }
...
export def "timeseries-change-point-detect create" [
  ...
  let full_url = (build-url $base "/timeseries/changePoint/detect" $auth.query)
