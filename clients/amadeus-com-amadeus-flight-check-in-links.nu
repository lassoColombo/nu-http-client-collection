...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o FLIGHT_CHECK_IN_LINKS_TOKEN | default "" }
...
export def "reference-data-urls-checkin-links get-ur-ls" [
  ...
  let full_url = (build-url $base "/reference-data/urls/checkin-links" $qp $auth.query)
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"airlineCode": $airline_code, "language": $language} | compact)
