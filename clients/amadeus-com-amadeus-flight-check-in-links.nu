...
export def "reference-data-urls-checkin-links get-ur-ls" [
  ...
  let qp = [(serialize-qp "airlineCode" $airline_code "scalar") (serialize-qp "language" $language "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/reference-data/urls/checkin-links" $qp)
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"airlineCode": $airline_code, "language": $language} | compact), body: null}
}
