...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o IP_GEOLOCATION_API_TOKEN | default "" }
...
export def "data-ip-geolocation-full get-with-confidence-area-and-hazard-report" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"ip": $ip, "localityLanguage": $locality_language, "key": $key} | compact), body: null}
}
