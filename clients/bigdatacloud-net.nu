# Auto-generated client for IP Geolocation API v1.0.0
# Source: https://api.apis.guru/v2/specs/bigdatacloud.net/1.0.0/openapi.json
# Auth: --token flag or $env.IP_GEOLOCATION_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o IP_GEOLOCATION_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.bigdatacloud.net"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "data-ip-geolocation-full get-with-confidence-area-and-hazard-report" } } | get name | first)
...
export def "data-ip-geolocation-full get-with-confidence-area-and-hazard-report" [
  ...
  let full_url = (build-url $base "/data/ip-geolocation-full" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"ip": $ip, "localityLanguage": $locality_language, "key": $key} | compact)
...
export def "data-ip-geolocation-with-confidence get-area" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "ip" $ip "scalar") (serialize-qp "localityLanguage" $locality_language "scalar") (serialize-qp "key" $key "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/data/ip-geolocation-with-confidence" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
