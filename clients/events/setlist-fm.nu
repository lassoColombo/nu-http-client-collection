...
# Source: https://api.setlist.fm/docs/1.0/ui/swagger.json
...
]: nothing -> record<mbid: string, tmid: float, name: string, sortName: string, disambiguation: string, url: string> {
...
]: nothing -> record<setlist: table<artist: record, venue: record, tour: record, set: list, info: string, url: string, id: string, versionId: string, lastFmEventId: float, eventDate: string, lastUpdated: string>, total: float, page: float, itemsPerPage: float> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
...
export def "10-city GET" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --accept: string@accept-completer # Response content type
]: nothing -> record<id: string, name: string, stateCode: string, state: string, coords: record<long: float, lat: float>, country: record<code: string, name: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
...
export def "10-search-artists GET" [
  ...
  --artistTmid: int # the artist's Ticketmaster Identifier (tmid) (deprecated)
...
]: nothing -> record<artist: table<mbid: string, tmid: float, name: string, sortName: string, disambiguation: string, url: string>, total: float, page: float, itemsPerPage: float> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
...
]: nothing -> record<cities: table<id: string, name: string, stateCode: string, state: string, coords: record, country: record>, total: float, page: float, itemsPerPage: float> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
...
export def "10-search-countries GET" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --accept: string@accept-completer # Response content type
]: nothing -> record<country: table<code: string, name: string>, total: float, page: float, itemsPerPage: float> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
...
]: nothing -> record<venue: table<city: record, url: string, id: string, name: string>, total: float, page: float, itemsPerPage: float> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
...
export def "10-setlist-version GET" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --accept: string@accept-completer # Response content type
]: nothing -> record<artist: record<mbid: string, tmid: float, name: string, sortName: string, disambiguation: string, url: string>, venue: record<city: record<id: string, name: string, stateCode: string, state: string, coords: record, country: record>, url: string, id: string, name: string>, tour: record<name: string>, set: table<name: string, encore: float, song: list>, info: string, url: string, id: string, versionId: string, lastFmEventId: float, eventDate: string, lastUpdated: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
...
export def "10-user GET" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --accept: string@accept-completer # Response content type
]: nothing -> record<userId: string, fullname: string, lastFm: string, mySpace: string, twitter: string, flickr: string, website: string, about: string, url: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
...
export def "10-venue GET" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --accept: string@accept-completer # Response content type
]: nothing -> record<city: record<id: string, name: string, stateCode: string, state: string, coords: record<long: float, lat: float>, country: record<code: string, name: string>>, url: string, id: string, name: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
