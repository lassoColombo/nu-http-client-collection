...
export def "get-images get" [
  ...
  --max-results: int # The maximum number of images to be returned by the API. The default limit is 100 images per API response. The additional results will be paginated.
  --next-token: string # A token that specifies where to start paginating the next set of Images. This is the GetImages:NextToken from a previously truncated response.
...
export def "list-fragments list" [
  ...
  --max-results: int # The total number of fragments to return. If the total number of fragments available is more than the value specified in max-results, then a ListFragmentsOutput$NextToken is provided in the output that you can use to resume pagination.
  --next-token: string # A token to specify where to start paginating. This is the ListFragmentsOutput$NextToken from a previously truncated response.
  ...
  let req_body = {"StreamName": $stream_name, "StreamARN": $stream_arn, "MaxResults": $max_results, "NextToken": $next_token, "FragmentSelector": $fragment_selector} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"X-Amz-Content-Sha256": $x_amz_content_sha256, "X-Amz-Date": $x_amz_date, "X-Amz-Algorithm": $x_amz_algorithm, "X-Amz-Credential": $x_amz_credential, "X-Amz-Security-Token": $x_amz_security_token, "X-Amz-Signature": $x_amz_signature, "X-Amz-SignedHeaders": $x_amz_signed_headers} | compact
