...
# Source: https://api.apis.guru/v2/specs/azure.com/automation-hybridRunbookWorkerGroup/2015-10-31/swagger.json
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-hybrid-runbook-worker-groups list-by" } } | get name | first)
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-hybrid-runbook-worker-groups list-by" [
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-hybrid-runbook-worker-groups delete" [
  ...
  let full_url = (build-url $base ({subscription_id: $subscription_id, resource_group_name: $resource_group_name, automation_account_name: $automation_account_name, hybrid_runbook_worker_group_name: $hybrid_runbook_worker_group_name} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Automation/automationAccounts/{automation_account_name}/hybridRunbookWorkerGroups/{hybrid_runbook_worker_group_name}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-hybrid-runbook-worker-groups get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client Api Version.
]: nothing -> record<credential: record<name: string>, groupType: string, hybridRunbookWorkers: table<ip: string, lastSeenDateTime: string, name: string, registrationTime: string>, id: string, name: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: $subscription_id, resource_group_name: $resource_group_name, automation_account_name: $automation_account_name, hybrid_runbook_worker_group_name: $hybrid_runbook_worker_group_name} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Automation/automationAccounts/{automation_account_name}/hybridRunbookWorkerGroups/{hybrid_runbook_worker_group_name}") $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-hybrid-runbook-worker-groups update" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client Api Version.
  --credential: any # Definition of RunAs credential to use for hybrid worker. — shape: {name?: string}
]: any -> record<credential: record<name: string>, groupType: string, hybridRunbookWorkers: table<ip: string, lastSeenDateTime: string, name: string, registrationTime: string>, id: string, name: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: $subscription_id, resource_group_name: $resource_group_name, automation_account_name: $automation_account_name, hybrid_runbook_worker_group_name: $hybrid_runbook_worker_group_name} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Automation/automationAccounts/{automation_account_name}/hybridRunbookWorkerGroups/{hybrid_runbook_worker_group_name}") $qp)
  let body = {"credential": $credential} | compact
