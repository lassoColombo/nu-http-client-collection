...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-hybrid-runbook-worker-groups delete" [
  ...
  if ($resource_group_name | is-empty) { error make --unspanned { msg: "path parameter 'resourceGroupName' must be non-empty" } }
  if ($automation_account_name | is-empty) { error make --unspanned { msg: "path parameter 'automationAccountName' must be non-empty" } }
  if ($hybrid_runbook_worker_group_name | is-empty) { error make --unspanned { msg: "path parameter 'hybridRunbookWorkerGroupName' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), automation_account_name: (encode-path-segment $automation_account_name), hybrid_runbook_worker_group_name: (encode-path-segment $hybrid_runbook_worker_group_name)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Automation/automationAccounts/{automation_account_name}/hybridRunbookWorkerGroups/{hybrid_runbook_worker_group_name}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-hybrid-runbook-worker-groups update" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), automation_account_name: (encode-path-segment $automation_account_name), hybrid_runbook_worker_group_name: (encode-path-segment $hybrid_runbook_worker_group_name)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Automation/automationAccounts/{automation_account_name}/hybridRunbookWorkerGroups/{hybrid_runbook_worker_group_name}") $qp $auth.query)
  let req_body = {"credential": $credential} | compact
