# Auto-generated client for ISBNdb API v1.0.1
# Source: https://api.apis.guru/v2/specs/isbndb.com/1.0.1/swagger.json
# Auth: --token flag or $env.ISBNDB_API_TOKEN

const BASE_URL = "https://api.isbndb.com"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o ISBNDB_API_TOKEN | default "" }
...
export def "books get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"page": $page, "author": $author, "pageSize": $page_size} | compact), body: null}
}
