# Auto-generated client for BOARD OF HIGHER SECONDARY EXAMINATION, KERALA, Kerala v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/dhsekerala/3.0.0/openapi.json
# Auth: --token flag or $env.BOARD_OF_HIGHER_SECONDARY_EXAMINATION_KERALA_KERALA_TOKEN

const BASE_URL = "https://apisetu.gov.in/dhsekerala/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o BOARD_OF_HIGHER_SECONDARY_EXAMINATION_KERALA_KERALA_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/dhsekerala/v3"] }
...
# operationId: hpcer
# --certificateParameters shape: {DOB: string, FullName: string, regno: string, type: string, year: string}
...
export def "hpcer-certificate create" [
  ...
  --certificate-parameters: record # shape: {DOB: string, FullName: string, regno: string, type: string, year: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o BOARD_OF_HIGHER_SECONDARY_EXAMINATION_KERALA_KERALA_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o BOARD_OF_HIGHER_SECONDARY_EXAMINATION_KERALA_KERALA_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
