...
# Source: https://api.apis.guru/v2/specs/azure.com/automation-softwareUpdateConfigurationRun/2017-05-15-preview/swagger.json
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-software-update-configuration-runs list" } } | get name | first)
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-software-update-configuration-runs list" [
...
# Docs: http://aka.ms/azureautomationsdk/softwareupdateconfigurationrunoperations
# operationId: SoftwareUpdateConfigurationRuns_GetById
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-software-update-configuration-runs get" [
  subscription_id: string
  resource_group_name: string
  automation_account_name: string
  software_update_configuration_run_id: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --api-version: string # Client Api Version.
  --client-request-id: string # Identifies this specific client request.
]: nothing -> record<id: string, name: string, properties: record<computerCount: int, configuredDuration: string, createdBy: string, creationTime: string, endTime: string, failedCount: int, lastModifiedBy: string, lastModifiedTime: string, osType: string, softwareUpdateConfiguration: record<name: string>, startTime: string, status: string, tasks: record<postTask: record, preTask: record>>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
  if ($resource_group_name | is-empty) { error make --unspanned { msg: "path parameter 'resourceGroupName' must be non-empty" } }
  if ($automation_account_name | is-empty) { error make --unspanned { msg: "path parameter 'automationAccountName' must be non-empty" } }
  if ($software_update_configuration_run_id | is-empty) { error make --unspanned { msg: "path parameter 'softwareUpdateConfigurationRunId' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), automation_account_name: (encode-path-segment $automation_account_name), software_update_configuration_run_id: (encode-path-segment $software_update_configuration_run_id)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Automation/automationAccounts/{automation_account_name}/softwareUpdateConfigurationRuns/{software_update_configuration_run_id}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"clientRequestId": $client_request_id} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
