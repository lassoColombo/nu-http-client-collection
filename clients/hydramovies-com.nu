# Auto-generated client for Hydra Movies v1.1
# Source: https://api.apis.guru/v2/specs/hydramovies.com/1.1/swagger.json
# Auth: --token flag or $env.HYDRA_MOVIES_TOKEN

const BASE_URL = "https://hydramovies.com/api-v2/%3Fsource=http:/hydramovies.com/api-v2"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o HYDRA_MOVIES_TOKEN | default "" }
...
export def "current-movie-data-csvimdb-id-imd-bid get-csv" [
  ...
  let full_url = (build-url $base ({imd_bid: (encode-path-segment $imd_bid)} | format pattern "/current-Movie-Data.csv&imdb_id={imd_bid}") $auth.query)
...
export def "current-movie-data-csvmovie-year-movie-year get-csv-get2" [
  ...
  let full_url = (build-url $base ({movie_year: (encode-path-segment $movie_year)} | format pattern "/current-Movie-Data.csv&movie_year={movie_year}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
