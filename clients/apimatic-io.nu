# Auto-generated client for APIMATIC API Transformer v1.0
# Source: https://api.apis.guru/v2/specs/apimatic.io/1.0/openapi.json
# Auth: --token flag or $env.APIMATIC_API_TRANSFORMER_TOKEN

const BASE_URL = "https://apimatic.io/api/transform"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o APIMATIC_API_TRANSFORMER_TOKEN | default "" }
...
export def "transform create-convert" [
  ...
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full $mp.content_type $mp.body {query: ({"format": $format} | compact), body: $req_body}
}
