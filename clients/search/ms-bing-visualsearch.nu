...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "images-visualsearch VisualSearch" } } | get name | first)
...
export def "images-visualsearch VisualSearch" [
  ...
  let qp = [(serialize-qp "mkt" $mkt "scalar") (serialize-qp "safeSearch" $safeSearch "scalar") (serialize-qp "setLang" $setLang "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/images/visualsearch" $qp)
  let body = {knowledgeRequest: $knowledgeRequest, image: $image} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let extra_headers = {"X-BingApis-SDK": $X_BingApis_SDK, "Accept": $Accept, "Accept-Language": $Accept_Language, "Content-Type": $Content_Type, "User-Agent": $User_Agent, "X-MSEdge-ClientID": $X_MSEdge_ClientID, "X-MSEdge-ClientIP": $X_MSEdge_ClientIP, "X-Search-Location": $X_Search_Location} | compact
