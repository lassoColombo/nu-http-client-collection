...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o LOCAL_SEARCH_CLIENT_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "v70-localbusinesses-search Search" } } | get name | first)
...
export def "v70-localbusinesses-search Search" [
  ...
  --ResponseFormat: list # The media type to use for the response. The following are the possible case-insensitive values: JSON, JSONLD. The default is JSON. If you specify JSONLD, the response body includes JSON-LD objects that contain the search results.
  ...
  let qp = [(serialize-qp "cc" $cc "scalar") (serialize-qp "mkt" $mkt "scalar") (serialize-qp "q" $q "scalar") (serialize-qp "localcategories" $localcategories "scalar") (serialize-qp "localcircularview" $localcircularview "scalar") (serialize-qp "localmapview" $localmapview "scalar") (serialize-qp "count" $count "scalar") (serialize-qp "first" $first "scalar") (serialize-qp "ResponseFormat" $ResponseFormat "csv") (serialize-qp "SafeSearch" $SafeSearch "scalar") (serialize-qp "SetLang" $SetLang "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/v7.0/localbusinesses/search" $qp)
