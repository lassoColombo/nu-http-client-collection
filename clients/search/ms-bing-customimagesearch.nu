...
def imageContent-completer [] { ["Face" "Portrait"] }
def imageType-completer [] { ["AnimatedGif" "Clipart" "Line" "Photo" "Shopping" "Transparent"] }
def license-completer [] { ["All" "Any" "Modify" "ModifyCommercially" "Public" "Share" "ShareCommercially"] }
def safeSearch-completer [] { ["Moderate" "Off" "Strict"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "images-search ImageSearch" } } | get name | first)
...
export def "images-search ImageSearch" [
  ...
  --maxFileSize: int # Filter images that are less than or equal to the specified file size. The maximum file size that you may specify is 520,192 bytes. If you specify a larger value, the API uses 520,192. It is possible that the response may include images that are slightly larger than the specified maximum. You may specify this filter and minFileSize to filter images within a range of file sizes. (format: int64)
  --maxHeight: int # Filter images that have a height that is less than or equal to the specified height. Specify the height in pixels. You may specify this filter and minHeight to filter images within a range of heights. This filter and the height filter are mutually exclusive. (format: int64)
  --maxWidth: int # Filter images that have a width that is less than or equal to the specified width. Specify the width in pixels. You may specify this filter and maxWidth to filter images within a range of widths. This filter and the width filter are mutually exclusive. (format: int64)
  --minFileSize: int # Filter images that are greater than or equal to the specified file size. The maximum file size that you may specify is 520,192 bytes. If you specify a larger value, the API uses 520,192. It is possible that the response may include images that are slightly smaller than the specified minimum. You may specify this filter and maxFileSize to filter images within a range of file sizes. (format: int64)
  --minHeight: int # Filter images that have a height that is greater than or equal to the specified height. Specify the height in pixels. You may specify this filter and maxHeight to filter images within a range of heights. This filter and the height filter are mutually exclusive. (format: int64)
  --minWidth: int # Filter images that have a width that is greater than or equal to the specified width. Specify the width in pixels. You may specify this filter and maxWidth to filter images within a range of widths. This filter and the width filter are mutually exclusive. (format: int64)
  ...
  let qp = [(serialize-qp "customConfig" $customConfig "scalar") (serialize-qp "aspect" $aspect "scalar") (serialize-qp "color" $color "scalar") (serialize-qp "cc" $cc "scalar") (serialize-qp "count" $count "scalar") (serialize-qp "freshness" $freshness "scalar") (serialize-qp "height" $height "scalar") (serialize-qp "id" $id "scalar") (serialize-qp "imageContent" $imageContent "scalar") (serialize-qp "imageType" $imageType "scalar") (serialize-qp "license" $license "scalar") (serialize-qp "mkt" $mkt "scalar") (serialize-qp "maxFileSize" $maxFileSize "scalar") (serialize-qp "maxHeight" $maxHeight "scalar") (serialize-qp "maxWidth" $maxWidth "scalar") (serialize-qp "minFileSize" $minFileSize "scalar") (serialize-qp "minHeight" $minHeight "scalar") (serialize-qp "minWidth" $minWidth "scalar") (serialize-qp "offset" $offset "scalar") (serialize-qp "q" $q "scalar") (serialize-qp "safeSearch" $safeSearch "scalar") (serialize-qp "size" $size "scalar") (serialize-qp "setLang" $setLang "scalar") (serialize-qp "width" $width "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/images/search" $qp)
  let extra_headers = {"X-BingApis-SDK": $X_BingApis_SDK, "Accept": $Accept, "Accept-Language": $Accept_Language, "User-Agent": $User_Agent, "X-MSEdge-ClientID": $X_MSEdge_ClientID, "X-MSEdge-ClientIP": $X_MSEdge_ClientIP, "X-Search-Location": $X_Search_Location} | compact
