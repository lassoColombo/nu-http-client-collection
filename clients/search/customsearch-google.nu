...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o CUSTOM_SEARCH_API_TOKEN | default "" }
...
def imgColorType-completer [] { ["color" "gray" "imgColorTypeUndefined" "mono" "trans"] }
def imgDominantColor-completer [] { ["black" "blue" "brown" "gray" "green" "imgDominantColorUndefined" "orange" "pink" "purple" "red" "teal" "white" "yellow"] }
def imgSize-completer [] { ["HUGE" "ICON" "LARGE" "MEDIUM" "SMALL" "XLARGE" "XXLARGE" "imgSizeUndefined"] }
def imgType-completer [] { ["animated" "clipart" "face" "imgTypeUndefined" "lineart" "photo" "stock"] }
def safe-completer [] { ["active" "high" "medium" "off" "safeUndefined"] }
def searchType-completer [] { ["image" "searchTypeUndefined"] }
def siteSearchFilter-completer [] { ["e" "i" "siteSearchFilterUndefined"] }

# List all available API commands with their parameters
export def commands []: nothing -> table {
  let builtin_flags = ["base-url" "token" "auth-scheme" "insecure" "max-time" "raw" "allow-errors" "dry-run" "accept" "help"]
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "customsearch searchcselist" } } | get name | first)
...
export def "customsearch searchcselist" [
  ...
  --imgColorType: string@imgColorType-completer # Returns black and white, grayscale, transparent, or color images. Acceptable values are: * `"color"` * `"gray"` * `"mono"`: black and white * `"trans"`: transparent background
  --imgDominantColor: string@imgDominantColor-completer # Returns images of a specific dominant color. Acceptable values are: * `"black"` * `"blue"` * `"brown"` * `"gray"` * `"green"` * `"orange"` * `"pink"` * `"purple"` * `"red"` * `"teal"` * `"white"` * `"yellow"`
  --imgSize: string@imgSize-completer # Returns images of a specified size. Acceptable values are: * `"huge"` * `"icon"` * `"large"` * `"medium"` * `"small"` * `"xlarge"` * `"xxlarge"`
  --imgType: string@imgType-completer # Returns images of a type. Acceptable values are: * `"clipart"` * `"face"` * `"lineart"` * `"stock"` * `"photo"` * `"animated"`
  ...
  --searchType: string@searchType-completer # Specifies the search type: `image`. If unspecified, results are limited to webpages. Acceptable values are: * `"image"`: custom image search.
  ...
  --siteSearchFilter: string@siteSearchFilter-completer # Controls whether to include or exclude results from the site named in the `siteSearch` parameter. Acceptable values are: * `"e"`: exclude * `"i"`: include
  ...
  let qp = [(serialize-qp "$.xgafv" $xgafv "scalar") (serialize-qp "access_token" $access_token "scalar") (serialize-qp "alt" $alt "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "fields" $qp_fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $prettyPrint "scalar") (serialize-qp "quotaUser" $quotaUser "scalar") (serialize-qp "upload_protocol" $upload_protocol "scalar") (serialize-qp "uploadType" $uploadType "scalar") (serialize-qp "c2coff" $c2coff "scalar") (serialize-qp "cr" $cr "scalar") (serialize-qp "cx" $cx "scalar") (serialize-qp "dateRestrict" $dateRestrict "scalar") (serialize-qp "exactTerms" $exactTerms "scalar") (serialize-qp "excludeTerms" $excludeTerms "scalar") (serialize-qp "fileType" $fileType "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "gl" $gl "scalar") (serialize-qp "googlehost" $googlehost "scalar") (serialize-qp "highRange" $highRange "scalar") (serialize-qp "hl" $hl "scalar") (serialize-qp "hq" $hq "scalar") (serialize-qp "imgColorType" $imgColorType "scalar") (serialize-qp "imgDominantColor" $imgDominantColor "scalar") (serialize-qp "imgSize" $imgSize "scalar") (serialize-qp "imgType" $imgType "scalar") (serialize-qp "linkSite" $linkSite "scalar") (serialize-qp "lowRange" $lowRange "scalar") (serialize-qp "lr" $lr "scalar") (serialize-qp "num" $num "scalar") (serialize-qp "orTerms" $orTerms "scalar") (serialize-qp "q" $q "scalar") (serialize-qp "relatedSite" $relatedSite "scalar") (serialize-qp "rights" $rights "scalar") (serialize-qp "safe" $safe "scalar") (serialize-qp "searchType" $searchType "scalar") (serialize-qp "siteSearch" $siteSearch "scalar") (serialize-qp "siteSearchFilter" $siteSearchFilter "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "start" $start "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/customsearch/v1" $qp)
...
export def "customsearch-siterestrict searchcsesiterestrictlist" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "$.xgafv" $xgafv "scalar") (serialize-qp "access_token" $access_token "scalar") (serialize-qp "alt" $alt "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "fields" $qp_fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $prettyPrint "scalar") (serialize-qp "quotaUser" $quotaUser "scalar") (serialize-qp "upload_protocol" $upload_protocol "scalar") (serialize-qp "uploadType" $uploadType "scalar") (serialize-qp "c2coff" $c2coff "scalar") (serialize-qp "cr" $cr "scalar") (serialize-qp "cx" $cx "scalar") (serialize-qp "dateRestrict" $dateRestrict "scalar") (serialize-qp "exactTerms" $exactTerms "scalar") (serialize-qp "excludeTerms" $excludeTerms "scalar") (serialize-qp "fileType" $fileType "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "gl" $gl "scalar") (serialize-qp "googlehost" $googlehost "scalar") (serialize-qp "highRange" $highRange "scalar") (serialize-qp "hl" $hl "scalar") (serialize-qp "hq" $hq "scalar") (serialize-qp "imgColorType" $imgColorType "scalar") (serialize-qp "imgDominantColor" $imgDominantColor "scalar") (serialize-qp "imgSize" $imgSize "scalar") (serialize-qp "imgType" $imgType "scalar") (serialize-qp "linkSite" $linkSite "scalar") (serialize-qp "lowRange" $lowRange "scalar") (serialize-qp "lr" $lr "scalar") (serialize-qp "num" $num "scalar") (serialize-qp "orTerms" $orTerms "scalar") (serialize-qp "q" $q "scalar") (serialize-qp "relatedSite" $relatedSite "scalar") (serialize-qp "rights" $rights "scalar") (serialize-qp "safe" $safe "scalar") (serialize-qp "searchType" $searchType "scalar") (serialize-qp "siteSearch" $siteSearch "scalar") (serialize-qp "siteSearchFilter" $siteSearchFilter "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "start" $start "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/customsearch/v1/siterestrict" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
