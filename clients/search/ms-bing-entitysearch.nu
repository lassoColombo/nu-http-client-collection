...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "entities Search" } } | get name | first)
...
export def "entities Search" [
  ...
  --ResponseFilter: list # A comma-delimited list of answers to include in the response. If you do not specify this parameter, the response includes all search answers for which there's relevant data.
  ...
  let qp = [(serialize-qp "cc" $cc "scalar") (serialize-qp "mkt" $mkt "scalar") (serialize-qp "q" $q "scalar") (serialize-qp "ResponseFilter" $ResponseFilter "csv") (serialize-qp "ResponseFormat" $ResponseFormat "csv") (serialize-qp "SafeSearch" $SafeSearch "scalar") (serialize-qp "SetLang" $SetLang "scalar")] | flatten | str join "&"
