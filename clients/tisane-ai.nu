...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o TISANE_API_DOCUMENTATION_TOKEN | default "" }
...
export def "hypernyms list" [
  ...
  let full_url = (build-url $base "/hypernyms" $qp $auth.query)
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"family": $family, "maxLevel": $max_level} | compact)
...
export def "hyponyms list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "family" $family "scalar") (serialize-qp "maxLevel" $max_level "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/hyponyms" $qp $auth.query)
...
export def "inflections list-inflected-forms" [
  ...
  let full_url = (build-url $base "/inflections" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Ocp-Apim-Subscription-Key": $ocp_apim_subscription_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "get"
    url: $full_url
    query: ({"language": $language, "lexeme": $lexeme, "family": $family} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "lm-family get-details" [
  ...
  let full_url = (build-url $base "/lm/family" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Ocp-Apim-Subscription-Key": $ocp_apim_subscription_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "senses list-word" [
  ...
  let full_url = (build-url $base "/senses" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Ocp-Apim-Subscription-Key": $ocp_apim_subscription_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "get"
    url: $full_url
    query: ({"language": $language, "word": $word} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "values list-feature" [
  ...
  let full_url = (build-url $base "/values" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Ocp-Apim-Subscription-Key": $ocp_apim_subscription_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "get"
    url: $full_url
    query: ({"language": $language, "type": $type, "description": $description} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
