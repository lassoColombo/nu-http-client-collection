# Auto-generated client for Tisane API Documentation v1.0.0
# Source: https://api.apis.guru/v2/specs/tisane.ai/1.0.0/openapi.json
# Auth: --token flag or $env.TISANE_API_DOCUMENTATION_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o TISANE_API_DOCUMENTATION_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.tisane.ai"] }
...
export def "hypernyms list" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"family": $family, "maxLevel": $max_level} | compact), body: null}
}
...
export def "inflections list-inflected-forms" [
  ...
  let extra_headers = {"Ocp-Apim-Subscription-Key": $ocp_apim_subscription_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"language": $language, "lexeme": $lexeme, "family": $family} | compact), body: null}
}
...
export def "senses list-word" [
  ...
  let extra_headers = {"Ocp-Apim-Subscription-Key": $ocp_apim_subscription_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"language": $language, "word": $word} | compact), body: null}
}
...
export def "values list-feature" [
  ...
  let extra_headers = {"Ocp-Apim-Subscription-Key": $ocp_apim_subscription_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"language": $language, "type": $type, "description": $description} | compact), body: null}
}
