...
export def "currencies-history get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"pairs": $pairs, "start": $start, "end": $end, "interval": $interval, "format": $format} | compact), body: null}
}
...
export def "currencies-rate get" [
  ...
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"pairs": $pairs, "format": $format} | compact), body: null}
}
...
export def "metals-benchmark-history get" [
  ...
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"metals": $metals, "start": $start, "end": $end, "interval": $interval, "historicalfx": $historicalfx, "currency": $currency, "unitofmeasure": $unitofmeasure, "format": $format} | compact), body: null}
}
...
export def "metals-benchmark-summary get" [
  ...
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"metals": $metals, "currency": $currency, "unitofmeasure": $unitofmeasure, "format": $format} | compact), body: null}
}
...
export def "metals-spot-performance-annual get-historical" [
  ...
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"metals": $metals, "currency": $currency, "unitofmeasure": $unitofmeasure, "format": $format, "years": $years} | compact), body: null}
}
