...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o VA_FACILITIES_TOKEN | default "" }
...
export def "facilities get-by-location" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"ids": $ids, "zip": $zip, "state": $state, "lat": $lat, "long": $long, "bbox[]": $bbox, "visn": $visn, "type": $type, "services[]": $services, "mobile": $mobile, "page": $page, "per_page": $per_page} | compact)
...
export def "facilities-all get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/facilities/all" $auth.query)
  let accept_val = ($accept | default "application/geo+json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "ids get-facility" [
  ...
  let full_url = (build-url $base "/ids" $qp $auth.query)
...
export def "nearby get-facilities" [
  ...
  let full_url = (build-url $base "/nearby" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"street_address": $street_address, "city": $city, "state": $state, "zip": $zip, "lat": $lat, "lng": $lng, "drive_time": $drive_time, "services[]": $services, "page": $page, "per_page": $per_page} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
