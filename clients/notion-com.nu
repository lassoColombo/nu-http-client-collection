...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o NOTION_API_TOKEN | default "" }
...
export def "blocks delete" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/v1/blocks/{id}") $auth.query)
...
export def "blocks-children get" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/v1/blocks/{id}/children") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Notion-Version": $notion_version} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "get"
    url: $full_url
    query: ({"page_size": $page_size} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "blocks-children create" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/v1/blocks/{id}/children") $auth.query)
...
export def "comments get" [
  ...
  --page-size: string # e.g. 100
  --notion-version: string # e.g. {{NOTION_VERSION}}
]: nothing -> record<comment: record, has_more: bool, next_cursor: any, object: string, results: table<created_by: record, created_time: string, discussion_id: string, id: string, last_edited_time: string, object: string, parent: record, rich_text: list>, type: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "block_id" $block_id "scalar") (serialize-qp "page_size" $page_size "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/v1/comments" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Notion-Version": $notion_version} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "get"
    url: $full_url
    query: ({"block_id": $block_id, "page_size": $page_size} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "databases get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/v1/databases/{id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Notion-Version": $notion_version} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "databases-query list" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/v1/databases/{id}/query") $auth.query)
...
export def "pages get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/v1/pages/{id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "pages-properties get-item" [
  ...
  let full_url = (build-url $base ({page_id: (encode-path-segment $page_id), property_id: (encode-path-segment $property_id)} | format pattern "/v1/pages/{page_id}/properties/{property_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --notion-version: string # e.g. {{NOTION_VERSION}}
]: nothing -> record<avatar_url: any, id: string, name: string, object: string, person: record<email: string>, type: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
