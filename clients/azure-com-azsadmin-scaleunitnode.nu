...
# Source: https://api.apis.guru/v2/specs/azure.com/azsadmin-ScaleUnitNode/2016-05-01/swagger.json
...
export def "subscriptions-resource-groups-providers-microsoft-fabric-admin-fabric-locations-scale-unit-nodes list" [
  ...
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), location: (encode-path-segment $location)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Fabric.Admin/fabricLocations/{location}/scaleUnitNodes") $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-fabric-admin-fabric-locations-scale-unit-nodes get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client API Version. (default: 2016-05-01)
]: nothing -> record<properties: record<biosVersion: string, bmcAddress: string, canPowerOff: bool, capacity: record<cores: int, memoryGB: float>, model: string, powerState: string, scaleUnitName: string, scaleUnitNodeStatus: string, scaleUnitUri: string, serialNumber: string, vendor: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
  if ($resource_group_name | is-empty) { error make --unspanned { msg: "path parameter 'resourceGroupName' must be non-empty" } }
  if ($location | is-empty) { error make --unspanned { msg: "path parameter 'location' must be non-empty" } }
  if ($scale_unit_node | is-empty) { error make --unspanned { msg: "path parameter 'scaleUnitNode' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), location: (encode-path-segment $location), scale_unit_node: (encode-path-segment $scale_unit_node)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Fabric.Admin/fabricLocations/{location}/scaleUnitNodes/{scale_unit_node}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "subscriptions-resource-groups-providers-microsoft-fabric-admin-fabric-locations-scale-unit-nodes-repair create" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client API Version. (default: 2016-05-01)
  --bios-version: string # Bios version of the physical machine.
  --bmc-ipv4-address: string # BMC address of the physical machine.
  --cluster-name: string # Name of the cluster.
  --computer-name: string # Name of the computer.
  --mac-address: string # Name of the MAC address of the bare metal node.
  --model: string # Model of the physical machine.
  --serial-number: string # Serial number of the physical machine.
  ...
  let req_body = {"biosVersion": $bios_version, "bmcIpv4Address": $bmc_ipv4_address, "clusterName": $cluster_name, "computerName": $computer_name, "macAddress": $mac_address, "model": $model, "serialNumber": $serial_number, "vendor": $vendor} | compact
