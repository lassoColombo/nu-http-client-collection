...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o MAHARASHTRA_STATE_BOARD_OF_SECONDARY_AND_HIGHER_SECONDARY_EDUCATION_MAHARASHTRA_TOKEN | default "" }
