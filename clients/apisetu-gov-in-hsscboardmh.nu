# Auto-generated client for Maharashtra State Board of Secondary and Higher Secondary Education, Maharashtra v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/hsscboardmh/3.0.0/openapi.json
# Auth: --token flag or $env.MAHARASHTRA_STATE_BOARD_OF_SECONDARY_AND_HIGHER_SECONDARY_EDUCATION_MAHARASHTRA_TOKEN

const BASE_URL = "https://apisetu.gov.in/hsscboardmh/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o MAHARASHTRA_STATE_BOARD_OF_SECONDARY_AND_HIGHER_SECONDARY_EDUCATION_MAHARASHTRA_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/hsscboardmh/v3"] }
...
export def "hpcer-certificate create" [
  ...
  --certificate-parameters: record # shape: {FullName: string, exsession: string, rollnumber: string, totalmarks: string, year: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o MAHARASHTRA_STATE_BOARD_OF_SECONDARY_AND_HIGHER_SECONDARY_EDUCATION_MAHARASHTRA_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o MAHARASHTRA_STATE_BOARD_OF_SECONDARY_AND_HIGHER_SECONDARY_EDUCATION_MAHARASHTRA_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
