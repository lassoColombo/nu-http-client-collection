# Auto-generated client for Binary Authorization API vv1
# Source: https://api.apis.guru/v2/specs/googleapis.com/binaryauthorization/v1/openapi.json
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "projects binaryauthorizationprojectsattestorsvalidateAttestationOccurrence" } } | get name | first)
...
export def "projects binaryauthorizationprojectsattestorsvalidateAttestationOccurrence" [
  attestor: string
  ...
  let full_url = (build-url $base $"/v1/($attestor):validateAttestationOccurrence" $qp)
  let body = {attestation: $attestation, occurrenceNote: $occurrenceNote, occurrenceResourceUri: $occurrenceResourceUri} | compact
...
export def "projects binaryauthorizationprojectsattestorsdelete" [
  name: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
export def "systempolicy binaryauthorizationsystempolicygetPolicy" [
  name: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
# --userOwnedGrafeasNote shape: {noteReference?: string, publicKeys?: list}
export def "projects binaryauthorizationprojectsattestorsupdate" [
  name: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  --userOwnedGrafeasNote: record # An user owned Grafeas note references a Grafeas Attestation.Authority Note created by the user. — shape: {noteReference?: string, publicKeys?: list}
]: any -> record<description: string, etag: string, name: string, updateTime: string, userOwnedGrafeasNote: record<delegationServiceAccountEmail: string, noteReference: string, publicKeys: list<record>>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "$.xgafv" $xgafv "scalar") (serialize-qp "access_token" $access_token "scalar") (serialize-qp "alt" $alt "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "fields" $qp_fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $prettyPrint "scalar") (serialize-qp "quotaUser" $quotaUser "scalar") (serialize-qp "upload_protocol" $upload_protocol "scalar") (serialize-qp "uploadType" $uploadType "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v1/($name)" $qp)
  let body = {description: $description, etag: $etag, name: $body_name, userOwnedGrafeasNote: $userOwnedGrafeasNote} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "attestors binaryauthorizationprojectsattestorslist" [
  parent: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
]: nothing -> record<attestors: table<description: string, etag: string, name: string, updateTime: string, userOwnedGrafeasNote: record>, nextPageToken: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  ...
  let full_url = (build-url $base $"/v1/($parent)/attestors" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
# --userOwnedGrafeasNote shape: {noteReference?: string, publicKeys?: list}
export def "attestors binaryauthorizationprojectsattestorscreate" [
  parent: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "$.xgafv" $xgafv "scalar") (serialize-qp "access_token" $access_token "scalar") (serialize-qp "alt" $alt "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "fields" $qp_fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $prettyPrint "scalar") (serialize-qp "quotaUser" $quotaUser "scalar") (serialize-qp "upload_protocol" $upload_protocol "scalar") (serialize-qp "uploadType" $uploadType "scalar") (serialize-qp "attestorId" $attestorId "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v1/($parent)/attestors" $qp)
  let body = {description: $description, etag: $etag, name: $name, userOwnedGrafeasNote: $userOwnedGrafeasNote} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "projects binaryauthorizationprojectspolicygetIamPolicy" [
  resource: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
export def "projects binaryauthorizationprojectspolicysetIamPolicy" [
  resource: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
export def "projects binaryauthorizationprojectspolicytestIamPermissions" [
  resource: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
