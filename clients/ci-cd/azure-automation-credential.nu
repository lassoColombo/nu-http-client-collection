...
# Source: https://api.apis.guru/v2/specs/azure.com/automation-credential/2015-10-31/swagger.json
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-credentials ListByAutomationAccount" } } | get name | first)
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-credentials ListByAutomationAccount" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Automation/automationAccounts/($automationAccountName)/credentials" $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-credentials Delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Automation/automationAccounts/($automationAccountName)/credentials/($credentialName)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-credentials Get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client Api Version.
]: nothing -> record<properties: record<creationTime: string, description: string, lastModifiedTime: string, userName: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Automation/automationAccounts/($automationAccountName)/credentials/($credentialName)" $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-credentials Update" [
  ...
  --properties: any # The properties of the Update credential — shape: {description?: string, password?: string, userName?: string}
]: any -> record<properties: record<creationTime: string, description: string, lastModifiedTime: string, userName: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
...
# --properties shape: {description?: string, password: string, userName: string}
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-credentials CreateOrUpdate" [
  resourceGroupName: string
  automationAccountName: string
  credentialName: string
  subscriptionId: string
  ...
  --api-version: string # Client Api Version.
  name: string # Gets or sets the name of the credential.
  properties: any # The properties of the create credential operation. — shape: {description?: string, password: string, userName: string}
]: any -> record<properties: record<creationTime: string, description: string, lastModifiedTime: string, userName: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
