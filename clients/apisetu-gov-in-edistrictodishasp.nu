# Auto-generated client for eDistrict Odisha ServicePlus, Odisha v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/edistrictodishasp/3.0.0/openapi.json
# Auth: --token flag or $env.EDISTRICT_ODISHA_SERVICEPLUS_ODISHA_TOKEN

const BASE_URL = "https://apisetu.gov.in/edistrictodishasp/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o EDISTRICT_ODISHA_SERVICEPLUS_ODISHA_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/edistrictodishasp/v3"] }
...
export def "egcer-certificate create" [
  ...
  --certificate-parameters: record # shape: {FullName: string, RefNo: string, TokenNo: string, UID: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o EDISTRICT_ODISHA_SERVICEPLUS_ODISHA_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o EDISTRICT_ODISHA_SERVICEPLUS_ODISHA_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/egcer/certificate" $auth.query)
