...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o EDISTRICT_ODISHA_SERVICEPLUS_ODISHA_TOKEN | default "" }
...
export def "egcer-certificate create" [
  ...
  let full_url = (build-url $base "/egcer/certificate")
