# Auto-generated client for PDF Generator API v3.1.1
# Source: https://api.apis.guru/v2/specs/pdfgeneratorapi.com/3.1.1/openapi.json
# Auth: --token flag or $env.PDF_GENERATOR_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o PDF_GENERATOR_API_TOKEN | default "" }
...
def format-completer [] { ["html" "pdf" "xlsx" "zip"] }
def output-completer [] { ["I" "base64" "url"] }
def language-completer [] { ["cs" "en" "et" "ru" "sk"] }
...
export def "templates-output create-merge" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"name": $name, "format": $format, "output": $output} | compact), body: $req_body}
}
...
export def "templates-template-id delete" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"templateId": $template_id} | compact), body: null}
}
...
export def "templates-template-id get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"templateId": $template_id} | compact), body: null}
}
...
export def "templates-template-id update" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"templateId": $template_id} | compact), body: $req_body}
}
...
export def "templates-template-id-copy copy" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"templateId": $template_id, "name": $name} | compact), body: null}
}
...
export def "templates-template-id-editor get-url" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"templateId": $template_id, "language": $language} | compact), body: $req_body}
}
...
export def "templates-template-id-output create-merge" [
  ...
  --output: string@output-completer # Response format. With the url option, the document is stored for 30 days and automatically deleted. (default: base64, e.g. base64)
  --id: int # e.g. 12312
  --name-body: string # e.g. Sample Data (body field)
]: any -> any {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  ...
  let req_body = {"id": $id, "name": $name_body} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"templateId": $template_id, "name": $name, "format": $format, "output": $output} | compact), body: $req_body}
}
...
export def "workspaces-workspace-id delete" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"workspaceId": $workspace_id} | compact), body: null}
}
...
export def "workspaces-workspace-id get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"workspaceId": $workspace_id} | compact), body: null}
}
