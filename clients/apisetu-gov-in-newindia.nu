# Auto-generated client for New India Assurance Co. Ltd. v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/newindia/3.0.0/openapi.json
# Auth: --token flag or $env.NEW_INDIA_ASSURANCE_CO_LTD_TOKEN

const BASE_URL = "https://apisetu.gov.in/newindia/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o NEW_INDIA_ASSURANCE_CO_LTD_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/newindia/v3"] }
...
# operationId: cripc
# --certificateParameters shape: {FullName: string, UDF1: string, UDF2: string, UDF3: string}
...
export def "cripc-certificate create" [
  ...
  --certificate-parameters: record # shape: {FullName: string, UDF1: string, UDF2: string, UDF3: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o NEW_INDIA_ASSURANCE_CO_LTD_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o NEW_INDIA_ASSURANCE_CO_LTD_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
