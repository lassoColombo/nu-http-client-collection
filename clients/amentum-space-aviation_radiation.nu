...
export def "cari7-ambient-dose get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"altitude": $altitude, "latitude": $latitude, "longitude": $longitude, "year": $year, "month": $month, "day": $day, "utc": $utc, "particle": $particle} | compact), body: null}
}
...
export def "parma-ambient-dose get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"altitude": $altitude, "atmospheric_depth": $atmospheric_depth, "latitude": $latitude, "longitude": $longitude, "year": $year, "month": $month, "day": $day, "particle": $particle} | compact), body: null}
}
...
export def "parma-differential-intensity get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"altitude": $altitude, "atmospheric_depth": $atmospheric_depth, "latitude": $latitude, "longitude": $longitude, "year": $year, "month": $month, "day": $day, "particle": $particle, "angle": $angle} | compact), body: null}
}
...
export def "route-ambient-dose get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"origin": $origin, "destination": $destination, "altitude": $altitude, "duration": $duration, "initial_altitude": $initial_altitude, "cruising_altitudes": $cruising_altitudes, "climb_times": $climb_times, "cruising_times": $cruising_times, "descent_time": $descent_time, "final_altitude": $final_altitude, "year": $year, "month": $month, "day": $day} | compact), body: null}
}
