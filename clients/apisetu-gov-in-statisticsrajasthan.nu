...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o DIRECTORATE_OF_ECONOMICS_AND_STATISTICS_CUM_CHIEF_REGISTRAR_RAJASTHAN_RAJASTHAN_TOKEN | default "" }
