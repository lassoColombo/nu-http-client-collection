# Auto-generated client for AuthorizationManagementClient v2018-09-01-preview
# Source: https://api.apis.guru/v2/specs/azure.com/authorization-authorization-RoleAssignmentsCalls/2018-09-01-preview/swagger.json
...
export def "role-assignments delete-by" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # The API version to use for this operation.
]: nothing -> record<id: string, name: string, properties: record<canDelegate: bool, principalId: string, principalType: string, roleDefinitionId: string, scope: string>, type: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({role_id: $role_id} | format pattern "/{role_id}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "role-assignments create-by" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # The API version to use for this operation.
  properties: any # Role assignment properties. — shape: {canDelegate?: bool, principalId: string, principalType?: "User"|"Group"|"ServicePrincipal"|"Unknown"|"DirectoryRoleTemplate"|"ForeignGroup"|"Application"|"MSI"|"DirectoryObjectOrGroup"|"Everyone", roleDefinitionId: string}
]: any -> record<id: string, name: string, properties: record<canDelegate: bool, principalId: string, principalType: string, roleDefinitionId: string, scope: string>, type: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
