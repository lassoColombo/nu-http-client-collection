...
export def "subscriptions-providers-microsoftinsights-logprofiles List" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/providers/microsoft.insights/logprofiles" $qp)
...
export def "subscriptions-providers-microsoftinsights-logprofiles Delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/providers/microsoft.insights/logprofiles/($logProfileName)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
# --properties shape: {categories: list, locations: list, retentionPolicy: any, serviceBusRuleId?: string, storageAccountId?: string}
export def "subscriptions-providers-microsoftinsights-logprofiles CreateOrUpdate" [
  logProfileName: string
  subscriptionId: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
