...
export def "check-results-assets get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v1/check-results/($checkId)/($checkResultId)/assets")
  let extra_headers = {"x-checkly-account": $x_checkly_account} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "checks-heartbeats-availability get" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
  --startTime: string # format: date, default: 2026-06-10T07:07:30.295Z
  --endTime: string # format: date, default: 2026-06-11T07:07:30.295Z
  --x-checkly-account: string # Your Checkly account ID, you can find it at https://app.checklyhq.com/settings/account/general
...
export def "checks-heartbeats-events list" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
  --startTime: string # format: date, default: 2026-06-10T07:07:30.298Z
  --endTime: string # format: date, default: 2026-06-11T07:07:30.298Z
...
export def "status-pages-incidents-incident-updates post" [
  ...
  --publicIncidentUpdateDate: string # format: date-time, default: 2026-06-11T07:07:31.599Z
  --created-at: string # format: date
...
export def "test-sessions-results-assets get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v1/test-sessions/($testSessionId)/results/($testSessionResultId)/assets")
  let extra_headers = {"x-checkly-account": $x_checkly_account} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
