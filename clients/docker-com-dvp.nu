...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o DVP_DATA_API_TOKEN | default "" }
...
export def "namespaces get" [
  ...
  let full_url = (build-url $base ({namespace: (encode-path-segment $namespace)} | format pattern "/namespaces/{namespace}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "namespaces-pulls-exports-years get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($namespace | is-empty) { error make --unspanned { msg: "path parameter 'namespace' must be non-empty" } }
  let full_url = (build-url $base ({namespace: (encode-path-segment $namespace)} | format pattern "/namespaces/{namespace}/pulls/exports/years") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "namespaces-pulls-exports-years get-timespans" [
  ...
  let full_url = (build-url $base ({namespace: (encode-path-segment $namespace), year: (encode-path-segment $year), timespantype: (encode-path-segment $timespantype)} | format pattern "/namespaces/{namespace}/pulls/exports/years/{year}/{timespantype}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "namespaces-pulls-exports-years get-metadata" [
  ...
  let full_url = (build-url $base ({namespace: (encode-path-segment $namespace), year: (encode-path-segment $year), timespantype: (encode-path-segment $timespantype), timespan: (encode-path-segment $timespan)} | format pattern "/namespaces/{namespace}/pulls/exports/years/{year}/{timespantype}/{timespan}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
