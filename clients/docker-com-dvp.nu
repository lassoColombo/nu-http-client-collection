# Auto-generated client for DVP Data API v1.0.0
# Source: https://api.apis.guru/v2/specs/docker.com/dvp/1.0.0/openapi.json
# Auth: --token flag or $env.DVP_DATA_API_TOKEN

const BASE_URL = "https://hub.docker.com/api/publisher/analytics/v1"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o DVP_DATA_API_TOKEN | default "" }
...
]: nothing -> record<namespaces: list<string>> {
...
export def "namespaces-pulls-exports-years get-timespans" [
  ...
  if ($timespantype | is-empty) { error make --unspanned { msg: "path parameter 'timespantype' must be non-empty" } }
...
export def "namespaces-pulls-exports-years get-metadata" [
  ...
  if ($year | is-empty) { error make --unspanned { msg: "path parameter 'year' must be non-empty" } }
  if ($timespantype | is-empty) { error make --unspanned { msg: "path parameter 'timespantype' must be non-empty" } }
  if ($timespan | is-empty) { error make --unspanned { msg: "path parameter 'timespan' must be non-empty" } }
...
export def "namespaces-pulls-exports-years get-data" [
  ...
  if ($timespantype | is-empty) { error make --unspanned { msg: "path parameter 'timespantype' must be non-empty" } }
  if ($timespan | is-empty) { error make --unspanned { msg: "path parameter 'timespan' must be non-empty" } }
  if ($dataview | is-empty) { error make --unspanned { msg: "path parameter 'dataview' must be non-empty" } }
