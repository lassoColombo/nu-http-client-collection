...
export def "company-basicsearch list-basic" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"q": $q, "fields": $fields, "limit": $limit, "format": $format} | compact), body: null}
}
...
export def "company-competitorpremium-id get" [
  ...
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"pagination_id": $pagination_id, "format": $format} | compact), body: null}
}
...
export def "feed get" [
  ...
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"format": $format, "company_id": $company_id, "limit": $limit, "pagination_id": $pagination_id, "category": $category} | compact), body: null}
}
...
export def "feed-url get" [
  ...
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"format": $format, "domain": $domain, "limit": $limit, "pagination_id": $pagination_id, "category": $category} | compact), body: null}
}
