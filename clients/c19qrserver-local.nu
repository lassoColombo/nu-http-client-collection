...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o API_FOR_THE_COVID_19_TRACKING_QR_CODE_SIGNIN_SERVER_TOKEN | default "" }
...
export def "change-password create" [
  ...
  let full_url = (build-url $base "/changePassword" $auth.query)
...
export def "request-password-reset create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "token"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/requestPasswordReset" $auth.query)
...
export def "signin delete" [
  ...
  let full_url = (build-url $base ({signin_id: (encode-path-segment $signin_id)} | format pattern "/signin/{signin_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "signins get" [
  ...
  let full_url = (build-url $base "/signins" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"less_than": $less_than, "return_count": $return_count} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "verify-password-change create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "token"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/verifyPasswordChange" $auth.query)
