...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o DOMAINS_INDEX_API_TOKEN | default "" }
...
export def "domains-search get-item" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api_key": $api_key, "date": $date, "page": $page, "limit": $limit, "domain": $domain, "zone": $zone, "country": $country, "isDead": $is_dead, "A": $a, "NS": $ns, "CNAME": $cname, "MX": $mx, "TXT": $txt} | compact), body: null}
}
...
export def "domains-tld get-item" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api_key": $api_key, "date": $date, "page": $page, "limit": $limit, "domain": $domain, "country": $country, "isDead": $is_dead, "A": $a, "NS": $ns, "CNAME": $cname, "MX": $mx, "TXT": $txt} | compact), body: null}
}
...
export def "domains-tld-download get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api_key": $api_key, "date": $date} | compact), body: null}
}
...
export def "domains-updates-added get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api_key": $api_key, "date": $date, "page": $page, "limit": $limit} | compact), body: null}
}
