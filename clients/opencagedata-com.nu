...
export def "vversion get" [
  ...
  let full_url = (build-url $base ({version: (encode-path-segment $version), format: (encode-path-segment $format)} | format pattern "/v{version}/{format}") $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"q": $q, "key": $key, "abbrv": $abbrv, "address_only": $address_only, "add_request": $add_request, "bounds": $bounds, "countrycode": $countrycode, "jsonp": $jsonp, "language": $language, "limit": $limit, "min_confidence": $min_confidence, "no_annotations": $no_annotations, "no_dedupe": $no_dedupe, "no_record": $no_record, "pretty": $pretty, "proximity": $proximity, "roadinfo": $roadinfo} | compact), body: null}
}
