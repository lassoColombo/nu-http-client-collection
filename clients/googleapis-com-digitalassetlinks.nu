...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o DIGITAL_ASSET_LINKS_API_TOKEN | default "" }
...
export def "assetlinks-bulk-check check" [
  ...
  let full_url = (build-url $base "/v1/assetlinks:bulkCheck" $qp $auth.query)
...
export def "assetlinks-check check" [
  ...
  let full_url = (build-url $base "/v1/assetlinks:check" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"$.xgafv": $xgafv, "access_token": $access_token, "alt": $alt, "callback": $callback, "fields": $fields, "key": $key, "oauth_token": $oauth_token, "prettyPrint": $pretty_print, "quotaUser": $quota_user, "upload_protocol": $upload_protocol, "uploadType": $upload_type, "relation": $relation, "source.androidApp.certificate.sha256Fingerprint": $source_android_app_certificate_sha256_fingerprint, "source.androidApp.packageName": $source_android_app_package_name, "source.web.site": $source_web_site, "target.androidApp.certificate.sha256Fingerprint": $target_android_app_certificate_sha256_fingerprint, "target.androidApp.packageName": $target_android_app_package_name, "target.web.site": $target_web_site} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "statements-list list" [
  ...
  let full_url = (build-url $base "/v1/statements:list" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"$.xgafv": $xgafv, "access_token": $access_token, "alt": $alt, "callback": $callback, "fields": $fields, "key": $key, "oauth_token": $oauth_token, "prettyPrint": $pretty_print, "quotaUser": $quota_user, "upload_protocol": $upload_protocol, "uploadType": $upload_type, "relation": $relation, "source.androidApp.certificate.sha256Fingerprint": $source_android_app_certificate_sha256_fingerprint, "source.androidApp.packageName": $source_android_app_package_name, "source.web.site": $source_web_site} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
