...
export def "personas list" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"name": $name, "count": $count, "page": $page, "fields": $fields, "expand": $expand} | compact), body: null}
}
...
export def "personas get" [
  ...
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"fields": $fields, "expand": $expand} | compact), body: null}
}
...
export def "playlists list" [
  ...
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"start": $start, "end": $end, "show_id": $show_id, "persona_id": $persona_id, "count": $count, "page": $page, "fields": $fields, "expand": $expand} | compact), body: null}
}
...
export def "shows list" [
  ...
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"start": $start, "end": $end, "count": $count, "page": $page, "fields": $fields, "expand": $expand} | compact), body: null}
}
...
export def "spins list" [
  ...
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"start": $start, "end": $end, "playlist_id": $playlist_id, "show_id": $show_id, "count": $count, "page": $page, "fields": $fields, "expand": $expand} | compact), body: null}
}
