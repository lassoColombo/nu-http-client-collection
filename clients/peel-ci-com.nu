...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o PEEL_TUNE_IN_API_TOKEN | default "" }
...
export def "hashtag-related get" [
  ...
  let full_url = (build-url $base "/hashtag/related" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"showID": $show_id, "timeWindow": $time_window} | compact)
...
export def "hashtag-trending-shows get" [
  ...
  let full_url = (build-url $base "/hashtag/trendingShows" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"limit": $limit, "timeWindow": $time_window} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "hashtag-tuneinlinks get-tunein-links" [
  ...
  let full_url = (build-url $base "/hashtag/tuneinlinks" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"tweet": $tweet, "hashtags": $hashtags, "showID": $show_id} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "status get-statuses" [
  ...
  let full_url = (build-url $base ({show_id: (encode-path-segment $show_id)} | format pattern "/status/{show_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
