# Auto-generated client for Peel Tune-in API v1.0.0
# Source: https://api.apis.guru/v2/specs/peel-ci.com/1.0.0/swagger.json
# Auth: --token flag or $env.PEEL_TUNE_IN_API_TOKEN

const BASE_URL = "http://hashtag.peel-ci.com"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o PEEL_TUNE_IN_API_TOKEN | default "" }
...
def base-url-completer [] { ["http://hashtag.peel-ci.com"] }
...
export def "hashtag-related get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"showID": $show_id, "timeWindow": $time_window} | compact), body: null}
}
...
export def "hashtag-trending-shows get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"limit": $limit, "timeWindow": $time_window} | compact), body: null}
}
...
export def "hashtag-tuneinlinks get-tunein-links" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"tweet": $tweet, "hashtags": $hashtags, "showID": $show_id} | compact), body: null}
}
...
export def "status get-statuses" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($show_id | is-empty) { error make --unspanned { msg: "path parameter 'showID' must be non-empty" } }
  let full_url = (build-url $base ({show_id: (encode-path-segment $show_id)} | format pattern "/status/{show_id}"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
