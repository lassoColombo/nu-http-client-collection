...
export def "subscriptions-providers-microsoft-capacity-resource-providers-locations-service-limits-requests list-quota-status" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version, "$filter": $filter, "$top": $top, "$skiptoken": $skiptoken} | compact), body: null}
}
