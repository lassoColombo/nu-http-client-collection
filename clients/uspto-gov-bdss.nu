...
export def "products-byname get-by-name" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --from-year: int # Year from when the product files are needed, for example, 1999. (format: int32)
  --to-year: int # Year until when the product files are needed, for example, 2000. (format: int32)
  --from-month: int # Month from when the product files are needed, for example, 01. (format: int32)
  --to-month: int # Month until when the product files are needed, for example, 12. (format: int32)
  --from-day: int # Day from when the product files are needed, for example, 01. (format: int32)
  --to-day: int # Day until when the product files are needed, for example, 31. (format: int32)
  ...
  --max-files: int # Max. number of files to retrieve, per product. Set value to -1 to get all files (format: int32, default: 20)
]: nothing -> any {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"fromYear": $from_year, "toYear": $to_year, "fromMonth": $from_month, "toMonth": $to_month, "fromDay": $from_day, "toDay": $to_day, "hierarchy": $hierarchy, "maxFiles": $max_files} | compact), body: null}
}
...
export def "products get-by-short-name" [
  ...
  --from-day: int # Day from when the product files are needed, for example, 01. (format: int32)
  --to-day: int # Day until when the product files are needed, for example, 31. (format: int32)
  --from-date: string # Year from when the product files are needed, for example, 1999-01-01.
  --to-date: string # Year until when the product files are needed, for example, 2001-12-31.
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"fromYear": $from_year, "toYear": $to_year, "fromMonth": $from_month, "toMonth": $to_month, "fromDay": $from_day, "toDay": $to_day, "fromDate": $from_date, "toDate": $to_date, "hierarchy": $hierarchy} | compact), body: null}
}
...
export def "products-latest get-files-by-and-time" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"year": $year, "hierarchy": $hierarchy} | compact), body: null}
}
