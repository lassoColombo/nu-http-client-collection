...
# Auth: --token flag or $env.OPEN_POLICY_AGENT_OPA_REST_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o OPEN_POLICY_AGENT_OPA_REST_API_TOKEN | default "" }
...
export def "query-api create-simple" [
  ...
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"pretty": $pretty} | compact), body: $req_body}
}
...
export def "health get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"bundles": $bundles, "plugins": $plugins} | compact), body: null}
}
...
export def "data get-document-with-web-hook" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/x-yaml" $req_body {query: ({"pretty": $pretty} | compact), body: $req_body}
}
...
export def "compile create" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"pretty": $pretty, "explain": $explain, "metrics": $metrics, "instrument": $instrument} | compact), body: $req_body}
}
...
export def "data get-document-by-path" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"input": $input, "pretty": $pretty, "provenance": $provenance, "explain": $explain, "metrics": $metrics, "instrument": $instrument} | compact), body: null}
}
...
export def "data get-document-by-path-1" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/x-yaml" $req_body {query: ({"pretty": $pretty, "provenance": $provenance, "explain": $explain, "metrics": $metrics, "instrument": $instrument} | compact), body: $req_body}
}
...
export def "policies delete-policy-module" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"pretty": $pretty} | compact), body: null}
}
...
export def "policies update-policy-module" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "text/plain" $req_body {query: ({"pretty": $pretty, "metrics": $metrics} | compact), body: $req_body}
}
...
export def "query get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"q": $q, "pretty": $pretty, "explain": $explain, "metrics": $metrics} | compact), body: null}
}
...
export def "query create" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/x-yaml" $req_body {query: ({"pretty": $pretty, "explain": $explain, "metrics": $metrics} | compact), body: $req_body}
}
