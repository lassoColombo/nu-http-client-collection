# Auto-generated client for AzureDataManagementClient v2017-03-01-preview
# Source: https://api.apis.guru/v2/specs/azure.com/azuredata/2017-03-01-preview/swagger.json
# Auth: --token flag or $env.AZUREDATAMANAGEMENTCLIENT_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o AZUREDATAMANAGEMENTCLIENT_TOKEN | default "" }
...
export def "providers-microsoft-azure-data-operations list" [
  ...
  let full_url = (build-url $base "/providers/Microsoft.AzureData/operations" $qp)
...
export def "subscriptions-providers-microsoft-azure-data-sql-server-registrations list" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id)} | format pattern "/subscriptions/{subscription_id}/providers/Microsoft.AzureData/sqlServerRegistrations") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version} | compact), body: null}
}
...
export def "subscriptions-resource-groups-providers-microsoft-azure-data-sql-server-registrations list" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.AzureData/sqlServerRegistrations") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version} | compact), body: null}
}
...
export def "subscriptions-resource-groups-providers-microsoft-azure-data-sql-server-registrations delete" [
  ...
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
  if ($resource_group_name | is-empty) { error make --unspanned { msg: "path parameter 'resourceGroupName' must be non-empty" } }
  if ($sql_server_registration_name | is-empty) { error make --unspanned { msg: "path parameter 'sqlServerRegistrationName' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), sql_server_registration_name: (encode-path-segment $sql_server_registration_name)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.AzureData/sqlServerRegistrations/{sql_server_registration_name}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "subscriptions-resource-groups-providers-microsoft-azure-data-sql-server-registrations get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # API version to use for the request.
]: nothing -> record<properties: record<propertyBag: string, resourceGroup: string, subscriptionId: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
  if ($resource_group_name | is-empty) { error make --unspanned { msg: "path parameter 'resourceGroupName' must be non-empty" } }
...
]: any -> record<properties: record<propertyBag: string, resourceGroup: string, subscriptionId: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
...
export def "subscriptions-resource-groups-providers-microsoft-azure-data-sql-server-registrations create-or-update" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # API version to use for the request.
  --properties: record # The SQL server Registration properties. — shape: {propertyBag?: string, resourceGroup?: string, subscriptionId?: string}
...
export def "subscriptions-resource-groups-providers-microsoft-azure-data-sql-server-registrations-sql-servers delete" [
  ...
  if ($resource_group_name | is-empty) { error make --unspanned { msg: "path parameter 'resourceGroupName' must be non-empty" } }
  if ($sql_server_registration_name | is-empty) { error make --unspanned { msg: "path parameter 'sqlServerRegistrationName' must be non-empty" } }
  if ($sql_server_name | is-empty) { error make --unspanned { msg: "path parameter 'sqlServerName' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), sql_server_registration_name: (encode-path-segment $sql_server_registration_name), sql_server_name: (encode-path-segment $sql_server_name)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.AzureData/sqlServerRegistrations/{sql_server_registration_name}/sqlServers/{sql_server_name}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version} | compact), body: null}
}
...
export def "subscriptions-resource-groups-providers-microsoft-azure-data-sql-server-registrations-sql-servers get" [
  ...
  --expand: string # The child resources to include in the response.
  --api-version: string # API version to use for the request.
]: nothing -> record<properties: record<cores: int, edition: string, propertyBag: string, registrationID: string, version: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
  if ($resource_group_name | is-empty) { error make --unspanned { msg: "path parameter 'resourceGroupName' must be non-empty" } }
...
export def "subscriptions-resource-groups-providers-microsoft-azure-data-sql-server-registrations-sql-servers create-or-update" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # API version to use for the request.
  --properties: record # The SQL server properties. — shape: {cores?: int, edition?: string, propertyBag?: string, registrationID?: string, version?: string}
]: any -> record<properties: record<cores: int, edition: string, propertyBag: string, registrationID: string, version: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
