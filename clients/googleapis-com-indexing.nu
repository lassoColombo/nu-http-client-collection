...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o INDEXING_API_TOKEN | default "" }
...
export def "url-notifications-metadata get" [
  ...
  let full_url = (build-url $base "/v3/urlNotifications/metadata" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"$.xgafv": $xgafv, "access_token": $access_token, "alt": $alt, "callback": $callback, "fields": $fields, "key": $key, "oauth_token": $oauth_token, "prettyPrint": $pretty_print, "quotaUser": $quota_user, "upload_protocol": $upload_protocol, "uploadType": $upload_type, "url": $url} | compact)
...
export def "url-notifications-publish publish" [
  ...
  let full_url = (build-url $base "/v3/urlNotifications:publish" $qp $auth.query)
  let req_body = {"notifyTime": $notify_time, "type": $type, "url": $url} | compact
