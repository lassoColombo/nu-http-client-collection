# Auto-generated client for PolicyMetadataClient v2019-10-01
# Source: https://api.apis.guru/v2/specs/azure.com/policyinsights-policyMetadata/2019-10-01/swagger.json
# Auth: --token flag or $env.POLICYMETADATACLIENT_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o POLICYMETADATACLIENT_TOKEN | default "" }
...
export def "providers-microsoft-policy-insights-policy-metadata list" [
  ...
  let full_url = (build-url $base "/providers/Microsoft.PolicyInsights/policyMetadata" $qp)
...
export def "providers-microsoft-policy-insights-policy-metadata get-resource" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client Api Version.
]: nothing -> record<id: string, name: string, properties: record<description: string, requirements: string, additionalContentUrl: string, category: string, metadata: record, metadataId: string, owner: string, title: string>, type: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($resource_name | is-empty) { error make --unspanned { msg: "path parameter 'resourceName' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({resource_name: (encode-path-segment $resource_name)} | format pattern "/providers/Microsoft.PolicyInsights/policyMetadata/{resource_name}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
