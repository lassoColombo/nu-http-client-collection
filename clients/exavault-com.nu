...
def responseVersion-completer [] { ["v1" "v2"] }
...
export def "account updateAccount" [
  ...
  --accountOnboarding: oneof<nothing, bool> # Whether extra help popups can be enabled for users in the web file manager. (e.g. true)
  --allowedIpRanges: list # IP Address Ranges for restricting account access — item shape: {ipEnd?: string, ipStart?: string}
  --brandingSettings: record # shape: {companyName?: string, customEmail?: string, theme?: string}
  --complexPasswords: oneof<nothing, bool> # Whether to require complex passwords for all passwords. (e.g. false)
  --customSignature: string # Signature to be automatically added to the bottom of emails generated by the account.
  --emailContent: string # Content of welcome email template. (e.g. Great news, your new account is ready! For your records, we've listed information you'll use to log in below: FTP Server: [[ftpserver]] Username (Web and FTP access): [[username]] [[setpassword]])
  --emailSubject: string # Subject line for welcome emails (e.g. ExaVault File Sharing Account)
  ...
  --secureOnly: oneof<nothing, bool> # Whether unencrypted FTP connections should be denied for the account. (e.g. false)
  --showReferralLinks: oneof<nothing, bool> # Whether to display links for others to sign up on share views and invitation emails (e.g. false)
  ...
  let body = {accountOnboarding: $accountOnboarding, allowedIpRanges: $allowedIpRanges, brandingSettings: $brandingSettings, complexPasswords: $complexPasswords, customSignature: $customSignature, emailContent: $emailContent, emailSubject: $emailSubject, externalDomain: $externalDomain, quota: $quota, secureOnly: $secureOnly, showReferralLinks: $showReferralLinks} | compact
...
export def "activity-session get" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --allow-errors(-e) # Return full response without error handling
  --dry-run(-n) # Return the request that would be sent without executing it
  --startDate: string # Start date of the filter data range (format: date-time, e.g. 2019-10-18T06:48:40Z)
  --endDate: string # End date of the filter data range (format: date-time, e.g. 2019-10-18T06:48:40Z)
  --ipAddress: string # Used to filter session logs by ip address. (e.g. 127.0.0.1)
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "startDate" $startDate "scalar") (serialize-qp "endDate" $endDate "scalar") (serialize-qp "ipAddress" $ipAddress "scalar") (serialize-qp "username" $username "scalar") (serialize-qp "path" $path "scalar") (serialize-qp "type" $type "scalar") (serialize-qp "offset" $offset "scalar") (serialize-qp "limit" $limit "scalar") (serialize-qp "sort" $qp_sort "scalar")] | flatten | str join "&"
...
export def "activity-webhooks get" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --allow-errors(-e) # Return full response without error handling
  --dry-run(-n) # Return the request that would be sent without executing it
  --startDate: string # Earliest date of entries to include in list (format: date-time)
  --endDate: string # Latest date of entries to include in list (format: date-time)
  --endpointUrl: string # Webhook listener endpoint (format: uri)
  --event: string # Type of activity that triggered the webhook attempt (e.g. resources.upload)
  --statusCode: int # Response code from the webhook endpoint (e.g. 200)
  --resourcePath: string # Path of the resource that triggered the webhook attempt (e.g. /Production)
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "startDate" $startDate "scalar") (serialize-qp "endDate" $endDate "scalar") (serialize-qp "endpointUrl" $endpointUrl "scalar") (serialize-qp "event" $event "scalar") (serialize-qp "statusCode" $statusCode "scalar") (serialize-qp "resourcePath" $resourcePath "scalar") (serialize-qp "username" $username "scalar") (serialize-qp "offset" $offset "scalar") (serialize-qp "limit" $limit "scalar") (serialize-qp "sort" $qp_sort "scalar")] | flatten | str join "&"
...
export def "email-lists addEmailList" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
...
export def "email-lists delete" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/email-lists/($id)")
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "email-lists get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "include" $include "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/email-lists/($id)" $qp)
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "email-lists updateEmailListById" [
  id: int
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
export def "email-referral sendReferralEmail" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
...
export def "email-welcome sendWelcomeEmail" [
  username: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/email/welcome/($username)")
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "forms list" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "shareHash" $shareHash "scalar") (serialize-qp "include" $include "scalar")] | flatten | str join "&"
...
export def "forms-entries delete" [
  id: int
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/forms/entries/($id)")
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "forms-entries get" [
  ...
  let full_url = (build-url $base $"/forms/entries/($id)" $qp)
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "forms updateFormById" [
  id: int
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let body = {cssStyles: $cssStyles, elements: $elements, formDescription: $formDescription, submitButtonText: $submitButtonText, successMessage: $successMessage} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
...
export def "notifications addNotification" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --sendEmail: oneof<nothing, bool> # Set to true if the user should be notified by email when the notification is triggered. (e.g. true)
  ...
  let body = {action: $action, message: $message, recipients: $recipients, resource: $resource, sendEmail: $sendEmail, type: $type, usernames: $usernames} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
...
export def "notifications updateNotificationById" [
  id: int
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  --sendEmail: oneof<nothing, bool> # Whether an email should be sent to the recipients when matching activity happens. (e.g. true)
  ...
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/notifications/($id)")
  let body = {action: $action, message: $message, recipients: $recipients, sendEmail: $sendEmail, usernames: $usernames} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
...
export def "recipients-shares-invites resendInvitationsForShare" [
  ...
  --ev-api-key: string # API Key required to make the API call.
  --ev-access-token: string # Access token required to make the API call. (e.g. 5dc97cc607985eb8da033220e7447647e7915fbf73808)
  --recipientId: int # ID number of recipient to send a new invitation to. (format: int32)
]: any -> record<data: list<string>, responseStatus: int> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/recipients/shares/invites/($shareId)")
  let body = {recipientId: $recipientId} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
...
export def "resources addFolder" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/resources")
  let body = {name: $name, parentResource: $parentResource, path: $path} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
...
export def "resources-compress compressFiles" [
  ...
  let body = {archiveName: $archiveName, parentResource: $parentResource, resources: $resources} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
...
export def "resources-copy copyResources" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let body = {parentResource: $parentResource, resources: $resources} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
...
export def "resources-download download" [
  ...
  --downloadArchiveName: string # When downloading multiple files, this will be used as the name of the zip file that is created.
  --ev-api-key: string # API Key required to make the API call.
  --ev-access-token: string # Access token required to make the API call. (e.g. 5dc97cc607985eb8da033220e7447647e7915fbf73808)
]: nothing -> any {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "resources[]" $resources "multi") (serialize-qp "downloadArchiveName" $downloadArchiveName "scalar")] | flatten | str join "&"
...
export def "resources-extract extractFiles" [
  ...
  let body = {parentResource: $parentResource, resource: $resource} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
...
export def "resources-list listResources" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
...
export def "resources-list listResourceContents" [
  id: int
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let full_url = (build-url $base $"/resources/list/($id)" $qp)
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "resources-move moveResources" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --ev-api-key: string # API Key required to make the API call.
  --ev-access-token: string # Access token required to make the API call. (e.g. 5dc97cc607985eb8da033220e7447647e7915fbf73808)
  parentResource: string # Resource identifier of folder to move files/folders to. (e.g. /copyhere)
...
export def "resources-preview get" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
...
export def "resources-upload uploadFile" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --fileSize: int # File size, in bits, of the file being uploaded. (e.g. 2935)
  --resume: oneof<nothing, bool> # True if upload resume is supported, false if it isn't.  (default: true, e.g. true)
  --allowOverwrite: oneof<nothing, bool> # True if a file with the same name is found in the designated path, should be overwritten. False if different file names should be generated.  (default: false, e.g. true)
  --ev-api-key: string # API Key required to make the API call.
  --ev-access-token: string # Access token required to make the API call. (e.g. 5dc97cc607985eb8da033220e7447647e7915fbf73808)
  --offsetBytes: int # Allows a file upload to resume at a certain number of bytes. (e.g. 4852)
  --file: string # format: binary
]: any -> record<data: record<attributes: record<accessedAt: string, accessedTime: int, createdAt: string, createdBy: string, createdTime: int, extension: string, fileCount: int, hash: string, name: string, path: string, previewable: bool, size: int, type: string, updatedAt: string, updatedTime: int, uploadDate: string>, id: int, relationships: record<directFile: record, notifications: list, parentResource: record, share: record>, type: string>, included: list<any>, responseStatus: int> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "path" $path "scalar") (serialize-qp "fileSize" $fileSize "scalar") (serialize-qp "resume" $resume "scalar") (serialize-qp "allowOverwrite" $allowOverwrite "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/resources/upload" $qp)
  let body = {file: $file} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token, "offsetBytes": $offsetBytes} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "resources delete-by-id" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/resources/($id)")
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "resources get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "include" $include "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/resources/($id)" $qp)
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "resources updateResourceById" [
  id: int
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
export def "shares addShare" [
  ...
  --ev-api-key: string # API Key required to make the API call. (e.g. exampleaccount-zwSuWUZ8S38h33qPS8v0s)
  --ev-access-token: string # Access token required to make the API call. (e.g. 5dc97cc607985eb8da033220e7447647e7915fbf73808)
  --accessMode: record # An object defining what a not-logged-in visitor can do with the share contents — shape: {delete?: bool, download?: bool, modify?: bool, upload?: bool}
  ...
  --fileDropCreateFolders: oneof<nothing, bool> # Only used for **receive** shares. If true, uploads will be automatically placed into sub-folders of the folder, named after the chosen field on your form.  (e.g. false)
  --hasNotification: oneof<nothing, bool> # Whether delivery receipts should be sent. (e.g. false)
  --isPublic: oneof<nothing, bool> # Whether someone can visit the share without following a personalized recipient link. (e.g. true)
  ...
  --messageSubject: string # Subject to use on emails inviting recipients to the share. Ignored if you have not also provided `recipients` and a `messageBody` (e.g. Invitation to a shared folder)
  name: string # A name for the share. This will be visible on the page that recipients visit.  (e.g. Shared Folder)
  --notificationEmails: list # Emails that will receive delivery receipts for this share. `hasNotification` must be **true** for delivery receipts will be sent. (e.g. [notify@example.com, notify2@example.com])
  ...
  --requireEmail: oneof<nothing, bool> # True if recipients must provide their email to view the share. (e.g. false)
  ...
  let body = {accessMode: $accessMode, embed: $embed, expiration: $expiration, fileDropCreateFolders: $fileDropCreateFolders, hasNotification: $hasNotification, isPublic: $isPublic, messageBody: $messageBody, messageSubject: $messageSubject, name: $name, notificationEmails: $notificationEmails, password: $password, recipients: $recipients, requireEmail: $requireEmail, resources: $resources, sendingLocalFiles: $sendingLocalFiles, type: $type} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
...
export def "shares-complete-send completeDirectSend" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/shares/complete-send/($id)")
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "shares delete" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/shares/($id)")
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
# --accessMode shape: {delete?: bool, download?: bool, modify?: bool, upload?: bool}
# --recipients item shape: {email?: string, type?: string}
export def "shares updateShareById" [
  id: int
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  --fileDropCreateFolders: oneof<nothing, bool> # Whether uploads to a receive folder should be automatically placed into subfolders. See our [receive folder documentation](/docs/account/05-file-sharing/05-form-builder#advanced-form-settings) (e.g. false)
  --hasNotification: oneof<nothing, bool> # Whether delivery receipts should be sent for this share. (e.g. false)
  --isPublic: oneof<nothing, bool> # Whether people can visit the share without following a link from an invitation email (e.g. true)
  ...
  --messageSubject: string # Subject to use on emails inviting recipients to the share. Ignored if you have not also provided `recipients` and a `message` (e.g. Invitation to a shared folder)
  --name: string # Name of the share. (e.g. Shared Folder)
  --notificationEmails: list # List of email addresses to send delivery receipts to. Ignored if `hasNotification` is false.  (e.g. [notify@example.com, notify2@example.com])
  ...
  --requireEmail: oneof<nothing, bool> # Whether visitors to the share will be required to enter their email in order to access the share. (e.g. false)
  ...
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/shares/($id)")
  let body = {accessMode: $accessMode, embed: $embed, expiration: $expiration, fileDropCreateFolders: $fileDropCreateFolders, hasNotification: $hasNotification, isPublic: $isPublic, messageBody: $messageBody, messageSubject: $messageSubject, name: $name, notificationEmails: $notificationEmails, password: $password, recipients: $recipients, requireEmail: $requireEmail, resources: $resources, status: $status} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
...
export def "ssh-keys list" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --dry-run(-n) # Return the request that would be sent without executing it
  --userId: string #  Only return results for the given user ID. This is not the username, but the numeric ID of the user.
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "userId" $userId "scalar") (serialize-qp "limit" $limit "scalar") (serialize-qp "offset" $offset "scalar")] | flatten | str join "&"
...
export def "ssh-keys addSSHKey" [
  ...
  userId: int # ID of the user to assign the new key to. 
  ...
  let body = {publicKey: $publicKey, userId: $userId} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
...
export def "ssh-keys delete" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/ssh-keys/($id)")
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "users listUsers" [
  ...
  --homeResource: string # Resource identifier for user's home directory. Does not support wildcard searches.
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "username" $username "scalar") (serialize-qp "homeResource" $homeResource "scalar") (serialize-qp "nickname" $nickname "scalar") (serialize-qp "email" $email "scalar") (serialize-qp "role" $role "scalar") (serialize-qp "status" $status "scalar") (serialize-qp "search" $search "scalar") (serialize-qp "offset" $offset "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "limit" $limit "scalar") (serialize-qp "include" $include "scalar")] | flatten | str join "&"
...
export def "users addUser" [
  ...
  timeZone: string # Time zone, used for accurate time display within the application. See <a href='https://php.net/manual/en/timezones.php' target='blank'>this page</a> for allowed values.  (e.g. America/Los_Angeles)
  ...
  --welcomeEmail: oneof<nothing, bool> # If **true**, send this new user a welcome email upon creation. The content of the welcome email can be configured with the [PATCH /accounts](#operation/updateAccount) method. (e.g. true)
  ...
  let body = {email: $email, expiration: $expiration, homeResource: $homeResource, locked: $locked, nickname: $nickname, onboarding: $onboarding, password: $password, permissions: $permissions, role: $role, timeZone: $timeZone, username: $username, welcomeEmail: $welcomeEmail} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
...
export def "users updateUser" [
  ...
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/users/($id)")
  let body = {email: $email, expiration: $expiration, homeResource: $homeResource, locked: $locked, nickname: $nickname, onboarding: $onboarding, password: $password, permissions: $permissions, role: $role, timeZone: $timeZone, username: $username} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
...
export def "webhooks addWebhook" [
  ...
  --ev-api-key: string # API key required to make the API call.
  --ev-access-token: string # Access token required to make the API call. (e.g. 5dc97cc607985eb8da033220e7447647e7915fbf73808)
  endpointUrl: string # The endpoint is where the webhook request will be sent. (format: uri, e.g. https://example.com/webhook)
  --resource: string # Resource identifier for the top folder this webhook is associated with (e.g. /uploads-folder)
  --responseVersion: string@responseVersion-completer # What version of webhook request should be sent to the endpoint URL when messages are sent (e.g. v2)
  ...
  let body = {endpointUrl: $endpointUrl, resource: $resource, responseVersion: $responseVersion, triggers: $triggers} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
...
export def "webhooks-regenerate-token regenerateWebhookToken" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/webhooks/regenerate-token/($id)")
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "webhooks-resend resendWebhookActivityEntry" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/webhooks/resend/($activityId)")
  let extra_headers = {"ev-api-key": $ev_api_key, "ev-access-token": $ev_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "webhooks updateWebhook" [
  ...
  --ev-api-key: string # API key required to make the API call.
  --ev-access-token: string # Access token required to make the API call. (e.g. 5dc97cc607985eb8da033220e7447647e7915fbf73808)
  --endpointUrl: string # New endpoint URL to use for the webhook configuration (format: uri, e.g. https://example.com/new-endpoint)
  --resource: string # Resource identifier of the top folder watched by this webhook. (e.g. /newfolder)
  --responseVersion: string@responseVersion-completer # Version of the webhooks message to send to the endpoint (e.g. v1)
  --triggers: record # shape: {resources?: record, shares?: record}
]: any -> record<data: table<attributes: record, id: int, relationships: record, type: string>, included: list<any>, responseStatus: int> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
