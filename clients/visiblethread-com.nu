# Auto-generated client for VisibleThread API v1.0
# Source: https://api.apis.guru/v2/specs/visiblethread.com/1.0/swagger.json
# Auth: --token flag or $env.VISIBLETHREAD_API_TOKEN

const BASE_URL = "https://api.visiblethread.com/api/v1"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o VISIBLETHREAD_API_TOKEN | default "" }
...
export def "dictionaries get" [
  ...
  let full_url = (build-url $base "/dictionaries" $auth.query)
...
export def "dictionaries upload-dictionary" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  file: path # The uploaded CSV dictionary
]: any -> any {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
...
export def "searches get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/searches" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "searches get-list-results" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($doc_id | is-empty) { error make --unspanned { msg: "path parameter 'docId' must be non-empty" } }
  if ($dictionary_id | is-empty) { error make --unspanned { msg: "path parameter 'dictionaryId' must be non-empty" } }
  let qp = [(serialize-qp "matchingOnly" $matching_only "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({doc_id: (encode-path-segment $doc_id), dictionary_id: (encode-path-segment $dictionary_id)} | format pattern "/searches/{doc_id}/{dictionary_id}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"matchingOnly": $matching_only} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "webscans get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> table<completed: string, eta: string, id: int, scan: record<title: string, webUrls: list>, started: string> {
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/webscans" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "webscans get-scan" [
  ...
  let full_url = (build-url $base ({scan_id: (encode-path-segment $scan_id)} | format pattern "/webscans/{scan_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "webscans-web-urls get-scan" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($scan_id | is-empty) { error make --unspanned { msg: "path parameter 'scanId' must be non-empty" } }
  if ($url_id | is-empty) { error make --unspanned { msg: "path parameter 'urlId' must be non-empty" } }
  let full_url = (build-url $base ({scan_id: (encode-path-segment $scan_id), url_id: (encode-path-segment $url_id)} | format pattern "/webscans/{scan_id}/webUrls/{url_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
