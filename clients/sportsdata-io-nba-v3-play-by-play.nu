# Auto-generated client for NBA v3 Play-by-Play v1.0
# Source: https://api.apis.guru/v2/specs/sportsdata.io/nba-v3-play-by-play/1.0/openapi.json
# Auth: --token flag or $env.NBA_V3_PLAY_BY_PLAY_TOKEN

const BASE_URL = "http://azure-api.sportsdata.io/v3/nba/pbp"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o NBA_V3_PLAY_BY_PLAY_TOKEN | default "" }
...
def base-url-completer [] { ["http://azure-api.sportsdata.io/v3/nba/pbp" "https://azure-api.sportsdata.io/v3/nba/pbp"] }
...
export def "play-by-play get" [
  ...
  let full_url = (build-url $base ({format: (encode-path-segment $format), gameid: (encode-path-segment $gameid)} | format pattern "/{format}/PlayByPlay/{gameid}") $auth.query)
...
export def "play-by-play-delta get" [
  ...
  let full_url = (build-url $base ({format: (encode-path-segment $format), date: (encode-path-segment $date), minutes: (encode-path-segment $minutes)} | format pattern "/{format}/PlayByPlayDelta/{date}/{minutes}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
