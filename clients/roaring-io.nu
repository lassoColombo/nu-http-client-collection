# Auto-generated client for CompanyAPI v1.0
# Source: https://api.apis.guru/v2/specs/roaring.io/1.0/swagger.json
# Auth: --token flag or $env.COMPANYAPI_TOKEN

const BASE_URL = "https://api.roaring.io/company/1.0"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o COMPANYAPI_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.roaring.io/company/1.0" "http://api.roaring.io/company/1.0"] }
...
export def "company-board-members get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"countryCode": $country_code, "companyId": $company_id} | compact), body: null}
}
...
export def "company-board-members create" [
  ...
  let full_url = (build-url $base "/company-board-members" $qp)
  let req_body = {"companyIds": $company_ids} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"countryCode": $country_code} | compact), body: $req_body}
}
...
export def "company-credit-decision get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"countryCode": $country_code, "companyId": $company_id, "template": $template} | compact), body: null}
}
...
export def "company-simple-search get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"countryCode": $country_code, "companyName": $company_name, "town": $town} | compact), body: null}
}
