# Auto-generated client for Stellastra v1.0
# Source: https://api.apis.guru/v2/specs/stellastra.com/1.0/openapi.json
# Auth: --token flag or $env.STELLASTRA_TOKEN

const BASE_URL = "https://stellastra.com/api"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o STELLASTRA_TOKEN | default "" }
...
export def "post-review create" [
  ...
  --rating-body: int # e.g. 5 (body field)
  --user-email-body: string # e.g. johnsmith@usercompanyxyz.com (body field)
  --user-name-body: string # e.g. John (body field)
  ...
  let req_body = {"rating": $rating_body, "user_email": $user_email_body, "user_name": $user_name_body} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"user_email": $user_email, "user_name": $user_name, "rating": $rating} | compact), body: $req_body}
}
