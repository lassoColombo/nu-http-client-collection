...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o STELLASTRA_TOKEN | default "" }
...
export def "post-review create" [
  ...
  let full_url = (build-url $base "/post-review" $qp $auth.query)
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"user_email": $user_email, "user_name": $user_name, "rating": $rating} | compact)
