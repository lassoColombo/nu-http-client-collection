# Auto-generated client for Anomaly Finder Client v2.0
# Source: https://api.apis.guru/v2/specs/azure.com/cognitiveservices-AnomalyFinder/2.0/swagger.json
# Auth: --token flag or $env.ANOMALY_FINDER_CLIENT_TOKEN

const BASE_URL = "{Endpoint}/anomalyfinder/v2.0"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o ANOMALY_FINDER_CLIENT_TOKEN | default "" }
...
def base-url-completer [] { ["{Endpoint}/anomalyfinder/v2.0"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "timeseries-entire-detect create" } } | get name | first)
...
export def "timeseries-entire-detect create" [
  ...
  let full_url = (build-url $base "/timeseries/entire/detect" $auth.query)
...
export def "timeseries-last-detect create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "ocp-apim-subscription-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/timeseries/last/detect" $auth.query)
  let req_body = {"customInterval": $custom_interval, "granularity": $granularity, "maxAnomalyRatio": $max_anomaly_ratio, "period": $period, "sensitivity": $sensitivity, "series": $series} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
