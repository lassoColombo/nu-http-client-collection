# Auto-generated client for Azure Stack Azure Bridge Client v2017-06-01
# Source: https://api.apis.guru/v2/specs/azure.com/azurestack-Registration/2017-06-01/swagger.json
# Auth: --token flag or $env.AZURE_STACK_AZURE_BRIDGE_CLIENT_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AZURE_STACK_AZURE_BRIDGE_CLIENT_TOKEN | default "" }
...
export def "subscriptions-resource-groups-providers-microsoft-azure-stack-registrations list" [
  ...
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group: (encode-path-segment $resource_group)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group}/providers/Microsoft.AzureStack/registrations") $qp $auth.query)
...
export def "subscriptions-resource-groups-providers-microsoft-azure-stack-registrations delete" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group: (encode-path-segment $resource_group), registration_name: (encode-path-segment $registration_name)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group}/providers/Microsoft.AzureStack/registrations/{registration_name}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "subscriptions-resource-groups-providers-microsoft-azure-stack-registrations get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client API Version. (default: 2017-06-01)
]: nothing -> record<properties: record<billingModel: string, cloudId: string, objectId: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
  if ($resource_group | is-empty) { error make --unspanned { msg: "path parameter 'resourceGroup' must be non-empty" } }
...
export def "subscriptions-resource-groups-providers-microsoft-azure-stack-registrations update" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client API Version. (default: 2017-06-01)
  --location: string@location-completer # Location of the resource.
  --properties: record # Properties of the Azure Stack registration resource — shape: {registrationToken: string}
]: any -> record<properties: record<billingModel: string, cloudId: string, objectId: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
...
export def "subscriptions-resource-groups-providers-microsoft-azure-stack-registrations-get-activationkey get-activation-key" [
  ...
  if ($registration_name | is-empty) { error make --unspanned { msg: "path parameter 'registrationName' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group: (encode-path-segment $resource_group), registration_name: (encode-path-segment $registration_name)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group}/providers/Microsoft.AzureStack/registrations/{registration_name}/getactivationkey") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
