# Auto-generated client for Georg API v2.1
# Source: https://api.apis.guru/v2/specs/nrm.se/georg/2.1/swagger.json
# Auth: --token flag or $env.GEORG_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o GEORG_API_TOKEN | default "" }
...
export def "autocomplete complete-auto" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"text": $text, "sources": $sources, "layers": $layers, "countryCode": $country_code, "size": $size} | compact), body: null}
}
...
export def "coordinates list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"coordinates": $coordinates} | compact), body: null}
}
...
export def "reverse get-geo-code" [
  ...
  let full_url = (build-url $base "/reverse" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "upload upload-file" [
  ...
  let req_body = {"formData": $form_data, "formDataMap": $form_data_map, "parts": $parts, "preamble": $preamble} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/octet-stream"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req_body = if ($req_body | describe | str starts-with "record") { $req_body } else { {} }
  let mp = (build-multipart-body $req_body [] $dry_run)
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full $mp.content_type $mp.body {query: ({"type": $type} | compact), body: $req_body}
}
