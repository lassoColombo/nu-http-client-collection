...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o GEORG_API_TOKEN | default "" }
...
export def "autocomplete complete-auto" [
  ...
  let full_url = (build-url $base "/autocomplete" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"text": $text, "sources": $sources, "layers": $layers, "countryCode": $country_code, "size": $size} | compact)
...
export def "coordinates list" [
  ...
  let full_url = (build-url $base "/coordinates" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"coordinates": $coordinates} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "reverse get-geo-code" [
  ...
  let full_url = (build-url $base "/reverse" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
