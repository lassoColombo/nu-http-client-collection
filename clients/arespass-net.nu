# Auto-generated client for Arespass v1.0
# Source: https://api.apis.guru/v2/specs/arespass.net/1.0/openapi.json
# Auth: --token flag or $env.ARESPASS_TOKEN

const BASE_URL = "http://arespass.net/v1.0"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o ARESPASS_TOKEN | default "" }
...
def base-url-completer [] { ["http://arespass.net/v1.0"] }
def auth-scheme-completer [] { ["bearer"] }

# Completers for enum parameters
def accept-completer [] { ["application/json" "application/x-yaml" "application/xml"] }
...
export def "ec get" [
  ...
  let full_url = (build-url $base "/ec" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"password": $password, "outputFormat": $output_format, "penalty": $penalty, "reqId": $req_id} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
