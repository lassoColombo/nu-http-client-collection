...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o PAGESPEED_INSIGHTS_API_TOKEN | default "" }
...
export def "pagespeedonline-run-pagespeed get-runpagespeed" [
  ...
  --category: list<string> # A Lighthouse category to run; if none are given, only Performance category will be run
  ...
  let full_url = (build-url $base "/pagespeedonline/v5/runPagespeed" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"$.xgafv": $xgafv, "access_token": $access_token, "alt": $alt, "callback": $callback, "fields": $fields, "key": $key, "oauth_token": $oauth_token, "prettyPrint": $pretty_print, "quotaUser": $quota_user, "upload_protocol": $upload_protocol, "uploadType": $upload_type, "url": $url, "captchaToken": $captcha_token, "category": $category, "locale": $locale, "strategy": $strategy, "utm_campaign": $utm_campaign, "utm_source": $utm_source} | compact)
