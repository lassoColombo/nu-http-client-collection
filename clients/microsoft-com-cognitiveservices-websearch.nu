...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o WEB_SEARCH_CLIENT_TOKEN | default "" }
...
export def "search list-web" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"answerCount": $answer_count, "cc": $cc, "count": $count, "freshness": $freshness, "mkt": $mkt, "offset": $offset, "promote": $promote, "q": $q, "responseFilter": $response_filter, "safeSearch": $safe_search, "setLang": $set_lang, "textDecorations": $text_decorations, "textFormat": $text_format} | compact)
