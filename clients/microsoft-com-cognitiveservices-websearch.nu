# Auto-generated client for Web Search Client v1.0
# Source: https://api.apis.guru/v2/specs/microsoft.com/cognitiveservices-WebSearch/1.0/swagger.json
# Auth: --token flag or $env.WEB_SEARCH_CLIENT_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o WEB_SEARCH_CLIENT_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "search list-web" } } | get name | first)
...
export def "search list-web" [
...
]: nothing -> record<computation: record<expression: string, value: string>, images: record<nextOffset: int, queryExpansions: list<record>, relatedSearches: list<record>, similarTerms: list<record>, value: list<record>>, news: record<location: string, value: list<record>>, queryContext: record<adultIntent: bool, alterationOverrideQuery: string, alteredQuery: string, askUserForLocation: bool, isTransactional: bool, originalQuery: string>, rankingResponse: record<mainline: record<items: list>, pole: record<items: list>, sidebar: record<items: list>>, relatedSearches: record<value: list<record>>, spellSuggestions: record<value: list<record>>, timeZone: record<otherCityTimes: list<record>, primaryCityTime: record<location: string, time: string, utcOffset: string>>, videos: record<nextOffset: int, queryExpansions: list<record>, relatedSearches: list<record>, value: list<record>>, webPages: record<someResultsRemoved: bool, value: list<record>>> {
  let auth = (build-auth $token ($auth_scheme | default "ocp-apim-subscription-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "answerCount" $answer_count "scalar") (serialize-qp "cc" $cc "scalar") (serialize-qp "count" $count "scalar") (serialize-qp "freshness" $freshness "scalar") (serialize-qp "mkt" $mkt "scalar") (serialize-qp "offset" $offset "scalar") (serialize-qp "promote" $promote "csv") (serialize-qp "q" $q "scalar") (serialize-qp "responseFilter" $response_filter "csv") (serialize-qp "safeSearch" $safe_search "scalar") (serialize-qp "setLang" $set_lang "scalar") (serialize-qp "textDecorations" $text_decorations "scalar") (serialize-qp "textFormat" $text_format "scalar")] | flatten | str join "&"
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"answerCount": $answer_count, "cc": $cc, "count": $count, "freshness": $freshness, "mkt": $mkt, "offset": $offset, "promote": $promote, "q": $q, "responseFilter": $response_filter, "safeSearch": $safe_search, "setLang": $set_lang, "textDecorations": $text_decorations, "textFormat": $text_format} | compact), body: null}
}
