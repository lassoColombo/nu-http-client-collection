...
# Auth: --token flag or $env.PANCHAYATI_RAJ_DEPARTMENT_HIMACHAL_PRADESH__HIMACHAL_PRADESH_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o PANCHAYATI_RAJ_DEPARTMENT_HIMACHAL_PRADESH__HIMACHAL_PRADESH_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "fmcer-certificate fmcer" } } | get name | first)
