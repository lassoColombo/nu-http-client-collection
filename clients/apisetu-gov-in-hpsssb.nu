...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o HP_STAFF_SELECTION_COMMISSION_HPSSC_HIMACHAL_PRADESH_HIMACHAL_PRADESH_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/hpsssb/v3"] }
...
# operationId: adcrd
# --certificateParameters shape: {DOB: string, FatherName: string, FullName: string}
...
export def "adcrd-certificate create" [
  ...
  --certificate-parameters: record # shape: {DOB: string, FatherName: string, FullName: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o HP_STAFF_SELECTION_COMMISSION_HPSSC_HIMACHAL_PRADESH_HIMACHAL_PRADESH_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o HP_STAFF_SELECTION_COMMISSION_HPSSC_HIMACHAL_PRADESH_HIMACHAL_PRADESH_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
