...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o CUSTOM_SEARCH_CLIENT_TOKEN | default "" }
...
export def "search list-custom-instance" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"customConfig": $custom_config, "cc": $cc, "count": $count, "mkt": $mkt, "offset": $offset, "q": $q, "safeSearch": $safe_search, "setLang": $set_lang, "textDecorations": $text_decorations, "textFormat": $text_format} | compact)
