...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o AUTOSUGGEST_CLIENT_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "suggestions AutoSuggest" } } | get name | first)
...
export def "suggestions AutoSuggest" [
  ...
  let qp = [(serialize-qp "cc" $cc "scalar") (serialize-qp "mkt" $mkt "scalar") (serialize-qp "q" $q "scalar") (serialize-qp "safeSearch" $safeSearch "scalar") (serialize-qp "setLang" $setLang "scalar") (serialize-qp "ResponseFormat" $ResponseFormat "csv")] | flatten | str join "&"
