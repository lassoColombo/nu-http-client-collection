...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o FLIGHT_AVAILIBILITIES_SEARCH_TOKEN | default "" }
...
export def "shopping-availability-flight-availabilities list" [
  ...
  let full_url = (build-url $base "/shopping/availability/flight-availabilities" $auth.query)
  let req_body = $body
