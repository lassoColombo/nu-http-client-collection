...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o NOWPAYMENTS_API_TOKEN | default "" }
...
export def "estimate get-estimated-price" [
  ...
  let full_url = (build-url $base "/v1/estimate" $qp $auth.query)
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"amount": $amount, "currency_from": $currency_from, "currency_to": $currency_to} | compact)
...
export def "min-amount get-minimum-payment" [
  ...
  let full_url = (build-url $base "/v1/min-amount" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-api-key": $x_api_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "get"
    url: $full_url
    query: ({"currency_from": $currency_from, "currency_to": $currency_to} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "payment get-list" [
  ...
  let full_url = (build-url $base "/v1/payment/" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-api-key": $x_api_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "get"
    url: $full_url
    query: ({"limit": $limit, "page": $page, "sortBy": $sort_by, "orderBy": $order_by, "dateFrom": $date_from, "dateTo": $date_to} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "payment-update-merchant-estimate get" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/v1/payment/{id}/update-merchant-estimate") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-api-key": $x_api_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "payment get-status" [
  ...
  let full_url = (build-url $base ({payment_id: (encode-path-segment $payment_id)} | format pattern "/v1/payment/{payment_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-api-key": $x_api_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "payout-verify verify" [
  ...
  let full_url = (build-url $base ({withdrawals_id: (encode-path-segment $withdrawals_id)} | format pattern "/v1/payout/{withdrawals_id}/verify") $auth.query)
...
export def "sub-partner get" [
  ...
  let full_url = (build-url $base "/v1/sub-partner" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"id": $id, "offset": $offset, "limit": $limit, "order": $order} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "sub-partner-balance get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/v1/sub-partner/balance/{id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-api-key": $x_api_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "sub-partner-transfer get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/v1/sub-partner/transfer/{id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "sub-partner-transfers get-list" [
  ...
  let full_url = (build-url $base "/v1/sub-partner/transfers" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"id": $id, "status": $status, "limit": $limit, "offset": $offset, "order": $order} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "subscriptions get-many-recurring-payments" [
  ...
  --offset: string # e.g. 0
  --x-api-key: string # e.g. {{your_api_key}}
]: nothing -> record<count: float, result: table<created_at: string, expire_date: string, id: string, is_active: bool, status: string, subscriber: record, subscription_plan_id: string, updated_at: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"status": $status, "subscription_plan_id": $subscription_plan_id, "is_active": $is_active, "limit": $limit, "offset": $offset} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "subscriptions-plans get-many" [
  ...
  let full_url = (build-url $base "/v1/subscriptions/plans" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-api-key": $x_api_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "subscriptions-plans get-one" [
  ...
  let full_url = (build-url $base ({plan_id: (encode-path-segment $plan_id)} | format pattern "/v1/subscriptions/plans/{plan_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-api-key": $x_api_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "subscriptions delete-recurring-payment" [
  ...
  let full_url = (build-url $base ({sub_id: (encode-path-segment $sub_id)} | format pattern "/v1/subscriptions/{sub_id}") $auth.query)
  let req_body = $body
...
export def "subscriptions get-one-recurring-payment" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --x-api-key: string # e.g. {{your_api_key}}
]: nothing -> record<result: record<created_at: string, expire_date: string, id: string, is_active: bool, status: string, subscriber: record<sub_partner_id: string>, subscription_plan_id: string, updated_at: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($sub_id | is-empty) { error make --unspanned { msg: "path parameter 'sub_id' must be non-empty" } }
  let full_url = (build-url $base ({sub_id: (encode-path-segment $sub_id)} | format pattern "/v1/subscriptions/{sub_id}") $auth.query)
