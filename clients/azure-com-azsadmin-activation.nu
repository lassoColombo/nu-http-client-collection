...
export def "subscriptions-resourcegroups-providers-microsoft-azure-bridge-admin-activations List" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourcegroups/($resourceGroup)/providers/Microsoft.AzureBridge.Admin/activations" $qp)
...
export def "subscriptions-resourcegroups-providers-microsoft-azure-bridge-admin-activations Delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourcegroups/($resourceGroup)/providers/Microsoft.AzureBridge.Admin/activations/($activationName)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "subscriptions-resourcegroups-providers-microsoft-azure-bridge-admin-activations CreateOrUpdate" [
  subscriptionId: string
  resourceGroup: string
  activationName: string
  --base-url(-b): string@base-url-completer # API base URL
