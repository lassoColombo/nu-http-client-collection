# Auto-generated client for NASCAR v2 v1.0
# Source: https://api.apis.guru/v2/specs/sportsdata.io/nascar-v2/1.0/openapi.json
# Auth: --token flag or $env.NASCAR_V2_TOKEN

const BASE_URL = "http://azure-api.sportsdata.io/nascar/v2"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o NASCAR_V2_TOKEN | default "" }
...
def base-url-completer [] { ["http://azure-api.sportsdata.io/nascar/v2" "https://azure-api.sportsdata.io/nascar/v2"] }
...
export def "driver-race-projections list-entry" [
  ...
  if ($raceid | is-empty) { error make --unspanned { msg: "path parameter 'raceid' must be non-empty" } }
  let full_url = (build-url $base ({format: (encode-path-segment $format), raceid: (encode-path-segment $raceid)} | format pattern "/{format}/DriverRaceProjections/{raceid}") $auth.query)
...
export def "driver get-details" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($format | is-empty) { error make --unspanned { msg: "path parameter 'format' must be non-empty" } }
  if ($driverid | is-empty) { error make --unspanned { msg: "path parameter 'driverid' must be non-empty" } }
  let full_url = (build-url $base ({format: (encode-path-segment $format), driverid: (encode-path-segment $driverid)} | format pattern "/{format}/driver/{driverid}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "drivers get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($format | is-empty) { error make --unspanned { msg: "path parameter 'format' must be non-empty" } }
  let full_url = (build-url $base ({format: (encode-path-segment $format)} | format pattern "/{format}/drivers") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "raceresult get-race-results" [
  ...
  if ($format | is-empty) { error make --unspanned { msg: "path parameter 'format' must be non-empty" } }
  if ($raceid | is-empty) { error make --unspanned { msg: "path parameter 'raceid' must be non-empty" } }
  let full_url = (build-url $base ({format: (encode-path-segment $format), raceid: (encode-path-segment $raceid)} | format pattern "/{format}/raceresult/{raceid}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "races get-schedule" [
  ...
  let full_url = (build-url $base ({format: (encode-path-segment $format), season: (encode-path-segment $season)} | format pattern "/{format}/races/{season}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "series get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($format | is-empty) { error make --unspanned { msg: "path parameter 'format' must be non-empty" } }
  let full_url = (build-url $base ({format: (encode-path-segment $format)} | format pattern "/{format}/series") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
