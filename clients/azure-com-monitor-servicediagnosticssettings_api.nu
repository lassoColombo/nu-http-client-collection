# Auto-generated client for MonitorManagementClient v2016-09-01
# Source: https://api.apis.guru/v2/specs/azure.com/monitor-serviceDiagnosticsSettings_API/2016-09-01/swagger.json
...
export def "providers-microsoft-insights-diagnostic-settings-service get" [
...
]: nothing -> record<properties: record<eventHubAuthorizationRuleId: string, logs: list<record>, metrics: list<record>, serviceBusRuleId: string, storageAccountId: string, workspaceId: string>> {
  ...
  let full_url = (build-url $base ({resource_uri: (encode-path-segment $resource_uri)} | format pattern "/{resource_uri}/providers/microsoft.insights/diagnosticSettings/service") $qp $auth.query)
...
export def "providers-microsoft-insights-diagnostic-settings-service update" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client Api Version.
  --properties: any # The diagnostic settings for service. — shape: {eventHubAuthorizationRuleId?: string, logs?: list, metrics?: list, serviceBusRuleId?: string, storageAccountId?: string, workspaceId?: string}
  --tags: any # Resource tags
]: any -> record<properties: record<eventHubAuthorizationRuleId: string, logs: list<record>, metrics: list<record>, serviceBusRuleId: string, storageAccountId: string, workspaceId: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($resource_uri | is-empty) { error make --unspanned { msg: "path parameter 'resourceUri' must be non-empty" } }
