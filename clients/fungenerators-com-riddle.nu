...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o FUN_GENERATORS_API_TOKEN | default "" }
...
export def "riddle delete" [
  ...
  let full_url = (build-url $base "/riddle" $qp $auth.query)
...
export def "riddle create" [
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"question": $question, "category": $category, "answer": $answer} | compact)
    headers: $auth.headers
    body: null
    content_type: "application/json"
    timeout: ($max_time | default 30min)
...
export def "riddle-random get" [
  ...
  let full_url = (build-url $base "/riddle/random" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"category": $category} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "riddle-search get" [
  ...
  let full_url = (build-url $base "/riddle/search" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"query": $query, "category": $category} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
