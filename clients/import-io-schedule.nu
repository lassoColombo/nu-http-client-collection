...
# Source: https://api.apis.guru/v2/specs/import.io/schedule/1.0/swagger.json
# Auth: --token flag or $env.IMPORT_IO_TOKEN

const BASE_URL = "https://schedule.import.io"
...
def base-url-completer [] { ["https://schedule.import.io"] }
...
export def "extractor list" [
  ...
  let full_url = (build-url $base "/extractor" $auth.query)
...
export def "extractor create" [
  ...
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/extractor" $auth.query)
  let req_body = {"extractorId": $extractor_id, "interval": $interval, "startTimestamp": $start_timestamp} | compact
...
export def "extractor delete" [
  ...
  let full_url = (build-url $base ({extractor_id: (encode-path-segment $extractor_id)} | format pattern "/extractor/{extractor_id}/") $auth.query)
  let accept_val = "application/json;charset=UTF-8"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
