# Auto-generated client for Conversation API v1.0.1
# Source: https://api.apis.guru/v2/specs/nexmo.com/conversation.v2/1.0.1/openapi.json
# Auth: --token flag or $env.CONVERSATION_API_TOKEN

const BASE_URL = "https://api.nexmo.com/v0.2"
...
def base-url-completer [] { ["https://api.nexmo.com/v0.2"] }
...
export def "conversations get" [
  ...
  let qp = [(serialize-qp "page_size" $page_size "scalar") (serialize-qp "order" $order "scalar") (serialize-qp "cursor" $cursor "scalar") (serialize-qp "date_start" $date_start "scalar") (serialize-qp "date_end" $date_end "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/conversations" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"page_size": $page_size, "order": $order, "cursor": $cursor, "date_start": $date_start, "date_end": $date_end} | compact), body: null}
}
...
export def "conversations-events get" [
...
]: nothing -> record<_embedded: record<data: record<events: list>>, _links: record<first: record<href: string>, next: record<href: string>, prev: record<href: string>, self: record<href: string>>, page_size: int> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"page_size": $page_size, "order": $order, "cursor": $cursor, "start_id": $start_id, "end_id": $end_id, "event_type": $event_type} | compact), body: null}
}
...
export def "conversations-members get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"page_size": $page_size, "order": $order, "cursor": $cursor} | compact), body: null}
}
