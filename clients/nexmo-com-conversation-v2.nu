...
export def "conversations get" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"page_size": $page_size, "order": $order, "cursor": $cursor, "date_start": $date_start, "date_end": $date_end} | compact)
...
export def "conversations-events get" [
  ...
  let full_url = (build-url $base ({conversation_id: (encode-path-segment $conversation_id)} | format pattern "/conversations/{conversation_id}/events") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"page_size": $page_size, "order": $order, "cursor": $cursor, "start_id": $start_id, "end_id": $end_id, "event_type": $event_type} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "conversations-members get" [
  ...
  let full_url = (build-url $base ({conversation_id: (encode-path-segment $conversation_id)} | format pattern "/conversations/{conversation_id}/members") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"page_size": $page_size, "order": $order, "cursor": $cursor} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
