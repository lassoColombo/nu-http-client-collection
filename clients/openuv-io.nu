...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o OPENUV_GLOBAL_REAL_TIME_UV_INDEX_FORECAST_API_TOKEN | default "" }
...
export def "forecast get" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"lat": $lat, "lng": $lng, "alt": $alt, "ozone": $ozone, "dt": $dt} | compact)
...
export def "protection get" [
  ...
  let full_url = (build-url $base "/protection" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-access-token": $x_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "get"
    url: $full_url
    query: ({"lat": $lat, "lng": $lng, "from": $qp_from, "to": $qp_to, "alt": $alt, "ozone": $ozone} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "uv get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "lat" $lat "scalar") (serialize-qp "lng" $lng "scalar") (serialize-qp "alt" $alt "scalar") (serialize-qp "ozone" $ozone "scalar") (serialize-qp "dt" $dt "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/uv" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-access-token": $x_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
