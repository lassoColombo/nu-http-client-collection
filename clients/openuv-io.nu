...
export def "forecast get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"lat": $lat, "lng": $lng, "alt": $alt, "ozone": $ozone, "dt": $dt} | compact), body: null}
}
...
export def "protection get" [
  ...
  let extra_headers = {"x-access-token": $x_access_token} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"lat": $lat, "lng": $lng, "from": $qp_from, "to": $qp_to, "alt": $alt, "ozone": $ozone} | compact), body: null}
}
