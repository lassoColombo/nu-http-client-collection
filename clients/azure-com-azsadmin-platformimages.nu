...
export def "subscriptions-providers-microsoft-compute-admin-locations-artifact-types-platform-image List" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/providers/Microsoft.Compute.Admin/locations/($location)/artifactTypes/platformImage" $qp)
...
export def "subscriptions-providers-microsoft-compute-admin-locations-artifact-types-platform-image-publishers-offers-skus-versions Delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/providers/Microsoft.Compute.Admin/locations/($location)/artifactTypes/platformImage/publishers/($publisher)/offers/($offer)/skus/($sku)/versions/($version)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
