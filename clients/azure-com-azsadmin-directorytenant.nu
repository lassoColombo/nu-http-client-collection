...
# Source: https://api.apis.guru/v2/specs/azure.com/azsadmin-DirectoryTenant/2015-11-01/swagger.json
...
export def "subscriptions-resourcegroups-providers-microsoft-subscriptions-admin-directory-tenants list" [
  ...
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name)} | format pattern "/subscriptions/{subscription_id}/resourcegroups/{resource_group_name}/providers/Microsoft.Subscriptions.Admin/directoryTenants") $qp $auth.query)
...
export def "subscriptions-resourcegroups-providers-microsoft-subscriptions-admin-directory-tenants delete" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), tenant: (encode-path-segment $tenant)} | format pattern "/subscriptions/{subscription_id}/resourcegroups/{resource_group_name}/providers/Microsoft.Subscriptions.Admin/directoryTenants/{tenant}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "subscriptions-resourcegroups-providers-microsoft-subscriptions-admin-directory-tenants create-or-update" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client Api Version. (default: 2015-11-01)
  --properties: record # Directory tenant. — shape: {tenantId?: string}
