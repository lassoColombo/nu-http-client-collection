# Auto-generated client for SqlManagementClient v2019-06-01-preview
# Source: https://api.apis.guru/v2/specs/azure.com/sql-WorkloadGroups/2019-06-01-preview/swagger.json
...
export def "subscriptions-resource-groups-providers-microsoft-sql-servers-databases-workload-groups list" [
  ...
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), server_name: (encode-path-segment $server_name), database_name: (encode-path-segment $database_name)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Sql/servers/{server_name}/databases/{database_name}/workloadGroups") $qp $auth.query)
...
export def "subscriptions-resource-groups-providers-microsoft-sql-servers-databases-workload-groups delete" [
  ...
  if ($server_name | is-empty) { error make --unspanned { msg: "path parameter 'serverName' must be non-empty" } }
  if ($database_name | is-empty) { error make --unspanned { msg: "path parameter 'databaseName' must be non-empty" } }
  if ($workload_group_name | is-empty) { error make --unspanned { msg: "path parameter 'workloadGroupName' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), server_name: (encode-path-segment $server_name), database_name: (encode-path-segment $database_name), workload_group_name: (encode-path-segment $workload_group_name)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Sql/servers/{server_name}/databases/{database_name}/workloadGroups/{workload_group_name}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "subscriptions-resource-groups-providers-microsoft-sql-servers-databases-workload-groups get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # The API version to use for the request.
]: nothing -> record<properties: record<importance: string, maxResourcePercent: int, maxResourcePercentPerRequest: float, minResourcePercent: int, minResourcePercentPerRequest: float, queryExecutionTimeout: int>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
  if ($resource_group_name | is-empty) { error make --unspanned { msg: "path parameter 'resourceGroupName' must be non-empty" } }
...
export def "subscriptions-resource-groups-providers-microsoft-sql-servers-databases-workload-groups create-or-update" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # The API version to use for the request.
  --properties: record # Workload group definition. For more information look at sys.workload_management_workload_groups (DMV). — shape: {importance?: string, maxResourcePercent: int, maxResourcePercentPerRequest?: float, minResourcePercent: int, minResourcePercentPerRequest: float, queryExecutionTimeout?: int}
]: any -> record<properties: record<importance: string, maxResourcePercent: int, maxResourcePercentPerRequest: float, minResourcePercent: int, minResourcePercentPerRequest: float, queryExecutionTimeout: int>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
