# Auto-generated client for Swagger Petstore - OpenAPI 3.0 v1.0.27
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o SWAGGER_PETSTORE_OPENAPI_3_0_TOKEN | default "" }
...
]: any -> record<id: int, name: string, category: record<id: int, name: string>, photoUrls: list<string>, tags: table<id: int, name: string>, status: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/pet")
  let body = {id: $id, name: $name, category: $category, photoUrls: $photoUrls, tags: $tags, status: $status} | compact
...
export def "pet-find-by-status findPetsByStatus" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --accept: string@accept-completer # Response content type
  --status: string@status-completer # Status values that need to be considered for filter (default: available)
]: nothing -> table<id: int, name: string, category: record<id: int, name: string>, photoUrls: list<string>, tags: list<record>, status: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
...
export def "pet get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/pet/($petId)")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "pet updatePetWithForm" [
  ...
  let full_url = (build-url $base $"/pet/($petId)" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "store-order placeOrder" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --id: int # format: int64, e.g. 10
  --petId: int # format: int64, e.g. 198772
...
export def "store-order get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/store/order/($orderId)")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "user createUser" [
  ...
  --accept: string@accept-completer # Response content type
  --id: int # format: int64, e.g. 10
  --username: string # e.g. theUser
  --firstName: string # e.g. John
  --lastName: string # e.g. James
  --email: string # e.g. john@email.com
  --password: string # e.g. 12345
  --phone: string # e.g. 12345
  --userStatus: int # User Status (format: int32, e.g. 1)
]: any -> record<id: int, username: string, firstName: string, lastName: string, email: string, password: string, phone: string, userStatus: int> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
...
export def "user updateUser" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --id: int # format: int64, e.g. 10
  --body-username: string # e.g. theUser
  --firstName: string # e.g. John
  --lastName: string # e.g. James
  --email: string # e.g. john@email.com
  --password: string # e.g. 12345
