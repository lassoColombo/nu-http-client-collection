# Auto-generated client for RESTful4Up v1.0.0
# Source: https://api.apis.guru/v2/specs/restful4up.local/1.0.0/openapi.json
# Auth: --token flag or $env.RESTFUL4UP_TOKEN

const BASE_URL = "http://restful4up.local"
const DEFAULT_AUTH = "bearer"

# Build auth: returns {headers: record, query: string}
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o RESTFUL4UP_TOKEN | default "" }
...
def base-url-completer [] { ["http://restful4up.local" "http://localhost/v1"] }
def auth-scheme-completer [] { ["bearer"] }

# Completers for enum parameters
def is-unpacking-required-completer [] { ["false" "true"] }

# List all available API commands with their parameters
export def commands []: nothing -> table {
  let builtin_flags = ["base-url" "token" "auth-scheme" "insecure" "max-time" "raw" "allow-errors" "dry-run" "accept" "help"]
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "apply-yara-rules applyYaraRules" } } | get name | first)
...
export def "apply-yara-rules applyYaraRules" [
...
export def "emulation-output emulationOutput" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
...
export def "generate-partial-yara-rules generatePartialYaraRule" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
