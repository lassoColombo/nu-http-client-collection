...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_TOKEN | default "" }
...
export def "hpcer-certificate create" [
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/hpcer/certificate" $auth.query)
...
export def "hscer-certificate create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/hscer/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
...
export def "hsmgr-certificate create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/hsmgr/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "nchsc-certificate create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/nchsc/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "nctsc-certificate create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/nctsc/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "nsesc-certificate create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/nsesc/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "nstsc-certificate create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/nstsc/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "ntltr-certificate create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/ntltr/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "ntmks-certificate create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/ntmks/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "skhsc-certificate create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/skhsc/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "sktsc-certificate create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/sktsc/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "spcer-certificate create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/spcer/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "sscer-certificate create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/sscer/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = ($accept | default "application/pdf")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "ssmgr-certificate create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/ssmgr/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "tetcr-certificate create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/tetcr/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "tetms-certificate create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o CENTRAL_BOARD_OF_SECONDARY_EDUCATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/tetms/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
