# Auto-generated client for LambdaTest Screenshots API Documentation v1.0.1
# Source: https://api.apis.guru/v2/specs/lambdatest.com/1.0.1/openapi.json
# Auth: --token flag or $env.LAMBDATEST_SCREENSHOTS_API_DOCUMENTATION_TOKEN

const BASE_URL = "https://api.lambdatest.com/screenshots/v1"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o LAMBDATEST_SCREENSHOTS_API_DOCUMENTATION_TOKEN | default "" }
...
]: nothing -> record<android_7_0: record<devices: list<string>>, ios_12_0: record<devices: list<string>>> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "os" $os "scalar")] | flatten | str join "&"
...
export def "locations get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> record<locations: table<code: string, name: string>> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
...
export def "os-browsers get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --os: string # Fetch details for a particular OS
]: nothing -> record<macos_mojave: record<chrome: list<string>, firefox: list<string>, opera: list<string>>, windows_10: record<chrome: list<string>, firefox: list<string>, opera: list<string>>> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "os" $os "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/os-browsers" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "profiles get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> record<profiles: table<name: string, profile_config: record>> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/profiles" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "resolutions get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> record<mac_res: list<string>, win_res: list<string>> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/resolutions" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "stop stop-screenshots" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  if ($test_id | is-empty) { error make --unspanned { msg: "path parameter 'test_id' must be non-empty" } }
  let full_url = (build-url $base ({test_id: (encode-path-segment $test_id)} | format pattern "/stop/{test_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "put"
...
export def "get-screenshots get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($test_id | is-empty) { error make --unspanned { msg: "path parameter 'test_id' must be non-empty" } }
  let full_url = (build-url $base ({test_id: (encode-path-segment $test_id)} | format pattern "/{test_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "zip get-zipped-screenshots" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($test_id | is-empty) { error make --unspanned { msg: "path parameter 'test_id' must be non-empty" } }
  let full_url = (build-url $base ({test_id: (encode-path-segment $test_id)} | format pattern "/{test_id}/zip") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
