...
export def "bet-complex create-place" [
  ...
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"fields": $fields, "include": $include, "exclude": $exclude} | compact), body: $req_body}
}
...
export def "betslips validate" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"expanded": $expanded} | compact), body: $req_body}
}
...
export def "history get-bet" [
  ...
  let extra_headers = {"apiKey": $api_key, "apiSecret": $api_secret, "apiTicket": $api_ticket} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"dateFrom": $date_from, "dateTo": $date_to, "fields": $fields, "include": $include, "exclude": $exclude, "page": $page, "pageSize": $page_size, "sort": $qp_sort, "settled": $settled} | compact), body: null}
}
...
export def "cashin update" [
  ...
  let extra_headers = {"apiKey": $api_key, "apiSecret": $api_secret, "apiTicket": $api_ticket} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"cashInValue": $cash_in_value, "cashinBetDelayId": $cashin_bet_delay_id} | compact), body: null}
}
