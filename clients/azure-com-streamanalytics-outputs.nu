...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "subscriptions-resourcegroups-providers-microsoft-stream-analytics-streamingjobs-outputs list-by" } } | get name | first)
...
export def "subscriptions-resourcegroups-providers-microsoft-stream-analytics-streamingjobs-outputs list-by" [
  ...
  let full_url = (build-url $base ({subscription_id: $subscription_id, resource_group_name: $resource_group_name, job_name: $job_name} | format pattern "/subscriptions/{subscription_id}/resourcegroups/{resource_group_name}/providers/Microsoft.StreamAnalytics/streamingjobs/{job_name}/outputs") $qp)
...
export def "subscriptions-resourcegroups-providers-microsoft-stream-analytics-streamingjobs-outputs delete" [
  ...
  let full_url = (build-url $base ({subscription_id: $subscription_id, resource_group_name: $resource_group_name, job_name: $job_name, output_name: $output_name} | format pattern "/subscriptions/{subscription_id}/resourcegroups/{resource_group_name}/providers/Microsoft.StreamAnalytics/streamingjobs/{job_name}/outputs/{output_name}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
# --properties shape: {datasource?: any, diagnostics?: any, serialization?: any}
export def "subscriptions-resourcegroups-providers-microsoft-stream-analytics-streamingjobs-outputs create-or-replace" [
  subscription_id: string
  resource_group_name: string
  job_name: string
  output_name: string
...
export def "subscriptions-resourcegroups-providers-microsoft-stream-analytics-streamingjobs-outputs-test test" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: $subscription_id, resource_group_name: $resource_group_name, job_name: $job_name, output_name: $output_name} | format pattern "/subscriptions/{subscription_id}/resourcegroups/{resource_group_name}/providers/Microsoft.StreamAnalytics/streamingjobs/{job_name}/outputs/{output_name}/test") $qp)
  let body = {"properties": $properties, "name": $name} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
