# Auto-generated client for Points of Interest v1.1.1
# Source: https://api.apis.guru/v2/specs/amadeus.com/amadeus-points-of-interest/1.1.1/swagger.json
# Auth: --token flag or $env.POINTS_OF_INTEREST_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o POINTS_OF_INTEREST_TOKEN | default "" }
...
export def "reference-data-locations-pois get-points-of-interest" [
  ...
  --categories: list<string> # category of the location. Multiple value can be selected using a comma i.e. SIGHTS, SHOPPING
]: nothing -> any {
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"latitude": $latitude, "longitude": $longitude, "radius": $radius, "page[limit]": $page_limit, "page[offset]": $page_offset, "categories": $categories} | compact), body: null}
}
...
export def "reference-data-locations-pois-by-square get-points-of-interest" [
  ...
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"north": $north, "west": $west, "south": $south, "east": $east, "page[limit]": $page_limit, "page[offset]": $page_offset, "categories": $categories} | compact), body: null}
}
...
export def "reference-data-locations-pois get-point-of-interest" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($pois_id | is-empty) { error make --unspanned { msg: "path parameter 'poisId' must be non-empty" } }
  let full_url = (build-url $base ({pois_id: (encode-path-segment $pois_id)} | format pattern "/reference-data/locations/pois/{pois_id}"))
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
