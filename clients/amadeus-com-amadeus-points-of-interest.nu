...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o POINTS_OF_INTEREST_TOKEN | default "" }
...
export def "reference-data-locations-pois get-points-of-interest" [
  ...
  let full_url = (build-url $base "/reference-data/locations/pois" $qp $auth.query)
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"latitude": $latitude, "longitude": $longitude, "radius": $radius, "page[limit]": $page_limit, "page[offset]": $page_offset, "categories": $categories} | compact)
...
export def "reference-data-locations-pois-by-square get-points-of-interest" [
  ...
  let full_url = (build-url $base "/reference-data/locations/pois/by-square" $qp $auth.query)
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"north": $north, "west": $west, "south": $south, "east": $east, "page[limit]": $page_limit, "page[offset]": $page_offset, "categories": $categories} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "reference-data-locations-pois get-point-of-interest" [
  ...
  let full_url = (build-url $base ({pois_id: (encode-path-segment $pois_id)} | format pattern "/reference-data/locations/pois/{pois_id}") $auth.query)
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
