# Auto-generated client for graphql.anilist v0.0.0
# Source: https://graphql.anilist.co
# Auth: --token flag or $env.GRAPHQL_ANILIST_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o GRAPHQL_ANILIST_TOKEN | default "" }
...
def base-url-completer [] { ["https://graphql.anilist.co"] }
...
def type-completer-2 [] { ["ANIME_LIST" "MANGA_LIST" "MEDIA_LIST" "MESSAGE" "TEXT"] }
def type-not-completer [] { ["ANIME_LIST" "MANGA_LIST" "MEDIA_LIST" "MESSAGE" "TEXT"] }
def type-in-completer-1 [] { ["ANIME_LIST" "MANGA_LIST" "MEDIA_LIST" "MESSAGE" "TEXT"] }
def type-not-in-completer [] { ["ANIME_LIST" "MANGA_LIST" "MEDIA_LIST" "MESSAGE" "TEXT"] }
def sort-completer-9 [] { ["ID" "ID_DESC" "PINNED"] }
def sort-completer-10 [] { ["CREATED_AT" "CREATED_AT_DESC" "ID" "ID_DESC" "IS_STICKY" "REPLIED_AT" "REPLIED_AT_DESC" "REPLY_COUNT" "REPLY_COUNT_DESC" "SEARCH_MATCH" "TITLE" "TITLE_DESC" "UPDATED_AT" "UPDATED_AT_DESC" "VIEW_COUNT" "VIEW_COUNT_DESC"] }
def sort-completer-11 [] { ["ID" "ID_DESC"] }
def sort-completer-12 [] { ["ID" "ID_DESC" "RATING" "RATING_DESC"] }
def type-completer-3 [] { ["ACTIVITY" "ACTIVITY_REPLY" "THREAD" "THREAD_COMMENT"] }
def type-completer-4 [] { ["INFO" "SOCIAL" "STREAMING"] }
def media-type-completer-1 [] { ["ANIME" "MANGA" "STAFF"] }
...
def rating-completer [] { ["DOWN_VOTE" "NO_VOTE" "UP_VOTE"] }
def rating-completer-1 [] { ["NO_RATING" "RATE_DOWN" "RATE_UP"] }
...
export def "query media" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "Media" }
}
...
export def "query media-trend" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "MediaTrend" }
}
...
export def "query airing-schedule" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "AiringSchedule" }
}
...
export def "query staff" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "Staff" }
}
...
export def "query media-list" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "MediaList" }
}
...
export def "query media-list-collection" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "MediaListCollection" }
}
...
export def "query genre-collection" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "GenreCollection" }
}
...
export def "query media-tag-collection" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "MediaTagCollection" }
}
...
export def "query notification" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "Notification" }
}
...
export def "query review" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "Review" }
}

# Activity query
#
# operationId: Activity
export def "query activity" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --user-id: int # Filter by the owner user id
  --messenger-id: int # Filter by the id of the user who sent a message
  --media-id: int # Filter by the associated media id of the activity
  --type: string@type-completer-2 # Filter by the type of activity
  --is-following: string@bool-completer # Filter activity to users who are being followed by the authenticated user
  --has-replies: string@bool-completer # Filter activity to only activity with replies
  --has-replies-or-type-text: string@bool-completer # Filter activity to only activity with replies or is of type text
  --created-at: int # Filter by the time the activity was created
  --id-not: int # Filter by the activity id
  --id-in: int # Filter by the activity id (max 10,000 items)
  --id-not-in: int # Filter by the activity id (max 10,000 items)
  --user-id-not: int # Filter by the owner user id
  --user-id-in: int # Filter by the owner user id (max 10,000 items)
  --user-id-not-in: int # Filter by the owner user id (max 10,000 items)
  --messenger-id-not: int # Filter by the id of the user who sent a message
  --messenger-id-in: int # Filter by the id of the user who sent a message (max 10,000 items)
  --messenger-id-not-in: int # Filter by the id of the user who sent a message (max 10,000 items)
  --media-id-not: int # Filter by the associated media id of the activity
  --media-id-in: int # Filter by the associated media id of the activity (max 10,000 items)
  --media-id-not-in: int # Filter by the associated media id of the activity (max 10,000 items)
  --type-not: string@type-not-completer # Filter by the type of activity
  --type-in: string@type-in-completer-1 # Filter by the type of activity (max 10,000 items)
  --type-not-in: string@type-not-in-completer # Filter by the type of activity (max 10,000 items)
  --created-at-greater: int # Filter by the time the activity was created
  --created-at-lesser: int # Filter by the time the activity was created
  --qp-sort: string@sort-completer-9 # The order the results will be returned in
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"id": $id, "userId": $user_id, "messengerId": $messenger_id, "mediaId": $media_id, "type": $type, "isFollowing": $is_following, "hasReplies": $has_replies, "hasRepliesOrTypeText": $has_replies_or_type_text, "createdAt": $created_at, "id_not": $id_not, "id_in": $id_in, "id_not_in": $id_not_in, "userId_not": $user_id_not, "userId_in": $user_id_in, "userId_not_in": $user_id_not_in, "messengerId_not": $messenger_id_not, "messengerId_in": $messenger_id_in, "messengerId_not_in": $messenger_id_not_in, "mediaId_not": $media_id_not, "mediaId_in": $media_id_in, "mediaId_not_in": $media_id_not_in, "type_not": $type_not, "type_in": $type_in, "type_not_in": $type_not_in, "createdAt_greater": $created_at_greater, "createdAt_lesser": $created_at_lesser, "sort": $qp_sort} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "query activity-reply" [
  ...
  --activity-id: int # Filter by the parent id
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"id": $id, "activityId": $activity_id} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
    ...
    let body = {query: ("query($id: Int, $activityId: Int) { ActivityReply(id: $id, activityId: $activityId) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "ActivityReply" }
}

# Following query
#
# operationId: Following
export def "query following" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --fields: list<string> # Fields to select
  --query: string # Raw GraphQL query (overrides auto-generated)
  user_id: int # User id of the follower/followed
  --qp-sort: string@sort-completer-6 # The order the results will be returned in
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"userId": $user_id, "sort": $qp_sort} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
    ...
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "id name about bannerImage isFollowing isFollower isBlocked bans unreadNotificationCount siteUrl donatorTier donatorBadge moderatorRoles createdAt updatedAt moderatorStatus" }
    let body = {query: ("query($userId: Int!, $sort: [UserSort]) { Following(userId: $userId, sort: $sort) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "Following" }
}

# Follower query
#
# operationId: Follower
export def "query follower" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let result = if ($query | is-not-empty) {
    ...
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "id name about bannerImage isFollowing isFollower isBlocked bans unreadNotificationCount siteUrl donatorTier donatorBadge moderatorRoles createdAt updatedAt moderatorStatus" }
    let body = {query: ("query($userId: Int!, $sort: [UserSort]) { Follower(userId: $userId, sort: $sort) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "Follower" }
}

# Thread query
#
# operationId: Thread
export def "query thread" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --user-id: int # Filter by the user id of the thread's creator
  --reply-user-id: int # Filter by the user id of the last user to comment on the thread
  --subscribed: string@bool-completer # Filter by if the currently authenticated user's subscribed threads
  --category-id: int # Filter by thread category id
  --media-category-id: int # Filter by thread media id category
  --search: string # Filter by search query
  --id-in: int # Filter by the thread id (max 10,000 items)
  --qp-sort: string@sort-completer-10 # The order the results will be returned in
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"id": $id, "userId": $user_id, "replyUserId": $reply_user_id, "subscribed": $subscribed, "categoryId": $category_id, "mediaCategoryId": $media_category_id, "search": $search, "id_in": $id_in, "sort": $qp_sort} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  } else {
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "id title body userId replyUserId replyCommentId replyCount viewCount isLocked isSticky isSubscribed likeCount isLiked repliedAt createdAt updatedAt siteUrl" }
    let body = {query: ("query($id: Int, $userId: Int, $replyUserId: Int, $subscribed: Boolean, $categoryId: Int, $mediaCategoryId: Int, $search: String, $id_in: [Int], $sort: [ThreadSort]) { Thread(id: $id, userId: $userId, replyUserId: $replyUserId, subscribed: $subscribed, categoryId: $categoryId, mediaCategoryId: $mediaCategoryId, search: $search, id_in: $id_in, sort: $sort) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "Thread" }
}

# Comment query
#
# operationId: ThreadComment
export def "query thread-comment" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --thread-id: int # Filter by the thread id
  --user-id: int # Filter by the user id of the comment's creator
  --qp-sort: string@sort-completer-11 # The order the results will be returned in
]: any -> list {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"id": $id, "threadId": $thread_id, "userId": $user_id, "sort": $qp_sort} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  } else {
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "id userId threadId comment likeCount isLiked siteUrl createdAt updatedAt childComments isLocked" }
    let body = {query: ("query($id: Int, $threadId: Int, $userId: Int, $sort: [ThreadCommentSort]) { ThreadComment(id: $id, threadId: $threadId, userId: $userId, sort: $sort) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "ThreadComment" }
}

# Recommendation query
#
# operationId: Recommendation
export def "query recommendation" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --media-id: int # Filter by media id
  --media-recommendation-id: int # Filter by media recommendation id
  --user-id: int # Filter by user who created the recommendation
  --rating: int # Filter by total rating of the recommendation
  --on-list: string@bool-completer # Filter by the media on the authenticated user's lists
  --rating-greater: int # Filter by total rating of the recommendation
  --rating-lesser: int # Filter by total rating of the recommendation
  --qp-sort: string@sort-completer-12 # The order the results will be returned in
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"id": $id, "mediaId": $media_id, "mediaRecommendationId": $media_recommendation_id, "userId": $user_id, "rating": $rating, "onList": $on_list, "rating_greater": $rating_greater, "rating_lesser": $rating_lesser, "sort": $qp_sort} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  } else {
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "id rating userRating" }
    let body = {query: ("query($id: Int, $mediaId: Int, $mediaRecommendationId: Int, $userId: Int, $rating: Int, $onList: Boolean, $rating_greater: Int, $rating_lesser: Int, $sort: [RecommendationSort]) { Recommendation(id: $id, mediaId: $mediaId, mediaRecommendationId: $mediaRecommendationId, userId: $userId, rating: $rating, onList: $onList, rating_greater: $rating_greater, rating_lesser: $rating_lesser, sort: $sort) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "Recommendation" }
}

# Like query
#
# operationId: Like
export def "query like" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --fields: list<string> # Fields to select
  --query: string # Raw GraphQL query (overrides auto-generated)
  --likeable-id: int # The id of the likeable type
  --type: string@type-completer-3 # The type of model the id applies to
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"likeableId": $likeable_id, "type": $type} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
    ...
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "id name about bannerImage isFollowing isFollower isBlocked bans unreadNotificationCount siteUrl donatorTier donatorBadge moderatorRoles createdAt updatedAt moderatorStatus" }
    let body = {query: ("query($likeableId: Int, $type: LikeableType) { Like(likeableId: $likeableId, type: $type) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "Like" }
}

# Provide AniList markdown to be converted to html (Requires auth)
#
# operationId: Markdown
export def "query markdown" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --fields: list<string> # Fields to select
  --query: string # Raw GraphQL query (overrides auto-generated)
  markdown: string # The markdown to be parsed to html
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"markdown": $markdown} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  } else {
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "html" }
    let body = {query: ("query($markdown: String!) { Markdown(markdown: $markdown) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "Markdown" }
}

# GraphQL query: AniChartUser
#
# operationId: AniChartUser
export def "query ani-chart-user" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let result = if ($query | is-not-empty) {
    ...
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "settings highlights" }
    let body = {query: ("query { AniChartUser { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "AniChartUser" }
}

# Site statistics query
#
# operationId: SiteStatistics
export def "query site-statistics" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let result = if ($query | is-not-empty) {
    ...
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "__typename" }
    let body = {query: ("query { SiteStatistics { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "SiteStatistics" }
}

# ExternalLinkSource collection query
#
# operationId: ExternalLinkSourceCollection
export def "query external-link-source-collection" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --fields: list<string> # Fields to select
  --query: string # Raw GraphQL query (overrides auto-generated)
  --id: int # Filter by the link id
  --type: string@type-completer-4
  --media-type: string@media-type-completer-1
]: any -> list {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"id": $id, "type": $type, "mediaType": $media_type} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  } else {
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "id url site siteId type language color icon notes isDisabled" }
    let body = {query: ("query($id: Int, $type: ExternalLinkType, $mediaType: ExternalLinkMediaType) { ExternalLinkSourceCollection(id: $id, type: $type, mediaType: $mediaType) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "ExternalLinkSourceCollection" }
}
...
export def "mutation update-user" [
  ...
  --anime-list-options-splitCompletedSectionByFormat: string@bool-completer # If the completed sections of the list should be separated by format
  ...
  --anime-list-options-advancedScoringEnabled: string@bool-completer # If advanced scoring is enabled
  ...
  --manga-list-options-splitCompletedSectionByFormat: string@bool-completer # If the completed sections of the list should be separated by format
  ...
  --manga-list-options-advancedScoringEnabled: string@bool-completer # If advanced scoring is enabled
...
export def "mutation save-media-list-entry" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "SaveMediaListEntry" }
}
...
export def "mutation update-media-list-entries" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "UpdateMediaListEntries" }
}
...
export def "mutation delete-media-list-entry" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "DeleteMediaListEntry" }
}
...
export def "mutation delete-custom-list" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "DeleteCustomList" }
}
...
export def "mutation save-text-activity" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "SaveTextActivity" }
}
...
export def "mutation save-message-activity" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "SaveMessageActivity" }
}
...
export def "mutation save-list-activity" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "SaveListActivity" }
}
...
export def "mutation delete-activity" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "DeleteActivity" }
}
...
export def "mutation toggle-activity-pin" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "ToggleActivityPin" }
}
...
export def "mutation toggle-activity-subscription" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "ToggleActivitySubscription" }
}
...
export def "mutation save-activity-reply" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "SaveActivityReply" }
}
...
export def "mutation delete-activity-reply" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "DeleteActivityReply" }
}
...
export def "mutation toggle-like" [
  ...
  --type: string@type-completer-3 # The type of model to be un/liked
]: any -> list {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
...
export def "mutation toggle-like-v2" [
  ...
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "ToggleLikeV2" }
}

# Toggle the un/following of a user
#
# operationId: ToggleFollow
export def "mutation toggle-follow" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --fields: list<string> # Fields to select
  --query: string # Raw GraphQL query (overrides auto-generated)
  --user-id: int # The id of the user to un/follow
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  ...
  let result = if ($query | is-not-empty) {
    ...
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "id name about bannerImage isFollowing isFollower isBlocked bans unreadNotificationCount siteUrl donatorTier donatorBadge moderatorRoles createdAt updatedAt moderatorStatus" }
    let body = {query: ("mutation($userId: Int) { ToggleFollow(userId: $userId) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "ToggleFollow" }
}

# Favourite or unfavourite an anime, manga, character, staff member, or studio
#
# operationId: ToggleFavourite
export def "mutation toggle-favourite" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --fields: list<string> # Fields to select
  --query: string # Raw GraphQL query (overrides auto-generated)
  --anime-id: int # The id of the anime to un/favourite
  --manga-id: int # The id of the manga to un/favourite
  --character-id: int # The id of the character to un/favourite
  --staff-id: int # The id of the staff to un/favourite
  --studio-id: int # The id of the studio to un/favourite
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"animeId": $anime_id, "mangaId": $manga_id, "characterId": $character_id, "staffId": $staff_id, "studioId": $studio_id} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
    ...
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "__typename" }
    let body = {query: ("mutation($animeId: Int, $mangaId: Int, $characterId: Int, $staffId: Int, $studioId: Int) { ToggleFavourite(animeId: $animeId, mangaId: $mangaId, characterId: $characterId, staffId: $staffId, studioId: $studioId) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "ToggleFavourite" }
}

# Update the order favourites are displayed in
#
# operationId: UpdateFavouriteOrder
export def "mutation update-favourite-order" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --fields: list<string> # Fields to select
  --query: string # Raw GraphQL query (overrides auto-generated)
  --anime-ids: int # The id of the anime to un/favourite
  --manga-ids: int # The id of the manga to un/favourite
  --character-ids: int # The id of the character to un/favourite
  --staff-ids: int # The id of the staff to un/favourite
  --studio-ids: int # The id of the studio to un/favourite
  --anime-order: int # List of integers which the anime should be ordered by (Asc)
  --manga-order: int # List of integers which the manga should be ordered by (Asc)
  --character-order: int # List of integers which the character should be ordered by (Asc)
  --staff-order: int # List of integers which the staff should be ordered by (Asc)
  --studio-order: int # List of integers which the studio should be ordered by (Asc)
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"animeIds": $anime_ids, "mangaIds": $manga_ids, "characterIds": $character_ids, "staffIds": $staff_ids, "studioIds": $studio_ids, "animeOrder": $anime_order, "mangaOrder": $manga_order, "characterOrder": $character_order, "staffOrder": $staff_order, "studioOrder": $studio_order} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "mutation save-review" [
  ...
  --media-id: int # The id of the media the review is of
  --qp-body: string # The main review text. Min:2200 characters
  ...
  --private: string@bool-completer # If the review should only be visible to its creator
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"id": $id, "mediaId": $media_id, "body": $qp_body, "summary": $summary, "score": $score, "private": $private} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
    ...
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "id userId mediaId mediaType summary body rating ratingAmount userRating score private siteUrl createdAt updatedAt" }
    let body = {query: ("mutation($id: Int, $mediaId: Int, $body: String, $summary: String, $score: Int, $private: Boolean) { SaveReview(id: $id, mediaId: $mediaId, body: $body, summary: $summary, score: $score, private: $private) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "SaveReview" }
}

# Delete a review
#
# operationId: DeleteReview
export def "mutation delete-review" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let result = if ($query | is-not-empty) {
    ...
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "deleted" }
    let body = {query: ("mutation($id: Int) { DeleteReview(id: $id) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "DeleteReview" }
}

# Rate a review
#
# operationId: RateReview
export def "mutation rate-review" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --fields: list<string> # Fields to select
  --query: string # Raw GraphQL query (overrides auto-generated)
  --review-id: int # The id of the review to rate
  --rating: string@rating-completer # The rating to apply to the review
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"reviewId": $review_id, "rating": $rating} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
    ...
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "id userId mediaId mediaType summary body rating ratingAmount userRating score private siteUrl createdAt updatedAt" }
    let body = {query: ("mutation($reviewId: Int, $rating: ReviewRating) { RateReview(reviewId: $reviewId, rating: $rating) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "RateReview" }
}

# Recommendation a media
#
# operationId: SaveRecommendation
export def "mutation save-recommendation" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --fields: list<string> # Fields to select
  --query: string # Raw GraphQL query (overrides auto-generated)
  --media-id: int # The id of the base media
  --media-recommendation-id: int # The id of the media to recommend
  --rating: string@rating-completer-1 # The rating to give the recommendation
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"mediaId": $media_id, "mediaRecommendationId": $media_recommendation_id, "rating": $rating} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
    ...
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "id rating userRating" }
    let body = {query: ("mutation($mediaId: Int, $mediaRecommendationId: Int, $rating: RecommendationRating) { SaveRecommendation(mediaId: $mediaId, mediaRecommendationId: $mediaRecommendationId, rating: $rating) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "SaveRecommendation" }
}

# Create or update a forum thread
#
# operationId: SaveThread
export def "mutation save-thread" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --qp-body: string # The main text body of the thread
  --categories: int # Forum categories the thread should be within
  --media-categories: int # Media related to the contents of the thread
  --sticky: string@bool-completer # If the thread should be stickied. (Mod Only)
  --locked: string@bool-completer # If the thread should be locked. (Mod Only)
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"id": $id, "title": $title, "body": $qp_body, "categories": $categories, "mediaCategories": $media_categories, "sticky": $sticky, "locked": $locked} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
    ...
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "id title body userId replyUserId replyCommentId replyCount viewCount isLocked isSticky isSubscribed likeCount isLiked repliedAt createdAt updatedAt siteUrl" }
    let body = {query: ("mutation($id: Int, $title: String, $body: String, $categories: [Int], $mediaCategories: [Int], $sticky: Boolean, $locked: Boolean) { SaveThread(id: $id, title: $title, body: $body, categories: $categories, mediaCategories: $mediaCategories, sticky: $sticky, locked: $locked) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "SaveThread" }
}

# Delete a thread
#
# operationId: DeleteThread
export def "mutation delete-thread" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let result = if ($query | is-not-empty) {
    ...
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "deleted" }
    let body = {query: ("mutation($id: Int) { DeleteThread(id: $id) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "DeleteThread" }
}

# Toggle the subscription of a forum thread
#
# operationId: ToggleThreadSubscription
export def "mutation toggle-thread-subscription" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --fields: list<string> # Fields to select
  --query: string # Raw GraphQL query (overrides auto-generated)
  --thread-id: int # The id of the forum thread to un/subscribe
  --subscribe: string@bool-completer # Whether to subscribe or unsubscribe from the forum thread
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"threadId": $thread_id, "subscribe": $subscribe} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
    ...
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "id title body userId replyUserId replyCommentId replyCount viewCount isLocked isSticky isSubscribed likeCount isLiked repliedAt createdAt updatedAt siteUrl" }
    let body = {query: ("mutation($threadId: Int, $subscribe: Boolean) { ToggleThreadSubscription(threadId: $threadId, subscribe: $subscribe) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "ToggleThreadSubscription" }
}

# Create or update a thread comment
#
# operationId: SaveThreadComment
export def "mutation save-thread-comment" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --thread-id: int # The id of thread the comment belongs to
  --parent-comment-id: int # The id of thread comment to reply to
  --comment: string # The comment markdown text
  --locked: string@bool-completer # If the comment tree should be locked. (Mod Only)
]: any -> record {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"id": $id, "threadId": $thread_id, "parentCommentId": $parent_comment_id, "comment": $comment, "locked": $locked} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
    ...
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "id userId threadId comment likeCount isLiked siteUrl createdAt updatedAt childComments isLocked" }
    let body = {query: ("mutation($id: Int, $threadId: Int, $parentCommentId: Int, $comment: String, $locked: Boolean) { SaveThreadComment(id: $id, threadId: $threadId, parentCommentId: $parentCommentId, comment: $comment, locked: $locked) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "SaveThreadComment" }
}

# Delete a thread comment
#
# operationId: DeleteThreadComment
export def "mutation delete-thread-comment" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let result = if ($query | is-not-empty) {
    ...
    let sel = if ($fields | is-not-empty) { $fields | str join " " } else { "deleted" }
    let body = {query: ("mutation($id: Int) { DeleteThreadComment(id: $id) { " + $sel + " } }"), variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "DeleteThreadComment" }
}

# GraphQL mutation: UpdateAniChartSettings
#
# operationId: UpdateAniChartSettings
export def "mutation update-ani-chart-settings" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"titleLanguage": $title_language, "outgoingLinkProvider": $outgoing_link_provider, "theme": $theme, "sort": $qp_sort} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  } else {
    let body = {query: "mutation($titleLanguage: String, $outgoingLinkProvider: String, $theme: String, $sort: String) { UpdateAniChartSettings(titleLanguage: $titleLanguage, outgoingLinkProvider: $outgoingLinkProvider, theme: $theme, sort: $sort) }", variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "UpdateAniChartSettings" }
}

# GraphQL mutation: UpdateAniChartHighlights
#
# operationId: UpdateAniChartHighlights
# --highlights item shape: {mediaId?: int, highlight?: string}
export def "mutation update-ani-chart-highlights" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --fields: list<string> # Fields to select
  --query: string # Raw GraphQL query (overrides auto-generated)
  --highlights: record # item shape: {mediaId?: int, highlight?: string}
]: any -> any {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default $DEFAULT_AUTH))
  let base = ($base_url | default $BASE_URL)
  let variables = {"highlights": $highlights} | compact
  let variables = if ($input | describe | str starts-with "record") { $input | merge deep $variables } else { $variables }
  let result = if ($query | is-not-empty) {
    let body = {query: $query, variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  } else {
    let body = {query: "mutation($highlights: [AniChartHighlightInput]) { UpdateAniChartHighlights(highlights: $highlights) }", variables: $variables}
    do-request "post" $base $auth $insecure $raw $max_time $allow_errors "application/json" $body
  }
  if $raw or $allow_errors { $result } else { unwrap-graphql $result "UpdateAniChartHighlights" }
}
