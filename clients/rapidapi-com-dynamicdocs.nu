# Auto-generated client for DynamicDocs v1.0
# Source: https://api.apis.guru/v2/specs/rapidapi.com/dynamicdocs/1.0/openapi.json
# Auth: --token flag or $env.DYNAMICDOCS_TOKEN

const BASE_URL = "https://dynamicdocs.p.rapidapi.com"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o DYNAMICDOCS_TOKEN | default "" }
  let scheme = ($auth_scheme | default "bearer")
  if ($scheme == "none") or ($token_val | is-empty) { return {scheme: $scheme, headers: {}, query: "", location: "none"} }
  match $scheme {
    "advicement api key" => { {scheme: $scheme, headers: {ADVICEment API Key: $token_val}, query: "", location: "header"} }
    "rapidapi.com api key" => { {scheme: $scheme, headers: {RapidAPI.com API Key: $token_val}, query: "", location: "header"} }
...
def base-url-completer [] { ["https://dynamicdocs.p.rapidapi.com"] }
def auth-scheme-completer [] { ["advicement api key" "rapidapi.com api key"] }

# Completers for enum parameters
def latex-compiler-completer [] { ["lualatex" "pdflatex"] }
...
export def "templates-compile create" [
  ...
  if ($template_token | is-empty) { error make --unspanned { msg: "path parameter 'template-token' must be non-empty" } }
  ...
  let full_url = (build-url $base ({template_token: (encode-path-segment $template_token)} | format pattern "/templates/{template_token}/compile") $qp)
  let req_body = $body
  ...
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full $effective_ct $req_body_wire {query: ({"doc-url-expires-in": $doc_url_expires_in, "latex-compiler": $latex_compiler, "latex-runs ": $latex_runs, "main-file-name": $main_file_name, "doc-file-name": $doc_file_name} | compact), body: $req_body}
}
