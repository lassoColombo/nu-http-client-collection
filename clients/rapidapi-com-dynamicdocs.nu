...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o DYNAMICDOCS_TOKEN | default "" }
...
export def "templates-compile create" [
  ...
  let full_url = (build-url $base ({template_token: (encode-path-segment $template_token)} | format pattern "/templates/{template_token}/compile") $qp $auth.query)
  let req_body = $body
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"doc-url-expires-in": $doc_url_expires_in, "latex-compiler": $latex_compiler, "latex-runs ": $latex_runs, "main-file-name": $main_file_name, "doc-file-name": $doc_file_name} | compact)
