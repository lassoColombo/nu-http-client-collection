# Auto-generated client for Provider API Client v2018-02-01
# Source: https://api.apis.guru/v2/specs/azure.com/web-Provider/2018-02-01/swagger.json
# Auth: --token flag or $env.PROVIDER_API_CLIENT_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o PROVIDER_API_CLIENT_TOKEN | default "" }
...
export def "providers-microsoft-web-available-stacks get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"osTypeSelected": $os_type_selected, "api-version": $api_version} | compact), body: null}
}
...
export def "providers-microsoft-web-operations list" [
  ...
  let full_url = (build-url $base "/providers/Microsoft.Web/operations" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "subscriptions-providers-microsoft-web-available-stacks get-on-prem" [
  ...
  let qp = [(serialize-qp "osTypeSelected" $os_type_selected "scalar") (serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id)} | format pattern "/subscriptions/{subscription_id}/providers/Microsoft.Web/availableStacks") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"osTypeSelected": $os_type_selected, "api-version": $api_version} | compact), body: null}
}
