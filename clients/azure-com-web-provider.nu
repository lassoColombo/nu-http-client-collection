...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o PROVIDER_API_CLIENT_TOKEN | default "" }
...
export def "providers-microsoft-web-available-stacks get" [
  ...
  let full_url = (build-url $base "/providers/Microsoft.Web/availableStacks" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"osTypeSelected": $os_type_selected, "api-version": $api_version} | compact)
...
export def "providers-microsoft-web-operations list" [
  ...
  let full_url = (build-url $base "/providers/Microsoft.Web/operations" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "subscriptions-providers-microsoft-web-available-stacks get-on-prem" [
  ...
  let qp = [(serialize-qp "osTypeSelected" $os_type_selected "scalar") (serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id)} | format pattern "/subscriptions/{subscription_id}/providers/Microsoft.Web/availableStacks") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
