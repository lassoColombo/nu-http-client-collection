...
export def "subscriptions-resource-groups-providers-microsoft-insights-query-packs-queries list" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version, "$top": $top, "includeBody": $include_body, "$skipToken": $skip_token} | compact), body: null}
}
...
export def "subscriptions-resource-groups-providers-microsoft-insights-query-packs-queries-search list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"api-version": $api_version, "$top": $top, "includeBody": $include_body, "$skipToken": $skip_token} | compact), body: $req_body}
}
