...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o IDEA_HUB_API_TOKEN | default "" }
...
export def "v1alpha update" [
  ...
  let full_url = (build-url $base ({name: (encode-path-segment $name)} | format pattern "/v1alpha/{name}") $qp $auth.query)
  let req_body = {"dismissed": $dismissed, "name": $body_name, "saved": $saved} | compact
...
export def "v1alpha-idea-activities create" [
  ...
  --upload-protocol: string # Upload protocol for media (e.g. "raw", "multipart").
  --upload-type: string # Legacy upload protocol for media (e.g. "media", "multipart").
  --ideas: list<string> # The Idea IDs for this entry. If empty, topics should be set.
  ...
  --topics: list<string> # The Topic IDs for this entry. If empty, ideas should be set.
  ...
  let full_url = (build-url $base ({parent: (encode-path-segment $parent)} | format pattern "/v1alpha/{parent}/ideaActivities") $qp $auth.query)
  let req_body = {"ideas": $ideas, "name": $name, "topics": $topics, "type": $type, "uri": $uri} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
...
export def "v1alpha-ideas list-1" [
  ...
  let full_url = (build-url $base ({parent: (encode-path-segment $parent)} | format pattern "/v1alpha/{parent}/ideas") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "v1alpha-locales list" [
  ...
  let full_url = (build-url $base ({parent: (encode-path-segment $parent)} | format pattern "/v1alpha/{parent}/locales") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
