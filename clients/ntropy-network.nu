# Auto-generated client for Ntropy Transaction API v1 v1.0.0
# Source: https://api.apis.guru/v2/specs/ntropy.network/1.0.0/openapi.json
# Auth: --token flag or $env.NTROPY_TRANSACTION_API_V1_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o NTROPY_TRANSACTION_API_V1_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.ntropy.network"] }
...
export def "classifier-business-batch get-of-transaction-classification-results" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/classifier/business/batch/{id}") $auth.query)
...
export def "classifier-consumer-batch get-of-transaction-classification-results" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/classifier/consumer/batch/{id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
