...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o NTROPY_TRANSACTION_API_V1_TOKEN | default "" }
...
export def "classifier-business-batch get-of-transaction-classification-results" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/classifier/business/batch/{id}"))
...
export def "classifier-consumer-batch get-of-transaction-classification-results" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/classifier/consumer/batch/{id}"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
