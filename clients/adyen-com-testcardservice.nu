# Auto-generated client for Adyen Test Cards API v1
# Source: https://api.apis.guru/v2/specs/adyen.com/TestCardService/1/openapi.json
# Auth: --token flag or $env.ADYEN_TEST_CARDS_API_TOKEN

const BASE_URL = "https://pal-test.adyen.com/pal/services/TestCard/v1"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o ADYEN_TEST_CARDS_API_TOKEN | default "" }
...
export def "create-test-card-ranges create" [
  ...
  let full_url = (build-url $base "/createTestCardRanges" $auth.query)
