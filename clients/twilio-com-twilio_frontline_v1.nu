# Auto-generated client for Twilio - Frontline v1.42.0
# Source: https://api.apis.guru/v2/specs/twilio.com/twilio_frontline_v1/1.42.0/openapi.json
# Auth: --token flag or $env.TWILIO_FRONTLINE_TOKEN

const BASE_URL = "https://frontline-api.twilio.com"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o TWILIO_FRONTLINE_TOKEN | default "" }
...
def base-url-completer [] { ["https://frontline-api.twilio.com"] }
...
export def "users update" [
  ...
  if ($sid | is-empty) { error make --unspanned { msg: "path parameter 'Sid' must be non-empty" } }
  let full_url = (build-url $base ({sid: (encode-path-segment $sid)} | format pattern "/v1/Users/{sid}"))
  let req_body = {"Avatar": $avatar, "FriendlyName": $friendly_name, "IsAvailable": $is_available, "State": $state} | compact
