...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o CUSTOM_VISION_PREDICTION_CLIENT_TOKEN | default "" }
  let scheme = ($auth_scheme | default "bearer")
  if ($scheme == "none") or ($token_val | is-empty) { return {scheme: $scheme, headers: {}, query: "", location: "none"} }
  match $scheme {
    "prediction-key" => { {scheme: $scheme, headers: {Prediction-Key: $token_val}, query: "", location: "header"} }
...
export def "classify-iterations-image create" [
  ...
  image_data: string # Binary image data. Supported formats are JPEG, GIF, PNG, and BMP. Supports images up to 4MB. (format: binary)
  ...
  if ($published_name | is-empty) { error make --unspanned { msg: "path parameter 'publishedName' must be non-empty" } }
  let qp = [(serialize-qp "application" $application "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({project_id: (encode-path-segment $project_id), published_name: (encode-path-segment $published_name)} | format pattern "/{project_id}/classify/iterations/{published_name}/image") $qp $auth.query)
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"application": $application} | compact)
...
export def "classify-iterations-image-nostore create-with-no-store" [
  ...
  --accept: string@accept-completer # Response content type
  --application: string # Optional. Specifies the name of application using the endpoint.
  image_data: string # Binary image data. Supported formats are JPEG, GIF, PNG, and BMP. Supports images up to 0MB. (format: binary)
]: any -> record<created: string, id: string, iteration: string, predictions: table<boundingBox: record, probability: float, tagId: string, tagName: string>, project: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "prediction-key"))
  let base = ($base_url | default $BASE_URL)
  ...
  if ($published_name | is-empty) { error make --unspanned { msg: "path parameter 'publishedName' must be non-empty" } }
  let qp = [(serialize-qp "application" $application "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({project_id: (encode-path-segment $project_id), published_name: (encode-path-segment $published_name)} | format pattern "/{project_id}/classify/iterations/{published_name}/image/nostore") $qp $auth.query)
  let req_body = {"imageData": $image_data} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "classify-iterations-url create-image" [
  ...
  if ($published_name | is-empty) { error make --unspanned { msg: "path parameter 'publishedName' must be non-empty" } }
  let qp = [(serialize-qp "application" $application "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({project_id: (encode-path-segment $project_id), published_name: (encode-path-segment $published_name)} | format pattern "/{project_id}/classify/iterations/{published_name}/url") $qp $auth.query)
...
export def "classify-iterations-url-nostore create-image-with-no-store" [
  ...
  if ($published_name | is-empty) { error make --unspanned { msg: "path parameter 'publishedName' must be non-empty" } }
  let qp = [(serialize-qp "application" $application "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({project_id: (encode-path-segment $project_id), published_name: (encode-path-segment $published_name)} | format pattern "/{project_id}/classify/iterations/{published_name}/url/nostore") $qp $auth.query)
  let req_body = {"url": $url} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "detect-iterations-image create" [
  ...
  if ($published_name | is-empty) { error make --unspanned { msg: "path parameter 'publishedName' must be non-empty" } }
  let qp = [(serialize-qp "application" $application "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({project_id: (encode-path-segment $project_id), published_name: (encode-path-segment $published_name)} | format pattern "/{project_id}/detect/iterations/{published_name}/image") $qp $auth.query)
  let req_body = {"imageData": $image_data} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "detect-iterations-image-nostore create-with-no-store" [
  ...
  if ($published_name | is-empty) { error make --unspanned { msg: "path parameter 'publishedName' must be non-empty" } }
  let qp = [(serialize-qp "application" $application "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({project_id: (encode-path-segment $project_id), published_name: (encode-path-segment $published_name)} | format pattern "/{project_id}/detect/iterations/{published_name}/image/nostore") $qp $auth.query)
  let req_body = {"imageData": $image_data} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "detect-iterations-url create-image" [
  ...
  if ($published_name | is-empty) { error make --unspanned { msg: "path parameter 'publishedName' must be non-empty" } }
  let qp = [(serialize-qp "application" $application "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({project_id: (encode-path-segment $project_id), published_name: (encode-path-segment $published_name)} | format pattern "/{project_id}/detect/iterations/{published_name}/url") $qp $auth.query)
  let req_body = {"url": $url} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "detect-iterations-url-nostore create-image-with-no-store" [
  ...
  if ($published_name | is-empty) { error make --unspanned { msg: "path parameter 'publishedName' must be non-empty" } }
  let qp = [(serialize-qp "application" $application "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({project_id: (encode-path-segment $project_id), published_name: (encode-path-segment $published_name)} | format pattern "/{project_id}/detect/iterations/{published_name}/url/nostore") $qp $auth.query)
  let req_body = {"url": $url} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
