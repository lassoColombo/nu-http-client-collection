...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o XERO_OAUTH_2_IDENTITY_SERVICE_API_TOKEN | default "" }
...
export def "connections get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"authEventId": $auth_event_id} | compact), body: null}
}
