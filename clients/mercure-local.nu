# Auto-generated client for The Mercure protocol v0.3.2
# Source: https://api.apis.guru/v2/specs/mercure.local/0.3.2/openapi.json
# Auth: --token flag or $env.THE_MERCURE_PROTOCOL_TOKEN

const BASE_URL = "http://mercure.local"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o THE_MERCURE_PROTOCOL_TOKEN | default "" }
  let scheme = ($auth_scheme | default "bearer")
  if ($scheme == "none") or ($token_val | is-empty) { return {scheme: $scheme, headers: {}, query: "", location: "none"} }
  match $scheme {
    "bearer" => { {scheme: $scheme, headers: {Authorization: $"Bearer ($token_val)"}, query: "", location: "header"} }
    "cookie-mercureAuthorization" => { {scheme: $scheme, headers: {Cookie: $"(encode-path-segment "mercureAuthorization")=(encode-path-segment $token_val)"}, query: "", location: "cookie"} }
...
def base-url-completer [] { ["http://mercure.local"] }
def auth-scheme-completer [] { ["bearer" "cookie-mercureAuthorization"] }
...
export def "well-known-mercure get" [
  ...
  --last-event-id-2: string # The last received event id, to retrieve missed events, takes precedence over the query parameter. (disambiguated-2)
]: nothing -> any {
  ...
  let extra_headers = {"Last-Event-ID": $last_event_id_2} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"topic": $topic, "Last-Event-ID": $last_event_id} | compact), body: null}
}
...
export def "well-known-mercure-subscriptions get-by-topic-subscriber" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($topic | is-empty) { error make --unspanned { msg: "path parameter 'topic' must be non-empty" } }
  if ($subscriber | is-empty) { error make --unspanned { msg: "path parameter 'subscriber' must be non-empty" } }
