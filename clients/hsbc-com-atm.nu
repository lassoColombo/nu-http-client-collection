...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o ATM_LOCATOR_API_TOKEN | default "" }
...
export def "open-banking-v2-2-atms get" [
  ...
  let full_url = (build-url $base "/open-banking/v2.2/atms" $auth.query)
...
export def "x-open-banking-v2-2-atms-country get" [
  ...
  let full_url = (build-url $base ({country: (encode-path-segment $country)} | format pattern "/x-open-banking/v2.2/atms/country/{country}") $auth.query)
  let accept_val = "application/prs.openbanking.opendata.v2.2+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "x-open-banking-v2-2-atms-country-town get" [
  ...
  let full_url = (build-url $base ({country: (encode-path-segment $country), town: (encode-path-segment $town)} | format pattern "/x-open-banking/v2.2/atms/country/{country}/town/{town}") $auth.query)
  let accept_val = "application/prs.openbanking.opendata.v2.2+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "x-open-banking-v2-2-atms-geo-location-lat-long get" [
  ...
  let full_url = (build-url $base ({latitude: (encode-path-segment $latitude), longitude: (encode-path-segment $longitude)} | format pattern "/x-open-banking/v2.2/atms/geo-location/lat/{latitude}/long/{longitude}") $qp $auth.query)
  let accept_val = "application/prs.openbanking.opendata.v2.2+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "x-open-banking-v2-2-atms-postcode get" [
  ...
  let full_url = (build-url $base ({postcode: (encode-path-segment $postcode)} | format pattern "/x-open-banking/v2.2/atms/postcode/{postcode}") $auth.query)
  let accept_val = "application/prs.openbanking.opendata.v2.2+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
