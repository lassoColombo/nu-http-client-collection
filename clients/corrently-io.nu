# Auto-generated client for Corrently.io v2.0.0
# Source: https://api.apis.guru/v2/specs/corrently.io/2.0.0/openapi.json
# Auth: --token flag or $env.CORRENTLY_IO_TOKEN

const BASE_URL = "https://api.corrently.io/v2.0"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o CORRENTLY_IO_TOKEN | default "" }
...
def variation-completer [] { ["baeume" "co2" "eigenstrom" "erzeugung" "gsb"] }
...
export def "alternative-easee-last-sessions get" [
  ...
  let full_url = (build-url $base "/alternative/easee/lastSessions" $qp $auth.query)
...
export def "alternative-ocpp-last-sessions get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/alternative/ocpp/lastSessions" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "alternative-openmeter-activities get-om" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/alternative/openmeter/activities" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "alternative-openmeter-meters get-om" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/alternative/openmeter/meters" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "alternative-openmeter-readings get-om" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/alternative/openmeter/readings" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "gsi-best-hour get-besthour" [
  ...
  let full_url = (build-url $base "/gsi/bestHour" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"zip": $zip, "key": $key, "timeframe": $timeframe, "hours": $hours} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "gsi-dispatch get" [
  ...
  --zip: string # Zipcode (Postleitzahl) of a city in Germany.
  --key: string # Any valid Stromkonto account (address).
]: nothing -> record<avg_distance_km: float, dispatch_from: table<energy: float, location: record>, dispatch_target: table<energy: float, location: record>, postmix: record, premix: record, timeframe: record<end: int, start: int>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "zip" $zip "scalar") (serialize-qp "key" $key "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/gsi/dispatch" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"zip": $zip, "key": $key} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "gsi-marketdata get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "zip" $zip "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/gsi/marketdata" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"zip": $zip} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "gsi-prediction get" [
  ...
  --zip: string # Zipcode (Postleitzahl) of a city in Germany.
  --key: string # Any valid Stromkonto account (address).
]: nothing -> record<forecast: table<co2_g_oekostrom: int, energyprice: float, epochtime: int, gsi: int, scaled: bool, sci: int, timeStamp: int>, location: record<city: string, zip: string>, matrix: record<h0: record<avg_1: string, avg_2: string, avg_3: string>>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "zip" $zip "scalar") (serialize-qp "key" $key "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/gsi/prediction" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "metering-reading get" [
  ...
  let full_url = (build-url $base "/metering/reading" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"account": $account} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "metering-reading create" [
  ...
  --value: int # Alias for 1.8.0
  --zip: string # Zipcode (Postleitzahl) of metered location
]: any -> record<1_8_0: int, 1_8_1: int, 1_8_2: int, _processingTime: int, account: string, co2_g_oekostrom: int, co2_g_standard: int, timeStamp: int> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/metering/reading" $auth.query)
...
export def "quittung-commit create-comit" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/quittung/commit" $auth.query)
...
export def "quittung-create create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/quittung/create" $auth.query)
...
export def "quittung-prepare create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/quittung/prepare" $auth.query)
  let req_body = {"account": $account} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "quittung-tse create" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "account" $account "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/quittung/tse" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "quittung-tsedata create-tse-data" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "account" $account "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/quittung/tsedata" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "quittung-tsesignature create-ts-esignature" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "account" $account "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/quittung/tsesignature" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "quittung-zugferd get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "account" $account "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/quittung/zugferd" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "stromkonto-balances get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "account" $account "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/stromkonto/balances" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "stromkonto-choices get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "account" $account "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/stromkonto/choices" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "stromkonto-login create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/stromkonto/login" $auth.query)
  let req_body = {"email": $email} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "stromkonto-prepare-transaction create" [
  ...
  --body-to: string # Stromkonto account address of reciever
  --value: int # Amount to transfer (in Watthours for electricity, or pcs for trees)
  --variation: string@variation-completer
]: any -> any {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/stromkonto/prepareTransaction" $auth.query)
...
export def "stromkonto-register create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/stromkonto/register" $auth.query)
...
export def "tariff-components get-tariffcomponents" [
  ...
  --kwha: int # Total amount of energy in kilo-watt-hours per year. (sample 2100)
  --milliseconds: int # If provided all results will be scaled to this timeframe
  --wh: int # If provided together with milliseconds, a cost component stament for a particular event (like charging a car) will be created.
]: nothing -> record<components: list<any>, describtion: string, mutlityplier: string, per: float, sum: float> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "zipcode" $zipcode "scalar") (serialize-qp "email" $email "scalar") (serialize-qp "kwha" $kwha "scalar") (serialize-qp "milliseconds" $milliseconds "scalar") (serialize-qp "wh" $wh "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/tariff/components" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"zipcode": $zipcode, "email": $email, "kwha": $kwha, "milliseconds": $milliseconds, "wh": $wh} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "tariff-slph0 get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "zipcode" $zipcode "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/tariff/slph0" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"zipcode": $zipcode} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "wim-status get-wimstatus" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "vid" $vid "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/wim/status" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"vid": $vid} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
