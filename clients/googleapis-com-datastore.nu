...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "v1beta1-projects datastoreprojectsexport" } } | get name | first)
...
# --entityFilter shape: {kinds?: list, namespaceIds?: list}
export def "v1beta1-projects datastoreprojectsexport" [
  projectId: string
  ...
  let full_url = (build-url $base $"/v1beta1/projects/($projectId):export" $qp)
  let body = {entityFilter: $entityFilter, labels: $labels, outputUrlPrefix: $outputUrlPrefix} | compact
...
# --entityFilter shape: {kinds?: list, namespaceIds?: list}
export def "v1beta1-projects datastoreprojectsimport" [
  projectId: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "$.xgafv" $xgafv "scalar") (serialize-qp "access_token" $access_token "scalar") (serialize-qp "alt" $alt "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "fields" $qp_fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $prettyPrint "scalar") (serialize-qp "quotaUser" $quotaUser "scalar") (serialize-qp "upload_protocol" $upload_protocol "scalar") (serialize-qp "uploadType" $uploadType "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v1beta1/projects/($projectId):import" $qp)
  let body = {entityFilter: $entityFilter, inputUrl: $inputUrl, labels: $labels} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json" $body
