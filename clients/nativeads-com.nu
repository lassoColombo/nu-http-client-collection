# Auto-generated client for Native Ads Publisher API v1.0.0
# Source: https://api.apis.guru/v2/specs/nativeads.com/1.0.0/swagger.json
# Auth: --token flag or $env.NATIVE_ADS_PUBLISHER_API_TOKEN

const BASE_URL = "https://api.nativeads.com"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o NATIVE_ADS_PUBLISHER_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.nativeads.com"] }
...
]: any -> record<success: bool, token: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/auth/default/login")
  let req_body = {"username": $username, "password": $password} | compact
...
export def "publisher-reports-daily get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"token": $qp_token, "startDate": $start_date, "endDate": $end_date, "limit": $limit, "page": $page} | compact), body: null}
}
