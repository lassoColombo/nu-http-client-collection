# Auto-generated client for rePower vV2
# Source: https://api.apis.guru/v2/specs/mastercard.com/Repower/V2/swagger.json
# Auth: --token flag or $env.REPOWER_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o REPOWER_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "repower-repower create-post2" } } | get name | first)
...
export def "repower-repower create-post2" [
  ...
  let req_body = {"RepowerRequest": $repower_request} | compact
...
# --RepowerReversalRequest shape: {ICA: string, ReversalReason: string, TransactionReference: string}
export def "repower-repowerreversal create-reversal-post2" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let req_body = {"RepowerReversalRequest": $repower_reversal_request} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/xml"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json" $req_body
