...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o PROFILE_TOKEN | default "" }
...
export def "h5-profiles-appearance get-halo-5" [
  ...
  let full_url = (build-url $base ({player: (encode-path-segment $player)} | format pattern "/h5/profiles/{player}/appearance") $auth.query)
...
export def "h5-profiles-emblem get-halo-5-image" [
  ...
  let full_url = (build-url $base ({player: (encode-path-segment $player)} | format pattern "/h5/profiles/{player}/emblem") $qp $auth.query)
...
export def "h5-profiles-spartan get-halo-5-image" [
  ...
  let full_url = (build-url $base ({player: (encode-path-segment $player)} | format pattern "/h5/profiles/{player}/spartan") $qp $auth.query)
  let accept_val = "image/png"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"size": $size, "crop": $crop} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
