# Auto-generated client for Cloud Speech-to-Text API vv2beta1
# Source: https://api.apis.guru/v2/specs/googleapis.com/speech/v2beta1/openapi.json
# Auth: --token flag or $env.CLOUD_SPEECH_TO_TEXT_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o CLOUD_SPEECH_TO_TEXT_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://speech.googleapis.com"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "v2beta1 get" } } | get name | first)
...
export def "v2beta1 get" [
  ...
  let auth = (merge-auth [(build-auth ($token_oauth2 | default ($env | get -o CLOUD_SPEECH_TO_TEXT_API_OAUTH2_TOKEN | default "")) "bearer") (build-auth ($token_oauth2c | default ($env | get -o CLOUD_SPEECH_TO_TEXT_API_OAUTH2C_TOKEN | default "")) "bearer")])
...
export def "v2beta1-operations list" [
  name: string
  --base-url(-b): string@base-url-completer # API base URL
  --token-oauth2: string # Auth token for Oauth2 (Authorization)
  --token-oauth2c: string # Auth token for Oauth2c (Authorization)
  ...
  let full_url = (build-url $base ({name: (encode-path-segment $name)} | format pattern "/v2beta1/{name}/operations") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
