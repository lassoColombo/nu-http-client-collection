# Auto-generated client for CodeScan API v1.0.0
# Source: https://api.apis.guru/v2/specs/code-scan.com/1.0.0/swagger.json
# Auth: --token flag or $env.CODESCAN_API_TOKEN

const BASE_URL = "https://app.code-scan.com/api"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o CODESCAN_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://app.code-scan.com/api"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "job get" } } | get name | first)
...
export def "job get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"jobId": $job_id} | compact), body: null}
}
...
export def "job create" [
  ...
  let req_body = {"analysisMode": $analysis_mode, "commitOverride": $commit_override, "emailReportTo": $email_report_to, "projectBranch": $project_branch, "projectKey": $project_key, "version": $version} | compact
