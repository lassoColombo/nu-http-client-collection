...
# Source: https://api.apis.guru/v2/specs/azure.com/applicationinsights-componentFeaturesAndPricing_API/2017-10-01/swagger.json
...
]: nothing -> record<properties: record<cap: float, maxHistoryCap: float, planType: string, resetHour: int, stopSendNotificationWhenHitCap: bool, stopSendNotificationWhenHitThreshold: bool, warningThreshold: int>> {
...
export def "subscriptions-resource-groups-providers-microsoft-insights-components-pricing-plans-current update" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # The API version to use for this operation.
  --properties: record # An Application Insights component daily data volume cap — shape: {cap?: float, planType?: string, stopSendNotificationWhenHitCap?: bool, stopSendNotificationWhenHitThreshold?: bool, warningThreshold?: int}
]: any -> record<properties: record<cap: float, maxHistoryCap: float, planType: string, resetHour: int, stopSendNotificationWhenHitCap: bool, stopSendNotificationWhenHitThreshold: bool, warningThreshold: int>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
