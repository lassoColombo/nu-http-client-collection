# Auto-generated client for API for the Settlement Currency Rate converter v1.0.0
# Source: https://api.apis.guru/v2/specs/mastercard.com/CurrencyConversionCalculator/1.0.0/swagger.json
# Auth: --token flag or $env.API_FOR_THE_SETTLEMENT_CURRENCY_RATE_CONVERTER_TOKEN

const BASE_URL = "https://api.mastercard.com/mcapi/settlement/currencyrate"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o API_FOR_THE_SETTLEMENT_CURRENCY_RATE_CONVERTER_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.mastercard.com/mcapi/settlement/currencyrate"] }
...
export def "conversion-rate get-detail-using" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"fxDate": $fx_date, "transCurr": $trans_curr, "crdhldBillCurr": $crdhld_bill_curr, "bankFee": $bank_fee, "transAmt": $trans_amt} | compact), body: null}
}
...
]: nothing -> record<data: record<rateIssued: string>, date: string, description: string, name: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "date" $date "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/conversion-rate-issued" $qp)
  let accept_val = "application/json;charset=UTF-8"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "settlement-currencies get-currency-rate-data-using" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> record<data: record<currencies: list<record>>, date: string, description: string, name: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/settlement-currencies")
  let accept_val = "application/json;charset=UTF-8"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
