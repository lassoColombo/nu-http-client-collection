...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o API_FOR_THE_SETTLEMENT_CURRENCY_RATE_CONVERTER_TOKEN | default "" }
...
export def "conversion-rate get-detail-using" [
  ...
  let full_url = (build-url $base "/conversion-rate" $qp $auth.query)
  let accept_val = "application/json;charset=UTF-8"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"fxDate": $fx_date, "transCurr": $trans_curr, "crdhldBillCurr": $crdhld_bill_curr, "bankFee": $bank_fee, "transAmt": $trans_amt} | compact)
...
export def "conversion-rate-issued get-is-using" [
  ...
  let full_url = (build-url $base "/conversion-rate-issued" $qp $auth.query)
  let accept_val = "application/json;charset=UTF-8"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "settlement-currencies get-currency-rate-data-using" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/settlement-currencies" $auth.query)
  let accept_val = "application/json;charset=UTF-8"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
