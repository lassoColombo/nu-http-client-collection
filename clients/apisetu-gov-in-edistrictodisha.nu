...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "ctcer-certificate ctcer" } } | get name | first)
...
export def "ctcer-certificate ctcer" [
...
# --certificateParameters shape: {UID: string, refno: string}
# --consentArtifact shape: {consent: record, signature: record}
export def "slcer-certificate slcer" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
