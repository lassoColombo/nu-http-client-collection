...
export def "providers-microsoft-security-device-security-groups list" [
  ...
  let full_url = (build-url $base ({resource_id: (encode-path-segment $resource_id)} | format pattern "/{resource_id}/providers/Microsoft.Security/deviceSecurityGroups") $qp)
...
export def "providers-microsoft-security-device-security-groups delete" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({resource_id: (encode-path-segment $resource_id), device_security_group_name: (encode-path-segment $device_security_group_name)} | format pattern "/{resource_id}/providers/Microsoft.Security/deviceSecurityGroups/{device_security_group_name}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
