...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o DATALAKESTOREFILESYSTEMMANAGEMENTCLIENT_TOKEN | default "" }
...
export def "web-hdfs-ext create-file-system-concurrent" [
  ...
  let full_url = (build-url $base ({path: (encode-path-segment $path)} | format pattern "/WebHdfsExt/{path}") $qp $auth.query)
  let req_body = $body
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"appendMode": $append_mode, "op": $op, "syncFlag": $sync_flag, "api-version": $api_version} | compact)
...
export def "web-hdfs-ext update-file-system-file-expiry" [
  ...
  let req = {
    method: "put"
    url: $full_url
    query: ({"expiryOption": $expiry_option, "expireTime": $expire_time, "op": $op, "api-version": $api_version} | compact)
    headers: $auth.headers
    body: null
...
export def "webhdfs check-file-system-access" [
  ...
  let full_url = (build-url $base ({path: (encode-path-segment $path)} | format pattern "/webhdfs/v1/{path}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"fsaction": $fsaction, "op": $op, "api-version": $api_version} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
