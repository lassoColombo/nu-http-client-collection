...
# Source: https://api.apis.guru/v2/specs/azure.com/sql-WorkloadClassifiers/2019-06-01-preview/swagger.json
...
export def "subscriptions-resource-groups-providers-microsoft-sql-servers-databases-workload-groups-workload-classifiers delete" [
  ...
  if ($database_name | is-empty) { error make --unspanned { msg: "path parameter 'databaseName' must be non-empty" } }
  if ($workload_group_name | is-empty) { error make --unspanned { msg: "path parameter 'workloadGroupName' must be non-empty" } }
  if ($workload_classifier_name | is-empty) { error make --unspanned { msg: "path parameter 'workloadClassifierName' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), server_name: (encode-path-segment $server_name), database_name: (encode-path-segment $database_name), workload_group_name: (encode-path-segment $workload_group_name), workload_classifier_name: (encode-path-segment $workload_classifier_name)} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Sql/servers/{server_name}/databases/{database_name}/workloadGroups/{workload_group_name}/workloadClassifiers/{workload_classifier_name}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "subscriptions-resource-groups-providers-microsoft-sql-servers-databases-workload-groups-workload-classifiers get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # The API version to use for the request.
]: nothing -> record<properties: record<context: string, endTime: string, importance: string, label: string, memberName: string, startTime: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
  if ($resource_group_name | is-empty) { error make --unspanned { msg: "path parameter 'resourceGroupName' must be non-empty" } }
...
export def "subscriptions-resource-groups-providers-microsoft-sql-servers-databases-workload-groups-workload-classifiers create-or-update" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # The API version to use for the request.
  --properties: record # Workload classifier definition. For more information look at sys.workload_management_workload_classifiers (DMV). — shape: {context?: string, endTime?: string, importance?: string, label?: string, memberName: string, startTime?: string}
]: any -> record<properties: record<context: string, endTime: string, importance: string, label: string, memberName: string, startTime: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
