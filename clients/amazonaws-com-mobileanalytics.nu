...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AMAZON_MOBILE_ANALYTICS_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "2014-06-05-events update" } } | get name | first)
...
export def "2014-06-05-events update" [
  ...
  let full_url = (build-url $base "/2014-06-05/events" $auth.query)
