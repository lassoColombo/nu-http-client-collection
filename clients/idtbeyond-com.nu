...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o ACTIVE_DOCUMENTATION_FOR_V1_TOKEN | default "" }
...
export def "iatu-balance get" [
  ...
  let full_url = (build-url $base "/iatu/balance" $auth.query)
...
export def "iatu-charges-reports-all get" [
  ...
  let full_url = (build-url $base "/iatu/charges/reports/all" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-idt-beyond-app-id": $x_idt_beyond_app_id, "x-idt-beyond-app-key": $x_idt_beyond_app_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "iatu-charges-reports-all-csv get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "date_from" $date_from "scalar") (serialize-qp "date_to" $date_to "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/iatu/charges/reports/all.csv" $qp $auth.query)
...
export def "iatu-number-validator get" [
  ...
  let full_url = (build-url $base "/iatu/number-validator" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-idt-beyond-app-id": $x_idt_beyond_app_id, "x-idt-beyond-app-key": $x_idt_beyond_app_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "get"
    url: $full_url
    query: ({"country_code": $country_code, "mobile_number": $mobile_number} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "iatu-products-promotions get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/iatu/products/promotions" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-idt-beyond-app-id": $x_idt_beyond_app_id, "x-idt-beyond-app-key": $x_idt_beyond_app_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "iatu-products-reports-all get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/iatu/products/reports/all" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-idt-beyond-app-id": $x_idt_beyond_app_id, "x-idt-beyond-app-key": $x_idt_beyond_app_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "iatu-products-reports-all-csv get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/iatu/products/reports/all.csv" $auth.query)
  let accept_val = "application/csv"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-idt-beyond-app-id": $x_idt_beyond_app_id, "x-idt-beyond-app-key": $x_idt_beyond_app_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "iatu-products-reports-local-value get" [
  ...
  let full_url = (build-url $base "/iatu/products/reports/local-value" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-idt-beyond-app-id": $x_idt_beyond_app_id, "x-idt-beyond-app-key": $x_idt_beyond_app_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "get"
    url: $full_url
    query: ({"country_code": $country_code, "carrier_code": $carrier_code, "amount": $amount, "currency_code": $currency_code} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "iatu-topups create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/iatu/topups" $auth.query)
...
export def "iatu-topups-reports create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/iatu/topups/reports" $auth.query)
...
export def "iatu-topups-reports-all get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "date_from" $date_from "scalar") (serialize-qp "date_to" $date_to "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/iatu/topups/reports/all" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-idt-beyond-app-id": $x_idt_beyond_app_id, "x-idt-beyond-app-key": $x_idt_beyond_app_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "iatu-topups-reports-all-csv get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "date_from" $date_from "scalar") (serialize-qp "date_to" $date_to "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/iatu/topups/reports/all.csv" $qp $auth.query)
  let accept_val = "application/csv"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-idt-beyond-app-id": $x_idt_beyond_app_id, "x-idt-beyond-app-key": $x_idt_beyond_app_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "iatu-topups-reports-totals get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "date_from" $date_from "scalar") (serialize-qp "date_to" $date_to "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/iatu/topups/reports/totals" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"x-idt-beyond-app-id": $x_idt_beyond_app_id, "x-idt-beyond-app-key": $x_idt_beyond_app_key} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "iatu-topups-reverse create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/iatu/topups/reverse" $auth.query)
