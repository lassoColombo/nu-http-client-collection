# Auto-generated client for Afterbanks API v3.0.0
# Source: https://api.apis.guru/v2/specs/afterbanks.com/3.0.0/swagger.json
# Auth: --token flag or $env.AFTERBANKS_API_TOKEN

const BASE_URL = "https://www.afterbanks.com"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o AFTERBANKS_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://www.afterbanks.com"] }
...
export def "forms get" [
  ...
  --country-code: string # Código del país, formato ISO 3166-1 alpha-2 (https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2). Por ejemplo ES para España. Si no se proporciona ningún código, se devolverá la lista completa.
]: nothing -> table<business: bool, country_code: string, documentType: string, fullname: string, image: string, pass: string, pass2: string, pass2desc: string, pass2type: string, passdesc: string, passtype: string, service: string, swift: string, user: string, userdesc: string, usertype: string> {
...
]: nothing -> record<date_renewal: string, remaining_calls: int> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "servicekey" $servicekey "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"servicekey": $servicekey} | compact), body: null}
}
...
export def "service-v3 create" [
  ...
  --startdate: string # Fecha desde la que queremos obtener los movimientos en formato dd-mm-aaaa. Obligatorio cuando el valor de "products" es distinto de GLOBAL.
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"servicekey": $servicekey, "service": $service, "documentType": $document_type, "user": $user, "pass": $pass, "pass2": $pass2, "products": $products, "startdate": $startdate} | compact), body: null}
}
