...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AFTERBANKS_API_TOKEN | default "" }
...
export def "me create" [
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"servicekey": $servicekey} | compact)
    headers: $auth.headers
    body: null
...
export def "service-v3 create" [
  ...
  let full_url = (build-url $base "/serviceV3" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
    url: $full_url
    query: ({"servicekey": $servicekey, "service": $service, "documentType": $document_type, "user": $user, "pass": $pass, "pass2": $pass2, "products": $products, "startdate": $startdate} | compact)
    headers: $auth.headers
    body: null
    content_type: "application/json"
    timeout: ($max_time | default 30min)
