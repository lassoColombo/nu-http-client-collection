# Auto-generated client for SpectroCoin Merchant v1.0.0
# Source: https://api.apis.guru/v2/specs/spectrocoin.com/1.0.0/swagger.json
# Auth: --token flag or $env.SPECTROCOIN_MERCHANT_TOKEN

const BASE_URL = "https://spectrocoin.com/api/merchant/1"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o SPECTROCOIN_MERCHANT_TOKEN | default "" }
...
def base-url-completer [] { ["https://spectrocoin.com/api/merchant/1"] }
def auth-scheme-completer [] { ["bearer"] }

# Completers for enum parameters
def culture-completer [] { ["de" "en" "lt" "ru"] }
