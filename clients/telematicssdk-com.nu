...
export def "mobilesdk-stage-track-get-track get-trips-trip-details" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"trackToken": $track_token} | compact), body: null}
}
...
export def "statistics-scorings-consolidated get" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"DeviceToken": $device_token, "StartDate": $start_date, "EndDate": $end_date, "Tag": $tag, "InstanceId": $instance_id, "AppId": $app_id, "CompanyId": $company_id} | compact), body: null}
}
...
export def "statistics-scorings-individual-daily get-user-safe-value" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"Tag": $tag, "StartDate": $start_date, "EndDate": $end_date} | compact), body: null}
}
