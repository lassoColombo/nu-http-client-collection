...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o HOTEL_NAME_AUTOCOMPLETE_TOKEN | default "" }
...
export def "reference-data-locations-hotel get" [
  ...
  let full_url = (build-url $base "/reference-data/locations/hotel" $qp $auth.query)
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"keyword": $keyword, "subType": $sub_type, "countryCode": $country_code, "lang": $lang, "max": $max} | compact)
