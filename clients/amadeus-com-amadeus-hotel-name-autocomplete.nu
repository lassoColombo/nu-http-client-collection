# Auto-generated client for Hotel Name Autocomplete v1.0.3
# Source: https://api.apis.guru/v2/specs/amadeus.com/amadeus-hotel-name-autocomplete/1.0.3/swagger.json
# Auth: --token flag or $env.HOTEL_NAME_AUTOCOMPLETE_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o HOTEL_NAME_AUTOCOMPLETE_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "reference-data-locations-hotel get-gethotels" } } | get name | first)
...
export def "reference-data-locations-hotel get-gethotels" [
  ...
  --sub-type: list<string> # Category of search - To enter several values, repeat the query parameter Use HOTEL_LEISURE to target aggregators or HOTEL_GDS to target directly the chains
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"keyword": $keyword, "subType": $sub_type, "countryCode": $country_code, "lang": $lang, "max": $max} | compact), body: null}
}
