# Auto-generated client for City of Surrey Open511 API v0.1
# Source: https://api.apis.guru/v2/specs/surrey.ca/open511/0.1/swagger.json
# Auth: --token flag or $env.CITY_OF_SURREY_OPEN511_API_TOKEN

const BASE_URL = "http://data.surrey.ca/open511"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o CITY_OF_SURREY_OPEN511_API_TOKEN | default "" }
...
def base-url-completer [] { ["http://data.surrey.ca/open511"] }
...
export def "events get" [
  ...
  --event-type: string # Limits the response to events tagged with one of the listed event types. The possible values include: [CONSTRUCTION, INCIDENT, SPECIAL_EVENT, WEATHER_CONDITION]. Multiple values may be listed, and should be separated by a comma. The default is to return events of all types.
  ...
  --road-name: string # Limits the response to events on a given road as specified by the road name. An example of a valid road name is 'Highway 1'. The default is to return events on all roads.
  --area-id: string # Limits the response to events within one of the specified areas. An area must be specified as the ID of an item returned by the /areas resource. For example: an area_id of 'drivebc.ca/1' limits events to those within the Lower Mainland District. The default is to return events in all areas.
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "format" $format "scalar") (serialize-qp "status" $status "scalar") (serialize-qp "severity" $severity "scalar") (serialize-qp "jurisdiction" $jurisdiction "scalar") (serialize-qp "event_type" $event_type "scalar") (serialize-qp "created" $created "scalar") (serialize-qp "updated" $updated "scalar") (serialize-qp "road_name" $road_name "scalar") (serialize-qp "area_id" $area_id "scalar") (serialize-qp "bbox" $bbox "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/events" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"format": $format, "status": $status, "severity": $severity, "jurisdiction": $jurisdiction, "event_type": $event_type, "created": $created, "updated": $updated, "road_name": $road_name, "area_id": $area_id, "bbox": $bbox} | compact), body: null}
}
...
export def "jurisdiction get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "format" $format "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/jurisdiction" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"format": $format} | compact), body: null}
}
...
export def "jurisdictiongeography get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "format" $format "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/jurisdictiongeography" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"format": $format} | compact), body: null}
}
