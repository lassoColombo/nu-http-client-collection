...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o SIMPLYRETS_TOKEN | default "" }
...
export def "openhouses list" [
  ...
  let full_url = (build-url $base "/openhouses" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"type": $type, "listingId": $listing_id, "cities": $cities, "brokers": $brokers, "agent": $agent, "minprice": $minprice, "startdate": $startdate, "offset": $offset, "lastId": $last_id, "limit": $limit, "sort": $qp_sort, "include": $include} | compact)
...
export def "openhouses get" [
  ...
  let full_url = (build-url $base ({open_house_key: (encode-path-segment $open_house_key)} | format pattern "/openhouses/{open_house_key}") $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "properties get" [
  ...
  let qp = [(serialize-qp "include" $include "multi")] | flatten | str join "&"
  let full_url = (build-url $base ({mls_id: (encode-path-segment $mls_id)} | format pattern "/properties/{mls_id}") $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
