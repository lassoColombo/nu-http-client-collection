# Auto-generated client for Airport Nearest Relevant v1.1.2
# Source: https://api.apis.guru/v2/specs/amadeus.com/amadeus-airport-nearest-relevant/1.1.2/swagger.json
# Auth: --token flag or $env.AIRPORT_NEAREST_RELEVANT_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o AIRPORT_NEAREST_RELEVANT_TOKEN | default "" }
...
def sort-completer [] { ["analytics.flights.score" "analytics.travelers.score" "distance" "relevance"] }
...
export def "reference-data-locations-airports get-nearest-relevant" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"latitude": $latitude, "longitude": $longitude, "radius": $radius, "page[limit]": $page_limit, "page[offset]": $page_offset, "sort": $qp_sort} | compact), body: null}
}
