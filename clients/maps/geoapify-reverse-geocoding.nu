# Auto-generated client for Reverse Geocoding API v1.0.0
# Source: https://raw.githubusercontent.com/geoapify/geoapify-openapi-specs/main/api-specs/geocoding/reverse_geocoding.yaml
# Auth: --token flag or $env.REVERSE_GEOCODING_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o REVERSE_GEOCODING_API_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "geocode-reverse get" } } | get name | first)
...
export def "geocode-reverse get" [
  ...
  --lat: float # The latitude of the location to reverse geocode. (format: float, e.g. 36.47032)
  --lon: float # The longitude of the location to reverse geocode. (format: float, e.g. -86.65138)
  ...
  --format: string@format-completer # The format of the response (JSON, XML, or GeoJSON). (e.g. json)
  --limit: int # The maximum number of results to return. (e.g. 1)
  ...
  let qp = [(serialize-qp "lat" $lat "scalar") (serialize-qp "lon" $lon "scalar") (serialize-qp "apiKey" $apiKey "scalar") (serialize-qp "format" $format "scalar") (serialize-qp "limit" $limit "scalar") (serialize-qp "type" $type "scalar") (serialize-qp "lang" $lang "scalar")] | flatten | str join "&"
