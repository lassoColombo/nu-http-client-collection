...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o FLIGHT_CREATE_ORDERS_TOKEN | default "" }
...
export def "booking-flight-orders create-fligt" [
  ...
  let full_url = (build-url $base "/booking/flight-orders" $auth.query)
  let req_body = $body
