...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o SOCCER_V3_PROJECTIONS_TOKEN | default "" }
...
export def "injured-players get" [
  ...
  let full_url = (build-url $base ({format: (encode-path-segment $format), competition: (encode-path-segment $competition)} | format pattern "/{format}/InjuredPlayers/{competition}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "player-game-projection-stats-by-competition stats-projected-w-dfs-salaries" [
  ...
  if ($competition | is-empty) { error make --unspanned { msg: "path parameter 'competition' must be non-empty" } }
  if ($date | is-empty) { error make --unspanned { msg: "path parameter 'date' must be non-empty" } }
  let full_url = (build-url $base ({format: (encode-path-segment $format), competition: (encode-path-segment $competition), date: (encode-path-segment $date)} | format pattern "/{format}/PlayerGameProjectionStatsByCompetition/{competition}/{date}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
