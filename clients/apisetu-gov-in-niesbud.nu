# Auto-generated client for NIESBUD v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/niesbud/3.0.0/openapi.json
# Auth: --token flag or $env.NIESBUD_TOKEN

const BASE_URL = "https://apisetu.gov.in/niesbud/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o NIESBUD_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/niesbud/v3"] }
...
# operationId: skcer
# --certificateParameters shape: {COURSE_NO: string, FullName: string, certificate_type: string}
...
export def "skcer-certificate create" [
  ...
  --certificate-parameters: record # shape: {COURSE_NO: string, FullName: string, certificate_type: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o NIESBUD_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o NIESBUD_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
