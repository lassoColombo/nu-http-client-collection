...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o AZURE_ADDONS_RESOURCE_PROVIDER_TOKEN | default "" }
...
export def "providers-microsoft-addons-operations list" [
  ...
  let full_url = (build-url $base "/providers/Microsoft.Addons/operations" $qp)
...
export def "subscriptions-providers-microsoft-addons-support-providers-canonical-list-support-plan-info list-types" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id)} | format pattern "/subscriptions/{subscription_id}/providers/Microsoft.Addons/supportProviders/canonical/listSupportPlanInfo") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
