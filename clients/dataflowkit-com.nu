...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o DATAFLOW_KIT_WEB_SCRAPER_TOKEN | default "" }
...
export def "convert-url-pdf create" [
  ...
  let full_url = (build-url $base "/convert/url/pdf" $auth.query)
...
export def "convert-url-screenshot create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "query-api_key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/convert/url/screenshot" $auth.query)
...
export def "fetch get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "query-api_key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/fetch" $auth.query)
...
export def "serp create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "query-api_key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/serp" $auth.query)
