# Auto-generated client for TVmaze user API v1.0
# Source: https://api.apis.guru/v2/specs/tvmaze.com/1.0/openapi.json
# Auth: --token flag or $env.TVMAZE_USER_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o TVMAZE_USER_API_TOKEN | default "" }
...
def embed-completer-1 [] { ["network"] }
def embed-completer-2 [] { ["person"] }
def embed-completer-3 [] { ["show"] }
def embed-completer-4 [] { ["webchannel"] }
...
export def "auth-poll create" [
  ...
  --body-token: string
]: any -> record<apikey: string, username: string> {
  let input = $in
...
export def "auth-validate get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> record<premium: int> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/auth/validate")
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "scrobble-episodes create" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --body: list
]: any -> table<code: int, errors: record, input: record, message: string, result: record> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
...
export def "scrobble-episodes update" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --embedded: record # shape: {episode?: record}
  --marked-at: int # Epoch timestamp for when the user watched the episode, or 0 for unknown.
  --type: any # 0 for watched, 1 for acquired, 2 for skipped
]: any -> record<_embedded: record<episode: record>, episode_id: int, marked_at: int, type: any> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  if ($episode_id | is-empty) { error make --unspanned { msg: "path parameter 'episode_id' must be non-empty" } }
...
export def "scrobble-shows create" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --tvmaze-id: int # The show's TVmaze ID
  --thetvdb-id: int # The show's TheTVDB ID
  --imdb-id: int # The show's IMDB ID
  --body: list
]: any -> table<code: int, errors: record, input: record, message: string, result: record> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"tvmaze_id": $tvmaze_id, "thetvdb_id": $thetvdb_id, "imdb_id": $imdb_id} | compact), body: $req_body}
}
...
export def "scrobble-shows get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --embed: string@embed-completer # Embed full episode info
]: nothing -> table<_embedded: record<episode: record>, episode_id: int, marked_at: int, type: any> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  if ($show_id | is-empty) { error make --unspanned { msg: "path parameter 'show_id' must be non-empty" } }
...
export def "user-episodes list" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --show-id: int # Only return episodes from this specific show
]: nothing -> table<_embedded: record<episode: record>, episode_id: int, marked_at: int, type: any> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "show_id" $show_id "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/user/episodes" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"show_id": $show_id} | compact), body: null}
}
...
export def "user-follows-networks list" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --embed: string@embed-completer-1 # Embed full network info
]: nothing -> table<_embedded: record<network: record>, network_id: int> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "embed" $embed "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/user/follows/networks" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"embed": $embed} | compact), body: null}
}
...
export def "user-follows-people list" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --embed: string@embed-completer-2 # Embed full person info
]: nothing -> table<_embedded: record<person: record>, person_id: int> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "embed" $embed "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/user/follows/people" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"embed": $embed} | compact), body: null}
}
...
export def "user-follows-people delete" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  if ($person_id | is-empty) { error make --unspanned { msg: "path parameter 'person_id' must be non-empty" } }
...
export def "user-follows-shows list" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --embed: string@embed-completer-3 # Embed full show info
]: nothing -> table<_embedded: record<show: record>, show_id: int> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "embed" $embed "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/user/follows/shows" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"embed": $embed} | compact), body: null}
}
...
export def "user-follows-webchannels list" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --embed: string@embed-completer-4 # Embed full webchannel info
]: nothing -> table<_embedded: record<webchannel: record>, webchannel_id: int> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "embed" $embed "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/user/follows/webchannels" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"embed": $embed} | compact), body: null}
}
...
export def "user-follows-webchannels delete" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  if ($webchannel_id | is-empty) { error make --unspanned { msg: "path parameter 'webchannel_id' must be non-empty" } }
...
export def "user-votes-episodes list" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> table<episode_id: int, vote: int, voted_at: int> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/user/votes/episodes")
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
...
export def "user-votes-episodes update" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --vote: int # The vote number
]: any -> record<episode_id: int, vote: int, voted_at: int> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  if ($episode_id | is-empty) { error make --unspanned { msg: "path parameter 'episode_id' must be non-empty" } }
...
export def "user-votes-shows list" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --embed: string@embed-completer-3 # Embed full show info
]: nothing -> table<show_id: int, vote: int, voted_at: int> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "embed" $embed "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/user/votes/shows" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"embed": $embed} | compact), body: null}
}
...
export def "user-votes-shows update" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --vote: int # The vote number
]: any -> record<show_id: int, vote: int, voted_at: int> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  if ($show_id | is-empty) { error make --unspanned { msg: "path parameter 'show_id' must be non-empty" } }
