# Auto-generated client for ZEIT API vv2019-01-07
# Source: https://api.apis.guru/v2/specs/zeit.co/v2019-01-07/openapi.json
# Auth: --token flag or $env.ZEIT_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o ZEIT_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.zeit.co"] }
...
export def "integrations-webhooks get" [
  ...
  let full_url = (build-url $base "/v1/integrations/webhooks" $qp $auth.query)
...
export def "integrations-webhooks create" [
  ...
  url: string # The URL ZEIT will post to when events happen
  ...
  let qp = [(serialize-qp "teamId" $team_id "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/v1/integrations/webhooks" $qp $auth.query)
  let req_body = {"name": $name, "url": $url} | compact
...
export def "integrations-webhooks-id delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "teamId" $team_id "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/v1/integrations/webhooks/:id" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "domains get" [
  ...
  let qp = [(serialize-qp "teamId" $team_id "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({name: (encode-path-segment $name)} | format pattern "/v4/domains/{name}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
