...
export def "gcr-flux-dlr get-calculate" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"year": $year, "month": $month, "day": $day, "z": $z, "energy": $energy} | compact), body: null}
}
...
export def "trapped-flux-mean get-calculate" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"model": $model, "coord_sys": $coord_sys, "coord_units": $coord_units, "coord1": $coord1, "coord2": $coord2, "coord3": $coord3, "year": $year, "month": $month, "day": $day, "hour": $hour, "minute": $minute, "second": $second} | compact), body: null}
}
...
export def "trapped-flux-percentile get-calculate" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"model": $model, "coord_sys": $coord_sys, "coord_units": $coord_units, "coord1": $coord1, "coord2": $coord2, "coord3": $coord3, "year": $year, "month": $month, "day": $day, "hour": $hour, "minute": $minute, "second": $second, "percentile": $percentile} | compact), body: null}
}
