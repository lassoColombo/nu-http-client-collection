...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o VISUAL_STUDIO_RESOURCE_PROVIDER_CLIENT_TOKEN | default "" }
...
export def "providers-microsoft-visualstudio-operations list" [
  ...
  let full_url = (build-url $base "/providers/microsoft.visualstudio/operations" $auth.query)
...
export def "subscriptions-providers-microsoft-visualstudio-check-name-availability check-accounts" [
  ...
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id)} | format pattern "/subscriptions/{subscription_id}/providers/microsoft.visualstudio/checkNameAvailability") $qp $auth.query)
...
export def "subscriptions-resourcegroups-providers-microsoft-visualstudio-account list-by-resource-group" [
  ...
  if ($resource_group_name | is-empty) { error make --unspanned { msg: "path parameter 'resourceGroupName' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name)} | format pattern "/subscriptions/{subscription_id}/resourcegroups/{resource_group_name}/providers/microsoft.visualstudio/account") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
