...
export def "customers-search-ads360-search searchads360customerssearchAds360search" [
  ...
  --returnTotalResultsCount: oneof<nothing, bool> # If true, the total number of results that match the query ignoring the LIMIT clause will be included in the response. Default is false.
  ...
  --validateOnly: oneof<nothing, bool> # If true, the request is validated but not executed.
  ...
  let body = {pageSize: $pageSize, pageToken: $pageToken, query: $query, returnTotalResultsCount: $returnTotalResultsCount, summaryRowSetting: $summaryRowSetting, validateOnly: $validateOnly} | compact
...
export def "customers-search-ads360-search-stream searchads360customerssearchAds360searchStream" [
  ...
  let body = {batchSize: $batchSize, query: $query, summaryRowSetting: $summaryRowSetting} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json" $body
