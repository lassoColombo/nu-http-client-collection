...
# Auth: --token flag or $env.ANDAMAN_LAW_COLLEGE_ANDAMAN___NICOBAR_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o ANDAMAN_LAW_COLLEGE_ANDAMAN___NICOBAR_TOKEN | default "" }
