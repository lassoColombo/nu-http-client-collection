...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o RELIANCE_GENERAL_INSURANCE_COMPANY_LTD_TOKEN | default "" }
...
# --certificateParameters shape: {DOB: string, FullName: string, GENDER: string, PolicyNo: string, PolicyType: string}
# --consentArtifact shape: {consent: record, signature: record}
export def "miipc-certificate miipc" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
