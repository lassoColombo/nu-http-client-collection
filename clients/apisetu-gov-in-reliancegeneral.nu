# Auto-generated client for Reliance General Insurance Company Ltd v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/reliancegeneral/3.0.0/openapi.json
# Auth: --token flag or $env.RELIANCE_GENERAL_INSURANCE_COMPANY_LTD_TOKEN

const BASE_URL = "https://apisetu.gov.in/reliancegeneral/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o RELIANCE_GENERAL_INSURANCE_COMPANY_LTD_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/reliancegeneral/v3"] }
...
# operationId: cripc
# --certificateParameters shape: {DOB: string, FullName: string, GENDER: string, PolicyNo: string, PolicyType: string}
...
export def "cripc-certificate create" [
  ...
  --certificate-parameters: record # shape: {DOB: string, FullName: string, GENDER: string, PolicyNo: string, PolicyType: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o RELIANCE_GENERAL_INSURANCE_COMPANY_LTD_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o RELIANCE_GENERAL_INSURANCE_COMPANY_LTD_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
