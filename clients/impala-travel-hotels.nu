...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o IMPALA_HOTEL_BOOKING_API_TOKEN | default "" }
...
export def "bookings list" [
  ...
  let full_url = (build-url $base "/bookings" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"start": $start, "end": $end, "created": $created, "updated": $updated, "size": $size, "offset": $offset, "sortBy": $sort_by} | compact)
...
export def "bookings create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/bookings" $auth.query)
...
export def "bookings cancel" [
  ...
  let full_url = (build-url $base ({booking_id: (encode-path-segment $booking_id)} | format pattern "/bookings/{booking_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "bookings-booking-contact update" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($booking_id | is-empty) { error make --unspanned { msg: "path parameter 'bookingId' must be non-empty" } }
  let full_url = (build-url $base ({booking_id: (encode-path-segment $booking_id)} | format pattern "/bookings/{booking_id}/booking-contact") $auth.query)
...
export def "hotels list" [
  ...
  let full_url = (build-url $base "/hotels" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"name": $name, "starRating": $star_rating, "country": $country, "start": $start, "end": $end, "latitude": $latitude, "longitude": $longitude, "radius": $radius, "hotelIds": $hotel_ids, "created": $created, "updated": $updated, "size": $size, "offset": $offset, "sortBy": $sort_by} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "hotels get" [
  ...
  let full_url = (build-url $base ({hotel_id: (encode-path-segment $hotel_id)} | format pattern "/hotels/{hotel_id}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "hotels-rate-plans list" [
  ...
  let full_url = (build-url $base ({hotel_id: (encode-path-segment $hotel_id)} | format pattern "/hotels/{hotel_id}/rate-plans") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"updatedAt": $updated_at, "size": $size, "offset": $offset, "start": $start, "end": $end, "roomId": $room_id} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "hotels-rate-plans list-for" [
  ...
  let full_url = (build-url $base ({hotel_id: (encode-path-segment $hotel_id), rate_plan_id: (encode-path-segment $rate_plan_id)} | format pattern "/hotels/{hotel_id}/rate-plans/{rate_plan_id}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"updatedAt": $updated_at, "size": $size, "offset": $offset, "start": $start, "end": $end, "roomTypeId": $room_type_id} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
