...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o QUALTRICS_API_TOKEN | default "" }
...
export def "directories-mailinglists-contacts create" [
  ...
  if ($directory_id | is-empty) { error make --unspanned { msg: "path parameter 'DirectoryId' must be non-empty" } }
  if ($mailing_list_id | is-empty) { error make --unspanned { msg: "path parameter 'MailingListId' must be non-empty" } }
  let full_url = (build-url $base ({directory_id: (encode-path-segment $directory_id), mailing_list_id: (encode-path-segment $mailing_list_id)} | format pattern "/directories/{directory_id}/mailinglists/{mailing_list_id}/contacts") $auth.query)
  let req_body = {"email": $email, "firstName": $first_name, "lastName": $last_name, "unsubscribed": $unsubscribed} | compact
...
export def "distributions get" [
  ...
  let full_url = (build-url $base "/distributions" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"surveyId": $survey_id} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "distributions generate-links" [
  ...
  let req_body = {"action": $action, "description": $description, "expirationDate": $expiration_date, "linkType": $link_type, "mailingListId": $mailing_list_id, "surveyId": $survey_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
...
export def "distributions-links get-distributionlinks" [
  ...
  let qp = [(serialize-qp "surveyId" $survey_id "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({distribution_id: (encode-path-segment $distribution_id)} | format pattern "/distributions/{distribution_id}/links") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "eventsubscriptions delete-webhook" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-token"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/eventsubscriptions/" $auth.query)
  let req_body = {"encrypt": $encrypt, "publicationUrl": $publication_url, "topics": $topics} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
...
export def "eventsubscriptions get-event-subscriptions" [
  ...
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id)} | format pattern "/eventsubscriptions/{subscription_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "survey-definitions get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-token"))
  let base = ($base_url | default $BASE_URL)
  if ($survey_id | is-empty) { error make --unspanned { msg: "path parameter 'SurveyId' must be non-empty" } }
  let full_url = (build-url $base ({survey_id: (encode-path-segment $survey_id)} | format pattern "/survey-definitions/{survey_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
