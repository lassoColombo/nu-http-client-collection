# Auto-generated client for Safe Place v1.0.0
# Source: https://api.apis.guru/v2/specs/amadeus.com/amadeus-safe-place-/1.0.0/swagger.json
# Auth: --token flag or $env.SAFE_PLACE_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o SAFE_PLACE_TOKEN | default "" }
...
export def "safety-safety-rated-locations list" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"latitude": $latitude, "longitude": $longitude, "radius": $radius, "page[limit]": $page_limit, "page[offset]": $page_offset} | compact), body: null}
}
...
export def "safety-safety-rated-locations-by-square get-rank" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --north: float # Latitude north of bounding box (decimal coordinates) (format: double, e.g. 41.397158)
  ...
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"north": $north, "west": $west, "south": $south, "east": $east, "page[limit]": $page_limit, "page[offset]": $page_offset} | compact), body: null}
}
...
export def "safety-safety-rated-locations get-ranking" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($safety_rated_location_id | is-empty) { error make --unspanned { msg: "path parameter 'safety-rated-locationId' must be non-empty" } }
  let full_url = (build-url $base ({safety_rated_location_id: (encode-path-segment $safety_rated_location_id)} | format pattern "/safety/safety-rated-locations/{safety_rated_location_id}"))
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
