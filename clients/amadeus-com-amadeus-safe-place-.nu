...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o SAFE_PLACE_TOKEN | default "" }
...
export def "safety-safety-rated-locations list" [
  ...
  let full_url = (build-url $base "/safety/safety-rated-locations" $qp $auth.query)
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"latitude": $latitude, "longitude": $longitude, "radius": $radius, "page[limit]": $page_limit, "page[offset]": $page_offset} | compact)
...
export def "safety-safety-rated-locations-by-square get-rank" [
  ...
  let full_url = (build-url $base "/safety/safety-rated-locations/by-square" $qp $auth.query)
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"north": $north, "west": $west, "south": $south, "east": $east, "page[limit]": $page_limit, "page[offset]": $page_offset} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "safety-safety-rated-locations get-ranking" [
  ...
  let full_url = (build-url $base ({safety_rated_location_id: (encode-path-segment $safety_rated_location_id)} | format pattern "/safety/safety-rated-locations/{safety_rated_location_id}") $auth.query)
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
