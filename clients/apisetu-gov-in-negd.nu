# Auto-generated client for National e-Governance Division v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/negd/3.0.0/openapi.json
# Auth: --token flag or $env.NATIONAL_E_GOVERNANCE_DIVISION_TOKEN

const BASE_URL = "https://apisetu.gov.in/negd/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o NATIONAL_E_GOVERNANCE_DIVISION_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/negd/v3"] }
...
# operationId: govid
# --certificateParameters shape: {FullName: string, cardno: string}
...
export def "govid-certificate create" [
  ...
  --certificate-parameters: record # shape: {FullName: string, cardno: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o NATIONAL_E_GOVERNANCE_DIVISION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o NATIONAL_E_GOVERNANCE_DIVISION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
