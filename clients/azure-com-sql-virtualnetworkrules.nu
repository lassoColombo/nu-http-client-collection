...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "subscriptions-resource-groups-providers-microsoft-sql-servers-virtual-network-rules ListByServer" } } | get name | first)
...
export def "subscriptions-resource-groups-providers-microsoft-sql-servers-virtual-network-rules ListByServer" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Sql/servers/($serverName)/virtualNetworkRules" $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-sql-servers-virtual-network-rules Delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Sql/servers/($serverName)/virtualNetworkRules/($virtualNetworkRuleName)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "subscriptions-resource-groups-providers-microsoft-sql-servers-virtual-network-rules CreateOrUpdate" [
  resourceGroupName: string
  serverName: string
  virtualNetworkRuleName: string
  subscriptionId: string
