...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o NEXMO_CONVERSION_API_TOKEN | default "" }
...
export def "sms create-conversion" [
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"message-id": $message_id, "delivered": $delivered, "timestamp": $timestamp} | compact)
