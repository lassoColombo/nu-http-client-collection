# Auto-generated client for Nexmo Conversion API v1.0.1
# Source: https://api.apis.guru/v2/specs/nexmo.com/conversion/1.0.1/openapi.json
# Auth: --token flag or $env.NEXMO_CONVERSION_API_TOKEN

const BASE_URL = "https://api.nexmo.com/conversions"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o NEXMO_CONVERSION_API_TOKEN | default "" }
  ...
  match $scheme {
    ...
    "query-sig" => { {scheme: $scheme, headers: {}, query: $"(encode-path-segment "sig")=(encode-path-segment $token_val)", location: "query"} }
...
def base-url-completer [] { ["https://api.nexmo.com/conversions"] }
def auth-scheme-completer [] { ["query-api_key" "query-api_secret" "query-sig"] }

# Completers for enum parameters
def delivered-completer [] { ["0" "1" "false" "true"] }
...
export def "sms create-conversion" [
  ...
  --message-id: string # The ID you receive in the response to a request. * From the Verify API - use the `event_id` in the response to Verify Check. * From the SMS API - use the `message-id` * From the Text-To-Speech API - use the `call-id` * From the Text-To-Speech-Prompt API - use the `call-id` (e.g. 00A0B0C0)
  ...
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"message-id": $message_id, "delivered": $delivered, "timestamp": $timestamp} | compact), body: null}
}
