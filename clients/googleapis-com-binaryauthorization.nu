...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o BINARY_AUTHORIZATION_API_TOKEN | default "" }
...
export def "v1beta1 validate-attestation-occurrence" [
  ...
  let full_url = (build-url $base ({attestor: (encode-path-segment $attestor)} | format pattern "/v1beta1/{attestor}:validateAttestationOccurrence") $qp $auth.query)
...
export def "v1beta1-attestors list" [
  ...
  let full_url = (build-url $base ({parent: (encode-path-segment $parent)} | format pattern "/v1beta1/{parent}/attestors") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "v1beta1-attestors create" [
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"$.xgafv": $xgafv, "access_token": $access_token, "alt": $alt, "callback": $callback, "fields": $fields, "key": $key, "oauth_token": $oauth_token, "prettyPrint": $pretty_print, "quotaUser": $quota_user, "upload_protocol": $upload_protocol, "uploadType": $upload_type, "attestorId": $attestor_id} | compact)
    headers: $auth.headers
    body: $req_body
    content_type: "application/json"
    timeout: ($max_time | default 30min)
