...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o WAYBACK_API_TOKEN | default "" }
...
export def "wayback-available get" [
  ...
  let full_url = (build-url $base "/wayback/v1/available" $qp $auth.query)
  let accept_val = ($accept | default "application/javascript")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"url": $url, "timestamp": $timestamp, "callback": $callback, "timeout": $timeout, "closest": $closest, "status_code": $status_code, "tag": $tag} | compact)
