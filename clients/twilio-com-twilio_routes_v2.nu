# Auto-generated client for Twilio - Routes v1.42.0
# Source: https://api.apis.guru/v2/specs/twilio.com/twilio_routes_v2/1.42.0/openapi.json
# Auth: --token flag or $env.TWILIO_ROUTES_TOKEN

const BASE_URL = "https://routes.twilio.com"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o TWILIO_ROUTES_TOKEN | default "" }
...
def base-url-completer [] { ["https://routes.twilio.com"] }
...
export def "phone-numbers get" [
  ...
  let full_url = (build-url $base ({phone_number: (encode-path-segment $phone_number)} | format pattern "/v2/PhoneNumbers/{phone_number}") $auth.query)
...
export def "phone-numbers update" [
  ...
  if ($phone_number | is-empty) { error make --unspanned { msg: "path parameter 'PhoneNumber' must be non-empty" } }
  let full_url = (build-url $base ({phone_number: (encode-path-segment $phone_number)} | format pattern "/v2/PhoneNumbers/{phone_number}") $auth.query)
  let req_body = {"FriendlyName": $friendly_name, "VoiceRegion": $voice_region} | compact
...
export def "sip-domains get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://routes.twilio.com")
  if ($sip_domain | is-empty) { error make --unspanned { msg: "path parameter 'SipDomain' must be non-empty" } }
  let full_url = (build-url $base ({sip_domain: (encode-path-segment $sip_domain)} | format pattern "/v2/SipDomains/{sip_domain}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "trunks get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://routes.twilio.com")
  if ($sip_trunk_domain | is-empty) { error make --unspanned { msg: "path parameter 'SipTrunkDomain' must be non-empty" } }
  let full_url = (build-url $base ({sip_trunk_domain: (encode-path-segment $sip_trunk_domain)} | format pattern "/v2/Trunks/{sip_trunk_domain}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
