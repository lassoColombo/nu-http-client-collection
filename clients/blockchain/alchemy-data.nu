# Auto-generated client for 🧠 Data v1.0
# Source: https://raw.githubusercontent.com/alchemyplatform/docs-openapi-specs/main/data/v1.yaml
# Auth: --token flag or $env.DATA_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o DATA_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.g.alchemy.com/data/v1"] }
...
# POST /{apiKey}/assets/tokens/by-address
export def "assets-tokens-by-address post" [
  apiKey: any
...
export def "assets-tokens-balances-by-address post" [
  apiKey: any
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
export def "assets-nfts-by-address post" [
  apiKey: any
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
export def "assets-nfts-contracts-by-address post" [
  apiKey: any
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
export def "transactions-history-by-address post" [
  apiKey: any
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
#
# POST /{apiKey}/assets/nfts
# operationId: get-nfts-by-address-placeholder
# --addresses item shape: {address: string, networks: list}
export def "assets-nfts get-nfts-by-address-placeholder" [
  apiKey: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/($apiKey)/assets/nfts")
...
#
# POST /{apiKey}/assets/tokens
# operationId: get-tokens-by-address-placeholder
# --addresses item shape: {address: string, networks: list}
export def "assets-tokens get-tokens-by-address-placeholder" [
  apiKey: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/($apiKey)/assets/tokens")
  let body = {addresses: $addresses} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
#
# POST /{apiKey}/assets/prices
# operationId: get-tokens-prices-by-address-placeholder
# --addresses item shape: {address: string, networks: list}
export def "assets-prices get-tokens-prices-by-address-placeholder" [
  apiKey: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/($apiKey)/assets/prices")
  let body = {addresses: $addresses} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
#
# POST /{apiKey}/assets/transfers
# operationId: get-transfers-by-address-placeholder
# --addresses item shape: {address: string, networks: list}
export def "assets-transfers get-transfers-by-address-placeholder" [
  apiKey: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/($apiKey)/assets/transfers")
  let body = {addresses: $addresses} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
# Blocks by Timestamp
#
# GET /{apiKey}/utility/blocks/by-timestamp
# operationId: blocks-by-timestamp
export def "utility-blocks-by-timestamp blocks-by-timestamp" [
  apiKey: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
]: nothing -> record<data: table<network: string, block: record>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "networks" $networks "multi") (serialize-qp "timestamp" $timestamp "scalar") (serialize-qp "direction" $direction "scalar")] | flatten | str join "&"
...
#
# POST /{apiKey}/subgraphs
# operationId: subgraphs-placeholder
# --addresses item shape: {address: string, networks: list}
export def "subgraphs subgraphs-placeholder" [
  apiKey: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/($apiKey)/subgraphs")
  let body = {addresses: $addresses} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
#
# POST /{apiKey}/assets/webhooks
# operationId: webhooks-placeholder
# --addresses item shape: {address: string, networks: list}
export def "assets-webhooks webhooks-placeholder" [
  apiKey: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/($apiKey)/assets/webhooks")
  let body = {addresses: $addresses} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
