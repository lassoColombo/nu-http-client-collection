...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AMAZON_SAGEMAKER_RUNTIME_TOKEN | default "" }
...
export def "endpoints-invocations create-invoke" [
  ...
  let full_url = (build-url $base ({endpoint_name: (encode-path-segment $endpoint_name)} | format pattern "/endpoints/{endpoint_name}/invocations") $auth.query)
...
export def "endpoints-async-invocations create-invoke" [
  endpoint_name: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let base = ($base_url | default $BASE_URL)
  if ($endpoint_name | is-empty) { error make --unspanned { msg: "path parameter 'EndpointName' must be non-empty" } }
  let full_url = (build-url $base ({endpoint_name: (encode-path-segment $endpoint_name)} | format pattern "/endpoints/{endpoint_name}/async-invocations") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
