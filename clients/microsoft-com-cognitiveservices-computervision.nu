...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o COMPUTER_VISION_CLIENT_TOKEN | default "" }
...
export def "analyze create-image" [
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"visualFeatures": $visual_features, "details": $details, "language": $language, "descriptionExclude": $description_exclude} | compact)
...
export def "area-of-interest get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "ocp-apim-subscription-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/areaOfInterest" $auth.query)
  let req_body = {"url": $url} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "describe get-image" [
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"maxCandidates": $max_candidates, "language": $language, "descriptionExclude": $description_exclude} | compact)
    headers: $auth.headers
    body: $req_body
    content_type: "application/json"
    timeout: ($max_time | default 30min)
