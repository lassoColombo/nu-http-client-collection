# Auto-generated client for Game Services API vv1
# Source: https://api.apis.guru/v2/specs/googleapis.com/gameservices/v1/openapi.json
# Auth: --token flag or $env.GAME_SERVICES_API_TOKEN

const BASE_URL = "https://gameservices.googleapis.com"
const DEFAULT_AUTH = "bearer"

# Build auth: returns {headers: record, query: string}
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o GAME_SERVICES_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://gameservices.googleapis.com"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "projects gameservicesprojectslocationsoperationsdelete" } } | get name | first)
...
export def "projects gameservicesprojectslocationsoperationsdelete" [
  name: string
...
export def "projects gameservicesprojectslocationsoperationsget" [
  name: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
export def "locations gameservicesprojectslocationslist" [
  name: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
export def "operations gameservicesprojectslocationsoperationslist" [
  name: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
export def "projects gameservicesprojectslocationsoperationscancel" [
  name: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
export def "projects gameservicesprojectslocationsgameServerDeploymentsgetIamPolicy" [
  resource: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
]: nothing -> record<auditConfigs: table<auditLogConfigs: list, service: string>, bindings: table<bindingId: string, condition: record, members: list, role: string>, etag: string, rules: table<action: string, conditions: list, description: string, in: list, logConfig: list, notIn: list, permissions: list>, version: int> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
...
export def "projects gameservicesprojectslocationsgameServerDeploymentssetIamPolicy" [
  resource: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
]: any -> record<auditConfigs: table<auditLogConfigs: list, service: string>, bindings: table<bindingId: string, condition: record, members: list, role: string>, etag: string, rules: table<action: string, conditions: list, description: string, in: list, logConfig: list, notIn: list, permissions: list>, version: int> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "$.xgafv" $xgafv "scalar") (serialize-qp "access_token" $access_token "scalar") (serialize-qp "alt" $alt "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "fields" $qp_fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $prettyPrint "scalar") (serialize-qp "quotaUser" $quotaUser "scalar") (serialize-qp "upload_protocol" $upload_protocol "scalar") (serialize-qp "uploadType" $uploadType "scalar")] | flatten | str join "&"
...
export def "projects gameservicesprojectslocationsgameServerDeploymentstestIamPermissions" [
  resource: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
