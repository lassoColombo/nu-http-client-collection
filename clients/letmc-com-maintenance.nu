...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AGENTOS_API_V3_MAINTENANCE_CALL_GROUP_TOKEN | default "" }
...
export def "maintenance-maintenance-create-maintenancejob create-controller-job" [
  ...
  let full_url = (build-url $base ({short_name: (encode-path-segment $short_name), branch_id: (encode-path-segment $branch_id)} | format pattern "/v3/maintenance/{short_name}/maintenance/{branch_id}/createmaintenancejob") $auth.query)
