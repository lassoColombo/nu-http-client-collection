...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o IP2PROXY_PROXY_DETECTION_TOKEN | default "" }
...
export def "api get" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"package": $package, "ip": $ip, "format": $format, "key": $key} | compact)
