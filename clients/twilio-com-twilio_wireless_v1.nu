...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o TWILIO_WIRELESS_TOKEN | default "" }
...
export def "commands list" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"Sim": $sim, "Status": $status, "Direction": $direction, "Transport": $transport, "PageSize": $page_size, "Page": $page, "PageToken": $page_token} | compact)
...
export def "commands create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://wireless.twilio.com")
  let full_url = (build-url $base "/v1/Commands" $auth.query)
...
export def "commands delete" [
  ...
  let full_url = (build-url $base ({sid: (encode-path-segment $sid)} | format pattern "/v1/Commands/{sid}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "rate-plans list" [
  ...
  let full_url = (build-url $base "/v1/RatePlans" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "rate-plans create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://wireless.twilio.com")
  let full_url = (build-url $base "/v1/RatePlans" $auth.query)
...
export def "rate-plans delete" [
  ...
  let base = ($base_url | default "https://wireless.twilio.com")
  if ($sid | is-empty) { error make --unspanned { msg: "path parameter 'Sid' must be non-empty" } }
  let full_url = (build-url $base ({sid: (encode-path-segment $sid)} | format pattern "/v1/RatePlans/{sid}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "sims-data-sessions list" [
  ...
  let qp = [(serialize-qp "PageSize" $page_size "scalar") (serialize-qp "Page" $page "scalar") (serialize-qp "PageToken" $page_token "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({sim_sid: (encode-path-segment $sim_sid)} | format pattern "/v1/Sims/{sim_sid}/DataSessions") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "sims-usage-records list" [
  ...
  let full_url = (build-url $base ({sim_sid: (encode-path-segment $sim_sid)} | format pattern "/v1/Sims/{sim_sid}/UsageRecords") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"End": $end, "Start": $start, "Granularity": $granularity, "PageSize": $page_size, "Page": $page, "PageToken": $page_token} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
