...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o GLOBALWINESCORE_API_DOCUMENTATION_TOKEN | default "" }
...
export def "globalwinescores list-historical-gws" [
  ...
  let full_url = (build-url $base "/globalwinescores/" $qp $auth.query)
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"wine_id": $wine_id, "vintage": $vintage, "color": $color, "is_primeurs": $is_primeurs, "lwin": $lwin, "lwin_11": $lwin_11, "limit": $limit, "offset": $offset, "ordering": $ordering} | compact)
...
export def "globalwinescores-latest get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "wine_id" $wine_id "multi") (serialize-qp "vintage" $vintage "scalar") (serialize-qp "color" $color "scalar") (serialize-qp "is_primeurs" $is_primeurs "scalar") (serialize-qp "lwin" $lwin "scalar") (serialize-qp "lwin_11" $lwin_11 "scalar") (serialize-qp "limit" $limit "scalar") (serialize-qp "offset" $offset "scalar") (serialize-qp "ordering" $ordering "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/globalwinescores/latest/" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Authorization": $authorization} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
