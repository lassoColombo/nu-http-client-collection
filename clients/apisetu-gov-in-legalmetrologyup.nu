...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "delcs-certificate delcs" } } | get name | first)
...
export def "delcs-certificate delcs" [
  ...
  --certificateParameters: record # shape: {applicationNo: string, licenseNo: string}
...
# --certificateParameters shape: {applicationNo: string, licenseNo: string}
# --consentArtifact shape: {consent: record, signature: record}
export def "palcs-certificate palcs" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
...
# --certificateParameters shape: {applicationNo: string, licenseNo: string}
# --consentArtifact shape: {consent: record, signature: record}
export def "relcs-certificate relcs" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
