...
export def "subscriptions-resourcegroups-providers-microsoft-storage-admin-farms-queueservices Get" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourcegroups/($resourceGroupName)/providers/Microsoft.Storage.Admin/farms/($farmId)/queueservices/($serviceType)" $qp)
...
export def "subscriptions-resourcegroups-providers-microsoft-storage-admin-farms-queueservices-metricdefinitions ListMetricDefinitions" [
  subscriptionId: string
  resourceGroupName: string
  farmId: string
  serviceType: string
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourcegroups/($resourceGroupName)/providers/Microsoft.Storage.Admin/farms/($farmId)/queueservices/($serviceType)/metricdefinitions" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "subscriptions-resourcegroups-providers-microsoft-storage-admin-farms-queueservices-metrics ListMetrics" [
  subscriptionId: string
  resourceGroupName: string
  farmId: string
  serviceType: string
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourcegroups/($resourceGroupName)/providers/Microsoft.Storage.Admin/farms/($farmId)/queueservices/($serviceType)/metrics" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
