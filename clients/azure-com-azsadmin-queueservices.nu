...
export def "subscriptions-resourcegroups-providers-microsoft-storage-admin-farms-queueservices-metricdefinitions list-queue-services-metric-definitions" [
  ...
  if ($service_type | is-empty) { error make --unspanned { msg: "path parameter 'serviceType' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), farm_id: (encode-path-segment $farm_id), service_type: (encode-path-segment $service_type)} | format pattern "/subscriptions/{subscription_id}/resourcegroups/{resource_group_name}/providers/Microsoft.Storage.Admin/farms/{farm_id}/queueservices/{service_type}/metricdefinitions") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "subscriptions-resourcegroups-providers-microsoft-storage-admin-farms-queueservices-metrics list-queue-services" [
  ...
  if ($service_type | is-empty) { error make --unspanned { msg: "path parameter 'serviceType' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: (encode-path-segment $subscription_id), resource_group_name: (encode-path-segment $resource_group_name), farm_id: (encode-path-segment $farm_id), service_type: (encode-path-segment $service_type)} | format pattern "/subscriptions/{subscription_id}/resourcegroups/{resource_group_name}/providers/Microsoft.Storage.Admin/farms/{farm_id}/queueservices/{service_type}/metrics") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
