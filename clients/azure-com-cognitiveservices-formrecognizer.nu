...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o FORM_RECOGNIZER_CLIENT_TOKEN | default "" }
...
export def "custom-models list" [
  ...
  let full_url = (build-url $base "/custom/models" $qp $auth.query)
...
export def "custom-models create-train-async" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "ocp-apim-subscription-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/custom/models" $auth.query)
...
export def "custom-models delete" [
  ...
  let full_url = (build-url $base ({model_id: (encode-path-segment $model_id)} | format pattern "/custom/models/{model_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "custom-models get" [
  ...
  let full_url = (build-url $base ({model_id: (encode-path-segment $model_id)} | format pattern "/custom/models/{model_id}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"includeKeys": $include_keys} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "custom-models-analyze create" [
  ...
  let full_url = (build-url $base ({model_id: (encode-path-segment $model_id)} | format pattern "/custom/models/{model_id}/analyze") $qp $auth.query)
  let req_body = $body
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
    url: $full_url
    query: ({"includeTextDetails": $include_text_details} | compact)
    headers: $auth.headers
    body: $req_body
    content_type: "application/json"
    timeout: ($max_time | default 30min)
...
export def "custom-models-analyze-results get-form" [
  ...
  let full_url = (build-url $base ({model_id: (encode-path-segment $model_id), result_id: (encode-path-segment $result_id)} | format pattern "/custom/models/{model_id}/analyzeResults/{result_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "layout-analyze create-async" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "ocp-apim-subscription-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/layout/analyze" $auth.query)
  let req_body = $body
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "layout-analyze-results get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($result_id | is-empty) { error make --unspanned { msg: "path parameter 'resultId' must be non-empty" } }
  let full_url = (build-url $base ({result_id: (encode-path-segment $result_id)} | format pattern "/layout/analyzeResults/{result_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "prebuilt-receipt-analyze create-async" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "includeTextDetails" $include_text_details "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/prebuilt/receipt/analyze" $qp $auth.query)
  let req_body = $body
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "prebuilt-receipt-analyze-results get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($result_id | is-empty) { error make --unspanned { msg: "path parameter 'resultId' must be non-empty" } }
  let full_url = (build-url $base ({result_id: (encode-path-segment $result_id)} | format pattern "/prebuilt/receipt/analyzeResults/{result_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
