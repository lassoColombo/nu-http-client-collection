# Auto-generated client for Form Recognizer Client v2.0-preview
# Source: https://api.apis.guru/v2/specs/azure.com/cognitiveservices-FormRecognizer/2.0-preview/swagger.json
# Auth: --token flag or $env.FORM_RECOGNIZER_CLIENT_TOKEN

const BASE_URL = "{endpoint}/formrecognizer/v2.0-preview"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o FORM_RECOGNIZER_CLIENT_TOKEN | default "" }
...
def base-url-completer [] { ["{endpoint}/formrecognizer/v2.0-preview"] }
def auth-scheme-completer [] { ["ocp-apim-subscription-key"] }

# Completers for enum parameters
def op-completer [] { ["full" "summary"] }
...
export def "custom-models list" [
  ...
  --op: string@op-completer # Specify whether to return summary or full list of models. (default: full)
]: nothing -> record<modelList: table<createdDateTime: string, lastUpdatedDateTime: string, modelId: string, status: string>, nextLink: string, summary: record<count: int, lastUpdatedDateTime: string, limit: int>> {
  let auth = (build-auth $token ($auth_scheme | default "ocp-apim-subscription-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "op" $op "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/custom/models" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"op": $op} | compact), body: null}
}
...
export def "custom-models create-train-async" [
  ...
  let req_body = {"source": $body_source, "sourceFilter": $source_filter, "useLabelFile": $use_label_file} | compact
...
export def "custom-models delete" [
  ...
  let full_url = (build-url $base ({model_id: (encode-path-segment $model_id)} | format pattern "/custom/models/{model_id}"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "custom-models get" [
  ...
  let full_url = (build-url $base ({model_id: (encode-path-segment $model_id)} | format pattern "/custom/models/{model_id}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"includeKeys": $include_keys} | compact), body: null}
}
...
export def "custom-models-analyze create" [
  ...
  let full_url = (build-url $base ({model_id: (encode-path-segment $model_id)} | format pattern "/custom/models/{model_id}/analyze") $qp)
  let req_body = $body
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"includeTextDetails": $include_text_details} | compact), body: $req_body}
}
...
export def "custom-models-analyze-results get-form" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($model_id | is-empty) { error make --unspanned { msg: "path parameter 'modelId' must be non-empty" } }
  if ($result_id | is-empty) { error make --unspanned { msg: "path parameter 'resultId' must be non-empty" } }
  let full_url = (build-url $base ({model_id: (encode-path-segment $model_id), result_id: (encode-path-segment $result_id)} | format pattern "/custom/models/{model_id}/analyzeResults/{result_id}"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "layout-analyze create-async" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "ocp-apim-subscription-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/layout/analyze")
  let req_body = $body
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "layout-analyze-results get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($result_id | is-empty) { error make --unspanned { msg: "path parameter 'resultId' must be non-empty" } }
  let full_url = (build-url $base ({result_id: (encode-path-segment $result_id)} | format pattern "/layout/analyzeResults/{result_id}"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
...
export def "prebuilt-receipt-analyze-results get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($result_id | is-empty) { error make --unspanned { msg: "path parameter 'resultId' must be non-empty" } }
  let full_url = (build-url $base ({result_id: (encode-path-segment $result_id)} | format pattern "/prebuilt/receipt/analyzeResults/{result_id}"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
