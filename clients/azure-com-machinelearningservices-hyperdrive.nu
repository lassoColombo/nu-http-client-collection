# Auto-generated client for HyperDrive v2019-09-30
# Source: https://api.apis.guru/v2/specs/azure.com/machinelearningservices-hyperdrive/2019-09-30/swagger.json
# Auth: --token flag or $env.HYPERDRIVE_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o HYPERDRIVE_TOKEN | default "" }
...
export def "hyperdrive-v1-0-runs create-hyperparameter-tuning-experiment" [
  ...
  config: path # The configuration file with experiment JSON content. A text file that is a JSON-serialized '#/definitions/HyperDriveCreateExperiment' object.
]: any -> record<code: int, result: record> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($arm_scope | is-empty) { error make --unspanned { msg: "path parameter 'armScope' must be non-empty" } }
  let full_url = (build-url $base ({arm_scope: (encode-path-segment $arm_scope)} | format pattern "/hyperdrive/v1.0/{arm_scope}/runs") $auth.query)
  let req_body = {"config": $config} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req_body = if ($req_body | describe | str starts-with "record") { $req_body } else { {} }
  let mp = (build-multipart-body $req_body ["config"] $dry_run)
  let req = {
    method: "post"
    url: $full_url
    query: {}
...
export def "hyperdrive-v1-0-runs-cancel cancel-hyperparameter-tuning-experiment" [
  ...
  let full_url = (build-url $base ({arm_scope: (encode-path-segment $arm_scope), run_id: (encode-path-segment $run_id)} | format pattern "/hyperdrive/v1.0/{arm_scope}/runs/{run_id}/cancel") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
