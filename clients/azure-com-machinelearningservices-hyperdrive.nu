...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o HYPERDRIVE_TOKEN | default "" }
...
export def "hyperdrive-v1-0-runs create-hyperparameter-tuning-experiment" [
  ...
  let full_url = (build-url $base ({arm_scope: (encode-path-segment $arm_scope)} | format pattern "/hyperdrive/v1.0/{arm_scope}/runs"))
...
export def "hyperdrive-v1-0-runs-cancel cancel-hyperparameter-tuning-experiment" [
  ...
  let full_url = (build-url $base ({arm_scope: (encode-path-segment $arm_scope), run_id: (encode-path-segment $run_id)} | format pattern "/hyperdrive/v1.0/{arm_scope}/runs/{run_id}/cancel"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
