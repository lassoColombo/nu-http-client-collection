...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o VISUAL_SEARCH_CLIENT_TOKEN | default "" }
...
export def "images-visualsearch list-visual" [
  ...
  let full_url = (build-url $base "/images/visualsearch" $qp $auth.query)
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"mkt": $mkt, "safeSearch": $safe_search, "setLang": $set_lang} | compact)
