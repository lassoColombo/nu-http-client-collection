...
export def "fine-tuning-jobs job" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
  --dry-run: string # * If `true` the job is not spawned, instead the query returns a handful of useful metadata   for the user to perform sanity checks (see `LegacyJobMetadataOut` response). * Otherwise, the job is started and the query returns the job ID along with some of the   input parameters (see `JobOut` response).
  model: string
...
export def "connectors v1-1" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
  --query-filters: string # default: {fetch_user_data: false, fetch_customer_data: false, fetch_connection_secrets: false, fetch_execution_data: false}
  --cursor: string
  --page-size: int # default: 100
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "query_filters" $query_filters "scalar") (serialize-qp "cursor" $cursor "scalar") (serialize-qp "page_size" $page_size "scalar")] | flatten | str join "&"
