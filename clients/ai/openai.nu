...
# Source: /home/runner/work/nu-http-client-collection/nu-http-client-collection/specs/openai.yaml
...
export def "chat-completions createChatCompletion" [
  ...
  --store: oneof<nothing, bool> # Whether or not to store the output of this chat completion request for use in our [model distillation](/docs/guides/distillation) or [evals](/docs/guides/evals) products.  Supports text and image inputs. Note: image inputs over 8MB will be dropped.  (nullable, default: false)
  ...
  --logprobs: oneof<nothing, bool> # Whether to return log probabilities of the output tokens or not. If true, returns the log probabilities of each output token returned in the `content` of `message`.  (nullable, default: false)
  ...
  --parallel-tool-calls: oneof<nothing, bool> # Whether to enable [parallel function calling](/docs/guides/function-calling#configuring-parallel-function-calling) during tool use. (default: true)
...
export def "images-generations createImage" [
  ...
  --stream: oneof<nothing, bool> # Generate the image in streaming mode. Defaults to `false`. See the [Image generation guide](/docs/guides/image-generation) for more information. This parameter is only supported for the GPT image models.  (nullable, default: false, e.g. false)
  --partial-images: any
...
export def "organization-usage-completions usage-completions" [
  ...
  --api-key-ids: list # Return only usage for these API keys.
  --models: list # Return only usage for these models.
  --batch: oneof<nothing, bool> # If `true`, return batch jobs only. If `false`, return non-batch jobs only. By default, return both.
...
export def "realtime-calls-accept accept-realtime-call" [
  ...
  --parallel-tool-calls: oneof<nothing, bool> # Whether the model may call multiple tools in parallel. Only supported by reasoning Realtime models such as `gpt-realtime-2`.
...
export def "responses get" [
  ...
  --include-obfuscation: oneof<nothing, bool> # When true, stream obfuscation will be enabled. Stream obfuscation adds random characters to an `obfuscation` field on streaming delta events to normalize payload sizes as a mitigation to certain side-channel attacks. These obfuscation fields are included by default, but add a small amount of overhead to the data stream. You can set `include_obfuscation` to false to optimize for bandwidth if you trust the network links between your application and the OpenAI API.
...
export def "threads-runs createThreadAndRun" [
  ...
  --top-p: float # An alternative to sampling with temperature, called nucleus sampling, where the model considers the results of the tokens with top_p probability mass. So 0.1 means only the tokens comprising the top 10% probability mass are considered.  We generally recommend altering this or temperature but not both.  (nullable, default: 1, e.g. 1)
  --stream: oneof<nothing, bool> # If `true`, returns a stream of events that happen during the Run as server-sent events, terminating when the Run enters a terminal state with a `data: [DONE]` message.  (nullable)
