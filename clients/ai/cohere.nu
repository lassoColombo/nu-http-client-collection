...
export def "chat chat-stream" [
  ...
  --logprobs: oneof<nothing, bool> # Defaults to `false`. When set to `true`, the log probabilities of the generated tokens will be included in the response.
...
export def "datasets create" [
  ...
  --skip-malformed-input: oneof<nothing, bool> # Indicates whether rows with malformed input should be dropped (instead of failing the validation check). Dropped rows will be returned in the warnings field.
...
export def "models list" [
  ...
  --default-only: oneof<nothing, bool> # When provided, filters the list of models to only the default model to the endpoint. This parameter is only valid when `endpoint` is provided.
  --Authorization: string # Bearer authentication
...
export def "chat chat-stream-1" [
  ...
  --raw-prompting: oneof<nothing, bool> # When enabled, the user's prompt will be sent to the model without any pre-processing.  Compatible Deployments: Cohere Platform, Azure, AWS Sagemaker/Bedrock, Private Deployments
  ...
  --force-single-step: oneof<nothing, bool> # Forces the chat to be single step. Defaults to `false`.
...
export def "connectors create" [
  ...
  --active: oneof<nothing, bool> # Whether the connector is active or not. (default: true)
  --continue-on-failure: oneof<nothing, bool> # Whether a chat request should continue or not if the request to this connector fails. (default: false)
...
export def "connectors update" [
  ...
  --continue-on-failure: oneof<nothing, bool> # default: false
  --service-auth: record # shape: {type: "bearer"|"basic"|"noscheme", token: string}
]: any -> record<connector: record<id: string, organization_id: string, name: string, description: string, url: string, created_at: string, updated_at: string, excludes: list<string>, auth_type: string, oauth: record<client_id: string, client_secret: string, authorize_url: string, token_url: string, scope: string>, auth_status: string, active: bool, continue_on_failure: bool>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
...
export def "generate generate-stream" [
  ...
  --raw-prompting: oneof<nothing, bool> # When enabled, the user's prompt will be sent to the model without any pre-processing.
]: any -> any {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
