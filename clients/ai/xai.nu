...
export def "chat-completions request" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
  --deferred: oneof<nothing, bool> # If set to `true`, the request returns a `request_id`. You can then get the deferred response by GET `/v1/chat/deferred-completion/{request_id}`. (nullable, default: false)
  ...
  --logprobs: oneof<nothing, bool> # Whether to return log probabilities of the output tokens or not. If true, returns the log probabilities of each output token returned in the content of message. Not supported by models `grok-4.20` and newer; the field will be silently ignored if set. (nullable, default: false)
  ...
  --parallel-tool-calls: oneof<nothing, bool> # If set to false, the model can perform maximum one tool call. (nullable, default: true, e.g. false)
  ...
  --stream: oneof<nothing, bool> # If set, partial message deltas will be sent. Tokens will be sent as data-only server-sent events as they become available, with the stream terminated by a `data: [DONE]` message. (nullable, default: false, e.g. true)
  --stream-options: any
...
export def "complete request" [
  ...
  --stream: oneof<nothing, bool> # (Unsupported) If set, partial message deltas will be sent. Tokens will be sent as data-only server-sent events as they become available, with the stream terminated by a `data: [DONE]` message. (nullable)
  --temperature: float # What sampling temperature to use, between 0 and 2. Higher values like 0.8 will make the output more random, while lower values like 0.2 will make it more focused and deterministic. (nullable, format: float, default: 1, e.g. 0.2)
...
export def "completions request" [
  ...
  --echo: oneof<nothing, bool> # Option to include the original prompt in the response along with the generated completion. (nullable)
  ...
  --seed: int # If specified, our system will make a best effort to sample deterministically, such that repeated requests with the same seed and parameters should return the same result. Determinism is not guaranteed, and you should refer to the system_fingerprint response parameter to monitor changes in the backend. (nullable, format: int32)
  --stop: list # (Not supported by reasoning models) Up to 4 sequences where the API will stop generating further tokens. (nullable)
  --stream: oneof<nothing, bool> # Whether to stream back partial progress. If set, tokens will be sent as data-only server-sent events as they become available, with the stream terminated by a `data: [DONE]` message. (nullable)
  --stream-options: any
...
export def "embeddings request" [
  ...
  --preview: oneof<nothing, bool> # Flag to use the new format of the API. (nullable)
  --user: string # A unique identifier representing your end-user, which can help xAI to monitor and detect abuse. (nullable)
...
export def "messages request" [
  ...
  --stop-sequences: list # (Not supported by reasoning models) Up to 4 sequences where the API will stop generating further tokens. (nullable)
  --stream: oneof<nothing, bool> # If set, partial message deltas will be sent. Tokens will be sent as data-only server-sent events as they become available, with the stream terminated by a `data: [DONE]` message. (nullable)
  --system: any
...
export def "responses request" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
  --background: oneof<nothing, bool> # (Unsupported) Whether to process the response asynchronously in the background. (nullable, default: false)
  ...
  --model: string # Model name for the model to use. Obtainable from <https://console.x.ai/team/default/models> or <https://docs.x.ai/docs/models>. (e.g. latest)
  --parallel-tool-calls: oneof<nothing, bool> # Whether to allow the model to run parallel tool calls. (nullable, default: true)
  ...
  --search-parameters: any
  --service-tier: any
  --store: oneof<nothing, bool> # Whether to store the input message(s) and model response for later retrieval. (nullable, default: true)
  --stream: oneof<nothing, bool> # If set, partial message deltas will be sent. Tokens will be sent as data-only server-sent events as they become available, with the stream terminated by a `data: [DONE]` message. (nullable, default: false, e.g. true)
  --temperature: float # What sampling temperature to use, between 0 and 2. Higher values like 0.8 will make the output more random, while lower values like 0.2 will make it more focused and deterministic. (nullable, format: float, default: 1, e.g. 0.2)
  --text: any
  --tool-choice: any
