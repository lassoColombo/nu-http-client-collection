# Auto-generated client for InstanceMetadataClient v2019-08-15
# Source: https://api.apis.guru/v2/specs/azure.com/imds/2019-08-15/swagger.json
# Auth: --token flag or $env.INSTANCEMETADATACLIENT_TOKEN

const BASE_URL = "http://169.254.169.254/metadata"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o INSTANCEMETADATACLIENT_TOKEN | default "" }
...
export def "attested-document get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version, "nonce": $nonce} | compact), body: null}
}
...
export def "identity-oauth2-token get" [
  ...
  --api-version: string@api-version-completer # This is the API version to use.
  --client-id: string # This identifies, by Azure AD client id, a specific explicit identity to use when authenticating to Azure AD. Mutually exclusive with object_id and msi_res_id.
  --object-id: string # This identifies, by Azure AD object id, a specific explicit identity to use when authenticating to Azure AD. Mutually exclusive with client_id and msi_res_id.
  ...
  let extra_headers = {"Metadata": $metadata} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"resource": $resource, "api-version": $api_version, "client_id": $client_id, "object_id": $object_id, "msi_res_id": $msi_res_id, "authority": $authority, "bypass_cache": $bypass_cache} | compact), body: null}
}
