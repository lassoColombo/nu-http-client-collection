...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o INSTANCEMETADATACLIENT_TOKEN | default "" }
...
export def "attested-document get" [
  ...
  let full_url = (build-url $base "/attested/document" $qp $auth.query)
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"api-version": $api_version, "nonce": $nonce} | compact)
...
export def "identity-info get" [
  ...
  let full_url = (build-url $base "/identity/info" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Metadata": $metadata} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "identity-oauth2-token get" [
  ...
  let full_url = (build-url $base "/identity/oauth2/token" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Metadata": $metadata} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "get"
    url: $full_url
    query: ({"resource": $resource, "api-version": $api_version, "client_id": $client_id, "object_id": $object_id, "msi_res_id": $msi_res_id, "authority": $authority, "bypass_cache": $bypass_cache} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
