...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AMAZON_KINESIS_VIDEO_SIGNALING_CHANNELS_TOKEN | default "" }
...
export def "get-ice-server-config get" [
  ...
  let full_url = (build-url $base "/v1/get-ice-server-config" $auth.query)
...
export def "send-alexa-offer-to-master send" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/v1/send-alexa-offer-to-master" $auth.query)
