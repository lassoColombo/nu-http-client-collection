...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o WRITTEN_QUESTIONS_SERVICE_API_TOKEN | default "" }
...
export def "dailyreports-dailyreports get" [
  ...
  let full_url = (build-url $base "/api/dailyreports/dailyreports" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"dateFrom": $date_from, "dateTo": $date_to, "house": $house, "skip": $skip, "take": $take} | compact)
...
export def "writtenquestions-questions get-by-date-uin" [
  ...
  let full_url = (build-url $base ({date: (encode-path-segment $date), uin: (encode-path-segment $uin)} | format pattern "/api/writtenquestions/questions/{date}/{uin}") $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"expandMember": $expand_member} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "writtenquestions-questions get-by-id" [
  ...
  let qp = [(serialize-qp "expandMember" $expand_member "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/api/writtenquestions/questions/{id}") $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "writtenstatements-statements get" [
  ...
  let full_url = (build-url $base "/api/writtenstatements/statements" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"madeWhenFrom": $made_when_from, "madeWhenTo": $made_when_to, "searchTerm": $search_term, "uIN": $u_in, "answeringBodies": $answering_bodies, "members": $members, "house": $house, "skip": $skip, "take": $take, "expandMember": $expand_member} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "writtenstatements-statements get-by-date-uin" [
  ...
  if ($uin | is-empty) { error make --unspanned { msg: "path parameter 'uin' must be non-empty" } }
  let qp = [(serialize-qp "expandMember" $expand_member "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({date: (encode-path-segment $date), uin: (encode-path-segment $uin)} | format pattern "/api/writtenstatements/statements/{date}/{uin}") $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "writtenstatements-statements get-by-id" [
  ...
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let qp = [(serialize-qp "expandMember" $expand_member "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/api/writtenstatements/statements/{id}") $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
