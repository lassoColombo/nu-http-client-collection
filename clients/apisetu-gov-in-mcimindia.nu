# Auto-generated client for Maharashtra Council of Indian Medicine v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/mcimindia/3.0.0/openapi.json
# Auth: --token flag or $env.MAHARASHTRA_COUNCIL_OF_INDIAN_MEDICINE_TOKEN

const BASE_URL = "https://apisetu.gov.in/mcimindia/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o MAHARASHTRA_COUNCIL_OF_INDIAN_MEDICINE_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/mcimindia/v3"] }
...
# operationId: govid
# --certificateParameters shape: {DOB: string, FullName: string, REGN_NO: string, UID: string}
...
export def "govid-certificate create" [
  ...
  --certificate-parameters: record # shape: {DOB: string, FullName: string, REGN_NO: string, UID: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o MAHARASHTRA_COUNCIL_OF_INDIAN_MEDICINE_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o MAHARASHTRA_COUNCIL_OF_INDIAN_MEDICINE_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
