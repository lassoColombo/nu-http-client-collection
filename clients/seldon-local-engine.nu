...
export def "seldon-v1-0-feedback send" [
  ...
  let full_url = (build-url $base ({namespace: (encode-path-segment $namespace), deployment: (encode-path-segment $deployment)} | format pattern "/seldon/{namespace}/{deployment}/api/v1.0/feedback") $auth.query)
...
export def "seldon-v1-0-predictions create-predict" [
  ...
  if ($namespace | is-empty) { error make --unspanned { msg: "path parameter 'namespace' must be non-empty" } }
  if ($deployment | is-empty) { error make --unspanned { msg: "path parameter 'deployment' must be non-empty" } }
  let full_url = (build-url $base ({namespace: (encode-path-segment $namespace), deployment: (encode-path-segment $deployment)} | format pattern "/seldon/{namespace}/{deployment}/api/v1.0/predictions") $auth.query)
