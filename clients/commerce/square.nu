...
export def "oauth2-revoke RevokeToken" [
  ...
  --revoke-only-access-token: string@bool-completer # If `true`, terminate the given single access token, but do not terminate the entire authorization. Default: `false` (nullable)
...
export def "oauth2-token ObtainToken" [
  ...
  --short-lived: string@bool-completer # Indicates whether the returned access token should expire in 24 hours.  Optional for the code flow and PKCE flow for any grant type. The default value is `false`. (nullable)
  ...
  --use-jwt: string@bool-completer # Indicates whether to use a JWT (JSON Web Token) as the OAuth access token. When set to `true`, the OAuth flow returns a JWT to your application, used in the same way as a regular token. The default value is `false`. (nullable)
...
export def "bookings-team-member-booking-profiles ListTeamMemberBookingProfiles" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
  --bookable-only: string@bool-completer # Indicates whether to include only bookable team members in the returned result (`true`) or not (`false`). (default: false)
  --limit: int # The maximum number of results to return in a paged response.
  --cursor: string # The pagination cursor from the preceding response to return the next page of the results. Do not set this when retrieving the first page of the results.
...
export def "bookings-custom-attributes RetrieveBookingCustomAttribute" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
  --with-definition: string@bool-completer # Indicates whether to return the [custom attribute definition](entity:CustomAttributeDefinition) in the `definition` field of the custom attribute. Set this parameter to `true` to get the name and description of the custom attribute, information about the data type, or other definition details. The default value is `false`. (default: false)
...
export def "catalog-batch-retrieve BatchRetrieveCatalogObjects" [
  ...
  --include-deleted-objects: string@bool-completer # Indicates whether to include (`true`) or not (`false`) in the response deleted objects, namely, those with the `is_deleted` attribute set to `true`. (nullable)
  --include-category-path-to-root: string@bool-completer # Specifies whether or not to include the `path_to_root` list for each returned category instance. The `path_to_root` list consists of `CategoryPathToRootNode` objects and specifies the path that starts with the immediate parent category of the returned category and ends with its root category. If the returned category is a top-level category, the `path_to_root` list is empty and is not returned in the response payload. (nullable)
...
export def "catalog-search SearchCatalogObjects" [
  ...
  --include-deleted-objects: string@bool-completer # If `true`, deleted objects will be included in the results. Defaults to `false`. Deleted objects will have their `is_deleted` field set to `true`. If `include_deleted_objects` is `true`, then the `include_category_path_to_root` request parameter must be `false`. Both properties cannot be `true` at the same time.
...
export def "customers-search SearchCustomers" [
  ...
  --count: string@bool-completer # Indicates whether to return the total count of matching customers in the `count` field of the response.  The default value is `false`.
...
export def "inventory-batch-change DeprecatedBatchChangeInventory" [
  ...
  --ignore-unchanged-counts: string@bool-completer # Indicates whether the current physical count should be ignored if the quantity is unchanged since the last physical count. Default: `true`. (nullable)
...
export def "locations-checkouts CreateCheckout" [
  ...
  --ask-for-shipping-address: string@bool-completer # If `true`, Square Checkout collects shipping information on your behalf and stores  that information with the transaction information in the Square Seller Dashboard.  Default: `false`.
...
export def "orders-search SearchOrders" [
  ...
  --return-entries: string@bool-completer # A Boolean that controls the format of the search results. If `true`, `SearchOrders` returns [OrderEntry](entity:OrderEntry) objects. If `false`, `SearchOrders` returns complete order objects.  Default: `false`.
...
export def "orders-custom-attributes RetrieveOrderCustomAttribute" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --version: int # To enable [optimistic concurrency](https://developer.squareup.com/docs/build-basics/common-api-patterns/optimistic-concurrency) control, include this optional field and specify the current version of the custom attribute.
  --with-definition: string@bool-completer # Indicates whether to return the [custom attribute definition](entity:CustomAttributeDefinition) in the `definition` field of each  custom attribute. Set this parameter to `true` to get the name and description of each custom attribute,  information about the data type, or other definition details. The default value is `false`. (default: false)
]: nothing -> record<custom_attribute: record<key: string, value: any, version: int, visibility: string, definition: record<key: string, schema: record, name: string, description: string, visibility: string, version: int, updated_at: string, created_at: string>, updated_at: string, created_at: string>, errors: table<category: string, code: string, detail: string, field: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
...
export def "payments ListPayments" [
  ...
  --is-offline-payment: string@bool-completer # Whether the payment was taken offline or not. (default: false)
...
export def "payments CreatePayment" [
  ...
  --autocomplete: string@bool-completer # If set to `true`, this payment will be completed when possible. If set to `false`, this payment is held in an approved state until either explicitly completed (captured) or canceled (voided). For more information, see [Delayed capture](https://developer.squareup.com/docs/payments-api/take-payments/card-payments#delayed-capture-of-a-card-payment).  Default: true
  ...
  --accept-partial-authorization: string@bool-completer # If set to `true` and charging a Square Gift Card, a payment might be returned with `amount_money` equal to less than what was requested. For example, a request for $20 when charging a Square Gift Card with a balance of $5 results in an APPROVED payment of $5. You might choose to prompt the buyer for an additional payment to cover the remainder or cancel the Gift Card payment. This field cannot be `true` when `autocomplete = true`.  For more information, see [Partial amount with Square Gift Cards](https://developer.squareup.com/docs/payments-api/take-payments#partial-payment-gift-card).  Default: false
...
export def "refunds RefundPayment" [
  ...
  --unlinked: string@bool-completer # Indicates that the refund is not linked to a Square payment. If set to true, `destination_id` and `location_id` must be supplied while `payment_id` must not be provided. (nullable)
