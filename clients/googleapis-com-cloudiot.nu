...
export def "projects delete" [
  ...
  let auth = (merge-auth [(build-auth ($token_oauth2 | default ($env | get -o CLOUD_IOT_API_OAUTH2_TOKEN | default "")) "bearer") (build-auth ($token_oauth2c | default ($env | get -o CLOUD_IOT_API_OAUTH2C_TOKEN | default "")) "bearer")])
...
export def "config-versions list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"$.xgafv": $xgafv, "access_token": $access_token, "alt": $alt, "callback": $callback, "fields": $fields, "key": $key, "oauth_token": $oauth_token, "prettyPrint": $pretty_print, "quotaUser": $quota_user, "upload_protocol": $upload_protocol, "uploadType": $upload_type, "numVersions": $num_versions} | compact), body: null}
}
...
export def "states list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"$.xgafv": $xgafv, "access_token": $access_token, "alt": $alt, "callback": $callback, "fields": $fields, "key": $key, "oauth_token": $oauth_token, "prettyPrint": $pretty_print, "quotaUser": $quota_user, "upload_protocol": $upload_protocol, "uploadType": $upload_type, "numStates": $num_states} | compact), body: null}
}
...
export def "devices list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"$.xgafv": $xgafv, "access_token": $access_token, "alt": $alt, "callback": $callback, "fields": $fields, "key": $key, "oauth_token": $oauth_token, "prettyPrint": $pretty_print, "quotaUser": $quota_user, "upload_protocol": $upload_protocol, "uploadType": $upload_type, "deviceIds": $device_ids, "deviceNumIds": $device_num_ids, "fieldMask": $field_mask, "gatewayListOptions.associationsDeviceId": $gateway_list_options_associations_device_id, "gatewayListOptions.associationsGatewayId": $gateway_list_options_associations_gateway_id, "gatewayListOptions.gatewayType": $gateway_list_options_gateway_type, "pageSize": $page_size, "pageToken": $page_token} | compact), body: null}
}
