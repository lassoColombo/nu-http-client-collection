...
export def "providers-microsoft-insights-baseline get" [
  ...
  let full_url = (build-url $base ({resource_uri: (encode-path-segment $resource_uri)} | format pattern "/{resource_uri}/providers/microsoft.insights/baseline") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"metricnames": $metricnames, "timespan": $timespan, "interval": $interval, "aggregation": $aggregation, "sensitivities": $sensitivities, "resultType": $result_type, "api-version": $api_version, "metricnamespace": $metricnamespace, "$filter": $filter} | compact)
...
export def "providers-microsoft-insights-baseline get-metric" [
  ...
  let full_url = (build-url $base ({resource_uri: (encode-path-segment $resource_uri), metric_name: (encode-path-segment $metric_name)} | format pattern "/{resource_uri}/providers/microsoft.insights/baseline/{metric_name}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"timespan": $timespan, "interval": $interval, "aggregation": $aggregation, "sensitivities": $sensitivities, "resultType": $result_type, "api-version": $api_version, "metricnamespace": $metricnamespace, "$filter": $filter} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
