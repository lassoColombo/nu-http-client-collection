# Auto-generated client for MonitorManagementClient v2018-09-01
# Source: https://api.apis.guru/v2/specs/azure.com/monitor-baseline_API/2018-09-01/swagger.json
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "providers-microsoft-insights-baseline get" } } | get name | first)
...
export def "providers-microsoft-insights-baseline get" [
  resource_uri: string
  ...
  let full_url = (build-url $base ({resource_uri: (encode-path-segment $resource_uri)} | format pattern "/{resource_uri}/providers/microsoft.insights/baseline") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"metricnames": $metricnames, "timespan": $timespan, "interval": $interval, "aggregation": $aggregation, "sensitivities": $sensitivities, "resultType": $result_type, "api-version": $api_version, "metricnamespace": $metricnamespace, "$filter": $filter} | compact), body: null}
}
...
export def "providers-microsoft-insights-baseline get-metric" [
  resource_uri: string
  metric_name: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  ...
  let full_url = (build-url $base ({resource_uri: (encode-path-segment $resource_uri), metric_name: (encode-path-segment $metric_name)} | format pattern "/{resource_uri}/providers/microsoft.insights/baseline/{metric_name}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"timespan": $timespan, "interval": $interval, "aggregation": $aggregation, "sensitivities": $sensitivities, "resultType": $result_type, "api-version": $api_version, "metricnamespace": $metricnamespace, "$filter": $filter} | compact), body: null}
}
