...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AMAZON_PERSONALIZE_EVENTS_TOKEN | default "" }
...
export def "events update" [
  ...
  tracking_id: string # The tracking ID for the event. The ID is generated by a call to the CreateEventTracker (https://docs.aws.amazon.com/personalize/latest/dg/API_CreateEventTracker.html) API.
  --user-id: string # The user associated with the event. (format: password)
  session_id: string # The session ID associated with the user's visit. Your application generates the sessionId when a user first visits your website or uses your application. Amazon Personalize uses the sessionId to associate events with the user before they log in. For more information, see Recording Events (https://docs.aws.amazon.com/personalize/latest/dg/recording-events.html).
  event_list: list # A list of event data from the session. — item shape: {eventId?: any, eventType: any, eventValue?: any, itemId?: any, properties?: any, sentAt: any, recommendationId?: any, impression?: any, metricAttribution?: any}
]: any -> any {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/events" $auth.query)
  let req_body = {"trackingId": $tracking_id, "userId": $user_id, "sessionId": $session_id, "eventList": $event_list} | compact
...
export def "items update" [
  ...
  --x-amz-signature: string
  --x-amz-signed-headers: string
  dataset_arn: string # The Amazon Resource Name (ARN) of the Items dataset you are adding the item or items to.
  ...
  let req_body = {"datasetArn": $dataset_arn, "items": $items} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"X-Amz-Content-Sha256": $x_amz_content_sha256, "X-Amz-Date": $x_amz_date, "X-Amz-Algorithm": $x_amz_algorithm, "X-Amz-Credential": $x_amz_credential, "X-Amz-Security-Token": $x_amz_security_token, "X-Amz-Signature": $x_amz_signature, "X-Amz-SignedHeaders": $x_amz_signed_headers} | compact
...
export def "users update" [
  ...
  --x-amz-signature: string
  --x-amz-signed-headers: string
  dataset_arn: string # The Amazon Resource Name (ARN) of the Users dataset you are adding the user or users to.
  ...
  let req_body = {"datasetArn": $dataset_arn, "users": $users} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"X-Amz-Content-Sha256": $x_amz_content_sha256, "X-Amz-Date": $x_amz_date, "X-Amz-Algorithm": $x_amz_algorithm, "X-Amz-Credential": $x_amz_credential, "X-Amz-Security-Token": $x_amz_security_token, "X-Amz-Signature": $x_amz_signature, "X-Amz-SignedHeaders": $x_amz_signed_headers} | compact
