# Auto-generated client for Policies System API v1.0.0
# Source: https://api.apis.guru/v2/specs/vtex.local/Policies-System-API/1.0.0/openapi.json
# Auth: --token flag or $env.POLICIES_SYSTEM_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o POLICIES_SYSTEM_API_TOKEN | default "" }
...
export def "policy-engine-evaluate create" [
  ...
  let full_url = (build-url $base "/api/policy-engine/evaluate" $auth.query)
  let req_body = {"context": $context, "resource": $resource} | compact
...
export def "policy-engine-policies list" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/policy-engine/policies" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Content-Type": $content_type, "Accept": $hdr_accept} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "policy-engine-policies delete" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/api/policy-engine/policies/{id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Content-Type": $content_type, "Accept": $hdr_accept} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "policy-engine-policies create-or-update" [
  ...
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/api/policy-engine/policies/{id}") $auth.query)
  let req_body = {"description": $description, "name": $name, "statements": $statements} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
