...
# Auth: --token flag or $env.DEPARTMENT_OF_REGISTRATION_STAMPS__MAHARASHTRA_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o DEPARTMENT_OF_REGISTRATION_STAMPS__MAHARASHTRA_TOKEN | default "" }
