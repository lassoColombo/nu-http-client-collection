...
export def "subscriptions-resource-groups-providers-microsoft-insights-components-api-keys Delete" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Insights/components/($resourceName)/APIKeys/($keyId)" $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-insights-components-api-keys List" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Insights/components/($resourceName)/ApiKeys" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "subscriptions-resource-groups-providers-microsoft-insights-components-api-keys Create" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # The API version to use for this operation.
  --linkedReadProperties: list # The read access rights of this API Key.
  --linkedWriteProperties: list # The write access rights of this API Key.
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Insights/components/($resourceName)/ApiKeys" $qp)
  let body = {linkedReadProperties: $linkedReadProperties, linkedWriteProperties: $linkedWriteProperties, name: $name} | compact
