...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o DIRECTORATE_OF_INFORMATION_TECHNOLOGY_GOVERNMENT_OF_TRIPURA_TRIPURA_TOKEN | default "" }
