...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o TRANSPORT_DEPARTMENT_JAMMU_KASHMIR_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/transportjk/v3"] }
...
export def "drvlc-certificate create" [
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o TRANSPORT_DEPARTMENT_JAMMU_KASHMIR_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o TRANSPORT_DEPARTMENT_JAMMU_KASHMIR_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
