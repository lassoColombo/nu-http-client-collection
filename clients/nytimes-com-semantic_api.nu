...
export def "name get" [
  ...
  --qp-query: string # Precedes the search term string. Used in a Search Query. Except for &lt;specific_concept_name&gt;, Search Query will take the required parameters listed above (&lt;concept_type&gt;, &lt;concept_uri&gt;, &lt;article_uri&gt;) as an optional_parameter in addition to the query=&lt;query_term&gt;.
  ...
  let qp = [(serialize-qp "fields" $qp_fields "scalar") (serialize-qp "query" $qp_query "scalar")] | flatten | str join "&"
...
export def "searchjson get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "query-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "query" $qp_query "scalar") (serialize-qp "offset" $offset "scalar") (serialize-qp "fields" $qp_fields "scalar")] | flatten | str join "&"
