...
export def "products-get-codename get-code-name" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"locale_geo_id": $locale_geo_id} | compact), body: null}
}
...
export def "products-get-ordering-info get-orderinginfo" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"product_id": $product_id, "locale_geo_id": $locale_geo_id} | compact), body: null}
}
...
export def "products-get-products list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"locale_geo_id": $locale_geo_id, "category_id": $category_id, "product_id": $product_id, "highlights": $highlights, "sort": $qp_sort, "filters": $filters, "per_page": $per_page, "page_no": $page_no} | compact), body: null}
}
...
export def "products-get-products-info get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"locale_geo_id": $locale_geo_id, "product_id": $product_id, "include_reference": $include_reference} | compact), body: null}
}
