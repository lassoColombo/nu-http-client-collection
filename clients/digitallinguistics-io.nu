...
export def "languages list" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"deleted": $deleted, "public": $public} | compact), body: null}
}
...
export def "lexemes list" [
  ...
  let extra_headers = {"continuation": $continuation, "ifModifiedSince": $if_modified_since, "maxItemCount": $max_item_count} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"deleted": $deleted, "languageID": $language_id, "public": $public} | compact), body: null}
}
...
export def "lexemes create" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"languageID": $language_id} | compact), body: null}
}
...
export def "lexemes update" [
  ...
  let extra_headers = {"ifMatch": $if_match} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"languageID": $language_id} | compact), body: null}
}
