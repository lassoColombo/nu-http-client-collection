# Auto-generated client for WorkBC Job Posting API v1.0.0
# Source: https://api.apis.guru/v2/specs/gov.bc.ca/jobposting/1.0.0/openapi.json
# Auth: --token flag or $env.WORKBC_JOB_POSTING_API_TOKEN

const BASE_URL = "https://workbcjobs.api.gov.bc.ca/v1"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o WORKBC_JOB_POSTING_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://workbcjobs.api.gov.bc.ca/v1"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "industries get" } } | get name | first)
...
]: nothing -> table<Caption: string, ID: int, SubIndustries: list<record>> {
...
export def "job-types get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> table<Caption: string, ID: int> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/jobTypes" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "jobs create" [
  ...
  let req_body = {"city": $city, "jobTypes": $job_types, "lastRequestDate": $last_request_date, "majorProjects": $major_projects, "region": $region} | compact
...
export def "major-projects get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/majorProjects" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
