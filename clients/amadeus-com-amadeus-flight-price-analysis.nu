...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o FLIGHT_PRICE_ANALYSIS_API_TOKEN | default "" }
...
export def "analytics-itinerary-price-metrics get" [
  ...
  let full_url = (build-url $base "/analytics/itinerary-price-metrics" $qp $auth.query)
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"originIataCode": $origin_iata_code, "destinationIataCode": $destination_iata_code, "departureDate": $departure_date, "currencyCode": $currency_code, "oneWay": $one_way} | compact)
