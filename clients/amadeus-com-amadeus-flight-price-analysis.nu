# Auto-generated client for Flight Price Analysis API v1.0.1
# Source: https://api.apis.guru/v2/specs/amadeus.com/amadeus-flight-price-analysis/1.0.1/openapi.json
# Auth: --token flag or $env.FLIGHT_PRICE_ANALYSIS_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o FLIGHT_PRICE_ANALYSIS_API_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "analytics-itinerary-price-metrics get" } } | get name | first)
...
export def "analytics-itinerary-price-metrics get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"originIataCode": $origin_iata_code, "destinationIataCode": $destination_iata_code, "departureDate": $departure_date, "currencyCode": $currency_code, "oneWay": $one_way} | compact), body: null}
}
