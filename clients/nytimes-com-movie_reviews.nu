...
export def "reviews-search-json get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"query": $query, "critics-pick": $critics_pick, "reviewer": $reviewer, "publication-date": $publication_date, "opening-date": $opening_date, "offset": $offset, "order": $order} | compact), body: null}
}
...
export def "reviews get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"offset": $offset, "order": $order} | compact), body: null}
}
