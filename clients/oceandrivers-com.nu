# Auto-generated client for ODWeather v1.0
# Source: https://api.apis.guru/v2/specs/oceandrivers.com/1.0/openapi.json
# Auth: --token flag or $env.ODWEATHER_TOKEN

const BASE_URL = "https://api.oceandrivers.com"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o ODWEATHER_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.oceandrivers.com"] }
...
export def "v1-0-compare-station get" [
  ...
  if ($station_name | is-empty) { error make --unspanned { msg: "path parameter 'stationName' must be non-empty" } }
  let full_url = (build-url $base ({station_name: (encode-path-segment $station_name)} | format pattern "/v1.0/compareStation/{station_name}/") $auth.query)
...
export def "v1-0-get-aemet-station get" [
  ...
  let full_url = (build-url $base ({station_name: (encode-path-segment $station_name), period: (encode-path-segment $period)} | format pattern "/v1.0/getAemetStation/{station_name}/{period}/") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "v1-0-get-easy-wind get-easywind" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --period: string # Period of time to get the data latestdata|latesthour|latestday (default: latestdata)
]: nothing -> any {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($easywind_id | is-empty) { error make --unspanned { msg: "path parameter 'easywindId' must be non-empty" } }
  let qp = [(serialize-qp "period" $period "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({easywind_id: (encode-path-segment $easywind_id)} | format pattern "/v1.0/getEasyWind/{easywind_id}/") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "v1-0-get-event-stations get" [
  ...
  let full_url = (build-url $base ({event_id: (encode-path-segment $event_id)} | format pattern "/v1.0/getEventStations/{event_id}/") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "v1-0-get-forecast-points-language get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($yatchclubid | is-empty) { error make --unspanned { msg: "path parameter 'yatchclubid' must be non-empty" } }
  if ($language | is-empty) { error make --unspanned { msg: "path parameter 'language' must be non-empty" } }
  let full_url = (build-url $base ({yatchclubid: (encode-path-segment $yatchclubid), language: (encode-path-segment $language)} | format pattern "/v1.0/getForecastPoints/{yatchclubid}/language/{language}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "v1-0-get-forecast-time-series get" [
  ...
  let full_url = (build-url $base ({latitude: (encode-path-segment $latitude), longitude: (encode-path-segment $longitude)} | format pattern "/v1.0/getForecastTimeSeries/{latitude}/{longitude}/") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"inittime": $inittime, "endtime": $endtime, "days": $days, "hours": $hours, "weather": $weather, "wave": $wave, "entryid": $entryid} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "v1-0-get-forecast-time-series-wrf get" [
  ...
  if ($longitude | is-empty) { error make --unspanned { msg: "path parameter 'longitude' must be non-empty" } }
  let qp = [(serialize-qp "inittime" $inittime "scalar") (serialize-qp "endtime" $endtime "scalar") (serialize-qp "days" $days "scalar") (serialize-qp "hours" $hours "scalar") (serialize-qp "weather" $weather "scalar") (serialize-qp "wave" $wave "scalar") (serialize-qp "entryid" $entryid "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({latitude: (encode-path-segment $latitude), longitude: (encode-path-segment $longitude)} | format pattern "/v1.0/getForecastTimeSeriesWrf/{latitude}/{longitude}/") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "v1-0-get-socib-weather-station get" [
  ...
  if ($station_name | is-empty) { error make --unspanned { msg: "path parameter 'stationName' must be non-empty" } }
  if ($period | is-empty) { error make --unspanned { msg: "path parameter 'period' must be non-empty" } }
  let full_url = (build-url $base ({station_name: (encode-path-segment $station_name), period: (encode-path-segment $period)} | format pattern "/v1.0/getSocibWeatherStation/{station_name}/{period}/") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "v1-0-get-weather-display get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --period: string # Period of time to get the data latestdata|latesthour|latestday|dailylog (default: latestdata)
]: nothing -> any {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($station_name | is-empty) { error make --unspanned { msg: "path parameter 'stationName' must be non-empty" } }
  let qp = [(serialize-qp "period" $period "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({station_name: (encode-path-segment $station_name)} | format pattern "/v1.0/getWeatherDisplay/{station_name}/") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "v1-0-get-web-cams get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/v1.0/getWebCams/" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
