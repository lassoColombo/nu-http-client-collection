...
def alertTenantType-completer [] { ["Company" "Council" "Employed" "OwnMeans" "Retired" "SelfEmployed" "Student" "Unemployed"] }
...
export def "tier2-area-areas list" [
  ...
  let full_url = (build-url $base $"/v2/tier2/($shortName)/area/areas" $qp)
...
export def "tier2-area-areas get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v2/tier2/($shortName)/area/areas/($areaID)")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-branch-branches list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/branch/branches" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-branch-branches get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v2/tier2/($shortName)/branch/branches/($branchID)")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-company GetCompany" [
  shortName: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v2/tier2/($shortName)/company")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-county-counties list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/county/counties" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-county-counties get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v2/tier2/($shortName)/county/counties/($countyID)")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-county-counties-branches GetCountiesBranches" [
  shortName: string
  countyID: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/county/counties/($countyID)/branches" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-diary-allocations list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/diary/allocations" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-diary-allocations get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v2/tier2/($shortName)/diary/allocations/($diaryAllocationID)")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-diary-appointments list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/diary/appointments" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-diary-appointments get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v2/tier2/($shortName)/diary/appointments/($diaryAppointmentID)")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-diary-appointmenttypes list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/diary/appointmenttypes" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-diary-appointmenttypes get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v2/tier2/($shortName)/diary/appointmenttypes/($diaryAppointmentTypeID)")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-lettings-advertised GetAdvertised" [
  shortName: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let full_url = (build-url $base $"/v2/tier2/($shortName)/lettings/advertised" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-lettings-advertisedbetweendates GetAdvertisedBetweenDates" [
  shortName: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let full_url = (build-url $base $"/v2/tier2/($shortName)/lettings/advertisedbetweendates" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-lettings-tenancies list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/lettings/tenancies" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-lettings-tenancies get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v2/tier2/($shortName)/lettings/tenancies/($tenancyID)")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-lettings-tenancies-brochure GetTenancyBrochure" [
  shortName: string
  tenancyID: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v2/tier2/($shortName)/lettings/tenancies/($tenancyID)/brochure")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-photo-photos list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/photo/photos" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-photo-photos get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v2/tier2/($shortName)/photo/photos/($photoID)")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-photos-photo-download GetPhotoDownload" [
  shortName: string
  photoID: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  ...
  let full_url = (build-url $base $"/v2/tier2/($shortName)/photos/photo/($photoID)/download" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-property-properties list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/property/properties" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-property-properties get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v2/tier2/($shortName)/property/properties/($propertyID)")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-property-properties-facilities GetPropertiesFacilities" [
  shortName: string
  propertyID: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/property/properties/($propertyID)/facilities" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-property-properties-photos GetPropertiesPhotos" [
  shortName: string
  propertyID: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/property/properties/($propertyID)/photos" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-property-properties-rooms GetPropertiesRooms" [
  shortName: string
  propertyID: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/property/properties/($propertyID)/rooms" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-property-properties-tenancies GetPropertiesTenancies" [
  shortName: string
  propertyID: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/property/properties/($propertyID)/tenancies" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-property-structures-reports-eer GetPropertyEERDownload" [
  shortName: string
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v2/tier2/($shortName)/property/structures/($propertyStructureID)/reports/eer")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-property-structures-reports-eir GetPropertyEIRDownload" [
  shortName: string
  propertyStructureID: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v2/tier2/($shortName)/property/structures/($propertyStructureID)/reports/eir")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-sales-advertisedsales GetAdvertisedSales" [
  shortName: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let full_url = (build-url $base $"/v2/tier2/($shortName)/sales/advertisedsales" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-sales-reports-eer GetEER" [
  shortName: string
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v2/tier2/($shortName)/sales/reports/eer/($salesInstructionID)")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-sales-reports-eir GetEIR" [
  shortName: string
  salesInstructionID: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v2/tier2/($shortName)/sales/reports/eir/($salesInstructionID)")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-sales-salesfeaturetypes list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/sales/salesfeaturetypes" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-sales-salesfeaturetypes get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v2/tier2/($shortName)/sales/salesfeaturetypes/($salesFeatureTypeID)")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-sales-salesinstructions list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/sales/salesinstructions" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-sales-salesinstructions get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v2/tier2/($shortName)/sales/salesinstructions/($salesInstructionID)")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-sales-salesinstructions-features GetSalesInstructionsFeatures" [
  shortName: string
  salesInstructionID: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/sales/salesinstructions/($salesInstructionID)/features" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-sales-salesinstructions-floorplans GetSalesInstructionsFloorPlans" [
  shortName: string
  salesInstructionID: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/sales/salesinstructions/($salesInstructionID)/floorplans" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-sales-salesinstructions-photos GetSalesInstructionsPhotos" [
  shortName: string
  salesInstructionID: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/sales/salesinstructions/($salesInstructionID)/photos" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-sales-salesinstructions-rooms GetSalesInstructionsRooms" [
  shortName: string
  salesInstructionID: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/sales/salesinstructions/($salesInstructionID)/rooms" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-staff-staff list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "offset" $offset "scalar") (serialize-qp "count" $count "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/staff/staff" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-staff-staff get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/v2/tier2/($shortName)/staff/staff/($applicationStaffID)")
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-viewing-bookings GetBookings" [
  shortName: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --accept: string@accept-completer # Response content type
  --preferredDate: string # The preferred date for a viewing (format: date-time)
  --propertyIDsToView: list # An array of unique IDs of properties to view
]: nothing -> table<End: string, StaffID: string, StaffName: string, Start: string> {
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "preferredDate" $preferredDate "scalar") (serialize-qp "propertyIDsToView" $propertyIDsToView "multi")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/viewing/bookings" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "tier2-viewing-bookings MakeBooking" [
  shortName: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  --propertyIDsToView: list # An array of unique IDs of properties to view
  --wantRoomInSharedProperty: oneof<nothing, bool> # Whether the prospect wants a shared property
  --alertMinRent: float # The minimum rent amount the prospect is looking for (format: double)
  --alertMaxRent: float # The maximum rent amount the prospect is looking for (format: double)
  --alertNumberOfBeds: int # The minimum number of beds the prospect is looking for (format: int32)
  --alertAreaID: string # The unique ID of the area the prospect is looking for
  --alertTenantType: string@alertTenantType-completer # The tenanct type the prospect is looking for
  --subscribeToEmailAlerts: oneof<nothing, bool> # Whether to subscribe the prospect to email alerts
  --subscribeToSMSAlerts: oneof<nothing, bool> # Whether to subscribe the prospect to SMS alerts
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "forename" $forename "scalar") (serialize-qp "surname" $surname "scalar") (serialize-qp "mobilePhone" $mobilePhone "scalar") (serialize-qp "emailAddress" $emailAddress "scalar") (serialize-qp "propertyIDsToView" $propertyIDsToView "multi") (serialize-qp "wantRoomInSharedProperty" $wantRoomInSharedProperty "scalar") (serialize-qp "alertMinRent" $alertMinRent "scalar") (serialize-qp "alertMaxRent" $alertMaxRent "scalar") (serialize-qp "alertNumberOfBeds" $alertNumberOfBeds "scalar") (serialize-qp "alertAreaID" $alertAreaID "scalar") (serialize-qp "alertTenantType" $alertTenantType "scalar") (serialize-qp "subscribeToEmailAlerts" $subscribeToEmailAlerts "scalar") (serialize-qp "subscribeToSMSAlerts" $subscribeToSMSAlerts "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v2/tier2/($shortName)/viewing/bookings" $qp)
  let body = {End: $End, StaffID: $StaffID, StaffName: $StaffName, Start: $Start} | compact
