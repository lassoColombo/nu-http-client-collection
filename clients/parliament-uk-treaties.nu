# Auto-generated client for Treaties API vv1
# Source: https://api.apis.guru/v2/specs/parliament.uk/treaties/v1/openapi.json
# Auth: --token flag or $env.TREATIES_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o TREATIES_API_TOKEN | default "" }
...
export def "business-item get" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/api/BusinessItem/{id}") $auth.query)
...
export def "government-organisation get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/GovernmentOrganisation" $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "series-membership get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/SeriesMembership" $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "treaty get-treaties" [
  ...
  let full_url = (build-url $base "/api/Treaty" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"SearchText": $search_text, "GovernmentOrganisationId": $government_organisation_id, "Series": $series, "ParliamentaryProcess": $parliamentary_process, "DebateScheduled": $debate_scheduled, "MotionToNotRatify": $motion_to_not_ratify, "RecommendedNotRatify": $recommended_not_ratify, "House": $house, "Skip": $skip, "Take": $take} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "treaty get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/api/Treaty/{id}") $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "treaty-business-items get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/api/Treaty/{id}/BusinessItems") $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
