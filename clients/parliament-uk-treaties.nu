...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o TREATIES_API_TOKEN | default "" }
...
export def "treaty get-treaties" [
  ...
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"SearchText": $search_text, "GovernmentOrganisationId": $government_organisation_id, "Series": $series, "ParliamentaryProcess": $parliamentary_process, "DebateScheduled": $debate_scheduled, "MotionToNotRatify": $motion_to_not_ratify, "RecommendedNotRatify": $recommended_not_ratify, "House": $house, "Skip": $skip, "Take": $take} | compact), body: null}
}
