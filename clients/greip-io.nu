...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o GREIP_API_TOKEN | default "" }
...
export def "bulk-lookup get" [
  ...
  let full_url = (build-url $base "/BulkLookup" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"key": $key, "ips": $ips, "params": $params, "lang": $lang, "format": $format} | compact)
...
export def "country get" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"key": $key, "CountryCode": $country_code, "params": $params, "lang": $lang, "format": $format} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "geo-ip get" [
  ...
  let full_url = (build-url $base "/GeoIP" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"key": $key, "params": $params, "lang": $lang, "format": $format} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "ip-lookup get" [
  ...
  let full_url = (build-url $base "/IPLookup" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"key": $key, "ip": $ip, "params": $params, "lang": $lang, "format": $format} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "bad-words get" [
  ...
  let full_url = (build-url $base "/badWords" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"key": $key, "text": $text, "listBadWords": $list_bad_words, "scoreOnly": $score_only, "format": $format} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
