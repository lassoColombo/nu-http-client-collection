...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o REAL_TIME_BUS_PREDICTIONS_TOKEN | default "" }
...
export def "predictions get-5476365e031f5909e4fe331e" [
  ...
  let full_url = (build-url $base "/Predictions" $qp $auth.query)
  let accept_val = ($accept | default "text/xml")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"StopID": $stop_id} | compact)
...
export def "json-j-predictions get-5476365e031f5909e4fe331d" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "StopID" $stop_id "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/json/jPredictions" $qp $auth.query)
