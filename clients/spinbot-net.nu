...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o ARTICLE_REWRITER_AND_ARTICLE_EXTRACTOR_API_TOKEN | default "" }
...
export def "acc get" [
  ...
  let full_url = (build-url $base "/api/acc" $qp $auth.query)
...
export def "pretty-spinner create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "query-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/pretty-spinner" $auth.query)
...
export def "spinner create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "query-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/spinner" $auth.query)
...
export def "spintax create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "query-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/spintax" $auth.query)
