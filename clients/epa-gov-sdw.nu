...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o U_S_EPA_ENFORCEMENT_AND_COMPLIANCE_HISTORY_ONLINE_ECHO_SAFE_DRINKING_WATER_ACT_TOKEN | default "" }
...
export def "sdw-rest-services-get-download get" [
  ...
  let full_url = (build-url $base "/sdw_rest_services.get_download" $qp $auth.query)
...
export def "sdw-rest-services-get-download create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/sdw_rest_services.get_download" $auth.query)
...
export def "sdw-rest-services-get-qid get" [
  ...
  let full_url = (build-url $base "/sdw_rest_services.get_qid" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "sdw-rest-services-get-qid create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/sdw_rest_services.get_qid" $auth.query)
...
export def "sdw-rest-services-get-systems create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/sdw_rest_services.get_systems" $auth.query)
...
export def "sdw-rest-services-metadata get" [
  ...
  let full_url = (build-url $base "/sdw_rest_services.metadata" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "sdw-rest-services-metadata create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/sdw_rest_services.metadata" $auth.query)
