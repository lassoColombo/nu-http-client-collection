...
export def "subscriptions-resource-groups-providers-microsoft-hd-insight-clusters-configurations List" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.HDInsight/clusters/($clusterName)/configurations" $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-hd-insight-clusters-configurations Get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.HDInsight/clusters/($clusterName)/configurations/($configurationName)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
