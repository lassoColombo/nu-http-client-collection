...
export def "action-create-application-version CreateApplicationVersion" [
  ...
  --AutoCreateApplication: oneof<nothing, bool> # Set to <code>true</code> to create an application with the specified name if it doesn't already exist.
...
export def "action-describe-environments DescribeEnvironments" [
  ...
  --IncludeDeleted: oneof<nothing, bool> # <p>Indicates whether to include deleted environments:</p> <p> <code>true</code>: Environments that have been deleted after <code>IncludedDeletedBackTo</code> are displayed.</p> <p> <code>false</code>: Do not include deleted environments.</p>
...
export def "action-terminate-environment TerminateEnvironment" [
  ...
  --ForceTerminate: oneof<nothing, bool> # Terminates the target environment even if another environment in the same group is dependent on it.
