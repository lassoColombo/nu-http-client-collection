...
export def "contracts list-get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"duns": $duns, "naics": $naics, "sort": $qp_sort, "direction": $direction, "page": $page} | compact), body: null}
}
...
export def "vendor get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($duns | is-empty) { error make --unspanned { msg: "path parameter 'duns' must be non-empty" } }
...
export def "vendors list-get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"naics": $naics, "setasides": $setasides, "vehicle": $vehicle} | compact), body: null}
}
