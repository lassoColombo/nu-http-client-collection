# Auto-generated client for HubSpot Events API vv3
# Source: https://api.apis.guru/v2/specs/hubapi.com/events/v3/openapi.json
# Auth: --token flag or $env.HUBSPOT_EVENTS_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o HUBSPOT_EVENTS_API_TOKEN | default "" }
...
export def "events-events get-page" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"occurredAfter": $occurred_after, "occurredBefore": $occurred_before, "objectType": $object_type, "objectId": $object_id, "eventType": $event_type, "after": $after, "before": $before, "limit": $limit, "sort": $qp_sort} | compact), body: null}
}
