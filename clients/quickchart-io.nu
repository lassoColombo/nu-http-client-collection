# Auto-generated client for QuickChart API v1.0.0
# Source: https://api.apis.guru/v2/specs/quickchart.io/1.0.0/openapi.json
# Auth: --token flag or $env.QUICKCHART_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o QUICKCHART_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://quickchart.io"] }
def auth-scheme-completer [] { ["bearer"] }

# Completers for enum parameters
def accept-completer [] { ["image/jpeg" "image/png" "image/svg+xml" "image/webp"] }
...
export def "chart get" [
  ...
  --chart: string # The chart configuration in Chart.js format (JSON or Javascript).
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"chart": $chart, "width": $width, "height": $height, "format": $format, "backgroundColor": $background_color} | compact), body: null}
}
...
export def "chart create" [
  ...
  let req_body = {"backgroundColor": $background_color, "chart": $chart, "format": $format, "height": $height, "width": $width} | compact
...
export def "qr get" [
  ...
  let full_url = (build-url $base "/qr" $qp)
  let accept_val = ($accept | default "image/png")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"text": $text, "width": $width, "height": $height, "format": $format, "margin": $margin} | compact), body: null}
}
...
export def "qr create" [
  ...
  let req_body = {"format": $format, "height": $height, "margin": $margin, "text": $text, "width": $width} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = ($accept | default "image/png")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: {}, body: $req_body}
