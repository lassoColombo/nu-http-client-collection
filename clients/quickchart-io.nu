...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o QUICKCHART_API_TOKEN | default "" }
...
export def "chart get" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"chart": $chart, "width": $width, "height": $height, "format": $format, "backgroundColor": $background_color} | compact)
...
export def "qr get" [
  ...
  let full_url = (build-url $base "/qr" $qp $auth.query)
  let accept_val = ($accept | default "image/png")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"text": $text, "width": $width, "height": $height, "format": $format, "margin": $margin} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "qr create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/qr" $auth.query)
