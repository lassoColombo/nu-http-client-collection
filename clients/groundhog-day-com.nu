...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o GROUNDHOG_DAY_API_TOKEN | default "" }
...
export def "info get-root" [
  ...
  let full_url = (build-url $base "/api/v1" $auth.query)
...
export def "groundhogs list" [
  ...
  let full_url = (build-url $base "/api/v1/groundhogs" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"country": $country, "isGroundhog": $is_groundhog} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "groundhogs get" [
  ...
  let full_url = (build-url $base ({slug: (encode-path-segment $slug)} | format pattern "/api/v1/groundhogs/{slug}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "predictions get" [
  ...
  let full_url = (build-url $base "/api/v1/predictions" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "spec get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v1/spec" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
