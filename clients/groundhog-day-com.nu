# Auto-generated client for Groundhog Day API v1.2.1
# Source: https://api.apis.guru/v2/specs/groundhog-day.com/1.2.1/openapi.json
# Auth: --token flag or $env.GROUNDHOG_DAY_API_TOKEN

const BASE_URL = "https://virtserver.swaggerhub.com/pcraig3/groundhog-day-api/1.2.1"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o GROUNDHOG_DAY_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://virtserver.swaggerhub.com/pcraig3/groundhog-day-api/1.2.1" "https://groundhog-day.com"] }
...
]: nothing -> record<_links: record<groundhog: record<href: string>, groundhogs: record<href: string>, predictions: record<href: string>, self: record<href: string>, spec: record<href: string>>, message: string> {
...
export def "groundhogs list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"country": $country, "isGroundhog": $is_groundhog} | compact), body: null}
}
...
export def "groundhogs get" [
  ...
  let full_url = (build-url $base ({slug: (encode-path-segment $slug)} | format pattern "/api/v1/groundhogs/{slug}"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
...
export def "predictions get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --year: int # A calendar year
]: nothing -> record<predictions: table<details: string, groundhog: record, shadow: int, year: int>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "year" $year "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/api/v1/predictions" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
