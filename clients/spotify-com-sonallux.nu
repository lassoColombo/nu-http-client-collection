...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o SPOTIFY_WEB_API_WITH_FIXES_AND_IMPROVEMENTS_FROM_SONALLUX_TOKEN | default "" }
...
export def "me-top-artists get-users" [
  ...
  let full_url = (build-url $base "/me/top/artists" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-top-tracks get-users" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "time_range" $time_range "scalar") (serialize-qp "limit" $limit "scalar") (serialize-qp "offset" $offset "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/top/tracks" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
