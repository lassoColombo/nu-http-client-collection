# Auto-generated client for Charity API vv1.2.1
# Source: https://api.apis.guru/v2/specs/ebay.com/commerce-charity/v1.2.1/openapi.json
# Auth: --token flag or $env.CHARITY_API_TOKEN

const BASE_URL = "https://api.ebay.com/commerce/charity/v1"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o CHARITY_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.ebay.com/commerce/charity/v1"] }
...
export def "charity-org list" [
  ...
  --limit: string # The number of items, from the result set, returned in a single page.Valid Values: 1-100Default: 20
  ...
  --registration-ids: string # A comma-separated list of charitable organization registration IDs.Note: Do not specify this parameter for query-based searches. Specify either the q or registration_ids parameter, but not both.Maximum Limit: 20
  --x-ebay-c-marketplace-id: string # A header used to specify the eBay marketplace ID.Valid Values: EBAY_GB and EBAY_US
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"limit": $limit, "offset": $offset, "q": $q, "registration_ids": $registration_ids} | compact), body: null}
}
...
export def "charity-org get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($charity_org_id | is-empty) { error make --unspanned { msg: "path parameter 'charity_org_id' must be non-empty" } }
  let full_url = (build-url $base ({charity_org_id: (encode-path-segment $charity_org_id)} | format pattern "/charity_org/{charity_org_id}"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"X-EBAY-C-MARKETPLACE-ID": $x_ebay_c_marketplace_id} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
