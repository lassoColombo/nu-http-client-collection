...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o CHARITY_API_TOKEN | default "" }
...
export def "charity-org list" [
  ...
  let full_url = (build-url $base "/charity_org" $qp $auth.query)
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"limit": $limit, "offset": $offset, "q": $q, "registration_ids": $registration_ids} | compact)
...
export def "charity-org get" [
  ...
  let full_url = (build-url $base ({charity_org_id: (encode-path-segment $charity_org_id)} | format pattern "/charity_org/{charity_org_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"X-EBAY-C-MARKETPLACE-ID": $x_ebay_c_marketplace_id} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
