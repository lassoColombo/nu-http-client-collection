# Auto-generated client for FORTNITE REST API v3.1.5
# Source: https://api.apis.guru/v2/specs/skynewz-api-fortnite.herokuapp.com/3.1.5/swagger.json
# Auth: --token flag or $env.FORTNITE_REST_API_TOKEN

const BASE_URL = "https://skynewz-api-fortnite.herokuapp.com/api"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o FORTNITE_REST_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://skynewz-api-fortnite.herokuapp.com/api"] }
...
export def "check get" [
  ...
  let full_url = (build-url $base "/check" $auth.query)
...
export def "news get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> record<br: record<_type: string, body: string, image: string, title: string>, common: record<_type: string, body: string, title: string>, loginmessage: record<_type: string, body: string, title: string>, survivalmessage: record<_type: string, body: string, title: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/news" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "oauth-token create" [
  ...
  email: string
  password: string
]: any -> record<access_toekn: string, expiresIn: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
...
export def "pve-info get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/pve/info" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "pve-user get" [
  ...
  let full_url = (build-url $base ({username: (encode-path-segment $username)} | format pattern "/pve/user/{username}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "stats-id get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($plateform | is-empty) { error make --unspanned { msg: "path parameter 'plateform' must be non-empty" } }
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({plateform: (encode-path-segment $plateform), id: (encode-path-segment $id)} | format pattern "/stats/id/{plateform}/{id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "stats get" [
  ...
  if ($plateform | is-empty) { error make --unspanned { msg: "path parameter 'plateform' must be non-empty" } }
  if ($username | is-empty) { error make --unspanned { msg: "path parameter 'username' must be non-empty" } }
  let full_url = (build-url $base ({plateform: (encode-path-segment $plateform), username: (encode-path-segment $username)} | format pattern "/stats/{plateform}/{username}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "store get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/store" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "user get" [
  ...
  if ($plateform | is-empty) { error make --unspanned { msg: "path parameter 'plateform' must be non-empty" } }
  if ($username | is-empty) { error make --unspanned { msg: "path parameter 'username' must be non-empty" } }
  let full_url = (build-url $base ({plateform: (encode-path-segment $plateform), username: (encode-path-segment $username)} | format pattern "/user/{plateform}/{username}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
