...
export def "providers-microsoft-blueprint-blueprint-assignments-assignment-operations List" [
  ...
  let full_url = (build-url $base $"/($scope)/providers/Microsoft.Blueprint/blueprintAssignments/($assignmentName)/assignmentOperations" $qp)
...
export def "providers-microsoft-blueprint-blueprint-assignments-assignment-operations Get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/($scope)/providers/Microsoft.Blueprint/blueprintAssignments/($assignmentName)/assignmentOperations/($assignmentOperationName)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
