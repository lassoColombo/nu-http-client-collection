# Auto-generated client for Self Service Developer API v1.0.0
# Source: https://api.apis.guru/v2/specs/peoplefinderspro.com/1.0.0/openapi.json
# Auth: --token flag or $env.SELF_SERVICE_DEVELOPER_API_TOKEN

const BASE_URL = "https://api.peoplefinderspro.com"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o SELF_SERVICE_DEVELOPER_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.peoplefinderspro.com"] }
...
export def "address-autocomplete create" [
  ...
  let full_url = (build-url $base "/address/autocomplete" $auth.query)
...
export def "contact-enrich create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/contact/enrich" $auth.query)
  let req_body = {"Address": $address, "Age": $age, "Dob": $dob, "Email": $email, "FirstName": $first_name, "LastName": $last_name, "MiddleName": $middle_name, "PhoneNumber": $phone_number} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"galaxy-ap-name": $galaxy_ap_name, "galaxy-ap-password": $galaxy_ap_password, "galaxy-search-type": $galaxy_search_type} | compact
...
export def "email-enrich create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/email/enrich" $auth.query)
...
export def "identity-verify-id list" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/identity/verify_id" $auth.query)
  let req_body = {"Address": $address, "Age": $age, "Dob": $dob, "Email": $email, "FirstName": $first_name, "LastName": $last_name, "MiddleName": $middle_name, "PhoneNumber": $phone_number} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "phone-enrich create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/phone/enrich" $auth.query)
  let req_body = {"Phone": $phone} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"galaxy-ap-name": $galaxy_ap_name, "galaxy-ap-password": $galaxy_ap_password, "galaxy-search-type": $galaxy_search_type} | compact
