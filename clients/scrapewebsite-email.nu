...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o SCRAPE_WEBSITE_EMAIL_API_TOKEN | default "" }
...
export def "ping-json get-format" [
  ...
  let full_url = (build-url $base "/v1/ping.json" $auth.query)
...
export def "scrape-emails-json get-format" [
  ...
  let full_url = (build-url $base "/v1/scrape_emails.json" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"website": $website, "must_include": $must_include} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "scrape-store-links-json get-format" [
  ...
  let full_url = (build-url $base "/v1/scrape_store_links.json" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
