# Auto-generated client for Scrape Website Email API v0.1
# Source: https://api.apis.guru/v2/specs/scrapewebsite.email/0.1/swagger.json
# Auth: --token flag or $env.SCRAPE_WEBSITE_EMAIL_API_TOKEN

const BASE_URL = "http://scrapewebsite.email"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o SCRAPE_WEBSITE_EMAIL_API_TOKEN | default "" }
...
def base-url-completer [] { ["http://scrapewebsite.email"] }
...
export def "ping-json get-format" [
  ...
  let full_url = (build-url $base "/v1/ping.json")
...
export def "scrape-emails-json get-format" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "website" $website "scalar") (serialize-qp "must_include" $must_include "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/v1/scrape_emails.json" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"website": $website, "must_include": $must_include} | compact), body: null}
}
...
export def "scrape-store-links-json get-format" [
  ...
  let full_url = (build-url $base "/v1/scrape_store_links.json" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
