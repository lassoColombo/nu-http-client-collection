...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o BIKEWISE_API_V2_TOKEN | default "" }
...
export def "incidents list" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"page": $page, "per_page": $per_page, "occurred_before": $occurred_before, "occurred_after": $occurred_after, "incident_type": $incident_type, "proximity": $proximity, "proximity_square": $proximity_square, "query": $query} | compact), body: null}
}
...
export def "locations get-version-format" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"occurred_before": $occurred_before, "occurred_after": $occurred_after, "incident_type": $incident_type, "proximity": $proximity, "proximity_square": $proximity_square, "query": $query, "limit": $limit, "all": $all} | compact), body: null}
}
