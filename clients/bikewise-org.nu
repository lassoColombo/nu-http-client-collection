# Auto-generated client for BikeWise API v2 vv2
# Source: https://api.apis.guru/v2/specs/bikewise.org/v2/openapi.json
# Auth: --token flag or $env.BIKEWISE_API_V2_TOKEN

const BASE_URL = "https://bikewise.org/api"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o BIKEWISE_API_V2_TOKEN | default "" }
...
def base-url-completer [] { ["https://bikewise.org/api"] }
def auth-scheme-completer [] { ["bearer"] }

# Completers for enum parameters
def incident-type-completer [] { ["chop_shop" "crash" "hazard" "infrastructure_issue" "theft" "unconfirmed"] }
...
export def "incidents list" [
  ...
  let full_url = (build-url $base "/v2/incidents" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"page": $page, "per_page": $per_page, "occurred_before": $occurred_before, "occurred_after": $occurred_after, "incident_type": $incident_type, "proximity": $proximity, "proximity_square": $proximity_square, "query": $query} | compact)
...
export def "incidents get-version-format" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/v2/incidents/{id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "locations get-version-format" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"occurred_before": $occurred_before, "occurred_after": $occurred_after, "incident_type": $incident_type, "proximity": $proximity, "proximity_square": $proximity_square, "query": $query, "limit": $limit, "all": $all} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "locations-markers get-version-format" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "occurred_before" $occurred_before "scalar") (serialize-qp "occurred_after" $occurred_after "scalar") (serialize-qp "incident_type" $incident_type "scalar") (serialize-qp "proximity" $proximity "scalar") (serialize-qp "proximity_square" $proximity_square "scalar") (serialize-qp "query" $query "scalar") (serialize-qp "limit" $limit "scalar") (serialize-qp "all" $all "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/v2/locations/markers" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
