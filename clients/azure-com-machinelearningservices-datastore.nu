...
export def "datastore-v1-0-subscriptions-resource-groups-providers-microsoft-machine-learning-services-workspaces-datastores list-data-stores" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"dataStoreNames": $data_store_names, "continuationToken": $continuation_token, "count": $count, "includeSecret": $include_secret} | compact), body: null}
}
...
export def "datastore-v1-0-subscriptions-resource-groups-providers-microsoft-machine-learning-services-workspaces-datastores create-data-stores" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"createIfNotExists": $create_if_not_exists, "skipValidation": $skip_validation} | compact), body: $req_body}
}
...
export def "datastore-v1-0-subscriptions-resource-groups-providers-microsoft-machine-learning-services-workspaces-datastores update-data-stores" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"createIfNotExists": $create_if_not_exists, "skipValidation": $skip_validation} | compact), body: $req_body}
}
