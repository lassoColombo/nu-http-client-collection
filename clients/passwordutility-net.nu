...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o PASSWORDUTILITY_WEB_TOKEN | default "" }
...
export def "password-generate generate" [
  ...
  let full_url = (build-url $base "/api/password/generate" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
    url: $full_url
    query: ({"length": $length, "upperCase": $upper_case, "digits": $digits, "specialCharacters": $special_characters} | compact)
...
export def "password-validate validate" [
  ...
  let full_url = (build-url $base "/api/password/validate" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
