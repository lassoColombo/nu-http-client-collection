...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AUTHENTIQ_CONNECT_API_TOKEN | default "" }
...
export def "authorize get" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"client_id": $client_id, "response_type": $response_type, "scope": $scope, "redirect_uri": $redirect_uri, "state": $state, "response_mode": $response_mode, "nonce": $nonce, "display": $display, "prompt": $prompt, "max_age": $max_age, "ui_locales": $ui_locales} | compact)
  ...
  send-get $req $insecure $raw $allow_errors $full [302 303]
}
...
export def "userinfo get-user" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/userinfo" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "iframe get-authorize" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($client_id | is-empty) { error make --unspanned { msg: "path parameter 'client_id' must be non-empty" } }
  let full_url = (build-url $base ({client_id: (encode-path-segment $client_id)} | format pattern "/{client_id}/iframe") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
