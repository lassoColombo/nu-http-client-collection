...
# Auth: --token flag or $env.REVENUE_DISASTER_MANAGEMENT_DEPARTMENT__ASSAM__ASSAM_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o REVENUE_DISASTER_MANAGEMENT_DEPARTMENT__ASSAM__ASSAM_TOKEN | default "" }
