# Auto-generated client for Shorten.REST API Documentation v1.0.0
# Source: https://api.apis.guru/v2/specs/shorten.rest/1.0.0/openapi.json
# Auth: --token flag or $env.SHORTEN_REST_API_DOCUMENTATION_TOKEN

const BASE_URL = "https://api.shorten.rest"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o SHORTEN_REST_API_DOCUMENTATION_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.shorten.rest"] }
...
export def "aliases delete-alias" [
  ...
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"domainName": $domain_name, "aliasName": $alias_name} | compact), body: null}
}
...
export def "aliases get-alias" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"domainName": $domain_name, "aliasName": $alias_name} | compact), body: null}
}
...
export def "aliases create-alias" [
...
]: any -> record<aliasName: string, domainName: string, shortUrl: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "domainName" $domain_name "scalar") (serialize-qp "aliasName" $alias_name "scalar")] | flatten | str join "&"
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"domainName": $domain_name, "aliasName": $alias_name} | compact), body: $req_body}
}
...
export def "aliases update-alias" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"domainName": $domain_name, "aliasName": $alias_name} | compact), body: $req_body}
}
...
export def "aliases-all get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"domainName": $domain_name, "continueFrom": $continue_from, "limit": $limit} | compact), body: null}
}
...
export def "clicks get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"continueFrom": $continue_from, "limit": $limit} | compact), body: null}
}
