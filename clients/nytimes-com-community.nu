# Auto-generated client for Community API v3.0.0
# Source: https://api.apis.guru/v2/specs/nytimes.com/community/3.0.0/openapi.json
# Auth: --token flag or $env.COMMUNITY_API_TOKEN

const BASE_URL = "http://api.nytimes.com/svc/community/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o COMMUNITY_API_TOKEN | default "" }
...
def base-url-completer [] { ["http://api.nytimes.com/svc/community/v3"] }
...
export def "user-content-by-date-json get" [
  ...
  let full_url = (build-url $base "/user-content/by-date.json" $qp $auth.query)
...
export def "user-content-recent-json get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> record<copyright: string, debug: record<version: float>, results: record<api_timestamp: string, comments: list<record>, totalCommentsReturned: int>, status: string> {
  let auth = (build-auth $token ($auth_scheme | default "query-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/user-content/recent.json" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "user-content-url-json get" [
  ...
  let full_url = (build-url $base "/user-content/url.json" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "user-content-user-json get" [
  ...
  let full_url = (build-url $base "/user-content/user.json" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
