...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o COMMUNITY_API_TOKEN | default "" }
...
export def "user-content-by-date-json get" [
  ...
  let full_url = (build-url $base "/user-content/by-date.json" $qp)
...
export def "user-content-recent-json get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "query-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/user-content/recent.json")
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
