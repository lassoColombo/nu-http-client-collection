...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o IMAGE_CHARTS_TOKEN | default "" }
...
export def "chart get" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"cht": $cht, "chd": $chd, "chds": $chds, "choe": $choe, "chld": $chld, "chxr": $chxr, "chxp": $chxp, "chof": $chof, "chs": $chs, "chdl": $chdl, "chdls": $chdls, "chg": $chg, "chco": $chco, "chtt": $chtt, "chts": $chts, "chxt": $chxt, "chxl": $chxl, "chxs": $chxs, "chm": $chm, "chls": $chls, "chl": $chl, "chlps": $chlps, "chma": $chma, "chdlp": $chdlp, "chf": $chf, "chbh": $chbh, "chbr": $chbr, "chan": $chan, "chli": $chli, "icac": $icac, "ichm": $ichm, "icff": $icff, "icfs": $icfs, "iclocale": $iclocale, "icwt": $icwt, "icretina": $icretina, "icqrb": $icqrb, "icqrf": $icqrf} | compact)
...
export def "chart-js-2-8-0 get-chartjs280" [
  ...
  let full_url = (build-url $base "/chart.js/2.8.0" $qp $auth.query)
  let accept_val = ($accept | default "application/png")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"c": $c, "chart": $chart, "width": $width, "height": $height, "backgroundColor": $background_color, "bkg": $bkg, "encoding": $encoding, "icac": $icac, "ichm": $ichm, "icretina": $icretina} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
