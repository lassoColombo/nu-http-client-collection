...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AMAZON_PERSONALIZE_RUNTIME_TOKEN | default "" }
...
export def "personalize-ranking get-personalized" [
  ...
  let full_url = (build-url $base "/personalize-ranking" $auth.query)
...
export def "recommendations get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/recommendations" $auth.query)
