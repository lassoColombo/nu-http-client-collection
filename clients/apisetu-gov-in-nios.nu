# Auto-generated client for National Institute of Open Schooling v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/nios/3.0.0/openapi.json
# Auth: --token flag or $env.NATIONAL_INSTITUTE_OF_OPEN_SCHOOLING_TOKEN

const BASE_URL = "https://apisetu.gov.in/nios/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o NATIONAL_INSTITUTE_OF_OPEN_SCHOOLING_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/nios/v3"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "hscer-certificate create" } } | get name | first)
...
# operationId: hscer
# --certificateParameters shape: {FullName: string, RROLL: string, YEAR: string}
...
export def "hscer-certificate create" [
  ...
  --certificate-parameters: record # shape: {FullName: string, RROLL: string, YEAR: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o NATIONAL_INSTITUTE_OF_OPEN_SCHOOLING_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o NATIONAL_INSTITUTE_OF_OPEN_SCHOOLING_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
...
export def "hvcer-certificate create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o NATIONAL_INSTITUTE_OF_OPEN_SCHOOLING_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o NATIONAL_INSTITUTE_OF_OPEN_SCHOOLING_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/hvcer/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "svcer-certificate create" [
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o NATIONAL_INSTITUTE_OF_OPEN_SCHOOLING_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o NATIONAL_INSTITUTE_OF_OPEN_SCHOOLING_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/svcer/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
