...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o HIGH_PERFORMANCE_BUILDING_DATABASE_TOKEN | default "" }
...
export def "project-output-format get" [
  ...
  let full_url = (build-url $base ({output_format: (encode-path-segment $output_format)} | format pattern "/project.{output_format}") $qp $auth.query)
  let accept_val = "text/xml"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"api_key": $api_key, "search": $search, "portal": $portal, "page": $page, "city": $city, "province": $province, "region": $region} | compact)
...
export def "project get-document" [
  ...
  let full_url = (build-url $base ({project_id: (encode-path-segment $project_id), output_format: (encode-path-segment $output_format)} | format pattern "/project/{project_id}.{output_format}") $qp $auth.query)
  let accept_val = "text/xml"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
