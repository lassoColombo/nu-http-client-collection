# Auto-generated client for High Performance Building Database v1.0
# Source: https://api.apis.guru/v2/specs/nrel.gov/building-case-studies/1.0/swagger.json
# Auth: --token flag or $env.HIGH_PERFORMANCE_BUILDING_DATABASE_TOKEN

const BASE_URL = "https://localhost/api/building-case-studies"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o HIGH_PERFORMANCE_BUILDING_DATABASE_TOKEN | default "" }
...
def base-url-completer [] { ["https://localhost/api/building-case-studies"] }
...
export def "project-output-format get" [
  ...
  let full_url = (build-url $base ({output_format: (encode-path-segment $output_format)} | format pattern "/project.{output_format}") $qp)
  let accept_val = "text/xml"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api_key": $api_key, "search": $search, "portal": $portal, "page": $page, "city": $city, "province": $province, "region": $region} | compact), body: null}
}
...
export def "project get-document" [
  ...
  let full_url = (build-url $base ({project_id: (encode-path-segment $project_id), output_format: (encode-path-segment $output_format)} | format pattern "/project/{project_id}.{output_format}") $qp)
  let accept_val = "text/xml"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
