...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "x-amz-target-qldb-session-send-command SendCommand" } } | get name | first)
...
export def "x-amz-target-qldb-session-send-command SendCommand" [
  ...
  let body = {SessionToken: $SessionToken, StartSession: $StartSession, StartTransaction: $StartTransaction, EndSession: $EndSession, CommitTransaction: $CommitTransaction, AbortTransaction: $AbortTransaction, ExecuteStatement: $ExecuteStatement, FetchPage: $FetchPage} | compact
