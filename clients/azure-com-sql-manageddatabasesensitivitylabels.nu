...
export def "subscriptions-resource-groups-providers-microsoft-sql-managed-instances-databases-recommended-sensitivity-labels list" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"includeDisabledRecommendations": $include_disabled_recommendations, "$skipToken": $skip_token, "$filter": $filter, "api-version": $api_version} | compact), body: null}
}
