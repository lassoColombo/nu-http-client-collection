# Auto-generated client for Firebase Cloud Messaging API vv1
# Source: https://api.apis.guru/v2/specs/googleapis.com/fcm/v1/openapi.json
# Auth: --token flag or $env.FIREBASE_CLOUD_MESSAGING_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o FIREBASE_CLOUD_MESSAGING_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://fcm.googleapis.com"] }
...
export def "messages-send send" [
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_oauth2 | default ($env | get -o FIREBASE_CLOUD_MESSAGING_API_OAUTH2_TOKEN | default "")) "bearer") (build-auth ($token_oauth2c | default ($env | get -o FIREBASE_CLOUD_MESSAGING_API_OAUTH2C_TOKEN | default "")) "bearer")])
  ...
  let full_url = (build-url $base ({parent: (encode-path-segment $parent)} | format pattern "/v1/{parent}/messages:send") $qp)
  let req_body = {"message": $message, "validateOnly": $validate_only} | compact
