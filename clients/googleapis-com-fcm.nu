...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o FIREBASE_CLOUD_MESSAGING_API_TOKEN | default "" }
...
export def "messages-send send" [
  ...
  let full_url = (build-url $base ({parent: (encode-path-segment $parent)} | format pattern "/v1/{parent}/messages:send") $qp $auth.query)
