...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o GREENWIRE_PUBLIC_API_TOKEN | default "" }
...
export def "events list" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"domain": $domain, "limit": $limit} | compact), body: null}
}
...
export def "volunteers list" [
  ...
  let full_url = (build-url $base "/volunteers" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"domain": $domain, "limit": $limit, "must_have_default_avatar": $must_have_default_avatar} | compact), body: null}
}
