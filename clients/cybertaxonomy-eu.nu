# Auto-generated client for EU BON UTIS v1.0
# Source: https://api.apis.guru/v2/specs/cybertaxonomy.eu/1.0/swagger.json
# Auth: --token flag or $env.EU_BON_UTIS_TOKEN

const BASE_URL = "http://cybertaxonomy.eu/eu-bon/utis/1.0"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o EU_BON_UTIS_TOKEN | default "" }
...
def base-url-completer [] { ["http://cybertaxonomy.eu/eu-bon/utis/1.0"] }
...
]: nothing -> table<copyrightUrl: string, documentationUrl: string, id: string, label: string, searchModes: list<string>, subChecklists: list<any>, version: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/capabilities")
...
export def "search list" [
  ...
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"query": $query, "providers": $providers, "searchMode": $search_mode, "addSynonymy": $add_synonymy, "timeout": $timeout} | compact), body: null}
}
