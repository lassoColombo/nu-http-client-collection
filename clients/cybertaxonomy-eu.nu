...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o EU_BON_UTIS_TOKEN | default "" }
...
export def "capabilities get" [
  ...
  let full_url = (build-url $base "/capabilities" $auth.query)
...
export def "search list" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"query": $query, "providers": $providers, "searchMode": $search_mode, "addSynonymy": $add_synonymy, "timeout": $timeout} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
