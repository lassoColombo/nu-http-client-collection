# Auto-generated client for CoinGecko Demo API v3.0.0
# Source: https://raw.githubusercontent.com/coingecko/coingecko-api-oas/main/demo-api.json
# Auth: --token flag or $env.COINGECKO_DEMO_API_TOKEN

const BASE_URL = "https://api.coingecko.com/api/v3"
const DEFAULT_AUTH = "x-cg-demo-api-key"

# Build auth: returns {headers: record, query: string}
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o COINGECKO_DEMO_API_TOKEN | default "" }
  let scheme = ($auth_scheme | default "bearer")
  if ($scheme == "none") or ($token_val | is-empty) { return {headers: {}, query: ""} }
  match $scheme {
    "x-cg-demo-api-key" => { {headers: {x-cg-demo-api-key: $token_val}, query: ""} }
    "query-x_cg_demo_api_key" => { {headers: {}, query: $"x_cg_demo_api_key=($token_val)"} }
    "none" => { {headers: {}, query: ""} }
...
def base-url-completer [] { ["https://api.coingecko.com/api/v3"] }
def auth-scheme-completer [] { ["x-cg-demo-api-key" "query-x_cg_demo_api_key"] }
...
def interval-completer [] { ["daily" "hourly"] }
def days-completer [] { ["1" "14" "180" "30" "365" "7" "90"] }
...
def duration-completer [] { ["1h" "24h" "5m" "6h"] }
def sort-completer [] { ["h24_tx_count_desc" "h24_volume_usd_desc"] }
def sort-completer-1 [] { ["h24_tx_count_desc" "h24_volume_usd_desc" "h24_volume_usd_liquidity_desc"] }
...
export def "coins-market-chart-range coins-id-market-chart-range" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-cg-demo-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "vs_currency" $vs_currency "scalar") (serialize-qp "from" $qp_from "scalar") (serialize-qp "to" $qp_to "scalar") (serialize-qp "precision" $precision "scalar")] | flatten | str join "&"
...
export def "coins-ohlc coins-id-ohlc" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-cg-demo-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "vs_currency" $vs_currency "scalar") (serialize-qp "days" $days "scalar") (serialize-qp "precision" $precision "scalar")] | flatten | str join "&"
...
export def "onchain-networks-tokens-pools top-pools-contract-address" [
  ...
  --page: int # Page through results.  Default value: 1
  --qp-sort: string@sort-completer-1 # Sort the pools by field.  Default: `h24_volume_usd_liquidity_desc`
  --include-gt-community-data: string@bool-completer # Include GeckoTerminal community data (sentiment votes, suspicious reports).  Default: `false`
]: nothing -> record<data: table<id: string, type: string, attributes: record, relationships: record>, included: table<id: string, type: string, attributes: record>> {
  let auth = (build-auth $token ($auth_scheme | default "x-cg-demo-api-key"))
  let base = ($base_url | default $BASE_URL)
