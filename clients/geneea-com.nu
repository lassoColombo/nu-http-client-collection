...
export def "s1-correction get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"id": $id, "text": $text, "url": $url, "extractor": $extractor, "language": $language, "returnTextInfo": $return_text_info} | compact), body: null}
}
