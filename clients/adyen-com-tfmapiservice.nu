...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o POS_TERMINAL_MANAGEMENT_API_TOKEN | default "" }
...
export def "assign-terminals create" [
  ...
  let full_url = (build-url $base "/assignTerminals" $auth.query)
...
export def "find-terminal create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/findTerminal" $auth.query)
...
export def "get-stores-under-account create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/getStoresUnderAccount" $auth.query)
...
export def "get-terminal-details create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/getTerminalDetails" $auth.query)
  let req_body = {"terminal": $terminal} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "get-terminals-under-account create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/getTerminalsUnderAccount" $auth.query)
