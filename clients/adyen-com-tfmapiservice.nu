# Auto-generated client for POS Terminal Management API v1
# Source: https://api.apis.guru/v2/specs/adyen.com/TfmAPIService/1/openapi.json
# Auth: --token flag or $env.POS_TERMINAL_MANAGEMENT_API_TOKEN

const BASE_URL = "https://postfmapi-test.adyen.com/postfmapi/terminal/v1"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o POS_TERMINAL_MANAGEMENT_API_TOKEN | default "" }
