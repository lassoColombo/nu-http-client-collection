# Auto-generated client for Marketplace RP Service v2020-01-01
# Source: https://api.apis.guru/v2/specs/azure.com/marketplace-Marketplace/2020-01-01/swagger.json
# Auth: --token flag or $env.MARKETPLACE_RP_SERVICE_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o MARKETPLACE_RP_SERVICE_TOKEN | default "" }
...
def availability-completer [] { ["disabled" "enabled"] }
...
export def "providers-microsoft-marketplace-operations list" [
  ...
  let full_url = (build-url $base "/providers/Microsoft.Marketplace/operations" $qp)
...
export def "providers-microsoft-marketplace-private-stores list" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # The API version to use for the request.
]: nothing -> record<nextLink: string, value: table<availability: string, name: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/providers/Microsoft.Marketplace/privateStores" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version} | compact), body: null}
}
...
export def "providers-microsoft-marketplace-private-stores delete" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($private_store_id | is-empty) { error make --unspanned { msg: "path parameter 'PrivateStoreId' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({private_store_id: (encode-path-segment $private_store_id)} | format pattern "/providers/Microsoft.Marketplace/privateStores/{private_store_id}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "providers-microsoft-marketplace-private-stores get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # The API version to use for the request.
]: nothing -> record<availability: string, name: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($private_store_id | is-empty) { error make --unspanned { msg: "path parameter 'PrivateStoreId' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
...
export def "providers-microsoft-marketplace-private-stores create-or-update" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # The API version to use for the request.
  --availability: string@availability-completer # Indicates private store availability
  --name: string # Private Store name
]: any -> record<availability: string, name: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($private_store_id | is-empty) { error make --unspanned { msg: "path parameter 'PrivateStoreId' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({private_store_id: (encode-path-segment $private_store_id)} | format pattern "/providers/Microsoft.Marketplace/privateStores/{private_store_id}") $qp)
  let req_body = {"availability": $availability, "name": $name} | compact
...
export def "providers-microsoft-marketplace-private-stores-offers list" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # The API version to use for the request.
]: nothing -> record<nextLink: string, value: table<description: string, displayName: string, eTag: string, id: string, longSummary: string, plans: list, publisherDisplayName: string, summary: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($private_store_id | is-empty) { error make --unspanned { msg: "path parameter 'PrivateStoreId' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({private_store_id: (encode-path-segment $private_store_id)} | format pattern "/providers/Microsoft.Marketplace/privateStores/{private_store_id}/offers") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version} | compact), body: null}
}
...
export def "providers-microsoft-marketplace-private-stores-offers delete" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({private_store_id: (encode-path-segment $private_store_id), offer_id: (encode-path-segment $offer_id)} | format pattern "/providers/Microsoft.Marketplace/privateStores/{private_store_id}/offers/{offer_id}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api-version": $api_version} | compact), body: null}
}
...
export def "providers-microsoft-marketplace-private-stores-offers get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # The API version to use for the request.
]: nothing -> record<description: string, displayName: string, eTag: string, id: string, longSummary: string, plans: table<displayName: string, planId: string>, publisherDisplayName: string, summary: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($private_store_id | is-empty) { error make --unspanned { msg: "path parameter 'PrivateStoreId' must be non-empty" } }
  if ($offer_id | is-empty) { error make --unspanned { msg: "path parameter 'OfferId' must be non-empty" } }
...
export def "providers-microsoft-marketplace-private-stores-offers create-or-update" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({private_store_id: (encode-path-segment $private_store_id), offer_id: (encode-path-segment $offer_id)} | format pattern "/providers/Microsoft.Marketplace/privateStores/{private_store_id}/offers/{offer_id}") $qp)
  let req_body = {"eTag": $e_tag} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"api-version": $api_version} | compact), body: $req_body}
