...
export def "providers-microsoft-security-assessments Delete" [
  ...
  let full_url = (build-url $base $"/($resourceId)/providers/Microsoft.Security/assessments/($assessmentName)" $qp)
...
export def "providers-microsoft-security-assessments CreateOrUpdate" [
  resourceId: string
  assessmentName: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
...
export def "providers-microsoft-security-assessments List" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/($scope)/providers/Microsoft.Security/assessments" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
