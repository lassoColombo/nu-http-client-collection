# Auto-generated client for Pricing API v0.0.3
# Source: https://api.apis.guru/v2/specs/nexmo.com/pricing/0.0.3/openapi.json
# Auth: --token flag or $env.PRICING_API_TOKEN

const BASE_URL = "https://rest.nexmo.com/account"
...
def base-url-completer [] { ["https://rest.nexmo.com/account"] }
...
export def "get-full-pricing-outbound get-list-countries" [
  ...
  --api-key: string # Your Nexmo API key.
  --api-secret: string # Your Nexmo API secret.
]: nothing -> record<count: string, countries: table<countryDisplayName: string, countryName: string, currency: string, defaultPrice: string, dialingPrefix: string, networks: list>> {
  ...
  let full_url = (build-url $base ({type: (encode-path-segment $type)} | format pattern "/get-full-pricing/outbound/{type}") $qp)
...
export def "get-prefix-pricing-outbound get" [
  ...
  let full_url = (build-url $base ({type: (encode-path-segment $type)} | format pattern "/get-prefix-pricing/outbound/{type}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api_key": $api_key, "api_secret": $api_secret, "prefix": $prefix} | compact), body: null}
}
...
export def "get-pricing-outbound get-country" [
...
]: nothing -> record<countryCode: string, countryDisplayName: string, countryName: string, currency: string, defaultPrice: string, dialingPrefix: string, networks: table<currency: string, mcc: string, mnc: string, networkCode: string, networkName: string, price: string, type: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($type | is-empty) { error make --unspanned { msg: "path parameter 'type' must be non-empty" } }
  ...
  let full_url = (build-url $base ({type: (encode-path-segment $type)} | format pattern "/get-pricing/outbound/{type}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"api_key": $api_key, "api_secret": $api_secret, "country": $country} | compact), body: null}
}
