...
export def "get-full-pricing-outbound get-list-countries" [
  ...
  let full_url = (build-url $base ({type: (encode-path-segment $type)} | format pattern "/get-full-pricing/outbound/{type}") $qp $auth.query)
...
export def "get-prefix-pricing-outbound get" [
  ...
  let full_url = (build-url $base ({type: (encode-path-segment $type)} | format pattern "/get-prefix-pricing/outbound/{type}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"api_key": $api_key, "api_secret": $api_secret, "prefix": $prefix} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "get-pricing-outbound get-country" [
  ...
  let full_url = (build-url $base ({type: (encode-path-segment $type)} | format pattern "/get-pricing/outbound/{type}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"api_key": $api_key, "api_secret": $api_secret, "country": $country} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
