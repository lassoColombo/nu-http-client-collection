...
export def "providers-management-groups-providers-microsoft-policy-insights-remediations-list-deployments list-at" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"$top": $top, "api-version": $api_version} | compact), body: null}
}
