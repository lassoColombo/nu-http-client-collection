...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o GO_UPC_BARCODE_LOOKUP_API_TOKEN | default "" }
...
export def "code get-product" [
  ...
  let full_url = (build-url $base ({code: (encode-path-segment $code)} | format pattern "/code/{code}") $auth.query)
