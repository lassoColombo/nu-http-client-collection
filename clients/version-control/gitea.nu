# Auto-generated client for Gitea API v1.26.0+dev-1029-gc1dc3907a7
# Source: https://gitea.com/swagger.v1.json
...
export def "admin-actions-jobs listAdminWorkflowJobs" [
  ...
  --qp-sort: string # sort jobs by attribute. Supported values are "id". Default is "id"
  --order: string # sort order, either "asc" (ascending) or "desc" (descending). Default is "asc"
]: nothing -> any {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "status" $status "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "limit" $limit "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "order" $order "scalar")] | flatten | str join "&"
...
export def "orgs-repos orgDeleteRepos" [
  org: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
export def "repos repoEdit" [
  ...
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/repos/($owner)/($repo)")
  let body = {allow_fast_forward_only_merge: $allow_fast_forward_only_merge, allow_manual_merge: $allow_manual_merge, allow_merge_commits: $allow_merge_commits, allow_merge_update: $allow_merge_update, allow_rebase: $allow_rebase, allow_rebase_explicit: $allow_rebase_explicit, allow_rebase_update: $allow_rebase_update, allow_squash_merge: $allow_squash_merge, archived: $archived, autodetect_manual_merge: $autodetect_manual_merge, default_allow_maintainer_edit: $default_allow_maintainer_edit, default_branch: $default_branch, default_delete_branch_after_merge: $default_delete_branch_after_merge, default_merge_style: $default_merge_style, default_update_style: $default_update_style, description: $description, enable_prune: $enable_prune, external_tracker: $external_tracker, external_wiki: $external_wiki, has_actions: $has_actions, has_code: $has_code, has_issues: $has_issues, has_packages: $has_packages, has_projects: $has_projects, has_pull_requests: $has_pull_requests, has_releases: $has_releases, has_wiki: $has_wiki, ignore_whitespace_conflicts: $ignore_whitespace_conflicts, internal_tracker: $internal_tracker, mirror_interval: $mirror_interval, mirror_password: $mirror_password, mirror_token: $mirror_token, mirror_username: $mirror_username, name: $name, private: $private, projects_mode: $projects_mode, template: $template, website: $website} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "patch" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "repos-actions-runs-attempts get" [
  owner: string
  repo: string
  run: int
  attempt: int
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/repos/($owner)/($repo)/actions/runs/($run)/attempts/($attempt)")
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "repos-actions-runs-attempts-jobs listWorkflowRunAttemptJobs" [
  owner: string
  repo: string
  run: int
  attempt: int
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "status" $status "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "limit" $limit "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/repos/($owner)/($repo)/actions/runs/($run)/attempts/($attempt)/jobs" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "repos-branch-protections repoEditBranchProtection" [
  ...
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/repos/($owner)/($repo)/branch_protections/($name)")
  let body = {approvals_whitelist_teams: $approvals_whitelist_teams, approvals_whitelist_username: $approvals_whitelist_username, block_admin_merge_override: $block_admin_merge_override, block_on_official_review_requests: $block_on_official_review_requests, block_on_outdated_branch: $block_on_outdated_branch, block_on_rejected_reviews: $block_on_rejected_reviews, bypass_allowlist_teams: $bypass_allowlist_teams, bypass_allowlist_usernames: $bypass_allowlist_usernames, dismiss_stale_approvals: $dismiss_stale_approvals, enable_approvals_whitelist: $enable_approvals_whitelist, enable_bypass_allowlist: $enable_bypass_allowlist, enable_force_push: $enable_force_push, enable_force_push_allowlist: $enable_force_push_allowlist, enable_merge_whitelist: $enable_merge_whitelist, enable_push: $enable_push, enable_push_whitelist: $enable_push_whitelist, enable_status_check: $enable_status_check, force_push_allowlist_deploy_keys: $force_push_allowlist_deploy_keys, force_push_allowlist_teams: $force_push_allowlist_teams, force_push_allowlist_usernames: $force_push_allowlist_usernames, ignore_stale_approvals: $ignore_stale_approvals, merge_whitelist_teams: $merge_whitelist_teams, merge_whitelist_usernames: $merge_whitelist_usernames, priority: $priority, protected_file_patterns: $protected_file_patterns, push_whitelist_deploy_keys: $push_whitelist_deploy_keys, push_whitelist_teams: $push_whitelist_teams, push_whitelist_usernames: $push_whitelist_usernames, require_signed_commits: $require_signed_commits, required_approvals: $required_approvals, status_check_contexts: $status_check_contexts, unprotected_file_patterns: $unprotected_file_patterns} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "patch" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "repos-collaborators repoAddCollaborator" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
  --permission: string@permission-completer # Permission level to grant the collaborator read RepoWritePermissionRead write RepoWritePermissionWrite admin RepoWritePermissionAdmin
]: any -> any {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
...
export def "repos-issues issueCreateIssue" [
  ...
  let body = {assignee: $assignee, assignees: $assignees, body: $body_body, closed: $closed, due_date: $due_date, labels: $labels, milestone: $milestone, projects: $projects, ref: $ref, title: $title} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "repos-issues issueEditIssue" [
  ...
  --projects: list # list of project ids to set (replaces existing projects)
  --ref: string
  --state: string
  --title: string
  ...
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/repos/($owner)/($repo)/issues/($index)")
  let body = {assignee: $assignee, assignees: $assignees, body: $body_body, content_version: $content_version, due_date: $due_date, milestone: $milestone, projects: $projects, ref: $ref, state: $state, title: $title, unset_due_date: $unset_due_date} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "patch" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "repos-pulls-comments-replies repoCreatePullReviewCommentReply" [
  owner: string
  repo: string
  index: int
  id: int
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/repos/($owner)/($repo)/pulls/($index)/comments/($id)/replies")
  let body = {body: $body_body} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
