# Auto-generated client for Forgejo API v11.0.15+gitea-1.22.0
# Source: https://v11.next.forgejo.org/swagger.v1.json
...
def auth-scheme-completer [] { ["query-access_token" "bearer" "basic" "sudo" "query-sudo" "x-forgejo-otp" "query-token"] }
...
def sort-completer-1 [] { ["highestindex" "leastindex" "leastupdate" "oldest" "recentupdate"] }
...
def sort-completer-3 [] { ["leastcomment" "leastupdate" "mostcomment" "oldest" "priority" "recentupdate"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "activitypub-actor activitypubInstanceActor" } } | get name | first)
...
export def "admin-runners-registration-token adminGetRunnerRegistrationToken" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
...
export def "orgs-quota-check orgCheckQuota" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/orgs/($org)/quota/check")
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "repos-actions-workflows-dispatches DispatchWorkflow" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/repos/($owner)/($repo)/actions/workflows/($workflowname)/dispatches")
...
export def "repos-pulls repoListPullRequests" [
  ...
  --qp-sort: string@sort-completer-3 # Type of sort
...
export def "repos-push-mirrors repoAddPushMirror" [
  ...
  let body = {interval: $interval, remote_address: $remote_address, remote_password: $remote_password, remote_username: $remote_username, sync_on_commit: $sync_on_commit, use_ssh: $use_ssh} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "repos-statuses repoCreateStatus" [
  ...
  --context: string
  --description: string
  --state: string # CommitStatusState holds the state of a CommitStatus It can be "pending", "success", "error" and "failure"
  --target-url: string
]: any -> any {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "basic"))
...
export def "user-quota-check userCheckQuota" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/user/quota/check")
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
