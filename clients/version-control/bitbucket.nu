...
export def "repositories-pullrequests-tasks post" [
  ...
  --comment: any
  --pending: string@bool-completer
]: any -> record<comment: record<type: string, id: int, created_on: string, updated_on: string, content: record<raw: string, markup: string, html: string>, user: record<type: string, links: record, created_on: string, display_name: string, uuid: string>, deleted: bool, parent: record<type: string, id: int, created_on: string, updated_on: string, content: record, user: record, deleted: bool, parent: record, inline: record, links: record>, inline: record<from: int, to: int, start_from: int, start_to: int, path: string>, links: record<self: record, html: record, code: record>>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
