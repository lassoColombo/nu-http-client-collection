...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o RADARR_TOKEN | default "" }
...
def monitor-completer [] { ["movieAndCollection" "movieOnly" "none"] }
def listType-completer [] { ["advanced" "other" "plex" "program" "simkl" "tmdb" "trakt"] }
def downloadPropersAndRepacks-completer [] { ["doNotPrefer" "doNotUpgrade" "preferAndUpgrade"] }
def fileDate-completer [] { ["cinemas" "none" "release"] }
def rescanAfterRefresh-completer [] { ["afterManual" "always" "never"] }
def certificationCountry-completer [] { ["au" "br" "ca" "de" "es" "fr" "gb" "ie" "in" "it" "nz" "ro" "us"] }
def status-completer-1 [] { ["announced" "deleted" "inCinemas" "released" "tba"] }
def colonReplacementFormat-completer [] { ["dash" "delete" "smart" "spaceDash" "spaceDashSpace"] }
def movieRuntimeFormat-completer [] { ["hoursMinutes" "minutes"] }

# List all available API commands with their parameters
export def commands []: nothing -> table {
  let builtin_flags = ["base-url" "token" "auth-scheme" "insecure" "max-time" "raw" "allow-errors" "accept" "help"]
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "alttitle list" } } | get name | first)
...
export def "alttitle list" [
  ...
  --movieMetadataId: int # format: int32
]: nothing -> table<id: int, sourceType: string, movieMetadataId: int, title: string, cleanTitle: string> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "movieId" $movieId "scalar") (serialize-qp "movieMetadataId" $movieMetadataId "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/api/v3/alttitle" $qp)
...
export def "alttitle get" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --accept: string@accept-completer # Response content type
]: nothing -> record<id: int, sourceType: string, movieMetadataId: int, title: string, cleanTitle: string> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/alttitle/($id)")
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "blocklist get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "page" $page "scalar") (serialize-qp "pageSize" $pageSize "scalar") (serialize-qp "sortKey" $sortKey "scalar") (serialize-qp "sortDirection" $sortDirection "scalar") (serialize-qp "movieIds" $movieIds "multi") (serialize-qp "protocols" $protocols "multi")] | flatten | str join "&"
...
export def "blocklist-movie get" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
...
export def "calendar get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "start" $start "scalar") (serialize-qp "end" $end "scalar") (serialize-qp "unmonitored" $unmonitored "scalar") (serialize-qp "tags" $tags "scalar")] | flatten | str join "&"
...
export def "feed-calendar-radarrics get" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pastDays" $pastDays "scalar") (serialize-qp "futureDays" $futureDays "scalar") (serialize-qp "tags" $tags "scalar") (serialize-qp "unmonitored" $unmonitored "scalar") (serialize-qp "releaseTypes" $releaseTypes "multi")] | flatten | str join "&"
  let full_url = (build-url $base "/feed/v3/calendar/radarr.ics" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
]: nothing -> table<id: int, title: string, sortTitle: string, tmdbId: int, images: list<record>, overview: string, monitored: bool, rootFolderPath: string, qualityProfileId: int, searchOnAdd: bool, minimumAvailability: string, movies: list<record>, missingMovies: int, tags: list<int>> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "tmdbId" $tmdbId "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/api/v3/collection" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "collection put" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/collection")
  let body = {collectionIds: $collectionIds, monitored: $monitored, monitorMovies: $monitorMovies, searchOnAdd: $searchOnAdd, qualityProfileId: $qualityProfileId, rootFolderPath: $rootFolderPath, minimumAvailability: $minimumAvailability} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
# --movies item shape: {tmdbId?: int, imdbId?: string, title?: string, cleanTitle?: string, sortTitle?: string, status?: "tba"|"announced"|"inCinemas"|"released"|"deleted", overview?: string, runtime?: int, images?: list, year?: int, ratings?: record, genres?: list, folder?: string, isExisting?: bool, isExcluded?: bool}
export def "collection put-by-id" [
  id: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  --tags: list # nullable
]: any -> record<id: int, title: string, sortTitle: string, tmdbId: int, images: table<coverType: string, url: string, remoteUrl: string>, overview: string, monitored: bool, rootFolderPath: string, qualityProfileId: int, searchOnAdd: bool, minimumAvailability: string, movies: table<tmdbId: int, imdbId: string, title: string, cleanTitle: string, sortTitle: string, status: string, overview: string, runtime: int, images: list, year: int, ratings: record, genres: list, folder: string, isExisting: bool, isExcluded: bool>, missingMovies: int, tags: list<int>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/collection/($id)")
  let body = {id: $body_id, title: $title, sortTitle: $sortTitle, tmdbId: $tmdbId, images: $images, overview: $overview, monitored: $monitored, rootFolderPath: $rootFolderPath, qualityProfileId: $qualityProfileId, searchOnAdd: $searchOnAdd, minimumAvailability: $minimumAvailability, movies: $movies, missingMovies: $missingMovies, tags: $tags} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "collection get" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --accept: string@accept-completer # Response content type
]: nothing -> record<id: int, title: string, sortTitle: string, tmdbId: int, images: table<coverType: string, url: string, remoteUrl: string>, overview: string, monitored: bool, rootFolderPath: string, qualityProfileId: int, searchOnAdd: bool, minimumAvailability: string, movies: table<tmdbId: int, imdbId: string, title: string, cleanTitle: string, sortTitle: string, status: string, overview: string, runtime: int, images: list, year: int, ratings: record, genres: list, folder: string, isExisting: bool, isExcluded: bool>, missingMovies: int, tags: list<int>> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/collection/($id)")
  let accept_val = ($accept | default "text/plain")
...
export def "credit list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "movieId" $movieId "scalar") (serialize-qp "movieMetadataId" $movieMetadataId "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/api/v3/credit" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "credit get" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --accept: string@accept-completer # Response content type
]: nothing -> record<id: int, personName: string, creditTmdbId: string, personTmdbId: int, movieMetadataId: int, images: table<coverType: string, url: string, remoteUrl: string>, department: string, job: string, character: string, order: int, type: string> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/credit/($id)")
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "wanted-cutoff get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "page" $page "scalar") (serialize-qp "pageSize" $pageSize "scalar") (serialize-qp "sortKey" $sortKey "scalar") (serialize-qp "sortDirection" $sortDirection "scalar") (serialize-qp "monitored" $monitored "scalar")] | flatten | str join "&"
...
export def "config-downloadclient list" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
]: nothing -> record<id: int, downloadClientWorkingFolders: string, enableCompletedDownloadHandling: bool, checkForFinishedDownloadInterval: int, autoRedownloadFailed: bool, autoRedownloadFailedFromInteractiveSearch: bool> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
export def "config-downloadclient put" [
  ...
  let body = {id: $body_id, downloadClientWorkingFolders: $downloadClientWorkingFolders, enableCompletedDownloadHandling: $enableCompletedDownloadHandling, checkForFinishedDownloadInterval: $checkForFinishedDownloadInterval, autoRedownloadFailed: $autoRedownloadFailed, autoRedownloadFailedFromInteractiveSearch: $autoRedownloadFailedFromInteractiveSearch} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "extrafile get" [
  ...
  --accept: string@accept-completer # Response content type
  --movieId: int # format: int32
]: nothing -> table<id: int, movieId: int, movieFileId: int, relativePath: string, extension: string, languageTags: list<string>, title: string, type: string> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "movieId" $movieId "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/api/v3/extrafile" $qp)
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "history get" [
  ...
  --languages: list
  --quality: list
]: nothing -> record<page: int, pageSize: int, sortKey: string, sortDirection: string, totalRecords: int, records: table<id: int, movieId: int, sourceTitle: string, languages: list, quality: record, customFormats: list, customFormatScore: int, qualityCutoffNotMet: bool, date: string, downloadId: string, eventType: string, data: record, movie: record>> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "page" $page "scalar") (serialize-qp "pageSize" $pageSize "scalar") (serialize-qp "sortKey" $sortKey "scalar") (serialize-qp "sortDirection" $sortDirection "scalar") (serialize-qp "includeMovie" $includeMovie "scalar") (serialize-qp "eventType" $eventType "multi") (serialize-qp "downloadId" $downloadId "scalar") (serialize-qp "movieIds" $movieIds "multi") (serialize-qp "languages" $languages "multi") (serialize-qp "quality" $quality "multi")] | flatten | str join "&"
...
export def "history-since get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "date" $date "scalar") (serialize-qp "eventType" $eventType "scalar") (serialize-qp "includeMovie" $includeMovie "scalar")] | flatten | str join "&"
...
export def "history-movie get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "movieId" $movieId "scalar") (serialize-qp "eventType" $eventType "scalar") (serialize-qp "includeMovie" $includeMovie "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/api/v3/history/movie" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "importlist list" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
]: nothing -> table<id: int, name: string, fields: list<record>, implementationName: string, implementation: string, configContract: string, infoLink: string, message: record<message: string, type: string>, tags: list<int>, presets: list<any>, enabled: bool, enableAuto: bool, monitor: string, rootFolderPath: string, qualityProfileId: int, searchOnAdd: bool, minimumAvailability: string, listType: string, listOrder: int, minRefreshInterval: string> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
export def "importlist post" [
  ...
  let body = {id: $id, name: $name, fields: $body_fields, implementationName: $implementationName, implementation: $implementation, configContract: $configContract, infoLink: $infoLink, message: $message, tags: $tags, presets: $presets, enabled: $enabled, enableAuto: $enableAuto, monitor: $monitor, rootFolderPath: $rootFolderPath, qualityProfileId: $qualityProfileId, searchOnAdd: $searchOnAdd, minimumAvailability: $minimumAvailability, listType: $listType, listOrder: $listOrder, minRefreshInterval: $minRefreshInterval} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "importlist-bulk put" [
  ...
  let body = {ids: $ids, tags: $tags, applyTags: $applyTags, enabled: $enabled, enableAuto: $enableAuto, rootFolderPath: $rootFolderPath, qualityProfileId: $qualityProfileId, minimumAvailability: $minimumAvailability} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "config-importlist list" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
]: nothing -> record<id: int, listSyncLevel: string> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
export def "config-importlist put" [
  ...
  --accept: string@accept-completer # Response content type
  --body-id: int # format: int32
  --listSyncLevel: string # nullable
]: any -> record<id: int, listSyncLevel: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/config/importlist/($id)")
  let body = {id: $body_id, listSyncLevel: $listSyncLevel} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "exclusions list" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
]: nothing -> table<id: int, name: string, fields: list<record>, implementationName: string, implementation: string, configContract: string, infoLink: string, message: record<message: string, type: string>, tags: list<int>, presets: list<any>, tmdbId: int, movieTitle: string, movieYear: int> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/exclusions")
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
# --fields item shape: {order?: int, name?: string, label?: string, unit?: string, helpText?: string, helpTextWarning?: string, helpLink?: string, value?: any, type?: string, advanced?: bool, selectOptions?: list, selectOptionsProviderAction?: string, section?: string, hidden?: string, privacy?: "normal"|"password"|"apiKey"|"userName", placeholder?: string, isFloat?: bool}
# --message shape: {message?: string, type?: "info"|"warning"|"error"}
# --presets item shape: {id?: int, name?: string, fields?: list, implementationName?: string, implementation?: string, configContract?: string, infoLink?: string, message?: record, tags?: list, presets?: list, tmdbId?: int, movieTitle?: string, movieYear?: int}
export def "exclusions post" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --message: record # shape: {message?: string, type?: "info"|"warning"|"error"}
  --tags: list # nullable
  --presets: list # nullable — item shape: {id?: int, name?: string, fields?: list, implementationName?: string, implementation?: string, configContract?: string, infoLink?: string, message?: record, tags?: list, presets?: list, tmdbId?: int, movieTitle?: string, movieYear?: int}
  --tmdbId: int # format: int32
  --movieTitle: string # nullable
  --movieYear: int # format: int32
]: any -> record<id: int, name: string, fields: table<order: int, name: string, label: string, unit: string, helpText: string, helpTextWarning: string, helpLink: string, value: any, type: string, advanced: bool, selectOptions: list, selectOptionsProviderAction: string, section: string, hidden: string, privacy: string, placeholder: string, isFloat: bool>, implementationName: string, implementation: string, configContract: string, infoLink: string, message: record<message: string, type: string>, tags: list<int>, presets: list<any>, tmdbId: int, movieTitle: string, movieYear: int> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/exclusions")
  let body = {id: $id, name: $name, fields: $body_fields, implementationName: $implementationName, implementation: $implementation, configContract: $configContract, infoLink: $infoLink, message: $message, tags: $tags, presets: $presets, tmdbId: $tmdbId, movieTitle: $movieTitle, movieYear: $movieYear} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "exclusions-paged get" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let full_url = (build-url $base "/api/v3/exclusions/paged" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "exclusions put" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/exclusions/($id)")
  let body = {id: $body_id, name: $name, fields: $body_fields, implementationName: $implementationName, implementation: $implementation, configContract: $configContract, infoLink: $infoLink, message: $message, tags: $tags, presets: $presets, tmdbId: $tmdbId, movieTitle: $movieTitle, movieYear: $movieYear} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "exclusions-bulk post" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/exclusions/bulk")
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "exclusions-bulk delete" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
...
export def "importlist-movie get" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "includeRecommendations" $includeRecommendations "scalar") (serialize-qp "includeTrending" $includeTrending "scalar") (serialize-qp "includePopular" $includePopular "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/api/v3/importlist/movie" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "importlist-movie post" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/importlist/movie")
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "config-indexer list" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
]: nothing -> record<id: int, minimumAge: int, maximumSize: int, retention: int, rssSyncInterval: int, preferIndexerFlags: bool, availabilityDelay: int, allowHardcodedSubs: bool, whitelistedHardcodedSubs: string> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
export def "config-indexer put" [
  ...
  let body = {id: $body_id, minimumAge: $minimumAge, maximumSize: $maximumSize, retention: $retention, rssSyncInterval: $rssSyncInterval, preferIndexerFlags: $preferIndexerFlags, availabilityDelay: $availabilityDelay, allowHardcodedSubs: $allowHardcodedSubs, whitelistedHardcodedSubs: $whitelistedHardcodedSubs} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "manualimport get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "folder" $folder "scalar") (serialize-qp "downloadId" $downloadId "scalar") (serialize-qp "movieId" $movieId "scalar") (serialize-qp "filterExistingFiles" $filterExistingFiles "scalar")] | flatten | str join "&"
...
export def "mediacover get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/mediacover/($movieId)/($filename)")
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "config-metadata list" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
]: nothing -> record<id: int, certificationCountry: string> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/config/metadata")
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "config-metadata put" [
  id: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  --accept: string@accept-completer # Response content type
  --body-id: int # format: int32
  --certificationCountry: string@certificationCountry-completer
]: any -> record<id: int, certificationCountry: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/config/metadata/($id)")
  let body = {id: $body_id, certificationCountry: $certificationCountry} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "config-metadata get" [
  id: int
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
export def "movie list" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "tmdbId" $tmdbId "scalar") (serialize-qp "excludeLocalCovers" $excludeLocalCovers "scalar") (serialize-qp "languageId" $languageId "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/api/v3/movie" $qp)
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
# --alternateTitles item shape: {id?: int, sourceType?: "tmdb"|"mappings"|"user"|"indexer", movieMetadataId?: int, title?: string, cleanTitle?: string}
# --images item shape: {coverType?: "unknown"|"poster"|"banner"|"fanart"|"screenshot"|"headshot"|"clearlogo", url?: string, remoteUrl?: string}
# --addOptions shape: {ignoreEpisodesWithFiles?: bool, ignoreEpisodesWithoutFiles?: bool, monitor?: "movieOnly"|"movieAndCollection"|"none", searchForMovie?: bool, addMethod?: "manual"|"list"|"collection"}
# --ratings shape: {imdb?: record, tmdb?: record, metacritic?: record, rottenTomatoes?: record, trakt?: record}
# --movieFile shape: {id?: int, movieId?: int, relativePath?: string, path?: string, size?: int, dateAdded?: string, sceneName?: string, releaseGroup?: string, edition?: string, languages?: list, quality?: record, customFormats?: list, customFormatScore?: int, indexerFlags?: int, mediaInfo?: record, originalFilePath?: string, qualityCutoffNotMet?: bool}
# --collection shape: {title?: string, tmdbId?: int}
# --statistics shape: {movieFileCount?: int, sizeOnDisk?: int, releaseGroups?: list}
export def "movie post" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  --alternateTitles: list # nullable — item shape: {id?: int, sourceType?: "tmdb"|"mappings"|"user"|"indexer", movieMetadataId?: int, title?: string, cleanTitle?: string}
  --secondaryYear: int # nullable, format: int32
  --secondaryYearSourceId: int # format: int32
  --sortTitle: string # nullable
  --sizeOnDisk: int # nullable, format: int64
  --status: string@status-completer-1
  --overview: string # nullable
  --inCinemas: string # nullable, format: date-time
  --physicalRelease: string # nullable, format: date-time
  --digitalRelease: string # nullable, format: date-time
  --releaseDate: string # nullable, format: date-time
  --physicalReleaseNote: string # nullable
  --images: list # nullable — item shape: {coverType?: "unknown"|"poster"|"banner"|"fanart"|"screenshot"|"headshot"|"clearlogo", url?: string, remoteUrl?: string}
  --website: string # nullable
  --remotePoster: string # nullable
  --year: int # format: int32
  --youTubeTrailerId: string # nullable
  ...
  --keywords: list # nullable
  --tags: list # nullable
  --added: string # format: date-time
  --addOptions: record # shape: {ignoreEpisodesWithFiles?: bool, ignoreEpisodesWithoutFiles?: bool, monitor?: "movieOnly"|"movieAndCollection"|"none", searchForMovie?: bool, addMethod?: "manual"|"list"|"collection"}
  --ratings: record # shape: {imdb?: record, tmdb?: record, metacritic?: record, rottenTomatoes?: record, trakt?: record}
  --movieFile: record # shape: {id?: int, movieId?: int, relativePath?: string, path?: string, size?: int, dateAdded?: string, sceneName?: string, releaseGroup?: string, edition?: string, languages?: list, quality?: record, customFormats?: list, customFormatScore?: int, indexerFlags?: int, mediaInfo?: record, originalFilePath?: string, qualityCutoffNotMet?: bool}
  --collection: record # shape: {title?: string, tmdbId?: int}
  --popularity: float # format: float
  --lastSearchTime: string # nullable, format: date-time
  --statistics: record # shape: {movieFileCount?: int, sizeOnDisk?: int, releaseGroups?: list}
]: any -> record<id: int, title: string, originalTitle: string, originalLanguage: record<id: int, name: string>, alternateTitles: table<id: int, sourceType: string, movieMetadataId: int, title: string, cleanTitle: string>, secondaryYear: int, secondaryYearSourceId: int, sortTitle: string, sizeOnDisk: int, status: string, overview: string, inCinemas: string, physicalRelease: string, digitalRelease: string, releaseDate: string, physicalReleaseNote: string, images: table<coverType: string, url: string, remoteUrl: string>, website: string, remotePoster: string, year: int, youTubeTrailerId: string, studio: string, path: string, qualityProfileId: int, hasFile: bool, movieFileId: int, monitored: bool, minimumAvailability: string, isAvailable: bool, folderName: string, runtime: int, cleanTitle: string, imdbId: string, tmdbId: int, titleSlug: string, rootFolderPath: string, folder: string, certification: string, genres: list<string>, keywords: list<string>, tags: list<int>, added: string, addOptions: record<ignoreEpisodesWithFiles: bool, ignoreEpisodesWithoutFiles: bool, monitor: string, searchForMovie: bool, addMethod: string>, ratings: record<imdb: record<votes: int, value: float, type: string>, tmdb: record<votes: int, value: float, type: string>, metacritic: record<votes: int, value: float, type: string>, rottenTomatoes: record<votes: int, value: float, type: string>, trakt: record<votes: int, value: float, type: string>>, movieFile: record<id: int, movieId: int, relativePath: string, path: string, size: int, dateAdded: string, sceneName: string, releaseGroup: string, edition: string, languages: list<record>, quality: record<quality: record, revision: record>, customFormats: list<record>, customFormatScore: int, indexerFlags: int, mediaInfo: record<id: int, audioBitrate: int, audioChannels: float, audioCodec: string, audioLanguages: string, audioStreamCount: int, videoBitDepth: int, videoBitrate: int, videoCodec: string, videoFps: float, videoDynamicRange: string, videoDynamicRangeType: string, resolution: string, runTime: string, scanType: string, subtitles: string>, originalFilePath: string, qualityCutoffNotMet: bool>, collection: record<title: string, tmdbId: int>, popularity: float, lastSearchTime: string, statistics: record<movieFileCount: int, sizeOnDisk: int, releaseGroups: list<string>>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/movie")
  let body = {id: $id, title: $title, originalTitle: $originalTitle, originalLanguage: $originalLanguage, alternateTitles: $alternateTitles, secondaryYear: $secondaryYear, secondaryYearSourceId: $secondaryYearSourceId, sortTitle: $sortTitle, sizeOnDisk: $sizeOnDisk, status: $status, overview: $overview, inCinemas: $inCinemas, physicalRelease: $physicalRelease, digitalRelease: $digitalRelease, releaseDate: $releaseDate, physicalReleaseNote: $physicalReleaseNote, images: $images, website: $website, remotePoster: $remotePoster, year: $year, youTubeTrailerId: $youTubeTrailerId, studio: $studio, path: $path, qualityProfileId: $qualityProfileId, hasFile: $hasFile, movieFileId: $movieFileId, monitored: $monitored, minimumAvailability: $minimumAvailability, isAvailable: $isAvailable, folderName: $folderName, runtime: $runtime, cleanTitle: $cleanTitle, imdbId: $imdbId, tmdbId: $tmdbId, titleSlug: $titleSlug, rootFolderPath: $rootFolderPath, folder: $folder, certification: $certification, genres: $genres, keywords: $keywords, tags: $tags, added: $added, addOptions: $addOptions, ratings: $ratings, movieFile: $movieFile, collection: $collection, popularity: $popularity, lastSearchTime: $lastSearchTime, statistics: $statistics} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "movie put" [
  ...
  let full_url = (build-url $base $"/api/v3/movie/($id)" $qp)
  let body = {id: $body_id, title: $title, originalTitle: $originalTitle, originalLanguage: $originalLanguage, alternateTitles: $alternateTitles, secondaryYear: $secondaryYear, secondaryYearSourceId: $secondaryYearSourceId, sortTitle: $sortTitle, sizeOnDisk: $sizeOnDisk, status: $status, overview: $overview, inCinemas: $inCinemas, physicalRelease: $physicalRelease, digitalRelease: $digitalRelease, releaseDate: $releaseDate, physicalReleaseNote: $physicalReleaseNote, images: $images, website: $website, remotePoster: $remotePoster, year: $year, youTubeTrailerId: $youTubeTrailerId, studio: $studio, path: $path, qualityProfileId: $qualityProfileId, hasFile: $hasFile, movieFileId: $movieFileId, monitored: $monitored, minimumAvailability: $minimumAvailability, isAvailable: $isAvailable, folderName: $folderName, runtime: $runtime, cleanTitle: $cleanTitle, imdbId: $imdbId, tmdbId: $tmdbId, titleSlug: $titleSlug, rootFolderPath: $rootFolderPath, folder: $folder, certification: $certification, genres: $genres, keywords: $keywords, tags: $tags, added: $added, addOptions: $addOptions, ratings: $ratings, movieFile: $movieFile, collection: $collection, popularity: $popularity, lastSearchTime: $lastSearchTime, statistics: $statistics} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "movie delete" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "deleteFiles" $deleteFiles "scalar") (serialize-qp "addImportExclusion" $addImportExclusion "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/api/v3/movie/($id)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "delete" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
...
export def "movie get" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --accept: string@accept-completer # Response content type
]: nothing -> record<id: int, title: string, originalTitle: string, originalLanguage: record<id: int, name: string>, alternateTitles: table<id: int, sourceType: string, movieMetadataId: int, title: string, cleanTitle: string>, secondaryYear: int, secondaryYearSourceId: int, sortTitle: string, sizeOnDisk: int, status: string, overview: string, inCinemas: string, physicalRelease: string, digitalRelease: string, releaseDate: string, physicalReleaseNote: string, images: table<coverType: string, url: string, remoteUrl: string>, website: string, remotePoster: string, year: int, youTubeTrailerId: string, studio: string, path: string, qualityProfileId: int, hasFile: bool, movieFileId: int, monitored: bool, minimumAvailability: string, isAvailable: bool, folderName: string, runtime: int, cleanTitle: string, imdbId: string, tmdbId: int, titleSlug: string, rootFolderPath: string, folder: string, certification: string, genres: list<string>, keywords: list<string>, tags: list<int>, added: string, addOptions: record<ignoreEpisodesWithFiles: bool, ignoreEpisodesWithoutFiles: bool, monitor: string, searchForMovie: bool, addMethod: string>, ratings: record<imdb: record<votes: int, value: float, type: string>, tmdb: record<votes: int, value: float, type: string>, metacritic: record<votes: int, value: float, type: string>, rottenTomatoes: record<votes: int, value: float, type: string>, trakt: record<votes: int, value: float, type: string>>, movieFile: record<id: int, movieId: int, relativePath: string, path: string, size: int, dateAdded: string, sceneName: string, releaseGroup: string, edition: string, languages: list<record>, quality: record<quality: record, revision: record>, customFormats: list<record>, customFormatScore: int, indexerFlags: int, mediaInfo: record<id: int, audioBitrate: int, audioChannels: float, audioCodec: string, audioLanguages: string, audioStreamCount: int, videoBitDepth: int, videoBitrate: int, videoCodec: string, videoFps: float, videoDynamicRange: string, videoDynamicRangeType: string, resolution: string, runTime: string, scanType: string, subtitles: string>, originalFilePath: string, qualityCutoffNotMet: bool>, collection: record<title: string, tmdbId: int>, popularity: float, lastSearchTime: string, statistics: record<movieFileCount: int, sizeOnDisk: int, releaseGroups: list<string>>> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/movie/($id)")
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "movie-editor put" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/movie/editor")
  let body = {movieIds: $movieIds, monitored: $monitored, qualityProfileId: $qualityProfileId, minimumAvailability: $minimumAvailability, rootFolderPath: $rootFolderPath, tags: $tags, applyTags: $applyTags, moveFiles: $moveFiles, deleteFiles: $deleteFiles, addImportExclusion: $addImportExclusion} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "movie-editor delete" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
...
export def "moviefile list" [
  ...
  --movieId: list
  --movieFileIds: list
]: nothing -> table<id: int, movieId: int, relativePath: string, path: string, size: int, dateAdded: string, sceneName: string, releaseGroup: string, edition: string, languages: list<record>, quality: record<quality: record, revision: record>, customFormats: list<record>, customFormatScore: int, indexerFlags: int, mediaInfo: record<id: int, audioBitrate: int, audioChannels: float, audioCodec: string, audioLanguages: string, audioStreamCount: int, videoBitDepth: int, videoBitrate: int, videoCodec: string, videoFps: float, videoDynamicRange: string, videoDynamicRangeType: string, resolution: string, runTime: string, scanType: string, subtitles: string>, originalFilePath: string, qualityCutoffNotMet: bool> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "movieId" $movieId "multi") (serialize-qp "movieFileIds" $movieFileIds "multi")] | flatten | str join "&"
  let full_url = (build-url $base "/api/v3/moviefile" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "moviefile put" [
  ...
  --customFormatScore: int # nullable, format: int32
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/moviefile/($id)")
  let body = {id: $body_id, movieId: $movieId, relativePath: $relativePath, path: $path, size: $size, dateAdded: $dateAdded, sceneName: $sceneName, releaseGroup: $releaseGroup, edition: $edition, languages: $languages, quality: $quality, customFormats: $customFormats, customFormatScore: $customFormatScore, indexerFlags: $indexerFlags, mediaInfo: $mediaInfo, originalFilePath: $originalFilePath, qualityCutoffNotMet: $qualityCutoffNotMet} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "moviefile get" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --accept: string@accept-completer # Response content type
]: nothing -> record<id: int, movieId: int, relativePath: string, path: string, size: int, dateAdded: string, sceneName: string, releaseGroup: string, edition: string, languages: table<id: int, name: string>, quality: record<quality: record<id: int, name: string, source: string, resolution: int, modifier: string>, revision: record<version: int, real: int, isRepack: bool>>, customFormats: table<id: int, name: string, includeCustomFormatWhenRenaming: bool, specifications: list>, customFormatScore: int, indexerFlags: int, mediaInfo: record<id: int, audioBitrate: int, audioChannels: float, audioCodec: string, audioLanguages: string, audioStreamCount: int, videoBitDepth: int, videoBitrate: int, videoCodec: string, videoFps: float, videoDynamicRange: string, videoDynamicRangeType: string, resolution: string, runTime: string, scanType: string, subtitles: string>, originalFilePath: string, qualityCutoffNotMet: bool> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/moviefile/($id)")
  let accept_val = ($accept | default "text/plain")
...
# --quality shape: {quality?: record, revision?: record}
@deprecated
export def "moviefile-editor put" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/moviefile/editor")
  let body = {movieFileIds: $movieFileIds, languages: $languages, quality: $quality, edition: $edition, releaseGroup: $releaseGroup, sceneName: $sceneName, indexerFlags: $indexerFlags} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
# --languages item shape: {id?: int, name?: string}
# --quality shape: {quality?: record, revision?: record}
export def "moviefile-bulk delete" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/moviefile/bulk")
  let body = {movieFileIds: $movieFileIds, languages: $languages, quality: $quality, edition: $edition, releaseGroup: $releaseGroup, sceneName: $sceneName, indexerFlags: $indexerFlags} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "movie-folder get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/movie/($id)/folder")
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "movie-import post" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --body: record
]: any -> table<id: int, title: string, originalTitle: string, originalLanguage: record<id: int, name: string>, alternateTitles: list<record>, secondaryYear: int, secondaryYearSourceId: int, sortTitle: string, sizeOnDisk: int, status: string, overview: string, inCinemas: string, physicalRelease: string, digitalRelease: string, releaseDate: string, physicalReleaseNote: string, images: list<record>, website: string, remotePoster: string, year: int, youTubeTrailerId: string, studio: string, path: string, qualityProfileId: int, hasFile: bool, movieFileId: int, monitored: bool, minimumAvailability: string, isAvailable: bool, folderName: string, runtime: int, cleanTitle: string, imdbId: string, tmdbId: int, titleSlug: string, rootFolderPath: string, folder: string, certification: string, genres: list<string>, keywords: list<string>, tags: list<int>, added: string, addOptions: record<ignoreEpisodesWithFiles: bool, ignoreEpisodesWithoutFiles: bool, monitor: string, searchForMovie: bool, addMethod: string>, ratings: record<imdb: record, tmdb: record, metacritic: record, rottenTomatoes: record, trakt: record>, movieFile: record<id: int, movieId: int, relativePath: string, path: string, size: int, dateAdded: string, sceneName: string, releaseGroup: string, edition: string, languages: list, quality: record, customFormats: list, customFormatScore: int, indexerFlags: int, mediaInfo: record, originalFilePath: string, qualityCutoffNotMet: bool>, collection: record<title: string, tmdbId: int>, popularity: float, lastSearchTime: string, statistics: record<movieFileCount: int, sizeOnDisk: int, releaseGroups: list>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/movie/import")
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "movie-lookup-tmdb get" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "tmdbId" $tmdbId "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/api/v3/movie/lookup/tmdb" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "movie-lookup-imdb get" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "imdbId" $imdbId "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/api/v3/movie/lookup/imdb" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "movie-lookup get" [
  ...
  let full_url = (build-url $base "/api/v3/movie/lookup" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
export def "config-naming list" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --accept: string@accept-completer # Response content type
]: nothing -> record<id: int, renameMovies: bool, replaceIllegalCharacters: bool, colonReplacementFormat: string, standardMovieFormat: string, movieFolderFormat: string> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
]: any -> record<id: int, renameMovies: bool, replaceIllegalCharacters: bool, colonReplacementFormat: string, standardMovieFormat: string, movieFolderFormat: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/config/naming/($id)")
  let body = {id: $body_id, renameMovies: $renameMovies, replaceIllegalCharacters: $replaceIllegalCharacters, colonReplacementFormat: $colonReplacementFormat, standardMovieFormat: $standardMovieFormat, movieFolderFormat: $movieFolderFormat} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "config-naming-examples get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "renameMovies" $renameMovies "scalar") (serialize-qp "replaceIllegalCharacters" $replaceIllegalCharacters "scalar") (serialize-qp "colonReplacementFormat" $colonReplacementFormat "scalar") (serialize-qp "standardMovieFormat" $standardMovieFormat "scalar") (serialize-qp "movieFolderFormat" $movieFolderFormat "scalar") (serialize-qp "id" $id "scalar") (serialize-qp "resourceName" $resourceName "scalar")] | flatten | str join "&"
...
export def "notification post" [
  ...
  let body = {id: $id, name: $name, fields: $body_fields, implementationName: $implementationName, implementation: $implementation, configContract: $configContract, infoLink: $infoLink, message: $message, tags: $tags, presets: $presets, link: $link, onGrab: $onGrab, onDownload: $onDownload, onUpgrade: $onUpgrade, onRename: $onRename, onMovieAdded: $onMovieAdded, onMovieDelete: $onMovieDelete, onMovieFileDelete: $onMovieFileDelete, onMovieFileDeleteForUpgrade: $onMovieFileDeleteForUpgrade, onHealthIssue: $onHealthIssue, includeHealthWarnings: $includeHealthWarnings, onHealthRestored: $onHealthRestored, onApplicationUpdate: $onApplicationUpdate, onManualInteractionRequired: $onManualInteractionRequired, supportsOnGrab: $supportsOnGrab, supportsOnDownload: $supportsOnDownload, supportsOnUpgrade: $supportsOnUpgrade, supportsOnRename: $supportsOnRename, supportsOnMovieAdded: $supportsOnMovieAdded, supportsOnMovieDelete: $supportsOnMovieDelete, supportsOnMovieFileDelete: $supportsOnMovieFileDelete, supportsOnMovieFileDeleteForUpgrade: $supportsOnMovieFileDeleteForUpgrade, supportsOnHealthIssue: $supportsOnHealthIssue, supportsOnHealthRestored: $supportsOnHealthRestored, supportsOnApplicationUpdate: $supportsOnApplicationUpdate, supportsOnManualInteractionRequired: $supportsOnManualInteractionRequired, testCommand: $testCommand} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "parse get" [
  ...
  --accept: string@accept-completer # Response content type
  --title: string
]: nothing -> record<id: int, title: string, parsedMovieInfo: record<movieTitles: list<string>, originalTitle: string, releaseTitle: string, simpleReleaseTitle: string, quality: record<quality: record, revision: record>, languages: list<record>, releaseGroup: string, releaseHash: string, edition: string, year: int, imdbId: string, tmdbId: int, hardcodedSubs: string, movieTitle: string, primaryMovieTitle: string>, movie: record<id: int, title: string, originalTitle: string, originalLanguage: record<id: int, name: string>, alternateTitles: list<record>, secondaryYear: int, secondaryYearSourceId: int, sortTitle: string, sizeOnDisk: int, status: string, overview: string, inCinemas: string, physicalRelease: string, digitalRelease: string, releaseDate: string, physicalReleaseNote: string, images: list<record>, website: string, remotePoster: string, year: int, youTubeTrailerId: string, studio: string, path: string, qualityProfileId: int, hasFile: bool, movieFileId: int, monitored: bool, minimumAvailability: string, isAvailable: bool, folderName: string, runtime: int, cleanTitle: string, imdbId: string, tmdbId: int, titleSlug: string, rootFolderPath: string, folder: string, certification: string, genres: list<string>, keywords: list<string>, tags: list<int>, added: string, addOptions: record<ignoreEpisodesWithFiles: bool, ignoreEpisodesWithoutFiles: bool, monitor: string, searchForMovie: bool, addMethod: string>, ratings: record<imdb: record, tmdb: record, metacritic: record, rottenTomatoes: record, trakt: record>, movieFile: record<id: int, movieId: int, relativePath: string, path: string, size: int, dateAdded: string, sceneName: string, releaseGroup: string, edition: string, languages: list, quality: record, customFormats: list, customFormatScore: int, indexerFlags: int, mediaInfo: record, originalFilePath: string, qualityCutoffNotMet: bool>, collection: record<title: string, tmdbId: int>, popularity: float, lastSearchTime: string, statistics: record<movieFileCount: int, sizeOnDisk: int, releaseGroups: list>>, languages: table<id: int, name: string>, customFormats: table<id: int, name: string, includeCustomFormatWhenRenaming: bool, specifications: list>, customFormatScore: int> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
#
# --quality shape: {id?: int, name?: string, source?: "unknown"|"cam"|"telesync"|"telecine"|"workprint"|"dvd"|"tv"|"webdl"|"webrip"|"bluray", resolution?: int, modifier?: "none"|"regional"|"screener"|"rawhd"|"brdisk"|"remux"}
export def "qualitydefinition put" [
  ...
  --accept: string@accept-completer # Response content type
  --body-id: int # format: int32
  --quality: record # shape: {id?: int, name?: string, source?: "unknown"|"cam"|"telesync"|"telecine"|"workprint"|"dvd"|"tv"|"webdl"|"webrip"|"bluray", resolution?: int, modifier?: "none"|"regional"|"screener"|"rawhd"|"brdisk"|"remux"}
  --title: string # nullable
...
]: any -> record<id: int, quality: record<id: int, name: string, source: string, resolution: int, modifier: string>, title: string, weight: int, minSize: float, maxSize: float, preferredSize: float> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
export def "qualitydefinition get" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --accept: string@accept-completer # Response content type
]: nothing -> record<id: int, quality: record<id: int, name: string, source: string, resolution: int, modifier: string>, title: string, weight: int, minSize: float, maxSize: float, preferredSize: float> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/qualitydefinition/($id)")
  let accept_val = ($accept | default "text/plain")
...
export def "qualitydefinition list" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --accept: string@accept-completer # Response content type
]: nothing -> table<id: int, quality: record<id: int, name: string, source: string, resolution: int, modifier: string>, title: string, weight: int, minSize: float, maxSize: float, preferredSize: float> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
# --language shape: {id?: int, name?: string}
export def "qualityprofile post" [
  ...
  --language: record # shape: {id?: int, name?: string}
]: any -> record<id: int, name: string, upgradeAllowed: bool, cutoff: int, items: table<id: int, name: string, quality: record, items: list, allowed: bool>, minFormatScore: int, cutoffFormatScore: int, minUpgradeFormatScore: int, formatItems: table<id: int, format: int, name: string, score: int>, language: record<id: int, name: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/qualityprofile")
  let body = {id: $id, name: $name, upgradeAllowed: $upgradeAllowed, cutoff: $cutoff, items: $items, minFormatScore: $minFormatScore, cutoffFormatScore: $cutoffFormatScore, minUpgradeFormatScore: $minUpgradeFormatScore, formatItems: $formatItems, language: $language} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "qualityprofile list" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --accept: string@accept-completer # Response content type
]: nothing -> table<id: int, name: string, upgradeAllowed: bool, cutoff: int, items: list<record>, minFormatScore: int, cutoffFormatScore: int, minUpgradeFormatScore: int, formatItems: list<record>, language: record<id: int, name: string>> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/v3/qualityprofile")
  let accept_val = ($accept | default "text/plain")
...
export def "qualityprofile put" [
  ...
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/qualityprofile/($id)")
  let body = {id: $body_id, name: $name, upgradeAllowed: $upgradeAllowed, cutoff: $cutoff, items: $items, minFormatScore: $minFormatScore, cutoffFormatScore: $cutoffFormatScore, minUpgradeFormatScore: $minUpgradeFormatScore, formatItems: $formatItems, language: $language} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "qualityprofile get" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --accept: string@accept-completer # Response content type
]: nothing -> record<id: int, name: string, upgradeAllowed: bool, cutoff: int, items: table<id: int, name: string, quality: record, items: list, allowed: bool>, minFormatScore: int, cutoffFormatScore: int, minUpgradeFormatScore: int, formatItems: table<id: int, format: int, name: string, score: int>, language: record<id: int, name: string>> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/qualityprofile/($id)")
  let accept_val = ($accept | default "text/plain")
...
export def "queue get" [
  ...
  --quality: list
  --status: list
]: nothing -> record<page: int, pageSize: int, sortKey: string, sortDirection: string, totalRecords: int, records: table<id: int, movieId: int, movie: record, languages: list, quality: record, customFormats: list, customFormatScore: int, size: float, title: string, estimatedCompletionTime: string, added: string, status: string, trackedDownloadStatus: string, trackedDownloadState: string, statusMessages: list, errorMessage: string, downloadId: string, protocol: string, downloadClient: string, downloadClientHasPostImportCategory: bool, indexer: string, outputPath: string, sizeleft: float, timeleft: string>> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "page" $page "scalar") (serialize-qp "pageSize" $pageSize "scalar") (serialize-qp "sortKey" $sortKey "scalar") (serialize-qp "sortDirection" $sortDirection "scalar") (serialize-qp "includeUnknownMovieItems" $includeUnknownMovieItems "scalar") (serialize-qp "includeMovie" $includeMovie "scalar") (serialize-qp "movieIds" $movieIds "multi") (serialize-qp "protocol" $protocol "scalar") (serialize-qp "languages" $languages "multi") (serialize-qp "quality" $quality "multi") (serialize-qp "status" $status "multi")] | flatten | str join "&"
...
export def "queue-details get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "movieId" $movieId "scalar") (serialize-qp "includeMovie" $includeMovie "scalar")] | flatten | str join "&"
...
export def "release post" [
  ...
  let body = {id: $id, guid: $guid, quality: $quality, customFormats: $customFormats, customFormatScore: $customFormatScore, qualityWeight: $qualityWeight, age: $age, ageHours: $ageHours, ageMinutes: $ageMinutes, size: $size, indexerId: $indexerId, indexer: $indexer, releaseGroup: $releaseGroup, subGroup: $subGroup, releaseHash: $releaseHash, title: $title, sceneSource: $sceneSource, movieTitles: $movieTitles, languages: $languages, mappedMovieId: $mappedMovieId, approved: $approved, temporarilyRejected: $temporarilyRejected, rejected: $rejected, tmdbId: $tmdbId, imdbId: $imdbId, rejections: $rejections, publishDate: $publishDate, commentUrl: $commentUrl, downloadUrl: $downloadUrl, infoUrl: $infoUrl, movieRequested: $movieRequested, downloadAllowed: $downloadAllowed, releaseWeight: $releaseWeight, edition: $edition, magnetUrl: $magnetUrl, infoHash: $infoHash, seeders: $seeders, leechers: $leechers, protocol: $protocol, indexerFlags: $indexerFlags, movieId: $movieId, downloadClientId: $downloadClientId, downloadClient: $downloadClient, shouldOverride: $shouldOverride} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "rename get" [
  ...
  --accept: string@accept-completer # Response content type
  --movieId: list
]: nothing -> table<id: int, movieId: int, movieFileId: int, existingPath: string, newPath: string> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "movieId" $movieId "multi")] | flatten | str join "&"
...
export def "tag-detail list" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --accept: string@accept-completer # Response content type
]: nothing -> table<id: int, label: string, delayProfileIds: list<int>, importListIds: list<int>, notificationIds: list<int>, releaseProfileIds: list<int>, indexerIds: list<int>, downloadClientIds: list<int>, autoTagIds: list<int>, movieIds: list<int>> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
export def "tag-detail get" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --accept: string@accept-completer # Response content type
]: nothing -> record<id: int, label: string, delayProfileIds: list<int>, importListIds: list<int>, notificationIds: list<int>, releaseProfileIds: list<int>, indexerIds: list<int>, downloadClientIds: list<int>, autoTagIds: list<int>, movieIds: list<int>> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
...
export def "config-ui put" [
  ...
  --movieRuntimeFormat: string@movieRuntimeFormat-completer
...
]: any -> record<id: int, firstDayOfWeek: int, calendarWeekColumnHeader: string, movieRuntimeFormat: string, shortDateFormat: string, longDateFormat: string, timeFormat: string, showRelativeDates: bool, enableColorImpairedMode: bool, movieInfoLanguage: int, uiLanguage: int, theme: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/config/ui/($id)")
  let body = {id: $body_id, firstDayOfWeek: $firstDayOfWeek, calendarWeekColumnHeader: $calendarWeekColumnHeader, movieRuntimeFormat: $movieRuntimeFormat, shortDateFormat: $shortDateFormat, longDateFormat: $longDateFormat, timeFormat: $timeFormat, showRelativeDates: $showRelativeDates, enableColorImpairedMode: $enableColorImpairedMode, movieInfoLanguage: $movieInfoLanguage, uiLanguage: $uiLanguage, theme: $theme} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $max_time $allow_errors "application/json" $body
...
export def "config-ui get" [
  ...
  --allow-errors(-e) # Return full response without error handling
  --accept: string@accept-completer # Response content type
]: nothing -> record<id: int, firstDayOfWeek: int, calendarWeekColumnHeader: string, movieRuntimeFormat: string, shortDateFormat: string, longDateFormat: string, timeFormat: string, showRelativeDates: bool, enableColorImpairedMode: bool, movieInfoLanguage: int, uiLanguage: int, theme: string> {
  let auth = (build-auth $token ($auth_scheme | default "x-api-key"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base $"/api/v3/config/ui/($id)")
  let accept_val = ($accept | default "text/plain")
