...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o DRIVEBC_S_OPEN511_API_TOKEN | default "" }
...
export def "events get" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"format": $format, "status": $status, "severity": $severity, "jurisdiction": $jurisdiction, "event_type": $event_type, "created": $created, "updated": $updated, "road_name": $road_name, "area_id": $area_id, "bbox": $bbox} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "jurisdiction get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "format" $format "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/jurisdiction" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "jurisdictiongeography get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "format" $format "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/jurisdictiongeography" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
