...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o EX_LIBRIS_APIS_TOKEN | default "" }
...
export def "almaws-task-lists-printouts list" [
  ...
  let full_url = (build-url $base "/almaws/v1/task-lists/printouts" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"letter": $letter, "status": $status, "printer_id": $printer_id, "printout_id": $printout_id, "limit": $limit, "offset": $offset} | compact)
...
export def "almaws-task-lists-printouts create" [
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"letter": $letter, "status": $status, "printer_id": $printer_id, "printout_id": $printout_id, "op": $op} | compact)
    headers: $auth.headers
    body: null
...
export def "almaws-task-lists-printouts get" [
  ...
  let full_url = (build-url $base ({printout_id: (encode-path-segment $printout_id)} | format pattern "/almaws/v1/task-lists/printouts/{printout_id}") $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "almaws-task-lists-printouts create-by-printout-id" [
  ...
  let full_url = (build-url $base ({printout_id: (encode-path-segment $printout_id)} | format pattern "/almaws/v1/task-lists/printouts/{printout_id}") $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "almaws-task-lists-requested-resources get" [
  ...
  let full_url = (build-url $base "/almaws/v1/task-lists/requested-resources" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"library": $library, "circ_desk": $circ_desk, "location": $location, "order_by": $order_by, "direction": $direction, "pickup_inst": $pickup_inst, "reported": $reported, "printed": $printed, "limit": $limit, "offset": $offset} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "almaws-task-lists-requested-resources create" [
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"library": $library, "circ_desk": $circ_desk, "op": $op, "location": $location, "pickup_inst": $pickup_inst, "reported": $reported, "printed": $printed} | compact)
    headers: $auth.headers
    body: null
    content_type: "application/json"
    timeout: ($max_time | default 30min)
...
export def "almaws-task-lists-rs-lending-requests get" [
  ...
  let full_url = (build-url $base "/almaws/v1/task-lists/rs/lending-requests" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"library": $library, "status": $status, "printed": $printed, "reported": $reported, "partner": $partner, "requested_format": $requested_format, "supplied_format": $supplied_format} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "almaws-task-lists-rs-lending-requests create" [
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"library": $library, "op": $op, "status": $status, "printed": $printed, "reported": $reported, "partner": $partner, "requested_format": $requested_format, "supplied_format": $supplied_format} | compact)
    headers: $auth.headers
    body: null
    content_type: "application/json"
    timeout: ($max_time | default 30min)
...
export def "almaws-task-lists-test get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "query-apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/almaws/v1/task-lists/test" $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
