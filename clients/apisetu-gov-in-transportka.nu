# Auto-generated client for Karnataka Department of Transport, Karnataka v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/transportka/3.0.0/openapi.json
# Auth: --token flag or $env.KARNATAKA_DEPARTMENT_OF_TRANSPORT_KARNATAKA_TOKEN

const BASE_URL = "https://apisetu.gov.in/transportka/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o KARNATAKA_DEPARTMENT_OF_TRANSPORT_KARNATAKA_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/transportka/v3"] }
...
# operationId: drvlc
# --certificateParameters shape: {DOB: string, FullName: string, dlno: string}
...
export def "drvlc-certificate create" [
  ...
  --certificate-parameters: record # shape: {DOB: string, FullName: string, dlno: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o KARNATAKA_DEPARTMENT_OF_TRANSPORT_KARNATAKA_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o KARNATAKA_DEPARTMENT_OF_TRANSPORT_KARNATAKA_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
