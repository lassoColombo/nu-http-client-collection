...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o GOOG_IO_UNOFFICAL_GOOGLE_SEARCH_API_TOKEN | default "" }
...
export def "crawl get" [
  ...
  let full_url = (build-url $base ({query: (encode-path-segment $query)} | format pattern "/v1/crawl/{query}"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
...
export def "images get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($query | is-empty) { error make --unspanned { msg: "path parameter 'query' must be non-empty" } }
  let full_url = (build-url $base ({query: (encode-path-segment $query)} | format pattern "/v1/images/{query}"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
...
export def "news get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($query | is-empty) { error make --unspanned { msg: "path parameter 'query' must be non-empty" } }
  let full_url = (build-url $base ({query: (encode-path-segment $query)} | format pattern "/v1/news/{query}"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
...
export def "search list" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($query | is-empty) { error make --unspanned { msg: "path parameter 'query' must be non-empty" } }
  let full_url = (build-url $base ({query: (encode-path-segment $query)} | format pattern "/v1/search/{query}"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
