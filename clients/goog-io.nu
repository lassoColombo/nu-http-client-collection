# Auto-generated client for goog.io | Unoffical Google Search API v0.1.0
# Source: https://api.apis.guru/v2/specs/goog.io/0.1.0/openapi.json
# Auth: --token flag or $env.GOOG_IO_UNOFFICAL_GOOGLE_SEARCH_API_TOKEN

const BASE_URL = "https://api.goog.io"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o GOOG_IO_UNOFFICAL_GOOGLE_SEARCH_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.goog.io"] }
...
export def "crawl get" [
  ...
  let full_url = (build-url $base ({query: (encode-path-segment $query)} | format pattern "/v1/crawl/{query}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "images get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($query | is-empty) { error make --unspanned { msg: "path parameter 'query' must be non-empty" } }
  let full_url = (build-url $base ({query: (encode-path-segment $query)} | format pattern "/v1/images/{query}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "news get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($query | is-empty) { error make --unspanned { msg: "path parameter 'query' must be non-empty" } }
  let full_url = (build-url $base ({query: (encode-path-segment $query)} | format pattern "/v1/news/{query}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "search list" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($query | is-empty) { error make --unspanned { msg: "path parameter 'query' must be non-empty" } }
  let full_url = (build-url $base ({query: (encode-path-segment $query)} | format pattern "/v1/search/{query}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "serp create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/v1/serp/" $auth.query)
  let req_body = {"query": $query, "website": $website} | compact
