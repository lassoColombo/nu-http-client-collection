# Auto-generated client for Revenue Department - Land Records, Karnataka v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/landrecordskar/3.0.0/openapi.json
# Auth: --token flag or $env.REVENUE_DEPARTMENT_LAND_RECORDS_KARNATAKA_TOKEN

const BASE_URL = "https://apisetu.gov.in/landrecordskar/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o REVENUE_DEPARTMENT_LAND_RECORDS_KARNATAKA_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/landrecordskar/v3"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "cncer-certificate create" } } | get name | first)
...
export def "cncer-certificate create" [
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o REVENUE_DEPARTMENT_LAND_RECORDS_KARNATAKA_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o REVENUE_DEPARTMENT_LAND_RECORDS_KARNATAKA_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
...
#
# POST /mutan/certificate
# operationId: mutan
# --certificateParameters shape: {DocumentNumber: string}
# --consentArtifact shape: {consent: record, signature: record}
export def "mutan-certificate create" [
  --base-url(-b): string@base-url-completer # API base URL
  --token-apikey: string # Auth token for apiKey (X-APISETU-APIKEY)
  --token-clientid: string # Auth token for clientId (X-APISETU-CLIENTID)
  --insecure(-k) # Skip TLS verification
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o REVENUE_DEPARTMENT_LAND_RECORDS_KARNATAKA_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o REVENUE_DEPARTMENT_LAND_RECORDS_KARNATAKA_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/mutan/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = ($accept | default "application/pdf")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
