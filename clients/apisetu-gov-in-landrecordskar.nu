...
# Auth: --token flag or $env.REVENUE_DEPARTMENT_LAND_RECORDS__KARNATAKA_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o REVENUE_DEPARTMENT_LAND_RECORDS__KARNATAKA_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "cncer-certificate cncer" } } | get name | first)
...
export def "cncer-certificate cncer" [
...
# --certificateParameters shape: {DocumentNumber: string}
# --consentArtifact shape: {consent: record, signature: record}
export def "mutan-certificate mutan" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
