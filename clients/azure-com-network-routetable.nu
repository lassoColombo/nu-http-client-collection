...
# Source: https://api.apis.guru/v2/specs/azure.com/network-routeTable/2019-08-01/swagger.json
...
]: nothing -> record<etag: string, properties: record<disableBgpRoutePropagation: bool, provisioningState: string, routes: list<record>, subnets: list<any>>, id: string, location: string, name: string, tags: record, type: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
  if ($resource_group_name | is-empty) { error make --unspanned { msg: "path parameter 'resourceGroupName' must be non-empty" } }
...
]: any -> record<etag: string, properties: record<disableBgpRoutePropagation: bool, provisioningState: string, routes: list<record>, subnets: list<any>>, id: string, location: string, name: string, tags: record, type: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($subscription_id | is-empty) { error make --unspanned { msg: "path parameter 'subscriptionId' must be non-empty" } }
...
export def "subscriptions-resource-groups-providers-microsoft-network-route-tables create-or-update" [
  ...
  --properties: any # Route Table resource. — shape: {disableBgpRoutePropagation?: bool, routes?: list}
...
export def "subscriptions-resource-groups-providers-microsoft-network-route-tables-routes create-or-update" [
  ...
  --properties: any # Route resource. — shape: {addressPrefix?: string, nextHopIpAddress?: string, nextHopType: "VirtualNetworkGateway"|"VnetLocal"|"Internet"|"VirtualAppliance"|"None"}
  --id: string # Resource ID.
