...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o CARBONDOOMSDAY_TOKEN | default "" }
...
export def "co2 list" [
  ...
  let full_url = (build-url $base "/co2/" $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"ppm": $ppm, "date": $date, "date__range": $date_range, "ordering": $ordering, "page": $page, "limit": $limit} | compact), body: null}
}
