...
export def "providers-microsoft-blueprint-blueprint-assignments list" [
  ...
  let full_url = (build-url $base ({scope: (encode-path-segment $scope)} | format pattern "/{scope}/providers/Microsoft.Blueprint/blueprintAssignments") $qp)
...
export def "providers-microsoft-blueprint-blueprint-assignments delete" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({scope: (encode-path-segment $scope), assignment_name: (encode-path-segment $assignment_name)} | format pattern "/{scope}/providers/Microsoft.Blueprint/blueprintAssignments/{assignment_name}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "providers-microsoft-blueprint-blueprint-assignments-who-is-blueprint create" [
  ...
  if ($assignment_name | is-empty) { error make --unspanned { msg: "path parameter 'assignmentName' must be non-empty" } }
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({scope: (encode-path-segment $scope), assignment_name: (encode-path-segment $assignment_name)} | format pattern "/{scope}/providers/Microsoft.Blueprint/blueprintAssignments/{assignment_name}/WhoIsBlueprint") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
