...
export def "protected-resources-summary get" [
  ...
  let full_url = (build-url $base ({name: (encode-path-segment $name)} | format pattern "/v1/{name}/protectedResourcesSummary") $qp)
...
export def "protected-resources-search list" [
  ...
  let full_url = (build-url $base ({scope: (encode-path-segment $scope)} | format pattern "/v1/{scope}/protectedResources:search") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"$.xgafv": $xgafv, "access_token": $access_token, "alt": $alt, "callback": $callback, "fields": $fields, "key": $key, "oauth_token": $oauth_token, "prettyPrint": $pretty_print, "quotaUser": $quota_user, "upload_protocol": $upload_protocol, "uploadType": $upload_type, "cryptoKey": $crypto_key, "pageSize": $page_size, "pageToken": $page_token} | compact), body: null}
}
