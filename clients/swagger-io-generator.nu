# Auto-generated client for Swagger Generator v2.4.30
# Source: https://api.apis.guru/v2/specs/swagger.io/generator/2.4.30/swagger.json
# Auth: --token flag or $env.SWAGGER_GENERATOR_TOKEN

const BASE_URL = "https://generator.swagger.io/api"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o SWAGGER_GENERATOR_TOKEN | default "" }
...
def base-url-completer [] { ["https://generator.swagger.io/api"] }
...
export def "gen-clients list" [
  ...
  let full_url = (build-url $base "/gen/clients" $auth.query)
...
export def "gen-clients get-options" [
  ...
  let full_url = (build-url $base ({language: (encode-path-segment $language)} | format pattern "/gen/clients/{language}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "gen-download download-file" [
  ...
  let full_url = (build-url $base ({file_id: (encode-path-segment $file_id)} | format pattern "/gen/download/{file_id}") $auth.query)
...
export def "gen-servers list" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/gen/servers" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "gen-servers get-options" [
  ...
  let full_url = (build-url $base ({framework: (encode-path-segment $framework)} | format pattern "/gen/servers/{framework}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
