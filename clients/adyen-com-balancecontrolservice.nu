# Auto-generated client for Adyen Balance Control API v1
# Source: https://api.apis.guru/v2/specs/adyen.com/BalanceControlService/1/openapi.json
# Auth: --token flag or $env.ADYEN_BALANCE_CONTROL_API_TOKEN

const BASE_URL = "https://pal-test.adyen.com/pal/servlet/BalanceControl/v1"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o ADYEN_BALANCE_CONTROL_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://pal-test.adyen.com/pal/servlet/BalanceControl/v1"] }
def auth-scheme-completer [] { ["x-api-key" "basic" "basic-credentials"] }

# Completers for enum parameters
def type-completer [] { ["adjustment" "credit" "debit" "fee" "tax" "terminalSale"] }

# List all available API commands with their parameters
export def commands []: nothing -> table {
  let builtin_flags = ["base-url" "token" "auth-scheme" "insecure" "max-time" "raw" "allow-errors" "full" "dry-run" "accept" "help"]
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "balance-transfer create" } } | get name | first)
...
export def "balance-transfer create" [
  ...
  let full_url = (build-url $base "/balanceTransfer" $auth.query)
  let req_body = {"amount": $amount, "description": $description, "fromMerchant": $from_merchant, "reference": $reference, "toMerchant": $to_merchant, "type": $type} | compact
