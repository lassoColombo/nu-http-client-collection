...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o BUY_MARKETING_API_TOKEN | default "" }
...
export def "merchandised-product get" [
  ...
  let full_url = (build-url $base "/merchandised_product" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"aspect_filter": $aspect_filter, "category_id": $category_id, "limit": $limit, "metric_name": $metric_name} | compact)
