# Auto-generated client for Employees' Provident Fund Organization v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/epfindia/3.0.0/openapi.json
# Auth: --token flag or $env.EMPLOYEES_PROVIDENT_FUND_ORGANIZATION_TOKEN

const BASE_URL = "https://apisetu.gov.in/epfindia/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o EMPLOYEES_PROVIDENT_FUND_ORGANIZATION_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/epfindia/v3"] }
...
export def "epfsc-certificate create" [
  ...
  --certificate-parameters: record # shape: {DOB: string, FullName: string, SCNO: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o EMPLOYEES_PROVIDENT_FUND_ORGANIZATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o EMPLOYEES_PROVIDENT_FUND_ORGANIZATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/epfsc/certificate" $auth.query)
...
export def "pecer-certificate create" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --certificate-parameters: record # shape: {DOB: string, PPONO: string}
  --consent-artifact: any # shape: {consent: record, signature: record}
  format: string@format-completer # The format of the certificate in response.
  txn_id: string # A unique transaction id for this request in UUID format. It is used for tracking the request. (format: uuid, e.g. f7f1469c-29b0-4325-9dfc-c567200a70f7)
]: any -> any {
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o EMPLOYEES_PROVIDENT_FUND_ORGANIZATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o EMPLOYEES_PROVIDENT_FUND_ORGANIZATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/pecer/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "uncrd-certificate create" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --certificate-parameters: record # shape: {DOB: string, UAN: string}
  --consent-artifact: any # shape: {consent: record, signature: record}
  format: string@format-completer # The format of the certificate in response.
  txn_id: string # A unique transaction id for this request in UUID format. It is used for tracking the request. (format: uuid, e.g. f7f1469c-29b0-4325-9dfc-c567200a70f7)
]: any -> any {
  ...
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o EMPLOYEES_PROVIDENT_FUND_ORGANIZATION_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o EMPLOYEES_PROVIDENT_FUND_ORGANIZATION_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/uncrd/certificate" $auth.query)
  let req_body = {"certificateParameters": $certificate_parameters, "consentArtifact": $consent_artifact, "format": $format, "txnId": $txn_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/pdf"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
