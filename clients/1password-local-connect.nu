...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o 1PASSWORD_CONNECT_TOKEN | default "" }
...
export def "activity get" [
  ...
  let full_url = (build-url $base "/activity" $qp $auth.query)
...
export def "heartbeat get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default "http://localhost:8080")
  let full_url = (build-url $base "/heartbeat" $auth.query)
...
export def "vaults list" [
  ...
  let full_url = (build-url $base "/vaults" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "vaults get" [
  ...
  let full_url = (build-url $base ({vault_uuid: (encode-path-segment $vault_uuid)} | format pattern "/vaults/{vault_uuid}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "vaults-items list" [
  ...
  if ($vault_uuid | is-empty) { error make --unspanned { msg: "path parameter 'vaultUuid' must be non-empty" } }
  let qp = [(serialize-qp "filter" $filter "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({vault_uuid: (encode-path-segment $vault_uuid)} | format pattern "/vaults/{vault_uuid}/items") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "vaults-items create" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($vault_uuid | is-empty) { error make --unspanned { msg: "path parameter 'vaultUuid' must be non-empty" } }
  let full_url = (build-url $base ({vault_uuid: (encode-path-segment $vault_uuid)} | format pattern "/vaults/{vault_uuid}/items") $auth.query)
...
export def "vaults-items delete" [
  ...
  let full_url = (build-url $base ({vault_uuid: (encode-path-segment $vault_uuid), item_uuid: (encode-path-segment $item_uuid)} | format pattern "/vaults/{vault_uuid}/items/{item_uuid}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "vaults-items-files get" [
  ...
  let full_url = (build-url $base ({vault_uuid: (encode-path-segment $vault_uuid), item_uuid: (encode-path-segment $item_uuid)} | format pattern "/vaults/{vault_uuid}/items/{item_uuid}/files") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"inline_files": $inline_files} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "vaults-items-files get-details-of" [
  ...
  let qp = [(serialize-qp "inline_files" $inline_files "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({vault_uuid: (encode-path-segment $vault_uuid), item_uuid: (encode-path-segment $item_uuid), file_uuid: (encode-path-segment $file_uuid)} | format pattern "/vaults/{vault_uuid}/items/{item_uuid}/files/{file_uuid}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "vaults-items-files-content download" [
  ...
  if ($item_uuid | is-empty) { error make --unspanned { msg: "path parameter 'itemUuid' must be non-empty" } }
  if ($file_uuid | is-empty) { error make --unspanned { msg: "path parameter 'fileUuid' must be non-empty" } }
  let full_url = (build-url $base ({vault_uuid: (encode-path-segment $vault_uuid), item_uuid: (encode-path-segment $item_uuid), file_uuid: (encode-path-segment $file_uuid)} | format pattern "/vaults/{vault_uuid}/items/{item_uuid}/files/{file_uuid}/content") $auth.query)
