...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o FRAUDLABS_PRO_SMS_VERIFICATION_TOKEN | default "" }
...
export def "verification-result get" [
  ...
  let full_url = (build-url $base "/v1/verification/result" $qp $auth.query)
  let accept_val = "application/json; charset=utf-8"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"tran_id": $tran_id, "key": $key, "format": $format, "otp": $otp} | compact)
...
export def "verification-send create" [
  ...
  let full_url = (build-url $base "/v1/verification/send" $qp $auth.query)
  let accept_val = "application/json; charset=utf-8"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
    url: $full_url
    query: ({"country_code": $country_code, "format": $format, "tel": $tel, "key": $key, "mesg": $mesg} | compact)
    headers: $auth.headers
    body: null
