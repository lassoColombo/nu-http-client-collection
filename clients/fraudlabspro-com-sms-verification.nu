# Auto-generated client for FraudLabs Pro SMS Verification v1.0
# Source: https://api.apis.guru/v2/specs/fraudlabspro.com/sms-verification/1.0/openapi.json
# Auth: --token flag or $env.FRAUDLABS_PRO_SMS_VERIFICATION_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o FRAUDLABS_PRO_SMS_VERIFICATION_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.fraudlabspro.com"] }
...
export def "verification-result get" [
  ...
  --tran-id: string # The unique ID that was returned by the Send Verification SMS API that triggered the OTP sms.
  ...
  let full_url = (build-url $base "/v1/verification/result" $qp)
  let accept_val = "application/json; charset=utf-8"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"tran_id": $tran_id, "key": $key, "format": $format, "otp": $otp} | compact), body: null}
}
...
export def "verification-send create" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --country-code: string # ISO 3166 country code for the recipient mobile phone number. If parameter is supplied, then some basic telephone number validation is done.
  --format: string@format-completer # Returns the API response in json (default) or xml format.
  ...
  let full_url = (build-url $base "/v1/verification/send" $qp)
  let accept_val = "application/json; charset=utf-8"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"country_code": $country_code, "format": $format, "tel": $tel, "key": $key, "mesg": $mesg} | compact), body: null}
}
