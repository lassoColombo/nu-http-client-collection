# Auto-generated client for hashlookup CIRCL API v1.2
# Source: https://api.apis.guru/v2/specs/circl.lu/hashlookup/1.2/openapi.json
# Auth: --token flag or $env.HASHLOOKUP_CIRCL_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o HASHLOOKUP_CIRCL_API_TOKEN | default "" }
...
export def "bulk-md5 create-bulkmd5" [
  ...
  let full_url = (build-url $base "/bulk/md5")
...
export def "bulk-sha1 create-bulksha1" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/bulk/sha1")
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
...
export def "children get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($sha1 | is-empty) { error make --unspanned { msg: "path parameter 'sha1' must be non-empty" } }
  if ($count | is-empty) { error make --unspanned { msg: "path parameter 'count' must be non-empty" } }
  if ($cursor | is-empty) { error make --unspanned { msg: "path parameter 'cursor' must be non-empty" } }
...
export def "lookup-md5 get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($md5 | is-empty) { error make --unspanned { msg: "path parameter 'md5' must be non-empty" } }
...
export def "lookup-sha256 get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($sha256 | is-empty) { error make --unspanned { msg: "path parameter 'sha256' must be non-empty" } }
...
export def "stats-top get-stattop" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/stats/top")
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
