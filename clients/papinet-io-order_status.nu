# Auto-generated client for papiNet API v1.0.0
# Source: https://api.apis.guru/v2/specs/papinet.io/order_status/1.0.0/openapi.json
# Auth: --token flag or $env.PAPINET_API_TOKEN

const BASE_URL = "https://papinet.papinet.io"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o PAPINET_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://papinet.papinet.io"] }
...
export def "orders list" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"orderStatus": $order_status, "offset": $offset, "limit": $limit} | compact), body: null}
}
