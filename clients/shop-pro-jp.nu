...
export def "appstore-recurring-application-charges-usage-charges-json create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($recurring_application_charge_id | is-empty) { error make --unspanned { msg: "path parameter 'recurringApplicationChargeId' must be non-empty" } }
...
export def "appstore-script-tags delete" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($script_tag_id | is-empty) { error make --unspanned { msg: "path parameter 'scriptTagId' must be non-empty" } }
...
export def "inline-script-tags delete" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($inline_script_tag_id | is-empty) { error make --unspanned { msg: "path parameter 'inlineScriptTagId' must be non-empty" } }
