...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o VAT_API_TOKEN | default "" }
...
export def "country-code-check check" [
  ...
  let full_url = (build-url $base "/country-code-check" $qp $auth.query)
...
export def "currency-conversion get" [
  ...
  let full_url = (build-url $base "/currency-conversion" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Response-Type": $response_type} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "get"
    url: $full_url
    query: ({"currency_from": $currency_from, "currency_to": $currency_to, "amount": $amount} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "invoice create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/invoice" $auth.query)
...
export def "invoice delete" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/invoice/{id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Response-Type": $response_type} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "ip-check check" [
  ...
  let full_url = (build-url $base "/ip-check" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Response-Type": $response_type} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "usage-check get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/usage-check" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Response-Type": $response_type} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "vat-number-check validate" [
  ...
  let full_url = (build-url $base "/vat-number-check" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Response-Type": $response_type} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
...
export def "vat-price get-convert" [
  ...
  let full_url = (build-url $base "/vat-price" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Response-Type": $response_type} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  let req = {
    method: "get"
    url: $full_url
    query: ({"code": $code, "country_rate": $country_rate, "price": $price, "type": $type} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "vat-rates get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "apikey"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/vat-rates" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Response-Type": $response_type} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
