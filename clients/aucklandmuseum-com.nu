# Auto-generated client for Auckland Museum API v2.0.0
# Source: https://api.apis.guru/v2/specs/aucklandmuseum.com/2.0.0/swagger.json
# Auth: --token flag or $env.AUCKLAND_MUSEUM_API_TOKEN

const BASE_URL = "https://api.aucklandmuseum.com"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AUCKLAND_MUSEUM_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.aucklandmuseum.com"] }
def auth-scheme-completer [] { ["bearer"] }

# Completers for enum parameters
def accept-completer [] { ["application/pdf" "image/jpeg"] }
def accept-completer-1 [] { ["application/json" "application/ld+json" "text/html"] }
def accept-completer-2 [] { ["application/javascript" "application/sparql-results+json" "application/sparql-results+xml"] }
def accept-completer-3 [] { ["application/sparql-results+json" "application/sparql-results+xml"] }
...
export def "id-media get" [
  ...
  let full_url = (build-url $base ({path: (encode-path-segment $path)} | format pattern "/id/media/{path}") $qp $auth.query)
  let accept_val = ($accept | default "image/jpeg")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"rendering": $rendering} | compact)
...
export def "id get-subject" [
  ...
  let full_url = (build-url $base ({identifier: (encode-path-segment $identifier)} | format pattern "/id/{identifier}") $auth.query)
...
export def "search get" [
  ...
  let full_url = (build-url $base ({index: (encode-path-segment $index), operation: (encode-path-segment $operation)} | format pattern "/search/{index}/{operation}") $qp $auth.query)
...
export def "search create" [
  ...
  if ($index | is-empty) { error make --unspanned { msg: "path parameter 'index' must be non-empty" } }
  if ($operation | is-empty) { error make --unspanned { msg: "path parameter 'operation' must be non-empty" } }
  let full_url = (build-url $base ({index: (encode-path-segment $index), operation: (encode-path-segment $operation)} | format pattern "/search/{index}/{operation}") $auth.query)
  let req_body = $body
...
export def "sparql get" [
  ...
  let full_url = (build-url $base "/sparql" $qp $auth.query)
  let accept_val = ($accept | default "application/sparql-results+json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"query": $query, "callback": $callback, "infer": $infer} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "sparql create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/sparql" $auth.query)
