...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o SOCIO_DEMO_API_TOKEN | default "" }
...
export def "age get" [
  ...
  let full_url = (build-url $base ({location: (encode-path-segment $location)} | format pattern "/age/{location}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"ageGroup": $age_group, "occurenceType": $occurence_type, "hour": $hour} | compact)
...
export def "gender get" [
  ...
  let full_url = (build-url $base ({location: (encode-path-segment $location)} | format pattern "/gender/{location}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"g": $g, "occurenceType": $occurence_type, "hour": $hour} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
