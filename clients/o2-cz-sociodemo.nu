# Auto-generated client for Socio-demo API v1.2.0
# Source: https://api.apis.guru/v2/specs/o2.cz/sociodemo/1.2.0/swagger.json
# Auth: --token flag or $env.SOCIO_DEMO_API_TOKEN

const BASE_URL = "https://developer.o2.cz/sociodemo/sandbox/api"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o SOCIO_DEMO_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://developer.o2.cz/sociodemo/sandbox/api"] }
...
export def "age get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"ageGroup": $age_group, "occurenceType": $occurence_type, "hour": $hour} | compact), body: null}
}
...
export def "gender get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"g": $g, "occurenceType": $occurence_type, "hour": $hour} | compact), body: null}
}
...
export def "info get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> record<appName: string, appVendor: string, appVersion: string, backendAppName: string, backendAppVendor: string, backendAppVersion: string, backendDataFrom: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
