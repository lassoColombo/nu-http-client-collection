# Auto-generated client for Meark Enterprise Pvt. Ltd. v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/meark/3.0.0/openapi.json
# Auth: --token flag or $env.MEARK_ENTERPRISE_PVT_LTD_TOKEN

const BASE_URL = "https://apisetu.gov.in/meark/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o MEARK_ENTERPRISE_PVT_LTD_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/meark/v3"] }
...
# operationId: adcrd
# --certificateParameters shape: {DOB: string, PrnNo: string, RequestId: string}
...
export def "adcrd-certificate create" [
  ...
  --certificate-parameters: record # shape: {DOB: string, PrnNo: string, RequestId: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o MEARK_ENTERPRISE_PVT_LTD_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o MEARK_ENTERPRISE_PVT_LTD_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
