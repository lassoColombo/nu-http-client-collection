...
export def "v1beta workflowsprojectslocationsworkflowspatch" [
  ...
  let full_url = (build-url $base $"/v1beta/($name)" $qp)
  let body = {description: $description, labels: $labels, name: $body_name, serviceAccount: $serviceAccount, sourceContents: $sourceContents} | compact
...
export def "v1beta-workflows workflowsprojectslocationsworkflowslist" [
  ...
  --pageSize: int # Maximum number of workflows to return per call. The service may return fewer than this value. If the value is not specified, a default value of 500 will be used. The maximum permitted value is 1000 and values greater than 1000 will be coerced down to 1000.
  ...
  let full_url = (build-url $base $"/v1beta/($parent)/workflows" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "v1beta-workflows workflowsprojectslocationsworkflowscreate" [
  ...
  let full_url = (build-url $base $"/v1beta/($parent)/workflows" $qp)
  let body = {description: $description, labels: $labels, name: $name, serviceAccount: $serviceAccount, sourceContents: $sourceContents} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
