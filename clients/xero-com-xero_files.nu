...
export def "files list" [
  ...
  let extra_headers = {"xero-tenant-id": $xero_tenant_id} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"pagesize": $pagesize, "page": $page, "sort": $qp_sort} | compact), body: null}
}
...
export def "files upload" [
  ...
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full $mp.content_type $mp.body {query: ({"FolderId": $folder_id} | compact), body: $req_body}
}
