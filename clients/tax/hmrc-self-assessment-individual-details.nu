# Auto-generated client for Self Assessment Individual Details (MTD) v2.0
# Source: https://developer.service.hmrc.gov.uk/api-documentation/docs/api/service/self-assessment-individual-details-api/2.0/oas/resolved
# Auth: --token flag or $env.SELF_ASSESSMENT_INDIVIDUAL_DETAILS_MTD_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o SELF_ASSESSMENT_INDIVIDUAL_DETAILS_MTD_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "individuals-person-itsa-status get" } } | get name | first)
...
export def "individuals-person-itsa-status get" [
  nino: string
  taxYear: string
...
]: nothing -> record<itsaStatuses: table<taxYear: string, itsaStatusDetails: list>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "futureYears" $futureYears "scalar") (serialize-qp "history" $history "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/individuals/person/itsa-status/($nino)/($taxYear)" $qp)
