# Auto-generated client for Dados Abertos - API v1.0
# Source: https://api.apis.guru/v2/specs/inpe.br/dados-abertos/1.0/swagger.json
# Auth: --token flag or $env.DADOS_ABERTOS_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o DADOS_ABERTOS_API_TOKEN | default "" }
...
export def "auxiliar-estados get-resource" [
  ...
  let qp = [(serialize-qp "pais_id" $pais_id "multi")] | flatten | str join "&"
  let full_url = (build-url $base "/auxiliar/estados" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"pais_id": $pais_id} | compact), body: null}
}
...
export def "auxiliar-municipios get-resource" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pais_id" $pais_id "scalar") (serialize-qp "estado_id" $estado_id "multi")] | flatten | str join "&"
  let full_url = (build-url $base "/auxiliar/municipios" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"pais_id": $pais_id, "estado_id": $estado_id} | compact), body: null}
}
...
export def "auxiliar-paises get-resource" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/auxiliar/paises")
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "auxiliar-satelites get-resource" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/auxiliar/satelites")
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
...
export def "focos get-resource" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"pais_id": $pais_id, "estado_id": $estado_id, "municipio_id": $municipio_id, "satelite": $satelite} | compact), body: null}
}
...
export def "focos-count get-resource" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pais_id" $pais_id "scalar") (serialize-qp "estado_id" $estado_id "scalar") (serialize-qp "municipio_id" $municipio_id "scalar") (serialize-qp "satelite" $satelite "multi")] | flatten | str join "&"
  let full_url = (build-url $base "/focos/count" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"pais_id": $pais_id, "estado_id": $estado_id, "municipio_id": $municipio_id, "satelite": $satelite} | compact), body: null}
}
