...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o DADOS_ABERTOS_API_TOKEN | default "" }
...
export def "auxiliar-estados get-resource" [
  ...
  let full_url = (build-url $base "/auxiliar/estados" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"pais_id": $pais_id} | compact)
...
export def "auxiliar-municipios get-resource" [
  ...
  let full_url = (build-url $base "/auxiliar/municipios" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"pais_id": $pais_id, "estado_id": $estado_id} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "auxiliar-paises get-resource" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/auxiliar/paises" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "auxiliar-satelites get-resource" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/auxiliar/satelites" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "focos get-resource" [
  ...
  let full_url = (build-url $base "/focos/" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"pais_id": $pais_id, "estado_id": $estado_id, "municipio_id": $municipio_id, "satelite": $satelite} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "focos-count get-resource" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pais_id" $pais_id "scalar") (serialize-qp "estado_id" $estado_id "scalar") (serialize-qp "municipio_id" $municipio_id "scalar") (serialize-qp "satelite" $satelite "multi")] | flatten | str join "&"
  let full_url = (build-url $base "/focos/count" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
