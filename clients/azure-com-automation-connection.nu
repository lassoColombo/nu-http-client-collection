...
# Source: https://api.apis.guru/v2/specs/azure.com/automation-connection/2015-10-31/swagger.json
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-connections ListByAutomationAccount" } } | get name | first)
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-connections ListByAutomationAccount" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Automation/automationAccounts/($automationAccountName)/connections" $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-connections Delete" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --api-version: string # Client Api Version.
]: nothing -> record<properties: record<connectionType: record<name: string>, creationTime: string, description: string, fieldDefinitionValues: record, lastModifiedTime: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Automation/automationAccounts/($automationAccountName)/connections/($connectionName)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-connections Update" [
  ...
  --properties: any # The properties of the update connection operation. — shape: {description?: string, fieldDefinitionValues?: record}
]: any -> record<properties: record<connectionType: record<name: string>, creationTime: string, description: string, fieldDefinitionValues: record, lastModifiedTime: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
...
export def "subscriptions-resource-groups-providers-microsoft-automation-automation-accounts-connections CreateOrUpdate" [
  resourceGroupName: string
  automationAccountName: string
  connectionName: string
  subscriptionId: string
  ...
  --api-version: string # Client Api Version.
  name: string # Gets or sets the name of the connection.
  properties: any # The properties of the create connection properties — shape: {connectionType: any, description?: string, fieldDefinitionValues?: record}
]: any -> record<properties: record<connectionType: record<name: string>, creationTime: string, description: string, fieldDefinitionValues: record, lastModifiedTime: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
