...
export def "reports list" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"account_id": $account_id, "status": $status, "date_from": $date_from, "date_to": $date_to} | compact), body: null}
}
...
export def "reports-records get" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"account_id": $account_id, "product": $product, "direction": $direction, "id": $id, "date_start": $date_start, "date_end": $date_end, "include_message": $include_message, "show_concatenated": $show_concatenated, "status": $status} | compact), body: null}
}
