...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "subscriptions-resource-groups-providers-microsoft-sql-servers-elastic-pools ListByServer" } } | get name | first)
...
export def "subscriptions-resource-groups-providers-microsoft-sql-servers-elastic-pools ListByServer" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Sql/servers/($serverName)/elasticPools" $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-sql-servers-elastic-pools Delete" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Sql/servers/($serverName)/elasticPools/($elasticPoolName)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
# --properties shape: {licenseType?: "LicenseIncluded"|"BasePrice", maxSizeBytes?: int, perDatabaseSettings?: record, zoneRedundant?: bool}
# --sku shape: {capacity?: int, family?: string, name: string, size?: string, tier?: string}
export def "subscriptions-resource-groups-providers-microsoft-sql-servers-elastic-pools CreateOrUpdate" [
  resourceGroupName: string
  serverName: string
  elasticPoolName: string
  subscriptionId: string
