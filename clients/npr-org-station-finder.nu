...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o NPR_STATION_FINDER_SERVICE_TOKEN | default "" }
...
export def "stations list" [
  ...
  let full_url = (build-url $base "/v3/stations" $qp $auth.query)
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"q": $q, "city": $city, "state": $state, "lat": $lat, "lon": $lon} | compact)
...
export def "stations get" [
  ...
  let full_url = (build-url $base ({station_id: (encode-path-segment $station_id)} | format pattern "/v3/stations/{station_id}") $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Authorization": $authorization} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
