# Auto-generated client for NPR Station Finder Service v3
# Source: https://api.apis.guru/v2/specs/npr.org/station-finder/3/swagger.json
# Auth: --token flag or $env.NPR_STATION_FINDER_SERVICE_TOKEN

const BASE_URL = "https://station.api.npr.org"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o NPR_STATION_FINDER_SERVICE_TOKEN | default "" }
...
def base-url-completer [] { ["https://station.api.npr.org"] }
...
export def "stations list" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"q": $q, "city": $city, "state": $state, "lat": $lat, "lon": $lon} | compact), body: null}
}
...
export def "stations get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($station_id | is-empty) { error make --unspanned { msg: "path parameter 'stationId' must be non-empty" } }
  let full_url = (build-url $base ({station_id: (encode-path-segment $station_id)} | format pattern "/v3/stations/{station_id}"))
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let extra_headers = {"Authorization": $authorization} | compact
  let auth = ($auth | update headers ($auth.headers | merge $extra_headers))
