...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o JOKES_ONE_API_TOKEN | default "" }
...
export def "jod get" [
  ...
  let full_url = (build-url $base "/jod" $qp $auth.query)
...
export def "jod-categories get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-jokesone-api-secret"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/jod/categories" $auth.query)
...
export def "joke delete" [
  ...
  let full_url = (build-url $base "/joke" $qp $auth.query)
...
export def "joke update" [
  ...
  let req = {
    method: "patch"
    url: $full_url
    query: ({"id": $id, "title": $title, "text": $text, "author": $author, "tags": $tags} | compact)
    headers: $auth.headers
    body: null
    content_type: "application/json"
    timeout: ($max_time | default 30min)
...
export def "joke update-1" [
  ...
  let req = {
    method: "put"
    url: $full_url
    query: ({"title": $title, "text": $text, "author": $author, "tags": $tags} | compact)
    headers: $auth.headers
    body: null
    content_type: "application/json"
    timeout: ($max_time | default 30min)
...
export def "joke-categories-search get" [
  ...
  let full_url = (build-url $base "/joke/categories/search" $qp $auth.query)
  let accept_val = ($accept | default "application/xml")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"query": $query, "start": $start} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "joke-list get" [
  ...
  let full_url = (build-url $base "/joke/list" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "joke-random get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "x-jokesone-api-secret"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/joke/random" $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "joke-search get" [
  ...
  let full_url = (build-url $base "/joke/search" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"category": $category, "query": $query, "minlength": $minlength, "maxlength": $maxlength, "author": $author, "private": $private} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "joke-tags-add create" [
  ...
  let full_url = (build-url $base "/joke/tags/add" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "joke-tags-remove create" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "id" $id "scalar") (serialize-qp "tags" $tags "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/joke/tags/remove" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
