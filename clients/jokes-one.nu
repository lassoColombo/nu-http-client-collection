...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  ...
  match $scheme {
    "x-jokesone-api-secret" => { {scheme: $scheme, headers: {X-JokesOne-Api-Secret: $token_val}, query: "", location: "header"} }
...
export def "joke update" [
  ...
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "patch" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"id": $id, "title": $title, "text": $text, "author": $author, "tags": $tags} | compact), body: null}
}
...
export def "joke update-1" [
  ...
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "put" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"title": $title, "text": $text, "author": $author, "tags": $tags} | compact), body: null}
}
...
export def "joke-categories-search get" [
  ...
  let accept_val = ($accept | default "application/xml")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"query": $query, "start": $start} | compact), body: null}
}
...
export def "joke-search get" [
  ...
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"category": $category, "query": $query, "minlength": $minlength, "maxlength": $maxlength, "author": $author, "private": $private} | compact), body: null}
}
