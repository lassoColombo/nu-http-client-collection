# Auto-generated client for Stoplight vapi-v1
# Source: https://api.apis.guru/v2/specs/stoplight.io/api-v1/openapi.json
# Auth: --token flag or $env.STOPLIGHT_TOKEN

const BASE_URL = "https://api.stoplight.io/v1"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o STOPLIGHT_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.stoplight.io/v1"] }
def auth-scheme-completer [] { ["bearer"] }

# Completers for enum parameters
def accept-completer [] { ["application/json" "text/yaml"] }
...
export def "versions-publish-anon create" [
  ...
  let full_url = (build-url $base "/versions/publish/anon" $auth.query)
  let req_body = {"specData": $spec_data, "url": $url} | compact
...
export def "versions-export get" [
  ...
  let full_url = (build-url $base ({version_id: (encode-path-segment $version_id), format: (encode-path-segment $format)} | format pattern "/versions/{version_id}/export/{format}") $auth.query)
...
export def "versions-import update" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($version_id | is-empty) { error make --unspanned { msg: "path parameter 'versionId' must be non-empty" } }
  let full_url = (build-url $base ({version_id: (encode-path-segment $version_id)} | format pattern "/versions/{version_id}/import") $auth.query)
  let req_body = {"options": $options, "specData": $spec_data, "url": $url} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
...
export def "versions-publish create" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($version_id | is-empty) { error make --unspanned { msg: "path parameter 'versionId' must be non-empty" } }
  let full_url = (build-url $base ({version_id: (encode-path-segment $version_id)} | format pattern "/versions/{version_id}/publish") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "versions-unpublish update" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($version_id | is-empty) { error make --unspanned { msg: "path parameter 'versionId' must be non-empty" } }
  let full_url = (build-url $base ({version_id: (encode-path-segment $version_id)} | format pattern "/versions/{version_id}/unpublish") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "put"
