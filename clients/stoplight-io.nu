...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o STOPLIGHT_TOKEN | default "" }
...
export def "versions-export get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base ({version_id: (encode-path-segment $version_id), format: (encode-path-segment $format)} | format pattern "/versions/{version_id}/export/{format}"))
...
export def "versions-import update" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base ({version_id: (encode-path-segment $version_id)} | format pattern "/versions/{version_id}/import"))
...
export def "versions-publish create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base ({version_id: (encode-path-segment $version_id)} | format pattern "/versions/{version_id}/publish"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "versions-unpublish update" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base ({version_id: (encode-path-segment $version_id)} | format pattern "/versions/{version_id}/unpublish"))
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
