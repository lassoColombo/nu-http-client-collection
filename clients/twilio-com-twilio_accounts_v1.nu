...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o TWILIO_ACCOUNTS_TOKEN | default "" }
...
export def "auth-tokens-promote update-promotion" [
  ...
  let full_url = (build-url $base "/v1/AuthTokens/Promote" $auth.query)
...
export def "auth-tokens-secondary delete" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://accounts.twilio.com")
  let full_url = (build-url $base "/v1/AuthTokens/Secondary" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "credentials-aws list" [
  ...
  let full_url = (build-url $base "/v1/Credentials/AWS" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "credentials-aws create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://accounts.twilio.com")
  let full_url = (build-url $base "/v1/Credentials/AWS" $auth.query)
...
export def "credentials-aws delete" [
  ...
  let full_url = (build-url $base ({sid: (encode-path-segment $sid)} | format pattern "/v1/Credentials/AWS/{sid}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "credentials-public-keys list" [
  ...
  let base = ($base_url | default "https://accounts.twilio.com")
  let qp = [(serialize-qp "PageSize" $page_size "scalar") (serialize-qp "Page" $page "scalar") (serialize-qp "PageToken" $page_token "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/v1/Credentials/PublicKeys" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "credentials-public-keys create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://accounts.twilio.com")
  let full_url = (build-url $base "/v1/Credentials/PublicKeys" $auth.query)
...
export def "credentials-public-keys delete" [
  ...
  let base = ($base_url | default "https://accounts.twilio.com")
  if ($sid | is-empty) { error make --unspanned { msg: "path parameter 'Sid' must be non-empty" } }
  let full_url = (build-url $base ({sid: (encode-path-segment $sid)} | format pattern "/v1/Credentials/PublicKeys/{sid}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
