...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o UP_API_TOKEN | default "" }
...
export def "accounts list" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"page[size]": $page_size, "filter[accountType]": $filter_account_type, "filter[ownershipType]": $filter_ownership_type} | compact)
...
export def "accounts-transactions get" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"page[size]": $page_size, "filter[status]": $filter_status, "filter[since]": $filter_since, "filter[until]": $filter_until, "filter[category]": $filter_category, "filter[tag]": $filter_tag} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "categories list" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"filter[parent]": $filter_parent} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "tags get" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"page[size]": $page_size} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "transactions-relationships-category update" [
  ...
  let full_url = (build-url $base ({transaction_id: (encode-path-segment $transaction_id)} | format pattern "/transactions/{transaction_id}/relationships/category") $auth.query)
...
export def "transactions-relationships-tags delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($transaction_id | is-empty) { error make --unspanned { msg: "path parameter 'transactionId' must be non-empty" } }
  let full_url = (build-url $base ({transaction_id: (encode-path-segment $transaction_id)} | format pattern "/transactions/{transaction_id}/relationships/tags") $auth.query)
  let req_body = {"data": $data} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "util-ping get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/util/ping" $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "webhooks-logs get" [
  ...
  let qp = [(serialize-qp "page[size]" $page_size "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({webhook_id: (encode-path-segment $webhook_id)} | format pattern "/webhooks/{webhook_id}/logs") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "webhooks-ping create" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($webhook_id | is-empty) { error make --unspanned { msg: "path parameter 'webhookId' must be non-empty" } }
  let full_url = (build-url $base ({webhook_id: (encode-path-segment $webhook_id)} | format pattern "/webhooks/{webhook_id}/ping") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
