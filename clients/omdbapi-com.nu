...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o OMDB_TOKEN | default "" }
...
export def "api get-om-db-list" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"t": $t, "i": $i, "s": $s, "y": $y, "type": $type, "plot": $plot, "tomatoes": $tomatoes, "r": $r, "v": $v, "page": $page, "callback": $callback} | compact)
