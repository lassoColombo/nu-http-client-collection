# Auto-generated client for OMDb v1
# Source: https://api.apis.guru/v2/specs/omdbapi.com/1/swagger.json
# Auth: --token flag or $env.OMDB_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o OMDB_TOKEN | default "" }
...
def plot-completer [] { ["full" "short"] }
def r-completer [] { ["json" "xml"] }
def accept-completer [] { ["application/json" "application/xml"] }

# List all available API commands with their parameters
export def commands []: nothing -> table {
  let builtin_flags = ["base-url" "token" "auth-scheme" "insecure" "max-time" "raw" "allow-errors" "full" "dry-run" "accept" "help"]
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "api get-om-db-list" } } | get name | first)
...
export def "api get-om-db-list" [
  ...
  --y: int # Year of release.
  --type: string@type-completer # Type of result to return.
  --plot: string@plot-completer # Return short or full plot. (default: short)
  --tomatoes: oneof<nothing, bool> # Include Rotten Tomatoes ratings. (default: false)
  --r: string@r-completer # The data type to return. (default: json)
  --v: int # API version (reserved for future use). (default: 1)
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"t": $t, "i": $i, "s": $s, "y": $y, "type": $type, "plot": $plot, "tomatoes": $tomatoes, "r": $r, "v": $v, "page": $page, "callback": $callback} | compact), body: null}
}
