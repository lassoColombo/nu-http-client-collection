...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o STACKEXCHANGE_TOKEN | default "" }
...
export def "access-tokens get" [
...
]: nothing -> table<access_token: string, account_id: int, expires_on_date: int, scope: list<any>> {
  ...
  let full_url = (build-url $base ({access_tokens: (encode-path-segment $access_tokens)} | format pattern "/access-tokens/{access_tokens}") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"pagesize": $pagesize, "page": $page, "filter": $filter, "callback": $callback} | compact)
...
export def "access-tokens-invalidate get" [
  ...
  if ($access_tokens | is-empty) { error make --unspanned { msg: "path parameter 'accessTokens' must be non-empty" } }
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({access_tokens: (encode-path-segment $access_tokens)} | format pattern "/access-tokens/{access_tokens}/invalidate") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "answers list" [
...
]: nothing -> table<accepted: bool, answer_id: int, awarded_bounty_amount: int, awarded_bounty_users: list<any>, body: string, body_markdown: string, can_flag: bool, comment_count: int, comments: list<any>, community_owned_date: int, creation_date: int, down_vote_count: int, downvoted: bool, is_accepted: bool, last_activity_date: int, last_edit_date: int, last_editor: record<accept_rate: int, badge_counts: record, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>, link: string, locked_date: int, owner: record<accept_rate: int, badge_counts: record, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>, question_id: int, score: int, share_link: string, tags: list<any>, title: string, up_vote_count: int, upvoted: bool> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  ...
  let full_url = (build-url $base "/answers" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"order": $order, "max": $max, "min": $min, "sort": $qp_sort, "fromdate": $fromdate, "todate": $todate, "pagesize": $pagesize, "page": $page, "filter": $filter, "callback": $callback, "site": $site} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "answers get" [
  ...
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/answers/{ids}") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "answers-comments get" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<body: string, body_markdown: string, can_flag: bool, comment_id: int, creation_date: int, edited: bool, link: string, owner: record<accept_rate: int, badge_counts: record, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>, post_id: int, post_type: string, reply_to_user: record<accept_rate: int, badge_counts: record, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>, score: int, upvoted: bool> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/answers/{ids}/comments") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "apps-de-authenticate get" [
  ...
  if ($access_tokens | is-empty) { error make --unspanned { msg: "path parameter 'accessTokens' must be non-empty" } }
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({access_tokens: (encode-path-segment $access_tokens)} | format pattern "/apps/{access_tokens}/de-authenticate") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "badges list" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<award_count: int, badge_id: int, badge_type: string, description: string, link: string, name: string, rank: string, user: record<accept_rate: int, badge_counts: record, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "inname" $inname "scalar") (serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/badges" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"inname": $inname, "order": $order, "max": $max, "min": $min, "sort": $qp_sort, "fromdate": $fromdate, "todate": $todate, "pagesize": $pagesize, "page": $page, "filter": $filter, "callback": $callback, "site": $site} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "badges-name get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "inname" $inname "scalar") (serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/badges/name" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "badges-recipients list" [
  ...
  let full_url = (build-url $base "/badges/recipients" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"fromdate": $fromdate, "todate": $todate, "pagesize": $pagesize, "page": $page, "filter": $filter, "callback": $callback, "site": $site} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "badges-tags get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "inname" $inname "scalar") (serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/badges/tags" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "badges get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/badges/{ids}") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "badges-recipients get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/badges/{ids}/recipients") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "comments get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/comments/{ids}") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "comments-delete create" [
  id: int
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/comments/{id}/delete") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
    url: $full_url
    query: ({"filter": $filter, "callback": $callback, "site": $site, "preview": $preview} | compact)
    headers: $auth.headers
    body: null
...
export def "comments-edit create" [
  id: int
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  --body: string
  --preview: oneof<nothing, bool>
]: nothing -> record<body: string, body_markdown: string, can_flag: bool, comment_id: int, creation_date: int, edited: bool, link: string, owner: record<accept_rate: int, badge_counts: record<bronze: int, gold: int, silver: int>, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>, post_id: int, post_type: string, reply_to_user: record<accept_rate: int, badge_counts: record<bronze: int, gold: int, silver: int>, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>, score: int, upvoted: bool> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/comments/{id}/edit") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
    url: $full_url
    query: ({"filter": $filter, "callback": $callback, "site": $site, "body": $body, "preview": $preview} | compact)
    headers: $auth.headers
    body: null
    content_type: "application/json"
    timeout: ($max_time | default 30min)
...
export def "errors list" [
  ...
  --filter: string # #Discussion The Stack Exchange API allows applications to exclude almost every field returned. For example, if an application did not care about a user's badge counts it could exclude user.badge_counts whenever it calls a method that returns users. An application excludes fields by creating a filter (via /filter/create) and passing it to a method in the filter parameter. Filters are immutable and non-expiring. An application can safely "bake in" any filters that are created, it is not necessary (or advisable) to create filters at runtime. The motivation for filters are several fold. Filters allow applications to reduce API responses to just the fields they are concerned with, saving bandwidth. With the list of fields an application is actually concerned with, the API can avoid unneccessary queries thereby decreasing response time (and reducing load on our infrastructure). Finally, filters allow us to be more conservative in what the API returns by default without a proliferation of parameters (as was seen with body, answers, and comments in the 1.x API family). #Safety Filters also carry a notion of safety, which is defined as follows. Any string returned as a result of an API call with a safe filter will be inline-able into HTML without script-injection concerns. That is to say, no additional sanitizing (encoding, HTML tag stripping, etc.) will be necessary on returned strings. Applications that wish to handle sanitizing themselves should create an unsafe filter. All filters are safe by default, under the assumption that double-encoding bugs are more desirable than script injections. Note that this does not mean that "safe" filter is mearly an "unsafe" one with all fields passed though UrlEncode(...). Many fields can and will contain HTML in all filter types (most notably, the *.body fields). When using unsafe filters, the API returns the highest fidelity data it can reasonably access for the given request. This means that in cases where the "safe" data is the only accessible data it will be returned even in "unsafe" filters. Notably the *.body fields are unchanged, as they are stored in that form. Fields that are unchanged between safe and unsafe filters are denoted in their types documentation. #Built In Filters The following filters are built in: default, each type documents which fields are returned under the default filter (for example, answers). withbody, which is default plus the *.body fields none, which is empty total, which includes just .total #Compatibility with V1.x For ease of transition from earlier API versions, the filters _b, _ba, _bc, _bca, _a, _ac, and _c are also built in. These are unsafe, and exclude a combination of question and answer body, comments, and answers so as to mimic the body, answers, and comments parameters that have been removed in V2.0. New applications should not use these filters.
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
]: nothing -> table<description: string, error_id: int, error_name: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/errors" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "errors get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> record<error_id: float, error_message: string, error_name: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/errors/{id}") $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "events get" [
  ...
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
  --since: int # Unix date.
]: nothing -> table<creation_date: int, event_type: string, excerpt: string, link: string, the_id_of_the_object__answer__comment__question__or_user__the_event_describes: int> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"pagesize": $pagesize, "page": $page, "filter": $filter, "callback": $callback, "site": $site, "since": $since} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "filters-create get" [
...
]: nothing -> record<filter: string, filter_type: string, included_fields: list<any>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  ...
  let full_url = (build-url $base "/filters/create" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"base": $qp_base, "exclude": $exclude, "include": $include, "unsafe": $unsafe} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "filters get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
]: nothing -> table<filter: string, filter_type: string, included_fields: list<any>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($filters | is-empty) { error make --unspanned { msg: "path parameter 'filters' must be non-empty" } }
  let full_url = (build-url $base ({filters: (encode-path-segment $filters)} | format pattern "/filters/{filters}") $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "inbox get" [
  ...
  --filter: string # #Discussion The Stack Exchange API allows applications to exclude almost every field returned. For example, if an application did not care about a user's badge counts it could exclude user.badge_counts whenever it calls a method that returns users. An application excludes fields by creating a filter (via /filter/create) and passing it to a method in the filter parameter. Filters are immutable and non-expiring. An application can safely "bake in" any filters that are created, it is not necessary (or advisable) to create filters at runtime. The motivation for filters are several fold. Filters allow applications to reduce API responses to just the fields they are concerned with, saving bandwidth. With the list of fields an application is actually concerned with, the API can avoid unneccessary queries thereby decreasing response time (and reducing load on our infrastructure). Finally, filters allow us to be more conservative in what the API returns by default without a proliferation of parameters (as was seen with body, answers, and comments in the 1.x API family). #Safety Filters also carry a notion of safety, which is defined as follows. Any string returned as a result of an API call with a safe filter will be inline-able into HTML without script-injection concerns. That is to say, no additional sanitizing (encoding, HTML tag stripping, etc.) will be necessary on returned strings. Applications that wish to handle sanitizing themselves should create an unsafe filter. All filters are safe by default, under the assumption that double-encoding bugs are more desirable than script injections. Note that this does not mean that "safe" filter is mearly an "unsafe" one with all fields passed though UrlEncode(...). Many fields can and will contain HTML in all filter types (most notably, the *.body fields). When using unsafe filters, the API returns the highest fidelity data it can reasonably access for the given request. This means that in cases where the "safe" data is the only accessible data it will be returned even in "unsafe" filters. Notably the *.body fields are unchanged, as they are stored in that form. Fields that are unchanged between safe and unsafe filters are denoted in their types documentation. #Built In Filters The following filters are built in: default, each type documents which fields are returned under the default filter (for example, answers). withbody, which is default plus the *.body fields none, which is empty total, which includes just .total #Compatibility with V1.x For ease of transition from earlier API versions, the filters _b, _ba, _bc, _bca, _a, _ac, and _c are also built in. These are unsafe, and exclude a combination of question and answer body, comments, and answers so as to mimic the body, answers, and comments parameters that have been removed in V2.0. New applications should not use these filters.
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
]: nothing -> table<answer_id: int, body: string, comment_id: int, creation_date: int, is_unread: bool, item_type: string, link: string, question_id: int, site: record<aliases: list, api_site_parameter: string, audience: string, closed_beta_date: int, favicon_url: string, high_resolution_icon_url: string, icon_url: string, launch_date: int, logo_url: string, markdown_extensions: list, name: string, open_beta_date: int, related_sites: list, site_state: string, site_type: string, site_url: string, styling: record, twitter_account: string>, title: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/inbox" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "inbox-unread get" [
  ...
  let full_url = (build-url $base "/inbox/unread" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"pagesize": $pagesize, "page": $page, "filter": $filter, "callback": $callback, "since": $since} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "info get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> record<answers_per_minute: float, api_revision: string, badges_per_minute: float, new_active_users: int, questions_per_minute: float, site: record<aliases: list<any>, api_site_parameter: string, audience: string, closed_beta_date: int, favicon_url: string, high_resolution_icon_url: string, icon_url: string, launch_date: int, logo_url: string, markdown_extensions: list<any>, name: string, open_beta_date: int, related_sites: list<any>, site_state: string, site_type: string, site_url: string, styling: record<link_color: string, tag_background_color: string, tag_foreground_color: string>, twitter_account: string>, total_accepted: int, total_answers: int, total_badges: int, total_comments: int, total_questions: int, total_unanswered: int, total_users: int, total_votes: int> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
...
export def "me get" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> record<about_me: string, accept_rate: int, account_id: int, age: int, answer_count: int, badge_counts: record<bronze: int, gold: int, silver: int>, creation_date: int, display_name: string, down_vote_count: int, is_employee: bool, last_access_date: int, last_modified_date: int, link: string, location: string, profile_image: string, question_count: int, reputation: int, reputation_change_day: int, reputation_change_month: int, reputation_change_quarter: int, reputation_change_week: int, reputation_change_year: int, timed_penalty_date: int, up_vote_count: int, user_id: int, user_type: string, view_count: int, website_url: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
...
export def "me-answers get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/answers" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-associated get" [
  ...
  --filter: string # #Discussion The Stack Exchange API allows applications to exclude almost every field returned. For example, if an application did not care about a user's badge counts it could exclude user.badge_counts whenever it calls a method that returns users. An application excludes fields by creating a filter (via /filter/create) and passing it to a method in the filter parameter. Filters are immutable and non-expiring. An application can safely "bake in" any filters that are created, it is not necessary (or advisable) to create filters at runtime. The motivation for filters are several fold. Filters allow applications to reduce API responses to just the fields they are concerned with, saving bandwidth. With the list of fields an application is actually concerned with, the API can avoid unneccessary queries thereby decreasing response time (and reducing load on our infrastructure). Finally, filters allow us to be more conservative in what the API returns by default without a proliferation of parameters (as was seen with body, answers, and comments in the 1.x API family). #Safety Filters also carry a notion of safety, which is defined as follows. Any string returned as a result of an API call with a safe filter will be inline-able into HTML without script-injection concerns. That is to say, no additional sanitizing (encoding, HTML tag stripping, etc.) will be necessary on returned strings. Applications that wish to handle sanitizing themselves should create an unsafe filter. All filters are safe by default, under the assumption that double-encoding bugs are more desirable than script injections. Note that this does not mean that "safe" filter is mearly an "unsafe" one with all fields passed though UrlEncode(...). Many fields can and will contain HTML in all filter types (most notably, the *.body fields). When using unsafe filters, the API returns the highest fidelity data it can reasonably access for the given request. This means that in cases where the "safe" data is the only accessible data it will be returned even in "unsafe" filters. Notably the *.body fields are unchanged, as they are stored in that form. Fields that are unchanged between safe and unsafe filters are denoted in their types documentation. #Built In Filters The following filters are built in: default, each type documents which fields are returned under the default filter (for example, answers). withbody, which is default plus the *.body fields none, which is empty total, which includes just .total #Compatibility with V1.x For ease of transition from earlier API versions, the filters _b, _ba, _bc, _bca, _a, _ac, and _c are also built in. These are unsafe, and exclude a combination of question and answer body, comments, and answers so as to mimic the body, answers, and comments parameters that have been removed in V2.0. New applications should not use these filters.
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
]: nothing -> table<account_id: int, answer_count: int, badge_counts: record<bronze: int, gold: int, silver: int>, creation_date: int, last_access_date: int, question_count: int, reputation: int, site_name: string, site_url: string, top_answers: list<any>, top_questions: list<any>, user_id: int, user_type: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/associated" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-badges get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/badges" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-comments list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/comments" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-comments get" [
  ...
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({to_id: (encode-path-segment $to_id)} | format pattern "/me/comments/{to_id}") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-favorites get" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<accepted_answer_id: int, answer_count: int, answers: list<any>, body: string, body_markdown: string, bounty_amount: int, bounty_closes_date: int, bounty_user: record<accept_rate: int, badge_counts: record, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>, can_close: bool, can_flag: bool, close_vote_count: int, closed_date: int, closed_details: record<by_users: list, description: string, on_hold: bool, original_questions: list, reason: string>, closed_reason: string, comment_count: int, comments: list<any>, community_owned_date: int, creation_date: int, delete_vote_count: int, down_vote_count: int, downvoted: bool, favorite_count: int, favorited: bool, is_answered: bool, last_activity_date: int, last_edit_date: int, last_editor: record<accept_rate: int, badge_counts: record, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>, link: string, locked_date: int, migrated_from: record<on_date: int, other_site: record, question_id: int>, migrated_to: record<on_date: int, other_site: record, question_id: int>, notice: record<body: string, creation_date: int, owner_user_id: int>, owner: record<accept_rate: int, badge_counts: record, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>, protected_date: int, question_id: int, reopen_vote_count: int, score: int, share_link: string, tags: list<any>, title: string, up_vote_count: int, upvoted: bool, view_count: int> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/favorites" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-inbox get" [
  ...
  let full_url = (build-url $base "/me/inbox" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"pagesize": $pagesize, "page": $page, "filter": $filter, "callback": $callback, "site": $site} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "me-inbox-unread get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar") (serialize-qp "since" $since "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/inbox/unread" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-mentioned get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/mentioned" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-merges get" [
  ...
  --filter: string # #Discussion The Stack Exchange API allows applications to exclude almost every field returned. For example, if an application did not care about a user's badge counts it could exclude user.badge_counts whenever it calls a method that returns users. An application excludes fields by creating a filter (via /filter/create) and passing it to a method in the filter parameter. Filters are immutable and non-expiring. An application can safely "bake in" any filters that are created, it is not necessary (or advisable) to create filters at runtime. The motivation for filters are several fold. Filters allow applications to reduce API responses to just the fields they are concerned with, saving bandwidth. With the list of fields an application is actually concerned with, the API can avoid unneccessary queries thereby decreasing response time (and reducing load on our infrastructure). Finally, filters allow us to be more conservative in what the API returns by default without a proliferation of parameters (as was seen with body, answers, and comments in the 1.x API family). #Safety Filters also carry a notion of safety, which is defined as follows. Any string returned as a result of an API call with a safe filter will be inline-able into HTML without script-injection concerns. That is to say, no additional sanitizing (encoding, HTML tag stripping, etc.) will be necessary on returned strings. Applications that wish to handle sanitizing themselves should create an unsafe filter. All filters are safe by default, under the assumption that double-encoding bugs are more desirable than script injections. Note that this does not mean that "safe" filter is mearly an "unsafe" one with all fields passed though UrlEncode(...). Many fields can and will contain HTML in all filter types (most notably, the *.body fields). When using unsafe filters, the API returns the highest fidelity data it can reasonably access for the given request. This means that in cases where the "safe" data is the only accessible data it will be returned even in "unsafe" filters. Notably the *.body fields are unchanged, as they are stored in that form. Fields that are unchanged between safe and unsafe filters are denoted in their types documentation. #Built In Filters The following filters are built in: default, each type documents which fields are returned under the default filter (for example, answers). withbody, which is default plus the *.body fields none, which is empty total, which includes just .total #Compatibility with V1.x For ease of transition from earlier API versions, the filters _b, _ba, _bc, _bca, _a, _ac, and _c are also built in. These are unsafe, and exclude a combination of question and answer body, comments, and answers so as to mimic the body, answers, and comments parameters that have been removed in V2.0. New applications should not use these filters.
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
]: nothing -> table<merge_date: int, new_account_id: int, old_account_id: int> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/merges" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-notifications get" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<body: string, creation_date: int, is_unread: bool, notification_type: string, post_id: int, site: record<aliases: list, api_site_parameter: string, audience: string, closed_beta_date: int, favicon_url: string, high_resolution_icon_url: string, icon_url: string, launch_date: int, logo_url: string, markdown_extensions: list, name: string, open_beta_date: int, related_sites: list, site_state: string, site_type: string, site_url: string, styling: record, twitter_account: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/notifications" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-notifications-unread get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/notifications/unread" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-privileges get" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<description: string, reputation: int, short_description: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/privileges" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-questions get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/questions" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-questions-featured get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/questions/featured" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-questions-no-answers get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/questions/no-answers" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-questions-unaccepted get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/questions/unaccepted" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-questions-unanswered get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/questions/unanswered" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-reputation get" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<link: string, on_date: int, post_id: int, post_type: string, reputation_change: int, title: string, user_id: int, vote_type: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  ...
  let full_url = (build-url $base "/me/reputation" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"filter": $filter, "callback": $callback, "site": $site} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "me-reputation-history get" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<creation_date: int, post_id: int, reputation_change: int, reputation_history_type: string, user_id: int> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/reputation-history" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-reputation-history-full get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/reputation-history/full" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-suggested-edits get" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<approval_date: int, body: string, comment: string, creation_date: int, post_id: int, post_type: string, proposing_user: record<accept_rate: int, badge_counts: record, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>, rejection_date: int, suggested_edit_id: int, tags: list<any>, title: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/suggested-edits" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-tags get" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<count: int, has_synonyms: bool, is_moderator_only: bool, is_required: bool, last_activity_date: int, name: string, synonyms: list<any>, user_id: int> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/tags" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-tags-top-answers get" [
  ...
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({tags: (encode-path-segment $tags)} | format pattern "/me/tags/{tags}/top-answers") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-tags-top-questions get" [
  ...
  if ($tags | is-empty) { error make --unspanned { msg: "path parameter 'tags' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({tags: (encode-path-segment $tags)} | format pattern "/me/tags/{tags}/top-questions") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-timeline get" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<badge_id: int, comment_id: int, creation_date: int, detail: string, link: string, post_id: int, post_type: string, suggested_edit_id: int, timeline_type: string, title: string, user_id: int> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/timeline" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-top-answer-tags get" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<answer_count: int, answer_score: int, question_count: int, question_score: int, tag_name: string, user_id: int> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/top-answer-tags" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-top-question-tags get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/top-question-tags" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "me-write-permissions get" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<can_add: bool, can_delete: bool, can_edit: bool, max_daily_actions: int, min_seconds_between_actions: int, object_type: string, user_id: int> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/me/write-permissions" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "notifications-unread get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/notifications/unread" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "posts list" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<body: string, body_markdown: string, comment_count: int, comments: list<any>, creation_date: int, down_vote_count: int, downvoted: bool, last_activity_date: int, last_edit_date: int, last_editor: record<accept_rate: int, badge_counts: record, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>, link: string, owner: record<accept_rate: int, badge_counts: record, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>, post_id: int, post_type: string, score: int, share_link: string, title: string, up_vote_count: int, upvoted: bool> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
...
export def "posts get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/posts/{ids}") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "posts-comments get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/posts/{ids}/comments") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "posts-revisions get" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<body: string, comment: string, creation_date: int, is_rollback: bool, last_body: string, last_tags: list<any>, last_title: string, post_id: int, post_type: string, revision_guid: string, revision_number: int, revision_type: string, set_community_wiki: bool, tags: list<any>, title: string, user: record<accept_rate: int, badge_counts: record, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/posts/{ids}/revisions") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "posts-suggested-edits get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/posts/{ids}/suggested-edits") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "posts-comments-add create" [
  id: int
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let qp = [(serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar") (serialize-qp "body" $body "scalar") (serialize-qp "preview" $preview "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/posts/{id}/comments/add") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "privileges get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/privileges" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "questions list" [
  ...
  let full_url = (build-url $base "/questions" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"tagged": $tagged, "order": $order, "max": $max, "min": $min, "sort": $qp_sort, "fromdate": $fromdate, "todate": $todate, "pagesize": $pagesize, "page": $page, "filter": $filter, "callback": $callback, "site": $site} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "questions-featured get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "tagged" $tagged "scalar") (serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/questions/featured" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "questions-no-answers get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "tagged" $tagged "scalar") (serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/questions/no-answers" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "questions-unanswered get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "tagged" $tagged "scalar") (serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/questions/unanswered" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "questions get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/questions/{ids}") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "questions-answers get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/questions/{ids}/answers") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "questions-comments get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/questions/{ids}/comments") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "questions-linked get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/questions/{ids}/linked") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "questions-related get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/questions/{ids}/related") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "questions-timeline get" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<comment_id: int, creation_date: int, down_vote_count: int, owner: record<accept_rate: int, badge_counts: record, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>, post_id: int, question_id: int, revision_guid: string, timeline_type: string, up_vote_count: int, user: record<accept_rate: int, badge_counts: record, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/questions/{ids}/timeline") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "revisions get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/revisions/{ids}") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "search get" [
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"tagged": $tagged, "order": $order, "max": $max, "min": $min, "sort": $qp_sort, "fromdate": $fromdate, "todate": $todate, "pagesize": $pagesize, "page": $page, "filter": $filter, "callback": $callback, "site": $site, "intitle": $intitle, "nottagged": $nottagged} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "search-advanced get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "tagged" $tagged "scalar") (serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar") (serialize-qp "accepted" $accepted "scalar") (serialize-qp "answers" $answers "scalar") (serialize-qp "body" $body "scalar") (serialize-qp "closed" $closed "scalar") (serialize-qp "migrated" $migrated "scalar") (serialize-qp "notice" $notice "scalar") (serialize-qp "nottagged" $nottagged "scalar") (serialize-qp "q" $q "scalar") (serialize-qp "title" $title "scalar") (serialize-qp "url" $url "scalar") (serialize-qp "user" $user "scalar") (serialize-qp "views" $views "scalar") (serialize-qp "wiki" $wiki "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/search/advanced" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"tagged": $tagged, "order": $order, "max": $max, "min": $min, "sort": $qp_sort, "fromdate": $fromdate, "todate": $todate, "pagesize": $pagesize, "page": $page, "filter": $filter, "callback": $callback, "site": $site, "accepted": $accepted, "answers": $answers, "body": $body, "closed": $closed, "migrated": $migrated, "notice": $notice, "nottagged": $nottagged, "q": $q, "title": $title, "url": $url, "user": $user, "views": $views, "wiki": $wiki} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "similar get" [
  ...
  let full_url = (build-url $base "/similar" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"tagged": $tagged, "order": $order, "max": $max, "min": $min, "sort": $qp_sort, "fromdate": $fromdate, "todate": $todate, "pagesize": $pagesize, "page": $page, "filter": $filter, "callback": $callback, "site": $site, "nottagged": $nottagged, "title": $title} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "sites get" [
  ...
  --filter: string # #Discussion The Stack Exchange API allows applications to exclude almost every field returned. For example, if an application did not care about a user's badge counts it could exclude user.badge_counts whenever it calls a method that returns users. An application excludes fields by creating a filter (via /filter/create) and passing it to a method in the filter parameter. Filters are immutable and non-expiring. An application can safely "bake in" any filters that are created, it is not necessary (or advisable) to create filters at runtime. The motivation for filters are several fold. Filters allow applications to reduce API responses to just the fields they are concerned with, saving bandwidth. With the list of fields an application is actually concerned with, the API can avoid unneccessary queries thereby decreasing response time (and reducing load on our infrastructure). Finally, filters allow us to be more conservative in what the API returns by default without a proliferation of parameters (as was seen with body, answers, and comments in the 1.x API family). #Safety Filters also carry a notion of safety, which is defined as follows. Any string returned as a result of an API call with a safe filter will be inline-able into HTML without script-injection concerns. That is to say, no additional sanitizing (encoding, HTML tag stripping, etc.) will be necessary on returned strings. Applications that wish to handle sanitizing themselves should create an unsafe filter. All filters are safe by default, under the assumption that double-encoding bugs are more desirable than script injections. Note that this does not mean that "safe" filter is mearly an "unsafe" one with all fields passed though UrlEncode(...). Many fields can and will contain HTML in all filter types (most notably, the *.body fields). When using unsafe filters, the API returns the highest fidelity data it can reasonably access for the given request. This means that in cases where the "safe" data is the only accessible data it will be returned even in "unsafe" filters. Notably the *.body fields are unchanged, as they are stored in that form. Fields that are unchanged between safe and unsafe filters are denoted in their types documentation. #Built In Filters The following filters are built in: default, each type documents which fields are returned under the default filter (for example, answers). withbody, which is default plus the *.body fields none, which is empty total, which includes just .total #Compatibility with V1.x For ease of transition from earlier API versions, the filters _b, _ba, _bc, _bca, _a, _ac, and _c are also built in. These are unsafe, and exclude a combination of question and answer body, comments, and answers so as to mimic the body, answers, and comments parameters that have been removed in V2.0. New applications should not use these filters.
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
]: nothing -> table<aliases: list<any>, api_site_parameter: string, audience: string, closed_beta_date: int, favicon_url: string, high_resolution_icon_url: string, icon_url: string, launch_date: int, logo_url: string, markdown_extensions: list<any>, name: string, open_beta_date: int, related_sites: list<any>, site_state: string, site_type: string, site_url: string, styling: record<link_color: string, tag_background_color: string, tag_foreground_color: string>, twitter_account: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar")] | flatten | str join "&"
...
export def "suggested-edits list" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/suggested-edits" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "suggested-edits get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/suggested-edits/{ids}") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "tags-moderator-only get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "inname" $inname "scalar") (serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/tags/moderator-only" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "tags-required get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "inname" $inname "scalar") (serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/tags/required" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "tags-synonyms list" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<applied_count: int, creation_date: int, from_tag: string, last_applied_date: int, to_tag: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/tags/synonyms" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "tags-faq get" [
  ...
  if ($tags | is-empty) { error make --unspanned { msg: "path parameter 'tags' must be non-empty" } }
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({tags: (encode-path-segment $tags)} | format pattern "/tags/{tags}/faq") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "tags-info get" [
  ...
  if ($tags | is-empty) { error make --unspanned { msg: "path parameter 'tags' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({tags: (encode-path-segment $tags)} | format pattern "/tags/{tags}/info") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "tags-related get" [
  ...
  if ($tags | is-empty) { error make --unspanned { msg: "path parameter 'tags' must be non-empty" } }
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({tags: (encode-path-segment $tags)} | format pattern "/tags/{tags}/related") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "tags-synonyms get" [
  ...
  if ($tags | is-empty) { error make --unspanned { msg: "path parameter 'tags' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({tags: (encode-path-segment $tags)} | format pattern "/tags/{tags}/synonyms") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "tags-wikis get" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<body: string, body_last_edit_date: int, excerpt: string, excerpt_last_edit_date: int, last_body_editor: record<accept_rate: int, badge_counts: record, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>, last_excerpt_editor: record<accept_rate: int, badge_counts: record, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>, tag_name: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($tags | is-empty) { error make --unspanned { msg: "path parameter 'tags' must be non-empty" } }
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({tags: (encode-path-segment $tags)} | format pattern "/tags/{tags}/wikis") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "tags-top-answerers get" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<post_count: int, score: int, user: record<accept_rate: int, badge_counts: record, display_name: string, link: string, profile_image: string, reputation: int, user_id: int, user_type: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  ...
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({tag: (encode-path-segment $tag), period: (encode-path-segment $period)} | format pattern "/tags/{tag}/top-answerers/{period}") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "tags-top-askers get" [
  ...
  if ($period | is-empty) { error make --unspanned { msg: "path parameter 'period' must be non-empty" } }
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({tag: (encode-path-segment $tag), period: (encode-path-segment $period)} | format pattern "/tags/{tag}/top-askers/{period}") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users list" [
  ...
  --callback: string # All API responses are JSON, we do support JSONP with the callback query parameter.
  --site: string # Each of these methods operates on a single site at a time, identified by the site parameter. This parameter can be the full domain name (ie. "stackoverflow.com"), or a short form identified by api_site_parameter on the site object.
]: nothing -> table<about_me: string, accept_rate: int, account_id: int, age: int, answer_count: int, badge_counts: record<bronze: int, gold: int, silver: int>, creation_date: int, display_name: string, down_vote_count: int, is_employee: bool, last_access_date: int, last_modified_date: int, link: string, location: string, profile_image: string, question_count: int, reputation: int, reputation_change_day: int, reputation_change_month: int, reputation_change_quarter: int, reputation_change_week: int, reputation_change_year: int, timed_penalty_date: int, up_vote_count: int, user_id: int, user_type: string, view_count: int, website_url: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "inname" $inname "scalar") (serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
...
export def "users-moderators get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/users/moderators" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-moderators-elected get" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/users/moderators/elected" $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/users/{ids}") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-answers get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/users/{ids}/answers") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-associated get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/users/{ids}/associated") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-badges get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/users/{ids}/badges") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-comments list" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/users/{ids}/comments") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-comments get" [
  ...
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids), toid: (encode-path-segment $toid)} | format pattern "/users/{ids}/comments/{toid}") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-favorites get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/users/{ids}/favorites") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-mentioned get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/users/{ids}/mentioned") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-merges get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/users/{ids}/merges") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-questions get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/users/{ids}/questions") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-questions-featured get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/users/{ids}/questions/featured") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-questions-no-answers get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/users/{ids}/questions/no-answers") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-questions-unaccepted get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/users/{ids}/questions/unaccepted") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-questions-unanswered get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/users/{ids}/questions/unanswered") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-reputation get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/users/{ids}/reputation") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-reputation-history get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/users/{ids}/reputation-history") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-suggested-edits get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/users/{ids}/suggested-edits") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-tags get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/users/{ids}/tags") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-timeline get" [
  ...
  if ($ids | is-empty) { error make --unspanned { msg: "path parameter 'ids' must be non-empty" } }
  let qp = [(serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({ids: (encode-path-segment $ids)} | format pattern "/users/{ids}/timeline") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-inbox get" [
  ...
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/users/{id}/inbox") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-inbox-unread get" [
  ...
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar") (serialize-qp "since" $since "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/users/{id}/inbox/unread") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-notifications get" [
  ...
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/users/{id}/notifications") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-notifications-unread get" [
  ...
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/users/{id}/notifications/unread") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-privileges get" [
  ...
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/users/{id}/privileges") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-reputation-history-full get" [
  ...
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/users/{id}/reputation-history/full") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-tags-top-answers get" [
  ...
  if ($tags | is-empty) { error make --unspanned { msg: "path parameter 'tags' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({id: (encode-path-segment $id), tags: (encode-path-segment $tags)} | format pattern "/users/{id}/tags/{tags}/top-answers") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-tags-top-questions get" [
  ...
  if ($tags | is-empty) { error make --unspanned { msg: "path parameter 'tags' must be non-empty" } }
  let qp = [(serialize-qp "order" $order "scalar") (serialize-qp "max" $max "scalar") (serialize-qp "min" $min "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "fromdate" $fromdate "scalar") (serialize-qp "todate" $todate "scalar") (serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({id: (encode-path-segment $id), tags: (encode-path-segment $tags)} | format pattern "/users/{id}/tags/{tags}/top-questions") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-top-answer-tags get" [
  ...
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/users/{id}/top-answer-tags") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-top-question-tags get" [
  ...
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/users/{id}/top-question-tags") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "users-write-permissions get" [
  ...
  if ($id | is-empty) { error make --unspanned { msg: "path parameter 'id' must be non-empty" } }
  let qp = [(serialize-qp "pagesize" $pagesize "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "site" $site "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/users/{id}/write-permissions") $qp $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
