...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o FLIGHT_MOST_BOOKED_DESTINATIONS_TOKEN | default "" }
...
export def "travel-analytics-air-traffic-booked get" [
  ...
  let full_url = (build-url $base "/travel/analytics/air-traffic/booked" $qp $auth.query)
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"originCityCode": $origin_city_code, "period": $period, "max": $max, "fields": $fields, "page[limit]": $page_limit, "page[offset]": $page_offset, "sort": $qp_sort} | compact)
