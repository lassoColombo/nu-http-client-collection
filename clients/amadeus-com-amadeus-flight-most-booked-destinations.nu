# Auto-generated client for Flight Most Booked Destinations v1.1.1
# Source: https://api.apis.guru/v2/specs/amadeus.com/amadeus-flight-most-booked-destinations/1.1.1/swagger.json
# Auth: --token flag or $env.FLIGHT_MOST_BOOKED_DESTINATIONS_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o FLIGHT_MOST_BOOKED_DESTINATIONS_TOKEN | default "" }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "travel-analytics-air-traffic-booked get" } } | get name | first)
...
export def "travel-analytics-air-traffic-booked get" [
