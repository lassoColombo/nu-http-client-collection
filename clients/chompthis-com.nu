...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o CHOMP_FOOD_DATABASE_API_DOCUMENTATION_TOKEN | default "" }
...
export def "food-branded-barcode-php get" [
  ...
  let full_url = (build-url $base "/food/branded/barcode.php" $qp $auth.query)
...
export def "food-branded-name-php get" [
  ...
  let full_url = (build-url $base "/food/branded/name.php" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"name": $name, "limit": $limit, "page": $page} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "food-branded-search-php get" [
  ...
  let full_url = (build-url $base "/food/branded/search.php" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"allergen": $allergen, "brand": $brand, "category": $category, "country": $country, "diet": $diet, "ingredient": $ingredient, "keyword": $keyword, "mineral": $mineral, "nutrient": $nutrient, "palm_oil": $palm_oil, "trace": $trace, "vitamin": $vitamin, "limit": $limit, "page": $page} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "food-ingredient-search-php get" [
  ...
  let full_url = (build-url $base "/food/ingredient/search.php" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"find": $find, "limit": $limit} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
