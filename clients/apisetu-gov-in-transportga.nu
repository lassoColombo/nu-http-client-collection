# Auto-generated client for Transport Department, Goa v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/transportga/3.0.0/openapi.json
# Auth: --token flag or $env.TRANSPORT_DEPARTMENT_GOA_TOKEN

const BASE_URL = "https://apisetu.gov.in/transportga/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o TRANSPORT_DEPARTMENT_GOA_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/transportga/v3"] }
...
export def "drvlc-certificate create" [
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o TRANSPORT_DEPARTMENT_GOA_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o TRANSPORT_DEPARTMENT_GOA_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
