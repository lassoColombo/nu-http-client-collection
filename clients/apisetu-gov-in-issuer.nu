# Auto-generated client for DigiLocker Issuer APIs v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/issuer/3.0.0/openapi.json
# Auth: --token flag or $env.DIGILOCKER_ISSUER_APIS_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o DIGILOCKER_ISSUER_APIS_TOKEN | default "" }
...
def base-url-completer [] { ["Your API URL"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "your-pull-doc-request-api-path pull" } } | get name | first)
...
export def "your-pull-doc-request-api-path pull" [
  ...
  let effective_ct = ($content_type | default "application/xml")
...
export def "your-pull-uri-request-api-path pull" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
