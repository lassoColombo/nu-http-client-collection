...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "subscriptions-providers-microsoft-network-firewall-policies ListAll" } } | get name | first)
...
export def "subscriptions-providers-microsoft-network-firewall-policies ListAll" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/providers/Microsoft.Network/firewallPolicies" $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-network-firewall-policies List" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Network/firewallPolicies" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "subscriptions-resource-groups-providers-microsoft-network-firewall-policies Delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Network/firewallPolicies/($firewallPolicyName)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "subscriptions-resource-groups-providers-microsoft-network-firewall-policies UpdateTags" [
  subscriptionId: string
  resourceGroupName: string
  firewallPolicyName: string
  --base-url(-b): string@base-url-completer # API base URL
...
export def "subscriptions-resource-groups-providers-microsoft-network-firewall-policies CreateOrUpdate" [
  resourceGroupName: string
  firewallPolicyName: string
  subscriptionId: string
  --base-url(-b): string@base-url-completer # API base URL
...
export def "subscriptions-resource-groups-providers-microsoft-network-firewall-policies-rule-groups List" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Network/firewallPolicies/($firewallPolicyName)/ruleGroups" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "subscriptions-resource-groups-providers-microsoft-network-firewall-policies-rule-groups Delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Network/firewallPolicies/($firewallPolicyName)/ruleGroups/($ruleGroupName)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "subscriptions-resource-groups-providers-microsoft-network-firewall-policies-rule-groups CreateOrUpdate" [
  resourceGroupName: string
  firewallPolicyName: string
  ruleGroupName: string
  subscriptionId: string
