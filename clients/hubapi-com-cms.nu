...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o DOMAINS_TOKEN | default "" }
...
export def "cms-domains get-page" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"createdAt": $created_at, "createdAfter": $created_after, "createdBefore": $created_before, "updatedAt": $updated_at, "updatedAfter": $updated_after, "updatedBefore": $updated_before, "sort": $qp_sort, "after": $after, "limit": $limit, "archived": $archived} | compact), body: null}
}
