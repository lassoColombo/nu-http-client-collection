...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o BEANSTREAM_PAYMENTS_TOKEN | default "" }
...
export def "payments get" [
  ...
  let full_url = (build-url $base ({trans_id: (encode-path-segment $trans_id)} | format pattern "/payments/{trans_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "payments-completions create" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($trans_id | is-empty) { error make --unspanned { msg: "path parameter 'transId' must be non-empty" } }
  let full_url = (build-url $base ({trans_id: (encode-path-segment $trans_id)} | format pattern "/payments/{trans_id}/completions") $auth.query)
  let req_body = {"amount": $amount, "billing": $billing, "card": $card, "comments": $comments, "custom": $custom, "customer_ip": $customer_ip, "language": $language, "order_number": $order_number, "payment_method": $payment_method, "payment_profile": $payment_profile, "shipping": $shipping, "term_url": $term_url, "token": $body_token} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "payments-returns create" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($trans_id | is-empty) { error make --unspanned { msg: "path parameter 'transId' must be non-empty" } }
  let full_url = (build-url $base ({trans_id: (encode-path-segment $trans_id)} | format pattern "/payments/{trans_id}/returns") $auth.query)
...
export def "payments-void create" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($trans_id | is-empty) { error make --unspanned { msg: "path parameter 'transId' must be non-empty" } }
  let full_url = (build-url $base ({trans_id: (encode-path-segment $trans_id)} | format pattern "/payments/{trans_id}/void") $auth.query)
...
export def "profiles delete" [
  ...
  let full_url = (build-url $base ({profile_id: (encode-path-segment $profile_id)} | format pattern "/profiles/{profile_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "profiles-cards get" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($profile_id | is-empty) { error make --unspanned { msg: "path parameter 'profileId' must be non-empty" } }
  let full_url = (build-url $base ({profile_id: (encode-path-segment $profile_id)} | format pattern "/profiles/{profile_id}/cards") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "profiles-cards delete" [
  ...
  let full_url = (build-url $base ({profile_id: (encode-path-segment $profile_id), card_id: (encode-path-segment $card_id)} | format pattern "/profiles/{profile_id}/cards/{card_id}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "delete"
...
export def "scripts-tokenization-tokens create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/scripts/tokenization/tokens" $auth.query)
