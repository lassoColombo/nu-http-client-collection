...
export def "subscriptions-resource-groups-providers-microsoft-hd-insight-clusters-extensions-clustermonitoring disable-monitoring" [
  ...
  let full_url = (build-url $base ({subscription_id: $subscription_id, resource_group_name: $resource_group_name, cluster_name: $cluster_name} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.HDInsight/clusters/{cluster_name}/extensions/clustermonitoring") $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-hd-insight-clusters-extensions-clustermonitoring enable-monitoring" [
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: $subscription_id, resource_group_name: $resource_group_name, cluster_name: $cluster_name} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.HDInsight/clusters/{cluster_name}/extensions/clustermonitoring") $qp)
  let body = {"primaryKey": $primary_key, "workspaceId": $workspace_id} | compact
...
export def "subscriptions-resource-groups-providers-microsoft-hd-insight-clusters-extensions delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({subscription_id: $subscription_id, resource_group_name: $resource_group_name, cluster_name: $cluster_name, extension_name: $extension_name} | format pattern "/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.HDInsight/clusters/{cluster_name}/extensions/{extension_name}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
