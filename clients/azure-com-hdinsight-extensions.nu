...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "subscriptions-resource-groups-providers-microsoft-hd-insight-clusters-extensions-clustermonitoring DisableMonitoring" } } | get name | first)
...
export def "subscriptions-resource-groups-providers-microsoft-hd-insight-clusters-extensions-clustermonitoring DisableMonitoring" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.HDInsight/clusters/($clusterName)/extensions/clustermonitoring" $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-hd-insight-clusters-extensions-clustermonitoring GetMonitoringStatus" [
  subscriptionId: string
  resourceGroupName: string
  clusterName: string
  --base-url(-b): string@base-url-completer # API base URL
...
export def "subscriptions-resource-groups-providers-microsoft-hd-insight-clusters-extensions-clustermonitoring EnableMonitoring" [
  subscriptionId: string
  resourceGroupName: string
  clusterName: string
  --base-url(-b): string@base-url-completer # API base URL
  ...
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.HDInsight/clusters/($clusterName)/extensions/clustermonitoring" $qp)
  let body = {primaryKey: $primaryKey, workspaceId: $workspaceId} | compact
...
export def "subscriptions-resource-groups-providers-microsoft-hd-insight-clusters-extensions Delete" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "api-version" $api_version "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.HDInsight/clusters/($clusterName)/extensions/($extensionName)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "delete" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
