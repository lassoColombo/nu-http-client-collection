# Auto-generated client for Browse API vv1.1.0
# Source: https://api.apis.guru/v2/specs/ebay.com/buy-browse/v1.1.0/swagger.json
# Auth: --token flag or $env.BROWSE_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o BROWSE_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.ebay.com/buy/browse/v1"] }
...
export def "item list" [
  ...
  --item-ids: string # A list of item IDs. Item IDs are the eBay RESTful identifier of items. RESTful Item ID Format: v1|#|# For example: v1|272394640372|0 or v1|162846450672|461882996982 In any given request, either item_ids or item_group_ids can be retrieved. Attempting to retrieve both will result in an error. In a request, multiple item_ids can be passed as comma separated values. Maximum allowed itemIDs: 20 For more information about item IDs for RESTful APIs, see the Legacy API compatibility section of the Buy APIs Overview.
  --item-group-ids: string # A list of item group IDs. Item group IDs are the eBay RESTful identifier of item groups. RESTful Group Item ID Format: ############ For example: 330017835749 or 330017835740 In any given request, either item_ids or item_group_ids can be retrieved. Attempting to retrieve both will result in an error. In a request, multiple item_group_ids can be passed as comma separated values. Maximum allowed itemGroupIDs: 10
]: nothing -> record<items: table<additionalImages: list, adultOnly: bool, ageGroup: string, availableCoupons: list, bidCount: int, brand: string, buyingOptions: list, categoryId: string, categoryPath: string, color: string, condition: string, conditionDescription: string, conditionId: string, currentBidPrice: record, description: string, eligibleForInlineCheckout: bool, enabledForGuestCheckout: bool, energyEfficiencyClass: string, epid: string, estimatedAvailabilities: list, gender: string, gtin: string, image: record, inferredEpid: string, itemAffiliateWebUrl: string, itemEndDate: string, itemId: string, itemLocation: record, itemWebUrl: string, legacyItemId: string, localizedAspects: list, lotSize: int, marketingPrice: record, material: string, minimumPriceToBid: record, mpn: string, pattern: string, paymentMethods: list, price: record, priceDisplayCondition: string, primaryItemGroup: record, primaryProductReviewRating: record, product: record, productFicheWebUrl: string, qualifiedPrograms: list, quantityLimitPerBuyer: int, reservePriceMet: bool, returnTerms: record, seller: record, sellerItemRevision: string, shipToLocations: record, shippingOptions: list, shortDescription: string, size: string, sizeSystem: string, sizeType: string, subtitle: string, taxes: list, title: string, topRatedBuyingExperience: bool, uniqueBidderCount: int, unitPrice: record, unitPricingMeasure: string>, total: int, warnings: table<category: string, domain: string, errorId: int, inputRefIds: list, longMessage: string, message: string, outputRefIds: list, parameters: list, subdomain: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "item_ids" $item_ids "scalar") (serialize-qp "item_group_ids" $item_group_ids "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/item/" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"item_ids": $item_ids, "item_group_ids": $item_group_ids} | compact), body: null}
}
...
export def "item-get-item-by-legacy-id get" [
  ...
  --legacy-item-id: string # Specifies either: The legacy item ID of an item that is not part of a group. The legacy item ID of a group, which is the ID of the "parent" of the group of items. Note: If you pass in a group ID, you must also use the legacy_variation_id field and pass in the legacy ID of the specific item variation (child ID). Legacy ids are returned by APIs, such as the Finding API. The following is an example of using the value of the ItemID field for a specific item from Finding to get the RESTful itemId value. browse/v1/item/get_item_by_legacy_id?legacy_item_id=110039490209 Maximum: 1
  --legacy-variation-id: string # Specifies the legacy item ID of a specific item in an item group, such as the red shirt size L. Legacy ids are returned by APIs, such as the Finding API. Maximum: 1 Requirement: You must always pass in the legacy_item_id with the legacy_variation_id
  --legacy-variation-sku: string # Specifics the legacy SKU of the item. SKU are item ids created by the seller. Legacy SKUs are returned by eBay the Shopping API. The following is an example of using the value of the ItemID and SKU fields to get the RESTful itemId value. browse/v1/item/get_item_by_legacy_id?legacy_item_id=110039490209&legacy_variation_sku=V-00031-WHM Maximum: 1 Requirement: You must always pass in the legacy_item_id with the legacy_variation_sku
]: nothing -> record<additionalImages: table<height: int, imageUrl: string, width: int>, adultOnly: bool, ageGroup: string, availableCoupons: table<constraint: string, discountAmount: record, discountType: string, message: string, redemptionCode: string, termsWebUrl: string>, bidCount: int, brand: string, buyingOptions: list<string>, categoryId: string, categoryPath: string, color: string, condition: string, conditionDescription: string, conditionId: string, currentBidPrice: record<convertedFromCurrency: string, convertedFromValue: string, currency: string, value: string>, description: string, eligibleForInlineCheckout: bool, enabledForGuestCheckout: bool, energyEfficiencyClass: string, epid: string, estimatedAvailabilities: table<availabilityThreshold: int, availabilityThresholdType: string, deliveryOptions: list, estimatedAvailabilityStatus: string, estimatedAvailableQuantity: int, estimatedSoldQuantity: int>, gender: string, gtin: string, image: record<height: int, imageUrl: string, width: int>, inferredEpid: string, itemAffiliateWebUrl: string, itemEndDate: string, itemId: string, itemLocation: record<addressLine1: string, addressLine2: string, city: string, country: string, county: string, postalCode: string, stateOrProvince: string>, itemWebUrl: string, legacyItemId: string, localizedAspects: table<name: string, type: string, value: string>, lotSize: int, marketingPrice: record<discountAmount: record<convertedFromCurrency: string, convertedFromValue: string, currency: string, value: string>, discountPercentage: string, originalPrice: record<convertedFromCurrency: string, convertedFromValue: string, currency: string, value: string>, priceTreatment: string>, material: string, minimumPriceToBid: record<convertedFromCurrency: string, convertedFromValue: string, currency: string, value: string>, mpn: string, pattern: string, paymentMethods: table<paymentInstructions: list, paymentMethodBrands: list, paymentMethodType: string, sellerInstructions: list>, price: record<convertedFromCurrency: string, convertedFromValue: string, currency: string, value: string>, priceDisplayCondition: string, primaryItemGroup: record<itemGroupAdditionalImages: list<record>, itemGroupHref: string, itemGroupId: string, itemGroupImage: record<height: int, imageUrl: string, width: int>, itemGroupTitle: string, itemGroupType: string>, primaryProductReviewRating: record<averageRating: string, ratingHistograms: list<record>, reviewCount: int>, product: record<additionalImages: list<record>, additionalProductIdentities: list<record>, aspectGroups: list<record>, brand: string, description: string, gtins: list<string>, image: record<height: int, imageUrl: string, width: int>, mpns: list<string>, title: string>, productFicheWebUrl: string, qualifiedPrograms: list<string>, quantityLimitPerBuyer: int, reservePriceMet: bool, returnTerms: record<extendedHolidayReturnsOffered: bool, refundMethod: string, restockingFeePercentage: string, returnInstructions: string, returnMethod: string, returnPeriod: record<unit: string, value: int>, returnShippingCostPayer: string, returnsAccepted: bool>, seller: record<feedbackPercentage: string, feedbackScore: int, sellerAccountType: string, sellerLegalInfo: record<email: string, fax: string, imprint: string, legalContactFirstName: string, legalContactLastName: string, name: string, phone: string, registrationNumber: string, sellerProvidedLegalAddress: record, termsOfService: string, vatDetails: list>, username: string>, sellerItemRevision: string, shipToLocations: record<regionExcluded: list<record>, regionIncluded: list<record>>, shippingOptions: table<additionalShippingCostPerUnit: record, cutOffDateUsedForEstimate: string, fulfilledThrough: string, guaranteedDelivery: bool, importCharges: record, maxEstimatedDeliveryDate: string, minEstimatedDeliveryDate: string, quantityUsedForEstimate: int, shipToLocationUsedForEstimate: record, shippingCarrierCode: string, shippingCost: record, shippingCostType: string, shippingServiceCode: string, trademarkSymbol: string, type: string>, shortDescription: string, size: string, sizeSystem: string, sizeType: string, subtitle: string, taxes: table<ebayCollectAndRemitTax: bool, includedInPrice: bool, shippingAndHandlingTaxed: bool, taxJurisdiction: record, taxPercentage: string, taxType: string>, title: string, topRatedBuyingExperience: bool, uniqueBidderCount: int, unitPrice: record<convertedFromCurrency: string, convertedFromValue: string, currency: string, value: string>, unitPricingMeasure: string, warnings: table<category: string, domain: string, errorId: int, inputRefIds: list, longMessage: string, message: string, outputRefIds: list, parameters: list, subdomain: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "fieldgroups" $fieldgroups "scalar") (serialize-qp "legacy_item_id" $legacy_item_id "scalar") (serialize-qp "legacy_variation_id" $legacy_variation_id "scalar") (serialize-qp "legacy_variation_sku" $legacy_variation_sku "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/item/get_item_by_legacy_id" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"fieldgroups": $fieldgroups, "legacy_item_id": $legacy_item_id, "legacy_variation_id": $legacy_variation_id, "legacy_variation_sku": $legacy_variation_sku} | compact), body: null}
}
...
export def "item-get-items-by-item-group get" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --item-group-id: string # Identifier of the item group to return. An item group is an item that has various aspect differences, such as color, size, storage capacity, etc. This ID is returned in the itemGroupHref field of the search and getItem methods. For Example: https://api.ebay.com/buy/browse/v1/item/get_items_by_item_group?item_group_id=351825690866
]: nothing -> record<commonDescriptions: table<description: string, itemIds: list>, items: table<additionalImages: list, adultOnly: bool, ageGroup: string, availableCoupons: list, bidCount: int, brand: string, buyingOptions: list, categoryId: string, categoryPath: string, color: string, condition: string, conditionDescription: string, conditionId: string, currentBidPrice: record, description: string, eligibleForInlineCheckout: bool, enabledForGuestCheckout: bool, energyEfficiencyClass: string, epid: string, estimatedAvailabilities: list, gender: string, gtin: string, image: record, inferredEpid: string, itemAffiliateWebUrl: string, itemEndDate: string, itemId: string, itemLocation: record, itemWebUrl: string, legacyItemId: string, localizedAspects: list, lotSize: int, marketingPrice: record, material: string, minimumPriceToBid: record, mpn: string, pattern: string, paymentMethods: list, price: record, priceDisplayCondition: string, primaryItemGroup: record, primaryProductReviewRating: record, product: record, productFicheWebUrl: string, qualifiedPrograms: list, quantityLimitPerBuyer: int, reservePriceMet: bool, returnTerms: record, seller: record, sellerItemRevision: string, shipToLocations: record, shippingOptions: list, shortDescription: string, size: string, sizeSystem: string, sizeType: string, subtitle: string, taxes: list, title: string, topRatedBuyingExperience: bool, uniqueBidderCount: int, unitPrice: record, unitPricingMeasure: string, warnings: list>, warnings: table<category: string, domain: string, errorId: int, inputRefIds: list, longMessage: string, message: string, outputRefIds: list, parameters: list, subdomain: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "item_group_id" $item_group_id "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/item/get_items_by_item_group" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"item_group_id": $item_group_id} | compact), body: null}
}
...
export def "item get" [
  ...
  let full_url = (build-url $base ({item_id: (encode-path-segment $item_id)} | format pattern "/item/{item_id}") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"fieldgroups": $fieldgroups} | compact), body: null}
}
...
export def "item-check-compatibility check" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($item_id | is-empty) { error make --unspanned { msg: "path parameter 'item_id' must be non-empty" } }
  let full_url = (build-url $base ({item_id: (encode-path-segment $item_id)} | format pattern "/item/{item_id}/check_compatibility"))
  let req_body = {"compatibilityProperties": $compatibility_properties} | compact
...
]: nothing -> record<autoCorrections: record<q: string>, href: string, itemSummaries: table<additionalImages: list, adultOnly: bool, availableCoupons: bool, bidCount: int, buyingOptions: list, categories: list, compatibilityMatch: string, compatibilityProperties: list, condition: string, conditionId: string, currentBidPrice: record, distanceFromPickupLocation: record, energyEfficiencyClass: string, epid: string, image: record, itemAffiliateWebUrl: string, itemGroupHref: string, itemGroupType: string, itemHref: string, itemId: string, itemLocation: record, itemWebUrl: string, legacyItemId: string, marketingPrice: record, pickupOptions: list, price: record, priceDisplayCondition: string, qualifiedPrograms: list, seller: record, shippingOptions: list, shortDescription: string, thumbnailImages: list, title: string, unitPrice: record, unitPricingMeasure: string>, limit: int, next: string, offset: int, prev: string, refinement: record<aspectDistributions: list<record>, buyingOptionDistributions: list<record>, categoryDistributions: list<record>, conditionDistributions: list<record>, dominantCategoryId: string>, total: int, warnings: table<category: string, domain: string, errorId: int, inputRefIds: list, longMessage: string, message: string, outputRefIds: list, parameters: list, subdomain: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "aspect_filter" $aspect_filter "scalar") (serialize-qp "auto_correct" $auto_correct "scalar") (serialize-qp "category_ids" $category_ids "scalar") (serialize-qp "charity_ids" $charity_ids "scalar") (serialize-qp "compatibility_filter" $compatibility_filter "scalar") (serialize-qp "epid" $epid "scalar") (serialize-qp "fieldgroups" $fieldgroups "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "gtin" $gtin "scalar") (serialize-qp "limit" $limit "scalar") (serialize-qp "offset" $offset "scalar") (serialize-qp "q" $q "scalar") (serialize-qp "sort" $qp_sort "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/item_summary/search" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"aspect_filter": $aspect_filter, "auto_correct": $auto_correct, "category_ids": $category_ids, "charity_ids": $charity_ids, "compatibility_filter": $compatibility_filter, "epid": $epid, "fieldgroups": $fieldgroups, "filter": $filter, "gtin": $gtin, "limit": $limit, "offset": $offset, "q": $q, "sort": $qp_sort} | compact), body: null}
}
...
export def "item-summary-search-by-image list" [
...
]: any -> record<autoCorrections: record<q: string>, href: string, itemSummaries: table<additionalImages: list, adultOnly: bool, availableCoupons: bool, bidCount: int, buyingOptions: list, categories: list, compatibilityMatch: string, compatibilityProperties: list, condition: string, conditionId: string, currentBidPrice: record, distanceFromPickupLocation: record, energyEfficiencyClass: string, epid: string, image: record, itemAffiliateWebUrl: string, itemGroupHref: string, itemGroupType: string, itemHref: string, itemId: string, itemLocation: record, itemWebUrl: string, legacyItemId: string, marketingPrice: record, pickupOptions: list, price: record, priceDisplayCondition: string, qualifiedPrograms: list, seller: record, shippingOptions: list, shortDescription: string, thumbnailImages: list, title: string, unitPrice: record, unitPricingMeasure: string>, limit: int, next: string, offset: int, prev: string, refinement: record<aspectDistributions: list<record>, buyingOptionDistributions: list<record>, categoryDistributions: list<record>, conditionDistributions: list<record>, dominantCategoryId: string>, total: int, warnings: table<category: string, domain: string, errorId: int, inputRefIds: list, longMessage: string, message: string, outputRefIds: list, parameters: list, subdomain: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"aspect_filter": $aspect_filter, "category_ids": $category_ids, "charity_ids": $charity_ids, "fieldgroups": $fieldgroups, "filter": $filter, "limit": $limit, "offset": $offset, "sort": $qp_sort} | compact), body: $req_body}
}
...
export def "shopping-cart-add-item create" [
  ...
  let req_body = {"itemId": $item_id, "quantity": $quantity} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: {}, body: $req_body}
...
export def "shopping-cart-remove-item delete" [
  ...
  let req_body = {"cartItemId": $cart_item_id} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: {}, body: $req_body}
...
export def "shopping-cart-update-quantity update" [
  ...
  let req_body = {"cartItemId": $cart_item_id, "quantity": $quantity} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: {}, body: $req_body}
