...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o ORGHUNTER_TOKEN | default "" }
...
export def "categories get" [
  ...
  let full_url = (build-url $base "/v1/categories" $auth.query)
...
export def "charitybasic create" [
  ...
  let full_url = (build-url $base "/v1/charitybasic" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "charityfinancial create" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "ein" $ein "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/v1/charityfinancial" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "charitygeolocation create" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "ein" $ein "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/v1/charitygeolocation" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "charitypremium create" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "ein" $ein "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/v1/charitypremium" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
...
export def "charitysearch get-summary" [
  ...
  let full_url = (build-url $base "/v1/charitysearch" $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "post"
    url: $full_url
    query: ({"ein": $ein, "searchTerm": $search_term, "city": $city, "state": $state, "zipCode": $zip_code, "category": $category, "eligible": $eligible, "start": $start, "rows": $rows} | compact)
    headers: $auth.headers
    body: null
    content_type: "application/json"
    timeout: ($max_time | default 30min)
