# Auto-generated client for OrgHunter v1.0.0
# Source: https://api.apis.guru/v2/specs/orghunter.com/1.0.0/swagger.json
# Auth: --token flag or $env.ORGHUNTER_TOKEN

const BASE_URL = "http://data.orghunter.com"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o ORGHUNTER_TOKEN | default "" }
...
def base-url-completer [] { ["http://data.orghunter.com"] }
...
export def "charitybasic create" [
  ...
  let full_url = (build-url $base "/v1/charitybasic" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"ein": $ein} | compact), body: null}
}
...
export def "charityfinancial create" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "ein" $ein "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/v1/charityfinancial" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"ein": $ein} | compact), body: null}
}
...
export def "charitygeolocation create" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "ein" $ein "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/v1/charitygeolocation" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"ein": $ein} | compact), body: null}
}
...
export def "charitypremium create" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "ein" $ein "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/v1/charitypremium" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"ein": $ein} | compact), body: null}
}
...
export def "charitysearch get-summary" [
  ...
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"ein": $ein, "searchTerm": $search_term, "city": $city, "state": $state, "zipCode": $zip_code, "category": $category, "eligible": $eligible, "start": $start, "rows": $rows} | compact), body: null}
}
