...
export def "v1beta1 delete" [
  ...
  let auth = (merge-auth [(build-auth ($token_oauth2 | default ($env | get -o EVENTARC_API_OAUTH2_TOKEN | default "")) "bearer") (build-auth ($token_oauth2c | default ($env | get -o EVENTARC_API_OAUTH2C_TOKEN | default "")) "bearer")])
...
export def "v1beta1 update" [
  ...
  let full_url = (build-url $base ({name: (encode-path-segment $name)} | format pattern "/v1beta1/{name}") $qp)
  let req_body = {"destination": $destination, "labels": $labels, "matchingCriteria": $matching_criteria, "name": $body_name, "serviceAccount": $service_account, "transport": $transport} | compact
...
export def "v1beta1-triggers list" [
  ...
  let full_url = (build-url $base ({parent: (encode-path-segment $parent)} | format pattern "/v1beta1/{parent}/triggers") $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "v1beta1-triggers create" [
  ...
  let full_url = (build-url $base ({parent: (encode-path-segment $parent)} | format pattern "/v1beta1/{parent}/triggers") $qp)
  let req_body = {"destination": $destination, "labels": $labels, "matchingCriteria": $matching_criteria, "name": $name, "serviceAccount": $service_account, "transport": $transport} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" $req_body {query: ({"$.xgafv": $xgafv, "access_token": $access_token, "alt": $alt, "callback": $callback, "fields": $fields, "key": $key, "oauth_token": $oauth_token, "prettyPrint": $pretty_print, "quotaUser": $quota_user, "upload_protocol": $upload_protocol, "uploadType": $upload_type, "triggerId": $trigger_id, "validateOnly": $validate_only} | compact), body: $req_body}
}
