...
export def "v1beta1 eventarcprojectslocationstriggersdelete" [
  ...
  --allowMissing: oneof<nothing, bool> # If set to true, and the trigger is not found, the request will succeed but no action will be taken on the server.
  --etag: string # If provided, the trigger will only be deleted if the etag matches the current etag on the resource.
  --validateOnly: oneof<nothing, bool> # Required. If set, validate the request and preview the review, but do not actually post it.
  ...
  let qp = [(serialize-qp "$.xgafv" $xgafv "scalar") (serialize-qp "access_token" $access_token "scalar") (serialize-qp "alt" $alt "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "fields" $fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $prettyPrint "scalar") (serialize-qp "quotaUser" $quotaUser "scalar") (serialize-qp "upload_protocol" $upload_protocol "scalar") (serialize-qp "uploadType" $uploadType "scalar") (serialize-qp "allowMissing" $allowMissing "scalar") (serialize-qp "etag" $etag "scalar") (serialize-qp "validateOnly" $validateOnly "scalar")] | flatten | str join "&"
...
export def "v1beta1 eventarcprojectslocationstriggerspatch" [
  ...
  --upload-protocol: string # Upload protocol for media (e.g. "raw", "multipart").
  --uploadType: string # Legacy upload protocol for media (e.g. "media", "multipart").
  --allowMissing: oneof<nothing, bool> # If set to true, and the trigger is not found, a new trigger will be created. In this situation, `update_mask` is ignored.
  ...
  --matchingCriteria: list # Required. Unordered list. The criteria by which events are filtered. Only events that match with this criteria will be sent to the destination. — item shape: {attribute?: string, value?: string}
  ...
  let full_url = (build-url $base $"/v1beta1/($name)" $qp)
  let body = {destination: $destination, labels: $labels, matchingCriteria: $matchingCriteria, name: $body_name, serviceAccount: $serviceAccount, transport: $transport} | compact
...
export def "v1beta1-triggers eventarcprojectslocationstriggerslist" [
  ...
  --pageSize: int # The maximum number of triggers to return on each page. Note: The service may send fewer.
  ...
  let full_url = (build-url $base $"/v1beta1/($parent)/triggers" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
...
export def "v1beta1-triggers eventarcprojectslocationstriggerscreate" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "$.xgafv" $xgafv "scalar") (serialize-qp "access_token" $access_token "scalar") (serialize-qp "alt" $alt "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "fields" $fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $prettyPrint "scalar") (serialize-qp "quotaUser" $quotaUser "scalar") (serialize-qp "upload_protocol" $upload_protocol "scalar") (serialize-qp "uploadType" $uploadType "scalar") (serialize-qp "triggerId" $triggerId "scalar") (serialize-qp "validateOnly" $validateOnly "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v1beta1/($parent)/triggers" $qp)
  let body = {destination: $destination, labels: $labels, matchingCriteria: $matchingCriteria, name: $name, serviceAccount: $serviceAccount, transport: $transport} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json" $body
