# Auto-generated client for Workflows API vv1
# Source: https://api.apis.guru/v2/specs/googleapis.com/workflows/v1/openapi.json
# Auth: --token flag or $env.WORKFLOWS_API_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o WORKFLOWS_API_TOKEN | default "" }
...
def base-url-completer [] { ["https://workflows.googleapis.com"] }
def auth-scheme-completer [] { ["bearer"] }

# Completers for enum parameters
def xgafv-completer [] { ["1" "2"] }
def alt-completer [] { ["json" "media" "proto"] }
def callLogLevel-completer [] { ["CALL_LOG_LEVEL_UNSPECIFIED" "LOG_ALL_CALLS" "LOG_ERRORS_ONLY" "LOG_NONE"] }

# List all available API commands with their parameters
export def commands []: nothing -> table {
  let builtin_flags = ["base-url" "token" "auth-scheme" "insecure" "max-time" "raw" "allow-errors" "accept" "help"]
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "projects workflowsprojectslocationsworkflowsdelete" } } | get name | first)
...
export def "projects workflowsprojectslocationsworkflowsdelete" [
  name: string
...
export def "projects workflowsprojectslocationsworkflowsget" [
  name: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
]: nothing -> record<callLogLevel: string, createTime: string, cryptoKeyName: string, description: string, labels: record, name: string, revisionCreateTime: string, revisionId: string, serviceAccount: string, sourceContents: string, state: string, stateError: record<details: string, type: string>, updateTime: string> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "$.xgafv" $xgafv "scalar") (serialize-qp "access_token" $access_token "scalar") (serialize-qp "alt" $alt "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "fields" $qp_fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $prettyPrint "scalar") (serialize-qp "quotaUser" $quotaUser "scalar") (serialize-qp "upload_protocol" $upload_protocol "scalar") (serialize-qp "uploadType" $uploadType "scalar") (serialize-qp "revisionId" $revisionId "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v1/($name)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
# --stateError shape: {details?: string, type?: "TYPE_UNSPECIFIED"|"KMS_ERROR"}
export def "projects workflowsprojectslocationsworkflowspatch" [
  name: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  --callLogLevel: string@callLogLevel-completer # Optional. Describes the level of platform logging to apply to calls and call responses during executions of this workflow. If both the workflow and the execution specify a logging level, the execution level takes precedence.
  ...
  --labels: record # Labels associated with this workflow. Labels can contain at most 64 entries. Keys and values can be no longer than 63 characters and can only contain lowercase letters, numeric characters, underscores, and dashes. Label keys must start with a letter. International characters are allowed.
  --body-name: string # The resource name of the workflow. Format: projects/{project}/locations/{location}/workflows/{workflow}
  ...
  --stateError: record # Describes an error related to the current state of the workflow. — shape: {details?: string, type?: "TYPE_UNSPECIFIED"|"KMS_ERROR"}
  ...
  let full_url = (build-url $base $"/v1/($name)" $qp)
  let body = {callLogLevel: $callLogLevel, cryptoKeyName: $cryptoKeyName, description: $description, labels: $labels, name: $body_name, serviceAccount: $serviceAccount, sourceContents: $sourceContents, stateError: $stateError} | compact
...
export def "locations workflowsprojectslocationslist" [
  name: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
export def "operations workflowsprojectslocationsoperationslist" [
  name: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
...
export def "workflows workflowsprojectslocationsworkflowslist" [
  parent: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  --pageSize: int # Maximum number of workflows to return per call. The service might return fewer than this value even if not at the end of the collection. If a value is not specified, a default value of 500 is used. The maximum permitted value is 1000 and values greater than 1000 are coerced down to 1000.
...
]: nothing -> record<nextPageToken: string, unreachable: list<string>, workflows: table<callLogLevel: string, createTime: string, cryptoKeyName: string, description: string, labels: record, name: string, revisionCreateTime: string, revisionId: string, serviceAccount: string, sourceContents: string, state: string, stateError: record, updateTime: string>> {
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  ...
  let full_url = (build-url $base $"/v1/($parent)/workflows" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $max_time $allow_errors "application/json"
}
...
# --stateError shape: {details?: string, type?: "TYPE_UNSPECIFIED"|"KMS_ERROR"}
export def "workflows workflowsprojectslocationsworkflowscreate" [
  parent: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "$.xgafv" $xgafv "scalar") (serialize-qp "access_token" $access_token "scalar") (serialize-qp "alt" $alt "scalar") (serialize-qp "callback" $callback "scalar") (serialize-qp "fields" $qp_fields "scalar") (serialize-qp "key" $key "scalar") (serialize-qp "oauth_token" $oauth_token "scalar") (serialize-qp "prettyPrint" $prettyPrint "scalar") (serialize-qp "quotaUser" $quotaUser "scalar") (serialize-qp "upload_protocol" $upload_protocol "scalar") (serialize-qp "uploadType" $uploadType "scalar") (serialize-qp "workflowId" $workflowId "scalar")] | flatten | str join "&"
  let full_url = (build-url $base $"/v1/($parent)/workflows" $qp)
  let body = {callLogLevel: $callLogLevel, cryptoKeyName: $cryptoKeyName, description: $description, labels: $labels, name: $name, serviceAccount: $serviceAccount, sourceContents: $sourceContents, stateError: $stateError} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
