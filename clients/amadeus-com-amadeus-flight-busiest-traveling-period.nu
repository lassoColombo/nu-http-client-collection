...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o FLIGHT_BUSIEST_TRAVELING_PERIOD_TOKEN | default "" }
...
export def "travel-analytics-air-traffic-busiest-period get" [
  ...
  let full_url = (build-url $base "/travel/analytics/air-traffic/busiest-period" $qp $auth.query)
  let accept_val = "application/vnd.amadeus+json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"cityCode": $city_code, "period": $period, "direction": $direction} | compact)
