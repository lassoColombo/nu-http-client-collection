# Auto-generated client for Flight Busiest Traveling Period v1.0.2
# Source: https://api.apis.guru/v2/specs/amadeus.com/amadeus-flight-busiest-traveling-period/1.0.2/swagger.json
# Auth: --token flag or $env.FLIGHT_BUSIEST_TRAVELING_PERIOD_TOKEN
...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o FLIGHT_BUSIEST_TRAVELING_PERIOD_TOKEN | default "" }
...
def direction-completer [] { ["ARRIVING" "DEPARTING"] }

# List all available API commands with their parameters
export def commands []: nothing -> table {
  let builtin_flags = ["base-url" "token" "auth-scheme" "insecure" "max-time" "raw" "allow-errors" "full" "dry-run" "accept" "help"]
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "travel-analytics-air-traffic-busiest-period get" } } | get name | first)
...
export def "travel-analytics-air-traffic-busiest-period get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"cityCode": $city_code, "period": $period, "direction": $direction} | compact), body: null}
}
