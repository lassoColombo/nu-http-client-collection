...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o NPR_SPONSORSHIP_SERVICE_TOKEN | default "" }
...
export def "ads get" [
  ...
  let full_url = (build-url $base "/v2/ads" $qp $auth.query)
  ...
  let req = {
    method: "get"
    url: $full_url
    query: ({"forceResult": $force_result, "adCount": $ad_count} | compact)
...
export def "ads receive-tracking" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/v2/ads" $auth.query)
