# Auto-generated client for NPR Sponsorship Service v2
# Source: https://api.apis.guru/v2/specs/npr.org/sponsorship/2/swagger.json
# Auth: --token flag or $env.NPR_SPONSORSHIP_SERVICE_TOKEN

const BASE_URL = "https://sponsorship.api.npr.org"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o NPR_SPONSORSHIP_SERVICE_TOKEN | default "" }
...
def base-url-completer [] { ["https://sponsorship.api.npr.org"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "ads get" } } | get name | first)
...
export def "ads get" [
  ...
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"forceResult": $force_result, "adCount": $ad_count} | compact), body: null}
}
...
export def "ads receive-tracking" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  items: list # All the information needed for the Sponsorship Service to send back tracking information to our external sponsorship provider — item shape: {attributes: any, errors: list, href: string, items: list, links: record, version: string}
  links: record
  version: string # The version of the Collection.Doc+JSON spec being used (default: 1.0)
]: any -> any {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/v2/ads")
  let req_body = {"attributes": $attributes, "errors": $errors, "href": $href, "items": $items, "links": $links, "version": $version} | compact
