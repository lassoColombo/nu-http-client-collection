...
export def "estimation-sendtoclient SendToClient" [
  ...
  --x-auth-key: string
  --x-auth-secret: string
  --AttachPdf: string@bool-completer # Should attach pdf file
  ...
  --SendToSelf: string@bool-completer # Should email copy be send to self
...
export def "invoice-new New" [
  ...
  --ShouldSendReminders: string@bool-completer # Should send email reminders to client?
...
export def "invoice-pdf Pdf" [
  ...
  --accept: string@accept-completer # Response content type
  --id: int # format: int32
  --signedVersion: string@bool-completer
  --x-auth-key: string
  --x-auth-secret: string
]: nothing -> record<Link: string> {
  let auth = (build-auth $token ($auth_scheme | default "x-auth-key"))
...
export def "product-new New" [
  ...
  --IsFeatured: string@bool-completer # Indicate that the product is set as featured
