# Auto-generated client for Twilio - Taskrouter v1.0.0
# Source: https://raw.githubusercontent.com/twilio/twilio-oai/main/spec/json/twilio_taskrouter_v1.json
...
]: nothing -> record<activities: table<account_sid: string, available: bool, date_created: string, date_updated: string, friendly_name: string, sid: string, workspace_sid: string, url: string, links: record>, meta: record<first_page_url: string, key: string, next_page_url: string, page: int, page_size: int, previous_page_url: string, url: string>> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
...
export def "workspaces-activities CreateActivity" [
  ...
  let body = {FriendlyName: $FriendlyName, Available: $Available} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/x-www-form-urlencoded" $body
...
export def "workspaces-tasks UpdateTask" [
  ...
  --VirtualStartTime: string # The task's new virtual start time value. When supplied, the Task takes on the specified virtual start time. Value can't be in the future or before the year of 1900. (format: date-time)
]: any -> record<account_sid: string, age: int, assignment_status: string, attributes: string, addons: string, date_created: string, date_updated: string, task_queue_entered_date: string, priority: int, reason: string, sid: string, task_queue_sid: string, task_queue_friendly_name: string, task_channel_sid: string, task_channel_unique_name: string, timeout: int, workflow_sid: string, workflow_friendly_name: string, workspace_sid: string, url: string, links: record, virtual_start_time: string, ignore_capacity: bool, routing_target: string> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
  let full_url = (build-url $base $"/v1/Workspaces/($WorkspaceSid)/Tasks/($Sid)")
  let body = {Attributes: $Attributes, AssignmentStatus: $AssignmentStatus, Reason: $Reason, Priority: $Priority, TaskChannel: $TaskChannel, VirtualStartTime: $VirtualStartTime} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
...
export def "workspaces-tasks ListTask" [
  ...
  --Page: int # The page index. This value is simply for client state.
  --PageToken: string # The page token. This is provided by the API.
]: nothing -> record<tasks: table<account_sid: string, age: int, assignment_status: string, attributes: string, addons: string, date_created: string, date_updated: string, task_queue_entered_date: string, priority: int, reason: string, sid: string, task_queue_sid: string, task_queue_friendly_name: string, task_channel_sid: string, task_channel_unique_name: string, timeout: int, workflow_sid: string, workflow_friendly_name: string, workspace_sid: string, url: string, links: record, virtual_start_time: string, ignore_capacity: bool, routing_target: string>, meta: record<first_page_url: string, key: string, next_page_url: string, page: int, page_size: int, previous_page_url: string, url: string>> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
  let qp = [(serialize-qp "Priority" $Priority "scalar") (serialize-qp "AssignmentStatus" $AssignmentStatus "multi") (serialize-qp "WorkflowSid" $WorkflowSid "scalar") (serialize-qp "WorkflowName" $WorkflowName "scalar") (serialize-qp "TaskQueueSid" $TaskQueueSid "scalar") (serialize-qp "TaskQueueName" $TaskQueueName "scalar") (serialize-qp "EvaluateTaskAttributes" $EvaluateTaskAttributes "scalar") (serialize-qp "RoutingTarget" $RoutingTarget "scalar") (serialize-qp "Ordering" $Ordering "scalar") (serialize-qp "HasAddons" $HasAddons "scalar") (serialize-qp "PageSize" $PageSize "scalar") (serialize-qp "Page" $Page "scalar") (serialize-qp "PageToken" $PageToken "scalar")] | flatten | str join "&"
...
export def "workspaces-tasks CreateTask" [
  ...
  --VirtualStartTime: string # The virtual start time to assign the new task and override the default. When supplied, the new task will have this virtual start time. When not supplied, the new task will have the virtual start time equal to `date_created`. Value can't be in the future or before the year of 1900. (format: date-time)
  --RoutingTarget: string # A SID of a Worker, Queue, or Workflow to route a Task to
  ...
  let body = {Timeout: $Timeout, Priority: $Priority, TaskChannel: $TaskChannel, WorkflowSid: $WorkflowSid, Attributes: $Attributes, VirtualStartTime: $VirtualStartTime, RoutingTarget: $RoutingTarget, IgnoreCapacity: $IgnoreCapacity, TaskQueueSid: $TaskQueueSid} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/x-www-form-urlencoded" $body
...
export def "workspaces-task-channels UpdateTaskChannel" [
  ...
  let base = ($base_url | default "https://taskrouter.twilio.com")
  let full_url = (build-url $base $"/v1/Workspaces/($WorkspaceSid)/TaskChannels/($Sid)")
  let body = {FriendlyName: $FriendlyName, ChannelOptimizedRouting: $ChannelOptimizedRouting} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/x-www-form-urlencoded" $body
...
export def "workspaces-task-channels ListTaskChannel" [
  ...
  --Page: int # The page index. This value is simply for client state.
  --PageToken: string # The page token. This is provided by the API.
]: nothing -> record<channels: table<account_sid: string, date_created: string, date_updated: string, friendly_name: string, sid: string, unique_name: string, workspace_sid: string, channel_optimized_routing: bool, url: string, links: record>, meta: record<first_page_url: string, key: string, next_page_url: string, page: int, page_size: int, previous_page_url: string, url: string>> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
...
export def "workspaces-task-channels CreateTaskChannel" [
  ...
  let body = {FriendlyName: $FriendlyName, UniqueName: $UniqueName, ChannelOptimizedRouting: $ChannelOptimizedRouting} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/x-www-form-urlencoded" $body
...
export def "workspaces-task-queues UpdateTaskQueue" [
  ...
  --TaskOrder: string@TaskOrder-completer # How Tasks will be assigned to Workers. Set this parameter to `LIFO` to assign most recently created Task first or `FIFO` to assign the oldest Task. Default is FIFO. [Click here](https://www.twilio.com/docs/taskrouter/queue-ordering-last-first-out-lifo) to learn more.
]: any -> record<account_sid: string, assignment_activity_sid: string, assignment_activity_name: string, date_created: string, date_updated: string, friendly_name: string, max_reserved_workers: int, reservation_activity_sid: string, reservation_activity_name: string, sid: string, target_workers: string, task_order: string, url: string, workspace_sid: string, links: record> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
  let full_url = (build-url $base $"/v1/Workspaces/($WorkspaceSid)/TaskQueues/($Sid)")
  let body = {FriendlyName: $FriendlyName, TargetWorkers: $TargetWorkers, ReservationActivitySid: $ReservationActivitySid, AssignmentActivitySid: $AssignmentActivitySid, MaxReservedWorkers: $MaxReservedWorkers, TaskOrder: $TaskOrder} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/x-www-form-urlencoded" $body
...
export def "workspaces-task-queues ListTaskQueue" [
  ...
  --Page: int # The page index. This value is simply for client state.
  --PageToken: string # The page token. This is provided by the API.
]: nothing -> record<task_queues: table<account_sid: string, assignment_activity_sid: string, assignment_activity_name: string, date_created: string, date_updated: string, friendly_name: string, max_reserved_workers: int, reservation_activity_sid: string, reservation_activity_name: string, sid: string, target_workers: string, task_order: string, url: string, workspace_sid: string, links: record>, meta: record<first_page_url: string, key: string, next_page_url: string, page: int, page_size: int, previous_page_url: string, url: string>> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
...
export def "workspaces-task-queues CreateTaskQueue" [
  ...
  let body = {FriendlyName: $FriendlyName, TargetWorkers: $TargetWorkers, MaxReservedWorkers: $MaxReservedWorkers, TaskOrder: $TaskOrder, ReservationActivitySid: $ReservationActivitySid, AssignmentActivitySid: $AssignmentActivitySid} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/x-www-form-urlencoded" $body
...
export def "workspaces-task-queues-real-time-statistics CreateTaskQueueBulkRealTimeStatistics" [
  WorkspaceSid: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
  let full_url = (build-url $base $"/v1/Workspaces/($WorkspaceSid)/TaskQueues/RealTimeStatistics")
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
]: nothing -> record<account_sid: string, avg_task_acceptance_time: int, start_time: string, end_time: string, reservations_created: int, reservations_accepted: int, reservations_rejected: int, reservations_timed_out: int, reservations_canceled: int, reservations_rescinded: int, split_by_wait_time: any, task_queue_sid: string, wait_duration_until_accepted: any, wait_duration_until_canceled: any, wait_duration_in_queue_until_accepted: any, tasks_canceled: int, tasks_completed: int, tasks_deleted: int, tasks_entered: int, tasks_moved: int, workspace_sid: string, url: string> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
...
]: nothing -> record<account_sid: string, cumulative: any, realtime: any, task_queue_sid: string, workspace_sid: string, url: string> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
  let qp = [(serialize-qp "EndDate" $EndDate "scalar") (serialize-qp "Minutes" $Minutes "scalar") (serialize-qp "StartDate" $StartDate "scalar") (serialize-qp "TaskChannel" $TaskChannel "scalar") (serialize-qp "SplitByWaitTime" $SplitByWaitTime "scalar")] | flatten | str join "&"
...
export def "workspaces-task-queues-statistics ListTaskQueuesStatistics" [
  ...
  --Page: int # The page index. This value is simply for client state.
  --PageToken: string # The page token. This is provided by the API.
]: nothing -> record<task_queues_statistics: table<account_sid: string, cumulative: any, realtime: any, task_queue_sid: string, workspace_sid: string>, meta: record<first_page_url: string, key: string, next_page_url: string, page: int, page_size: int, previous_page_url: string, url: string>> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
...
export def "workspaces-tasks-reservations ListTaskReservation" [
  ...
  --Page: int # The page index. This value is simply for client state.
  --PageToken: string # The page token. This is provided by the API.
]: nothing -> record<reservations: table<account_sid: string, date_created: string, date_updated: string, reservation_status: string, sid: string, task_sid: string, worker_name: string, worker_sid: string, workspace_sid: string, url: string, links: record>, meta: record<first_page_url: string, key: string, next_page_url: string, page: int, page_size: int, previous_page_url: string, url: string>> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
...
export def "workspaces-workers ListWorker" [
  ...
  --Page: int # The page index. This value is simply for client state.
  --PageToken: string # The page token. This is provided by the API.
]: nothing -> record<workers: table<account_sid: string, activity_name: string, activity_sid: string, attributes: string, available: bool, date_created: string, date_status_changed: string, date_updated: string, friendly_name: string, sid: string, workspace_sid: string, url: string, links: record>, meta: record<first_page_url: string, key: string, next_page_url: string, page: int, page_size: int, previous_page_url: string, url: string>> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
...
export def "workspaces-workers CreateWorker" [
  ...
  let body = {FriendlyName: $FriendlyName, ActivitySid: $ActivitySid, Attributes: $Attributes} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/x-www-form-urlencoded" $body
...
export def "workspaces-workers-channels ListWorkerChannel" [
  ...
  --Page: int # The page index. This value is simply for client state.
  --PageToken: string # The page token. This is provided by the API.
]: nothing -> record<channels: table<account_sid: string, assigned_tasks: int, available: bool, available_capacity_percentage: int, configured_capacity: int, date_created: string, date_updated: string, sid: string, task_channel_sid: string, task_channel_unique_name: string, worker_sid: string, workspace_sid: string, url: string>, meta: record<first_page_url: string, key: string, next_page_url: string, page: int, page_size: int, previous_page_url: string, url: string>> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
  let qp = [(serialize-qp "PageSize" $PageSize "scalar") (serialize-qp "Page" $Page "scalar") (serialize-qp "PageToken" $PageToken "scalar")] | flatten | str join "&"
...
export def "workspaces-workers-channels UpdateWorkerChannel" [
  ...
  let base = ($base_url | default "https://taskrouter.twilio.com")
  let full_url = (build-url $base $"/v1/Workspaces/($WorkspaceSid)/Workers/($WorkerSid)/Channels/($Sid)")
  let body = {Capacity: $Capacity, Available: $Available} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/x-www-form-urlencoded" $body
...
export def "workspaces-workers-statistics FetchWorkerInstanceStatistics" [
  ...
  --TaskChannel: string # Only calculate statistics on this TaskChannel. Can be the TaskChannel's SID or its `unique_name`, such as `voice`, `sms`, or `default`.
]: nothing -> record<account_sid: string, cumulative: any, worker_sid: string, workspace_sid: string, url: string> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
...
export def "workspaces-workers-statistics FetchWorkerStatistics" [
  ...
  --TaskChannel: string # Only calculate statistics on this TaskChannel. Can be the TaskChannel's SID or its `unique_name`, such as `voice`, `sms`, or `default`.
]: nothing -> record<realtime: any, cumulative: any, account_sid: string, workspace_sid: string, url: string> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
...
export def "workspaces-workers-cumulative-statistics FetchWorkersCumulativeStatistics" [
  ...
  --StartDate: string # Only calculate statistics from this date and time and later, specified in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format. (format: date-time)
  --TaskChannel: string # Only calculate cumulative statistics on this TaskChannel. Can be the TaskChannel's SID or its `unique_name`, such as `voice`, `sms`, or `default`.
]: nothing -> record<account_sid: string, start_time: string, end_time: string, activity_durations: list<any>, reservations_created: int, reservations_accepted: int, reservations_rejected: int, reservations_timed_out: int, reservations_canceled: int, reservations_rescinded: int, workspace_sid: string, url: string> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
...
]: any -> record<account_sid: string, assignment_callback_url: string, configuration: string, date_created: string, date_updated: string, document_content_type: string, fallback_assignment_callback_url: string, friendly_name: string, sid: string, task_reservation_timeout: int, workspace_sid: string, url: string, links: record> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
  let full_url = (build-url $base $"/v1/Workspaces/($WorkspaceSid)/Workflows/($Sid)")
  let body = {FriendlyName: $FriendlyName, AssignmentCallbackUrl: $AssignmentCallbackUrl, FallbackAssignmentCallbackUrl: $FallbackAssignmentCallbackUrl, Configuration: $Configuration, TaskReservationTimeout: $TaskReservationTimeout, ReEvaluateTasks: $ReEvaluateTasks} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/x-www-form-urlencoded" $body
...
export def "workspaces-workflows ListWorkflow" [
  ...
  --Page: int # The page index. This value is simply for client state.
  --PageToken: string # The page token. This is provided by the API.
]: nothing -> record<workflows: table<account_sid: string, assignment_callback_url: string, configuration: string, date_created: string, date_updated: string, document_content_type: string, fallback_assignment_callback_url: string, friendly_name: string, sid: string, task_reservation_timeout: int, workspace_sid: string, url: string, links: record>, meta: record<first_page_url: string, key: string, next_page_url: string, page: int, page_size: int, previous_page_url: string, url: string>> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
...
export def "workspaces-workflows CreateWorkflow" [
  ...
  let body = {FriendlyName: $FriendlyName, Configuration: $Configuration, AssignmentCallbackUrl: $AssignmentCallbackUrl, FallbackAssignmentCallbackUrl: $FallbackAssignmentCallbackUrl, TaskReservationTimeout: $TaskReservationTimeout} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/x-www-form-urlencoded" $body
...
]: nothing -> record<account_sid: string, avg_task_acceptance_time: int, start_time: string, end_time: string, reservations_created: int, reservations_accepted: int, reservations_rejected: int, reservations_timed_out: int, reservations_canceled: int, reservations_rescinded: int, split_by_wait_time: any, wait_duration_until_accepted: any, wait_duration_until_canceled: any, tasks_canceled: int, tasks_completed: int, tasks_entered: int, tasks_deleted: int, tasks_moved: int, tasks_timed_out_in_workflow: int, workflow_sid: string, workspace_sid: string, url: string> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
  let qp = [(serialize-qp "EndDate" $EndDate "scalar") (serialize-qp "Minutes" $Minutes "scalar") (serialize-qp "StartDate" $StartDate "scalar") (serialize-qp "TaskChannel" $TaskChannel "scalar") (serialize-qp "SplitByWaitTime" $SplitByWaitTime "scalar")] | flatten | str join "&"
...
]: nothing -> record<account_sid: string, cumulative: any, realtime: any, workflow_sid: string, workspace_sid: string, url: string> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
...
export def "workspaces UpdateWorkspace" [
  ...
  let base = ($base_url | default "https://taskrouter.twilio.com")
  let full_url = (build-url $base $"/v1/Workspaces/($Sid)")
  let body = {DefaultActivitySid: $DefaultActivitySid, EventCallbackUrl: $EventCallbackUrl, EventsFilter: $EventsFilter, FriendlyName: $FriendlyName, MultiTaskEnabled: $MultiTaskEnabled, TimeoutActivitySid: $TimeoutActivitySid, PrioritizeQueueOrder: $PrioritizeQueueOrder} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/x-www-form-urlencoded" $body
...
export def "workspaces ListWorkspace" [
  ...
  --Page: int # The page index. This value is simply for client state.
  --PageToken: string # The page token. This is provided by the API.
]: nothing -> record<workspaces: table<account_sid: string, date_created: string, date_updated: string, default_activity_name: string, default_activity_sid: string, event_callback_url: string, events_filter: string, friendly_name: string, multi_task_enabled: bool, sid: string, timeout_activity_name: string, timeout_activity_sid: string, prioritize_queue_order: string, url: string, links: record>, meta: record<first_page_url: string, key: string, next_page_url: string, page: int, page_size: int, previous_page_url: string, url: string>> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
  let qp = [(serialize-qp "FriendlyName" $FriendlyName "scalar") (serialize-qp "PageSize" $PageSize "scalar") (serialize-qp "Page" $Page "scalar") (serialize-qp "PageToken" $PageToken "scalar")] | flatten | str join "&"
...
export def "workspaces CreateWorkspace" [
  ...
  let body = {FriendlyName: $FriendlyName, EventCallbackUrl: $EventCallbackUrl, EventsFilter: $EventsFilter, MultiTaskEnabled: $MultiTaskEnabled, Template: $Template, PrioritizeQueueOrder: $PrioritizeQueueOrder} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "post" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/x-www-form-urlencoded" $body
...
export def "workspaces-cumulative-statistics FetchWorkspaceCumulativeStatistics" [
  ...
  --TaskChannel: string # Only calculate cumulative statistics on this TaskChannel. Can be the TaskChannel's SID or its `unique_name`, such as `voice`, `sms`, or `default`.
  --SplitByWaitTime: string # A comma separated list of values that describes the thresholds, in seconds, to calculate statistics on. For each threshold specified, the number of Tasks canceled and reservations accepted above and below the specified thresholds in seconds are computed. For example, `5,30` would show splits of Tasks that were canceled or accepted before and after 5 seconds and before and after 30 seconds. This can be used to show short abandoned Tasks or Tasks that failed to meet an SLA. TaskRouter will calculate statistics on up to 10,000 Tasks for any given threshold.
]: nothing -> record<account_sid: string, avg_task_acceptance_time: int, start_time: string, end_time: string, reservations_created: int, reservations_accepted: int, reservations_rejected: int, reservations_timed_out: int, reservations_canceled: int, reservations_rescinded: int, split_by_wait_time: any, wait_duration_until_accepted: any, wait_duration_until_canceled: any, tasks_canceled: int, tasks_completed: int, tasks_created: int, tasks_deleted: int, tasks_moved: int, tasks_timed_out_in_workflow: int, workspace_sid: string, url: string> {
  let auth = (build-auth $token ($auth_scheme | default "basic"))
  let base = ($base_url | default "https://taskrouter.twilio.com")
  let qp = [(serialize-qp "EndDate" $EndDate "scalar") (serialize-qp "Minutes" $Minutes "scalar") (serialize-qp "StartDate" $StartDate "scalar") (serialize-qp "TaskChannel" $TaskChannel "scalar") (serialize-qp "SplitByWaitTime" $SplitByWaitTime "scalar")] | flatten | str join "&"
