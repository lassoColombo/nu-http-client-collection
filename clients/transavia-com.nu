...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o AIRPORTS_API_V2_TOKEN | default "" }
...
export def "countrycode get-58d8bcb8a9e6240e200cff26" [
  ...
  let full_url = (build-url $base ({country_code: (encode-path-segment $country_code)} | format pattern "/countrycode/{country_code}") $auth.query)
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "nearest get-58d8bcb8a9e6240e200cff27" [
  ...
  let full_url = (build-url $base "/nearest" $qp $auth.query)
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"latitude": $latitude, "longitude": $longitude, "maxDistanceInKm": $max_distance_in_km, "limit": $limit} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "nearest get-58d8bcb8a9e6240e200cff28" [
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/nearest/{id}") $qp $auth.query)
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"maxDistanceInKm": $max_distance_in_km, "limit": $limit} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
