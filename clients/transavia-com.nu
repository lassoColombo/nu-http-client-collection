# Auto-generated client for Airports API v2 v1.0
# Source: https://api.apis.guru/v2/specs/transavia.com/1.0/swagger.json
# Auth: --token flag or $env.AIRPORTS_API_V2_TOKEN

const BASE_URL = "https://api.transavia.com/v2/airports"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o AIRPORTS_API_V2_TOKEN | default "" }
...
def base-url-completer [] { ["https://api.transavia.com/v2/airports"] }
def auth-scheme-completer [] { ["apikey" "query-subscription-key"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "api get-58d8bcb7a9e6240e200cff24" } } | get name | first)
...
export def "api get-58d8bcb7a9e6240e200cff24" [
...
export def "countrycode get-58d8bcb8a9e6240e200cff26" [
  country_code: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let full_url = (build-url $base ({country_code: (encode-path-segment $country_code)} | format pattern "/countrycode/{country_code}"))
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
...
export def "nearest get-58d8bcb8a9e6240e200cff27" [
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  --insecure(-k) # Skip TLS verification
  ...
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"latitude": $latitude, "longitude": $longitude, "maxDistanceInKm": $max_distance_in_km, "limit": $limit} | compact), body: null}
}
...
export def "nearest get-58d8bcb8a9e6240e200cff28" [
  id: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
  ...
  let full_url = (build-url $base ({id: (encode-path-segment $id)} | format pattern "/nearest/{id}") $qp)
  let accept_val = ($accept | default "text/plain")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"maxDistanceInKm": $max_distance_in_km, "limit": $limit} | compact), body: null}
}
...
export def "api get-58d8bcb7a9e6240e200cff25" [
  id: string
  --base-url(-b): string@base-url-completer # API base URL
  --token(-t): string # Auth token
  --auth-scheme(-a): string@auth-scheme-completer # Auth scheme
