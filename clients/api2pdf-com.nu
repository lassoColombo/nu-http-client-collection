...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o API2PDF_PDF_GENERATION_POWERED_BY_AWS_LAMBDA_TOKEN | default "" }
...
export def "chrome-html create-from" [
  ...
  let full_url = (build-url $base "/chrome/html" $auth.query)
...
export def "chrome-url get-from" [
  ...
  let full_url = (build-url $base "/chrome/url" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"url": $url, "output": $output} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
...
export def "chrome-url create-from" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/chrome/url" $auth.query)
...
export def "libreoffice-convert create-libre" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/libreoffice/convert" $auth.query)
...
export def "merge create" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/merge" $auth.query)
...
export def "wkhtmltopdf-html create-from" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/wkhtmltopdf/html" $auth.query)
  let req_body = {"fileName": $file_name, "html": $html, "inlinePdf": $inline_pdf, "options": $options} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "wkhtmltopdf-url get-from" [
  ...
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "url" $url "scalar") (serialize-qp "output" $output "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/wkhtmltopdf/url" $qp $auth.query)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "wkhtmltopdf-url create-from" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/wkhtmltopdf/url" $auth.query)
  let req_body = {"fileName": $file_name, "inlinePdf": $inline_pdf, "options": $options, "url": $url} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "zebra get" [
  ...
  let full_url = (build-url $base "/zebra" $qp $auth.query)
  let accept_val = "image/png"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"format": $format, "value": $value, "showlabel": $showlabel, "height": $height, "width": $width} | compact)
    headers: $auth.headers
    body: null
    content_type: null
    timeout: ($max_time | default 30min)
