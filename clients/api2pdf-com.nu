...
def base-url-completer [] { ["https://v2018.api2pdf.com"] }
def auth-scheme-completer [] { ["bearer" "query-apikey"] }
...
export def "chrome-url get-from" [
  ...
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"url": $url, "output": $output} | compact), body: null}
}
...
export def "zebra get" [
  ...
  let full_url = (build-url $base "/zebra" $qp)
  let accept_val = "image/png"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"format": $format, "value": $value, "showlabel": $showlabel, "height": $height, "width": $width} | compact), body: null}
}
