...
]: any -> record<url: string, request: string, verifier: string, deviceCode: string, expiresAt: float, connector: record<id: string, uid: string, type: string, service: string, serviceName: string, name: string>> {
  let input = $in
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
