...
export def "connectivity-problems post-connectivity-problems" [
  ...
  --runFromSiteShield: string@bool-completer # Runs Connectivity problems from a Site Shield map. To learn more, check [Site Shield requests](https://techdocs.akamai.com/edge-diagnostics/reference/site-shield-requests). (e.g. {{runFromSiteShield}})
...
export def "connectivity-problems-requests get-connectivity-problems-request" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
  --includeContentResponseBody: string@bool-completer # Includes response bodies in the response. (e.g. false)
  --accountSwitchKey: string # For customers who manage more than one account, this [runs the operation from another account](https://techdocs.akamai.com/developer/docs/manage-many-accounts-with-one-api-client). The Identity and Access Management API provides a [list of available account switch keys](https://techdocs.akamai.com/iam-api/reference/get-client-account-switch-keys). (e.g. 1-5C0YLB:1-8BYUX)
...
export def "content-problems post-content-problems" [
  ...
  --requestHeaders: list # Customized headers for the `curl` request in the format `header: value`. The request includes [Akamai Pragma headers](https://techdocs.akamai.com/edge-diagnostics/docs/pragma-headers) by default.
  --runFromSiteShield: string@bool-completer # Runs Content problems from a Site Shield map. To learn more, check [Site Shield requests](https://techdocs.akamai.com/edge-diagnostics/reference/site-shield-requests). (e.g. {{runFromSiteShield}})
  --sensitiveRequestHeaderKeys: list # Sensitive `requestHeaders` you don't want to store in the database after running the request. Check [Sensitive headers](https://techdocs.akamai.com/edge-diagnostics/reference/sensitive-headers) to see the list of request headers treated as sensitive by default.
  --spoofEdgeIp: string # IP of the edge server you want to serve traffic from. You can use the `ip` value from the `edgeIps` array in the [collected diagnostic data](https://techdocs.akamai.com/edge-diagnostics/reference/get-user-diagnostic-data-group-records). (e.g. {{spoofEdgeIp}})
...
export def "curl post-curl" [
  ...
  --runFromSiteShield: string@bool-completer # Runs `curl` from a Site Shield map. To learn more, check [Site Shield requests](https://techdocs.akamai.com/edge-diagnostics/reference/site-shield-requests). (e.g. {{runFromSiteShield}})
  --sensitiveRequestHeaderKeys: list # Sensitive `requestHeaders` you don't want to store in the database after running the request. Check [Sensitive headers](https://techdocs.akamai.com/edge-diagnostics/reference/sensitive-headers) to see the list of request headers treated as sensitive by default.
...
export def "dig post-dig" [
  ...
  --isGtmHostname: string@bool-completer # Specifies `hostname` is a GTM hostname. (e.g. {{isGtmHostname}})
...
export def "error-translator post-error-translator" [
  ...
  --traceForwardLogs: string@bool-completer # Gets logs from all edge servers involved in serving the request. When `false`, you get logs only from the edge server where the error occurred. Tracing forward logs may prolong the time of fetching data. The default value is `false`. (e.g. {{traceForwardLogs}})
...
export def "metadata-tracer post-mdt" [
  ...
  --useStaging: string@bool-completer # Runs the request on the staging environment, `false` by default.  (e.g. {{useStaging}})
...
export def "mtr post-mtr" [
  ...
  --resolveDns: string@bool-completer # Resolves DNS for each hop. (e.g. {{resolveDns}})
  --showIps: string@bool-completer # Shows IPs for each hop. (e.g. {{showIps}})
  --showLocations: string@bool-completer # Shows locations for each hop. (e.g. {{showLocations}})
...
export def "url-health-check post-url-health-check" [
  ...
  --requestHeaders: list # Customized headers for the `curl` request in the format `header: value`. The request includes [Akamai Pragma headers](https://techdocs.akamai.com/edge-diagnostics/docs/pragma-headers) by default.
  --runFromSiteShield: string@bool-completer # Runs a URL health check from a Site Shield map. To learn more, check [Site Shield requests](https://techdocs.akamai.com/edge-diagnostics/reference/site-shield-requests). (e.g. {{runFromSiteShield}})
  --sensitiveRequestHeaderKeys: list # Sensitive `requestHeaders` you don't want to store in the database after running the request. Check [Sensitive headers](https://techdocs.akamai.com/edge-diagnostics/reference/sensitive-headers) to see the list of request headers treated as sensitive by default.
...
export def "user-diagnostic-data-groups-records get-user-diagnostic-data-group-records" [
  ...
  --raw(-r) # Fetch as text
  --allow-errors(-e) # Return full response without error handling
  --includeCurl: string@bool-completer # Includes `curl` results in the response. (e.g. true)
  --includeDig: string@bool-completer # Includes `dig` results in the response. (e.g. true)
  --includeMtr: string@bool-completer # Includes MTR results in the response. (e.g. true)
  --accountSwitchKey: string # For customers who manage more than one account, this [runs the operation from another account](https://techdocs.akamai.com/developer/docs/manage-many-accounts-with-one-api-client). The Identity and Access Management API provides a [list of available account switch keys](https://techdocs.akamai.com/iam-api/reference/get-client-account-switch-keys). (e.g. 1-5C0YLB:1-8BYUX)
