...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o TWILIO_LOOKUPS_TOKEN | default "" }
...
export def "phone-numbers get" [
  ...
  let full_url = (build-url $base ({phone_number: (encode-path-segment $phone_number)} | format pattern "/v1/PhoneNumbers/{phone_number}") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"CountryCode": $country_code, "Type": $type, "AddOns": $add_ons, "AddOnsData": $add_ons_data} | compact)
