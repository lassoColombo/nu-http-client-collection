...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o TWILIO_BULKEXPORTS_TOKEN | default "" }
...
export def "exports-jobs delete" [
  ...
  if ($job_sid | is-empty) { error make --unspanned { msg: "path parameter 'JobSid' must be non-empty" } }
  let full_url = (build-url $base ({job_sid: (encode-path-segment $job_sid)} | format pattern "/v1/Exports/Jobs/{job_sid}") $auth.query)
...
export def "exports get" [
  ...
  let full_url = (build-url $base ({resource_type: (encode-path-segment $resource_type)} | format pattern "/v1/Exports/{resource_type}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "exports-configuration get" [
  ...
  let base = ($base_url | default "https://bulkexports.twilio.com")
  if ($resource_type | is-empty) { error make --unspanned { msg: "path parameter 'ResourceType' must be non-empty" } }
  let full_url = (build-url $base ({resource_type: (encode-path-segment $resource_type)} | format pattern "/v1/Exports/{resource_type}/Configuration") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "exports-configuration update" [
  ...
  if ($resource_type | is-empty) { error make --unspanned { msg: "path parameter 'ResourceType' must be non-empty" } }
  let full_url = (build-url $base ({resource_type: (encode-path-segment $resource_type)} | format pattern "/v1/Exports/{resource_type}/Configuration") $auth.query)
  let req_body = {"Enabled": $enabled, "WebhookMethod": $webhook_method, "WebhookUrl": $webhook_url} | compact
...
export def "exports-days list" [
  ...
  let full_url = (build-url $base ({resource_type: (encode-path-segment $resource_type)} | format pattern "/v1/Exports/{resource_type}/Days") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "exports-days get" [
  ...
  let full_url = (build-url $base ({resource_type: (encode-path-segment $resource_type), day: (encode-path-segment $day)} | format pattern "/v1/Exports/{resource_type}/Days/{day}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "exports-jobs list-custom" [
  ...
  if ($resource_type | is-empty) { error make --unspanned { msg: "path parameter 'ResourceType' must be non-empty" } }
  let qp = [(serialize-qp "PageSize" $page_size "scalar") (serialize-qp "Page" $page "scalar") (serialize-qp "PageToken" $page_token "scalar")] | flatten | str join "&"
  let full_url = (build-url $base ({resource_type: (encode-path-segment $resource_type)} | format pattern "/v1/Exports/{resource_type}/Jobs") $qp $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "exports-jobs create-custom" [
  ...
  let base = ($base_url | default "https://bulkexports.twilio.com")
  if ($resource_type | is-empty) { error make --unspanned { msg: "path parameter 'ResourceType' must be non-empty" } }
  let full_url = (build-url $base ({resource_type: (encode-path-segment $resource_type)} | format pattern "/v1/Exports/{resource_type}/Jobs") $auth.query)
  let req_body = {"Email": $email, "EndDay": $end_day, "FriendlyName": $friendly_name, "StartDay": $start_day, "WebhookMethod": $webhook_method, "WebhookUrl": $webhook_url} | compact
  let req_body = if ($input | describe | str starts-with "record") { $input | merge deep ($req_body | default {}) } else { $req_body }
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req_body_wire = (if (($req_body | describe) | str starts-with "record") { $req_body | transpose k v | where v != null | reduce -f {} {|p, acc| $acc | upsert $p.k $p.v } | url build-query } else if ($req_body == null) { "" } else { $req_body | into string })
