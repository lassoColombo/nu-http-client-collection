...
export def "qrcode-decode post" [
  ...
  let body = {qrimage: $qrimage} | compact
  let body = if ($input | describe | str starts-with "record") { $input | merge deep ($body | default {}) } else { $body }
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let body = if ($qrimage | is-not-empty) { $body | upsert qrimage (open -r $qrimage) } else { $body }
...
export def "qrcode-email get" [
  ...
  --qp-body: string # Body of the email(optional) (format: string)
  --format: string # Output image format. Must be one of png/png/eps/raw/svg (format: string)
]: nothing -> any {
  let auth = (build-auth $token ($auth_scheme | default "x-fungenerators-api-secret"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "email" $email "scalar") (serialize-qp "subject" $subject "scalar") (serialize-qp "body" $qp_body "scalar") (serialize-qp "format" $format "scalar")] | flatten | str join "&"
...
export def "qrcode-url get" [
  ...
  --dry-run(-n) # Return the request that would be sent without executing it
  --accept: string@accept-completer # Response content type
  --qp-url: string # URL value (format: string)
  --format: string # Output image format. Must be one of png/raw/eps/svg (format: string)
]: nothing -> any {
  let auth = (build-auth $token ($auth_scheme | default "x-fungenerators-api-secret"))
  let base = ($base_url | default $BASE_URL)
  let qp = [(serialize-qp "url" $qp_url "scalar") (serialize-qp "format" $format "scalar")] | flatten | str join "&"
