...
export def "subscriptions-resource-groups-providers-microsoft-fabric-admin-fabric-locations-edge-gateways List" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Fabric.Admin/fabricLocations/($location)/edgeGateways" $qp)
...
export def "subscriptions-resource-groups-providers-microsoft-fabric-admin-fabric-locations-edge-gateways Get" [
  ...
  let full_url = (build-url $base $"/subscriptions/($subscriptionId)/resourceGroups/($resourceGroupName)/providers/Microsoft.Fabric.Admin/fabricLocations/($location)/edgeGateways/($edgeGateway)" $qp)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors "application/json"
}
