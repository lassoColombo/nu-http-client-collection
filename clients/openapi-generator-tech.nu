# Auto-generated client for OpenAPI Generator Online v6.2.1
# Source: https://api.apis.guru/v2/specs/openapi-generator.tech/6.2.1/openapi.json
# Auth: --token flag or $env.OPENAPI_GENERATOR_ONLINE_TOKEN

const BASE_URL = "http://localhost//api.openapi-generator.tech"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o OPENAPI_GENERATOR_ONLINE_TOKEN | default "" }
...
def base-url-completer [] { ["http://localhost//api.openapi-generator.tech"] }
...
export def "gen-clients list" [
  ...
  let full_url = (build-url $base "/api/gen/clients" $auth.query)
  let accept_val = "*/*"
...
export def "gen-clients get-options" [
  ...
  let full_url = (build-url $base ({language: (encode-path-segment $language)} | format pattern "/api/gen/clients/{language}") $auth.query)
...
export def "gen-clients generate" [
  ...
  if ($language | is-empty) { error make --unspanned { msg: "path parameter 'language' must be non-empty" } }
  let full_url = (build-url $base ({language: (encode-path-segment $language)} | format pattern "/api/gen/clients/{language}") $auth.query)
  let req_body = {"authorizationValue": $authorization_value, "openAPIUrl": $open_api_url, "options": $options, "spec": $spec} | compact
...
export def "gen-download download-file" [
  ...
  let full_url = (build-url $base ({file_id: (encode-path-segment $file_id)} | format pattern "/api/gen/download/{file_id}") $auth.query)
...
export def "gen-servers list" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/api/gen/servers" $auth.query)
  let accept_val = "*/*"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "gen-servers get-options" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($framework | is-empty) { error make --unspanned { msg: "path parameter 'framework' must be non-empty" } }
  let full_url = (build-url $base ({framework: (encode-path-segment $framework)} | format pattern "/api/gen/servers/{framework}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
