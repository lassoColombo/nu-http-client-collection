...
export def "airport-autocomplete complete-auto-list" [
  ...
  let full_url = (build-url $base ({airport_name: (encode-path-segment $airport_name)} | format pattern "/v1/airport/autocomplete/{airport_name}") $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"airport_service_type": $airport_service_type, "optional_country_code": $optional_country_code} | compact), body: null}
}
...
export def "airport-iata list" [
  ...
  let full_url = (build-url $base ({airport_iata: (encode-path-segment $airport_iata)} | format pattern "/v1/airport/iata/{airport_iata}"))
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
...
export def "airport-name list-details" [
  ...
  let base = ($base_url | default $BASE_URL)
  if ($airport_name | is-empty) { error make --unspanned { msg: "path parameter 'airport_name' must be non-empty" } }
  let full_url = (build-url $base ({airport_name: (encode-path-segment $airport_name)} | format pattern "/v1/airport/name/{airport_name}"))
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
...
export def "airport-nearest list" [
  ...
  let full_url = (build-url $base ({result_count: (encode-path-segment $result_count), latitude: (encode-path-segment $latitude), longitude: (encode-path-segment $longitude)} | format pattern "/v1/airport/nearest/{result_count}/{latitude}/{longitude}"))
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: {}, body: null}
}
...
export def "country-code list-airport-airport" [
  ...
  let full_url = (build-url $base ({country_code: (encode-path-segment $country_code)} | format pattern "/v1/country/code/{country_code}") $qp)
  let accept_val = ($accept | default "application/json")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"airport_service_type": $airport_service_type} | compact), body: null}
}
