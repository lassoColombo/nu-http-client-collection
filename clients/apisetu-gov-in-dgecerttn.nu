...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o TAMIL_NADU_STATE_BOARD_TAMIL_NADU_DIRECTORATE_OF_GOVERNMENT_EXAMINATIONS_TAMIL_NADU_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/dgecerttn/v3"] }
...
# operationId: hscer
# --certificateParameters shape: {CertificateType: string, DOB: string, FullName: string, RROLL: string, month: string, year: string}
...
export def "hscer-certificate create" [
  ...
  --certificate-parameters: record # shape: {CertificateType: string, DOB: string, FullName: string, RROLL: string, month: string, year: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o TAMIL_NADU_STATE_BOARD_TAMIL_NADU_DIRECTORATE_OF_GOVERNMENT_EXAMINATIONS_TAMIL_NADU_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o TAMIL_NADU_STATE_BOARD_TAMIL_NADU_DIRECTORATE_OF_GOVERNMENT_EXAMINATIONS_TAMIL_NADU_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
