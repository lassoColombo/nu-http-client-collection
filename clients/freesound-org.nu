...
export def "search-text list" [
  ...
  let qp = [(serialize-qp "query" $query "scalar") (serialize-qp "filter" $filter "scalar") (serialize-qp "sort" $qp_sort "scalar") (serialize-qp "group_by_pack" $group_by_pack "scalar") (serialize-qp "page" $page "scalar") (serialize-qp "page_size" $page_size "scalar")] | flatten | str join "&"
  let full_url = (build-url $base "/search/text" $qp)
  let accept_val = ($accept | default "application/xml")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  do-request "get" $full_url $auth $insecure $raw $dry_run $max_time $allow_errors $full "application/json" null {query: ({"query": $query, "filter": $filter, "sort": $qp_sort, "group_by_pack": $group_by_pack, "page": $page, "page_size": $page_size} | compact), body: null}
}
...
export def "sounds get" [
  ...
  let auth = (build-auth $token ($auth_scheme | default "bearer"))
  let base = ($base_url | default $BASE_URL)
  if ($sound_id | is-empty) { error make --unspanned { msg: "path parameter 'soundId' must be non-empty" } }
