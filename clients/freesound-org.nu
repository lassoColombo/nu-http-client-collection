...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o FREESOUND_TOKEN | default "" }
...
export def "search-text list" [
  ...
  let full_url = (build-url $base "/search/text" $qp $auth.query)
  let accept_val = ($accept | default "application/xml")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
    url: $full_url
    query: ({"query": $query, "filter": $filter, "sort": $qp_sort, "group_by_pack": $group_by_pack, "page": $page, "page_size": $page_size} | compact)
...
export def "sounds get" [
  ...
  let full_url = (build-url $base ({sound_id: (encode-path-segment $sound_id)} | format pattern "/sounds/{sound_id}") $auth.query)
  let accept_val = ($accept | default "application/xml")
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
