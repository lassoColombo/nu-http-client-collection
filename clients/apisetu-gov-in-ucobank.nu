# Auto-generated client for UCO Bank v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/ucobank/3.0.0/openapi.json
# Auth: --token flag or $env.UCO_BANK_TOKEN

const BASE_URL = "https://apisetu.gov.in/ucobank/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token != null) and ($token | is-not-empty) { $token } else { $env | get -o UCO_BANK_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/ucobank/v3"] }
...
export def commands []: nothing -> table {
  ...
  let mod_name = (scope modules | where { $in.commands | any { $in.name == "tdcer-certificate create" } } | get name | first)
...
# operationId: tdcer
# --certificateParameters shape: {CustID: string, DOB: string, finYr: string}
# --consentArtifact shape: {consent: record, signature: record}
export def "tdcer-certificate create" [
  ...
  --certificate-parameters: record # shape: {CustID: string, DOB: string, finYr: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o UCO_BANK_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o UCO_BANK_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
  let base = ($base_url | default $BASE_URL)
  let full_url = (build-url $base "/tdcer/certificate")
