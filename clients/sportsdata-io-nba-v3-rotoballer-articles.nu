...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o NBA_V3_ROTOBALLER_ARTICLES_TOKEN | default "" }
...
export def "roto-baller-articles get-rotoballer" [
  ...
  let full_url = (build-url $base ({format: (encode-path-segment $format)} | format pattern "/{format}/RotoBallerArticles") $auth.query)
...
export def "roto-baller-articles-by-date get-rotoballer" [
  ...
  let full_url = (build-url $base ({format: (encode-path-segment $format), date: (encode-path-segment $date)} | format pattern "/{format}/RotoBallerArticlesByDate/{date}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
...
export def "roto-baller-articles-by-player-id get-rotoballer" [
  ...
  let full_url = (build-url $base ({format: (encode-path-segment $format), playerid: (encode-path-segment $playerid)} | format pattern "/{format}/RotoBallerArticlesByPlayerID/{playerid}") $auth.query)
  let accept_val = "application/json"
  let auth = ($auth | update headers ($auth.headers | merge {Accept: $accept_val}))
  let req = {
    method: "get"
