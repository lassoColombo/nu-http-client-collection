...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o LOCATION_SCORE_TOKEN | default "" }
...
export def "location-analytics-category-rated-areas get" [
  ...
  let full_url = (build-url $base "/location/analytics/category-rated-areas" $qp $auth.query)
