# Auto-generated client for Greater Chennai Corporation, Tamil Nadu v3.0.0
# Source: https://api.apis.guru/v2/specs/apisetu.gov.in/chennaicorp/3.0.0/openapi.json
# Auth: --token flag or $env.GREATER_CHENNAI_CORPORATION_TAMIL_NADU_TOKEN

const BASE_URL = "https://apisetu.gov.in/chennaicorp/v3"

# Build auth: returns {scheme: string, headers: record, query: string, location: string}.
# `location` is "header" | "query" | "cookie" | "none" and tells dry-run callers
# where the token went without inspecting headers/query themselves.
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o GREATER_CHENNAI_CORPORATION_TAMIL_NADU_TOKEN | default "" }
...
def base-url-completer [] { ["https://apisetu.gov.in/chennaicorp/v3"] }
...
# operationId: btcer
# --certificateParameters shape: {DATEOFBIRTH: string, REGNO: string}
...
export def "btcer-certificate create" [
  ...
  --certificate-parameters: record # shape: {DATEOFBIRTH: string, REGNO: string}
  ...
  let input = $in
  let auth = (merge-auth [(build-auth ($token_apikey | default ($env | get -o GREATER_CHENNAI_CORPORATION_TAMIL_NADU_APIKEY_TOKEN | default "")) "x-apisetu-apikey") (build-auth ($token_clientid | default ($env | get -o GREATER_CHENNAI_CORPORATION_TAMIL_NADU_CLIENTID_TOKEN | default "")) "x-apisetu-clientid")])
...
# operationId: dtcer
# --certificateParameters shape: {DATEOFDEATH: string, REGNO: string}
# --consentArtifact shape: {consent: record, signature: record}
export def "dtcer-certificate create" [
  ...
  --full(-F) # Return full response record {status, headers, body} while still raising on 4xx/5xx
  --dry-run(-n) # Return the request that would be sent without executing it
  --certificate-parameters: record # shape: {DATEOFDEATH: string, REGNO: string}
  --consent-artifact: any # shape: {consent: record, signature: record}
  format: string@format-completer # The format of the certificate in response.
  txn_id: string # A unique transaction id for this request in UUID format. It is used for tracking the request. (format: uuid, e.g. f7f1469c-29b0-4325-9dfc-c567200a70f7)
]: any -> any {
