...
def build-auth [token?: string, auth_scheme?: string]: nothing -> record {
  let token_val = if ($token | is-not-empty) { $token } else { $env | get -o RECOMMENDATION_API_TOKEN | default "" }
...
export def "find find-listing-recommendations" [
  ...
  let full_url = (build-url $base "/find" $qp $auth.query)
  ...
  let req = {
    method: "post"
    url: $full_url
    query: ({"filter": $filter, "limit": $limit, "offset": $offset} | compact)
